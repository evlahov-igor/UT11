...
Процедура СоглашениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			ПараметрыОткрытия.Вставить("КомиссионныеПродажи25", Объект.КомиссионныеПродажи25);
		КонецЕсли;
...
// Параметры:
//  МассивНайденныхДокументов - Массив
...
// СозданиеПередачаТоваровХранителюПоЗаказу
			...
			ИначеЕсли НЕ Отказ
				...
				Если Товары.Количество() > 0 Тогда
					СоздатьПередачуТоваровПоЗаказу(Товары, МассивНайденныхДокументов, ОснованиеОбъект, Отказ);
				КонецЕсли;
...
// СозданиеРасходногоОрдераНаТовары
			...
			Если Не Отказ
				...
				Если Товары.Количество() > 0 Тогда
					СоздатьРасходныеОрдера(Товары, МассивНайденныхДокументов, ОснованиеОбъект, БезПроведения, Отказ);
				КонецЕсли;
...
// ЗавершениеСозданияИзмененияДокументов
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка завершения создания документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли; 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка завершения создания документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ДокументыСформированы Тогда
		...
		Для Каждого Документ Из Объект.Документы Цикл
			...
			Если ТипДокументСсылка = Тип("ДокументСсылка.РеализацияТоваровУслуг")
				...
				Оповещение = Новый ОписаниеОповещения("ПечатьЧека_Завершение", ЭтотОбъект, Документ);
...
Процедура УправлениеДоверенностями()
	...
	Иначе
		...
		Если СтрокаДоверенность <> Неопределено
			...
			ДокументОбъект = СтрокаДоверенность.Документ.ПолучитьОбъект();
			ДокументОбъект.УстановитьПометкуУдаления(Истина);
...
&НаСервере
Процедура СоздатьРеализациюПоЗаказу(Товары, МассивНайденныхДокументов, ОснованиеОбъект, БезСменыСтатусаЗаказа, БезПроведения, Отказ)
...
&НаСервере
Процедура СоздатьПередачуТоваровПоЗаказу(Товары, МассивНайденныхДокументов, ОснованиеОбъект, Отказ)
...
&НаСервере
Процедура СоздатьРасходныеОрдера(Товары, МассивНайденныхДокументов, ОснованиеОбъект, БезПроведения, Отказ)
...
Процедура АдресДоставкиПриИзменении(Элемент)
	...
	ИменаРеквизитов = ИменаРеквизитовАдресовДоставки(Элемент); 
	
	ДоставкаТоваровКлиент.ПриИзмененииПредставленияАдреса(
		Элемент,
		Объект[ИменаРеквизитов.АдресДоставки],
		Объект[ИменаРеквизитов.ЗначенияПолей]);
...
&НаКлиентеНаСервереБезКонтекста
Функция ИменаРеквизитовАдресовДоставки(Элемент)
	
	СтруктураВозврата = Новый Структура("АдресДоставки, ЗначенияПолей");
	Если СтрНайти(Элемент.Имя, "АдресДоставкиПеревозчика") > 0 Тогда
		СтруктураВозврата.АдресДоставки = "АдресДоставкиПеревозчика";
		СтруктураВозврата.ЗначенияПолей = "АдресДоставкиПеревозчикаЗначенияПолей";
	Иначе
		СтруктураВозврата.АдресДоставки = "АдресДоставки";
		СтруктураВозврата.ЗначенияПолей = "АдресДоставкиЗначенияПолей";
	КонецЕсли;
...
Процедура ДобавитьДействияОбеспеченияНаКлиенте(СтруктураДействий, ПоляСтрокой)
	...
	Если Поля.Свойство("ВариантОбеспечения") Тогда
		СтруктураДействий.Вставить("ЗаполнитьОтгрузитьДляВариантаОформленияЗаказПлюсРеализация");
	КонецЕсли;
