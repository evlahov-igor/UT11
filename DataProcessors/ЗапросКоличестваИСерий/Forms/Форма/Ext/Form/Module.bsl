...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ИндексСтрокиДокумента > -1 Тогда
		...
		Если СтрокаДокумента.ПроверятьДоступность
			...
			КоличествоПодобрано = Макс(Мин(СтрокаДокумента.ВНаличии, Объект.КоличествоУпаковок), Объект.КоличествоОформлено);
		КонецЕсли;
...
#Область ОбработчикиСобытийТаблицыФормыСоставНабора
	...
	Если КлючевыеПараметры.ПодборСерий Тогда
		...
		Для Каждого Строка Из Объект.Товары Цикл
			Обработки.ПодборТоваровВДокументПродажи.ЗаполнитьСтатусУказанияСерииВСтроке(
				Объект.ВариантОбеспечения, Строка, Параметры.ПараметрыУказанияСерий);
		КонецЦикла;
		Если Объект.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.Отгрузить
...
Функция ОкруглятьКоличество()
	...
	ЕдиницаМерная = Справочники.УпаковкиЕдиницыИзмерения.ЭтоМернаяЕдиница(ЕдиницаИзмерения, ЕдиницаИзмеренияТипИзмеряемойВеличины);
...
Процедура УправлениеЭлементамиФормыПриСозданииИИзмененииРеквизитов(Элемент)
	...
	Если Элемент = Неопределено Или Элемент = Элементы.СоставНабора  Или Элемент = Элементы.Упаковка Тогда
		...
		ЭтоМернаяЕдиница = Справочники.УпаковкиЕдиницыИзмерения.ЭтоМернаяЕдиница(ЕдиницаИзмерения, "");
	...
	Если Элемент = Элементы.КУказаниюСерий Тогда
		...
		Элементы.ГруппаКоличество.ТолькоПросмотр = Не Объект.Товары.Количество() = 2;
		Элементы.СоставНабора.Доступность = Ложь;
		Элементы.ГруппаДействияИОбособленно.ТолькоПросмотр = Истина;
	...
	Если Элемент = Элементы.КВыборуДействий Тогда
		...
		Элементы.ГруппаКоличество.ТолькоПросмотр = Ложь;
		Элементы.ГруппаДействияИОбособленно.ТолькоПросмотр = Ложь;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'нет контроля остатков'"));
...
&НаСервере
Процедура ДобавитьПодобранныеТовары(ПодобранныеТовары, СтрокиТовары, Упаковка, Цена, КоэффициентУпаковки)
	...
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		...
		Для Каждого СтрокаСерии Из СтрокиСерий Цикл
			...
			СтрокаССерией.КоличествоУпаковок = СтрокаСерии.Подобрано / КоэффициентУпаковки;
		...
		Подобрано = СтрокаТовары.Подобрано - КоличествоСерий / КоэффициентУпаковки;
...
Функция ПодобранныеТовары()
	...
	Если Объект.ВариантКомплектацииНоменклатуры.Пустая() Тогда
		КоэффициентУпаковки = КоэффициентУпаковки(Объект.Упаковка, Объект.Номенклатура);
		ДобавитьПодобранныеТовары(ПодобранныеТовары, Объект.Товары, Объект.Упаковка, Объект.Цена, КоэффициентУпаковки);
	...
	Иначе
		...
		Для Каждого ПозицияНабора Из Объект.СоставНабора Цикл
			...
			КоэффициентУпаковки = КоэффициентУпаковки(ПозицияНабора.Упаковка, ПозицияНабора.Номенклатура);
			...
			ДобавитьПодобранныеТовары(
				ПодобранныеТовары,
				НайденныеСтроки,
				ПозицияНабора.Упаковка,
				ПозицияНабора.Цена,
				КоэффициентУпаковки);
