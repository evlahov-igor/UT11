...
Процедура УстановитьУсловноеОформление()
	...
	НоменклатураСервер.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(
		ЭтаФорма, "ДеревоЦен" + "Характеристика", "ДеревоЦен.ХарактеристикиИспользуются");
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	Для Каждого СтрокаТЧ Из ВыбранныеЦены Цикл
		...
		ОбластьВидЦены.Параметры.МинПартия = НСтр("ru = 'Мин. партия '") + СтрокаТЧ.Ссылка;
		
		ТабличныйДокумент.Присоединить(ОбластьВидЦены);
		
		Если ИспользоватьУпаковкиНоменклатуры Тогда
			ОбластьУпаковкаЗаказа = Макет.ПолучитьОбласть("УпаковкаЗаказа");
			ОбластьУпаковкаЗаказа.Параметры.УпаковкаЗаказа = НСтр("ru = 'Упаковка заказа '") + СтрокаТЧ.Ссылка;
...
Функция ОбновитьИнформационнуюСтроку()
	...
	Если ДанныеДляИнформационнойСтроки.НеРаспознанныхСтрок > 0 или ДанныеДляИнформационнойСтроки.ДублирующихСтрок > 0 Тогда
	...
	Для Каждого ТекСтрока Из ДеревоЦен Цикл
		...
		Если Не РегистрироватьТолькоРаспознаннуюНоменклатуру
			...
			Если ЗначениеЗаполнено(ТекСтрока.Номенклатура) Тогда
				...
				ТекСтрока.НоменклатураНаименование = ТекСтрока.Номенклатура.Наименование;
		...
		Если ИспользоватьУпаковкиНоменклатуры Тогда
			Для Каждого СтрокаТЧ Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(ЭтаФорма) Цикл
				...
				Если НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекСтрока.Номенклатура, "ИспользоватьУпаковки") Тогда
					Продолжить;
				КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	КэшДанных = Неопределено;
	ТаблицаТовары = УстановкаЦенСервер.ТаблицаТовары(ЭтаФорма, КэшДанных);
