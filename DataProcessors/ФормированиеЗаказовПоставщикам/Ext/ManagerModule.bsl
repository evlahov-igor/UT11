...
Функция ТаблицаОстатки()
	...
	Текст =
		...
		|	СУММА(ОстаткиНаСкладе.Запас)    КАК Остаток,
		|	СУММА(ОстаткиНаСкладе.Свободно) КАК СвободныйОстаток
		|ИЗ
		|	ВтТовары КАК Товары
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ОстаткиНаСкладе
		|		ПО ОстаткиНаСкладе.Номенклатура   = Товары.Номенклатура
		|		 И ОстаткиНаСкладе.Характеристика = Товары.Характеристика
		|		 И ОстаткиНаСкладе.Склад          = Товары.Склад
		...
		|	ОстаткиНаСкладе.Характеристика,
		|	ОстаткиНаСкладе.Склад
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура, Характеристика, Склад";
...
Функция ТаблицаЗаказыКПоступлению()
	...
	Текст =
		...
		|	СУММА(ИнформацияОДоступности.Запас)    КАК ЗаказыКПоступлению,
		|	СУММА(ИнформацияОДоступности.Свободно) КАК ЗаказыКПоступлениюСвободные
		|ИЗ
		|	ВтТовары КАК Товары
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ИнформацияОДоступности
		|		ПО ИнформацияОДоступности.Номенклатура   = Товары.Номенклатура
		|		 И ИнформацияОДоступности.Характеристика = Товары.Характеристика
		|		 И ИнформацияОДоступности.Склад          = Товары.Склад
		...
		|ГДЕ
		|	НЕ ИнформацияОДоступности.Номенклатура ЕСТЬ NULL
		|СГРУППИРОВАТЬ ПО
...
Функция ВременнаяТаблицаЗаказы()
	...
	Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ // Необеспеченные строки заказов на отгрузку
		...
		|	Потребности.НеОбеспечено                               КАК Требуется
		|ПОМЕСТИТЬ ВтЗаказы
		|	ИЗ
		...
		|			И Потребности.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.Обеспечить)
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура";
