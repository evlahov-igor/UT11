...
Процедура ТаблицаСпособовОбеспечения(Параметры, АдресРезультата) Экспорт
	...
	// Ниже ТекущаяДатаСеанса() заменен на переменную ТекущаяДатаСеанса  
	ТекущаяДатаСеанса = Неопределено;
	Если Не Параметры.Свойство("Период", ТекущаяДатаСеанса) Тогда 
	...
	Позиция = СтрНайти(Запрос.Текст, "УНИЧТОЖИТЬ ВтСпособыОбеспечения");
...
Функция МинимальныйИМаксимальныйЗапасы(Товар) Экспорт
	...
	Если Товар.МетодОбеспечения = МетодПоСтатистике Или Товар.МетодОбеспечения = МетодПоНорме Тогда
		...
		// << 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148  
		ИспользуетсяСпециальныйАлгоритмРасчета = ЗначениеЗаполнено(Товар.КР_ЭтапИзмененияАссортимента);
		ИспользуетсяРежимЗаморозки = (Товар.КР_ЭтапИзмененияАссортимента = Перечисления.ЭтапыИзмененияАссортимента.КР_Заморозка);
		// >> 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		...
		// << 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148  
		Если Товар.КР_НеУмножатьПотреблениеНаПериод Тогда   
			МинимальныйЗапас  = 0;  
			МаксимальныйЗапас = Товар.СреднедневноеПотребление; 
		Иначе
			// >> 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
			...
			Если Товар.ТипЕдиницыИзмерения = Перечисления.ТипыИзмеряемыхВеличин.КоличествоШтук Тогда
				...
				// << 21.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2156 
				Если Не ИспользуетсяСпециальныйАлгоритмРасчета Тогда 
					// >> 21.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2156
				...
				Иначе    
					...
					Если Окр(МинимальныйЗапас) <> МинимальныйЗапас Тогда
						МинимальныйЗапас = Окр(МинимальныйЗапас);
					КонецЕсли;                                             
					
					Если Окр(МаксимальныйЗапас) <> МаксимальныйЗапас Тогда
						МаксимальныйЗапас = Окр(МаксимальныйЗапас);  
						// A2105505-2418 минимальное значение = 1
		...
		// << 13.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		Если ИспользуетсяСпециальныйАлгоритмРасчета
			И Не Товар.КР_НеОграничиватьМаксимальнымЗапасом Тогда 
			МаксимальныйЗапас = Мин(МаксимальныйЗапас, Товар.МаксимальныйЗапас);
		КонецЕсли;	
		// >> 13.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
...
//
Функция КоличествоКЗаказуДляПоддержанияЗапаса(Товар
	// << 21.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720  
	, Период = Неопределено
	// >> 21.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720
	) Экспорт    
...
//
Функция КоличествоКЗаказуДляОбеспеченияЗаказов(Товар
	// << 21.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720  
	, Период = Неопределено
	// >> 21.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720
	) Экспорт
...
Функция СформироватьТекстЗапросаАктуализацииСпособовОбеспечения() Экспорт
	...
	// << 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-2004
...
Процедура ТаблицаЗапасы(Параметры, АдресРезультата) Экспорт
	...
	Текст =
		...
		//|		КОГДА ТоварныеОграничения.КР_ЭтапИзмененияАссортимента 
		//|				= ЗНАЧЕНИЕ(Перечисление.ЭтапыИзмененияАссортимента.КР_Заморозка)
		//|			ТОГДА ВЫБОР 
		//|				КОГДА НЕ Т.Склад.КР_РазрешитьУвеличиватьЗамороженнуюМатрицу  
		//|					ИЛИ ЕСТЬNULL(ТоварныеОграничения.НормаПотребления / ИнтервалМеждуПоставками.КоличествоДней, 0) > 
		//|						ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		//|							* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0)
		//|					ТОГДА ЕСТЬNULL(ТоварныеОграничения.НормаПотребления, 0)
		//|				КОГДА ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		//|					* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0) >
		//|						ЕСТЬNULL(ТоварныеОграничения.МаксимальноеКоличествоЗапаса / ИнтервалМеждуПоставками.КоличествоДней, 0)
		//|					ТОГДА ЕСТЬNULL(ТоварныеОграничения.МаксимальноеКоличествоЗапаса, 0)
		//|				ИНАЧЕ ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		//|					* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0)
		//|			КОНЕЦ
		// >> 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		// << 31.05.2024 Вдовенко С.А., КРОК, A2105505-2665
		|		КОГДА ТоварныеОграничения.КР_ЭтапИзмененияАссортимента 
		|				= ЗНАЧЕНИЕ(Перечисление.ЭтапыИзмененияАссортимента.КР_Заморозка)
		|			ТОГДА ВЫБОР 
		|				КОГДА НЕ Т.Склад.КР_РазрешитьУвеличиватьЗамороженнуюМатрицу
		|					ТОГДА ЕСТЬNULL(ТоварныеОграничения.НормаПотребления, 0)
		|				КОГДА ТоварныеОграничения.НормаПотребления ЕСТЬ НЕ NULL
		|					И ЕСТЬNULL(ТоварныеОграничения.НормаПотребления, 0) < &ПорогНормыПотребленияДляСравенияСоСтатистикой
		|					ТОГДА ЕСТЬNULL(ТоварныеОграничения.НормаПотребления, 0)
		|				КОГДА ЕСТЬNULL(ТоварныеОграничения.НормаПотребления / ИнтервалМеждуПоставками.КоличествоДней, 0) > 
		|						ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		|							* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0)
		|					ТОГДА ЕСТЬNULL(ТоварныеОграничения.НормаПотребления, 0)
		|				КОГДА ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		...
		// << 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		|	ТоварныеОграничения.КР_ЭтапИзмененияАссортимента 
		|				= ЗНАЧЕНИЕ(Перечисление.ЭтапыИзмененияАссортимента.КР_Заморозка)
		|		И (НЕ Т.Склад.КР_РазрешитьУвеличиватьЗамороженнуюМатрицу 
		|		ИЛИ ЕСТЬNULL(ТоварныеОграничения.НормаПотребления / ИнтервалМеждуПоставками.КоличествоДней, 0) > 
		|		ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		|		* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0)) КАК КР_НеОграничиватьМаксимальнымЗапасом,
		|	ТоварныеОграничения.КР_ЭтапИзмененияАссортимента 
		|				= ЗНАЧЕНИЕ(Перечисление.ЭтапыИзмененияАссортимента.КР_Заморозка)
		|		И (НЕ Т.Склад.КР_РазрешитьУвеличиватьЗамороженнуюМатрицу 
		|		ИЛИ ЕСТЬNULL(ТоварныеОграничения.НормаПотребления / ИнтервалМеждуПоставками.КоличествоДней, 0) > 
		|		ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		|		* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0)
		|		ИЛИ ЕСТЬNULL(КР_КоэффициентыКорректировки.КоэффициентКорректировки, 1)
		|		* ЕСТЬNULL(ТаблицаСтатистики.СреднедневноеПотребление, 0) >
		|		ЕСТЬNULL(ТоварныеОграничения.МаксимальноеКоличествоЗапаса / ИнтервалМеждуПоставками.КоличествоДней, 0)
		|		) КАК КР_НеУмножатьПотреблениеНаПериод,
		// >> 19.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		|
		// Диагностика проблем
		...
		// << 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		|	,
		|	ЕСТЬNULL(ТоварныеОграничения.КР_ЭтапИзмененияАссортимента,
		|		ЗНАЧЕНИЕ(Перечисление.ЭтапыИзмененияАссортимента.ПустаяСсылка)) КАК КР_ЭтапИзмененияАссортимента
		// >> 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		|ИЗ
		|	ВтТовары КАК Т
		|
		...
		// << 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КР_КоэффициентыКорректировкиПодсортировкиСкладов КАК КР_КоэффициентыКорректировки
		|		ПО Т.Склад = КР_КоэффициентыКорректировки.Склад 
		// >> 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
		|
	...
	// << 11.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720
	УчитыватьПрогнозПотребности = Неопределено;
	Если Не Параметры.Свойство("УчитыватьПрогнозПотребности", УчитыватьПрогнозПотребности) Тогда 
		УчитыватьПрогнозПотребности = Ложь; 
	КонецЕсли;	 
	Запрос.УстановитьПараметр("УчитыватьПрогнозПотребности", УчитыватьПрогнозПотребности); 
	// >> 11.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720 
	
	// << 12.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2604
	УчитыватьСвободныеОстатки = Неопределено;
	Если Не Параметры.Свойство("УчитыватьСвободныеОстатки", УчитыватьСвободныеОстатки) Тогда  
		УчитыватьСвободныеОстатки = Истина;
	КонецЕсли;	
	Запрос.УстановитьПараметр("УчитыватьСвободныеОстатки", УчитыватьСвободныеОстатки);
	// >> 12.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2604
	// << 31.05.2024 Вдовенко С.А., КРОК, A2105505-2664
	УчитыватьТоварыКОтгрузке = Неопределено;
	Если Не Параметры.Свойство("УчитыватьТоварыКОтгрузке", УчитыватьТоварыКОтгрузке) Тогда
		УчитыватьТоварыКОтгрузке = Ложь; 
	КонецЕсли;
	Запрос.УстановитьПараметр("УчитыватьТоварыКОтгрузке", УчитыватьТоварыКОтгрузке);
	// >> 31.05.2024 Вдовенко С.А., КРОК, A2105505-2664
	
	// << 31.05.2024 Вдовенко С.А., КРОК, A2105505-2665
	ПорогНормыПотребленияДляСравенияСоСтатистикой = 3;
	Запрос.УстановитьПараметр("ПорогНормыПотребленияДляСравенияСоСтатистикой", ПорогНормыПотребленияДляСравенияСоСтатистикой);
	// >> 31.05.2024 Вдовенко С.А., КРОК, A2105505-2665
...
Процедура ТаблицаПотребностейПоЗаказамИОстатков(Параметры, АдресРезультата) Экспорт
	...
	Текст =
		...
		|	Т.Характеристика                                 КАК Характеристика,
		|	Т.Склад                                          КАК Склад,
		|	ЕСТЬNULL(ВЫБОР КОГДА ТопологияСкладов.Центр <> Т.Склад ТОГДА
		|					ТопологияСкладов.Центр
		|				ИНАЧЕ
		|					NULL
		|			КОНЕЦ,
	...
	Если Параметры.ТипОтбора <> "ОтборПоЗначениям" Тогда
		...
		// << 14.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1720
		УчитыватьПрогнозПотребности = Неопределено;
		Если Не Параметры.Свойство("УчитыватьПрогнозПотребности", УчитыватьПрогнозПотребности) Тогда 
			УчитыватьПрогнозПотребности = Ложь; 
		КонецЕсли;	 
		...
		// << 11.04.2024 Фомичев А.Е., КРОК, JIRA№A2105505-2651
		УчитыватьСвободныеОстатки = Неопределено;
		Если Не Параметры.Свойство("УчитыватьСвободныеОстатки", УчитыватьСвободныеОстатки) Тогда 
			УчитыватьСвободныеОстатки = Ложь; 
		КонецЕсли;	 
		Запрос.УстановитьПараметр("УчитыватьСвободныеОстатки", УчитыватьСвободныеОстатки); 
		// >> 11.04.2024 Фомичев А.Е., КРОК, JIRA№A2105505-2651
		// << 31.05.2024 Вдовенко С.А., КРОК, A2105505-2664
		...
		Текст =
			...
			|	Работы.ПодразделениеПолучатель,
			|	Работы.Назначение";
		ТекстыЗапросов.Добавить(Текст);
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПеремещениемОбособленныхТоваров") Тогда
			...
			УстановитьПривилегированныйРежим(Истина);
			Запрос.Текст = ВременнаяТаблицаЗаказыНаПеремещениеВсехСкладов();
			Запрос.Выполнить();
			...
			Запрос.Текст =
				...
				|	ИЗ
				|		РегистрСведений.РаспределениеЗапасов КАК Запасы
				|	ГДЕ
				|		Запасы.Состояние В(
				|				ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе),
				|				ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление),
				...
				|	ОБЪЕДИНИТЬ ВСЕ
				|	
				// Вычет ожидаемых поступлений по неотгруженным заказам на перемещение на всех складах.
				|	ВЫБРАТЬ
				...
				|	ИЗ
				|		ЗаказыНаПеремещениеВсехСкладов КАК Таблица
				|	
				|	ОБЪЕДИНИТЬ ВСЕ
				|	
				// Потребности не попавщие в обработку.
				|	ВЫБРАТЬ
				...
				|						И Фильтр.Характеристика = Потребности.Характеристика
				|						И Фильтр.Склад = Потребности.Склад
				|						И Фильтр.Назначение = Потребности.Назначение)) КАК Набор
				|СГРУППИРОВАТЬ ПО
				|	Набор.Номенклатура,
...
Процедура СформироватьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.Свойство("ПредварительныйРасчет", ПредварительныйРасчет) 
		И ПредварительныйРасчет Тогда 

		Результат = Новый Структура("Таблицы,СписокСообщений", Таблицы, Новый Массив);
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		// Задача #4655
		ДанныеЗаполнения.Вставить("злРегламентнаяПодсортировка", Истина);
...
Функция ВременнаяТаблицаСпособовОбеспечения(ТоварыПоддерживаемогоЗапаса)
	...
	ТекстЗапроса =
		...
		|	СУММА(Т.НеОбеспечено + Т.Зарезервировано + Т.Обеспечено) КАК Количество
		...
		Топология = РегистрыСведений.СхемыОбеспечения.ВременнаяТаблицаТопологияСкладов("РазличнаяНоменклатура");
		Топология = СтрЗаменить(Топология, "ВтФорматыСкладов", "ВтФорматыСкладов1");
		Тексты.Добавить(Топология);
		Тексты.Добавить(ОбщегоНазначения.РазделительПакетаЗапросов());
		Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	Потребности.Номенклатура КАК Номенклатура,
...
Функция ВременнаяТаблицаКОбеспечениюВсехСкладов(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =
		...
		|	ТаблицаОтбора.Номенклатура КАК Номенклатура,
		|	ТаблицаОтбора.Характеристика КАК Характеристика,
		|	&ПодстановкаНазначенияСоединения КАК Назначение,
		|	ВЫБОР КОГДА МАКСИМУМ(ЕСТЬNULL(Остатки.Свободно, 0)) < СУММА(ОжидаемаяОтгрузка.ОтложитьРезервирование + ОжидаемаяОтгрузка.КОбеспечениюБезРезерва) ТОГДА
		|				МАКСИМУМ(ЕСТЬNULL(Остатки.Свободно, 0))
		|			ИНАЧЕ
		|				СУММА(ОжидаемаяОтгрузка.ОтложитьРезервирование + ОжидаемаяОтгрузка.КОбеспечениюБезРезерва)
		|		КОНЕЦ КАК ОбеспеченоСоСклада,
		|	СУММА(ОжидаемаяОтгрузка.ОтложитьРезервирование + ОжидаемаяОтгрузка.КОбеспечениюБезРезерва) КАК КОбеспечениюВсего
		...
		|		ПО ОжидаемаяОтгрузка.Номенклатура = ТаблицаОтбора.Номенклатура
		|		 И ОжидаемаяОтгрузка.Характеристика = ТаблицаОтбора.Характеристика
		|		 И ОжидаемаяОтгрузка.Склад = ТаблицаОтбора.Склад
		|		 И ОжидаемаяОтгрузка.Назначение = &ПодстановкаНазначенияСоединения
		...
		|ГДЕ
		|	НЕ ОжидаемаяОтгрузка.Номенклатура ЕСТЬ NULL
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаОтбора.Склад,
		...
		|	ТаблицаОтбора.Характеристика КАК Характеристика,
		|	&ПодстановкаНазначенияСоединения КАК Назначение,
		|	ВЫБОР КОГДА СУММА(ЕСТЬNULL(ОжидаемыеПоступления.Свободно, 0)) < МАКСИМУМ(КОбеспечениюВсехСкладов.КОбеспечениюВсего - КОбеспечениюВсехСкладов.ОбеспеченоСоСклада) ТОГДА
		|				СУММА(ЕСТЬNULL(ОжидаемыеПоступления.Свободно, 0))
		|			ИНАЧЕ
		|				МАКСИМУМ(КОбеспечениюВсехСкладов.КОбеспечениюВсего - КОбеспечениюВсехСкладов.ОбеспеченоСоСклада)
		|		КОНЕЦ КАК ОбеспеченоИзЗаказов,
		|	МАКСИМУМ(КОбеспечениюВсехСкладов.ОбеспеченоСоСклада) КАК ОбеспеченоСоСклада
		|ПОМЕСТИТЬ ВтКОбеспечениюВсехСкладов
		|ИЗ
		|	РазличныеТоварыИСклады КАК ТаблицаОтбора
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтКОбеспечениюВсехСкладовВременная КАК КОбеспечениюВсехСкладов
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ОжидаемыеПоступления
		|		ПО ОжидаемыеПоступления.Номенклатура = ТаблицаОтбора.Номенклатура
		|		 И ОжидаемыеПоступления.Характеристика = ТаблицаОтбора.Характеристика
		|		 И ОжидаемыеПоступления.Склад = ТаблицаОтбора.Склад
		|		 И ОжидаемыеПоступления.Назначение = &ПодстановкаНазначенияСоединения
		|		 И ОжидаемыеПоступления.Состояние В(
		|			ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление),
		|			ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеподтвержденное))
		|		 И ОжидаемыеПоступления.Свободно > 0
		|ГДЕ
		|	НЕ КОбеспечениюВсехСкладов.Номенклатура ЕСТЬ NULL
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаОтбора.Склад,
		|	ТаблицаОтбора.Номенклатура,
		|	ТаблицаОтбора.Характеристика,
...
Функция ВременнаяТаблицаСвободныеОстатки(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =
		"ВЫБРАТЬ
		// << 04.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2418  
		// Запас, Совбодно и Резерв - выбран такой способ расчета 
		//	так как это соответствует требованиям (конечный итог)
		//	Имена переменных не соответстуют их реальному функционалу, 
		//	по крайней мере в том блоке, который дорабатывается  
		|	Т.Номенклатура КАК Номенклатура,
		|	Т.Характеристика КАК Характеристика,
		|	Т.Склад КАК Склад,
		|	Т.Назначение КАК Назначение,
		|	Т.ВНаличииОстаток КАК Запас,
		|	Т.ВНаличииОстаток КАК Свободно,
		|	0 КАК Резерв
		|ПОМЕСТИТЬ ВтОстаткиДетальныеЗаписи
		|ИЗ
		|	РегистрНакопления.ТоварыНаСкладах.Остатки(
		|			КОНЕЦПЕРИОДА(&НачалоПериода, ДЕНЬ),
		|			(Номенклатура, Характеристика, Склад) В
		|				(ВЫБРАТЬ
		|					РазличныеТоварыИСклады.Номенклатура,
		|					РазличныеТоварыИСклады.Характеристика,
		|					РазличныеТоварыИСклады.Склад
		|				ИЗ
		|					РазличныеТоварыИСклады)) КАК Т  
		// << 12.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2604
		|ГДЕ
		|	&УчитыватьСвободныеОстатки
		// >> 12.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2604
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	Т.Склад КАК Склад,
		|	Т.Назначение КАК Назначение,
		|	СУММА(Т.Запас) КАК Запас,
		|	СУММА(Т.Свободно) КАК Свободно,
		|	СУММА(Т.Резерв) КАК Резерв
		|ПОМЕСТИТЬ ВтОстатки
		|ИЗ
		|	ВтОстаткиДетальныеЗаписи КАК Т
		|
		|СГРУППИРОВАТЬ ПО
		|	Т.Склад,
		|	Т.Номенклатура,
		...
		|ИМЕЮЩИЕ
		|	СУММА(Т.Запас) > 0
		|	И СУММА(Т.Свободно) >= 0
		|	И СУММА(Т.Резерв) >= 0
		|;
		|
		|///////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтОстаткиДетальныеЗаписи
		|;
		|
		|///////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ВременнаяТаблицаЗаказыКПоступлениюИОтгрузке(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =                                   
		...
		|	Т.Назначение КАК Назначение,
		|	Т.Склад КАК Склад,
		// << 22.04.2024 Фомичев А.Е., КРОК, JIRA№A2105505-2652
		//|	Т.КОформлениюПоступленийПоРаспоряжениюОстаток КАК Запас,
		//|	Т.КОформлениюПоступленийПоРаспоряжениюОстаток КАК Свободно
		|Т.КОформлениюПоступленийПоРаспоряжениюОстаток + ВЫБОР
		|	КОГДА Т.КОформлениюПоступленийПоОрдерамОстаток > 0
		|		ТОГДА 0
		|	ИНАЧЕ -Т.КОформлениюПоступленийПоОрдерамОстаток
		|КОНЕЦ КАК Запас,
		|Т.КОформлениюПоступленийПоРаспоряжениюОстаток + ВЫБОР
		|	КОГДА Т.КОформлениюПоступленийПоОрдерамОстаток > 0
		|		ТОГДА 0
		...
		|	СУММА(РаспределениеЗапасов.Свободно) - ЕСТЬNULL(МАКСИМУМ(КОбеспечениюВсехСкладов.ОбеспеченоИзЗаказов), 0) КАК Свободно
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтТоварыКПоступлению КАК РаспределениеЗапасов
		// >> 04.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2418
		...
		|	Т.Назначение 		КАК Назначение,
		|	СУММА(Т.Количество) КАК Запас,
		|	СУММА(Т.Количество) КАК Свободно
		|ИЗ
		|	РазличныеТоварыИСклады КАК ТаблицаОтбора
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КР_ПрогнозПотребностиВПодсортировкеМагазинов КАК Т
		|		ПО Т.Номенклатура = ТаблицаОтбора.Номенклатура
		|		 И Т.Характеристика = ТаблицаОтбора.Характеристика
		|		 И Т.СкладПолучатель = ТаблицаОтбора.Склад
		|		 И Т.Назначение = &ПодстановкаНазначенияСоединения
		|		
		|ГДЕ
		|	&УчитыватьПрогнозПотребности
		|	И НЕ Т.Номенклатура ЕСТЬ NULL
		|СГРУППИРОВАТЬ ПО
		|	Т.Номенклатура,
		|	Т.Характеристика,
		|	Т.СкладПолучатель,
		...
		|
		|///////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтТоварыКПоступлению
		// >> 04.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2418
		|;
		|
		|///////////////////////////////////////////////////////////////////
...
Функция ВременнаяТаблицаЗапретЗакупкиПоАссортименту(ИспользоватьАссортимент)
	...
	Иначе
		...
		ТекстЗапроса =
			...
			|			= ВЫБОР КОГДА ИспользоватьФорматыМагазинов.Значение   
			// << 20.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2400
			|				И ТИПЗНАЧЕНИЯ(Ассортимент.ОбъектПланирования) = ТИП(Справочник.ФорматыМагазинов)
			// >> 20.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2400
			|				ТОГДА
...
Функция ВременнаяТаблицаПотребностиВыбранныхСкладов()
	...
	ТекстЗапроса =
		...
		|	СУММА(ВЫБОР КОГДА Товары.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОбеспеченНаСкладе) ТОГДА
		|					Товары.Распределено
		|				ИНАЧЕ
		|					0
...
Функция ВременнаяТаблицаЗаказыНаПеремещениеВнутриСети(ВТаблицеОтбораЕстьНазначения)
	...
	Текст =
		...
		|					ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОжидаемоеПоступление),
		|					ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ПоступлениеНеподтвержденное))
		|			 И ТИПЗНАЧЕНИЯ(Сведения.ЗаказНаПоступление) В(
		|					ТИП(Документ.ЗаказНаПеремещение)
		|				)
		|			
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПеремещение КАК ЗаказыНаПеремещение
		...
		|					Фильтр.Номенклатура = Сведения.Номенклатура
		|						И Фильтр.Характеристика = Сведения.Характеристика
		|						И Фильтр.Склад = Сведения.Склад)) КАК Набор
		|СГРУППИРОВАТЬ ПО
		|	Набор.Склад,
		|	Набор.Номенклатура,
		|	Набор.Характеристика,
		|	Набор.Назначение,
		|	Набор.Заказ) КАК Таблица
		|СГРУППИРОВАТЬ ПО
		|	Таблица.Склад,
...
Функция ВременнаяТаблицаЗаказыНаПеремещениеВсехСкладов()
	...
	Текст =
		...
		|				- Таблица.Запас
		|		КОНЕЦ) КАК ОтгрузкаМинусЗапас,
		|	СУММА(ВЫБОР КОГДА Таблица.Распределено + Таблица.Резерв + Таблица.НеОбеспечено < Таблица.Запас ТОГДА
		|				- Таблица.Распределено - Таблица.НеОбеспечено
		|			ИНАЧЕ
		|				- Таблица.Запас
		|		КОНЕЦ) КАК РезервМинусЗапас
		|ПОМЕСТИТЬ ЗаказыНаПеремещениеВсехСкладов
		|ИЗ(
		|ВЫБРАТЬ
		|	Набор.Склад КАК Склад,
		|	Набор.Номенклатура КАК Номенклатура,
