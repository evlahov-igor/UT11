...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления() Тогда
		// << 11.08.2024 Петухов А.В., Фактор, #4342
	...
	Если ЗначениеЗаполнено(КаталогСкрипта) Тогда
		злОбновлениеКонфигурации.ОбработкаЛогаОбновления(КаталогСкрипта);
	КонецЕсли;
	// >> 24.07.2024 Петухов А.В., Фактор, #4316
	...
	// << 10.08.2024 Петухов А.В., Фактор, #4337
	Если Не Пользователи.РолиДоступны("ПолныеПрава") Тогда
		Элементы.КолонкаПредупреждения.Видимость = Ложь;
		Элементы.СтраницаПереключательОбновленияФайл.Видимость = Ложь;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	//Петухов А.В.  24.10.2023  #3269   {
	злОбновлениеКонфигурацииКлиент.ЗапуститьОбработчикОжиданияОбновленияКонфигурации();
	//}	
...
Процедура ПерейтиСоСтраницыФайлаОбновления()
	...
	// Стало:
	ПроверитьПрименимостьФайловОбновления("Продолжить", Неопределено);
	// ..#4337
...
Процедура ПерейтиКВыборуРежимаОбновления(ЭтоПереходДалее = Ложь)
	...
	// << 11.08.2024 Петухов А.В., Фактор, #4342
	Если ПараметрыАдминистрирования = Неопределено И ОбщегоНазначенияКлиент.ИнформационнаяБазаФайловая() Тогда
		злЗаполнитьПараметрыАдминистрирования(ПараметрыАдминистрирования);
...
// << 11.08.2024 Петухов А.В., Фактор, #4342
Процедура злЗаполнитьПараметрыАдминистрирования(ПараметрыАдминистрирования)
 
	//Взял из СтандартныеПодсистемыСервер.ПараметрыАдминистрирования();
	УстановитьПривилегированныйРежим(Истина);
	...
	ПараметрыАдминистрированияПоУмолчанию = ПараметрыАдминистрированияКластера;
	Для Каждого Элемент Из ПараметрыАдминистрированияИБ Цикл
		ПараметрыАдминистрированияПоУмолчанию.Вставить(Элемент.Ключ, Элемент.Значение);
	КонецЦикла;
	ПараметрыАдминистрированияПоУмолчанию.Вставить("ПринятоРешениеПоОткрытиюВнешнихОтчетовИОбработок", Ложь);
	...
	ИмяПользователя = "zlUpdater";
	ПарольПользователя = XMLСтрока(Новый УникальныйИдентификатор);
	...
	РольАдминистрирование = Метаданные.Роли.Найти("АдминистраторСистемы");
	ПользовательИБ.Роли.Добавить(РольАдминистрирование);
	РольПП = Метаданные.Роли.Найти("ПолныеПрава");
	ПользовательИБ.Роли.Добавить(РольПП);
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		СпрПользователь.Наименование = ПользовательИБ.Имя;
		СпрПользователь.ОбменДанными.Загрузка = Истина;
		СпрПользователь.ИдентификаторПользователяСервиса = ПользовательИБ.УникальныйИдентификатор;
		СпрПользователь.ИдентификаторПользователяИБ = ПользовательИБ.УникальныйИдентификатор;
		СпрПользователь.Недействителен = Ложь;
		СпрПользователь.Служебный = Истина;
		СпрПользователь.Подготовлен = Истина;
		СпрПользователь.Записать();
 	КонецЕсли;

	ПараметрыАдминистрированияИБ.ИмяАдминистратораИнформационнойБазы = ИмяПользователя;
	ПараметрыАдминистрированияИБ.ПарольАдминистратораИнформационнойБазы = ПарольПользователя;
