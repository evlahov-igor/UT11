	...
	Если СтруктураДанных.Свойство("Товары") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Если СтруктураДанных.СтруктураДействий.Свойство("ЗаполнитьСклад", ПараметрДействия) Тогда
				УстановитьЭлементОтбораСКД("Склад", ПараметрДействия, ЗначениеЗаполнено(ПараметрДействия));
				Объект.Склад = ПараметрДействия;
				// << 14.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
				КР_УстановитьОтборПомещение();
				// >> 14.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
			КонецЕсли;
			...
			// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
			Если СтруктураДанных.СтруктураДействий.Свойство("КР_НеПечататьСтарыеЦены", ПараметрДействия) Тогда
				ЭтотОбъект["КР_НеПечататьСтарыеЦены"] = ПараметрДействия;
			КонецЕсли;
			// >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
			// << 31.05.2023 Федотов А.М., КРОК, Jira№A2105505-1706
			Если СтруктураДанных.СтруктураДействий.Свойство("КР_ТолькоСОстаткамиНаСкладе") Тогда
				ЭлементОтбораТолькоСОстатками = УстановитьЭлементОтбораСКД("ОстатокНаСкладе", 0, Истина);	
				ЭлементОтбораТолькоСОстатками.ВидСравнения = ВидСравненияКомпоновкиДанных.Больше;
			КонецЕсли;
			// >> 31.05.2023 Федотов А.М., КРОК, Jira№A2105505-1706
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	// << 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
	Если Не КР_РасширенноеИспользованиеОбработки() Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	// >> 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	// << 14.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	ОбновитьВидимостьЗависимыхОтШаблонаПолей();
	// >> 14.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
Процедура ТоварыХарактеристикаПриИзменении(Элемент)
	...
	ТекущиеДанные = Новый Структура("Номенклатура, Характеристика, Серия, Упаковка");
	ЗаполнитьЗначенияСвойств(ТекущиеДанные, Элементы.Товары.ТекущиеДанные);
	// << 13.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	//ЗаполнитьЗначенияСвойств(Элементы.Товары.ТекущиеДанные, ПриИзмененииНоменклатуры(ТекущиеДанные));
	ИсключаяСвойства = "ШаблонЦенника";
	ЗаполнитьЗначенияСвойств(Элементы.Товары.ТекущиеДанные, ПриИзмененииНоменклатуры(ТекущиеДанные), , ИсключаяСвойства);
	// >> 13.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
Процедура Печать(Команда)
	...
	Если РезультатПроверки.ПроверкаВыполнена Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(
			"Обработка.ПечатьЭтикетокИЦенников2_5",
			ИменаМакетов,
			ПараметрКоманды,
			ЭтаФорма,
			ПолучитьПараметры());
...
Процедура ПодобратьТовары(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени(
		"Обработка.ПечатьЭтикетокИЦенников2_5.ФормаТовары.Команда.ПодобратьТовары",,Истина);
...
Процедура ВыбратьВариантЗаполненияУпаковок(Команда)
	...
	ОткрытьФорму("Обработка.ПечатьЭтикетокИЦенников2_5.Форма.ВыборВариантаЗаполненияУпаковками",
...
Функция ПолучитьПараметры()
	...
	ПараметрыПечати.Вставить("ПечатьDataMatrix", ЭтотОбъект["злПечатьDataMatrix"]);		// #4100.. Фомин Д.Ю. 24.05.2024.
...
Процедура УстановитьКоличествоПоУмолчаниюВСтрокеТЧНаСервере(СтрокаТЧ)
	...
	ИначеЕсли ЗаполнитьКоличествоЦенниковПоДокументу И ЗаполнитьКоличествоЦенниковПоОстаткамНаСкладе Тогда
		...
		Если СтрокаТЧ.КоличествоВДокументе > 0 Тогда
			КоличествоЦенников = СтрокаТЧ.КоличествоВДокументе;
		ИначеЕсли СтрокаТЧ.ОстатокНаСкладе > 0 Тогда
			КоличествоЦенников = СтрокаТЧ.ОстатокНаСкладе;
		Иначе
			КоличествоЦенников = 0;
		КонецЕсли;
		СтрокаТЧ.КоличествоЦенников = КоличествоЦенников;
		// >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	ИначеЕсли НЕ ЗаполнитьКоличествоЦенниковПоДокументу И ЗаполнитьКоличествоЦенниковПоОстаткамНаСкладе Тогда
		...
		СтрокаТЧ.КоличествоЦенников = ?(СтрокаТЧ.ОстатокНаСкладе > 0, СтрокаТЧ.ОстатокНаСкладе, 0);
		// >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	КонецЕсли;
...
Процедура УстановитьОбязательныеПоляДанных(СтруктураНастроек, ДобавитьШаблоны = Ложь)
	...
	Если ИспользоватьВидЦеныДополнительно Тогда
	// >> 13.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
Процедура ЗаполнитьТаблицуТоваровНаСервере(СохранятьШаблоныИКоличество, ОбновлениеДанныхПоВыбраннымТоварам = Ложь)
	...
	СтруктураНастроек = Обработки.ПечатьЭтикетокИЦенников2_5.СтруктураНастроек();
	// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	...
	Если Объект.ИсходныеДанныеТовары.Количество() > 0 ИЛИ ПечатьИзДокумента ИЛИ ОбновлениеДанныхПоВыбраннымТоварам Тогда
		...
		Иначе
			...
			СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных = "КР_ПоляШаблонаТовары";
			// >> 09.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		КонецЕсли;
	...
	Иначе
		Если КонтролироватьАссортимент Тогда
			// << 19.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1670
			//СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных = "ПоляШаблонаБДТоварыАссортимент";
			СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных = "КР_ПоляШаблонаБДТоварыАссортимент";
			// >> 19.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1670
		Иначе
			// << 19.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1670
			//СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных = "ПоляШаблонаБДТовары";
			СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных = "КР_ПоляШаблонаБДТовары";
			// >> 19.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1670
		КонецЕсли;
	КонецЕсли;
	...
	СтруктураРезультата = Обработки.ПечатьЭтикетокИЦенников2_5.ПодготовитьСтруктуруДанных(СтруктураНастроек, "Товары");
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.Таблица Цикл
		...
		// Расширим условие разработанное по задаче A2105505-1028, исключив вероятность кейса,
		// когда обработка открывается без инициализации "КР_НеПечататьСтарыеЦены"
		//// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		////Если ЗначениеЗаполнено(Объект.ВидЦеныДополнительно) Тогда
		//Если Не ЭтотОбъект["КР_НеПечататьСтарыеЦены"] Тогда
		//// >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		Если Не ЭтотОбъект["КР_НеПечататьСтарыеЦены"] 
			И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТЧ, "ЦенаДополнительно") Тогда
		// >> 07.04.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	// << 09.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	//СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников2_5.ПолучитьМакет("ПоляШаблонаТовары");
	СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников2_5.ПолучитьМакет("КР_ПоляШаблонаТовары");
	// >> 09.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
Функция ПриИзмененииНоменклатуры(ТекущиеДанные)
	...
	ЕстьЦенаДополнительно = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		СтруктураРезультата.Таблица.Колонки,
		"ЦенаДополнительно");
	// << 18.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1670
	//Если ЕстьЦенаДополнительно Тогда
	ЕстьСоотвестсвиеСКД = 
		Не СтруктураРезультата.СоответствиеПолейСКДКолонкамТаблицы.Получить("ЦенаДополнительно") = Неопределено;
	Если ЕстьЦенаДополнительно И ЕстьСоотвестсвиеСКД Тогда
	// >> 18.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1670
		ВозвращаемоеЗначение.Вставить("ЦенаДополнительно",
			Данные[СтруктураРезультата.СоответствиеПолейСКДКолонкамТаблицы["ЦенаДополнительно"]]);
	КонецЕсли;
	// >> 13.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
Функция ДобавитьНоменклатуруПоШтрихкодам(ДанныеШтрихкодов)
	...
	Для Каждого ТекШтрихкод Из ДанныеШтрихкодов Цикл
		...
		Иначе
			...
			// << 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
			НоваяСтрока.Количество     = ДобавляемыеДанные.КоличествоУпаковок;
			// >> 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
...
Процедура ДобавитьТовары(Таблица)
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.Таблица Цикл
		...
		нСтроки = Объект.Товары.НайтиСтроки(ПараметрыОтбора);
		Если нСтроки.Количество() Тогда
			НоваяСтрока = нСтроки[0];
		Иначе
		НоваяСтрока = Объект.Товары.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ);
		КонецЕсли;
		...
		// << 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
		НоваяСтрока.КоличествоЦенников = НоваяСтрока.КоличествоЦенников + СтрокаТЧ[СтруктураРезультата.СоответствиеПолейСКДКолонкамТаблицы["Количество"]];
		НоваяСтрока.КоличествоЭтикеток = НоваяСтрока.КоличествоЭтикеток + СтрокаТЧ[СтруктураРезультата.СоответствиеПолейСКДКолонкамТаблицы["Количество"]];
		// >> 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
	КонецЦикла;
...
Процедура ОбновитьВидимостьЗависимыхОтШаблонаПолей()
	...
	// << 09.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	...
	Элементы.ВидЦеныДополнительно.Видимость 	= Ложь;
	Элементы.ДатаВидЦеныДополнительно.Видимость = Ложь;
	Элементы.ТоварыЦенаДополнительно.Видимость 	= Истина;
	// >> 09.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
&НаСервере
Процедура КР_ДобавитьРеквизиты()
	
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_НеПечататьСтарыеЦены",
		Новый ОписаниеТипов("Булево"), ,
		НСтр("ru = 'Не печатать старые цены'"));
		
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_НеПечататьСтарыеЦены", Элементы.РежимЗаполненияЦенников);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// #4100.. Фомин Д.Ю. 23.05.2024.
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"злПечатьDataMatrix",
		Новый ОписаниеТипов("Булево"), ,
		НСтр("ru = 'Печать DataMatrix'"));

	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"злПечатьDataMatrix", Элементы.РежимЗаполненияЦенников);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	// ..#4100
	
КонецПроцедуры // >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028

// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
&НаСервере
Процедура КР_ПриСозданииНаСервереДополнительно(Отказ, СтандартнаяОбработка)
	
	НовыйЗаголовок = НСтр("ru = 'По остаткам на складе'");
	Элементы.ЗаполнитьКоличествоЦенниковПоОстаткамНаСкладе.Заголовок = НовыйЗаголовок;
	
	НовыйЗаголовок = НСтр("ru = 'По количеству в документе'");
	Элементы.ЗаполнитьКоличествоЦенниковПоДокументу.Заголовок = НовыйЗаголовок;
	
	// << 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
	Если Не ЗначениеЗаполнено(Объект.Склад) Тогда
		Объект.Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Пользователи.ТекущийПользователь(), "Подразделение.КР_Склад");
	КонецЕсли;
	КР_УстановитьВидимостиДоступности();
	// >> 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707    
	
	// << 12.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2571
	Если КР_ДополнительныеНастройкиПовтИсп.БазаЯвляетсяЦентральной() Тогда     
		
		Объект.РежимПолученияЦен = 1;
		Объект.ВидЦены = КР_ДополнительныеНастройкиПовтИсп.Значение("ВидЦеныРозничный", Ложь);
		
		// Процедура ПолеПереключательРежимаПолученияЦенПриИзменении
		ПереключательРежимаПолученияЦенПриИзмененииСервер();
		//
...
// << 14.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
&НаСервере
Процедура КР_УстановитьОтборПомещение()
	...
	Если Выборка.Следующий() Тогда
		...
		УстановитьЭлементОтбораСКД("Помещение", Выборка.Помещение, ЗначениеЗаполнено(Выборка.Помещение));
...
Процедура КР_УстановитьВидимостиДоступности()
	...
	РасширенноеИспользование = КР_РасширенноеИспользованиеОбработки();
	
	Для Каждого Элемент Из Элементы.Товары.ПодчиненныеЭлементы Цикл
		Элемент.ТолькоПросмотр = Не РасширенноеИспользование;
	КонецЦикла;
	
	Элементы.ГруппаШапка.ТолькоПросмотр = Не РасширенноеИспользование;
	Элементы.ГруппаЦены.ТолькоПросмотр = Не РасширенноеИспользование;
	Элементы.ГруппаРежимЗаполнения.ТолькоПросмотр = Не РасширенноеИспользование;
	
	Элементы.ТоварыДобавить.Доступность = РасширенноеИспользование;
	Элементы.ТоварыРазбитьСтроку.Доступность = РасширенноеИспользование;
	Элементы.ГруппаУстановить.Доступность = РасширенноеИспользование;
	Элементы.ДеревоЦенПодобратьТовары.Доступность = РасширенноеИспользование;
	Элементы.ТоварыВыбратьВариантЗаполненияУпаковок.Доступность = РасширенноеИспользование;
	
	Элементы.ТоварыЦенаДополнительно.Видимость = РасширенноеИспользование;
	
КонецПроцедуры // >> 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707

// << 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
&НаСервереБезКонтекста
Функция КР_РасширенноеИспользованиеОбработки()
	Возврат Пользователи.РолиДоступны("КР_РасширенноеИспользованиеОбработкиПечатьЭтикетокИЦенников");
КонецФункции // >> 07.06.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1707
