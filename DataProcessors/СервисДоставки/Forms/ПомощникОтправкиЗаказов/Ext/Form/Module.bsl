...
&НаКлиенте
Процедура ЗаказыСГруппировкамиОтгрузкаНаБлижайшееВремяПриИзменении(Элемент)

	Мультизаказ = Элементы.ЗаказыСГруппировками.ТекущиеДанные;
	
	Если Мультизаказ <> Неопределено Тогда
		ТекущаяДата = НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса());
		Если Мультизаказ.ОтгрузкаНаБлижайшееВремя Тогда
			Мультизаказ.ВремяОтгрузкиК = '00010101';
			Мультизаказ.ДатаОтгрузки = ТекущаяДата;
			Мультизаказ.ЗаказПредставление = ПредставлениеНовогоМультизаказа(Мультизаказ, ТипГрузоперевозки);
		КонецЕсли;
...
Процедура ЗаказыСГруппировкамиПеретаскиваниеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ПараметрыДляДобавления.Вставить("МультизаказИдентификаторСтроки", НовыйРодитель.ПолучитьИдентификатор());
		...
		Элементы.ЗаказыСГруппировками.Развернуть(НовыйРодитель.ПолучитьИдентификатор());
...
Функция ВыполнитьЗапросВФоне(ИнтернетПоддержкаПодключена, ПараметрыОперации)
	...
	Задание.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"%1: %2.",
		ИмяСервисаДоставки(ТипГрузоперевозки),
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяСервисаДоставки(ТипГрузоперевозки)
	
	Возврат ?(ТипГрузоперевозки = 2, ИмяСервисаДоставки1СКурьер(), ИмяСервисаДоставки1СДоставка());
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ИмяСервисаДоставки1СДоставка()
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяСервисаДоставки1СКурьер()
...
Функция ПараметрыЗапросаСоздатьМультизаказ(ПараметрыОперации, Отказ)
	...
	Если Не Мультизаказ.ОтгрузкаНаБлижайшееВремя Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеНовогоМультизаказа(Параметры, ТипГрузоперевозки)
...
Функция ПараметрыМультизаказа()
	...
	ПараметрыЗаказа.Вставить("ОтгрузкаНаБлижайшееВремя", Ложь);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыМультизаказаОбязательные(Параметры, ТипГрузоперевозки)
	...
	Если ТипГрузоперевозки = 2 Тогда
		...
		Если Не Параметры.ОтгрузкаНаБлижайшееВремя Тогда
...
&НаКлиентеНаСервереБезКонтекста
Функция ЗаполненыОбязательныеРеквизитыМультизаказаДляОформления(ПараметрыМультизаказа, ТипГрузоперевозки)
		
	Результат = Истина;
	ОбязательныеРеквизитыМультизаказа = ПараметрыМультизаказаОбязательные(ПараметрыМультизаказа, ТипГрузоперевозки);
	ЗаполнитьЗначенияСвойств(ОбязательныеРеквизитыМультизаказа, ПараметрыМультизаказа);
...
Процедура ПослеИзмененияРеквизитовМультизаказа(Мультизаказ)
	...
	Мультизаказ.ОбязательныеРеквизитыЗаполнены = ЗаполненыОбязательныеРеквизитыМультизаказаДляОформления(Мультизаказ, ТипГрузоперевозки);
...
Процедура ДобавитьЗаказВМультизаказ(Мультизаказ, ТекущийЗаказ, НомерСтрокиДляВставки = -1)
	...
	ИспользоватьВремяДоставки = ИспользоватьВремяДоставки(НоваяСтрока);
	Если ИспользоватьВремяДоставки Тогда
		НоваяСтрока.ВремяОтгрузкиК = Дата(1,1,1) + (НоваяСтрока.ДатаОтгрузки-НачалоДня(НоваяСтрока.ДатаОтгрузки));
		Если НачалоДня(НоваяСтрока.ДатаОтгрузки) = НачалоДня(ТекущаяДатаСеанса()) 
			И НоваяСтрока.ВремяОтгрузкиК = Дата(1,1,1) Тогда
			НоваяСтрока.ОтгрузкаНаБлижайшееВремя = Истина;
		Иначе
			НоваяСтрока.ОтгрузкаНаБлижайшееВремя = Ложь;
		КонецЕсли;
	Иначе
	...
	Если Мультизаказ.ПолучитьЭлементы().Количество() = 1 Тогда
		...
		ИспользоватьВремяДоставки = ИспользоватьВремяДоставки(Мультизаказ);
		Если ИспользоватьВремяДоставки Тогда
			Мультизаказ.ВремяОтгрузкиК = Дата(1,1,1) + (Мультизаказ.ДатаОтгрузки-НачалоДня(Мультизаказ.ДатаОтгрузки));
			Если НачалоДня(Мультизаказ.ДатаОтгрузки) = НачалоДня(ТекущаяДатаСеанса()) 
				И Мультизаказ.ВремяОтгрузкиК = Дата(1,1,1) Тогда
				Мультизаказ.ОтгрузкаНаБлижайшееВремя = Истина;
			Иначе
				Мультизаказ.ОтгрузкаНаБлижайшееВремя = Ложь;
			КонецЕсли;
		КонецЕсли;
				
	КонецЕсли;
	
	Если Не ИспользоватьВремяДоставки Тогда
...
&НаСервере
Функция ИспользоватьВремяДоставки(НоваяСтрока)
	Возврат ТипГрузоперевозки = 2;
КонецФункции
...
&НаКлиентеНаСервереБезКонтекста
Процедура СформироватьПредставлениеВремениОтгрузки(Мультизаказ)
	
	Мультизаказ.ВремяОтгрузкиПредставление = ПредставлениеВремени(Мультизаказ.ВремяОтгрузкиС, 
									Мультизаказ.ВремяОтгрузкиПо, 
									Мультизаказ.ВремяОтгрузкиОбедС, 
									Мультизаказ.ВремяОтгрузкиОбедПо);
...
Процедура ЗаказыСГруппировкамиПриАктивизацииСтрокиУстановитьВидимостьДоступность(ТекущаяСтрока);
	...
	Если ТекущаяСтрока.ЭтоМультизаказ Тогда
		...
		Элементы.МультизаказыДатаОтгрузки.Доступность = Не ТекущаяСтрока.ОтгрузкаНаБлижайшееВремя;
		Элементы.МультизаказыДатаОтгрузкиНадпись.Видимость = Ложь;
		
		ПоказыватьВремяОтгрузки = ТекущаяСтрока.ЗаборОтАдреса;
		Элементы.ГруппаПериодОтгрузки.Видимость = ПоказыватьВремяОтгрузки И (ТипГрузоперевозки = 1);
		Элементы.ПериодОтгрузкиПредставление.Видимость = ПоказыватьВремяОтгрузки;
		...
		Элементы.ОтгрузкаНаБлижайшееВремя.Видимость = ПоказыватьВремяОтгрузки;
		Элементы.ВремяОтгрузки.Видимость = ПоказыватьВремяОтгрузки;
		Элементы.ВремяОтгрузки.Доступность = Не ТекущаяСтрока.ОтгрузкаНаБлижайшееВремя;
		Элементы.ВремяОтгрузкиНадпись.Видимость = Ложь;
	...
	Иначе
		...
		Элементы.ГруппаВремяОтгрузки.Видимость = ПоказыватьВремяОтгрузки И Не Элементы.ГруппаПериодОтгрузки.Видимость;
		Элементы.ОтгрузкаНаБлижайшееВремя.Видимость = Ложь;
		Элементы.ВремяОтгрузки.Видимость = Ложь;
		Элементы.ВремяОтгрузкиНадпись.Видимость = ПоказыватьВремяОтгрузки;
