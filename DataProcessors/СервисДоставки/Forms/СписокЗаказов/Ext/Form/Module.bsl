...
Процедура СписокЗаказовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "СписокСостояние" 
			...
			И Не ТекущиеДанные.ЭтоМультизаказ Тогда 
...
Процедура Отправить(Команда)
	...
	ОткрытьФорму(
		...
		ПараметрыОткрытияФормы,
		ЭтаФорма,"ПомощникОтправкиЗаказов",,,,
		РежимОткрытияОкнаФормы.Независимый);
...
Процедура НастроитьФормуПоТипуГрузоперевозки()
	...
	Элементы.СписокГруппаНаложенныйПлатеж.Видимость = Не Это1СДоставка;
	Элементы.ГруппаОтборыНаложенныйПлатеж.Видимость = Не Это1СДоставка;
