#Область злОписаниеПеременных

&НаКлиенте
Перем злКэшированныеЗначения; // Используется механизмом обработки изменения реквизитов ТЧ
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// --> Евлахов Игорь Николаевич (Начало) 13.09.2024
	// Задача #4546
	злПриСозданииНаСервере(Отказ, СтандартнаяОбработка);
	// <-- Евлахов Игорь Николаевич (Конец) 13.09.2024
...
Процедура ЗаполнитьПоТоварамПересчетаНаСервере()
	...
	Обработки.ВыгрузкаБазыТоваровВТСД.КР_ИзменитьТекстЗапросаПриЦенообразование25(Запрос.Текст,
		"РегистрЦеныНоменклатуры");
	// >> 20.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955
...
Процедура злПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект["злПравоРегистрацииШтрихкодовНоменклатурыДоступно"] = ШтрихкодированиеНоменклатурыСервер.ПравоРегистрацииШтрихкодовНоменклатурыДоступно();
...
// Задача #4546
&НаКлиенте
Процедура Подключаемый_ПоискПоШтрихкодуВыполнить(Команда)
			
	ОчиститьСообщения();
	Оповещение = Новый ОписаниеОповещения("злПоискПоШтрихкодуЗавершение", ЭтотОбъект);
...
Процедура Подключаемый_ПоискПоСпискуШтрихкодовВыполнить(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда		       
		Оповещение = Новый ОписаниеОповещения("злВопросПоискПоШтрихкодуОчиститьТаблицуЗавершение", ЭтотОбъект);
...
// Задача #4546
&НаКлиенте
Процедура злПоискПоШтрихкодуЗавершение(ДанныхШтрихкода, ДополнительныеПараметры) Экспорт
	
	злОбработатьШтрихкоды(ДанныхШтрихкода);
...
// Задача #4546
&НаКлиенте
Процедура злВопросПоискПоШтрихкодуОчиститьТаблицуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура злМодификацияФормы()
	...
	// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
	// Задача #4546
	// Команда "Поиск по штрихкоду"
	ИмяКоманды 	= "злКомандаПоискПоШтрихкоду";
	ЗаголовокПоискПоШтрихкоду = НСтр("ru = 'Поиск по штрихкоду'");
	ДействиеПоискПоШтрихкоду = "Подключаемый_ПоискПоШтрихкодуВыполнить";
	
	КомандаПоискПоШтрихкоду = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект, 
																			ИмяКоманды, 
																			ЗаголовокПоискПоШтрихкоду, 
																			ДействиеПоискПоШтрихкоду);
	
	КомандаПоискПоШтрихкоду.Картинка = БиблиотекаКартинок.НовыйПоШтрихкоду;
	КомандаПоискПоШтрихкоду.Отображение = ОтображениеКнопки.Картинка;
	КомандаПоискПоШтрихкоду.ИзменяетСохраняемыеДанные = Истина;
	КомандаПоискПоШтрихкоду.Подсказка = ЗаголовокПоискПоШтрихкоду;
	КомандаПоискПоШтрихкоду.СочетаниеКлавиш = Новый СочетаниеКлавиш(Клавиша.F7);
	
	// Команда "Найти товары по списку штрихкодов"
	...
	#Область злДобавлениеРеквизитов
	
	// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
	// Задача #4546
	ИмяРеквизита = "злПравоРегистрацииШтрихкодовНоменклатурыДоступно";	
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект, ИмяРеквизита, Новый ОписаниеТипов("Булево"));     
	// Задача #4546
	// <-- Евлахов Игорь Николаевич (Конец) 05.02.2024
	...
	// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
	// Задача #4546
	// Кнопка "Поиск по штрихкоду"
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, 
												КомандаПоискПоШтрихкоду, 
												Элементы.ТоварыКоманднаяПанель,
												Элементы.ТоварыВыбратьВыделенныеСтроки, 
												"КнопкаФормы");
																
	// Кнопка "Найти товары по списку штрихкодов"															
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, 
...
// << 07.04.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525
Процедура КР_ПерезаполнитьЦены(Таблица)
	
	Если ЗначениеЗаполнено(ВидЦены) Тогда
		Возврат;
	КонецЕсли;
	
	КР_Склад = Неопределено;
	Для Каждого ЭлементОтбора Из КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если Строка(ЭлементОтбора.ЛевоеЗначение) = "Склад" И ЭлементОтбора.Использование = Истина
			И ЗначениеЗаполнено(ЭлементОтбора.ПравоеЗначение) Тогда
			КР_Склад = ЭлементОтбора.ПравоеЗначение;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(КР_Склад) Тогда
		Возврат;
	КонецЕсли;

	ГруппаВидовНоменклатурыАксессуаров = КР_ДополнительныеНастройкиПовтИсп.Значение("ГруппаВидовНоменклатурыАксессуаров");
	ПараметрыВыгрузки = Новый Структура;
	ПараметрыВыгрузки.Вставить("Склад" ,								КР_Склад);
	ПараметрыВыгрузки.Вставить("Дата" ,									КонецДня(ТекущаяДатаСеанса()));
	ПараметрыВыгрузки.Вставить("ГруппаВидовНоменклатурыАксессуаров" ,	ГруппаВидовНоменклатурыАксессуаров);
	ПараметрыВыгрузки.Вставить("Таблица" ,								Таблица);
	Обработки.ПечатьЭтикетокИЦенников2_5.КР_ПереопределитьЦены(ПараметрыВыгрузки);
	
КонецПроцедуры // >> 07.04.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525

#КонецОбласти

#Область злШтрихкодыИТорговоеОборудование

// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
// Задача #4546
&НаСервере
Процедура злДобавитьТоварыПоШтрихкодам(Таблица)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |		ПО ИсходныеДанные.Характеристика = ХарактеристикиНоменклатуры.Ссылка
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |						(ВЫБРАТЬ
	               |							ВТДанные.Номенклатура КАК Номенклатура,
	               |							ВТДанные.ХарактеристикаЦО КАК ХарактеристикаЦО
	               |						ИЗ
	               |							ВТДанные КАК ВТДанные)) КАК РегистрЦеныНоменклатуры
	               |		ПО ВТДанные.Номенклатура = РегистрЦеныНоменклатуры.Номенклатура
	               |			И ВТДанные.ХарактеристикаЦО = РегистрЦеныНоменклатуры.ХарактеристикаЦО
	               |ГДЕ
	               |	НЕ ВЫБОР
	               |				КОГДА ВТДанные.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	               |					ТОГДА ВЫБОР
	               |							КОГДА ВТДанные.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес)
	               |								ТОГДА ИСТИНА
	               |							ИНАЧЕ ЛОЖЬ
	               |						КОНЕЦ
	               |				ИНАЧЕ ВЫБОР
	               |						КОГДА ВТДанные.Упаковка.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес)
	               |							ТОГДА ИСТИНА
	               |						ИНАЧЕ ЛОЖЬ
	               |					КОНЕЦ
	               |			КОНЕЦ";
	...
	Пока Выборка.Следующий() Цикл		
		...
		СтруктураДанных.Вставить("Штрихкод", РегистрыСведений.ШтрихкодыНоменклатуры.ПодготовитьШтрихкод(Выборка.Штрихкод));
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры") Тогда
			СтруктураДанных.Вставить("Характеристика", Выборка.Характеристика);
		КонецЕсли;
		...
		СтрокиТаблицы = Объект.Товары.НайтиСтроки(СтруктураДанных);
...
// Задача #4546
&НаСервере
Функция злДобавитьНоменклатуруПоШтрихкодам(ДанныеШтрихкодов)
	...
	Таблица.Колонки.Добавить("Цена", ОбщегоНазначения.ОписаниеТипаЧисло(10, 2));
	
	злКэшированныеЗначения = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();	
	злКэшированныеЗначения.ПравоРегистрацииШтрихкодовНоменклатурыДоступно = ЭтотОбъект["злПравоРегистрацииШтрихкодовНоменклатурыДоступно"];
	...
	Для Каждого ТекШтрихкод Из ДанныеШтрихкодов Цикл
		...
		Если Не ЗначениеЗаполнено(злКэшированныеЗначения.Штрихкоды[ТекШтрихкод.Штрихкод])
			Или злКэшированныеЗначения.Штрихкоды[ТекШтрихкод.Штрихкод].НеизвестныйШтрихкод Тогда
			НеЗакешированныеРанееШтрихкоды.Добавить(ТекШтрихкод.Штрихкод);
		КонецЕсли;
	КонецЦикла;
	
	Если НеЗакешированныеРанееШтрихкоды.Количество() > 0 Тогда		
		РегистрыСведений.ШтрихкодыНоменклатуры.ПолучитьДанныеПоШтрихкодам(злКэшированныеЗначения, НеЗакешированныеРанееШтрихкоды);		
	КонецЕсли;
	
	Для Каждого ТекШтрихкод Из ДанныеШтрихкодов Цикл		
		ДобавляемыеДанные = злКэшированныеЗначения.Штрихкоды[ТекШтрихкод.Штрихкод];
	...
	злДобавитьТоварыПоШтрихкодам(Таблица);
...
// Задача #4546
&НаКлиенте
Процедура злОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	СтруктураПараметровДействия.Вставить("НеизвестныеШтрихкоды", злДобавитьНоменклатуруПоШтрихкодам(МассивШтрихкодов));
	СтруктураПараметровДействия.Вставить("ФормаВладелец",        УникальныйИдентификатор);
	
	Если ЭтотОбъект["злПравоРегистрацииШтрихкодовНоменклатурыДоступно"]
		И СтруктураПараметровДействия.НеизвестныеШтрихкоды.Количество() > 0 Тогда
		
		// ЭлектронноеВзаимодействие.РаботаСНоменклатурой
		ШтрихкодированиеНоменклатурыКлиент.ОбработатьНеизвестныеШтрихкоды(СтруктураПараметровДействия, злКэшированныеЗначения, ЭтаФорма);
