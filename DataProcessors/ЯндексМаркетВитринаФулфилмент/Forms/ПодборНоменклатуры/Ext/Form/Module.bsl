...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Документ = Неопределено и НЕ Параметры.ПодборВАссортиментЯндексМаркет Тогда
...
Процедура КорзинаНоменклатураПриИзменении(Элемент)
	...
	ЗаполнитьВариантОбеспеченияИФлаг(ЭтотОбъект, ТекущаяСтрока,
...
Процедура ПодборТаблицаНоменклатураВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если (РежимПодбораДляДокументовЦенообразования И ИспользуетсяЦенообразование25) 
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТолькоОбособленно");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	Если ЗапрашиватьКоличество И Не РежимПодбораБезКоличественныхПараметров Тогда
		...
		Если (ПодборВариантовОбеспечения
			...
			И (ЗначениеЗаполнено(Склад) Или ОстаткиПоВсемСкладам) Тогда
				...
				ДопустимыеНеобособленныеДействия = ПараметрыФормы.ДопустимыеНеобособленныеДействия;
				...
				ПараметрыФормы.ДопустимыеНеобособленныеДействия  = ДопустимыеНеобособленныеДействия;
				ПараметрыФормы.ДопустимыеОбособленныеДействия    = ПараметрыФормы.ДопустимыеНеобособленныеДействия.Скопировать();
				Если ТолькоОбособленно И ПараметрыТовара.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар") Тогда
...
Процедура ПеретащитьВКорзинуНаСервере(Данные)
	...
	Для Каждого НоваяСтрока Из НовыеСтроки Цикл
		...
		ПараметрыСтроки = Новый Структура("Номенклатура, Характеристика, ВариантОбеспечения, Обособленно, Упаковка, Цена, Количество, КоличествоУпаковок, Сумма, ВидЦены, Склад, ДатаОтгрузки, СрокПоставки, ИндексСовместноПродаваемыеТовары, ПроцентРучнойСкидки, СуммаРучнойСкидки");
		...
		ЗаполнитьВариантОбеспеченияИФлаг(ЭтотОбъект, ПараметрыСтроки,
			ВариантОбеспеченияИФлагОбособленно, ТолькоОбособленно, ОграничиватьВариантыОбеспечения);
...
Процедура ЗаполнитьВариантОбеспеченияИФлаг(Форма, ТекущаяСтрока, ВариантОбеспеченияИФлагОбособленно, ТолькоОбособленно, ОграничиватьВариантыОбеспечения)
	
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		
		Если Форма.РежимПодбораОстатковПоНаправлениюДеятельности Тогда
			
			ТекущаяСтрока.Обособленно = Форма.Обособленно
			И (ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар")
				Или ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа"));
			
		ИначеЕсли Форма.ПодборВариантовОбеспечения Тогда
			...
			Если ТолькоОбособленно И ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар") Тогда
				ДопустимыеНеобособленныеДействия.Очистить();
			КонецЕсли;
...
Процедура ЗагрузитьСписокВыбораВариантовОбеспечения()
	...
	ДопустимыеНеобособленныеДействия  = ОбеспечениеВДокументахКлиентСервер.ВсеДействия();
	ДопустимыеОбособленныеДействия    = ОбеспечениеВДокументахКлиентСервер.ВсеДействия();
	Если ТолькоОбособленно И ТекущиеДанные.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар") Тогда
		ДопустимыеНеобособленныеДействия.Очистить();
	КонецЕсли;
