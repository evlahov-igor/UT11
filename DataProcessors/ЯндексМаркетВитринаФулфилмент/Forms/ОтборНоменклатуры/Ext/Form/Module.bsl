...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если СтруктураДанных.Свойство("НастройкиКомпоновщикаКолонок") Тогда
	...
	Иначе
		...
		Если УпрощеннаяНастройка.НастройкиБудутПотеряны И ВидНастройки <> "Простой" Тогда
		...
		Если НЕ ТекСтрока.ЭтоГруппа Тогда
			Если ТекСтрока.Выбран Тогда 	
				Для Каждого ЗависимаяЦена Из ТекСтрока.ЗависимыеЦены Цикл
					...
					Если СтрокаЗависимойЦены <> Неопределено И Не СтрокаЗависимойЦены.Выбран И Не СтрокаЗависимойЦены.ЗапрещенныйВидЦены Тогда
						СтрокаЗависимойЦены.Выбран = Истина;
						СтрокаЗависимойЦены.Редактировать = Истина;
						Заполнять = Истина;
						ОбновитьФлагиВыбранныхЦенНаСервере(ТекСтрока);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		Иначе
		...
		Если НЕ ТекСтрока.ЭтоГруппа Тогда
			Если ТекСтрока.Выбран Тогда 	
				Для Каждого ВлияющаяЦена Из ТекСтрока.ВлияющиеЦены Цикл
					...
					Если СтрокаВлияющейЦены <> Неопределено И Не СтрокаВлияющейЦены.Выбран И Не СтрокаВлияющейЦены.ЗапрещенныйВидЦены Тогда
...
Процедура ПреобразоватьУпрощеннуюНастройкуВРасширенную(Форма)
	...
	Если Форма.ВариантУстановкиЦены = 1 Тогда
	...
	ИначеЕсли Форма.ВариантУстановкиЦены = 2 Тогда
