...
Процедура ЗаполнитьРезультат()
	...
	Для каждого Строка Из МестаПоставки Цикл
		...
		КодыАдреса = АдресныйКлассификатор.КодыАдреса(Строка.ДанныеМеста);
		Если НЕ ЗначениеЗаполнено(КодыАдреса.КодКЛАДР) Тогда
			КодыАдреса.КодКЛАДР = ЭлектронноеАктированиеЕИС.КодКЛАДРПроизвольногоАдреса();
		КонецЕсли;
		...
		ЗаписьМесто.Код = ЭлектронноеАктированиеЕИС.ПреобразоватьКодКладрДляЕИС(
				КодыАдреса.КодКЛАДР);
