...
Функция ПроверитьПриложенныеДокументыРасчетРазмера(ПроверятьВидДокументаТипФайла = Ложь, ВыводитьСообщениеОПревышенииРазмера = Ложь)
	...
	ЕщёМожноПриложить = Окр((МаксимальныйРазмерФайлов - ОбщийРазмер) / БайтовВМегабайте, 1);
	
	Если ЕщёМожноПриложить < 0 И ВыводитьСообщениеОПревышенииРазмера Тогда
	...
	Если ПриложенныеДокументы.Количество() > 0 Тогда
		...
		ОбщийРазмерМб = Окр(Результат.ОбщийРазмерМб, 1);
		ЕщёМожноПриложить = Макс(ЕщёМожноПриложить, -ЕщёМожноПриложить);
		
		// Вывод в надпись.
		Если ЕщёМожноПриложить < 0 Тогда
			Шаблон = НСтр("ru='Общий размер %1 Мб. Допустимый размер превышен на %2 Мб.'");
			Элементы.НадписьОбщийРазмерФайлов.ЦветТекста = ЦветТекстаКрасный();
		Иначе
			Шаблон = НСтр("ru='Общий размер %1 Мб. Можно дополнительно приложить файлы размером %2 Мб.'");
			Элементы.НадписьОбщийРазмерФайлов.ЦветТекста = ЦветТекстаОбычный();
		КонецЕсли;
		НадписьОбщийРазмерФайлов = СтрШаблон(Шаблон, ОбщийРазмерМб, ЕщёМожноПриложить);
