	...
	Попытка
		...
		Для Каждого СтрокаАгрегации Из ТаблицаАгрегаций Цикл
			...
			Для НомерУпаковки = 1 По СтрокаАгрегации.КоличествоУпаковок Цикл
				...
				СтрокаУпаковки.ВидУпаковки      = Перечисления.ВидыУпаковокИС.Логистическая;
		...
		|	GTIN
		|";
		ИнтеграцияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ТекстЗапроса, "ОписаниеНоменклатурыИС", "ПулКодовМаркировки.Номенклатура");
							...
							Для Каждого СтрокаУпаковки Из СтрокаАгрегации.СтрокиУпаковок Цикл
								Для Каждого СтрокаСостава Из СтрокиСостава Цикл
									Для НомерЭкземпляра = 1 По СтрокаСостава.Количество Цикл
										...
										СтрокаПродукции.ВидУпаковки  = Перечисления.ВидыУпаковокИС.Потребительская;
										...
										ЗаполнитьЗначенияСвойств(СтрокаПродукции, ВыборкаНоменклатура, "ТребуетВзвешивания,ПроизвольнаяЕдиницаУчета");
										Если СтрокаПродукции.ПроизвольнаяЕдиницаУчета Тогда
											СтрокаПродукции.КоличествоПотребительскихУпаковок = 1;
											СтрокаПродукции.Количество                        = ВыборкаНоменклатура.КоличествоВПотребительскойУпаковке;
										Иначе
									...
									ЗаполнитьЗначенияСвойств(СтрокаУпаковки, ВыборкаНоменклатура, "ТребуетВзвешивания,ПроизвольнаяЕдиницаУчета");
									Если СтрокаУпаковки.ПроизвольнаяЕдиницаУчета Тогда
										Если СтрокаУпаковки.ТипУпаковки = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка Тогда
											СтрокаУпаковки.КоличествоПотребительскихУпаковок = СтрокаУпаковки.Количество;
											СтрокаУпаковки.Количество                        = ВыборкаНоменклатура.КоличествоВПотребительскойУпаковке * СтрокаУпаковки.КоличествоПотребительскихУпаковок;
										Иначе
											СтрокаУпаковки.КоличествоПотребительскихУпаковок = 0;
											СтрокаУпаковки.Количество                        = 0;
										КонецЕсли;
									КонецЕсли;
								КонецЦикла;
							КонецЦикла;
		...
		ДанныеШапки           = ШтрихкодированиеИС.ПолучитьСтруктуруРеквизитовДокумента(Параметры.Документ);
...
Процедура ЗаполнитьОстаткиПродукцииПоДокументу(Параметры, ДанныеЗаполнения)
	...
	|	И ПулКодовМаркировки.Шаблон <> Значение(Перечисление.ШаблоныКодовМаркировкиСУЗ.АльтернативныйТабакБлок)
	|	И ПулКодовМаркировки.Шаблон <> Значение(Перечисление.ШаблоныКодовМаркировкиСУЗ.НикотиносодержащаяПродукцияБлок)
	|	
	|СГРУППИРОВАТЬ ПО
