...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ВыполненоПодключение() Тогда
		Отказ = Истина;
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	Если СценарийПрогнозирования.Пустая() Тогда
		ТекстОшибки = НСтр("ru = 'Не заполнен сценарий прогнозирования'");
...
&НаСервере
Процедура Подключаемый_ОбновитьПрогнозНаСервере()
	Если ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗаданияОбновитьПрогноз) Тогда
...
&НаСервере
Процедура Подключаемый_ЗагрузитьПрогнозНаСервере()
	Если ДлительныеОперации.ЗаданиеВыполнено(ИдентификаторЗаданияЗагрузитьПрогноз) Тогда
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	...
	// Сервис прогнозирования.
	ДоступностьФормированияОтчета = ЗначениеЗаполнено(ВидПлана)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПлана, "ЗаполнятьПоДаннымСервиса");
	Элементы.СформироватьОтчет.Доступность = ДоступностьФормированияОтчета;
	Элементы.СформироватьОтчетЕще.Доступность = ДоступностьФормированияОтчета;
	Элементы.ФормироватьСразу.Доступность = ДоступностьФормированияОтчета;
...
Процедура ОбновитьПрогнозНаСервере()
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		Если СервисПрогнозирования.ИдетВыгрузка() Тогда
...
Функция ОтладочнаяИнформацияПоВидуПлана()
	...
	Ответ = НСтр("ru = 'Не выбран вид плана'");
	...
	Если ЗначениеЗаполнено(ВидПлана) Тогда
		...
		|%1
		|========================================
		|Общая информация о виде плана (модели)
		|Вид плана: %2
		|Идентификатор модели: %3
		|========================================
		|Информация о последней загруженной модели
		|%4
		|========================================
		|Информация о последней обучающейся модели
		|%5
		|========================================
		|Общая информация (может отличаться от данных на сервере)
		|Последняя известная дата продаж (без учета отборов): %6
		|Идентификаторы коллекций:
		|%7
		|========================================
		|Настройки модели (могут отличаться от данных на сервере)
		|%8'");
		
		ШаблонАутентификационныеДанные = НСтр("ru = 'Логин: %1
		|Токены заданы: %2
		|Дата обновления токенов: %3
		|Идентификатор (application_uuid): %4'");
		
		ШаблонИнфоМодели = НСтр("ru = 'Идентификатор обучения модели: %1
		|Статус обучения: %2
		|Текст ошибки обучения: %3'");
		...
		ПараметрыШаблона.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПлана, "Наименование"));
		
		Если НастройкиСервиса.МоделиПрогнозирования.Получить(ВидПлана) <> Неопределено Тогда
			ПараметрыШаблона.Добавить(НастройкиСервиса.МоделиПрогнозирования.Получить(ВидПлана));
		Иначе
			ПараметрыШаблона.Добавить(НСтр("ru = 'Нет'"));
		КонецЕсли;
		...
		Если ИнфоМодели.ЗагруженнаяМодель <> Неопределено Тогда
			ИнформацияОМодели = СтрШаблон(ШаблонИнфоМодели,
				ИнфоМодели.ЗагруженнаяМодель.ИдОбучения,
				ИнфоМодели.ЗагруженнаяМодель.СтатусОбучения,
				ИнфоМодели.ЗагруженнаяМодель.ТекстОшибки);
			ПараметрыШаблона.Добавить(ИнформацияОМодели);
		Иначе
			ПараметрыШаблона.Добавить(НСтр("ru = 'Нет'"));
		КонецЕсли;
		...
		Если ИнфоМодели.ОбучающаясяМодель <> Неопределено Тогда
			ИнформацияОМодели = СтрШаблон(ШаблонИнфоМодели,
				ИнфоМодели.ОбучающаясяМодель.ИдОбучения,
				ИнфоМодели.ОбучающаясяМодель.СтатусОбучения,
				ИнфоМодели.ОбучающаясяМодель.ТекстОшибки);
			ПараметрыШаблона.Добавить(ИнформацияОМодели);
		Иначе
			ПараметрыШаблона.Добавить(НСтр("ru = 'Нет'"));
		КонецЕсли;
		...
		Для Каждого КлючЗначение Из НастройкиСервиса.ИдКоллекций Цикл
			ИнфоОКоллекцияхСтрокой = ИнфоОКоллекцияхСтрокой
				+ СтрШаблон(ШаблонСтрокиПараметровМодели, КлючЗначение.Ключ, Строка(КлючЗначение.Значение))
				+ Символы.ПС;
		КонецЦикла;
		ПараметрыШаблона.Добавить(ИнфоОКоллекцияхСтрокой);
		
		СписокРеквизитов = СервисПрогнозированияПереопределяемыйКлиентСервер.РеквизитыВидаПланаДляСервисаПрогнозирования();
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидПлана, СписокРеквизитов);
		
		ИнфоОРеквизитахСтрокой = "";
		Для Каждого КлючЗначение Из ЗначенияРеквизитов Цикл
			ИнфоОРеквизитахСтрокой = ИнфоОРеквизитахСтрокой
				+ СтрШаблон(ШаблонСтрокиПараметровМодели, КлючЗначение.Ключ, Строка(КлючЗначение.Значение))
				+ Символы.ПС;
		КонецЦикла;
		
		ПараметрыШаблона.Добавить(ИнфоОРеквизитахСтрокой);
...
&НаСервере
Функция ВыполненоПодключение()
	НастройкиСервиса = СервисПрогнозирования.ПолучитьНастройкиСервиса();
	Возврат ЗначениеЗаполнено(НастройкиСервиса.ТокенПриложения);
КонецФункции
