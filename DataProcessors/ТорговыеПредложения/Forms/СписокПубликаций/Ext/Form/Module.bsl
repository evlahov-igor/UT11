...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли СтрНайти(ПоляДополнительныхНастроекПубликации, Поле.Имя) Тогда
			...
			Элементы.Список.Обновить();
		ИначеЕсли Поле = Элементы.СписокОписаниеОшибки И Не ТекущиеДанные.Отключено
			И ТекущиеДанные.Состояние = ПредопределенноеЗначение(
...
Процедура УстановитьУсловноеОформление()
	...
	// Представление значения почты для уведомления из справочника организации.
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Представление = НСтр("ru = 'Представление значения почты для уведомления из справочника организации.'");
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СписокАдресЭлектроннойПочты.Имя);
	...
	ОтборЭлемента = ГруппаЭлементовОтбораДанных.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.ЭтоАдресЭлектроннойПочтыОрганизации");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура УстановитьЗапросДинамическогоСписка(ТаблицаТорговыхПредложений)
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(СостояниеСинхронизации.АдресЭлектроннойПочты КАК СТРОКА(100)) = """"
	|			ТОГДА ОрганизацииАдресЭлектроннойПочты.АдресЭП + "" <изменить>""
	|		ИНАЧЕ СостояниеСинхронизации.АдресЭлектроннойПочты
	|	КОНЕЦ КАК АдресЭлектроннойПочты,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(СостояниеСинхронизации.АдресЭлектроннойПочты КАК СТРОКА(100)) = """"
	|		И ОрганизацииАдресЭлектроннойПочты.АдресЭП <> """"
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоАдресЭлектроннойПочтыОрганизации,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Организации.КонтактнаяИнформация КАК ОрганизацииАдресЭлектроннойПочты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыКонтактнойИнформации КАК ВидыКонтактнойИнформации
	|			ПО ОрганизацииАдресЭлектроннойПочты.Вид = ВидыКонтактнойИнформации.Ссылка
	|			И ВидыКонтактнойИнформации.Предопределенный
	|		ПО СостояниеСинхронизации.Организация = ОрганизацииАдресЭлектроннойПочты.Ссылка
	|		И ОрганизацииАдресЭлектроннойПочты.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.EmailОрганизации)";
