...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РазрешитьИзменениеКонтрагента         = Не ФиксированныйОтборПоКонтрагенту 
	...
	Если ФоновоеЗаданиеПоиска <> Неопределено Тогда
		УстановитьКартинкуКомандыНайти(
			Истина, Элементы.НайтиПредложения, Элементы.ДекорацияДлительнаяЗагрузкаКатегорий);
	КонецЕсли;
	...
	ЗаполнитьФиксированныеОтборыДерева(
		ДеревоОтборов, НаименованиеКонтрагента, РазрешитьИзменениеКонтрагента, БыстрыеОтборы);
...
Процедура ПоказатьСкрытьИзображения(Команда)
	...
	ВидимостьИзображений = Элементы.ПоказатьСкрытьИзображения.Пометка;
	
	Элементы.ПоказатьСкрытьИзображения.Пометка    = НЕ ВидимостьИзображений;
	Элементы.ГруппаИзображение.Видимость          = НЕ ВидимостьИзображений;
...
Процедура АдресКартинкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Формат = ФорматКартинки.НеизвестныйФормат Тогда
		Возврат;
	КонецЕсли;
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	Если Не Элементы.ПоказатьСкрытьИзображения.Пометка Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТекущиеДанные = Неопределено 
		Или ПустаяСтрока(ТекущиеДанные.АдресаКартинок) Тогда
		Возврат;
	КонецЕсли;
	
	АдресаКартинок = СтрРазделить(ТекущиеДанные.АдресаКартинок, "|");
	
	Если АдресаКартинок.Количество() > 1 Тогда
		
		ВидимостьПереключателей = Истина;
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, "ГруппаУправлениеИзображениями", "Видимость", ВидимостьПереключателей);
		
	КонецЕсли;
	
	ДанныеИзображений = Новый Массив;
	ДанныеИзображенийПоСсылкам(АдресаКартинок, ДанныеИзображений);
	КартинкиТорговогоПредложения.ЗагрузитьЗначения(ДанныеИзображений);
	
	АдресКартинки = ПоместитьВоВременноеХранилище(
		КартинкиТорговогоПредложения[0].Значение, УникальныйИдентификатор);
...
Процедура ОбработатьРезультатИзмененияФиксированногоОтбора(ТекущаяСтрокаДерева, ДанныеОтбораИзменены, Знач Результат)
	...
	Если Идентификатор = "Поставщик" Тогда
		...
		Иначе
			...
			// При выборе значения отбора из справочника Контрагенты, в результат возвращается тип СправочникСсылка.Контрагенты
...
Процедура ДеревоОтборовПриИзменении(Элемент)
	...
	Если ТекущаяСтрока.ФиксированныйОтбор Тогда
		...
		Если Идентификатор = "Поставщик" Тогда
			...
			ЕстьОтбор = БыстрыеОтборы.НайтиПоЗначению(ТекущаяСтрока.Идентификатор);
			Если ЕстьОтбор = Неопределено Тогда
				Результат = ЗначениеЗаполнено(ТекущаяСтрока.ПредставлениеОтбора);
			КонецЕсли;
...
Процедура СледующаяКартинка(Команда)
	...
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено 
		Или КартинкиТорговогоПредложения.Количество() <= 1 Тогда
		Возврат;
	КонецЕсли;
	
	ЭлементСписка = КартинкиТорговогоПредложения.НайтиПоЗначению(ПолучитьИзВременногоХранилища(АдресКартинки));
	Если ЭлементСписка = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ИндексКартинки = КартинкиТорговогоПредложения.Индекс(ЭлементСписка);
	
	Если ИндексКартинки = КартинкиТорговогоПредложения.Количество() - 1 Тогда
		ИндексКартинки = 0;
	Иначе
	...
	АдресКартинки = ПоместитьВоВременноеХранилище(
		КартинкиТорговогоПредложения[ИндексКартинки].Значение, УникальныйИдентификатор);
...
Процедура ПредыдущаяКартинка(Команда)
	...
	Если ИндексКартинки = 0 Тогда
		ИндексКартинки = КартинкиТорговогоПредложения.Количество() - 1;
	Иначе
		ИндексКартинки = ИндексКартинки - 1;
	КонецЕсли;
...
Процедура ДобавитьПредложениеВКорзинуПослеОповещения(Параметр)
	...
	УстановитьГиперссылкуИконкуКоличества(СтрокаСписка, СтрокаСписка.ГиперссылкаДобавить);
...
Процедура НайтиТорговыеПредложения(СледующаяСтраница = Ложь)
	...
	ФоновоеЗаданиеПоиска = НайтиТорговыеПредложенияВФоне(ЗаполненныеПараметрыЗапроса, СледующаяСтраница, , Истина);
...
Функция НоваяСтруктураСпискаПредложений()
	...
	СтруктураПредложений.Вставить("АдресаКартинок",           "");
...
Процедура ЗаполнитьПерсональнуюЦену(СтруктураПредложений, ЭлементКоллекции, ОтборОрганизация)
	...
	ПоисковыеДанные = "";
	Если ОтборОрганизация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель Тогда
		ПоисковыеДанные = ОтборОрганизация.ИНН;
	ИначеЕсли ОтборОрганизация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
...
Процедура ЗаполнитьАдресаКартинок(СтруктураПредложений, АдресаКартинок)
	...
	СтруктураПредложений.АдресаКартинок = СтрСоединить(АдресаКартинок, "|");
...
Процедура УдалитьКорзину()
	...
	ТорговыеПредложенияКлиент.УдалитьКорзину(ПараметрыМетода, ЭтотОбъект, Оповещение);
...
&НаСервереБезКонтекста
Процедура ДанныеИзображенийПоСсылкам(Знач АдресаКартинок, ДанныеИзображений)
	
	ОтносительныеАдресаКартинок = Истина;
	
	Если СтрНайти(АдресаКартинок[0], "http") Тогда
		ОтносительныеАдресаКартинок = Ложь;
	КонецЕсли;
	
	ТорговыеПредложенияСлужебный.ДанныеИзображенийПоСсылкам(
		АдресаКартинок, 
		ДанныеИзображений, 
		ОтносительныеАдресаКартинок);
