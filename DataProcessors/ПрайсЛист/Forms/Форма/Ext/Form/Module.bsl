...
&НаСервере
Процедура УдалитьСтрокиИсключенийПоНоменклатуреИздереваЦен(Дерево, ОтборПоЗначениюЗапрета, МассивИменКолонок)
	...
	Пока Индекс >=0 Цикл
		...
		Если Стр.Строки.Количество()>0 Тогда
			УдалитьСтрокиИсключенийПоНоменклатуреИздереваЦен(Стр, ОтборПоЗначениюЗапрета, МассивИменКолонок);
		Иначе
			...
			Для Каждого Колонка Из МассивИменКолонок Цикл
				...
				КонецЕсли
			КонецЦикла;
			Если НЕ НайденоОтличноеЗначение Тогда
