...
Функция ДокументыВводаОстатковПодлежащиеОбработке(Действие)
	...
	|ГДЕ (Док.ДокументОснование ССЫЛКА Документ.ВводОстатковТоваров 
	|	ИЛИ Док.ДокументОснование ССЫЛКА Документ.ВводОстатковВзаиморасчетов
	|	ИЛИ Док.ДокументОснование ССЫЛКА Документ.ПервичныйДокумент)
	|	И  Док.ДокументОснование.Дата <= &КонДата " + ДопУсловиеСФ + "
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Док.Ссылка
	|ИЗ Документ.СчетФактураПолученныйАванс КАК Док
	|ГДЕ (Док.ДокументОснование ССЫЛКА Документ.ВводОстатковВзаиморасчетов)
	|	И  Док.ДокументОснование.Дата <= &КонДата " + ДопУсловиеСФ + "
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Док.Ссылка
...
Процедура ПометитьНаУдалениеДокументы(ИмяДокумента, МассивПропускаемыхДокументов, МассивУдаляемыхДокументов)
	...
	ТекстЗапроса = ТекстЗапроса + "УПОРЯДОЧИТЬ ПО Дата УБЫВ";	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"#ЭтоЗаменяется#", ИмяДокумента);
	...
	Если ВидРасчетов = "РасчетыСКлиентами" Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("ХозяйственнаяОперация") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл 
				НуженСФАванс = Ложь;
				Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам 
					Или Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов Тогда
					СистемаНалогообложения = НастройкиНалоговУчетныхПолитикЛокализация.СистемаНалогообложенияОрганизации(Выборка.Организация,
						КонецДня(ДатаСверткиИБ));
				...
				Пока Выборка.СледующийПоЗначениюПоля("Партнер") Цикл
					...
					Пока Выборка.СледующийПоЗначениюПоля("ОбъектРасчетов") Цикл
						...
						Если ЗначениеЗаполнено(НоваяСтрока.ОбъектРасчетов) Тогда
							...
							Иначе
								РеквизитыОбъекта.Вставить("Договор", НоваяСтрока.ДокументРасчетов.Договор);
							КонецЕсли;
						...
						Если НуженСФАванс Тогда
							Если ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам 
								...
								Если (ЗначениеЗаполнено(ДатаПлатежа) И ДатаПлатежа <> Выборка.ДатаПлатежа)
									Или (ЗначениеЗаполнено(Контрагент) И Выборка.Контрагент <> Контрагент) Тогда
									ИзменилисьДанныеДляСФАванс = Истина;
								ИначеЕсли ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов
									И ((ЗначениеЗаполнено(ДатаРасчетногоДокумента) И Выборка.ДатаРасчетногоДокумента <> ДатаРасчетногоДокумента)
										Или (ЗначениеЗаполнено(НомерРасчетногоДокумента) И Выборка.НомерРасчетногоДокумента <> НомерРасчетногоДокумента))
										Или (ЗначениеЗаполнено(Контрагент) И Выборка.Контрагент <> Контрагент) Тогда
									ИзменилисьДанныеДляСФАванс = Истина;
								КонецЕсли;
				...
				Если ДокОст.РасчетыСПартнерами.Количество() > 0 Тогда
					ДокОст.Записать(РежимЗаписиДокумента.Запись);
					КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
					Если НуженСФАванс И (ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам 
						Или ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовКлиентов) Тогда
...
Процедура СформироватьСчетФактуруНаАванс(ДокОст, Сумма, ДатаПлатежа, ДатаРасчетногоДокумента, НомерРасчетногоДокумента, НомерСФПоПорядку, Контрагент)
	...
	Если ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам 
		...
		Если ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАвансовПоставщикам  Тогда
				...
				Пока Выборка.СледующийПоЗначениюПоля("НалогообложениеНДС") Цикл
					Пока Выборка.СледующийПоЗначениюПоля("ДокументПартии") Цикл
						Пока Выборка.СледующийПоЗначениюПоля("СчетФактура") Цикл
							...
							Если НЕ ЗначениеЗаполнено(ДокОст.НалогообложениеНДС) Тогда
								//ДокОст.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию;
								ПараметрыЗаполнения = Документы.ЗаявкаНаВозвратТоваровОтКлиента.ПараметрыЗаполненияНалогообложенияНДСПродажи(ДокОст);
								УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(ДокОст.НалогообложениеНДС, ПараметрыЗаполнения);
							КонецЕсли;
