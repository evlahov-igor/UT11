...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671  
	КР_ДополнительныеКолонкиПриСозданииНаСервере();
	// >> 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	КР_ИнициализироватьДопНастройки();
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	НомерКолонки = 1;
		// << 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
	Если ЭтаФорма["КР_ПереместитьКолонкуЯчейка"] Тогда
		
		Если Не ЭтаФорма["КР_НеВыводитьПолеНоменклатурыАртикул"] Тогда
			НомераКолонок.Вставить("КолонкаАртикул", "C" + Формат(НомерКолонки+1, "ЧН=0; ЧГ=0"));
	...
	Иначе	
		...
		НомерКолонки = НомерКолонки +1;
		// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
		Если Не ЭтаФорма["КР_НеВыводитьПолеНоменклатурыКод"] Тогда
			// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
		...
		Если Не ЭтаФорма["КР_НеВыводитьПолеНоменклатурыАртикул"] Тогда
			...
			// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
		КонецЕсли;
	КонецЕсли;	// << 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035

	Если Не ЭтаФорма["КР_НеВыводитьПолеНоменклатура"] Тогда
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	НомераКолонок.Вставить("КолонкаНоменклатура", "C" + Формат(НомерКолонки, "ЧН=0; ЧГ=0"));
	НомерКолонки = НомерКолонки +1;
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	КонецЕсли;
	Если Не ЭтаФорма["КР_НеВыводитьПолеХарактеристикиНоменклатуры"] Тогда
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	...
	// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671  
	КР_ДополнитьНомераКолонокДополнительнымиКолонки(НомераКолонок, НомерКолонки);
	// >> 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671
	...
	Пока ЗаполненыНоменклатураКодАртикулШтрихкод Цикл
		...
		Попытка
			...
			НоваяСтрока.Код = ?(ЭтаФорма["КР_ПереместитьКолонкуЯчейка"],"",ТабличныйДокумент.Область("R" + СтроковыйНомер + НомераКолонок.Получить("КолонкаКод")).Текст);
		...
		// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671   
		КР_ЗаполнитьСтрокуДаннымиДополнительныхКолонок(
			НомераКолонок, НоваяСтрока, СтроковыйНомер, НомерСтроки, ВысотаШапки);			
		// >> 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671
		...
		// << 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
		Если ЭтаФорма["КР_ПереместитьКолонкуЯчейка"] Тогда
			НоваяСтрокаТовары.Ячейка = НоваяСтрока.Ячейка;
		КонецЕсли;
		// >> 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	Если ЭтаФорма["КР_ПереместитьКолонкуЯчейка"] Тогда
		КР_ИнициализироватьТабличныйДокументДополнительныеКолонки(МакетЗаполненияПоВнешнемуИсточнику, Истина);
	...
	// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671
	// << 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
	//КР_ИнициализироватьТабличныйДокументДополнительныеКолонки(МакетЗаполненияПоВнешнемуИсточнику);
	КР_ИнициализироватьТабличныйДокументДополнительныеКолонки(МакетЗаполненияПоВнешнемуИсточнику, Ложь);
	// << 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
...
#Область КР_ДополнительныеКолонки

// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671 
&НаСервере
Процедура КР_ДополнительныеКолонкиПриСозданииНаСервере()
	
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ДополнительныеКолонки", Новый ОписаниеТипов());
	
	Если Не Параметры.Свойство("КР_ДополнительныеКолонки", ЭтотОбъект.КР_ДополнительныеКолонки) Тогда 
		
		ЭтотОбъект.КР_ДополнительныеКолонки = Новый ФиксированныйМассив(Новый Массив);
		Возврат;
		
	КонецЕсли;	

	Для Каждого ДополнительнаяКолонка Из ЭтотОбъект.КР_ДополнительныеКолонки Цикл 
		
		РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
			ДополнительнаяКолонка.Имя, 
			ДополнительнаяКолонка.Тип, 
			"Объект.Товары", 
			ДополнительнаяКолонка.Заголовок
		);

		КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
			РеквизитФормы, Элементы.Товары);
		
		РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
			ДополнительнаяКолонка.Имя, ДополнительнаяКолонка.Тип, "УточнениеНоменклатуры");
			
	КонецЦикла;	
		
КонецПроцедуры // >> 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671

// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671 
&НаСервере
Процедура КР_ИнициализироватьТабличныйДокументДополнительныеКолонки(МакетЗаполненияПоВнешнемуИсточнику, ПереместитьКолонкуЯчейка)
	
	Для Каждого ДополнительнаяКолонка Из ЭтотОбъект.КР_ДополнительныеКолонки Цикл 
		
		ОбластьКолонка = МакетЗаполненияПоВнешнемуИсточнику.ПолучитьОбласть("Количество");
		
		Если ЗначениеЗаполнено(ДополнительнаяКолонка.Заголовок) Тогда 
			ЗаголовокКолонки = ДополнительнаяКолонка.Заголовок;
		Иначе
			ЗаголовокКолонки = ДополнительнаяКолонка.Имя;
		КонецЕсли;
		
		ОбластьКолонка.Область(1, 1).Текст = ЗаголовокКолонки; 
	// << 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
		Если ДополнительнаяКолонка.Имя = "Ячейка" и ПереместитьКолонкуЯчейка Тогда
			ТабличныйДокумент.Присоединить(ОбластьКолонка);
			Прервать 
		ИначеЕсли НЕ ДополнительнаяКолонка.Имя = "Ячейка" и НЕ ПереместитьКолонкуЯчейка Тогда 
			ТабличныйДокумент.Присоединить(ОбластьКолонка);
		КонецЕсли;	
	// >> 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
	КонецЦикла;	
...
// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671  
&НаСервере
Процедура КР_ДополнитьНомераКолонокДополнительнымиКолонки(
	НомераКолонок, НомерКолонки)

	Для Каждого ДополнительнаяКолонка Из ЭтотОбъект.КР_ДополнительныеКолонки Цикл 
		// << 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
		Если ДополнительнаяКолонка.Имя = "Ячейка" Тогда
			НомераКолонок.Вставить(ДополнительнаяКолонка.Имя, "C" + Формат(1, "ЧН=0; ЧГ=0"));
		Иначе
		// >> 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
			НомераКолонок.Вставить(ДополнительнаяКолонка.Имя, "C" + Формат(НомерКолонки, "ЧН=0; ЧГ=0"));
			НомерКолонки = НомерКолонки +1;
		КонецЕсли;	// << 02.08.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
	КонецЦикла;
...
// << 07.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-671   
&НаСервере
Процедура КР_ЗаполнитьСтрокуДаннымиДополнительныхКолонок(
	НомераКолонок, НоваяСтрока, СтроковыйНомер, НомерСтроки, ВысотаШапки)
	
	// На момент выполнения закодировано только для числовых и строковых типов колонки
	Для Каждого ДополнительнаяКолонка Из ЭтотОбъект.КР_ДополнительныеКолонки Цикл 
		
		ИмяКолонки = ДополнительнаяКолонка.Имя;
		ЗначениеКолонки = ТабличныйДокумент.Область("R" + СтроковыйНомер + НомераКолонок.Получить(ИмяКолонки)).Текст; 
		...
		Если ДополнительнаяКолонка.Тип.СодержитТип(Тип("Число")) Тогда 

			ПрисвоитьЧисловоеЗначениеПриЗаполненииТЧ(
				НоваяСтрока[ИмяКолонки], ЗначениеКолонки, НомерСтроки, ИмяКолонки, ВысотаШапки);
				
		// << 21.10.2022, Маскаев П.Ю., КРОК, JIRA№ A2105505-689
		ИначеЕсли ДополнительнаяКолонка.Тип.СодержитТип(Тип("СправочникСсылка.ФорматыМагазинов")) Тогда
			НоваяСтрока[ИмяКолонки] = Справочники.ФорматыМагазинов.НайтиПоНаименованию(ЗначениеКолонки);
		// >> 21.10.2022, Маскаев П.Ю., КРОК, JIRA№ A2105505-689
		// << 16.12.2022, Маскаев П.Ю., КРОК, JIRA№ A2105505-805
		ИначеЕсли ДополнительнаяКолонка.Тип.СодержитТип(Тип("СправочникСсылка.КР_ГруппыАналоговТоваров")) Тогда
			НоваяСтрока[ИмяКолонки] = Справочники.КР_ГруппыАналоговТоваров.НайтиПоНаименованию(ЗначениеКолонки);
		// >> 16.12.2022, Маскаев П.Ю., КРОК, JIRA№ A2105505-805
		// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
		ИначеЕсли ДополнительнаяКолонка.Тип.СодержитТип(Тип("СправочникСсылка.СерииНоменклатуры")) Тогда
			НоваяСтрока[ИмяКолонки] = Справочники.СерииНоменклатуры.НайтиПоНаименованию(ЗначениеКолонки);
		ИначеЕсли ДополнительнаяКолонка.Тип.СодержитТип(Тип("СправочникСсылка.Назначения")) Тогда
			НоваяСтрока[ИмяКолонки] = Справочники.Назначения.НайтиПоНаименованию(ЗначениеКолонки);
		ИначеЕсли ДополнительнаяКолонка.Тип.СодержитТип(Тип("СправочникСсылка.СкладскиеЯчейки")) Тогда
			НоваяСтрока[ИмяКолонки] = КР_НайтиСкладскиеЯчейки(ЗначениеКолонки, ЭтотОбъект["КР_СкладДокумента"]);
		// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860 
		// << 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
		ИначеЕсли ДополнительнаяКолонка.Тип.СодержитТип(Тип("СправочникСсылка.УпаковкиЕдиницыИзмерения")) Тогда
			НоваяСтрока[ИмяКолонки] = Справочники.УпаковкиЕдиницыИзмерения.НайтиПоНаименованию(ЗначениеКолонки);
		// >> 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
		Иначе
...
// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Процедура КР_ИнициализироватьДопНастройки()
	
	ОписаниеТипаБулево = Новый ОписаниеТипов("Булево");
	
	МассивПолейНеВыводить = Новый Массив;
	МассивПолейНеВыводить.Добавить("КР_НеВыводитьПолеНоменклатура");
	МассивПолейНеВыводить.Добавить("КР_НеВыводитьПолеНоменклатурыКод");
	МассивПолейНеВыводить.Добавить("КР_НеВыводитьПолеНоменклатурыАртикул");
	МассивПолейНеВыводить.Добавить("КР_НеВыводитьПолеХарактеристикиНоменклатуры");
	// << 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
	МассивПолейНеВыводить.Добавить("КР_ПереместитьКолонкуЯчейка");
	// >> 24.07.2023 Фомичев А.Е., КРОК  JIRA№A2105505-2035
	Для Каждого Имя Из МассивПолейНеВыводить Цикл
		КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект, Имя, ОписаниеТипаБулево);
		КР_ИнициализироватьНастройкуФормы(Имя);
	КонецЦикла;
	
	// КР_СкладДокумента
	ОписаниеТипаСклад = Новый ОписаниеТипов("СправочникСсылка.Склады");
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект, "КР_СкладДокумента", ОписаниеТипаСклад);
	Если Параметры.Свойство("КР_СкладДокумента") Тогда
		ЭтаФорма["КР_СкладДокумента"] = Параметры.КР_СкладДокумента;
	КонецЕсли;
	
	Элементы.ГруппаЗагружаемыеДанные.Видимость = Не ЭтаФорма["КР_НеВыводитьПолеНоменклатура"];
	
КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Процедура КР_ИнициализироватьНастройкуФормы(Имя)
	
	ЭтаФорма[Имя] = Параметры.Свойство(Имя) И Параметры[Имя];
	
КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Функция КР_НайтиСкладскиеЯчейки(Наименование, Склад)
	
	МетаданныеСкладскиеЯчейки = Метаданные.Справочники.СкладскиеЯчейки;
	...
	Ячейка = КР_ОбщегоНазначениеСервер.ПоискСсылки(МетаданныеСкладскиеЯчейки, ПараметрыПоиска, МассивОшибок);
