...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
...
Процедура ЗаполнитьНачальныеНастройки()
	...
	ИначеЕсли ТипЗаказа = "ЗаказПоставщику" ИЛИ ТипЗаказа = "ЗаказПереработчику" ИЛИ ТипЗаказа = "ЗаказПереработчику2_5" Тогда
...
Процедура ЗаполнитьОтклоненияПоЗаказамНаПеремещение()
	...
	|	СУММА(Остатки.Заказано),
	// << 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	|	Остатки.Заказ.СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки КАК КР_АдресныйСкладБезордернойОтгрузки,
	|	СУММА(Остатки.КР_СобраноНеотгружено) КАК КР_СобраноНеотгружено,
	// >> 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	|	СУММА(Остатки.КВыполнению)
	|ПОМЕСТИТЬ ВтОстатки
	|ИЗ (ВЫБРАТЬ
	...
	|		0                                         КАК Заказано,
	// << 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	|		ОстаткиКОтгрузке.СобраноКонечныйОстаток   КАК КР_СобраноНеотгружено,
	// >> 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	|		0                                         КАК КВыполнению
	|	ИЗ
	|		РегистрНакопления.ТоварыКОтгрузке.ОстаткиИОбороты(&НачПериодКОтгрузке,&КонПериодКОтгрузке,,,
	...
	|	Остатки.Серия,
	// << 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	|	Остатки.КР_АдресныйСкладБезордернойОтгрузки КАК КР_АдресныйСкладБезордернойОтгрузки,
	|	ВЫБОР КОГДА Остатки.КР_АдресныйСкладБезордернойОтгрузки
	|		ТОГДА Остатки.КР_СобраноНеотгружено
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК КР_СобраноНеотгружено, 
	|	ВЫБОР КОГДА Остатки.КР_АдресныйСкладБезордернойОтгрузки 
	|		И  // Ниже типовой блок запроса (см. далее по тексту)
	|			ВЫБОР КОГДА Остатки.КОформлениюОстаток >= 0 И Остатки.КОформлениюОстаток < Остатки.КОтгрузкеОстаток
	|				ИЛИ Остатки.КОформлениюОстаток < 0
	|				ИЛИ НЕ Остатки.ОрдернаяСхемаПриОтгрузке ТОГДА
	|				Остатки.КОформлениюОстаток
	|			ИНАЧЕ
	|				Остатки.КОтгрузкеОстаток
	|			КОНЕЦ + Остатки.КВыполнению // Конец типового блока   
	|		> Остатки.Заказано - Остатки.КР_СобраноНеотгружено 
	|		ТОГДА Остатки.Заказано - Остатки.КР_СобраноНеотгружено
	|	ИНАЧЕ
	// >> 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	|	ВЫБОР КОГДА Остатки.КОформлениюОстаток >= 0 И Остатки.КОформлениюОстаток < Остатки.КОтгрузкеОстаток
	|			ИЛИ Остатки.КОформлениюОстаток < 0
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	// ТоварыКОтмене.КР_АдресныйСкладБезордернойОтгрузки
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_АдресныйСкладБезордернойОтгрузки", Новый ОписаниеТипов("Булево"), "ТоварыКОтмене"); 
	
	// ТоварыКОтмене.КР_СобраноНеотгружено
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_СобраноНеотгружено", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 3)), "ТоварыКОтмене",
		НСтр("ru = 'Собрано, не отгружено'")); 
		
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		РеквизитФормы, , Элементы.ТоварыКОтменеОтгрузкаКоличество);
		
	ЭлементФормы.ТолькоПросмотр	= Истина; 
	ЭлементФормы.Ширина = 10;
	
КонецПроцедуры // >> 29.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
...
Процедура КР_УстановитьУсловноеОформление() 
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("ТоварыКОтменеКР_СобраноНеотгружено");
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТоварыКОтмене.КР_АдресныйСкладБезордернойОтгрузки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
