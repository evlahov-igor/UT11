...
Процедура ОбновитьОбластьПросмотра()
	...
	Если ПоказыватьОбластьПросмотра Тогда 
		
		Если ЗначениеЗаполнено(ТекущийЭлектронныйДокумент) Тогда
			...
			ТекущийДокументУчета = ДанныеПросмотра.ДокументУчетаСсылка;
...
Функция ДанныеОбластиПросмотра(ТекущийЭлектронныйДокумент, УникальныйИдентификатор, 
	...
	Если ДанныеЭлектронногоДокумента.ДанныеЭлементовСхемы.Количество() <> 0 Тогда	
		...
		Если ЭлементСхемыРегламента.ТипДокумента = Перечисления.ТипыДокументовЭДО.Прочее Тогда
			
			СтруктураВозврат.ФайлСсылка = ЭлементСхемыРегламента.ПрисоединенныйФайл;
			СтруктураВозврат.ФайлПредставление = Строка(ЭлементСхемыРегламента.ПрисоединенныйФайл);
			
			Расширение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементСхемыРегламента.ПрисоединенныйФайл, 
				"Расширение");
			СтруктураВозврат.ФайлНомерИконки = 
				РаботаСФайламиСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(Расширение);
			
			Если Не ЕстьОтборПоНепрочтенным Тогда
				ПрочтениеДолжноБытьУстановлено = Не РаботаСПрочтениямиВызовСервера.ПолучитьСвойствоПрочтен(ТекущийЭлектронныйДокумент);
			КонецЕсли;
			...
			ЗначенияПараметров.Вставить("Дата", Формат(ПараметрыДокумента.ДатаДокумента, "ДФ=dd.MM.yyyy"));
		...
		Иначе
			...
			Если ЭлементСхемыРегламента.ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату Тогда
...
Функция ПоляРаздела(Знач Раздел, Знач ПакетноеОтображение)
	...
	Если НЕ ОбщегоНазначения.ЭтоМобильныйКлиент() И  ПакетноеОтображение Тогда
		...
		Если РазделыБезДатыПакета.Найти(Раздел) = Неопределено Тогда
			ПоляРаздела = СтрЗаменить(ПоляРаздела, "Дата", "ДатаПакета");
		КонецЕсли;
...
Процедура ПоказатьРасширенныйПодборДокументовУчета()
	...
	ПараметрыФормы.Вставить("ДокументРаспознан", РаспознатьДокумент(ТекущиеДанные.ЭлектронныйДокумент));
...
Процедура ОформитьИнтерфейсДляМобильногоКлиентаНаСервере()
	...
	ВидимыеЭлементы = "ГруппаОсновная, ОтправитьИПолучитьЭД, ГруппаПраво, ГруппаКоманды, ВыбратьДействиеМК,
		...
		|ОзнакомитьсяЭДЭлектронныДокумент, ОбработкаЭлектронныеДокументыАрхивЭДООткрыть";
...
&НаКлиенте
Процедура РазвернутьУзлыДерева(Форма, ИмяЭлементаФормы,
		ИдентификаторСтрокиДерева = "*", РазвернутьСПодчиненными = Ложь)
