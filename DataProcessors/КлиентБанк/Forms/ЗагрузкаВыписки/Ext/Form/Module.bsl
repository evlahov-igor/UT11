	...
	Для каждого ВыделеннаяСтрока Из Элементы.БанковскиеСчета.ВыделенныеСтроки Цикл
		...
		Если Не ДанныеСтроки.Загружен И ТипЗнч(ДанныеСтроки.НастройкаОбмена) = Тип("Строка") Тогда
			...
			Если ДанныеСтроки.ФорматОбмена = "1CClientBankExchange" Тогда
				ТекстСообщения = "";
				СтрокиВыписки = ПрочитатьФайлВыпискиClientBank(
				...
				Если СтрокиВыписки <> Неопределено Тогда
					...
					ДанныеСтроки.Выписка.Вставить("СтрокиВыписки", СтрокиВыписки);
					ДанныеСтроки.Выписка.Вставить("ТекстСообщения", "");
				КонецЕсли;
			Иначе
...
Функция ВывестиОтчетНаСервере(ИндексыСтрок)
	...
	Для каждого ИндексСтроки Из ИндексыСтрок Цикл
		...
		Счет.АдресХранилищаДокументов = ПоместитьВоВременноеХранилище(Объект.ДокументыКЗагрузке.Выгрузить(СтрокиДокументовКЗагрузке));
	КонецЦикла;
...
Процедура ПрочитатьДанныеВыписки1CClientBankExchange(Счет)
	...
	ТекстСообщения = "";
	СтрокиВыписки = ПрочитатьФайлВыпискиClientBank(Счет.НастройкаОбмена, Счет.Кодировка, ТекстСообщения);
