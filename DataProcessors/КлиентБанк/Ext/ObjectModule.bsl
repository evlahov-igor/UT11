...
Процедура ПроверитьДокументыКВыгрузке()
	...
	Для каждого ДокументКВыгрузке Из ДокументыКВыгрузке Цикл
		...
		ИначеЕсли Счет.ФорматОбмена = "ISO20022" Тогда
...
Процедура ЗаполнитьИнформациюОКонтрагентах(ТаблицаПлатежей)
	...
	ДанныеКонтрагентов = ДенежныеСредстваСервер.СведенияОКонтрагентахПоДокументам(МассивДокументов);
	
	Для Каждого СтрокаТаблицы Из ТаблицаПлатежей Цикл
		
		Обработки.КлиентБанк.ЗаполнитьДанныеКонтрагента(ДанныеКонтрагентов,
