...
Процедура ЗаполнитьСостоянияЭтапов(ПараметрыЗапуска) Экспорт
	...
	ИдентификаторГруппыФормированиеЗатрат = ЗакрытиеМесяцаСервер.ИдентификаторГруппыФормированиеЗатрат();
	...
	Для Каждого ЭлементКоллекции Из СтрокиЭтапов Цикл
		...
		Если ПараметрыЗапуска.ВыполняетсяПредварительноеЗакрытиеМесяца
		 И (НЕ ТекущаяСтрока.ВыполняетсяПриПредварительномЗакрытииМесяца
		 ИЛИ ТекущаяСтрока.Родитель = ИдентификаторГруппыФормированиеЗатрат) Тогда
	 ...
	 И ПараметрыОбработчика.Свойство("МеждународныйУчет") Тогда
		...
		Если НЕ ЭтапДоступен Тогда
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеОтключено(
				ПараметрыОбработчика,
				НСтр("ru='Данный вид учета недоступен.'", ОбщегоНазначения.КодОсновногоЯзыка()));
			Возврат;
		 ...
		 ИЛИ СтрокаПредшествующегоЭтапа.Состояние = Перечисления.СостоянияОперацийЗакрытияМесяца.Неопределено Тогда
			...
			ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не выполнена предшествующая операция ""%1""'", ОбщегоНазначения.КодОсновногоЯзыка()),
					СтрокаПредшествующегоЭтапа.Наименование),
					,
					,
...
Функция ШаблонТекстаПроблемыОшибкаВыполненияКода() Экспорт
	...
	Возврат НСтр("ru='При выполнении расчета периода %2 по организации ""%1"" произошла исключительная ситуация'", ОбщегоНазначения.КодОсновногоЯзыка());
