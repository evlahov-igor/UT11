...
Процедура ЗаполнитьСписокСкладов()
	...
	|	ОстатокНаСкладе.Склад.Наименование КАК НаименованиеСклада
	|ИЗ
	|	РегистрСведений.РаспределениеЗапасов КАК ОстатокНаСкладе
	|ГДЕ
	|	ОстатокНаСкладе.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе)
...
Функция РезультатЗапросаОстатковПоВыводуИзАссортимента(ИмяТЧ)
	...
	Запрос=Новый Запрос(
		...
		|	СУММА(Остатки.ВНаличии) КАК КоличествоУпаковок
		|ИЗ
