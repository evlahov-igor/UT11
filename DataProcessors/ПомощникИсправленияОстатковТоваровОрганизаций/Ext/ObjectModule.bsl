...
Процедура ЗаполнитьМесяцИсправления() Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		// << 28.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2230
		// Ошибка 1С  
		// Следствие исправлений по рекомендациям 1С https://bugboard.v8.1c.ru/error/000129643  
		//
		//|	ПЕРВЫЕ 1
		...
		// Ошибка 1С  
		// Следствие исправлений по рекомендациям 1С https://bugboard.v8.1c.ru/error/000129643  
		// ВыборкаДетальныеЗаписи в типовом коде ниже переопроеделяется дважды. Перебор не возможен
		//ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		//Если ВыборкаДетальныеЗаписи.Следующий() Тогда   
		//	МесяцИсправления = ВыборкаДетальныеЗаписи.Период; 
		//
		ВыборкаПериоды = РезультатЗапроса.Выбрать();
		Пока ВыборкаПериоды.Следующий() Цикл  
			
			МесяцИсправления = ВыборкаПериоды.Период; 
			// >> 28.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2230
			...
			// Ошибка 1С  
			// Следствие исправлений по рекомендациям 1С https://bugboard.v8.1c.ru/error/000129643  
			Если КоличествоПозицийОтрицательныйОстатокБыло
				Или КоличествоПозицийРазвернутоеСальдоБыло Тогда 
				Прервать;
			КонецЕсли;	 
			
			// Очищаем так как месяц не актуален. 
			// Далее типовой код смотрит именно на ЗначениеЗаполнено(МесяцИсправления) что актуализировать "количества"
			МесяцИсправления = Неопределено;  
...
Функция ЗапросДетальныеОстаткиНаКонецМесяцаИсправления()
	...
	// << 05.06.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1768
