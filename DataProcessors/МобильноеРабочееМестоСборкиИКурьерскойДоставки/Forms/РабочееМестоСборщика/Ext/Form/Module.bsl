...
Процедура ЗакончитьСборку(Команда)
	...
	Если КСборке.Количество() > 0 Тогда
		Вопрос = НСтр("ru = 'Собраны не все товары. Завершить сборку заказа?';" + " en = 'Do you want to continue?'");
	ИначеЕсли Замены.Количество() > 0 Тогда
		Вопрос = НСтр("ru = 'Отработаны не все замены. Завершить сборку заказа?';" + " en = 'Do you want to continue?'");
	ИначеЕсли ВернутьНаПолку.Количество() > 0 Тогда
		Вопрос = НСтр("ru = 'Не все товары возвращены на полку. Завершить сборку заказа?';" + " en = 'Do you want to continue?'");
	Иначе
		Вопрос = НСтр("ru = 'Завершить сборку заказа?';" + " en = 'Do you want to continue?'");
	КонецЕсли;
...
Процедура ЗакончитьСборкуНаСервере()
	...
	СтруктураЗаказа.Вставить("Собрано", Собрано.Выгрузить());
	СтруктураЗаказа.Вставить("ТовараНет", ТовараНет.Выгрузить());
	
	Обработки.МобильноеРабочееМестоСборкиИКурьерскойДоставки.УстановитьСтатусРаспоряженияСобран(Заказ, СтруктураЗаказа);
