...
//++ Локализация
	Если Не ЗначениеЗаполнено(СсылкаНаОплату) Тогда
		...
		СсылкаНаОплату = ИнтеграцияСЯндексКассой.ПлатежнаяСсылка(Распоряжение);
		
		QRКод = Новый Картинка(ГенерацияШтрихкодаВызовСервера.ДанныеQRКода(СсылкаНаОплату, 0, 190));
