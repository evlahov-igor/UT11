...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ ЭтоЗаказ И НЕ ЗначениеЗаполнено(ГрафикОплаты) Тогда
		Если ТабличнаяЧасть.Количество() > 0 И ЗначениеЗаполнено(ТабличнаяЧасть[0].Заказ) Тогда
			ГрафикОплаты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТабличнаяЧасть[0].Заказ, "ГрафикОплаты");
		Иначе
			ГрафикОплаты = ГрафикОплатыСоглашения;
		КонецЕсли;
	КонецЕсли;;
		...
		ИначеЕсли УпрощенныйРежим Тогда
			...
			Если УпрощенныйРежим И СуммаОплатыПоДокументу + СуммаЗалогаПоДокументу > 0 И НЕ ПропуститьПроверку Тогда
				...
				ИначеЕсли ДатаПредоплата < ДатаЗаказа И СуммаПлатежаПредоплата + СуммаЗалогаЗаТаруПредоплата > 0 И ЭтоЗаказ Тогда
					ТекстОшибки = НСтр("ru='Дата платежа до отгрузки должна быть не больше даты документа %ДатаЗаказа%'");
				...
				ИначеЕсли ДатаКредит < ДатаЗаказа И СуммаПлатежаКредит + СуммаЗалогаЗаТаруКредит > 0 И ЭтоЗаказ Тогда
					ТекстОшибки = НСтр("ru='Дата платежа до отгрузки должна быть не больше даты отгрузки %ДатаКредит%'");
...
Процедура НастроитьЭлементыФормыПриИзмененииПорядкаРасчетов()
	...
	Элементы.ЭтапыОплатыСверхЗаказа.Видимость = ПоЗаказам И ИспользоватьРасширенныеВозможностиЗаказаКлиента;
...
Функция ПреобразоватьДанныеУпрощенногоРежимаВТаблицуЭтапов()
	...
	Если ЭтапСодержитДанные("Предоплата") И СуммаПлатежаПредоплата + СуммаЗалогаЗаТаруПредоплата > 0 Тогда
		...
		СтрокаПредоплаты.ВариантОплаты           = Перечисления.ВариантыКонтроляОплатыКлиентом.ПредоплатаДоОтгрузки;
	...
	Если ЭтапСодержитДанные("Кредит") И СуммаПлатежаКредит + СуммаЗалогаЗаТаруКредит > 0 Тогда
		...
		СтрокаКредита.ВариантОплаты           = Перечисления.ВариантыКонтроляОплатыКлиентом.КредитПослеОтгрузки;
