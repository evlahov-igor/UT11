...
Процедура ЗаполнитьТаблицуТоваровДляОтчетаОПродажах()
	...
	КомиссионнаяТорговляСервер.ЗапросПоТоварамКОформлениюОтчетовКомитентуЗаПериод(СтруктураПараметров,
																					МенеджерВременныхТаблиц);
	КомиссионнаяТорговляСервер.ЗапросПоТоварамКОформлениюОтчетовПринципалуЗаПериод(СтруктураПараметров,
																					МенеджерВременныхТаблиц);
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаТовары.Количество, 0) > 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ									КАК Выбран,
	...
	|		ТоварыКОформлению.Количество)		КАК КоличествоУпаковокВыбран,
	...
	|				ИЛИ ТоварыКОформлению.СчетФактура = ТаблицаТовары.СчетФактураВыставленный)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ТаблицаТовары.Количество,
	|		УслугиКОформлению.Количество)		КАК КоличествоУпаковокВыбран,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаТовары КАК ТаблицаТовары
	|		ПО УслугиКОформлению.Номенклатура = ТаблицаТовары.Номенклатура
	...
	|			И (ТаблицаТовары.СчетФактураВыставленный В (&НезаполненныеЗначенияСчетФактураВыставленный)
	|				ИЛИ УслугиКОформлению.СчетФактура = ТаблицаТовары.СчетФактураВыставленный)
	|
	|УПОРЯДОЧИТЬ ПО
