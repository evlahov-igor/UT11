...
Процедура ПроверитьЗаполнениеРеквизитов(Отказ)
	...
	Если ИспользоватьНомерСкважины И Не ЗначениеЗаполнено(НомерСкважины) Тогда
		ТекстСообщения = СтрШаблон(ШаблонСообщения, "Номер скважины");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"НомерСкважины",,Отказ);
	КонецЕсли;
