...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если СравниватьСПредыдущимПериодом Тогда
		...
		ПредыдущийПериод = ПредыдущийПериод(НастройкаПериодЗначение.ДатаНачала, НастройкаПериодЗначение.ДатаОкончания);
...
Функция ТекстЗапросаТаблицаЦенНоменклатуры()
	...
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		...
		|		ПО ЦеныНоменклатуры.ХарактеристикаЦО = ВтОтборНоменклатуры.ХарактеристикаЦО
		|
		|УПОРЯДОЧИТЬ ПО
