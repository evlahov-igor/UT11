...
Процедура ВывестиСегменты()
	...
	Если ОтображатьНадписиУСтрелок = "Проценты" Или ОтображатьНадписиУСтрелок = "ПроцентыЧисла" Тогда
		...
		Если (РезультатВыполнения.АктивныеНовыеНачало.Количество()+РезультатВыполнения.ПерваяПокупка.Количество()) <> 0 Тогда  
			ПроцентПереходАП = Формат(Окр(100*РезультатВыполнения.ПереходАП/(РезультатВыполнения.АктивныеНовыеНачало.Количество()+РезультатВыполнения.ПерваяПокупка.Количество())),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
			ПроцентПереходЗН = Формат(Окр(100*РезультатВыполнения.ПереходЗН/(РезультатВыполнения.АктивныеНовыеНачало.Количество()+РезультатВыполнения.ПерваяПокупка.Количество())),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
		Иначе
			ПроцентПереходАП = "0%";
			ПроцентПереходЗН = "0%";
		КонецЕсли;
		...
		Если (РезультатВыполнения.ЗасыпающиеНовыеНачало.Количество()+РезультатВыполнения.ПереходЗН) <> 0 Тогда  	
			ПроцентПереходСН = Формат(Окр(100*РезультатВыполнения.ПереходСН/(РезультатВыполнения.ЗасыпающиеНовыеНачало.Количество()+РезультатВыполнения.ПереходЗН)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");	
			ПроцентПереходЗНАП = Формат(Окр(100*РезультатВыполнения.ПереходЗНАП/(РезультатВыполнения.ЗасыпающиеНовыеНачало.Количество()+РезультатВыполнения.ПереходЗН)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");	
		Иначе
			ПроцентПереходСН = "0%";
		...
		Если (РезультатВыполнения.СпящиеНовыеНачало.Количество()+РезультатВыполнения.ПереходСН)<> 0 Тогда 
			ПроцентПереходСНАП = Формат(Окр(100*РезультатВыполнения.ПереходСНАП/(РезультатВыполнения.СпящиеНовыеНачало.Количество()+РезультатВыполнения.ПереходСН)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
		Иначе
		...
		Если (РезультатВыполнения.АктивныеПерспективныеНачало.Количество()+РезультатВыполнения.ПереходАП+РезультатВыполнения.ПереходЗНАП+РезультатВыполнения.ПереходСНАП+РезультатВыполнения.ПереходЗПАП) <> 0 Тогда
			ПроцентПереходАВ = Формат(Окр(100*РезультатВыполнения.ПереходАВ/(РезультатВыполнения.АктивныеПерспективныеНачало.Количество()
				+ РезультатВыполнения.ПереходАП+РезультатВыполнения.ПереходЗНАП+РезультатВыполнения.ПереходСНАП+РезультатВыполнения.ПереходЗПАП)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");	
		...
		Если (РезультатВыполнения.ЗасыпающиеПерспективныеНачало.Количество()+РезультатВыполнения.ПереходЗП+РезультатВыполнения.ПереходСПАП) <> 0 Тогда
			ПроцентПереходСП = Формат(Окр(100*РезультатВыполнения.ПереходСП/(РезультатВыполнения.ЗасыпающиеПерспективныеНачало.Количество()+РезультатВыполнения.ПереходЗП+РезультатВыполнения.ПереходСПАП)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
			ПроцентПереходЗПАП = Формат(Окр(100*РезультатВыполнения.ПереходЗПАП/(РезультатВыполнения.ЗасыпающиеПерспективныеНачало.Количество()+РезультатВыполнения.ПереходЗП+РезультатВыполнения.ПереходСПАП)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
			ПроцентПереходЗПАВ = Формат(Окр(100*РезультатВыполнения.ПереходЗПАВ/(РезультатВыполнения.ЗасыпающиеПерспективныеНачало.Количество()+РезультатВыполнения.ПереходЗП+РезультатВыполнения.ПереходСПАП)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
		Иначе
			ПроцентПереходСП = "0%";
		...
		Если (РезультатВыполнения.СпящиеПерспективныеСписокНачало.Количество()+РезультатВыполнения.ПереходСП) <> 0 Тогда 
			ПроцентПереходСПАП = Формат(Окр(100*РезультатВыполнения.ПереходСПАП/(РезультатВыполнения.СпящиеПерспективныеСписокНачало.Количество()+РезультатВыполнения.ПереходСП)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
			ПроцентПереходСПАВ = Формат(Окр(100*РезультатВыполнения.ПереходСПАВ/(РезультатВыполнения.СпящиеПерспективныеСписокНачало.Количество()+РезультатВыполнения.ПереходСП)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");;
		Иначе
		...
		Если (РезультатВыполнения.АктивныеВерныеНачало.Количество()+РезультатВыполнения.ПереходАВ+РезультатВыполнения.ПереходЗПАВ+РезультатВыполнения.ПереходСПАВ+РезультатВыполнения.ПереходЗВАВ) <> 0 Тогда 
			ПроцентПереходЗВ = Формат(Окр(100*РезультатВыполнения.ПереходЗВ/(РезультатВыполнения.АктивныеВерныеНачало.Количество()
				+ РезультатВыполнения.ПереходАВ+РезультатВыполнения.ПереходЗПАВ+РезультатВыполнения.ПереходСПАВ+РезультатВыполнения.ПереходЗВАВ)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");	
		Иначе
			ПроцентПереходЗВ = "0%";	
		КонецЕсли;
		...
		Если (РезультатВыполнения.ЗасыпающиеВерныеНачало.Количество()+РезультатВыполнения.ПереходЗВ+РезультатВыполнения.ПереходСВАВ) <> 0 Тогда  
			ПроцентПереходСВ = Формат(Окр(100*РезультатВыполнения.ПереходСВ/(РезультатВыполнения.ЗасыпающиеВерныеНачало.Количество()
				+ РезультатВыполнения.ПереходЗВ+РезультатВыполнения.ПереходСВАВ)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
		...
		Если (РезультатВыполнения.СпящиеВерныеСписокНачало.Количество()+РезультатВыполнения.ПереходСВ) <> 0 Тогда	
			ПроцентПереходСВАВ = Формат(Окр(100*РезультатВыполнения.ПереходСВАВ/(РезультатВыполнения.СпящиеВерныеСписокНачало.Количество()+РезультатВыполнения.ПереходСВ)),"ЧДЦ=0; ЧН=0%; ЧФ=Ч%");
		Иначе
