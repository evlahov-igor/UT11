...
Функция ТекстЗапросаВнутреннееПотребление(Запрос,СтруктураФормированияОтчета)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки ССЫЛКА Документ.ВнутреннееПотреблениеТоваров И &ТекстУсловияОтборДокументОтгрузки) КАК ТоварыКОтгрузкеОстатки
