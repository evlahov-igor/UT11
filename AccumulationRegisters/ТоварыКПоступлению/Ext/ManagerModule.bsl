...
Функция ТекстЗапросаОсталосьОформитьПоОрдерам(ДополнитьСериямиИзРегистратора = Ложь, ОтборПоИзмерениям = Неопределено) Экспорт
	...
	Если ДополнитьСериямиИзРегистратора Тогда
		...
		|	Таблица.Назначение,
		|	Таблица.Серия    
		// << 23.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-702
		// Аналогично запросу выще, только используется таблица Документ.ОтборРазмещениеТоваров.ТоварыРазмещение
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	Таблица.Серия,
		|	Сумма(Таблица.Количество) КАК Количество
		|ИЗ Документ.ОтборРазмещениеТоваров.ТоварыРазмещение КАК Таблица
		|ГДЕ
		|	Таблица.Ссылка В (Выбрать Т.Регистратор ИЗ ВтДанныеРегистра КАК Т)
		|СГРУППИРОВАТЬ ПО
		|	Таблица.Ссылка,
