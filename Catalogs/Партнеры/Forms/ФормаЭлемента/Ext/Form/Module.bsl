...
Процедура КР_ПриСозданииНаСервереДополнительно()
	...
	// злЭлектроннаяКоммерция
	СвойстваГруппа = Элементы.ГруппаОтношенияДополнительно;
	злЭлектроннаяКоммерцияЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злЭлектроннаяКоммерция", СвойстваГруппа);
	злЭлектроннаяКоммерцияЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// --> Евлахов Игорь Николаевич (Начало) 20.08.2024
	// Задача #4552
	// злНашиПодрядчики
	ДобавляемыйРеквизит = "Объект.злПодрядчик";
	СвойстваГруппа = Элементы.ГруппаОтношенияДополнительно;
	злПодрядчикЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, ДобавляемыйРеквизит, СвойстваГруппа);
	злПодрядчикЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
