...
Процедура ПриСозданииИЧтенииНаСервере()
	...
	// << 14.11.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-615
...
Процедура УстановитьВидимостьЦенообразования()
	...
	Элементы.ГруппаЦенообразование.Видимость = Объект.Клиент;
...
Процедура КР_ПриЧтенииСозданииНаСервереДополнительно()
	...
	КР_КодПоставщикаZollaРеквизит = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_КодПоставщикаZolla", ОбщегоНазначения.ОписаниеТипаСтрока(3), , НСтр("ru = 'Код поставщика'"));
	
	КР_КодПоставщикаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КодПоставщикаZollaРеквизит, Элементы.ГруппаНаименование);
	КР_КодПоставщикаЭлемент.ТолькоПросмотр = Истина;
	
	// ПриЧтенииСозданииНаСервере() вызывается позже заполнения реквизитов КонтрагентПартнера
	// Заполняем самостоятельно
	ЭтотОбъект.КР_КодПоставщикаZolla = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		КонтрагентПартнера, "КР_КодПоставщикаZolla");
	
КонецПроцедуры // >> 14.11.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-615
