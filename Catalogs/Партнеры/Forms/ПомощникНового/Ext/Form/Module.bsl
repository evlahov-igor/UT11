...
Функция ПодзапросПоФИО(Запрос)
	...
	Для Инд = 1 По МассивФИО.Количество() Цикл
		...
		|ГДЕ
		|	КонтактныеЛицаПартнеров.Наименование ПОДОБНО &ФИО_ И &УсловиеПоНеизвестномуПартнеру
		|";
		...
		ПодзапросПоКонтактнымЛицам = СтрЗаменить(ПодзапросПоКонтактнымЛицам, "&ФИО_", "&ФИО_" + Строка(Инд));
...
Процедура ПриИзмененииИспользуетсяБанкДляРасчетов(ЭтоГоловнойКонтрагент=Ложь)
	...
	Иначе
		...
		Для каждого КлючИЗначение Из Новый Структура(РеквизитыБанкаДляРасчетов(ЭтоГоловнойКонтрагент)) Цикл
...
Функция ЗаполнитьРеквизитыБанкаПоБИК(ТипБанка, Знач БИК = "", ЭтоГоловнойКонтрагент=Ложь)
	...
	Если ТипБанка = "Банк" Тогда
		СтруктураРеквизитов = Новый Структура(РеквизитыБанка(ЭтоГоловнойКонтрагент));
	Иначе
		СтруктураРеквизитов = Новый Структура(РеквизитыБанкаДляРасчетов(ЭтоГоловнойКонтрагент));
	КонецЕсли;
...
Процедура НомерСчетаПриИзмененииКлиент(ЭтоГоловнойКонтрагент = Ложь)
	...
	Элементы["ГруппаОшибкаВНомереСчета" + Постфикс].Видимость = Ложь;
	ЭтотОбъект["НомерСчета" + Постфикс] = СтрЗаменить(
...
Функция РасчетныйСчетЗаполненНеправильно()
	...
	Если УказатьБанковскийСчетКонтрагента Тогда
		...
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать валюту денежных средств'"), ИспользуютсяТолькоПартнеры, "ВалютаДенежныхСредств");
			Отказ = Истина;
		КонецЕсли;
...
Процедура СоздатьПартнера(СписокСозданных, ТребуетсяПереходНаСтраницуСовпадений)
	...
	ПартнерыИКонтрагентыЛокализация.ПередСозданиемПартнераВФормеПомощникаНового(ЭтотОбъект, Отказ);
