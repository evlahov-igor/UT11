...
Функция ПараметрыЗаполненияВидаДеятельностиНДС(Объект) Экспорт
	...
	ИначеЕсли Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком
		Или Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком2_5 Тогда
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем 
		...
		ИЛИ ТипДоговора = Перечисления.ТипыДоговоров.СКомиссионером И КомиссионныеПродажи25 Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|Заполняет реквизит ""Порядок оформления списания в производство товаров, принятых на хранение"" в договорах приема на ответственное хранение.
	|Заполняет реквизит Подразделение в давальческих договорах.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос;
	ТекстЗапросаОбъектыРасчетов =
	"ВЫБРАТЬ
	|	ДоговорыКонтрагентов.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|ГДЕ
	|	(ДоговорыКонтрагентов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|				И НЕ ДоговорыКонтрагентов.УдалитьСтавкаНДС = &ПустаяСтавкаНДС
	|			ИЛИ НЕ ДоговорыКонтрагентов.ОплатаВВалюте
	|					И (ДоговорыКонтрагентов.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|						ИЛИ ДоговорыКонтрагентов.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	...
	|						ГДЕ
	|							РасчетыСКлиентамиПоДокументам.УдалитьЗаказКлиента = ДоговорыКонтрагентов.Ссылка)
	|					ИЛИ ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	|						ИЗ
	...
	|						ГДЕ
	|							РасчетыСПоставщикамиПоДокументам.УдалитьЗаказПоставщику = ДоговорыКонтрагентов.Ссылка))
	|				И НЕ ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	|						ИЗ
	...
	|						ГДЕ
	|							Сверка.УдалитьОбъектРасчетов = ДоговорыКонтрагентов.Ссылка
	|							И (Сверка.Ссылка.Контрагент <> ДоговорыКонтрагентов.Контрагент
	|								ИЛИ Сверка.Партнер <> ДоговорыКонтрагентов.Партнер
	|								ИЛИ Сверка.Ссылка.Организация <> ДоговорыКонтрагентов.Организация
	|								ИЛИ Сверка.Валюта <> ДоговорыКонтрагентов.ВалютаВзаиморасчетов
	|								ИЛИ Сверка.ТипРасчетов <> ВЫБОР
	|									КОГДА ДоговорыКонтрагентов.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПокупателем)
	|											ИЛИ ДоговорыКонтрагентов.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СКомитентомНаЗакупку)
	|											ИЛИ ДоговорыКонтрагентов.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СКомиссионером)
	|											ИЛИ ДоговорыКонтрагентов.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СХранителем)
	...
	|									ГДЕ
	|										ОбъектыРасчетов.Объект = ДоговорыКонтрагентов.Ссылка
	|										И ОбъектыРасчетов.Контрагент = Сверка.Ссылка.Контрагент
	|										И ОбъектыРасчетов.Партнер = Сверка.Партнер
	|										И ОбъектыРасчетов.ТипРасчетов = Сверка.ТипРасчетов
	|										И ОбъектыРасчетов.ВалютаВзаиморасчетов = Сверка.Валюта
	|										И ОбъектыРасчетов.Организация = Сверка.Ссылка.Организация)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ГДЕ
	|				РасчетыСПоставщиками.УдалитьЗаказПоставщику = ДоговорыКонтрагентов.Ссылка
	|				И РасчетыСПоставщиками.Валюта <> ДоговорыКонтрагентов.ВалютаВзаиморасчетов
	|				И НЕ ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	|						ИЗ
	...
	|						ГДЕ
	|							ОбъектыРасчетов.Объект = ДоговорыКонтрагентов.Ссылка
	|							И ОбъектыРасчетов.Организация = ДоговорыКонтрагентов.Организация
	|							И ОбъектыРасчетов.Контрагент = ДоговорыКонтрагентов.Контрагент
	|							И ОбъектыРасчетов.Партнер = ДоговорыКонтрагентов.Партнер
	...
	|								ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|							КОНЕЦ
	|							И ОбъектыРасчетов.ВалютаВзаиморасчетов = РасчетыСПоставщиками.Валюта))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ГДЕ
	|				РасчетыСКлиентами.УдалитьЗаказКлиента = ДоговорыКонтрагентов.Ссылка
	|				И РасчетыСКлиентами.Валюта <> ДоговорыКонтрагентов.ВалютаВзаиморасчетов
	|				И НЕ ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	|						ИЗ
	...
	|								ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСПоставщиком)
	|							КОНЕЦ
	|							И ОбъектыРасчетов.ВалютаВзаиморасчетов = РасчетыСКлиентами.Валюта))
	|";
	...
	Тексты = Новый Массив;
	Тексты.Добавить(ТекстЗапросаОбъектыРасчетов);
	Тексты.Добавить(ТекстЗапросаДоговораСДвижениямиПоДочернимПартнерам());
	ТекстЗапроса = СтрСоединить(Тексты, Объединить);
	Запрос.Текст = ТекстЗапроса;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	#Область ТекстЗапросаДляОбработки
	Запрос = Новый Запрос;
	...
	|				Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|			ГДЕ
	|				ИспользованиеВРасчетныхРегистрах.Организация = ОбъектыРасчетов.Организация
	|				И ИспользованиеВРасчетныхРегистрах.Партнер = ОбъектыРасчетов.Партнер
	|				И ИспользованиеВРасчетныхРегистрах.Контрагент = ОбъектыРасчетов.Контрагент
	|				И ИспользованиеВРасчетныхРегистрах.ТипРасчетов = ОбъектыРасчетов.ТипРасчетов
	|				И ИспользованиеВРасчетныхРегистрах.ВалютаВзаиморасчетов = ОбъектыРасчетов.ВалютаВзаиморасчетов
	|				И ИспользованиеВРасчетныхРегистрах.ОбъектРасчетов = ОбъектыРасчетов.Объект))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИспользованиеВТЧ.ОбъектРасчетов КАК ОбъектРасчетов
	|ИЗ ВТИспользованиеВТЧ КАК ИспользованиеВТЧ
	|	ГДЕ	НЕ (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|			ГДЕ
	|				ИспользованиеВТЧ.Организация = ОбъектыРасчетов.Организация
	|				И ИспользованиеВТЧ.Партнер = ОбъектыРасчетов.Партнер
	|				И ИспользованиеВТЧ.Контрагент = ОбъектыРасчетов.Контрагент
	|				И ИспользованиеВТЧ.ТипРасчетов = ОбъектыРасчетов.ТипРасчетов
	|				И ИспользованиеВТЧ.ВалютаВзаиморасчетов = ОбъектыРасчетов.ВалютаВзаиморасчетов
	|				И ИспользованиеВТЧ.ОбъектРасчетов = ОбъектыРасчетов.Объект))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДляОбработки.Ссылка КАК ОбъектРасчетов
	|ИЗ
	|	ВТДляОбработки КАК ДляОбработки
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК Договоры
	|	ПО ДляОбработки.Ссылка = Договоры.Ссылка
	|ГДЕ
	|	Договоры.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
	|				И НЕ ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	|						ИЗ
	|							Справочник.ОбъектыРасчетов КАК ОбъектыРасчетов
	|						ГДЕ
	|							ОбъектыРасчетов.Объект = ДляОбработки.Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ВЫБОР
	|		КОГДА ТребуетсяГеренация.ОбъектРасчетов ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ТребуетсяСоздатьОбъектРасчетов,
	|	ВЫБОР
	|		КОГДА ДвиженияПоДочерним.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ТребуетсяРазрешениеНаДочернихПартнеров
	|ИЗ
	|	ВТДляОбработки КАК ДляОбработки
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТТребуетсяГеренация КАК ТребуетсяГеренация
	|		ПО (ТребуетсяГеренация.ОбъектРасчетов = ДляОбработки.Ссылка)
	|	ЛЕВОЕ СОЕДИНЕНИЕ втДвиженияДочернихПартнеров КАК ДвиженияПоДочерним
	|		ПО (ДвиженияПоДочерним.Ссылка = ДляОбработки.Ссылка)
	|";
	#КонецОбласти
	...
	ЭлементСправочника = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выбрать();
	...
	ИспользованиеВТабЧастях = МВТ.Таблицы[2].ПолучитьДанные().Выгрузить();
	...
	Пока ЭлементСправочника.Следующий() Цикл
		...
		Попытка
			...
			Если ЭлементСправочника.ТребуетсяСоздатьОбъектРасчетов Тогда
				...
				Для Каждого ПараметрГенерации Из ДополненныеПараметрыМеханизма.МассивПараметров Цикл
					...
					Если Не ЗначениеЗаполнено(НовыйОбъектРасчетов) Тогда
						СсылкаЯвляетсяОбъектомРасчетов = ОбъектыРасчетовСервер.СсылкаЯвляетсяОбъектомРасчетов(СправочникОбъект, ПараметрГенерации);
						Если СсылкаЯвляетсяОбъектомРасчетов И Не ПараметрГенерации.ТолькоОстатки
							ИЛИ Не СсылкаЯвляетсяОбъектомРасчетов И ПараметрГенерации.ТолькоОстатки Тогда
								ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Не удалось создать объект расчетов для договора: %1.';
										|en = 'Cannot create AR/AP object for the contract: %1.'"),
			...
			Если ЭлементСправочника.ТребуетсяРазрешениеНаДочернихПартнеров Тогда
				
				СправочникОбъект.РазрешенаРаботаСДочернимиПартнерами = Истина;
...
Функция ТекстЗапросаДоговораСДвижениямиПоДочернимПартнерам(ВременнаяТаблица = Ложь)
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Аналитика.Договор КАК Ссылка
	|ИЗ
	...
	|ГДЕ
	|	Аналитика.Партнер.Родитель = Договоры.Партнер
	...
	Если ВременнаяТаблица Тогда
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Аналитика.Договор КАК Ссылка
		|ПОМЕСТИТЬ втДвиженияДочернихПартнеров
		|ИЗ
		|	РегистрНакопления.РасчетыСКлиентами КАК Расчеты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК Аналитика
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК Договоры
