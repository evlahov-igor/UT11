...
Процедура НапечататьОбразец(Команда)
	...
	Если Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЦенникДляТоваров") Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ИмяОбработкиПечатьЭтикетокИЦенников, "ЦенникТовары", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЦенникаТовара());
	...
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляТоваров") Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ИмяОбработкиПечатьЭтикетокИЦенников, "ЭтикеткаТовары", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЭтикеткиТовара());
	...
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляСкладскихЯчеек") Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ИмяОбработкиПечатьЭтикетокИЦенников, "ЭтикеткаСкладскиеЯчейки", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЭтикеткиСкладскойЯчейки());
	...
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляДоставки") Тогда
		...
		Если ПараметрыПечати.ЕстьЭтикеткиДляПечати Тогда
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ИмяОбработкиПечатьЭтикетокИЦенников, "ЭтикеткаДоставки", ПараметрКоманды, Неопределено, ПараметрыПечати);
	...
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаСерииНоменклатуры") Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ИмяОбработкиПечатьЭтикетокИЦенников, "ЭтикеткаСерииНоменклатуры", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЭтикеткиСерииНоменклатуры());
	...
	ИначеЕсли Объект.Назначение = ПредопределенноеЗначение("Перечисление.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаУпаковочныхЛистов") Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(ИмяОбработкиПечатьЭтикетокИЦенников, "ЭтикеткаУпаковочныеЛисты", ПараметрКоманды, Неопределено, ПолучитьПараметрыДляПечатиОбразцаЭтикеткиУпаковочныеЛисты());
...
Функция ВставитьРисунокШтрихкода(ИмяТекущейОбласти
	...
	// << 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
	Если ЗначениеЗаполнено(КР_ИмяПараметраШтрихкод) Тогда
		РисунокШтрихкода.Имя = КР_ИмяПараметраШтрихкод
			+ СтрЗаменить(Новый УникальныйИдентификатор, "-", "_");	
	КонецЕсли;
	// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
...
Процедура ЗаполнитьДоступныеПоля()
	...
	Если Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляСкладскихЯчеек Тогда
		СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаСкладскиеЯчейки");
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЦенникДляТоваров Тогда
		// << 08.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		//СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаТовары");
	...
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляТоваров Тогда
		// << 08.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		//СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаТовары");
		СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("КР_ПоляШаблонаТовары");
		// >> 08.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляДоставки Тогда
		СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаДоставка");
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаСерииНоменклатуры Тогда
		СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаСерииНоменклатуры");
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаУпаковочныхЛистов Тогда
		// << 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
		//СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаУпаковочныеЛисты");
		СхемаКомпоновкиДанных = ОбработкаПечатьЭтикетокИЦенников.ПолучитьМакет("КР_ПоляШаблонаУпаковочныеЛисты");
		// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
	КонецЕсли;
	...
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЦенникДляТоваров
		...
		// << 08.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		КР_ЗаполнитьДоступныеПоля(ИменаПолей);
		// >> 08.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	...
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаУпаковочныхЛистов Тогда
		...
		// << 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
		ИменаПолей.Добавить("ШтрихкодСсылки");
		ИменаПолей.Добавить("УпаковочныйЛист.Дата");
		ИменаПолей.Добавить("Получатель");
		// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
...
Процедура УстановитьРазмер(ШиринаОбластиВМиллиметрах,
	...
	ЭталонРазмеров = ЦенообразованиеВызовСервера.ОбработкаПечатьЭтикетокИЦенников().ПолучитьМакет("ЭталонРазмерыЯчеек");
...
Функция ПолучитьПараметрыДляПечатиОбразцаЭтикеткиДоставки()
	...
	ПараметрыПечати = ЦенообразованиеВызовСервера.ОбработкаПечатьЭтикетокИЦенников().ДанныеДляПечатиЭтикетокДоставки(МассивОбъектов);
	Возврат ДополнитьПараметрыДляПечатиОбразца(ПараметрыПечати);
...
// Перед началом добавления необходимо выделить область в табличном документе.
	...
	Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод()
			// << 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
			ИЛИ ИмяПоляВШаблоне = "ШтрихкодСсылки"
			// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод()
			...
			ВставитьРисунокШтрихкода(ТекущаяОбласть.Имя
				// << 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
				, ?(ИмяПоляВШаблоне = "ШтрихкодСсылки", "ШтрихкодСсылки", Неопределено)
				// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
				);
		ИначеЕсли ИмяПоляВШаблоне = ИмяПараметраВалютаЦены() Тогда
...
&НаСервереБезКонтекста
Процедура КР_ЗаполнитьДоступныеПоля(ИменаПолей)
	
	ИменаПолей.Добавить("Организация.Наименование");
	ИменаПолей.Добавить("Организация.НаименованиеСокращенное");
	ИменаПолей.Добавить("Организация.НаименованиеПолное");
	ИменаПолей.Добавить("Организация.КПП");
	ИменаПолей.Добавить("Организация.ИНН");
	
	ИменаПолей.Добавить("НаименованиеНаЦеннике");
	ИменаПолей.Добавить("Пол");
	ИменаПолей.Добавить("АртикулСПробелами");
	ИменаПолей.Добавить("Цвет");
	ИменаПолей.Добавить("РазмерСРоссийским");
	ИменаПолей.Добавить("РазмерыТовара");
	ИменаПолей.Добавить("РазмерныеДанные");			// #4693.. Фомин Д.Ю. 24.09.2024.
	ИменаПолей.Добавить("Модель");
	ИменаПолей.Добавить("КодФлеша");
	ИменаПолей.Добавить("Размещение");
	ИменаПолей.Добавить("Состав");
	ИменаПолей.Добавить("Схема");
	ИменаПолей.Добавить("ТехРегламент");
	ИменаПолей.Добавить("СтранаИзготовителя");
	ИменаПолей.Добавить("Изготовитель");
	ИменаПолей.Добавить("ЮридическийАдресИзготовителя");
	ИменаПолей.Добавить("ДатаИзготовления");
	ИменаПолей.Добавить("ДатаПечати");  
	// << 18.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2553
	ИменаПолей.Добавить("Номенклатура.Марка");
	ИменаПолей.Добавить("ОрганизацияСокращенноеНаименование");
	// >> 18.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2553		
