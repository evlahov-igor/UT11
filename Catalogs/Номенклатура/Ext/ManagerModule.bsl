...
Процедура ОбработкаПолученияПолейПредставления(Поля, СтандартнаяОбработка)
	...
	// << 13.07.2023 Фомичев А.Е., КРОК, A2105505-1932 Отмена исправлений
	// << 11.05.2023 Федотов А.М., КРОК, Jira№A2105505-1646
	...
	//КонецЕсли;
	// >> 11.05.2023 Федотов А.М., КРОК, Jira№A2105505-1646
	// >> 13.07.2023 Фомичев А.Е., КРОК, A2105505-1932
КонецПроцедуры
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	//Если СтандартнаяОбработка Тогда
	//	СтандартнаяОбработка = Ложь;
	//	Представление = СтрШаблон("%1 - %2", Данные.Артикул, Данные.Наименование);
	//КонецЕсли;
	// >> 11.05.2023 Федотов А.М., КРОК, Jira№A2105505-1646
	// >> 13.07.2023 Фомичев А.Е., КРОК, A2105505-1932
КонецПроцедуры
...
Функция РеквизитыОтключенныеПоФО(ЭтоРеквизитыХарактеристик = Ложь) Экспорт
	...
	// << 08.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-991 
    // Так как ФО могут быть инициализированы позже, отключаем логику
	// Она "ломает" целостность номенклатуры    
	// Для фонового задания параметры запуска не заполняются
	Если ТипЗнч(ПараметрыСеанса.ПараметрыКлиентаНаСервере) = Тип("ФиксированноеСоответствие") Тогда 
		ПараметрыСеансаПараметрЗапуска = ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить("ПараметрЗапуска"); // A2105505-2550
		Если ТипЗнч(ПараметрыСеансаПараметрЗапуска) = Тип("Строка")
			И СтрНайти(ПараметрыСеансаПараметрЗапуска, "КР_РежимЗагрузкиДанных") > 0 Тогда 
...
Функция ЗависимостьРеквизитовОтТипаНоменклатуры(ТипНоменклатуры,
	...
	// << 07.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
	КР_ЗависимостьРеквизитовОтТипаНоменклатуры(ТаблицаРеквизитов,
													ТипНоменклатуры,
													ОсобенностьУчета,
													ИспользованиеХарактеристик,
													Ложь);
...
Функция ИспользованиеЭлементов(ПереданныйОбъект, РежимВидимостиПоказатьТолькоВажные, РежимПроверкиИспользованияРеквизитов) Экспорт
	...
	МассивРеквизитовДляУстановкиВидимости = КР_МассивРеквизитовДляУстановкиВидимостиТипТовар();
	
	Если Не РежимПроверкиИспользованияРеквизитов Тогда
		Для Каждого ИмяРеквизита Из МассивРеквизитовДляУстановкиВидимости Цикл
			ИспользованиеЭлементов.Вставить(ИмяРеквизита, ЭтоТовар);
		КонецЦикла;
		
		// Реквизит "КодТНВЭД" всегда не используется, не отображается на форме
		ИспользованиеЭлементов.Вставить("КодТНВЭД", Ложь);
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит Ставка НДС, а также заполняет реквизиты ""Единица измерения ТН ВЭД"" и ""Прослеживаемый товар"" в соответствии с классификатором ТН ВЭД'");
...
//
// Параметры:
//	Параметры - Структура - параметры обработки данных см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	&ТекстВложенногоЗапроса КАК ОбновляемыеДанные";
	...
	|ГДЕ
	|	ДанныеОбъекта.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|	И НЕ ДанныеОбъекта.УдалитьСтавкаНДС = &ПустаяСтавкаНДС";
	...
	|	Справочник.Номенклатура КАК ДанныеОбъекта
	|ГДЕ
	|	(ДанныеОбъекта.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|		ИЛИ ДанныеОбъекта.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	|	И ТИПЗНАЧЕНИЯ(ДанныеОбъекта.Принципал) = ТИП(Справочник.Партнеры)
	|	И НЕ ТИПЗНАЧЕНИЯ(ДанныеОбъекта.Контрагент) = ТИП(Справочник.Контрагенты)";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса);
	
	НоменклатураЛокализация.ДополнитьТекстВложенногоЗапросаРегистрацииДанныхКОбработке(ТекстыЗапроса);
	
	ТекстВложенногоЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстВложенногоЗапроса", "(" + ТекстВложенногоЗапроса + ")");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	"ВЫБРАТЬ
	|	ОбновляемыеДанные.Ссылка							КАК Ссылка,
	|	ОбновляемыеДанные.ВерсияДанных						КАК ВерсияДанных,
	|	МАКСИМУМ(ОбновляемыеДанные.КодТНВЭД)				КАК КодТНВЭД,
	|	МАКСИМУМ(ОбновляемыеДанные.ЕдиницаИзмерения)		КАК ЕдиницаИзмерения,
	|	МАКСИМУМ(ОбновляемыеДанные.ПрослеживаемыйТовар)		КАК ПрослеживаемыйТовар,
	|	МАКСИМУМ(ОбновляемыеДанные.ЗаполнениеСтавки)		КАК ЗаполнениеСтавки,
	|	МАКСИМУМ(ОбновляемыеДанные.ЗаполнитьКонтрагента)	КАК ЗаполнитьКонтрагента
	|ИЗ
	|	&ТекстВложенногоЗапроса КАК ОбновляемыеДанные
	|
	|СГРУППИРОВАТЬ ПО
	|	ОбновляемыеДанные.Ссылка,
	|	ОбновляемыеДанные.ВерсияДанных
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка";
	...
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ (ДанныеНоменклатуры.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|			И НЕ ДанныеНоменклатуры.УдалитьСтавкаНДС = &ПустаяСтавкаНДС)
	|	КОНЕЦ							КАК ЗаполнениеСтавки,
	...
	ТекстыЗапроса.Добавить(ТекстВложенногоЗапроса);
	...
	|	ЛОЖЬ									КАК ПрослеживаемыйТовар,
	|	ЛОЖЬ									КАК ЗаполнениеСтавки,
	|	ИСТИНА									КАК ЗаполнитьКонтрагента
	|ИЗ
	|	&ВТДанныеДляОбработки КАК ДанныеОбъекта
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК ДанныеНоменклатуры
	|		ПО ДанныеОбъекта.Ссылка = ДанныеНоменклатуры.Ссылка
	...
	|		ИЛИ ДанныеНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	|	И ТИПЗНАЧЕНИЯ(ДанныеНоменклатуры.Принципал) = ТИП(Справочник.Партнеры)
	|	И НЕ ТИПЗНАЧЕНИЯ(ДанныеНоменклатуры.Контрагент) = ТИП(Справочник.Контрагенты)";
	
	ТекстыЗапроса.Добавить(ТекстВложенногоЗапроса);
	
	НоменклатураЛокализация.ДополнитьТекстВложенногоЗапросаОбработкиДанных(ТекстыЗапроса);
	
	ТекстВложенногоЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстВложенногоЗапроса", "(" + ТекстВложенногоЗапроса + ")");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(Выборка.КодТНВЭД) Тогда
			...
			Если ЗначениеЗаполнено(Выборка.ЕдиницаИзмерения)
				И (Не ЗначениеЗаполнено(ДанныеОбъекта.ЕдиницаИзмеренияТНВЭД)
			...
			НоменклатураЛокализация.ОбработатьДанныеРеквизитовПрослеживаемостиДляПереходаНаНовуюВерсию(ДанныеОбъекта,
																										Выборка,
																										ОбъектИзменен);
			
			Если Выборка.ЗаполнениеСтавки
				И Не ЗначениеЗаполнено(ДанныеОбъекта.СтавкаНДС) Тогда
				
				ОбъектИзменен = Истина;
				
				ДанныеОбъекта.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(ДанныеОбъекта.УдалитьСтавкаНДС);
				
			КонецЕсли;
			
			Если Выборка.ЗаполнитьКонтрагента
				И ЗначениеЗаполнено(ДанныеОбъекта.Принципал)
				И Не ЗначениеЗаполнено(ДанныеОбъекта.Контрагент) Тогда
				
				ОбъектИзменен = Истина;
				
				ПартнерыИКонтрагенты.ЗаполнитьКонтрагентаПартнераПоУмолчанию(ДанныеОбъекта.Принципал, ДанныеОбъекта.Контрагент);
...
// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
Функция КР_МассивРеквизитовДляУстановкиВидимостиТипТовар()
	
	МассивРеквизитов = Новый Массив;
	
	МассивРеквизитов.Добавить("КР_Модель");
	МассивРеквизитов.Добавить("КР_Цвет");
	МассивРеквизитов.Добавить("КР_Линия");
	МассивРеквизитов.Добавить("КР_НаименованиеПоСертификату");
	МассивРеквизитов.Добавить("КР_НетВшивногоЯрлыка");
	МассивРеквизитов.Добавить("КР_ПечатьEAC");
	МассивРеквизитов.Добавить("КР_ПечатьРСТ");
	
	// << 21.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-489
	МассивРеквизитов.Добавить("КР_Мех");
	МассивРеквизитов.Добавить("КР_МехАнг");
	МассивРеквизитов.Добавить("КР_СоставПодкладки");
	МассивРеквизитов.Добавить("КР_СоставПодкладкиАнг");
	МассивРеквизитов.Добавить("КР_СоставТкани");
	МассивРеквизитов.Добавить("КР_СоставТканиАнг");
	МассивРеквизитов.Добавить("КР_Утеплитель");
	МассивРеквизитов.Добавить("КР_УтеплительАнг");
	// >> 21.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-489
	
	// << 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
	МассивРеквизитов.Добавить("КР_НаименованиеНаЦеннике");
	МассивРеквизитов.Добавить("КР_ТипТовара");
	МассивРеквизитов.Добавить("КР_ДатаЗаказа");
	// >> 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
	
	Возврат МассивРеквизитов;
	
КонецФункции // >> 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324

// << 07.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
Процедура КР_ЗависимостьРеквизитовОтТипаНоменклатуры(ТаблицаРеквизитов,
														ТипНоменклатуры,
														ОсобенностьУчета,
														ИспользованиеХарактеристик,
	...
	Если Не ЭтоРеквизитыХарактеристик Тогда
		Реквизиты = Метаданные.Справочники.Номенклатура.Реквизиты;
		
		НоваяСтрока = ТаблицаРеквизитов.Добавить();
		НоваяСтрока.Реквизит      = Реквизиты.КР_Цвет;
		НоваяСтрока.Использование = ЭтоТовар;
	КонецЕсли;
	
КонецПроцедуры // >> 07.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324

// << 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
// Возвращает актуаьный сезон по данным РС КР_ИсторияИзмененийСезонаНоменклатуры
//
// Параметры:
//  Номенклатура  - СправочникСсылка.Номенклатура - номенклатура для определения сезона
//
// Возвращаемое значение:
//   СправочникСсылка.КР_Сезоны   - Сезон номенклатуры
//
Функция КР_АктуальныйСезонПродажи(Номенклатура) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ИсторияИзмененийСезона.Сезон КАК Сезон
	|ИЗ
	|	РегистрСведений.КР_ИсторияИзмененийСезонаНоменклатуры.СрезПоследних(&Период, Номенклатура = &Номенклатура) КАК ИсторияИзмененийСезона";
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		Возврат Справочники.КР_Сезоны.ПустаяСсылка();	
	КонецЕсли;
	
	Выборка = РезультатЗапроса.Выбрать();
	Выборка.Следующий();
	Возврат Выборка.Сезон;
