...
Процедура ПриЗаписи(Отказ)
	...
	// << 01.03.2023 Федотов А.М., КРОК, Jira№A2105505-1235
...
Процедура КР_ОбработкаПроверкиЗаполненияДополнительно(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар Тогда
		МассивНепроверяемыхРеквизитов.Добавить("КР_Цвет");
	КонецЕсли;
...
Процедура КР_ПриЗаписиДополнительно(Отказ)
	...
	Если ЗначениеЗаполнено(КоллекцияНоменклатуры) Тогда
		СезонКоллекции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КоллекцияНоменклатуры, "КР_Сезон");
	Иначе
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СезонКоллекции) Тогда
		Возврат;	
	КонецЕсли;
	
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.КР_ИсторияИзмененийСезонаНоменклатуры");
	ЭлементБлокировки.УстановитьЗначение("Номенклатура", Ссылка);
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	Блокировка.Заблокировать();
	
	АктуальныйСезон = Справочники.Номенклатура.КР_АктуальныйСезонПродажи(Ссылка); 
	
	Если СезонКоллекции <> АктуальныйСезон Тогда
		МенеджерЗаписи = РегистрыСведений.КР_ИсторияИзмененийСезонаНоменклатуры.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Период = ТекущаяДатаСеанса();
		МенеджерЗаписи.Номенклатура = Ссылка;
		МенеджерЗаписи.Сезон = СезонКоллекции;
		МенеджерЗаписи.Записать();
	КонецЕсли;
	
КонецПроцедуры // >> 01.03.2023 Федотов А.М., КРОК, Jira№A2105505-1235
