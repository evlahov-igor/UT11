...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 23.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
	// Изменение формы
...
Функция ИменаСворачиваемыхГрупп()
	...
	// << 23.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
	КР_ИменаСворачиваемыхГруппДоработки(ИменаСворачиваемыхГрупп);
	// >> 23.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
...
Процедура КР_ПриСозданииНаСервереДополнительно()
	...
	// Служебный реквизит мТекущаяМодель
	мТекущаяМодельРеквизит = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(
		ЭтотОбъект, "мТекущаяМодель", ОбщегоНазначения.ОписаниеТипаСтрока(20));
	ЭтотОбъект.мТекущаяМодель = Объект.КР_Модель;
	
	// << 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
	// Служебный реквизит мАктуальныйСезон
	мАктуальныйСезонРеквизит = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"мАктуальныйСезон", Новый ОписаниеТипов("СправочникСсылка.КР_Сезоны"), , НСтр("ru = 'Актуальный сезон продажи'"));
	мАктуальныйСезонЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, мАктуальныйСезонРеквизит, , Элементы.СезоннаяГруппа);
	мАктуальныйСезонЭлемент.ТолькоПросмотр = Истина;
	ЭтотОбъект.мАктуальныйСезон = Справочники.Номенклатура.КР_АктуальныйСезонПродажи(Объект.Ссылка);
	// >> 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
	
	// Сворачиваемая группа "Свойства"
	СвойстваГруппа = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "СворачиваемаяГруппаСвойства", Элементы.ГруппаЛево, Элементы.СворачиваемаяГруппаДополнительныеРеквизиты);
	СвойстваГруппа.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	СвойстваГруппа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	СвойстваГруппа.Заголовок = НСтр("ru = 'Свойства'");
	СвойстваГруппа.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	СвойстваГруппа.ОтображениеУправления = ОтображениеУправленияОбычнойГруппы.Картинка;
	
	// КР_Модель
	КР_МодельЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.КР_Модель", СвойстваГруппа);
	КР_МодельЭлемент.УстановитьДействие("ПриИзменении", "КР_МодельПриИзменении");
	
	// КР_Цвет
	КР_ЦветЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.КР_Цвет", СвойстваГруппа);
	КР_ЦветЭлемент.АвтоОтметкаНезаполненного = Объект.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар;
	КР_ЦветЭлемент.УстановитьДействие("ПриИзменении", "КР_ЦветПриИзменении");
	
	// КР_Линия
	КР_ЛинияЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_Линия", СвойстваГруппа);
	КР_ЛинияЭлемент.УстановитьДействие("ПриИзменении", "КР_ЛинияПриИзменении"); 	// #3853.. Фомин Д.Ю. 15.05.2024.
	
	// << 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
	// КР_ТипТовара
	КР_ТипТовараЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ТипТовара", СвойстваГруппа);
	
	// КР_НаименованиеПоСертификату
	КР_НаименованиеПоСертификатуЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_НаименованиеПоСертификату", СвойстваГруппа);
	// >> 17.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1179
	
	// << 09.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	КР_НаименованиеНаЦенникеЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_НаименованиеНаЦеннике", СвойстваГруппа);
	
	КР_ДатаЗаказаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ДатаЗаказа", СвойстваГруппа);
	// >> 09.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	
	// КР_НетВшивногоЯрлыка
	КР_НетВшивногоЯрлыкаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_НетВшивногоЯрлыка", СвойстваГруппа);
	КР_НетВшивногоЯрлыкаЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// КР_ПечатьEAC
	КР_ПечатьEACЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ПечатьEAC", СвойстваГруппа);
	КР_ПечатьEACЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// КР_ПечатьРСТ
	КР_ПечатьРСТЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ПечатьРСТ", СвойстваГруппа);
	КР_ПечатьРСТЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// << 21.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-489
	// Сворачиваемая группа "Состав"
	СоставГруппа = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "СворачиваемаяГруппаСостав", Элементы.ГруппаЛево, Элементы.СворачиваемаяГруппаДополнительныеРеквизиты);
	СоставГруппа.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	СоставГруппа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	СоставГруппа.Заголовок = НСтр("ru = 'Состав'");
	СоставГруппа.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	СоставГруппа.ОтображениеУправления = ОтображениеУправленияОбычнойГруппы.Картинка;
	
	// Реквизиты состава номенклатуры
	МассивОтображаемыхРеквизитов = Новый Массив;
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_Мех");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_МехАнг");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_СоставПодкладки");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_СоставПодкладкиАнг");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_СоставТкани");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_СоставТканиАнг");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_Утеплитель");
	МассивОтображаемыхРеквизитов.Добавить("Объект.КР_УтеплительАнг");
	
	ОтображаемыеРеквизиты = КР_МетодыМодификацииФорм.ВставитьЭлементыФормы(
		ЭтотОбъект, МассивОтображаемыхРеквизитов, СоставГруппа);
	// >> 21.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-489
	
	// --> Евлахов Игорь Николаевич (Начало) 06.09.2024
	// Задача #4497
	РеквизитыДляЗаполненияЦенника = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "СворачиваемаяГруппаРеквизитыДляЗаполненияЦенника", Элементы.ГруппаЛево, Элементы.СворачиваемаяГруппаШаблоны);
	РеквизитыДляЗаполненияЦенника.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	РеквизитыДляЗаполненияЦенника.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	РеквизитыДляЗаполненияЦенника.Заголовок = НСтр("ru = 'Реквизиты для заполнения ценника'");
	РеквизитыДляЗаполненияЦенника.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	РеквизитыДляЗаполненияЦенника.ОтображениеУправления = ОтображениеУправленияОбычнойГруппы.Картинка;
	
	// Реквизиты для заполнения ценника
	МассивОтображаемыхРеквизитов = Новый Массив;
	МассивОтображаемыхРеквизитов.Добавить("Объект.злПроизводительДляЦенника");
	МассивОтображаемыхРеквизитов.Добавить("Объект.злСтранаДляЦенника");		// #4925.. Фомин Д.Ю. 17.10.2024.	
	МассивОтображаемыхРеквизитов.Добавить("Объект.злАдресПроизводителяДляЦенника");
	МассивОтображаемыхРеквизитов.Добавить("Объект.злСертификационныйЦентрДляЦенника");
	МассивОтображаемыхРеквизитов.Добавить("Объект.злНомерСертификатаДляЦенника");
	МассивОтображаемыхРеквизитов.Добавить("Объект.злДатаСертификатаДляЦенника");
	МассивОтображаемыхРеквизитов.Добавить("Объект.злСхемаДляЦенника");
	
	// --> Евлахов Игорь Николаевич (Начало) 20.09.2024
	// Задача #4661
	МассивОтображаемыхРеквизитов.Добавить("Объект.злРазмерДляАксессуаров");
	// <-- Евлахов Игорь Николаевич (Конец) 20.09.2024
	
	ОтображаемыеРеквизиты = КР_МетодыМодификацииФорм.ВставитьЭлементыФормы(
		ЭтотОбъект, МассивОтображаемыхРеквизитов, РеквизитыДляЗаполненияЦенника);
...
КонецПроцедуры // >> 23.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324

// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаКлиенте
Процедура КР_ПриОткрытииДополнительно(Отказ)
	
	Если Не Элементы.СтраницыКарточкаНоменклатуры.ТекущаяСтраница = Элементы.СтраницаРеквизитыНоменклатуры Тогда
		Элементы.СтраницыКарточкаНоменклатуры.ТекущаяСтраница = Элементы.СтраницаРеквизитыНоменклатуры;
		Элементы.СтраницаКарточкаНоменклатуры.Видимость = Ложь;
		Элементы.ГруппаГруппировкаЛевоПраво.Видимость = Истина;
	КонецЕсли;
...
// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаКлиенте
Процедура КР_МодельПриИзменении(Элемент)
	
	ТекущаяМодель = СокрЛП(Объект.КР_Модель);
	
	МинДлинаМодели = 5;
	
	Если СтрДлина(ТекущаяМодель) < МинДлинаМодели Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректно заполнена модель.'"));
		Объект.КР_Модель = ЭтотОбъект.мТекущаяМодель;
		Возврат;
	Иначе
		Объект.КР_Модель = ТекущаяМодель;
		ЭтотОбъект.мТекущаяМодель = ТекущаяМодель;
	КонецЕсли;
	
	КР_МодельПриИзмененииНаСервере();
	
	КР_ЗаполнитьАртикулНаСервере();
	
КонецПроцедуры // >> 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324

// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаКлиенте
Процедура КР_ЦветПриИзменении(Элемент)
	
	КР_ЗаполнитьАртикулНаСервере();
	
КонецПроцедуры // >> 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324

// #3853.. Фомин Д.Ю. 15.05.2024.
&НаКлиенте
Процедура КР_ЛинияПриИзменении(Элемент)
	СтруктураВозврата = КР_ЛинияПриИзмененииНаСервере(Объект.КР_Линия);
	Объект.Марка = СтруктураВозврата.Марка;
КонецПроцедуры	
// ..#3853
...
#Область КР_ПриИзмененииРеквизитов

// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаСервере
Процедура КР_МодельПриИзмененииНаСервере()
	
	КодыЛинииКоллекции = Лев(Объект.КР_Модель, 5);
	КодЛиния = Лев(КодыЛинииКоллекции, 2);
	КодКоллекция = Сред(КодыЛинииКоллекции, 3);
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ Ссылка, Марка ИЗ Справочник.КР_Линии       ГДЕ Код = &КодЛиния;	// #3853.. Фомин Д.Ю. 15.05.2024. Добавил в выборку поле Марка.
		|ВЫБРАТЬ Ссылка ИЗ Справочник.КоллекцииНоменклатуры ГДЕ Код = &КодКоллекция");
	Запрос.УстановитьПараметр("КодЛиния"    , КодЛиния);
	Запрос.УстановитьПараметр("КодКоллекция", КодКоллекция);
	
	ПакетРезультатов = Запрос.ВыполнитьПакет();
	
	Если ПакетРезультатов[0].Пустой() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Линия номенклатуры не найдена.'"));
	Иначе
		Выборка = ПакетРезультатов[0].Выбрать();
		Выборка.Следующий();
		Объект.КР_Линия = Выборка.Ссылка;
		Объект.Марка = Выборка.Марка;		// #3853.. Фомин Д.Ю. 15.05.2024.
	КонецЕсли;
	
	Если ПакетРезультатов[1].Пустой() Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Коллекция номенклатуры не найдена.'"));
	Иначе
		Выборка = ПакетРезультатов[1].Выбрать();
		Выборка.Следующий();
		Объект.КоллекцияНоменклатуры = Выборка.Ссылка;
	КонецЕсли;
...
// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаСервере
Процедура КР_ЗаполнитьАртикулНаСервере()
	
	Разделитель = ?(Не ПустаяСтрока(Объект.КР_Модель) И ЗначениеЗаполнено(Объект.КР_Цвет), " ", "");
	
	Объект.Артикул = Объект.КР_Модель + Разделитель + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КР_Цвет, "Код");
	
КонецПроцедуры // >> 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324

// #3853.. Фомин Д.Ю. 15.05.2024.
&НаСервереБезКонтекста
Функция КР_ЛинияПриИзмененииНаСервере(Линия)
	
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("Марка", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Линия, "Марка"));
...
// << 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаСервере
Процедура КР_ИменаСворачиваемыхГруппДоработки(ИменаСворачиваемыхГрупп)
	
	ИменаСворачиваемыхГрупп.Добавить(Элементы.СворачиваемаяГруппаСвойства.Имя);
	
	// << 21.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-489
	ИменаСворачиваемыхГрупп.Добавить(Элементы.СворачиваемаяГруппаСостав.Имя);
	// >> 21.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-489
	
	// --> Евлахов Игорь Николаевич (Начало) 06.09.2024
	// Задача #4497
	ИменаСворачиваемыхГрупп.Добавить(Элементы["СворачиваемаяГруппаРеквизитыДляЗаполненияЦенника"].Имя);
	// <-- Евлахов Игорь Николаевич (Конец) 06.09.2024
...
Процедура КР_УстановитьВидимостиДоступности()
	...
	Элементы.Артикул.ТолькоПросмотр = ЭтоТовар;
	
	Элементы.мАктуальныйСезон.Видимость = ЭтоТовар;
