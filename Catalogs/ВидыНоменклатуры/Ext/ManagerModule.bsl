...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"" и конвертирует шаблоны наименований в новый формат'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования

	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Справочники.ВидыНоменклатуры.ОбработатьДанныеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования";
	Обработчик.Версия = "11.5.6.26";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("510a881b-c363-4cf1-97fc-f2f23843137c");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ВидыНоменклатуры.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты:
	|""Настройки ключа цен по характеристике""
	|""Настройки ключа цен по серии""
	|""Настройки ключа цен по упаковке""'");
...
#Область ОбработатьДополнительныеРеквизитыНоменклатурыДляПереходаНаНовуюВерсию
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.ВидыНоменклатуры.ОбработатьДанныеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования";
	НоваяСтрока.Порядок = "Любой";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|ГДЕ
	|	(НЕ ВидыНоменклатуры.ШаблонНаименованияДляПечатиНоменклатуры ПОДОБНО """"
	|		И ВидыНоменклатуры.ШаблонНаименованияДляПечатиНоменклатурыИсторияПереходаНаНовыеФормулы ПОДОБНО """")
	|	ИЛИ (НЕ ВидыНоменклатуры.ШаблонНаименованияДляПечатиХарактеристики ПОДОБНО """"
	|		И ВидыНоменклатуры.ШаблонНаименованияДляПечатиХарактеристикиИсторияПереходаНаНовыеФормулы ПОДОБНО """")
	|	ИЛИ (НЕ ВидыНоменклатуры.ШаблонРабочегоНаименованияНоменклатуры ПОДОБНО """"
	|		И ВидыНоменклатуры.ШаблонРабочегоНаименованияНоменклатурыИсторияПереходаНаНовыеФормулы ПОДОБНО """")
	|	ИЛИ (НЕ ВидыНоменклатуры.ШаблонРабочегоНаименованияСерии ПОДОБНО """"
	|		И ВидыНоменклатуры.ШаблонРабочегоНаименованияСерииИсторияПереходаНаНовыеФормулы ПОДОБНО """")
	|	ИЛИ (НЕ ВидыНоменклатуры.ШаблонРабочегоНаименованияХарактеристики ПОДОБНО """"
	|		И ВидыНоменклатуры.ШаблонРабочегоНаименованияХарактеристикиИсторияПереходаНаНовыеФормулы ПОДОБНО """")
	|
	//++ Локализация
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.УстановитьПараметр("ВТДанныеДляОбработки", ОбновляемыеДанные);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ОбработатьИзменениеШаблоновНаименования(СправочникОбъект, ОбъектИзменен);
			...
			Если Выборка.ТребуетсяЗаменаВКонтроле Тогда
				НайденнаяСтрока = СправочникОбъект.РеквизитыДляКонтроляНоменклатуры.Найти(ИмяРеквизитаДляПодмены, "ИмяРеквизита");
			...
			Если Выборка.ТребуетсяЗаменаВОтборе Тогда
				НайденнаяСтрока = СправочникОбъект.РеквизитыБыстрогоОтбораНоменклатуры.Найти(ИмяРеквизитаДляПодмены, "ИмяРеквизита");
				Если НайденнаяСтрока <> Неопределено Тогда
					НайденнаяСтрока.ИмяРеквизита = НовоеИмяРеквизита;
					НайденнаяСтрока.ПредставлениеРеквизита = НовоеОписаниеРеквизита;
				КонецЕсли;
...
Процедура ЗарегистрироватьДополнительныеРеквизитыНоменклатурыКОбновлению(Параметры) Экспорт
	...
	|					ГДЕ
	|						ТЧДополнительныеРеквизиты.Ссылка = &Набор_Справочник_Номенклатура_Общие)
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|						Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК ТЧДополнительныеРеквизиты
	|					ГДЕ
	|						ТЧДополнительныеРеквизиты.Ссылка = &Набор_Справочник_Номенклатура_Общие)) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|						Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК ТЧДополнительныеРеквизиты
	|					ГДЕ
	|						ТЧДополнительныеРеквизиты.Ссылка = &Набор_Справочник_ХарактеристикиНоменклатуры_Общие)
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|						Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК ТЧДополнительныеРеквизиты
	|					ГДЕ
	|						ТЧДополнительныеРеквизиты.Ссылка = &Набор_Справочник_ХарактеристикиНоменклатуры_Общие)) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВложенныйЗапрос.Свойство,
	|		ВложенныйЗапрос.ВидНоменклатуры
	...
	|						Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК ТЧДополнительныеРеквизиты
	|					ГДЕ
	|						ТЧДополнительныеРеквизиты.Ссылка = &Набор_Справочник_СерииНоменклатуры_Общие)
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|						Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК ТЧДополнительныеРеквизиты
	|					ГДЕ
	|						ТЧДополнительныеРеквизиты.Ссылка = &Набор_Справочник_СерииНоменклатуры_Общие)) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВложенныйЗапрос.Свойство,
	|		ВложенныйЗапрос.ВидНоменклатуры
...
Процедура ОбработатьДополнительныеРеквизитыНоменклатурыДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	ОбщийНабор.Ссылка В (&Набор_Справочник_Номенклатура_Общие, &Набор_Справочник_ХарактеристикиНоменклатуры_Общие, &Набор_Справочник_СерииНоменклатуры_Общие)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И (ВтОбщиеСвойства.НаборСвойств = &Набор_Справочник_Номенклатура_Общие)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВтОбщиеСвойства.Свойство,
	|	РеквизитыДляКонтроляХарактеристик.Ссылка,
	|	&Набор_Справочник_ХарактеристикиНоменклатуры_Общие,
	|	""ВидНоменклатуры"",
	|	""ВСписке"",
	|	""ЗаполнятьОбязательно"",
	|	НЕ РеквизитыДляКонтроляХарактеристик.Свойство ЕСТЬ NULL
	...
	|			И (ВтОбщиеСвойства.НаборСвойств = &Набор_Справочник_ХарактеристикиНоменклатуры_Общие)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВтОбщиеСвойства.Свойство,
	|	РеквизитыДляКонтроляСерий.Ссылка,
	|	&Набор_Справочник_СерииНоменклатуры_Общие,
	|	""ВидНоменклатуры"",
	|	""ВСписке"",
	|	""ЗаполнятьОбязательно"",
	|	НЕ РеквизитыДляКонтроляСерий.Свойство ЕСТЬ NULL
	...
	|			И (ВтОбщиеСвойства.НаборСвойств = &Набор_Справочник_СерииНоменклатуры_Общие)
	|ИТОГИ
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования(Параметры) Экспорт
	...
	ПараметрыВыборки = Параметры.ПараметрыВыборки;//Структура
	ПараметрыВыборки.ПолныеИменаОбъектов = "Справочник.ВидыНоменклатуры";
	...
	|	Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|ГДЕ
	|	ВидыНоменклатуры.НастройкиКлючаЦенПоХарактеристике = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.ПустаяСсылка)
	|	ИЛИ ВидыНоменклатуры.НастройкиКлючаЦенПоСерии = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.ПустаяСсылка)
	|	ИЛИ ВидыНоменклатуры.НастройкиКлючаЦенПоУпаковке = ЗНАЧЕНИЕ(Перечисление.ВариантОтбораДляКлючаЦен.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования(Параметры) Экспорт
	...
	Для Каждого ЭлементСправочника Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если СправочникОбъект.ИспользованиеХарактеристик = Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать Тогда
				Если Не ЗначениеЗаполнено(СправочникОбъект.НастройкиКлючаЦенПоХарактеристике) Тогда
					СправочникОбъект.НастройкиКлючаЦенПоХарактеристике = Перечисления.ВариантОтбораДляКлючаЦен.НеИспользовать;
					ОбъектИзменен = Истина;
				КонецЕсли;
			Иначе
				Если Не ЗначениеЗаполнено(СправочникОбъект.НастройкиКлючаЦенПоХарактеристике) Тогда
					СправочникОбъект.НастройкиКлючаЦенПоХарактеристике = Перечисления.ВариантОтбораДляКлючаЦен.Использовать;
					ОбъектИзменен = Истина;
				КонецЕсли;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СправочникОбъект.НастройкиКлючаЦенПоСерии) Тогда
				СправочникОбъект.НастройкиКлючаЦенПоСерии = Перечисления.ВариантОтбораДляКлючаЦен.НеИспользовать;
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СправочникОбъект.НастройкиКлючаЦенПоУпаковке) Тогда
				СправочникОбъект.НастройкиКлючаЦенПоУпаковке = Перечисления.ВариантОтбораДляКлючаЦен.НеИспользовать;
				ОбъектИзменен = Истина;
			КонецЕсли;
...
Процедура ОбработатьИзменениеШаблоновНаименования(СправочникОбъект, ОбъектИзменен)
	
	Если НЕ ПустаяСтрока(СправочникОбъект.ШаблонНаименованияДляПечатиНоменклатуры)
		И ПустаяСтрока(СправочникОбъект.ШаблонНаименованияДляПечатиНоменклатурыИсторияПереходаНаНовыеФормулы) Тогда
		ОбработатьИзменениеШаблонаСФормулой(СправочникОбъект,
			"ШаблонНаименованияДляПечатиНоменклатуры",
			"ШаблонНаименованияДляПечатиНоменклатурыИсторияПереходаНаНовыеФормулы");
		ОбъектИзменен = Истина;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(СправочникОбъект.ШаблонНаименованияДляПечатиХарактеристики)
		И ПустаяСтрока(СправочникОбъект.ШаблонНаименованияДляПечатиХарактеристикиИсторияПереходаНаНовыеФормулы) Тогда
		ОбработатьИзменениеШаблонаСФормулой(СправочникОбъект,
			"ШаблонНаименованияДляПечатиХарактеристики",
			"ШаблонНаименованияДляПечатиХарактеристикиИсторияПереходаНаНовыеФормулы");
		ОбъектИзменен = Истина;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(СправочникОбъект.ШаблонРабочегоНаименованияНоменклатуры)
		И ПустаяСтрока(СправочникОбъект.ШаблонРабочегоНаименованияНоменклатурыИсторияПереходаНаНовыеФормулы) Тогда
		ОбработатьИзменениеШаблонаСФормулой(СправочникОбъект,
			"ШаблонРабочегоНаименованияНоменклатуры",
			"ШаблонРабочегоНаименованияНоменклатурыИсторияПереходаНаНовыеФормулы");
		ОбъектИзменен = Истина;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(СправочникОбъект.ШаблонРабочегоНаименованияСерии)
		И ПустаяСтрока(СправочникОбъект.ШаблонРабочегоНаименованияСерииИсторияПереходаНаНовыеФормулы) Тогда
		ОбработатьИзменениеШаблонаСФормулой(СправочникОбъект,
			"ШаблонРабочегоНаименованияСерии",
			"ШаблонРабочегоНаименованияСерииИсторияПереходаНаНовыеФормулы");
		ОбъектИзменен = Истина;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(СправочникОбъект.ШаблонРабочегоНаименованияХарактеристики)
		И ПустаяСтрока(СправочникОбъект.ШаблонРабочегоНаименованияХарактеристикиИсторияПереходаНаНовыеФормулы) Тогда
		ОбработатьИзменениеШаблонаСФормулой(СправочникОбъект,
			"ШаблонРабочегоНаименованияХарактеристики",
			"ШаблонРабочегоНаименованияХарактеристикиИсторияПереходаНаНовыеФормулы");
		ОбъектИзменен = Истина;
	КонецЕсли;
...
// Выполняет обработку изменений выражения шаблона наименования
// 
// Параметры:
// 	СправочникОбъект - СправочникОбъект.ВидыНоменклатуры - Обрабатываемый объект.
// 	РеквизитФормула - Строка - Имя реквизита, содержащего изменяемое выражение.
// 	РеквизитФормулаИсторияПерехода - Строка - Имя реквизита истории перехода.
Процедура ОбработатьИзменениеШаблонаСФормулой(СправочникОбъект, РеквизитФормула, РеквизитФормулаИсторияПерехода)
	
	Если ПустаяСтрока(СправочникОбъект[РеквизитФормула]) Тогда
		Возврат;
	КонецЕсли;
	
	Если НЕ ПустаяСтрока(СправочникОбъект[РеквизитФормулаИсторияПерехода]) Тогда
		Возврат;
	КонецЕсли;
	
	ИсходноеВыражение = СправочникОбъект[РеквизитФормула];
	
	Если РеквизитФормула = "ШаблонНаименованияДляПечатиНоменклатуры"
		ИЛИ РеквизитФормула = "ШаблонРабочегоНаименованияНоменклатуры" Тогда
		ТипНазначенияШаблона = Тип("СправочникСсылка.Номенклатура");
		НаборыДопРеквизитовПервогоУровня = Новый Массив;
		НаборыДопРеквизитовПервогоУровня.Добавить(СправочникОбъект.НаборСвойств);
		НаборыДопРеквизитовПервогоУровня.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_Номенклатура_Общие"));
	ИначеЕсли РеквизитФормула = "ШаблонНаименованияДляПечатиХарактеристики"
		ИЛИ РеквизитФормула = "ШаблонРабочегоНаименованияХарактеристики" Тогда
		ТипНазначенияШаблона = Тип("СправочникСсылка.ХарактеристикиНоменклатуры");
		НаборыДопРеквизитовПервогоУровня = Новый Массив;
		НаборыДопРеквизитовПервогоУровня.Добавить(СправочникОбъект.НаборСвойствХарактеристик);
		НаборыДопРеквизитовПервогоУровня.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_ХарактеристикиНоменклатуры_Общие"));
	Иначе
		ТипНазначенияШаблона = Тип("СправочникСсылка.СерииНоменклатуры");
		НаборыДопРеквизитовПервогоУровня = Новый Массив;
		НаборыДопРеквизитовПервогоУровня.Добавить(СправочникОбъект.НаборСвойствСерий);
		НаборыДопРеквизитовПервогоУровня.Добавить(УправлениеСвойствами.НаборСвойствПоИмени("Справочник_СерииНоменклатуры_Общие"));
	КонецЕсли;
	
	РезультатПреобразования = РаботаСФормулами.ПреобразоватьВФорматИдентификаторовВыражениеФормулыВидовНоменклатуры(
		ИсходноеВыражение, ТипНазначенияШаблона, НаборыДопРеквизитовПервогоУровня);
	
	Если НЕ РезультатПреобразования.ЕстьОшибки Тогда
		СправочникОбъект[РеквизитФормулаИсторияПерехода] = ИсходноеВыражение;
		СправочникОбъект[РеквизитФормула] = РезультатПреобразования.ПреобразованноеВыражение;
	Иначе
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияОшибкиЖР,
			...
			РезультатПреобразования.ТекстОшибки);
