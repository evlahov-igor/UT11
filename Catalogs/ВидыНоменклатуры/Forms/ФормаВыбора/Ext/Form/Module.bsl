...
// << 06.10.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
&НаСервере
Функция КР_НастроитьСписок()
	
	ТекстЗапросаДинСписка = Список.ТекстЗапроса;
	ТекстПоиска = "ВидНоменклатурыПереопределяемый.Ссылка,";
	ТекстЗамены = "ВидНоменклатурыПереопределяемый.Ссылка,"
		+ Символы.ПС + Символы.Таб + "ВидНоменклатурыПереопределяемый.Код,";
...
КонецФункции // >> 06.10.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
