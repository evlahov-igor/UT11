...
Функция КР_ЭтоПользовательМагазина(ПользовательСсылка) Экспорт   
		
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ИСТИНА КАК ЭтоПользовательМагазина
	|ИЗ
	|	Справочник.Пользователи КАК Т
	|ГДЕ
	|	Т.Ссылка = &ПользовательСсылка
	|	И Т.Подразделение.КР_Склад.КР_ВидСклада = ЗНАЧЕНИЕ(Перечисление.КР_ВидыСкладов.Магазин)");  
...
КонецФункции // >> 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
