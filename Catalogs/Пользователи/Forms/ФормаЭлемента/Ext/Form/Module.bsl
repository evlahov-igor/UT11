...
Процедура НастроитьФорму(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	// << 17.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1552
	Если Не (УровеньДоступа.АдминистраторСистемы Или УровеньДоступа.ПолныеПрава) Тогда
...
Процедура КР_ПриСозданииНаСервереДополнительно()
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ВыгружатьВКассовоеПО",
		Элементы.ОсновныеСвойства);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ВыгружатьВКассовоеПОПриИзменении");
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_КодПродавца",
		Элементы.ОсновныеСвойства);
		
	// << 14.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1552
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.Служебный", Элементы.ГруппаНаименование);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	// >> 14.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1552
	
	// --> Евлахов Игорь Николаевич (Начало) 30.05.2024
	// Задача #4182
	ПутьКДанным = "Объект.злРежимОсновногоОкна";
	РодительЭлемента = Элементы.СвойстваДополнительные;
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, ПутьКДанным, РодительЭлемента);
	
	// Настройка свойств элемента
	ЭлементФормы.РежимВыбораИзСписка = Истина;
	ЭлементФормы.СписокВыбора.Добавить(0, НСтр("ru = 'Обычный'"));
	ЭлементФормы.СписокВыбора.Добавить(1, НСтр("ru = 'АРМ'"));
	// <-- Евлахов Игорь Николаевич (Конец) 30.05.2024
	
	КР_УправлениеВидимостью();
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь() 
		...
		Элементы.ВходВПрограммуРазрешен.ТолькоПросмотр = Истина;
...
КонецПроцедуры // >> 27.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150

// << 07.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаСервере
Процедура КР_УправлениеВидимостью()
	
	Элементы["КР_КодПродавца"].Видимость = Объект.КР_ВыгружатьВКассовоеПО;
	
КонецПроцедуры // >> 07.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150

// << 07.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаКлиенте
Процедура КР_ВыгружатьВКассовоеПОПриИзменении(Элемент)
	
	Если Не Объект.КР_ВыгружатьВКассовоеПО Тогда
		Объект.КР_КодПродавца = "";
	КонецЕсли;
