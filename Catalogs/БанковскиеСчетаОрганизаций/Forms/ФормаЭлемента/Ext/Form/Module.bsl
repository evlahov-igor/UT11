...
Процедура ИспользуетсяБанкДляРасчетовПриИзменении(Элемент)
	...
	Если ИспользуетсяБанкДляРасчетов Тогда
		Элементы.СтраницаБанкДляРасчетов.Заголовок = НСтр("ru = 'Ввести реквизиты банка вручную...'");
	Иначе
...
Процедура ФорматОбменаПриИзменении(Элемент)
	...
	Если Объект.ФорматОбмена = "1CClientBankExchange" Тогда
		Объект.Кодировка = "Windows"
	Иначе
		Объект.Кодировка = "UTF-8"
	КонецЕсли;
	ДенежныеСредстваКлиентСервер.НастроитьЭлементыФормы(ЭтаФорма, "ФорматОбмена", РеквизитыФормы(ЭтаФорма));
...
Процедура ПриИзмененииБанка()
	...
	Объект.ЭтоIBAN = Объект.ЭтоIBAN
						И Не ЭтотОбъект.СтранаБанка = ПредопределенноеЗначение("Справочник.СтраныМира.Россия");
	...
	Если ЗначениеЗаполнено(Объект.Банк) Тогда
		ДенежныеСредстваКлиентСервер.ЗаполнитьОсновныеРеквизитыБанка(ЭтотОбъект);
	Иначе
		Элементы.СтраницаБанк.Заголовок = НСтр("ru = 'Ввести реквизиты банка вручную...'");
	КонецЕсли;
...
Процедура НомерСчетаПриИзмененииСервер()
	...
	Элементы.ГруппаОшибкаВНомереСчета.Видимость = Ложь;
	ЭтотОбъект.НомерСчета = СтрЗаменить(СтрЗаменить(СокрЛП(ЭтотОбъект.НомерСчета), " ", ""), ".", "");
	Объект.НомерСчета = ЭтотОбъект.НомерСчета;
	...
	Если МеждународныеРеквизитыБанковскихСчетов Тогда
		...
		Если Сред(ЭтотОбъект.НомерСчета, 1, 4) = "IBAN" Тогда
			ЭтотОбъект.НомерСчета = Сред(ЭтотОбъект.НомерСчета, 5);
			Объект.НомерСчета = ЭтотОбъект.НомерСчета;
		КонецЕсли;
...
Функция ПолучитьТекстовыйДокументИзВременногоХранилищаФайла(АдресФайла, Кодировка)
	...
	#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
		Возврат "";
	#Иначе
		Если Объект.ФорматОбмена = "ISO20022" Тогда
			ИмяВременногоФайла = ПолучитьИмяВременногоФайла("xml");
		ИначеЕсли Объект.ФорматОбмена = "1CClientBankExchange" Тогда
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ЭтотОбъект.НомерСчета = Объект.НомерСчета;
...
Процедура НомерСчетаПриИзмененииСерверЛокализация()
	...
	Если (ДенежныеСредстваКлиентСерверЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(Объект.НомерСчета)
		И Не ЗначениеЗаполнено(ЭтотОбъект.СтранаБанка))
		ИЛИ ЭтотОбъект.СтранаБанка = Справочники.СтраныМира.Россия Тогда
...
Процедура Подключаемый_НомерСчетаОкончаниеВводаТекстаЛокализация(
	...
	Текст = СтрЗаменить(СтрЗаменить(СокрЛП(Текст), " ", ""), ".", "");
...
Функция ЭтоБанкРФ()
	...
	Если ((ДенежныеСредстваКлиентСерверЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(Объект.НомерСчета)
		И НЕ ЗначениеЗаполнено(ЭтотОбъект.СтранаБанка)
		И НЕ ЭтотОбъект.МеждународныеРеквизитыБанковскихСчетов)
		ИЛИ ЭтотОбъект.СтранаБанка = ПредопределенноеЗначение("Справочник.СтраныМира.Россия")) Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура УстановитьПризнакИностранныйБанк()
	...
	Если ЭтотОбъект.МеждународныеРеквизитыБанковскихСчетов Тогда
		
		Объект.ИностранныйБанк = НЕ (ЗначениеЗаполнено(ЭтотОбъект.СтранаБанка)
									И ЭтотОбъект.СтранаБанка = ПредопределенноеЗначение("Справочник.СтраныМира.Россия"));
		ИзмененИностранныйБанк = Объект.ИностранныйБанк = Истина;
		Если ИзмененИностранныйБанк Тогда
			ИностранныйБанкПриИзменении();
		КонецЕсли;
