...
Функция НастройкиПолейФормы() Экспорт
	...
	Финансы.НовыйОтбор(Элемент.Условие, "ФорматОбмена", "1CClientBankExchange");
	Элемент.Свойства.Вставить("Видимость");
	
	Элемент = Настройки.Добавить();
	Элемент.Поля.Добавить("ВерсияФорматаВыгрузки");
	Финансы.НовыйОтбор(Элемент.Условие, "ФорматОбмена", "ISO20022");
	Элемент.Свойства.Вставить("Видимость");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("fa5a6d02-4dde-4339-9d3e-837b2154c57d");
	...
	Обработчик.Комментарий = НСтр("ru = 'Определение  IBAN счетов, заполнение формата обмена'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	БанковскиеСчетаОрганизаций.ИностранныйБанк,
	|	БанковскиеСчетаОрганизаций.ЭтоIBAN,
	|	БанковскиеСчетаОрганизаций.ФорматОбмена
	|ИЗ
	|	Справочник.БанковскиеСчетаОрганизаций КАК БанковскиеСчетаОрганизаций
	|ГДЕ
	|	БанковскиеСчетаОрганизаций.ИностранныйБанк
	|	И НЕ БанковскиеСчетаОрганизаций.ЭтоIBAN
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ИностранныйБанк И НЕ Выборка.ЭтоIBAN
			И ДенежныеСредстваКлиентСервер.ВерныйКонтрольныйКлючIBAN(Выборка.НомерСчета) Тогда
			СчетаКОбработке.Добавить(Выборка.Ссылка);
		КонецЕсли;
		Если Выборка.ФорматОбмена = "" Тогда
			СчетаКОбработке.Добавить(Выборка.Ссылка);
		КонецЕсли;
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ОбъектОбработки <> Неопределено Тогда
				ОпределитьСчетВФорматеIBAN(ОбъектОбработки, Записать);
				ЗаполнитьФорматОбмена(ОбъектОбработки, Записать);
			КонецЕсли;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации, УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Справочники.БанковскиеСчетаОрганизаций, , ТекстСообщения);
			Продолжить;
		КонецПопытки;
	КонецЦикла;

	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать некоторые банковские счета организаций (пропущены): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		МетаданныеОбъекта = Метаданные.Справочники.БанковскиеСчетаОрганизаций;
		ШаблонСообщения = НСтр("ru = 'Обработана очередная порция банковских счетов организаций: %1'");
...
Процедура ОпределитьСчетВФорматеIBAN(ОбъектОбработки, Записать)
	
	Если ОбъектОбработки.ЭтоIBAN = Ложь И ОбъектОбработки.ИностранныйБанк Тогда
		Если ДенежныеСредстваКлиентСервер.ВерныйКонтрольныйКлючIBAN(ОбъектОбработки.НомерСчета) Тогда
			ОбъектОбработки.ЭтоIBAN = Истина;
			Записать = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьФорматОбмена(ОбъектОбработки, Записать)
	
	Если ОбъектОбработки.ФорматОбмена = "" Тогда
		ОбъектОбработки.ФорматОбмена = "ISO20022";
		ОбъектОбработки.ВерсияФорматаВыгрузки = "pain.001.001.03";
//++ Локализация
		ОбъектОбработки.ФорматОбмена = "1CClientBankExchange";
		ОбъектОбработки.ВерсияФорматаВыгрузки = "";
//-- Локализация
		Записать = Истина;
	КонецЕсли;
