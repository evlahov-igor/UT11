...
Функция РеквизитыСПереопределеннойОбязательностьюЗаполнения(РеквизитыВидаНоменклатуры, Объект) Экспорт
	...
	РеквизитыСПереопределеннойОбязательностьюЗаполнения.Вставить("КР_Размер",
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Владелец, "ТипНоменклатуры") = Перечисления.ТипыНоменклатуры.Товар);
	// >> 24.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-324
...
Функция ИспользованиеЭлементов(ПереданныйОбъект, РеквизитыВидаНоменклатуры)
	...
	// << 11.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-324
	КР_ИспользованиеЭлементовДополнительно(ПереданныйОбъект,
		НастройкиРеквизитовПоВидуНоменклатуры, ИспользованиеЭлементов);
	// >> 11.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-324
...
// << 11.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-324
Процедура КР_ИспользованиеЭлементовДополнительно(
				ПереданныйОбъект,
				НастройкиРеквизитовПоВидуНоменклатуры,
				ИспользованиеЭлементов)
	
	МассивРеквизитовДляУстановкиВидимости = КР_МассивРеквизитовДляУстановкиВидимостиТипТовар();
	ТипНоменклатурыВладельца =
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПереданныйОбъект.Владелец, "ТипНоменклатуры");
	ЭтоХарактеристикаТовара = (ТипНоменклатурыВладельца = Перечисления.ТипыНоменклатуры.Товар);
	
	// #4304.. Фомин Д.Ю. 21.06.2024.
	//Для Каждого СтрТабл Из НастройкиРеквизитовПоВидуНоменклатуры Цикл
	//	ИмяРеквизита = СтрТабл.ИмяРеквизита;
	//	Если Не СтрТабл.ЭтоДопРеквизит Тогда
	//		Если Не МассивРеквизитовДляУстановкиВидимости.Найти(ИмяРеквизита) = Неопределено Тогда
	//			Использование = СтрТабл.Использование И ЭтоХарактеристикаТовара;
	//			ИспользованиеЭлементов.Вставить(ИмяРеквизита, Использование);
	//			
	//			НастройкиРеквизитовПоВидуНоменклатуры.Удалить(СтрТабл);
	//		КонецЕсли;
	//	КонецЕсли;
	//КонецЦикла;
	...
	Пока Сч < НастройкиРеквизитовПоВидуНоменклатуры.Количество() Цикл
		
		СтрТабл = НастройкиРеквизитовПоВидуНоменклатуры[Сч];
		ИмяРеквизита = СтрТабл.ИмяРеквизита;
		Если Не СтрТабл.ЭтоДопРеквизит 
			И Не МассивРеквизитовДляУстановкиВидимости.Найти(ИмяРеквизита) = Неопределено Тогда
			Использование = СтрТабл.Использование И ЭтоХарактеристикаТовара;
			ИспользованиеЭлементов.Вставить(ИмяРеквизита, Использование);
...
КонецПроцедуры // >> 11.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-324
...
Функция КР_МассивРеквизитовДляУстановкиВидимостиТипТовар()
	...
	МассивРеквизитов.Добавить("КР_Размер");
	МассивРеквизитов.Добавить("КР_КодТНВЭД");

	// << 09.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1802
	МассивРеквизитов.Добавить("КР_Штрихкод");
	// >> 09.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1802
