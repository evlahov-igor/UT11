...
Процедура КР_ПриСозданииНаСервереДополнительно(Отказ, СтандартнаяОбработка)
	...
	Если КР_НастроитьСписок() Тогда
		КР_ШтрихкодЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
			ЭтотОбъект, "Список.КР_Штрихкод", Элементы.Список);
	КонецЕсли;
...
Функция КР_НастроитьСписок()
	...
	ТекстПоискаПоле = "СправочникХарактеристикиНоменклатуры.Ссылка,";
	ТекстЗаменыПоле = "СправочникХарактеристикиНоменклатуры.КР_Штрихкод,
		|	СправочникХарактеристикиНоменклатуры.Ссылка,";
	
	Если СтрНайти(ТекстЗапроса, ТекстПоискаПоле) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ТекстПоискаПоле, ТекстЗаменыПоле);
		
		Список.ТекстЗапроса = ТекстЗапроса;
	Иначе
		ОбщегоНазначения.СообщитьПользователю(
			НСтр("ru = 'Текст запроса был изменен ранее.'"), , , , Отказ);
	КонецЕсли;
	
	Возврат Не Отказ;
	
КонецФункции // >> 04.10.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-619
