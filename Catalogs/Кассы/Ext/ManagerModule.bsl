...
Функция ПолучитьРеквизитыКассы(Касса) Экспорт
	...
	|	Кассы.Подразделение КАК Подразделение		// #4385.. Фомин Д.Ю. 26.07.2024.
	|ИЗ
	|	Справочник.Кассы КАК Кассы
	|ГДЕ
	|	Кассы.Ссылка = &Касса
	...
	Если Выборка.Следующий() Тогда
		...
		Подразделение = Выборка.Подразделение;								// #4385.. Фомин Д.Ю. 26.07.2024.
	Иначе
		Организация = Справочники.Организации.ПустаяСсылка();
		Валюта = Справочники.Валюты.ПустаяСсылка();
		РазрешитьПлатежиБезУказанияЗаявок = Ложь;
		Подразделение = Справочники.СтруктураПредприятия.ПустаяСсылка();	// #4385.. Фомин Д.Ю. 26.07.2024.
	КонецЕсли;
	...
	СтруктураРеквизитов = Новый Структура("Организация, Валюта, РазрешитьПлатежиБезУказанияЗаявок, Подразделение",
		Организация,
		Валюта,
		РазрешитьПлатежиБезУказанияЗаявок,
		Подразделение);
