...
Процедура ТипОборудованияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	// << 02.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	Элементы["КР_ПравилоОбменаПользователи"].Видимость = Элементы.ПравилоОбмена.Видимость;
	// >> 02.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ПравилоОбменаПользователи", , Элементы.ГруппаККТ);
	ЭлементФормы.Видимость = Элементы.ПравилоОбмена.Видимость;
	
	КР_УстановитьПараметрыВыбора();
	
КонецПроцедуры // >> 02.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150

// << 02.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаСервере
Процедура КР_УстановитьПараметрыВыбора()
	
	НовыйПараметр = Новый ПараметрВыбора("Отбор.КР_ОбменПользователями", Ложь);
	
	НовыйМассив = Новый Массив();
	НовыйМассив.Добавить(НовыйПараметр);
	НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
	Элементы.ПравилоОбмена.ПараметрыВыбора = НовыеПараметры;
