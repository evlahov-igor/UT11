...
#Область КР_ПервоначальноеЗаполнение
...
Процедура КР_ПервоначальноеЗаполнениеЛокализованныхДанных(ТекущаяВерсияЛокализации, НоваяВерсияЛокализации) Экспорт
	...
	Если Версия > ТекущаяВерсияЛокализации 
		...
		// На основе данных модуля менеджера справочниика ВидыКонтактнойИнформации.КР_ПриНачальномЗаполненииЭлементов() 
		#Область КР_ПриНачальномЗаполненииЭлементов
		
		Элемент													= Новый Структура;
		Элемент.Вставить("ИмяПредопределенныхДанных",			"КР_ТелефонАдминистратора");
		Элемент.Вставить("Тип",									Перечисления.ТипыКонтактнойИнформации.Телефон);
		Элемент.Вставить("Наименование",						НСтр("ru = 'Телефон администратора'"));
		Элемент.Вставить("МожноИзменятьСпособРедактирования",	Истина);
		Элемент.Вставить("РазрешитьВводНесколькихЗначений",		Истина);
		Элемент.Вставить("Родитель",							Справочники.ВидыКонтактнойИнформации.СправочникСклады);
		Элемент.Вставить("ИдентификаторДляФормул",				"КР_ТелефонАдминистратора");
		Элемент.Вставить("Используется",						Истина);
		Элемент.Вставить("ВидРедактирования",					"ПолеВвода");
		НовыеЭлементы.Добавить(Элемент);
		
		Элемент													= Новый Структура;
		Элемент.Вставить("ИмяПредопределенныхДанных",			"КР_ТелефонУправляющего");
		Элемент.Вставить("Тип",									Перечисления.ТипыКонтактнойИнформации.Телефон);
		Элемент.Вставить("Наименование",						НСтр("ru = 'Телефон управляющего'"));
		Элемент.Вставить("МожноИзменятьСпособРедактирования",	Истина);
		Элемент.Вставить("РазрешитьВводНесколькихЗначений",		Ложь);
		Элемент.Вставить("Родитель",							Справочники.ВидыКонтактнойИнформации.СправочникСклады);
		Элемент.Вставить("ИдентификаторДляФормул",				"КР_ТелефонУправляющего");
		Элемент.Вставить("Используется",						Истина);
		Элемент.Вставить("ВидРедактирования",					"ПолеВвода");
		НовыеЭлементы.Добавить(Элемент);
		#КонецОбласти
	...
	// << 16.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1294   
	Версия = 1067;
	Если Версия > ТекущаяВерсияЛокализации 
		И Версия <= НоваяВерсияЛокализации Тогда  
		
		ИмяПредопределенныхДанных = "КР_ЭлектроннаяПочтаСклада";
		Элемент = Новый Структура;
		Элемент.Вставить("ИмяПредопределенныхДанных", ИмяПредопределенныхДанных);
		Элемент.Вставить("Тип", Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
		Элемент.Вставить("Наименование", НСтр("ru = 'Электронная почта склада'"));
		Элемент.Вставить("МожноИзменятьСпособРедактирования", Истина);
		Элемент.Вставить("РазрешитьВводНесколькихЗначений", Истина);
		Элемент.Вставить("Родитель", Справочники.ВидыКонтактнойИнформации.СправочникСклады);
		Элемент.Вставить("ИдентификаторДляФормул", ИмяПредопределенныхДанных);
		Элемент.Вставить("Используется", Истина);
		Элемент.Вставить("ВидРедактирования", "ПолеВвода");
		НовыеЭлементы.Добавить(Элемент);                                     
	...
	Если Версия > ТекущаяВерсияЛокализации 
		...
		ИмяПредопределенныхДанных = "злФактАдресСклада";
		
		Элемент = Новый Структура;
		Элемент.Вставить("ИмяПредопределенныхДанных", ИмяПредопределенныхДанных);
		Элемент.Вставить("Тип", Перечисления.ТипыКонтактнойИнформации.Адрес);
		Элемент.Вставить("Наименование", НСтр("ru = 'Фактический адрес склада'"));
		Элемент.Вставить("МожноИзменятьСпособРедактирования", Истина);
		Элемент.Вставить("РазрешитьВводНесколькихЗначений", Ложь);
		Элемент.Вставить("Родитель", Справочники.ВидыКонтактнойИнформации.СправочникСклады);
		Элемент.Вставить("ИдентификаторДляФормул", ИмяПредопределенныхДанных);
		Элемент.Вставить("Используется", Истина);
		Элемент.Вставить("ВидРедактирования", "ПолеВводаИДиалог");
	...
	Для Каждого Элемент Из НовыеЭлементы Цикл
		...
		Если Не КР_ВидКонтактнойИнформацииУжеСуществует(Элемент) Тогда
			ВидИнформацииОбъект = КР_ПолучитьОбъект(Элемент);
			ЗаполнитьЗначенияСвойств(ВидИнформацииОбъект, Элемент);
			ВидИнформацииОбъект.Записать();
		КонецЕсли;
...
// << 12.12.2022, Федоров Д.Е., КРОК, Jira№ A2105505-872
Функция КР_ПолучитьОбъект(Элемент)
	...
	Запрос.УстановитьПараметр("ИмяПредопределенныхДанных",	Элемент.ИмяПредопределенныхДанных);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		РезультатОбъект = Выборка.Ссылка.ПолучитьОбъект();
	Иначе
		РезультатОбъект = Справочники.ВидыКонтактнойИнформации.СоздатьЭлемент();
	КонецЕсли;
...
// << 12.12.2022, Федоров Д.Е., КРОК, Jira№ A2105505-872
Функция КР_ВидКонтактнойИнформацииУжеСуществует(Элемент)
	...
	Запрос.Текст = 
		...
		|	И ВидыКонтактнойИнформации.ИмяПредопределенныхДанных = &ИмяПредопределенныхДанных
		|	И НЕ ВидыКонтактнойИнформации.ПометкаУдаления";
	
	Запрос.УстановитьПараметр("ИмяПредопределенныхДанных",	Элемент.ИмяПредопределенныхДанных);
	Запрос.УстановитьПараметр("Родитель",					Элемент.Родитель);
	Запрос.УстановитьПараметр("Тип",						Элемент.Тип);
	
	РезультатЗапроса = Запрос.Выполнить();
	ВидКонтактнойИнформацииУжеСуществует = Не РезультатЗапроса.Пустой();
	Возврат ВидКонтактнойИнформацииУжеСуществует;
