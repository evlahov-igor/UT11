...
Процедура ТипПодключаемогоОборудованияПриИзменении(Элемент)
	...
	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаОтборИзменен;
	// << 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	ККМОфлайн = ПредопределенноеЗначение("Перечисление.ТипыПодключаемогоОборудования.ККМОфлайн");
	ВидимостьЭлемента = Объект.ТипПодключаемогоОборудования = ККМОфлайн;
	Элементы["КР_ОбменПользователями"].Видимость = ВидимостьЭлемента;
	Если Не ВидимостьЭлемента Тогда
		Объект.КР_ОбменПользователями = Ложь;
	КонецЕсли;
	// >> 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	Если Объект.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ККМОфлайн Тогда
		// << 20.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
		//СхемаКомпоновкиДанных = Справочники.ПравилаОбменаСПодключаемымОборудованиемOffline.ПолучитьМакет("ОбновлениеКодовТоваровККМ");
		Если КР_ИнтеграцияСВнешнимиСистемами.КР_ЭтоArtix(Объект.Ссылка) Тогда
			СхемаКомпоновкиДанных =
				Справочники.ПравилаОбменаСПодключаемымОборудованиемOffline.ПолучитьМакет("КР_ОбновлениеКодовТоваровККМ");
		Иначе
			СхемаКомпоновкиДанных =
				Справочники.ПравилаОбменаСПодключаемымОборудованиемOffline.ПолучитьМакет("ОбновлениеКодовТоваровККМ");
		КонецЕсли;
		// >> 20.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
...
Процедура Настроить()
	...
	Если Объект.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ККМОфлайн Тогда
		...
		Если Объект.КР_ОбменПользователями Тогда
			СхемаКомпоновкиДанных =
				Справочники.ПравилаОбменаСПодключаемымОборудованиемOffline.ПолучитьМакет("КР_ВыгрузкаКассиры");
		КонецЕсли;
		// >> 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	ИначеЕсли Объект.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток Тогда
		СхемаКомпоновкиДанных = Справочники.ПравилаОбменаСПодключаемымОборудованиемOffline.ПолучитьМакет("ОбновлениеPLUКодовТоваров");
	...
	// << 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	ЕстьРеквизит = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "КР_СменаОбмена");
	ЗначениеИзОбъекта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "КР_ОбменПользователями");
	
	Если ЕстьРеквизит И ЭтотОбъект["КР_СменаОбмена"] И Объект.КР_ОбменПользователями <> ЗначениеИзОбъекта Тогда
		СсылкаДляСчитыванияНастроек = Справочники.ПравилаОбменаСПодключаемымОборудованиемOffline.ПустаяСсылка();
	КонецЕсли;
	// >> 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	...
	// << 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	ВидимостьЭлементов = Не Объект.КР_ОбменПользователями;
		
	Элементы.ГруппаГоризонтально.Видимость 	= ВидимостьЭлементов;
	Элементы.Страницы.Видимость 			= ВидимостьЭлементов;
	Элементы.ВыгружатьИзменения.Видимость 	= ВидимостьЭлементов;
	// >> 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
...
Функция ТаблицаТоваров(Товары, ВидЦены)
	...
	Если КР_ИнтеграцияСВнешнимиСистемами.КР_ЭтоArtix(Объект.Ссылка) Тогда
		Запрос.Текст = КР_ТекстЗапросаArtix();
...
Процедура ЗагрузитьКодыИзРегистра(ТекущийОбъект)
	...
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		НастройкиАссортимента = РегистрыСведений.ИсторияИзмененияФорматовМагазинов.ТекущийНастройкиАссортимента(ТекущийОбъект.Склад);
		...
		Иначе
			ВидыЦен.РозничныйВидЦены = ТекущийОбъект.Склад.РозничныйВидЦены;
			ВидыЦен.ИндивидуальныйВидЦены = ТекущийОбъект.Склад.ИндивидуальныйВидЦены;
		КонецЕсли;
	Иначе
		ВидыЦен.РозничныйВидЦены = ТекущийОбъект.Склад.РозничныйВидЦены;
	КонецЕсли;
	
	Таблица = ПодключаемоеОборудованиеOfflineВызовСервера.ДанныеТоваровПоПравилуОбмена(ТекущийОбъект.Ссылка, ВидыЦен.РозничныйВидЦены, ВидыЦен.ИндивидуальныйВидЦены);
...
Процедура ОбновитьСписокТоваровНаСервереПовторноеОткрытие()
	...
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		...
		Если НастройкиАссортимента.РозничныеЦеныИзФорматаМагазина Тогда
			...
			ВидыЦен.ИндивидуальныйВидЦены = РеквизитыОбъекта.ИндивидуальныйВидЦены;
		Иначе
			ВидыЦен.РозничныйВидЦены = Объект.Склад.РозничныйВидЦены;
			ВидыЦен.ИндивидуальныйВидЦены = Объект.Склад.ИндивидуальныйВидЦены;
		КонецЕсли;
	Иначе
		ВидыЦен.РозничныйВидЦены = Объект.Склад.РозничныйВидЦены;
	КонецЕсли;
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ОбменПользователями", , Элементы.ГруппаГоризонтально);
	ЭлементФормы.ВидФлажка = ВидФлажка.Тумблер;
	ЭлементФормы.ФорматРедактирования = "БИ=Пользователи; БЛ='Товары'";
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ОбменПользователямиПриИзменении");
	
	ВидимостьЭлемента = Объект.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ККМОфлайн;
	ЭлементФормы.Видимость = ВидимостьЭлемента;
	
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект, "КР_СменаОбмена", Новый ОписаниеТипов("Булево"));
	
КонецПроцедуры // >> 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150

// << 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаКлиенте
Процедура КР_ОбменПользователямиПриИзменении(Элемент)
	
	ЭтотОбъект["КР_СменаОбмена"] = Истина;
	Настроить();
	ЭтотОбъект["КР_СменаОбмена"] = Ложь;
	
КонецПроцедуры // >> 01.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150

// << 21.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаСервереБезКонтекста
Функция КР_ТекстЗапросаArtix()
	...
	|	КодыТоваров.ИзмененаАвтоматически КАК ИзмененаАвтоматически,
	|	КодыТоваров.Используется КАК Используется,
	|	ВЫРАЗИТЬ(КодыТоваров.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	ВЫРАЗИТЬ(КодыТоваров.Характеристика КАК Справочник.ХарактеристикиНоменклатуры) КАК Характеристика,
	|	КодыТоваров.Упаковка КАК Упаковка
	|ПОМЕСТИТЬ КодыТоваров
	|ИЗ
	|	&ТаблицаЗначений КАК КодыТоваров
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПравилаОбменаСПодключаемымОборудованиемOffline.Склад КАК Склад
	|ПОМЕСТИТЬ втСклад
	|ИЗ
	|	Справочник.ПравилаОбменаСПодключаемымОборудованиемOffline КАК ПравилаОбменаСПодключаемымОборудованиемOffline
	|ГДЕ
	...
	|					втСклад.Склад КАК Склад
	|				ИЗ
	|					втСклад КАК втСклад)) КАК ИсторияИзмененияФорматовМагазиновСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	КодыТоваров.Код КАК Код,
	|	КодыТоваров.Номенклатура КАК Номенклатура,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(КодыТоваров.Номенклатура КАК Справочник.Номенклатура).Наименование, """") КАК НоменклатураНаименование,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(КодыТоваров.Номенклатура КАК Справочник.Номенклатура).НаименованиеПолное, """") КАК НоменклатураНаименованиеПолное,
	|	КодыТоваров.Характеристика КАК Характеристика,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(КодыТоваров.Характеристика КАК Справочник.ХарактеристикиНоменклатуры).Наименование, """") КАК ХарактеристикаНаименование,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(КодыТоваров.Характеристика КАК Справочник.ХарактеристикиНоменклатуры).НаименованиеПолное, """") КАК ХарактеристикаНаименованиеПолное,
	|	КодыТоваров.Упаковка КАК Упаковка,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(КодыТоваров.Упаковка КАК Справочник.УпаковкиЕдиницыИзмерения).Наименование, """") КАК УпаковкаНаименование,
	|	ЕСТЬNULL(ШтрихкодыНоменклатуры.Штрихкод, """") КАК Штрихкод,
	|	ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) * ЕСТЬNULL(втЦеныОсновные.Цена, 0) КАК Цена,
	|	ВЫБОР
	|		КОГДА &ТекстЗапросаТипИзмеряемойВеличиныЕдиницыИзмерения = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|		ПО КодыТоваров.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|			И КодыТоваров.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ втЦеныОсновные КАК втЦеныОсновные
	|		ПО КодыТоваров.Номенклатура = втЦеныОсновные.Номенклатура
	|			И КодыТоваров.Характеристика = втЦеныОсновные.Характеристика
	|ИТОГИ
	|	МАКСИМУМ(Штрихкод)
	|ПО
	|	Код";
