...
Процедура ПриЗаписи(Отказ)
	...
	ПутьХраненияВерсийВнешнихОбработок = Константы.злПутьХраненияВерсийВнешнихОбработок.Получить();
	РезультатЗаписи = Справочники.ДополнительныеОтчетыИОбработки.злЗаписатьВерсиюВнешнейОбработки(Ссылка, Вид, ИмяОбъекта, ХранилищеОбработки, ПутьХраненияВерсийВнешнихОбработок); 
	Если Не РезультатЗаписи.Результат Тогда
		ОбщегоНазначения.СообщитьПользователю(РезультатЗаписи.Сообщение,,,,Отказ);
	КонецЕсли;	
	// ..#3181
