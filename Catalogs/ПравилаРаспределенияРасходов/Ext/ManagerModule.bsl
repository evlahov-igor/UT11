...
Функция КР_ПолучитьПравилоРаспределенияКоллекции(ПравилоРаспределенияБазовое, Коллекция) Экспорт 
	
	Если ЗначениеЗаполнено(ПравилоРаспределенияБазовое.КР_БазовоеПравило) Тогда  
		ТекстОшибки = НСтр("ru = 'В качестве базового правила передано правило коллекции'");
		ВызватьИсключение ТекстОшибки;            
	КонецЕсли;	
		
	Запрос = Новый Запрос(КР_ТекстЗапросаПолучитьПравилоРаспределенияКоллекции());
	Запрос.УстановитьПараметр("ПравилоРаспределенияБазовое", ПравилоРаспределенияБазовое);
	...
	Иначе	
		ЭлементОбъект = ПравилоРаспределенияБазовое.Скопировать();
	КонецЕсли;
	
	ШаблонНаименования = "%1 (%2)";
	ЭлементОбъект.КР_БазовоеПравило = ПравилоРаспределенияБазовое;
	ЭлементОбъект.КР_Коллекция = Коллекция;  
	ЭлементОбъект.Наименование = СтрШаблон(ШаблонНаименования,
		ПравилоРаспределенияБазовое.Наименование, Коллекция);  
		
	НастройкиКомпоновкиДанных = ПравилоРаспределенияБазовое.НастройкиБазыРаспределенияПоПартиям.Получить();
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(НастройкиКомпоновкиДанных,
		"Номенклатура.КоллекцияНоменклатуры", Коллекция, ВидСравненияКомпоновкиДанных.Равно, Истина);
	
	ЭлементОбъект.НастройкиБазыРаспределенияПоПартиям = Новый ХранилищеЗначения(НастройкиКомпоновкиДанных);
	ЭлементОбъект.НастройкиБазыРаспределенияПоПартиямИзменены = Истина;
...
&НаСервере
Функция КР_ТекстЗапросаПолучитьПравилоРаспределенияКоллекции()
	...
	|ГДЕ
	|	Т.КР_БазовоеПравило = &ПравилоРаспределенияБазовое
