...
Процедура СписокПриАктивизацииЯчейки(Элемент)
	...
	СопоставлениеЗакладок = ПолучитьСопостовлениеЗакладок("ОбластьДействия" , Элемент.ТекущийЭлемент.Имя);
	Если СопоставлениеЗакладок = Неопределено Тогда
		Возврат;
	КонецЕсли; 
...
Процедура Подключаемый_СписокПриАктивизацииСтроки()
	...
	Если ДанныеПоВидуЦен.ВидЦен <> АктивизированныйВидЦен Тогда
...
Процедура СписокПриАктивизацииСтрокиНаСервере(ДанныеПоВидуЦен)
	...
	Элементы.НадписьВФорматахМагазинов.Доступность  = ДоступностьФорматовМагазинов;
	...
	Если ДанныеПоВидуЦен.Назначение = Перечисления.НазначенияВидовЦен.Общий Тогда	
...
Процедура ЗаполнениеКомандногоМенюВЗакладках()
	...
	Для каждого Элемент Из СопоставлениеЗакладокСоСписками Цикл
		...
		Если Элемент.ИмяЗакладки = Элементы.ГруппаСклады.Имя Тогда
			...
			ЭлементПодменюВидЦены.Заголовок = "Розничные";
			...
			ЭлементПодменюДействует.Заголовок = "Установить";
			...
			ЭлементПодменюНеДействует.Заголовок = "Очистить";
			...
			ЭлементПодменюВидЦены = Элементы.Добавить(ИмяЭлемента, Тип("ГруппаФормы"),ЭлементПодменюУстановить);
			ЭлементПодменюВидЦены.Вид = ВидГруппыФормы.Подменю;
			ЭлементПодменюВидЦены.Заголовок = "Учетные";
...
Функция ТекстЗапросаДинамическогоСписка()
	...
	|	СправочникВидыЦен.ИспользоватьПриРозничнойПродаже КАК ИспользоватьПриРозничнойПродаже,
	...
	|	СправочникВидыЦен.Назначение КАК Назначение,
	|	ВЫБОР
	|		КОГДА СправочникВидыЦен.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияВидовЦен.Общий)
	|			ТОГДА 0
	|		КОГДА СправочникВидыЦен.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияВидовЦен.ИндивидуальныйДляСклада)
	|			ТОГДА 1
	|		КОГДА СправочникВидыЦен.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияВидовЦен.ИндивидуальныйДляФорматаМагазинов)
	|			ТОГДА 2
	|		КОГДА СправочникВидыЦен.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияВидовЦен.ИндивидуальныйДляПартнера)
	|			ТОГДА 3
	|		КОГДА СправочникВидыЦен.Назначение = ЗНАЧЕНИЕ(Перечисление.НазначенияВидовЦен.ИндивидуальныйДляСоглашения)
	|			ТОГДА 4
	|		ИНАЧЕ -1
	|	КОНЕЦ КАК ИндексКартинкиНазначение,
	|	СправочникВидыЦен.ВспомогательнаяЦена КАК ВспомогательнаяЦена,
	|	ВЫБОР
	|		КОГДА СправочникВидыЦен.ЭтоГруппа
	|			ТОГДА -1
	|		КОГДА СправочникВидыЦен.ВспомогательнаяЦена
	|			ТОГДА 0
	|		ИНАЧЕ 1	
	|	КОНЕЦ КАК ИндексКартинкиВспомогательнаяЦена,	
	|	ВЫБОР
	|		КОГДА СправочникВидыЦен.ЭтоГруппа
	|			ТОГДА 3
	...
	ДобавитьВЗапросТекстИнформациюПоФорматамМагазинов(ЗапросТекст);
	ДобавитьВЗапросТекстИнформациюПоПартнерам(ЗапросТекст);
	ДобавитьВЗапросТекстИнформациюПоСоглашениям(ЗапросТекст);
...
&НаСервере
Процедура ДобавитьВЗапросТекстИнформациюПоФорматамМагазинов(ЗапросТекст)
...
&НаСервере
Процедура ДобавитьВЗапросТекстИнформациюПоПартнерам(ЗапросТекст)
...
&НаСервере
Процедура ДобавитьВЗапросТекстИнформациюПоСоглашениям(ЗапросТекст)
...
Процедура ПереопределитьСвойстваОсновногоСписка()
	...
	СвойстваСписка.ОсновнаяТаблица = "Справочник.ВидыЦен";
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_НаборКонстант" Тогда
		Если Источник =	"ИспользоватьФорматыМагазинов" 
			...
			Или Источник =	"ИспользоватьПроизводствоИзДавальческогоСырья" Тогда
...
Процедура СформироватьИнформациюОКоличествеИспользуемыхВидовЦен(ДанныеПоВидуЦен)
	...
	Если Не ДанныеПоВидуЦен = Неопределено Тогда
		...
		Если ДанныеПоВидуЦен.КоличествоСкладовВсего > 0 Тогда
			...
			Элементы.ДекорацияНаСкладах.Заголовок = СтрШаблон(ШаблонЗаголовка, 
		...
		Если ДанныеПоВидуЦен.КоличествоФорматовМагазиновВсего > 0 Тогда
			...
			Элементы.ДекорацияВФорматахМагазинов.Заголовок = СтрШаблон(ШаблонЗаголовка, 
		...
		Если ДанныеПоВидуЦен.КоличествоПартнеровВсего > 0 Тогда
			...
			Элементы.ДекорацияВПартнерах.Заголовок = СтрШаблон(ШаблонЗаголовка, 
		...
		Если ДанныеПоВидуЦен.КоличествоСоглашенийВсего > 0 Тогда
			...
			Элементы.ДекорацияВСоглашениях.Заголовок = СтрШаблон(ШаблонЗаголовка, 
