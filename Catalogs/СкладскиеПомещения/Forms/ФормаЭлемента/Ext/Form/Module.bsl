...
Процедура НастройкаАдресногоХраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипСклада <> ПредопределенноеЗначение("Перечисление.ТипыСкладов.РозничныйМагазин") 
		// << 16.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
		Или ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Владелец, "КР_АдресныйСкладБезордернойОтгрузки")
		// >> 16.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
		Тогда
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТипСкладскогоПомещения",	, Элементы.ГруппаНастройкаАдресногоХранения);
