...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("bba3f2e4-3ecc-415d-9c10-afb48d79d7ca");
	...
	Обработчик.Комментарий = НСтр("ru = 'Определение IBAN счетов'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Справочник.БанковскиеСчетаКонтрагентов КАК БанковскиеСчетаКонтрагентов
	|ГДЕ
	|	БанковскиеСчетаКонтрагентов.ИностранныйБанк
	|	И НЕ БанковскиеСчетаКонтрагентов.ЭтоIBAN";
	
	Выборка = Запрос.Выполнить().Выбрать();
	СчетаКОбработке = Новый Массив;
	
	Пока Выборка.Следующий() Цикл
		Если ДенежныеСредстваКлиентСервер.ВерныйКонтрольныйКлючIBAN(Выборка.НомерСчета) Тогда
