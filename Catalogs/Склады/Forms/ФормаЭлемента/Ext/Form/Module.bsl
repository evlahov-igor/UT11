...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-434     
	// Данный блок кода перенесен ниже после вызова метода ПриЧтенииСозданииНаСервере
	//  так как вложенные в него вызовы конфликтуют при использовании программно добавленных элементов формы
	...
	// << 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-434
	// Это блок типового кода. см. выше
	Если Параметры.Свойство("ЗначенияЗаполнения") И Параметры.ЗначенияЗаполнения.Свойство("ТипСклада") Тогда
		ТипаСкладаПриИзмененииСервер();
	КонецЕсли;
	// >> 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-434
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// << 08.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-777
...
Процедура ИспользоватьОрдернуюСхемуПриОтгрузкеПриИзменении(Элемент)
	...
	Если Объект.ИспользоватьОрдернуюСхемуПриОтгрузке Тогда 
		Объект.КР_СкладСПомещениямиБезордернойОтгрузки = Ложь;
	КонецЕсли;	
	// >> 16.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751	
...
Процедура ИспользоватьСкладскиеПомещенияПриИзменении(Элемент)
	...
	// << 16.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	Если Объект.ИспользоватьСкладскиеПомещения Тогда 
		Если Не Объект.ИспользоватьОрдернуюСхемуПриОтгрузке Тогда 
			Объект.КР_СкладСПомещениямиБезордернойОтгрузки = Истина;  
		КонецЕсли;	
	Иначе
		Объект.КР_СкладСПомещениямиБезордернойОтгрузки = Ложь;
	КонецЕсли;	
...
Процедура НастройкаАдресногоХраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	Если Объект.КР_АдресныйСкладБезордернойОтгрузки 
		И Объект.ИспользоватьСкладскиеПомещения Тогда 
		ДанныеВыбора.Добавить(
			ПредопределенноеЗначение("Перечисление.НастройкиАдресногоХранения.ОпределяетсяНастройкамиПомещения"));   		
	КонецЕсли;	
	// >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	...
	// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	ИначеЕсли Объект.КР_АдресныйСкладБезордернойОтгрузки Тогда 
		ДанныеВыбора.Добавить(ПредопределенноеЗначение("Перечисление.НастройкиАдресногоХранения.ЯчейкиОстатки"));
	// >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// << 25.08.2022, Мельников А.А., КРОК, Jira№ A2105505-291 
	// Для добавления новых реквизитов
...
Процедура УстановитьДоступностьПолей()
	...
	// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	КР_УстановитьДоступностьЭлементовНастройкиРазмещенияОтбораНаСкладе();
	// >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	
	// << 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
	КР_УстановитьВидимости();
	// >> 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
...
Процедура ОбновитьИнформациюОФорматеМагазина()
	...
	// << 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
	КР_ОбновитьИнформациюОФорматеМагазинаДополнительно();
	// >> 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
...
Процедура ОбновитьПредставленияиВидимостьВидовЦенПоФорматуМагазина()
	...
	// На форму программно добавлены новые реквизиты. Они зависят от типа магазина
	КР_УстановитьВидимости();
	// >> 15.08.2022, Мельников А.А., КРОК, Jira№ A2105505-291 
...
Процедура КР_ПослеЗаписиНаСервереДополнительно(ТекущийОбъект, ПараметрыЗаписи)
	...
	Список = ЭтотОбъект.КР_ФорматыМагазина;
	Список.Параметры.УстановитьЗначениеПараметра("Склад", Объект.Ссылка);
	
	Список = ЭтотОбъект.КР_РеквизитыАренды;
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Список.Отбор, "Склад", Объект.Ссылка);
	
КонецПроцедуры // >> 08.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-777
...
Процедура КР_ДобавитьЭлементыНаФорму() 
	...
	// 30.08.2022 Марченко С.Н., КРОК. 
	// Рефакторинг: переписано под использование методов общего модуля
	//	плюс группа в которой размещены элементы вынесена отдельно 
		
	// КР_ГруппаНастройкиРозничногоМагазина 
	ГруппаНастройкиРозничногоМагазина = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаНастройкиРозничногоМагазина", Элементы.ОсновноеПраво); 
	ГруппаНастройкиРозничногоМагазина.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаНастройкиРозничногоМагазина.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаНастройкиРозничногоМагазина.Заголовок = НСтр("ru = 'Настройки розничного магазина'");	
	
	// << 31.08.2022 Марченко С.Н., КРОК, JIRA№A2105505-434 
	
	// Код 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Код", Элементы.ГруппаШапкаЛево);
	
	Элементы.ГруппаШапкаЛево.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	
	// КР_УчаствуетВПервоначальномРаспределении 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_УчаствуетВПервоначальномРаспределении", ГруппаНастройкиРозничногоМагазина);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	// >> 31.08.2022 Марченко С.Н., КРОК, JIRA№A2105505-434
	
	// << 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148    
	// КР_РазрешитьУвеличиватьЗамороженнуюМатрицу
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_РазрешитьУвеличиватьЗамороженнуюМатрицу", ГруппаНастройкиРозничногоМагазина);	 
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	// >> 24.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148       
	
	// КР_НомерМагазина 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_НомерМагазина", ГруппаНастройкиРозничногоМагазина);	 
	
	// << 09.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1292
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_СпецРаспределение", ГруппаНастройкиРозничногоМагазина);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_СтритРитейл", ГруппаНастройкиРозничногоМагазина);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТипПротивокражныхВорот", ГруппаНастройкиРозничногоМагазина);
	// >> 09.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1292
	
	// КР_Закрыт 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Закрыт", ГруппаНастройкиРозничногоМагазина);	
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;     
	
	// << 02.02.2023 Федотов А.М., КРОК, Jira№A2105505-1188
	// КР_ГруппаНастройкиЯчеекХранения
	ГруппаНастройкиЯчеекХранения = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаНастройкиЯчеекХранения", , Элементы.ГруппаДополнительныеНастройкиАдресныхСкладов); 
	ГруппаНастройкиЯчеекХранения.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаНастройкиЯчеекХранения.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаНастройкиЯчеекХранения.Заголовок = НСтр("ru = 'Настройки ячеек хранения'");	
	
	// КР_ЯчейкаХраненияФорменнойОдежды 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ЯчейкаХраненияФорменнойОдежды", ГруппаНастройкиЯчеекХранения);  
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 25;
	
	// КР_ЯчейкаХраненияБракованногоТовара 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ЯчейкаХраненияБракованногоТовара", ГруппаНастройкиЯчеекХранения);	
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 25;
	// >> 02.02.2023 Федотов А.М., КРОК, Jira№A2105505-1188
	
	// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	КР_ДобавитьЭлементыФормыНастройкиРазмещенияОтбораНаСкладе();
	// >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	
	// << 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
	КР_ДобавитьЭлементыФормыФорматМагазина();
	// >> 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
	
	// << 08.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-777
	КР_ДобавитьЭлементыРеквизитыАренды();
	// >> 08.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-777
...
// << 15.08.2022, Мельников А.А., КРОК, Jira№ A2105505-291 
&НаСервере
Процедура КР_УстановитьВидимости()
	
	ЭтоМагазин = Объект.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин;  
	Элементы["КР_ГруппаНастройкиРозничногоМагазина"].Видимость = ЭтоМагазин;
	
	// << 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
	Элементы.ФорматМагазина.Видимость = Не ЭтоМагазин;
	Элементы.КонтролироватьАссортимент.Видимость = Не ЭтоМагазин;
	Элементы.КР_ФорматыМагазина.Видимость = ЭтоМагазин;
	// >> 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
	
	// << 09.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1292
	Элементы.КР_СпецРаспределение.Видимость = ЭтоМагазин;
	Элементы.КР_СтритРитейл.Видимость = ЭтоМагазин;
	Элементы.КР_ТипПротивокражныхВорот.Видимость = ЭтоМагазин;
	// >> 09.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1292
	
	// << 02.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
	// Федотов А.М., КРОК
	Элементы.КР_ЯчейкаХраненияФорменнойОдежды.Доступность = ЗначениеЗаполнено(Объект.Ссылка);
	Элементы.КР_ЯчейкаХраненияБракованногоТовара.Доступность = ЗначениеЗаполнено(Объект.Ссылка);
	
	Элементы.КР_ЯчейкаХраненияФорменнойОдежды.Видимость = Объект.КР_АдресныйСкладБезордернойОтгрузки;
	Элементы.КР_ЯчейкаХраненияБракованногоТовара.Видимость = Объект.КР_АдресныйСкладБезордернойОтгрузки;
	// >> 02.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
	
	// << 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
	Элементы.КР_МаркетплейсСкладаРасхождений.Видимость = (Объект.КР_ВидСклада = 
		ПредопределенноеЗначение("Перечисление.КР_ВидыСкладов.СкладРазбораРасхожденийМаркетплейса"));
	// >> 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
	
	// --> Евлахов Игорь Николаевич (Начало) 29.07.2024
	// Задача #4448
	Если Объект.КР_ВидСклада = ПредопределенноеЗначение("Перечисление.КР_ВидыСкладов.Магазин") Тогда
		Элементы.злМарка.АвтоОтметкаНезаполненного = Истина; 
		Элементы.злМарка.ОтметкаНезаполненного = Истина;
	Иначе
		Элементы.злМарка.АвтоОтметкаНезаполненного = Ложь;
		Элементы.злМарка.ОтметкаНезаполненного = Ложь;
	КонецЕсли;
...
// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
&НаСервере
Процедура КР_ДобавитьЭлементыФормыНастройкиРазмещенияОтбораНаСкладе()
	
	// КР_ВидСклада
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ВидСклада", , Элементы.ТипСкладаОптовый);
			
	// << 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ВидСкладаПриИзменении");	
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_МаркетплейсСкладаРасхождений", Элементы.ГруппаТипСкладаКолонки);
	// >> 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
	...
	// злМарка
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.злМарка", , Элементы.ТипСкладаОптовый);
	// <-- Евлахов Игорь Николаевич (Конец) 29.07.2024
	
	#Область КР_ГруппаИспользоватьРазмещениеКакПриходныйОрдер   

	ГруппаИспользоватьРазмещениеКакПриходныйОрдер = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаИспользоватьРазмещениеКакПриходныйОрдер", , Элементы.ГруппаДатаНачалаОрдернойСхемыПриОтгрузке); 
	КР_МетодыМодификацииФорм.УстановитьСвойстваОбычнаяГруппаГоризонтальноБезОтображения(
		ГруппаИспользоватьРазмещениеКакПриходныйОрдер);
	
	// КР_ИспользоватьРазмещениеКакПриходныйОрдерОтступ
	Декор = КР_МетодыМодификацииФорм.ВставитьДекорациюФормы(ЭтотОбъект,
		"КР_ИспользоватьРазмещениеКакПриходныйОрдерОтступ", ГруппаИспользоватьРазмещениеКакПриходныйОрдер);
	Декор.Ширина = 1;
	
	// КР_ИспользоватьРазмещениеКакПриходныйОрдер
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ИспользоватьРазмещениеКакПриходныйОрдер", ГруппаИспользоватьРазмещениеКакПриходныйОрдер);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;     
	ЭлементФормы.Заголовок = НСтр("ru = 'использовать размещение как приходный ордер'");
	
	#КонецОбласти
	
	#Область КР_ГруппаСкладСПомещениямиБезордернойОтгрузки   
	
	ГруппаСкладСПомещениямиБезордернойОтгрузки = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаСкладСПомещениямиБезордернойОтгрузки", Элементы.ГруппаИспользоватьСкладскиеПомещения); 
	КР_МетодыМодификацииФорм.УстановитьСвойстваОбычнаяГруппаГоризонтальноБезОтображения(
		ГруппаСкладСПомещениямиБезордернойОтгрузки);
		
	// КР_СкладСПомещениямиБезордернойОтгрузкиОтступ
	Декор = КР_МетодыМодификацииФорм.ВставитьДекорациюФормы(ЭтотОбъект,
		"КР_СкладСПомещениямиБезордернойОтгрузкиОтступ", ГруппаСкладСПомещениямиБезордернойОтгрузки);
	Декор.Ширина = 1;
	
	// КР_СкладСПомещениямиБезордернойОтгрузки
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_СкладСПомещениямиБезордернойОтгрузки", ГруппаСкладСПомещениямиБезордернойОтгрузки);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право; 
	ЭлементФормы.Доступность = Ложь;
	
	#КонецОбласти
	
	// КР_АдресныйСкладБезордернойОтгрузки
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_АдресныйСкладБезордернойОтгрузки", , Элементы.ГруппаНастройкаАдресногоХранения);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право; 
	
	// КР_УчетОстатковКоробов
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_УчетОстатковКоробов", Элементы.КонтрольОтступ);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право; 
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_УчетОстатковКоробовПриИзменении");
	
	#Область КР_ГруппаКонтролироватьОстаткиКоробов

	ГруппаКонтролироватьОстаткиКоробов = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаКонтролироватьОстаткиКоробов", Элементы.КонтрольОтступ); 
	КР_МетодыМодификацииФорм.УстановитьСвойстваОбычнаяГруппаГоризонтальноБезОтображения(
		ГруппаКонтролироватьОстаткиКоробов);  
		
	// КР_КонтролироватьОстаткиКоробовОтступ
	Декор = КР_МетодыМодификацииФорм.ВставитьДекорациюФормы(ЭтотОбъект,
		"КР_КонтролироватьОстаткиКоробовОтступ", ГруппаКонтролироватьОстаткиКоробов);
	Декор.Ширина = 1;
		
	// КР_КонтролироватьОстаткиКоробов
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_КонтролироватьОстаткиКоробов", ГруппаКонтролироватьОстаткиКоробов);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право; 	
	ЭлементФормы.Заголовок = НСтр("ru = 'контролировать остатки коробов'");
	
	#КонецОбласти
		
КонецПроцедуры // >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751

// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
&НаСервере
Процедура КР_УстановитьДоступностьЭлементовНастройкиРазмещенияОтбораНаСкладе()
	
	//
	БезордерныйОтбор = Не Объект.ИспользоватьОрдернуюСхемуПриОтгрузке
		И Объект.ИспользоватьОрдернуюСхемуПриПоступлении     
		И Объект.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач;
		
	Элементы.КР_АдресныйСкладБезордернойОтгрузки.Доступность = БезордерныйОтбор;    
					
	//
	Элементы.КР_ИспользоватьРазмещениеКакПриходныйОрдер.Доступность = 
		Объект.ИспользоватьОрдернуюСхемуПриПоступлении;  
		
	// 
	Элементы.КР_КонтролироватьОстаткиКоробов.Доступность = Объект.КР_УчетОстатковКоробов;

	//
	Элементы.НастройкаАдресногоХранения.ТолькоПросмотр = Элементы.НастройкаАдресногоХранения.ТолькоПросмотр
		И Не Объект.КР_СкладСПомещениямиБезордернойОтгрузки;
	
КонецПроцедуры // >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751

// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
&НаКлиенте
Процедура КР_УчетОстатковКоробовПриИзменении(Элемент)
	
	Если Не Объект.КР_УчетОстатковКоробов Тогда 
		Объект.КР_КонтролироватьОстаткиКоробов = Ложь;	
	КонецЕсли;
...
// << 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868
&НаСервере
Процедура КР_ДобавитьЭлементыФормыФорматМагазина()
	
	КР_ФорматыМагазинаСписок = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(
		ЭтотОбъект, "КР_ФорматыМагазина", Новый ОписаниеТипов("ДинамическийСписок"));
	
	Список = ЭтотОбъект.КР_ФорматыМагазина;
	Список.ОсновнаяТаблица = "РегистрСведений.ИсторияИзмененияФорматовМагазинов.СрезПоследних";
	Список.ПроизвольныйЗапрос = Истина;
	Список.ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаФорматыМагазинов.Период КАК Период,
	|	ТаблицаФорматыМагазинов.Склад КАК Склад,
	// << 02.05.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525
	//|	ТаблицаФорматыМагазинов.КР_ТоварнаяГруппа КАК КР_ТоварнаяГруппа,
	|	ТаблицаФорматыМагазинов.КР_ФорматМагазинаАксессуары КАК КР_ФорматМагазинаАксессуары,
	// >> 02.05.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525
	|	ТаблицаФорматыМагазинов.ФорматМагазина КАК ФорматМагазина,
	|	ТаблицаФорматыМагазинов.КонтролироватьАссортимент КАК КонтролироватьАссортимент,
	|	ТаблицаФорматыМагазинов.РозничныеЦеныИзФорматаМагазина КАК РозничныеЦеныИзФорматаМагазина,
	|	ТаблицаФорматыМагазинов.Автор КАК Автор,
	|	ТаблицаФорматыМагазинов.ДатаЗаписи КАК ДатаЗаписи
	|ИЗ
	|	РегистрСведений.ИсторияИзмененияФорматовМагазинов.СрезПоследних(, Склад = &Склад) КАК ТаблицаФорматыМагазинов";
	Список.Параметры.УстановитьЗначениеПараметра("Склад", Объект.Ссылка);
	
	КР_ФорматыМагазинаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_ФорматыМагазинаСписок, , Элементы.ГруппаФорматМагазина);
	КР_ФорматыМагазинаЭлемент.ПутьКДаннымКартинкиСтроки = "";
	КР_ФорматыМагазинаЭлемент.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
	КР_ФорматыМагазинаЭлемент.ТолькоПросмотр = Истина;
	КР_ФорматыМагазинаЭлемент.УстановитьДействие("Выбор", "КР_ФорматыМагазинаВыбор");
	
	КР_ФорматМагазинаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "КР_ФорматыМагазина.ФорматМагазина", КР_ФорматыМагазинаЭлемент);
	КР_АссортиментЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "КР_ФорматыМагазина.КонтролироватьАссортимент", КР_ФорматыМагазинаЭлемент);
	КР_АссортиментЭлемент.Заголовок = НСтр("ru = 'Ассортимент'");
	// << 02.05.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525
	//КР_ТоварнаяГруппаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
	//	ЭтотОбъект, "КР_ФорматыМагазина.КР_ТоварнаяГруппа", КР_ФорматыМагазинаЭлемент);
	КР_ФорматМагазинаАксессуары = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "КР_ФорматыМагазина.КР_ФорматМагазинаАксессуары", КР_ФорматыМагазинаЭлемент);
	// >> 02.05.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525	
		
	КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект,
		"КР_УстановитьНовыйНастройкиАльтернативно",
		НСтр("ru = 'Установить новые настройки формата магазина'"),
		"КР_УстановитьНовыйНастройкиАльтернативно");
	Элементы.УстановитьНовыйФормат.ИмяКоманды = "КР_УстановитьНовыйНастройкиАльтернативно";
...
Процедура КР_УстановитьУсловноеОформлениеДополнительно()
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.КР_ФорматыМагазинаФорматМагазина.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КР_ФорматыМагазина.ФорматМагазина");
...
&НаКлиенте
Процедура КР_ФорматыМагазинаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
...
&НаКлиенте
Процедура КР_УстановитьНовыйНастройкиАльтернативно(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Записать'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		Оповещение = Новый ОписаниеОповещения("КР_УстановитьНовыйФорматЗавершениеАльтернативно", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, Кнопки);
		Возврат;
	
	КонецЕсли; 
	
	КР_УстановитьНовыйФорматОткрытьФормуАльтернативно();
	
КонецПроцедуры

&НаКлиенте
Процедура КР_УстановитьНовыйФорматЗавершениеАльтернативно(Результат, ДополнительныеПараметры) Экспорт 
...
&НаКлиенте
Процедура КР_УстановитьНовыйФорматОткрытьФормуАльтернативно()
	...
	ОткрытьФорму("РегистрСведений.ИсторияИзмененияФорматовМагазинов.ФормаЗаписи",
		ПараметрыФормы, ЭтотОбъект, , , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры

&НаСервере
Процедура КР_ОбновитьИнформациюОФорматеМагазинаДополнительно()

	Элементы.КР_ФорматыМагазина.Обновить();
	
КонецПроцедуры // >> 16.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-868

// << 08.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-777
&НаСервере
Процедура КР_ДобавитьЭлементыРеквизитыАренды()
	
	КР_РеквизитыАрендыСписок = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(
		ЭтотОбъект, "КР_РеквизитыАренды", Новый ОписаниеТипов("ДинамическийСписок"));
	
	Список = ЭтотОбъект.КР_РеквизитыАренды;
	Список.ОсновнаяТаблица = "РегистрСведений.КР_РеквизитыРасчетаАренды";
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Список.Отбор, "Склад", Объект.Ссылка);
	
	КР_ГруппаРеквизитыАренды = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "КР_РеквизитыАрендыГруппа", Элементы.ГруппаСтраницы);
	КР_ГруппаРеквизитыАренды.Заголовок = НСтр("ru = 'Реквизиты аренды'");
	
	КР_РеквизитыАрендыЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_РеквизитыАрендыСписок, КР_ГруппаРеквизитыАренды);
	КР_РеквизитыАрендыЭлемент.ПутьКДаннымКартинкиСтроки = "";
	КР_РеквизитыАрендыЭлемент.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
	КР_РеквизитыАрендыЭлемент.ТолькоПросмотр = Истина;
	КР_РеквизитыАрендыЭлемент.УстановитьДействие("Выбор", "КР_РеквизитыАрендыВыбор");
	
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Период", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Арендодатель", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Арендатор", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Валюта", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Макет", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.НомерДоговораАренды", КР_РеквизитыАрендыЭлемент);
	// --> Евлахов Игорь Николаевич (Начало) 02.08.2023 18:00:48
	// Задача #2905 e1cib/data/Справочник.узЗадачи?ref=811c00155d7d36f411ee198dfa54afa1
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.ДатаДоговораАренды", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Помещение", КР_РеквизитыАрендыЭлемент);
	// <-- Евлахов Игорь Николаевич (Конец) 02.08.2023 18:00:55
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.ПроцентОтВыручки", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.НомерФормулы", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.МинимальнаяАП", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.Курс", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.ИсключатьИнтернетПродажиПодарочныеКартыСертификаты", КР_РеквизитыАрендыЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_РеквизитыАренды.ИсключатьПодарочныеКартыСертификаты", КР_РеквизитыАрендыЭлемент);
	
КонецПроцедуры

&НаКлиенте
Процедура КР_РеквизитыАрендыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
...
// << 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
&НаКлиенте
Процедура КР_ВидСкладаПриИзменении(Элемент)	

	Если Объект.КР_ВидСклада <> 
		ПредопределенноеЗначение("Перечисление.КР_ВидыСкладов.СкладРазбораРасхожденийМаркетплейса") Тогда 
		
		Объект.КР_МаркетплейсСкладаРасхождений = Неопределено;

	КонецЕсли;	  
	
	КР_УстановитьВидимости();
	
КонецПроцедуры // >> 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
