...
Процедура ЗаполнятьПартнераПриИзменении(Элемент)
	...
	Если Объект.ЗаполнятьПоДаннымСервиса Тогда
		Объект.ЗаполнятьПартнераВТЧ = Объект.ЗаполнятьПартнера;
	КонецЕсли;
...
Процедура ЗаполнятьСкладПриИзменении(Элемент)
	...
	Если Объект.ЗаполнятьПоДаннымСервиса Тогда
		Объект.ЗаполнятьСкладВТЧ = Объект.ЗаполнятьСклад;
	КонецЕсли;
...
Процедура НастройкиПрогнозирования(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("НастройкиПрогнозированияЗавершение", ЭтаФорма);
	...
	ЗаполнитьЗначенияСвойств(РеквизитыДляЗаполнения, Объект, , "РеквизитыРасчетаСезонности");
...
Процедура УстановитьВидимость(Форма)
	...
	Если Форма.ЗаполнятьПоФормуле = 2 Тогда
		...
		Форма.Элементы.ГруппаВидЦеныЗакупки.Видимость = Ложь;
	...
	Иначе
		...
		Форма.Элементы.ГруппаВариантЗаполненияСостава.Видимость = Истина;
		Форма.Элементы.ГруппаВидЦеныЗакупки.Видимость = Истина;
...
Процедура НастройкиПрогнозированияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьЗначенияСвойств(Объект, Результат, , "ЗаполнятьПоДаннымСервиса, РеквизитыРасчетаСезонности");
