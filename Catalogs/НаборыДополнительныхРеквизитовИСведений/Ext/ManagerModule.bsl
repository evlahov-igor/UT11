...
// << 07.04.2023 Новиков Н.С., КРОК, JIRA№ A2105505-1465
Процедура КР_ПервоначальноеЗаполнениеЛокализованныхДанных(ТекущаяВерсияЛокализации, НоваяВерсияЛокализации) Экспорт
	...
	Если Версия > ТекущаяВерсияЛокализации 
		...
		// На основе данных модуля менеджера справочниика НаборыДополнительныхРеквизитовИСведений.КР_ПриНачальномЗаполненииЭлементов() 
		#Область КР_ПриНачальномЗаполненииЭлементов
		
		Элемент													= Новый Структура;
		Элемент.Вставить("ИмяПредопределенногоНабора",			"Документ_ТранспортнаяНакладная");
		Элемент.Вставить("Наименование",						НСтр("ru = 'Транспортные накладные'"));
		Элемент.Вставить("Используется",						Истина);
		Элемент.Вставить("Ключ",                                "02a89b5d-93b6-4680-9780-8a0de2634d78");
	...
	Для Каждого Элемент Из НовыеЭлементы Цикл
		...
		Если Не КР_НабоДополнительныхСвойствУжеСуществует(Элемент) Тогда
			Набор = КР_ПолучитьОбъект(Элемент);
...
КонецПроцедуры // >> 07.04.2023 Новиков Н.С., КРОК, JIRA№ A2105505-1465
...
Функция КР_ПолучитьОбъект(Элемент)
	...
	Запрос.УстановитьПараметр("ИмяПредопределенногоНабора",	Элемент.ИмяПредопределенногоНабора);
	...
	Иначе
		РезультатОбъект = Справочники.НаборыДополнительныхРеквизитовИСведений.СоздатьЭлемент();
		РезСсылка = XMLЗначение(Тип("СправочникСсылка.НаборыДополнительныхРеквизитовИСведений"), Элемент.Ключ);
		РезультатОбъект.УстановитьСсылкуНового(РезСсылка);
	КонецЕсли;
	Возврат РезультатОбъект;
	
КонецФункции // >> 07.04.2023 Новиков Н.С., КРОК, JIRA№ A2105505-1465

// << 07.04.2023 Новиков Н.С., КРОК, JIRA№ A2105505-1465
Функция КР_НабоДополнительныхСвойствУжеСуществует(Элемент)
	
	Если ОбщегоНазначения.СсылкаСуществует(XMLЗначение(Тип("СправочникСсылка.НаборыДополнительныхРеквизитовИСведений"), Элемент.Ключ)) Тогда
		Возврат Истина;	
	КонецЕсли;	
	...
	РезультатЗапроса = Запрос.Выполнить();
	НаборУжеСуществует = Не РезультатЗапроса.Пустой();
	Возврат НаборУжеСуществует;
