...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(ТекстВыборкаПоляАналитик, ЗапросОбщий, ТекстыЗапроса = Неопределено) Экспорт
	...
	//++ Локализация
	// При обновлении информационной базы в регистре сведений могут быть еще не обновлены ставки НДС,
	// поэтому пробуем найти ключ аналитики по ставке НДС из перечисления.  
	|				ИЛИ Т.СтавкаНДС.ПеречислениеСтавкаНДС = АналитикаПартий.УдалитьСтавкаНДС
	|				 И АналитикаПартий.УдалитьСтавкаНДС <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)
	//-- Локализация
	|				)
