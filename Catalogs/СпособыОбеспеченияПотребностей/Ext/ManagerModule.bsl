...
Процедура АктуализироватьГрафикЗаказовНаСервере(Объект, ДатаСегодня) Экспорт
	...
	// Логика (при формировании плановых заказов):
	// Основное отличие от типового функционала - 
	//	ПлановаяДатаПоставки и ДатаСледующейПоставки перезаполняются всегда исходя из графика 
	// ПлановаяДатаПоставки это ближайший день графика сборки плюс СрокИсполненияЗаказа
	// ДатаСледующейПоставки это ближайший основной день графика сборки плюс СрокИсполненияЗаказа
	Если Объект.ФормироватьПлановыеЗаказы Тогда 	 
		
		// Очищаем даты 
		Объект.ПлановаяДатаЗаказа = Неопределено;
		Объект.ПлановаяДатаПоставки = Неопределено;
		Объект.ДатаСледующейПоставки = Неопределено;
		
		Сутки = 3600 * 24;
		
		// ПлановаяДатаПоставки, ПлановаяДатаЗаказа
		ДеньГрафикаСборки = Неопределено;	
		НачальнаяДата = ДатаСегодня;
		НомерДняНеделиНачальнойДаты = ДеньНедели(НачальнаяДата);
		Если КР_ОпределенБлижайшийДеньСборки(Объект, НачальнаяДата, Ложь, ДеньГрафикаСборки) Тогда 
			
			КоличествоДнейДоДняСборки = ДеньГрафикаСборки - НомерДняНеделиНачальнойДаты; 
			Если КоличествоДнейДоДняСборки < 0 Тогда 
				КоличествоДнейДоДняСборки = КоличествоДнейДоДняСборки + 7;
			КонецЕсли;
			
			Объект.ПлановаяДатаЗаказа = НачальнаяДата + КоличествоДнейДоДняСборки * Сутки; 
			Объект.ПлановаяДатаПоставки = Объект.ПлановаяДатаЗаказа + Объект.СрокИсполненияЗаказа * Сутки;
			
		КонецЕсли;
		
		// ДатаСледующейПоставки
		ДеньГрафикаСборки = Неопределено;
		// << 29.05.2024 Вдовенко С.А., КРОК, A2105505-2666
		//НачальнаяДата = Объект.ПлановаяДатаЗаказа + Сутки;
		СегодняОсновнойДеньГрафика = КР_ЭтоОсновнойДеньГрафика(ДатаСегодня, Объект.КР_ГрафикСборки);
		Если СегодняОсновнойДеньГрафика Тогда
			НачальнаяДата = ДатаСегодня;
		Иначе
			НачальнаяДата = Объект.ПлановаяДатаЗаказа + Сутки;
		КонецЕсли;
		// >> 29.05.2024 Вдовенко С.А., КРОК, A2105505-2666
		НомерДняНеделиНачальнойДаты = ДеньНедели(НачальнаяДата);
		Если КР_ОпределенБлижайшийДеньСборки(Объект, НачальнаяДата, Истина, ДеньГрафикаСборки) Тогда 
			
			КоличествоДнейДоДняСборки = ДеньГрафикаСборки - НомерДняНеделиНачальнойДаты; 
			// << 29.05.2024 Вдовенко С.А., КРОК, A2105505-2666
			Если СегодняОсновнойДеньГрафика Тогда
				КоличествоДнейДоДняСборки = 7;
			КонецЕсли;
			// >> 29.05.2024 Вдовенко С.А., КРОК, A2105505-2666
			...
			Объект.ДатаСледующейПоставки = НачальнаяДата + (Объект.СрокИсполненияЗаказа + КоличествоДнейДоДняСборки) * Сутки;
...
Функция КР_ОпределенБлижайшийДеньСборки(Объект, НачальнаяДата, ТолькоОсновнойДень, ДеньГрафикаСборки)

	НомерДняНеделиНачальнойДаты = ДеньНедели(НачальнаяДата);
	Если ТолькоОсновнойДень Тогда 
		Отбор = Новый Структура("Основной", Истина);
		ДниГрафикаСборки = Объект.КР_ГрафикСборки.НайтиСтроки(Отбор);   
	Иначе
		ДниГрафикаСборки = Объект.КР_ГрафикСборки.Выгрузить();
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДниГрафикаСборки) Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого СтрокаДанных Из ДниГрафикаСборки Цикл 
		
		Если СтрокаДанных.НомерДняНедели >= НомерДняНеделиНачальнойДаты Тогда 
			ДеньГрафикаСборки = СтрокаДанных.НомерДняНедели;
			Возврат Истина;
		КонецЕсли;
		
		Если ДеньГрафикаСборки = Неопределено Тогда 
			ДеньГрафикаСборки = СтрокаДанных.НомерДняНедели;
		КонецЕсли;
...
КонецФункции // >> 10.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148	

// << 29.05.2024 Вдовенко С.А., КРОК, A2105505-2666
Функция КР_ЭтоОсновнойДеньГрафика(ПроверяемаяДата, ГрафикСборки)
	
	НомерДняНедели = ДеньНедели(ПроверяемаяДата);
	Отбор = Новый Структура;
	Отбор.Вставить("Основной", Истина);
	Отбор.Вставить("НомерДняНедели", НомерДняНедели);
	ДниГрафикаСборки = ГрафикСборки.НайтиСтроки(Отбор);
	Возврат ДниГрафикаСборки.Количество() <> 0;
