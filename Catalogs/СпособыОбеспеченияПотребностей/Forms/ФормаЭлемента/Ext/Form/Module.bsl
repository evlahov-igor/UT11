...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// << 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
	КР_ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи);
	// >> 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
...
Процедура КР_ПриЧтенииСозданииНаСервере()
	...
	// КР_ГрафикСборки реквизит формы и элемент
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ГрафикСборки", Новый ОписаниеТипов("ТаблицаЗначений"));
	ТаблицаГрафикСборки = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		РеквизитФормы, Элементы.ГруппаЗаказПоГрафику);   
	ТаблицаГрафикСборки.ИзменятьПорядокСтрок = Ложь;
	ТаблицаГрафикСборки.ИзменятьСоставСтрок = Ложь;
	ТаблицаГрафикСборки.Шапка = Ложь;  
	ТаблицаГрафикСборки.АвтоМаксимальнаяВысота = Ложь;
	ТаблицаГрафикСборки.ВысотаВСтрокахТаблицы = 7; 
	ТаблицаГрафикСборки.АвтоМаксимальнаяВысотаВСтрокахТаблицы = Ложь;
	ТаблицаГрафикСборки.АвтоМаксимальнаяШирина = Ложь;
	ТаблицаГрафикСборки.МаксимальнаяШирина = 30; 	
	ТаблицаГрафикСборки.Ширина = 30; 	
	ТаблицаГрафикСборки.РастягиватьПоГоризонтали = Ложь; 	
	ТаблицаГрафикСборки.РастягиватьПоВертикали = Ложь; 	 
	ТаблицаГрафикСборки.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
	
	// Добавим реквизит формы галочку для отметки для недели и элемент для него
	// КР_ГрафикСборки.Пометка
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"Пометка", Новый ОписаниеТипов("Булево"), "КР_ГрафикСборки");
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		РеквизитФормы, ТаблицаГрафикСборки);
	ЭлементФормы.ФиксацияВТаблице = ФиксацияВТаблице.Лево;
	ЭлементФормы.ОтображатьВШапке = Ложь; 
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ГрафикСборкиПометкаПриИзменении");
	
	// КР_ГрафикСборки.НомерДняНедели реквизит формы и элемент
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"НомерДняНедели", Новый ОписаниеТипов("Число"), "КР_ГрафикСборки");
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		РеквизитФормы, ТаблицаГрафикСборки); 
	ЭлементФормы.ТолькоПросмотр = Истина;	
	
	// КР_ГрафикСборки.Основной реквизит формы
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"Основной", Новый ОписаниеТипов("Булево"), "КР_ГрафикСборки");
		
	// КР_УстановитьОсновнойДеньГрафикаСборки
	// Создаем свою панель так как нет возможности убрать автозаполнение у типовой на тек. релизе платформы
	ТаблицаГрафикСборкиКоманднаяПанель = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаГрафикСборкиКоманднаяПанель", , ТаблицаГрафикСборки);
	ТаблицаГрафикСборкиКоманднаяПанель.Вид = ВидГруппыФормы.КоманднаяПанель;  
	ТаблицаГрафикСборкиКоманднаяПанель.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	ТаблицаГрафикСборкиКоманднаяПанель.Ширина = ТаблицаГрафикСборки.Ширина;
	ТаблицаГрафикСборкиКоманднаяПанель.РастягиватьПоГоризонтали = Ложь; 	 
		
	КомандаФормы = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект,
		"КР_УстановитьОсновнойДеньГрафикаСборки", НСтр("ru = 'Сделать основным'"));
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		КомандаФормы, ТаблицаГрафикСборкиКоманднаяПанель);  
	
	КР_ЗаполнитьГрафикСборки(); 
	
	КР_УстановитьУсловноеОформление();
	
КонецПроцедуры // >> 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148

// << 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
&НаСервере
Процедура КР_ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)

	ГрафикСборки = ТекущийОбъект.КР_ГрафикСборки;
	ГрафикСборки.Очистить();
	
	Для Каждого СтрокаДанных Из ЭтотОбъект.КР_ГрафикСборки Цикл 
		
		Если Не СтрокаДанных.Пометка Тогда 
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = ГрафикСборки.Добавить();
		НоваяСтрока.НомерДняНедели = СтрокаДанных.НомерДняНедели;
		НоваяСтрока.Основной = СтрокаДанных.Основной;
...
// << 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148  
&НаСервере
Процедура КР_ЗаполнитьГрафикСборки()
	
	ПометкаДнейНеделиГрафика = Новый Соответствие;
	ОсновниеДниГрафика = Новый Соответствие;
	Для Каждого СтрокаДанных Из Объект.КР_ГрафикСборки Цикл 
		ПометкаДнейНеделиГрафика[СтрокаДанных.НомерДняНедели] = Истина;	
		ОсновниеДниГрафика[СтрокаДанных.НомерДняНедели] = СтрокаДанных.Основной;	
	КонецЦикла;

	ГрафикСборки = ЭтотОбъект.КР_ГрафикСборки;
	Для НомерДняНедели = 1 По 7 Цикл 
		
		НоваяСтрока = ГрафикСборки.Добавить();
		НоваяСтрока.Пометка = ПометкаДнейНеделиГрафика[НомерДняНедели];
		НоваяСтрока.НомерДняНедели = НомерДняНедели;
		НоваяСтрока.Основной = ОсновниеДниГрафика[НомерДняНедели];
...
Процедура КР_УстановитьУсловноеОформление()
	...
	Для НомерДняНедели = 1 По 7 Цикл 
		...
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("КР_ГрафикСборкиНомерДняНедели");

		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КР_ГрафикСборки.НомерДняНедели");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;      
		ОтборЭлемента.ПравоеЗначение = НомерДняНедели;
		
		ДатаДняНедели = ДатаВоскресенье + НомерДняНедели * Сутки;
		Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Формат(ДатаДняНедели, "ДФ=дддд"));
		
	КонецЦикла;	
	
	// ОсновнойДеньГрафикаСборки
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("КР_ГрафикСборки");

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КР_ГрафикСборки.Основной");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;      
	ОтборЭлемента.ПравоеЗначение = Истина;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Шрифт", 
		Новый Шрифт( , , Истина, , , , 110));
	
КонецПроцедуры // >> 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148

// << 23.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
&НаКлиенте
Процедура КР_УстановитьОсновнойДеньГрафикаСборки()
	
	ТекущиеДанные = Элементы.КР_ГрафикСборки.ТекущиеДанные;
...
// << 10.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
&НаКлиенте
Процедура КР_ГрафикСборкиПометкаПриИзменении(Элемент)
...
КонецПроцедуры // >> 10.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1148
