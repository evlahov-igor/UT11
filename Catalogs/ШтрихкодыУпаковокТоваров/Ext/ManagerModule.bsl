...
Функция ПараметрыСозданияШтриходовУпаковокПоДеревуМаркируемойПродукции(ДеревоМаркированнойПродукции, ПараметрыСканирования) Экспорт
...
Функция РезультатСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(ДеревоМаркированнойПродукции,
	...
	ПараметрыПродолженияПоиска.Вставить("ПараметрыСоздания",                 ПараметрыСоздания);
...
Процедура ПродолжитьПослеПоискаШтрихкодаУпаковкиПоХешСумме(
	...
	Если РезультатПоиска.ШтрихкодУпаковки <> Неопределено Тогда
		...
		ПропуститьСохранениеКоличества = Ложь;
		Если ШтрихкодированиеИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
				...
				Истина) Тогда
			ПропуститьСохранениеКоличества = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаДерева, "ВидПродукции") Тогда
			ДанныеНовойУпаковки.ВидПродукции = ?(ЗначениеЗаполнено(СтрокаДерева.ВидПродукции), СтрокаДерева.ВидПродукции, Параметры.ВидПродукции);
		Иначе
			ДанныеНовойУпаковки.ВидПродукции = Параметры.ВидПродукции;
		КонецЕсли;
		...
		Если ШтрихкодированиеИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
			...
			Если СтрокаДерева.ЧастичноеВыбытиеВариантУчета = Перечисления["ВариантыУчетаЧастичногоВыбытияИСМП"]["НастроеннаяНоменклатура"] Тогда
		...
		Если Не Параметры.ЭтоДетализацияБезУпаковокНижнихУровней Тогда
	...
	Если ТипЗнч(ПараметрыПродолженияПоиска.ДеревоМаркированнойПродукции) = Тип("ДеревоЗначений")
		Или ТипЗнч(ПараметрыПродолженияПоиска.ДеревоМаркированнойПродукции.ТипУпаковки) <> Тип("ПеречислениеСсылка.ТипыУпаковок") Тогда
...
//
Процедура СоздатьШтрихкодыУпаковокПоДаннымДерева(ДеревоУпаковок, ПараметрыСканирования) Экспорт
	
	ПараметрыСозданияШтрихкодов = ПараметрыСозданияШтриходовУпаковокПоДеревуМаркируемойПродукции(ДеревоУпаковок, ПараметрыСканирования);
...
Процедура ЗаполнитьШтрихкодУпаковки(ШтрихкодУпаковки, ЗначенияРеквизитов, ОтключитьЗаменуСвойств = Неопределено) Экспорт
	...
	Для Каждого ЗначениеРеквизита Из ЗначенияРеквизитов Цикл
		Если ОтключитьЗаменуСвойств <> Неопределено
			И ОтключитьЗаменуСвойств[ЗначениеРеквизита.Ключ] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ШтрихкодУпаковкиОбъект[ЗначениеРеквизита.Ключ] = ЗначениеРеквизита.Значение;
	КонецЦикла;
			...
			Если НайденныйШтрихкодУпаковки = Неопределено
				...
				ИначеЕсли ДопустимоСравнение Тогда
					...
					Для Каждого СтрокаДереваВложенныхШтрихкодов Из ВложенныеШтрихкодыУпаковок.ДеревоУпаковок.Строки Цикл
						...
						Если СравнитьСтрокуДереваСДаннымиИБ(СтрокаДерева, СтрокаДереваВложенныхШтрихкодов, ТаблицаХешСумм, ПараметрыПоиска) Тогда
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
			...
			Если НайденнаяСтрока = Неопределено Тогда
				...
				СтрокаДерева.ШтрихкодУпаковки = СоздатьЭлементСправочникаШтрихкодыУпаковок(
					СтрокаДерева, ПараметрыСозданияШтрихкодов.КэшСтатусовУказанияСерий);
		...
		Иначе
			...
			Если РезультатПоиска <> Неопределено Тогда
				...
				Если РезультатПоиска.ШтрихкодУпаковки = Неопределено Тогда
					СтрокаДерева.ШтрихкодУпаковки = СоздатьЭлементСправочникаШтрихкодыУпаковок(
						СтрокаДерева, ПараметрыСозданияШтрихкодов.КэшСтатусовУказанияСерий, СтрокаДерева.ХешСумма, КэшСтроки.ВложенныеУпаковки);
				Иначе
					СтрокаДерева.ХешСумма = РезультатПоиска.ХешСумма;
				КонецЕсли;
