...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"ДействиеНедостачи", , Элементы.НедостачаНеПризнана); 
	...
	СписокВыбора.Добавить(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.КР_ПереместитьНаСкладРазбора);
	
	// Декорация
	Декорация = КР_МетодыМодификацииФорм.ВставитьДекорациюФормы(ЭтотОбъект,
		"КР_ПереместитьНаСкладРазбораДекорация", , Элементы.НедостачаНеПризнана); 
	Декорация.ЦветТекста = ЦветаСтиля["ТекстИнформационнойНадписи"];   
	Декорация.Заголовок = НСтр("ru = 'Недопоставленный товар перемещается на виртуальный склад разбора товарных " +
		"расхождений. Выполнение заказа на перемещение уменьшается на недопоставленное количество (можно будет " +
		"оформить дополнительное перемещение товаров)'");     
	...
	// Элемент
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"ДействиеИзлишки", , Элементы.ПерепоставленноеДарится); 
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	ЭлементФормы.Вид = ВидПоляФормы.ПолеПереключателя;
	СписокВыбора = ЭлементФормы.СписокВыбора;
	СписокВыбора.Добавить(Перечисления.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.КР_ПереместитьСоСкладаРазбора);
	
	// Декорация
	Декорация = КР_МетодыМодификацииФорм.ВставитьДекорациюФормы(ЭтотОбъект,
		"КР_ПереместитьСоСкладаРазбораДекорация", , Элементы.ПерепоставленноеДарится); 
	Декорация.ЦветТекста = ЦветаСтиля["ТекстИнформационнойНадписи"];   
	Декорация.Заголовок = НСтр("ru = 'Перепоставленный товар перемещается с виртуального склад разбора товарных " +
		"расхождений. Выполнение заказа на перемещение увеличивается на перепоставленное количество'");     
