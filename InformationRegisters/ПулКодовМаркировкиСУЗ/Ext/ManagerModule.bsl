...
Процедура РаспечататьЭтикеткиИЗафиксироватьДатуПечати(
	...
	Попытка
		...
		Если РезервироватьСвободныеКоды И ТаблицаДляРезервирования.Количество() > 0 Тогда
			...
			ПараметрыРезервирования.Вставить("СпособВводаВОборот",  МаркировкаОстатков);
		...
		ЕстьКолонкаПолныйКодМаркировкиИзменен = Ложь;
		Если ДанныеПечати.Свойство("ДополнитьПолныйКодМаркировки")
			И ДанныеПечати.ДополнитьПолныйКодМаркировки = "3103" Тогда
			ОбщаяТаблицаПечати.Колонки.Добавить("ПолныйКодМаркировкиИзменен", Новый ОписаниеТипов("Булево"));
			ЕстьКолонкаПолныйКодМаркировкиИзменен = Истина;
			...
			Для Каждого СтрокаТЧ Из ОбщаяТаблицаПечати Цикл
				...
				Если Лев(Прав(Штрихкод, 6), 2) = "93" Тогда
					Штрихкод = Штрихкод + Символ(29) + "3103" + СтрЗаменить(Формат(СтрокаТЧ.УчетноеКоличество, "ЧЦ=6; ЧДЦ=3; ЧН=000000; ЧВН=; ЧГ=0;"), ",", "");
		...
		УстановитьСостояниеРаспечатанПоТаблицеДляПечати(ОбщаяТаблицаПечати, ЕстьКолонкаПолныйКодМаркировкиИзменен);
...
Процедура ЗаписатьДанныеКодаМаркировки(ДанныеШтрихкода, ПараметрыСканирования, ДанныеЗаписи = Неопределено) Экспорт
	...
	ИначеЕсли ДанныеШтрихкода.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак Тогда
		...
		КонецЕсли;
	ИначеЕсли ДанныеШтрихкода.ВидПродукции = Перечисления.ВидыПродукцииИС.НикотиносодержащаяПродукция Тогда
		Если ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			МенеджерЗаписи.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.НикотиносодержащаяПродукцияПачка;
		ИначеЕсли ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
			МенеджерЗаписи.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.НикотиносодержащаяПродукцияБлок;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСостояниеРаспечатанПоТаблицеДляПечати(ТаблицаДляПечати, ЕстьКолонкаПолныйКодМаркировкиИзменен = Ложь)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДляПечати Цикл
		...
		Если МенеджерЗаписиРегистра.Выбран() Тогда
			...
			Если ЕстьКолонкаПолныйКодМаркировкиИзменен
				И СтрокаТаблицы.ПолныйКодМаркировкиИзменен Тогда
				МенеджерЗаписиРегистра.ПолныйКодМаркировки = СтрокаТаблицы.ПолныйКодМаркировки;
			КонецЕсли;
