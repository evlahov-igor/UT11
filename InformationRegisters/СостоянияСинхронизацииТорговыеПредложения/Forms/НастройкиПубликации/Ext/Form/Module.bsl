...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ТорговыеПредложения.ПравоНастройкиТорговыхПредложений(Истина)
		Или Параметры.ТорговоеПредложение.Пустая() Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Запрос.Параметры.Вставить("ПрайсЛист", Запись.ТорговоеПредложение);
	РезультатПакетаЗапросов = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	ТаблицаПрайсЛисты = РезультатПакетаЗапросов[0].Выгрузить();
	ДанныеПрайсЛиста = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ТаблицаПрайсЛисты[0]);
	
	ЗаполнитьВариантПубликацииОстатков(НастройкиПодсистемы);
	ЗаполнитьВариантыПубликацииСкидок(ДанныеПрайсЛиста, НастройкиПодсистемы);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ПустаяСтрока(Запись.АдресЭлектроннойПочты) 
...
&НаСервере
Процедура ЗаполнитьВариантыПубликацииСкидок(ДанныеПрайсЛиста, НастройкиПодсистемы)
	
	ОбновлятьОстаткиИЦены = ДанныеПрайсЛиста.ОбновлятьОстаткиИЦены;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаСкидкиГруппировка", "Доступность", ОбновлятьОстаткиИЦены);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ДекорацияОтображениеСкидок", "Видимость", Не ОбновлятьОстаткиИЦены);
