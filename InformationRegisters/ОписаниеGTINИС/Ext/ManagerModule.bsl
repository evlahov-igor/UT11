...
Функция УстановитьОписаниеПоТаблице(ТаблицаОписания) Экспорт
	...
	Попытка
		...
		|	ОписаниеGTINИС.КоличествоПотребительскихУпаковок
		|ИЗ
		|	ИсходнаяТаблица КАК ИсходнаяТаблица
		...
		ДанныеИБ = Запрос.Выполнить().Выгрузить();
		ДанныеИБ.Индексы.Добавить("GTIN");
		
		Для Каждого СтрокаТЧ Из ТаблицаОписания Цикл
			
			СохраненныеДанные = ДанныеИБ.Найти(СтрокаТЧ.GTIN);
			УстановитьОписание(СтрокаТЧ, СохраненныеДанные);
