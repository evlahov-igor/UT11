...
Процедура УстановитьНастройкиПоУмолчанию(Склад, Помещение, ИспользоватьАдресноеХранение,
										...
										СкопироватьНастройкиВладельца = Ложь   
	// << 31.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-751
	, ДополнительныеРесурсы = Неопределено 
	// >> 31.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-751										
	) Экспорт
	...
	// << 31.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-751
	Если ТипЗнч(ДополнительныеРесурсы) = Тип("Структура") Тогда 
		Для Каждого КлючЗначение Из ДополнительныеРесурсы Цикл 
			СтрокаНастроек[КлючЗначение.Ключ] = КлючЗначение.Значение; 
		КонецЦикла;	  
	КонецЕсли;	
	// >> 31.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-751										
