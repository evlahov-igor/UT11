...
Процедура ОбновитьРеквизитыТранспортныхНакладныхПриЗаписиДокументаОснования(ДокументОснованиеОбъект, Отказ) Экспорт
	...
	// << 05.05.2023 Федотов А.М., КРОК, Jira№A2105505-1619
	// Документы транспортых накладных не должны обновляться при изменении документов-оснований
	// так как они создаются интеграцией на основании данных, полученных из УТ10 Логистика
	Возврат;
	// >> 05.05.2023 Федотов А.М., КРОК, Jira№A2105505-1619
...
Функция ПроверитьДокументыОснования(ПроверяемыеОбъекты, ВыделенныеСтрокиАдресов = Неопределено, ВыдаватьСообщения = Истина) Экспорт
	...
	Для Каждого КлючЗначение Из ТипыОбъектов Цикл
		...
		Если ПараметрыФормированияТранспортныхНакладных.ЕстьЗаказы Тогда  
			// << 28.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1296 
			// Ошибка типового запроса. Возникает NULL если в перемещении не указан ЗаказНаПеремещение 
			...
			ФрагментЗапроса = СтрЗаменить(ФрагментЗапроса,    
				// Типовой кусок кода (см. выще)
				"ДокументТовары." +  ПараметрыФормированияТранспортныхНакладных.ИмяПоляЗаказВТЧТовары + ".СпособДоставки",
				// Оборачиваем его проверкой на NULL
				"ЕСТЬNULL(ДокументТовары." +  ПараметрыФормированияТранспортныхНакладных.ИмяПоляЗаказВТЧТовары 
				+ ".СпособДоставки,НЕОПРЕДЕЛЕНО)");                 
			// Типовой кусок + измененный фрагмент	
			ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении() + ФрагментЗапроса;
			// >> 28.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1296
		КонецЕсли;
...
// 
// Возвращаемое значение:
//  Массив - Массив из ДокументСсылка.ТранспортнаяНакладная
//
...
КонецФункции // >> 22.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-376

#КонецОбласти

// ГалкинАН #4309 (08.08.2024) galkin.an {
Процедура злЗаполнитьДанныеТранспортногоСредства(Объект) Экспорт
...
Процедура злЗаполнитьПолучателяОтправителяЛогистика(Объект) Экспорт
	...
	СкладКалининград = КР_ДополнительныеНастройкиПовтИсп.Значение("СкладОтгрузкиКалининград");
	БизнесРегионКалининград = КР_ДополнительныеНастройкиПовтИсп.Значение("БизнесРегионКалининград");
	
	БизнесРегионОтправителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КР_Отправитель, "БизнесРегион");
	
	Если Объект.КР_ЧерезЦС Тогда
		Если БизнесРегионОтправителя = БизнесРегионКалининград Тогда
			Объект.КР_ПолучательЛогистика = СкладКалининград;
		Иначе
			Объект.КР_ПолучательЛогистика = СкладОтгрузкиЦС;
		КонецЕсли;
	ИначеЕсли Не СкладыОтгрузки.Найти(Объект.КР_Получатель) = Неопределено Тогда
		Объект.КР_ПолучательЛогистика = СкладОтгрузкиЦС;
	Иначе
		Объект.КР_ПолучательЛогистика = Объект.КР_Получатель;
	КонецЕсли;
	
	Если СкладыОтгрузки.Найти(Объект.КР_Отправитель) = Неопределено Тогда
		Объект.КР_ОтправительЛогистика = Объект.КР_Отправитель;
	Иначе
		Объект.КР_ОтправительЛогистика = СкладОтгрузкиЦС;
	КонецЕсли;

КонецПроцедуры

Процедура злУстановитьАдресСклада(Объект, Склад, ИмяПоля) Экспорт
	
	Если ТипЗнч(Склад) = Тип("СправочникСсылка.СтруктураПредприятия") Тогда
		Возврат;
	КонецЕсли;
	
	АдресаСклада = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Склад, 
		Справочники.ВидыКонтактнойИнформации.АдресСклада, , Ложь);
		
	Если АдресаСклада.Количество()=0 Тогда
		Возврат;
	КонецЕсли;
	
	ИмяПоляЗначение = СтрШаблон("%1Значение", ИмяПоля);
	ИмяПоляЗначенияПолей = СтрШаблон("%1ЗначенияПолей", ИмяПоля);
	
	Объект[ИмяПоля] = АдресаСклада[0].Представление;
	Объект[ИмяПоляЗначение] = АдресаСклада[0].Значение;
	Объект[ИмяПоляЗначенияПолей] = АдресаСклада[0].ЗначенияПолей;
...
Функция КР_ТекстЗапросаТаблицаЗаданияНаПеревозку(Запрос, ТекстыЗапроса, Регистры)
	...
	// Доработки по A2105505-1403, A2105505-1524, A2105505-1617
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	&Период КАК Период,
	|	ДокументТранспортнаяНакладная.ЗаданиеНаПеревозку КАК ЗаданиеНаПеревозку,
	|	ДокументТранспортнаяНакладная.КР_ОтправительЛогистика КАК Отправитель,
	|	ДокументТранспортнаяНакладная.КР_ПолучательЛогистика КАК Получатель,
	|	ДокументТранспортнаяНакладная.КР_ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	НАЧАЛОПЕРИОДА(&Период, ДЕНЬ) КАК ДатаОтгрузки,
	|	1 КАК КВыполнению
	|ИЗ
	|	Документ.ТранспортнаяНакладная КАК ДокументТранспортнаяНакладная
	|ГДЕ
	|	ДокументТранспортнаяНакладная.Ссылка = &Ссылка
	|	И ДокументТранспортнаяНакладная.ЗаданиеНаПеревозку <> ЗНАЧЕНИЕ(Документ.ЗаданиеНаПеревозку.ПустаяСсылка)
	|	И НЕ ДокументТранспортнаяНакладная.КР_ИдентификаторСтроки ПОДОБНО """"";
...
// << 22.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-376
// Процедура неэкспортная, можно использовать префикс
Процедура КР_ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|ИЗ
	|	Документ.ТранспортнаяНакладная КАК ДанныеДокумента
	|ГДЕ
