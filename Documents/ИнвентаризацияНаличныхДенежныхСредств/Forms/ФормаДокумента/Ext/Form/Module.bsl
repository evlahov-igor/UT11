...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// --> Евлахов Игорь Николаевич (Начало) 02.10.2023
	// Задача 3089
	злПриСозданииНаСервере(Отказ, СтандартнаяОбработка);
	// <-- Евлахов Игорь Николаевич (Конец) 02.10.2023
...
Процедура ВыборКассовойКниги(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		// --> Евлахов Игорь Николаевич (Начало) 02.10.2023
		// Задача 3089
		ЭтотОбъект["злПодразделение"] = злПолучитьПодразделениеКассовойКниги(Объект.КассоваяКнига);
		// <-- Евлахов Игорь Николаевич (Конец) 02.10.2023
	КонецЕсли;
...
Процедура КассыКассаПриИзменении(Элемент)
	...
	// --> Евлахов Игорь Николаевич (Начало) 02.10.2023
	// Задача 3089	
	Если ЗначениеЗаполнено(ЭтотОбъект["злПодразделение"]) Тогда
		ТекущаяСтрока.Подразделение = ЭтотОбъект["злПодразделение"];
	Иначе
		ТекущаяСтрока.Подразделение = злПолучитьПодразделениеКассы(ТекущаяСтрока.Касса);
	КонецЕсли;
	
	РассчитатьРасхожденияВСтроке(ТекущаяСтрока);
	// <-- Евлахов Игорь Николаевич (Конец) 02.10.2023
...
Процедура ЗаполнитьКассыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	// Задача 3089	
	Если ЗначениеЗаполнено(ЭтотОбъект["злПодразделение"]) Тогда		
		Для каждого ТекущиеДанные Из Объект.Кассы Цикл
			ТекущиеДанные.Подразделение = ЭтотОбъект["злПодразделение"];			
		КонецЦикла;
	Иначе	
		МассивДанных = Новый Массив();
		
		Для каждого ТекущиеДанные Из Объект.Кассы Цикл
			Если ЗначениеЗаполнено(ТекущиеДанные.Касса) Тогда
				МассивДанных.Добавить(ТекущиеДанные.Касса);
		    КонецЕсли;
		КонецЦикла;
		
		СоответствиеПодразделениеКассы = злПолучитьВсеПодразделенияТаблицыКассы(МассивДанных);
		
		Для каждого ТекущиеДанные Из Объект.Кассы Цикл			
			ПодразделениеКассы = СоответствиеПодразделениеКассы.Получить(ТекущиеДанные.Касса);
			Если ПодразделениеКассы <> Неопределено Тогда
				ТекущиеДанные.Подразделение = ПодразделениеКассы;
			КонецЕсли;			
		КонецЦикла;
	КонецЕсли;
	// <-- Евлахов Игорь Николаевич (Конец) 02.10.2023
...
Процедура злПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	злУправлениеЭлементамиФормы();
	
КонецПроцедуры

&НаКлиенте
Процедура злПриОткрытии(Отказ)
		
	злОбновитьЗаголовкиСтраниц();	

КонецПроцедуры

#КонецОбласти

#Область злОбработчикиСобытийЭлементовШапкиФормы

&НаКлиенте
Процедура Подключаемый_злКассоваяКнигаПриИзменении(Элемент)
...
&НаКлиенте
Процедура Подключаемый_злСоставКомиссииЧленКомиссииПриИзменении(Элемент)
	
	ТекущиДанные = Элементы.злСоставКомиссии.ТекущиеДанные;
	
	Если ТекущиДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(ТекущиДанные.ЧленКомиссии) Тогда
			ТекущиДанные.Должность = злПолучитьДолжностьФизическогоЛица(ТекущиДанные.ЧленКомиссии);
		Иначе
			ТекущиДанные.Должность = "";
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_злПредседательКомиссии(Элемент)
...
#Область злОбработчикиСобытийЭлементовТаблицыФормыСоставКомиссии

&НаКлиенте
Процедура Подключаемый_злСоставКомиссииПриИзменении(Элемент)
...
#Область злОбработчикиСобытийЭлементовТаблицыФормыКассы

&НаКлиенте
Процедура Подключаемый_КассыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
&НаКлиенте
Процедура злОбновитьЗаголовкиСтраниц()

	КоличествоОтветственных = 0;
	
	Если ЗначениеЗаполнено(Объект.злПредседательКомиссии) Тогда
		КоличествоОтветственных = КоличествоОтветственных + 1;	
	КонецЕсли;
	
	Для каждого СтрокаТаблицы Из Объект.злСоставКомиссии Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.ЧленКомиссии) Тогда
			КоличествоОтветственных = КоличествоОтветственных + 1;
	    КонецЕсли;
	КонецЦикла;
	
	Если КоличествоОтветственных > 0 Тогда
		ТекстШаблона = НСтр("ru = 'Ответственные (%1)'");
		ТекстЗаголовкаСтраницы = СтрШаблон(ТекстШаблона, КоличествоОтветственных);
	Иначе
		ТекстЗаголовкаСтраницы = НСтр("ru = 'Ответственные'");
	КонецЕсли;
	
	ЭлементФормы = Элементы.Найти("злСтраницаОтветственные");
	
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = ТекстЗаголовкаСтраницы;	
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура злУправлениеЭлементамиФормы()

	// "ЧленКомиссии" колонка таблицы "злСоставКомиссии"
	
	ЭлементФормы = Элементы.Найти("злСоставКомиссииЧленКомиссии");
	
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
	    Если ЗначениеЗаполнено(ЭтотОбъект["злПодразделение"]) Тогда 
			НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.КР_Подразделение", "злПодразделение");
...
Процедура злМодификацияФормы(Отказ, СтандартнаяОбработка)
	...
	РеквизитПодразделение = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
											"злПодразделение", 
	...
	// Добавление события "ПриИзменении", элементу "КассоваяКнига"
	ЭлементФормы = Элементы.Найти("КассоваяКнига");
	
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.УстановитьДействие("ПриИзменении", "Подключаемый_злКассоваяКнигаПриИзменении");
	КонецЕсли;
	
	// Добавление события "ПриИзменении", элементу "Кассы"
	ЭлементТаблица = Элементы.Найти("Кассы");
	
	Если ЭлементТаблица <> Неопределено Тогда
		ЭлементТаблица.УстановитьДействие("ПриНачалеРедактирования", "Подключаемый_КассыПриНачалеРедактирования");
	КонецЕсли;
	
	// Добавление Страниц формы: Основная и Ответственные
	ЭлементСтраницы = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
							"злСтраницы",, Элементы.ГруппаНомераОрдеров);
																   
	ЭлементСтраницы.Вид = ВидГруппыФормы.Страницы;
	
	СтраницаОсновная = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект, "злСтраницаОсновная", ЭлементСтраницы);
	СтраницаОсновная.Вид 		= ВидГруппыФормы.Страница;
	СтраницаОсновная.Заголовок 	= НСтр("ru = 'Основная'"); 
	СтраницаОсновная.Картинка 	= БиблиотекаКартинок.РазделФинансы16;
	
	СтраницаОтветственные = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект, "злСтраницаОтветственные", ЭлементСтраницы);
	СтраницаОтветственные.Вид 		= ВидГруппыФормы.Страница;
	СтраницаОтветственные.Заголовок = НСтр("ru = 'Ответственные'");    
	СтраницаОтветственные.Картинка 	= БиблиотекаКартинок.ГрупповоеОбсуждение;
	
	// Переместим таблицу Кассы на страницу Основная
	Элементы.Переместить(Элементы.Кассы, СтраницаОсновная);

	// "злПредседательКомиссии" распологается на странице Ответственные
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
							"Объект.злПредседательКомиссии", СтраницаОтветственные);
	ЭлементФормы.УстановитьДействие("ПриИзменении", "Подключаемый_злПредседательКомиссии");
	
	// "злСоставКомиссии" таблица формы распологается на странице Ответсвенные
	ЭлементТаблица = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
							"Объект.злСоставКомиссии", СтраницаОтветственные);
	ЭлементТаблица.УстановитьДействие("ПриИзменении", "Подключаемый_злСоставКомиссииПриИзменении");
	
	// "ЧленКомиссии" колонка таблицы "злСоставКомиссии"
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
							"Объект.злСоставКомиссии.ЧленКомиссии", ЭлементТаблица);																 
	ЭлементФормы.КартинкаШапки = БиблиотекаКартинок.ПользовательСистемыВзаимодействия;	
	ЭлементФормы.УстановитьДействие("ПриИзменении", "Подключаемый_злСоставКомиссииЧленКомиссииПриИзменении");
	
    // "Должность" колонка таблицы "злСоставКомиссии"
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
							"Объект.злСоставКомиссии.Должность", ЭлементТаблица); 
																 
	ЭлементФормы.КартинкаШапки = БиблиотекаКартинок.НазначениеВидовЦенДляСоглашения;
...
&НаСервереБезКонтекста
Функция злПолучитьДолжностьФизическогоЛица(ФизическоеЛицо)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ФизическиеЛица.КР_Должность КАК Должность
	               |ИЗ
	...
	Пока Выборка.Следующий() Цикл
		Должность = СокрЛП(Выборка.Должность);
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция злПолучитьПодразделениеКассовойКниги(КассоваяКнига)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	КассовыеКниги.КР_Подразделение КАК Подразделение
	               |ИЗ
	               |	Справочник.КассовыеКниги КАК КассовыеКниги
	               |ГДЕ
	               |	КассовыеКниги.Ссылка = &Ссылка";
	
	#КонецОбласти
	
	Запрос.Текст = ТекстЗапроса;
	
	Запрос.УстановитьПараметр("Ссылка", КассоваяКнига);
...
&НаСервереБезКонтекста
Функция злПолучитьПодразделениеКассы(Касса)
...
&НаСервереБезКонтекста
Функция злПолучитьВсеПодразделенияТаблицыКассы(МассивДанных)
	
	СоответствиеПодразделениеКассы = Новый Соответствие();
	...
	Пока Выборка.Следующий() Цикл
		СоответствиеПодразделениеКассы.Вставить(Выборка.Касса, Выборка.Подразделение);
	КонецЦикла;

	Возврат(СоответствиеПодразделениеКассы);
