...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.ДатаИсправления)
		...
		Если Реквизиты.Корректировочный Тогда
			ИнформацияПоИсправлению = НСтр("ru='Исправление %НомерИсправления% от %ДатаИсправления% (корректировочный)'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.5.29";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("6eb8b0b2-3ceb-4e35-a02d-eb6b9e31d4db");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"", а так же служебные реквизиты документа ""Счет-фактура полученный (налоговый агент)"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ГДЕ
	|				СчетФактураПолученныйНалоговыйАгент.Ссылка = СчетФактураПолученныйНалоговыйАгентДокументыОснования.Ссылка
	|				И СчетФактураПолученныйНалоговыйАгентДокументыОснования.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|				И СчетФактураПолученныйНалоговыйАгентДокументыОснования.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МассивТЧ = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ДокументыОснования");
			...
			Для Каждого СтрокаТаблицы Из ДокументОбъект.ДокументыОснования Цикл
				...
				Если ВыборкаОснований.НайтиСледующий(ОтборОснования) Тогда
					...
					Для Каждого Реквизит Из НовыеРеквизиты Цикл
						...
						Если ЗначениеЗаполнено(ДокументОбъект[Реквизит])
							Или (ЗначенияРеквизитов.Свойство(Реквизит)
