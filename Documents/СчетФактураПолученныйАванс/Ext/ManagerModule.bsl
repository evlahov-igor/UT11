...
Функция ПолучитьВыданныеАвансы(Организация, НачалоПериода, КонецПериода, РасчетныйДокумент = Неопределено, ВРазрезеСтавокНДС = Ложь) Экспорт
	...
	ПоляГруппировки = "Поставщик, ДокументОснование, ТипДокументаОснования, НалогообложениеНДС,"
						...
						+ "Номер, Дата, НомерСчетаФактуры, ДатаСчетаФактуры, СчетФактура";
	...
	Пока ВыборкаАванс.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаАванс.ДокументОснование)
			...
			ИЛИ (ЗначениеЗаполнено(ВыборкаАванс.ОбъектРасчетов)
				И ТипЗнч(ВыборкаАванс.ОбъектРасчетов.Объект) = Тип("СправочникСсылка.Претензии")) Тогда
			Продолжить;
		КонецЕсли;
...
Функция РезультатЗапросаПоАвансам(НачалоПериода, КонецПериода, Организация, РасчетныйДокумент = Неопределено, МенеджерВременныхТаблиц)
	...
	|		И  СчетФактураПолученныйАванс.Контрагент     = СчетФактураПолученныйИсправление.Контрагент
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5757b340-1043-42f4-9f67-e5f0c1307020");
	...
	Блокируемые.Добавить(Метаданные.Документы.СчетФактураПолученныйАванс.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.СчетФактураПолученныйАванс";
	...
	|			ГДЕ
	|				СчетФактураПолученныйАванс.Ссылка = СчетФактураПолученныйАвансАвансы.Ссылка
	|				И ((СчетФактураПолученныйАвансАвансы.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|				И СчетФактураПолученныйАвансАвансы.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|				ИЛИ СчетФактураПолученныйАвансАвансы.ИдентификаторСтроки = """"))";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта        = "Документ.СчетФактураПолученныйАванс";
