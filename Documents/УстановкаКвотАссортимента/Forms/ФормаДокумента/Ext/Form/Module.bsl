...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
...
Процедура ДатаНачалаДействияПриИзменении(Элемент)
	// << 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
	// Дата начала действия теперь может быть любым числом месяца
...
Процедура КоллекцияНоменклатурыПриИзменении(Элемент)
	...
	// << 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
	ЭтаФорма["КР_ВремяГодаКалендарное"] = КР_ЗаполнитьВремяГодаКалендарное(Объект.КоллекцияНоменклатуры);
	// >> 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
...
Функция НаличиеПлана()
	...
	Если ЗначениеЗаполнено(Объект.ДатаНачалаДействия) Тогда
		...
		Возврат ЕстьПлан;
	КонецЕсли;
	// Дальнейшая проверка не актуальна
	// >> 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
	ЕстьПлан = Ложь;
...
Процедура ЗаполнитьПодчиненныеСтроки(ПриемникРодитель, ИсточникРодитель, ДополнительныеВиды)
	...
	Для Каждого СтрокаИсточника Из ИсточникРодитель.Строки Цикл
		...
		// << 18.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2574
		//// << 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
		//// Построение дерева иерархии останавливаем на уровне товарных категорий (не строим до марок)
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КР_ВремяГодаКалендарное = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ВремяГодаКалендарное",
		Новый ОписаниеТипов("ПеречислениеСсылка.КР_ВременаГодаКалендарные"),
		,
		"Время года календарное");
	
	#КонецОбласти
	
	#Область КР_ДобавлениеЭлементовФормы
	
	// Создадим набор страниц формы
	ЭлементФормыСтраницыФормы = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_СтраницыТабличныхЧастей", , Элементы.ГруппаВидДерева);
	ЭлементФормыСтраницыФормы.Вид = ВидГруппыФормы.Страницы;
	
	// Создадим страницу для типовых данных
	ЭлементСтраницаГруппаВидДерева = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_СтраницаГруппаВидДерева", ЭлементФормыСтраницыФормы);
	ЭлементСтраницаГруппаВидДерева.Вид = ВидГруппыФормы.Страница;
	ЭлементСтраницаГруппаВидДерева.Заголовок = НСтр("ru='Дерево категорий'");
	
	// Создадим страницу для новой табличой части "Склады"
	ЭлементСтраницаСклады = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_СтраницаСклады", ЭлементФормыСтраницыФормы);
	ЭлементСтраницаСклады.Вид = ВидГруппыФормы.Страница;
	ЭлементСтраницаСклады.Заголовок = НСтр("ru='Склады'");

	// Переместим типовую группу в созданную группу страниц
	Элементы.Переместить(Элементы.ГруппаВидДерева, ЭлементСтраницаГруппаВидДерева);

	// Добавим на страницу "Склады" одноименную таблицу значений
	ЭлементФормыСклады = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Склады", ЭлементСтраницаСклады);
	
	// Добавим в таблицу значений "Склады" реквизит "Склад"
	ЭлементФормыСтраницыФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Склады.Склад", ЭлементФормыСклады);
	
	// Добавим элемент формы КР_ВремяГодаКалендарное
	ЭлементВремяГодаКалендарное = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"КР_ВремяГодаКалендарное", Элементы.ШапкаПраво);
	ЭлементВремяГодаКалендарное.ТолькоПросмотр					= Истина;
	ЭлементВремяГодаКалендарное.АвтоМаксимальнаяШирина			= Ложь;
	ЗначениеМаксимальнойШирины									= 28;
	ЭлементВремяГодаКалендарное.МаксимальнаяШирина				= ЗначениеМаксимальнойШирины;
	
	#КонецОбласти
	
	Элементы.ОбъектПланирования.Видимость						= Ложь;
	// Отключение видимости команд формы
	Элементы.Заполнить.Видимость								= Ложь;
	Элементы.ДеревоКатегорийДобавитьМарку.Видимость				= Ложь;
	Элементы.ДеревоКатегорийЗагрузитьКвотыИзФайла.Видимость		= Ложь;
	// Отключение видимости колонок дерева категорий
	Элементы.ДеревоКатегорийКвота.Видимость						= Ложь;
	Элементы.ДеревоКатегорийПроцентОтклонения.Видимость			= Ложь;
	
	Элементы.ДеревоКатегорийПредставление.Заголовок				= Нстр("ru='Вид / Категория'");
	
	ЭтаФорма["КР_ВремяГодаКалендарное"] = КР_ЗаполнитьВремяГодаКалендарное(Объект.КоллекцияНоменклатуры);
	
КонецПроцедуры // >> 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921

// << 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
&НаСервереБезКонтекста
Функция КР_ЗаполнитьВремяГодаКалендарное(КоллекцияНоменклатуры)
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КоллекцияНоменклатуры, "КР_ВремяГодаКалендарное");
	
КонецФункции // >> 07.01.2023 Федоров Д.Е., КРОК, JIRA№A2105505-921
