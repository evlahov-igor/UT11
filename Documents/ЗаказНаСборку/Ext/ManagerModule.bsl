...
Процедура ВременныеТаблицыТоваровЗаказа(МенеджерВременныхТаблиц, Заказ) Экспорт
	...
	Запрос.Текст =
		...
		|					ВЫБОР КОГДА Товары.Ссылка.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|									И НЕ Товары.Ссылка.ОбосабливатьПоНазначениюЗаказа ТОГДА
...
//
Функция ДоступныеОстаткиПараметрыВстраиванияСборка() Экспорт
...
//  см. ОбеспечениеВДокументахСервер.ДоступныеОстаткиПараметрыВстраивания
//
Функция ДоступныеОстаткиПараметрыВстраиванияРазборка() Экспорт
...
Функция ПараметрыВыполненияКомандУстановкиДействий(ВсеЗаказы, КодДействия) Экспорт
	...
	Результат = Новый Структура("Заказы,ПараметрыВстраивания",
		...
		Документы.ЗаказНаСборку.ДоступныеОстаткиПараметрыВстраиванияСборка());
	Результаты.Добавить(Результат);
	Результат = Новый Структура("Заказы,ПараметрыВстраивания",
		РезультатЗапроса[1].Выгрузить().ВыгрузитьКолонку("Ссылка"),
		Документы.ЗаказНаСборку.ДоступныеОстаткиПараметрыВстраиванияРазборка());
	Результаты.Добавить(Результат);
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	ТабЧасть.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров)
		|		И ТабЧасть.Количество > 0
		|		И ТабЧасть.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КОбеспечению)
		|		И ТабЧасть.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить)";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|			И Реквизиты.ВариантОбеспечения В(
	...
	Запрос.Текст = ОбеспечениеВДокументахСервер.ДобавитьТекстВременнойТаблицыДокументыДляЗаменыКОбеспечениюНаТребуется(
		Запрос.Текст, Документы.ЗаказНаСборку);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказНаСборку"; ИмяТЧ = "Товары";
	...
	Текст = СтрЗаменить(Текст, "Товары.Ссылка = Реквизиты.Ссылка",
		"Товары.Ссылка = Реквизиты.Ссылка И Реквизиты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СборкаТоваров)");
	ТекстыЗапроса.Добавить(Текст);
	
	Текст =
		"ВЫБРАТЬ
		|	Реквизиты.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ДокументыДляОбработкиВариантаОбеспеченияШапка
		|ИЗ
		|	Документ.ЗаказНаСборку КАК Реквизиты
		|ГДЕ
		|	Реквизиты.Ссылка В(&Ссылки)
		|		И Реквизиты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров)
		|		И Реквизиты.ВариантОбеспечения В(
		|						ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОбособленно),
		|						ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно))
		|ИНДЕКСИРОВАТЬ ПО
	...
	Текст = ОбеспечениеВДокументахСервер.ТекстЗапросаВременнойТаблицыДокументыДляЗаменыКОбеспечениюНаТребуется(Документы.ЗаказНаСборку);
	ТекстыЗапроса.Добавить(Текст);
	...
	ТекстЗапроса =
		...
		|	НЕ ДокументыДляОбработкиВариантаОбеспеченияШапка.Ссылка ЕСТЬ NULL КАК ТребуетсяОбработкаВариантаОбеспеченияШапка,
		|	НЕ ДокументыДляЗаменыКОбеспечениюНаТребуется.Ссылка ЕСТЬ NULL КАК ЗаменитьКОбеспечениюНаТребуется
		|ИЗ
		|	Документ.ЗаказНаСборку КАК Документы
		|		
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыДляОбработкиВариантаОбеспеченияШапка КАК ДокументыДляОбработкиВариантаОбеспеченияШапка
		|		ПО ДокументыДляОбработкиВариантаОбеспеченияШапка.Ссылка = Документы.Ссылка
		|		
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ОбеспечениеВДокументахСервер.ОбработатьНеиспользуемыеВариантыОбеспечения(
					ДокументОбъект,
					ОбъектИзменен,
					Документ.ТребуетсяОбработкаВариантаОбеспеченияШапка,
					Документ.ЗаменитьКОбеспечениюНаТребуется,
					Ложь,
					Ложь);
...
Функция ТекстЗапросаДокументыДляЗаменыКОбеспечениюНаТребуется() Экспорт
	...
	ТекстЗапроса =
		...
		|		И Сведения.ЗаказНаОтгрузку ССЫЛКА Документ.ЗаказНаСборку
		...
		|				Документ.ЗаказНаСборку КАК Реквизиты
		|			ГДЕ
		|				Реквизиты.Ссылка = Сведения.ЗаказНаОтгрузку
		|					И Реквизиты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазборкаТоваров)
		|					И Реквизиты.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.РезервироватьПоМереПоступления))";
	Возврат ТекстЗапроса;
