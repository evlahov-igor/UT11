...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// Нетиповые движения по ТоварыКОтбору делаем не через общий модуль 
		//	так как потребуется много изменений типового кода
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеШапки.СкладОтправитель		КАК Склад,       
	// << 31.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	|	ДанныеШапки.СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки
	|										КАК СкладАдресныйСкладБезордернойОтгрузки,       
	// >> 31.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	|	ТоварыДокумента.Номенклатура		КАК Номенклатура,
	|	ТоварыДокумента.Характеристика		КАК Характеристика,
	|	ВЫБОР
	...
	ДоступноИзменениеТоварыКОтгрузке = КР_ПроведениеДокументовПереопределяемый.ДоступноИзменениеШаблоновПроведения(
		"ТоварыКОтгрузке", "ЗапланироватьОтгрузкуТоваров", "ШаблонТекстаЗапроса", Регистры);
	
	Если ДоступноИзменениеТоварыКОтгрузке Тогда 	
		
		ПараметрыШаблона = Регистры.ТоварыКОтгрузке.ШаблонТекстаЗапроса;
		// Переопределеное полей выборки
		ПараметрыШаблона.КР_КСборке = 
		"ВЫБОР 
		|	КОГДА ДанныеДокумента.СкладАдресныйСкладБезордернойОтгрузки
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если КомандаСоздатьНаОсновании <> Неопределено Тогда 
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Отбор товаров'");

		// A2105505-2520
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "КР_АдресныйСкладБезордернойОтгрузки";
		//
...
Функция КР_ТекстЗапросаТаблицаТоварыКОтбору(Запрос, ТекстыЗапроса, Регистры)
	...
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) 	КАК ВидДвижения,
	|	ДанныеШапки.Ссылка						КАК Регистратор,
	|	ДанныеШапки.Дата						КАК Период,
	|	ДанныеШапки.Ссылка						КАК Распоряжение,
	|	ТаблицаТовары.Номенклатура				КАК Номенклатура,
	|	ТаблицаТовары.Характеристика			КАК Характеристика,
	|	ТаблицаТовары.Назначение				КАК Назначение,
	|	ВЫБОР
	...
	|	ТаблицаТовары.Количество				КАК КОтбору,
	|	0										КАК Отобрано
	|
	|ИЗ
	|	Документ.ЗаказНаПеремещение.Товары КАК ТаблицаТовары
...
// Создан: 26.08.2022, Мельников А.А., КРОК, Jira№ A2105505-377 
// Проверяет разрешено ли редактирование данного объекта  
// Параметры: 
// РеквизитыДокумента - Структура, Объект - нечто, к чему можно через точку получить реквизиты,
//                                          указанные в КР_РеквизитыПроверкиРазрешенияРедактирования(). 
...
Процедура КР_ПроверитьРазрешениеРедактирования(РеквизитыДокумента, Отказ) Экспорт
	...
	Если  Пользователи.РолиДоступны("КР_ДобавлениеИзменениеЗаказовНаПеремещениеРасширенный") Тогда
		Возврат;
	КонецЕсли;
	      
	ВсеСтрокиВВариантеОтгрузить = Истина;
	Для Каждого СтрокаТовара Из РеквизитыДокумента.Товары Цикл
		Если СтрокаТовара.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.Отгрузить Тогда
			ВсеСтрокиВВариантеОтгрузить = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	ЗапретРедактирования = РеквизитыДокумента.Проведен И ВсеСтрокиВВариантеОтгрузить 
							И РеквизитыДокумента.Статус = Перечисления.СтатусыВнутреннихЗаказов.КВыполнению;   
	
	Если ЗапретРедактирования  Тогда
		ТекстОшибки = НСтр("ru='Документ в статусе ""К выполнению"" запрещен к редактированию. 
								|Обратитесь к администратору.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , , , Отказ);		
	КонецЕсли; 
	
КонецПроцедуры // >> 22.08.2022, Мельников А.А., КРОК, Jira№ A2105505-377 
...
Функция КР_РеквизитыПроверкиРазрешенияРедактирования() Экспорт
	...
	Возврат Новый Структура("Статус, Товары, Проведен");	  
	
КонецФункции // >> 22.08.2022, Мельников А.А., КРОК, Jira№ A2105505-377 

// << 28.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-933
// Очищает строки табличной части у которых заполнен <Короб>
// Параметры: 
//  ТабличнаяЧасть - ДанныеФормыКоллекция - Табличная часть текущего документа.
//
Процедура КР_ОчиститьСтрокиПриИзмененииСклада(ТабличнаяЧасть) Экспорт
	
	СчетчикСтрок = ТабличнаяЧасть.Количество();
	
	Пока СчетчикСтрок > 0  Цикл
		
		ТекущаяСтрока = ТабличнаяЧасть[СчетчикСтрок - 1];
		Если ЗначениеЗаполнено(ТекущаяСтрока.КР_Короб) Тогда
...
Функция КР_ТекстЗапросаТаблицаКоробаНаСкладах(ТекстыЗапроса, Регистры)
	...
	|	ТоварыДокумента.Ссылка.СкладОтправитель КАК Склад,
	...
	|	И ТоварыДокумента.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыВнутреннихЗаказов.КВыполнению)
	|	И ТоварыДокумента.Ссылка.СкладОтправитель.КР_УчетОстатковКоробов
...
Функция КР_ПолучитьЗначениеРасклипсовки(ЗаказНаПеремещение) Экспорт
	
	РеквизитыСкладаОтправителя	= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаказНаПеремещение.СкладОтправитель,
		"КР_ТипПротивокражныхВорот");
	РеквизитыСкладаПолучателя	= ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаказНаПеремещение.СкладПолучатель,
		"КР_ТипПротивокражныхВорот, КР_ВидСклада");
	
	Если РеквизитыСкладаОтправителя.КР_ТипПротивокражныхВорот = РеквизитыСкладаОтправителя.КР_ТипПротивокражныхВорот Тогда
		ПризнакРасклипсовки = Ложь;
	ИначеЕсли РеквизитыСкладаПолучателя.КР_ВидСклада = Перечисления.КР_ВидыСкладов.СкладКонсервации Тогда
		ПризнакРасклипсовки = Истина;
	Иначе
		ПризнакРасклипсовки = Истина;
	КонецЕсли;
	
	Возврат ПризнакРасклипсовки;
	
КонецФункции // >> 28.03.2023, Федоров Д.Е., КРОК, JIRA№ A2105505-1112

// << 28.03.2023, Федоров Д.Е., КРОК, JIRA№ A2105505-1112
Процедура КР_ЗаписатьЗначениеРасклипсовки(ЗаказНаПеремещение, Значение) Экспорт
	
	РасклипсовыватьТовар = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.КР_РасклипсовыватьТовар;
	ПараметрыПоискаСтрок = Новый Структура;
	ПараметрыПоискаСтрок.Вставить("Свойство", РасклипсовыватьТовар);
		
	ТЧРеквизиты = ЗаказНаПеремещение.ДополнительныеРеквизиты;
		
	МассивСтрок = ТЧРеквизиты.НайтиСтроки(ПараметрыПоискаСтрок);
	Если МассивСтрок.Количество() = 0 Тогда
		НоваяСтрока = ТЧРеквизиты.Добавить();
		НоваяСтрока.Свойство = РасклипсовыватьТовар;
