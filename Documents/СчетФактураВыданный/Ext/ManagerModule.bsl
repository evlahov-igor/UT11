...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	ИменаРеквизитов = "Склад,Дата,Перевыставленный";
...
//
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен = Истина, ПрефиксПредставления="") Экспорт
...
Функция ПараметрыЗаполненияПоСчетуФактуре(СчетФактура) Экспорт
	...
	Если РезультатЗапроса[4].Пустой() Тогда
		...
		Иначе
			...
			Выборка.Следующий();
			ПараметрыЗаполнения.Исправление = Выборка.Исправление;
			ПараметрыЗаполнения.Корректировочный = Выборка.Корректировочный;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("Перевыставленный",					Реквизиты.Перевыставленный);
	...
	Запрос.УстановитьПараметр("РеализацияЧерезКомиссионера",		Реквизиты.РеализацияЧерезКомиссионера);
	Запрос.УстановитьПараметр("ОснованиеДокументОснование",			Реквизиты.ОснованиеДокументОснование);
...
Функция ТекстЗапросаВтТаблицаДанныхДляЖурнала(Запрос, ТекстыЗапроса)
	...
	|	СУММА(СчетФактураТовары.СуммаНДСУменьшение),
	|	ИСТИНА,
	|	СчетФактураТовары.СчетФактураПолученныйОтПродавца.Контрагент,
	|	СчетФактураТовары.СчетФактураПолученныйОтПродавца.ИННКонтрагента,
	|	СчетФактураТовары.СчетФактураПолученныйОтПродавца.КППКонтрагента,
	|	СчетФактураТовары.СчетФактураПолученныйОтПродавца.Номер,
	|	СчетФактураТовары.СчетФактураПолученныйОтПродавца.ДатаСоставления,
	|	МИНИМУМ(СчетФактураТовары.НомерСтроки - 1)
	|ИЗ
	|	ВТТаблицаТоваров КАК СчетФактураТовары
	|ГДЕ
...
Функция ТаблицаТоваров(ПараметрыЗаполнения) Экспорт
	...
	|		ТОГДА вт_ТаблицаТоваров.ИсходныйСчетФактура
	|		ИНАЧЕ ЕСТЬNULL(вт_КрайниеСчетаФактуры.ПредыдущийСчетФактура, вт_ТаблицаТоваров.ИсходныйСчетФактура)
	|	КОНЕЦ КАК ПредыдущийСчетФактура,
	|	ВЫБОР КОГДА &РеализацияЧерезКомиссионера
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ЕСТЬNULL(вт_КрайниеСчетаФактуры.ПредыдущийСчетФактура.Корректировочный, ЛОЖЬ)
	|	КОНЕЦ КАК Корректировочный,
	|	ВЫБОР КОГДА &РеализацияЧерезКомиссионера
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ЕСТЬNULL(вт_КрайниеСчетаФактуры.ПредыдущийСчетФактура.Исправление, ЛОЖЬ)
...
Функция ТекстЗапросаДанныхПрослеживаемость(МассивОснований)
	...
	|	И РеализацияУслугПрочихАктивовПрослеживаемыеТовары.НомерГТД.РНПТ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	ОсновноеСредство
	...
	|	СУММА(ПрослеживаемыеТовары.КоличествоПоРНПТ) КАК КоличествоПоРНПТ
	|ИЗ
...
// Возвращает соответствие представлений номеров переданным счетам-фактурам.
// Учитывается цифровой индекс обособленного подразделения.
//
// Параметры:
//  МассивСчетовФактур - Массив - массив ссылок на документы Счет-фактура выданный, для которых нужно получить представление номера
//
// Возвращаемое значение:
...
Функция ПолучитьПокупателейПоОснованию(ДокументыОснования, ДатаСведений) Экспорт
	...
	Если ДокументыОснования.Количество() > 0 Тогда // И Объект.РеализацияЧерезКомиссионера 
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.Контрагенты.ЗаполнитьЭлементыНачальнымиДанными";
	НоваяСтрока.Порядок = "Любой";
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсиюТовары
	...
	Обработчик.Версия = "2.5.3.19";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5a899d9c-607e-49ec-a2df-9e1c3bf62c5d");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюТовары(Параметры) Экспорт
	...
	|	И ДокументыОснованияСчетаФактуры.Ссылка.Проведен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			Документ.СчетФактураВыданный.Товары КАК СчетФактураВыданныйТовары
	|		ГДЕ
	|			СчетФактураВыданный.Ссылка = СчетФактураВыданныйТовары.Ссылка
	|			И СчетФактураВыданныйТовары.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И СчетФактураВыданныйТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)))
	|"
	);
...
Процедура ПроверитьПредставлениеНомера(СчетФактураОбъект, ПредставленияНомеров, ОбъектИзменен)
	...
	// Заполним представление номера, если оно пустое или отличается от представления номера в счете-фактуре.
	
	ПредставлениеНомера = ПредставленияНомеров.Получить(СчетФактураОбъект.Ссылка);
	Если Не ПредставлениеНомера = Неопределено
		И (Не ЗначениеЗаполнено(СчетФактураОбъект.ПредставлениеНомера)
			Или Не СчетФактураОбъект.ПредставлениеНомера = ПредставлениеНомера)	Тогда
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(СчетФактураОбъект, ОбъектИзменен)
	...
	Если (СчетФактураОбъект.Исправление Или СчетФактураОбъект.Корректировочный)
		  И СчетФактураОбъект.Проведен
		  И СчетФактураОбъект.Товары.Количество() = 0 Тогда
		Попытка
			РезультатЗаполнения = СчетФактураОбъект.ТаблицаТоваров();
		Исключение
