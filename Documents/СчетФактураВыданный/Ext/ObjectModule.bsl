...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктВыполненныхРабот")
		Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РеализацияУслугПрочихАктивов") Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Перевыставленный И НЕ Исправление Тогда
...
Процедура ЗаполнитьПоДокументуОснованию(ДанныеЗаполнения)
	...
	Если Не ПараметрыСчетаФактуры.Покупатели = Неопределено
...
Функция ТаблицаТоваров(ДанныеЗаполнения = Неопределено) Экспорт
	...
	ПараметрыЗаполнения.Вставить("РеализацияЧерезКомиссионера", РеализацияЧерезКомиссионера);
...
Процедура ЗаполнитьНомераСтрокПриПечати(РезультатЗаполнения)
	...
	Иначе
		...
		Если РучнаяКорректировкаСуммДокумента И ДокументыОснования.Количество() > 0 Тогда
			...
			ИначеЕсли Исправление И Не Корректировочный И Не Перевыставленный И Не РучнаяКорректировкаСуммДокумента 
