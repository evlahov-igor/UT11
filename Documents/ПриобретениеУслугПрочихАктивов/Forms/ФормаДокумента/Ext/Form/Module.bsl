...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.СформироватьВедомость.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрослеживаемыхИмпортныхТоваров");
	...
	Для каждого Стр Из Объект.Расходы Цикл
		Стр.ВедетсяУчетПоРНПТ = ТоварПрослеживается(Стр.НомерГТД, Стр.КодТНВЭД);
	КонецЦикла;
	...
	УчетПрослеживаемыхТоваровЛокализация.УстановитьВидимостьКоличестваРНПТ(ЭтаФорма, Объект.Дата, "РасходыГруппаКоличествоПоРНПТ", Истина);
	...
	// << 01.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918 
...
Процедура РаспределитьНаПоступления(Команда)
	...
	КР_ДетализироватьДоКоллекций = Неопределено;  
	// >> 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
	...
	Иначе
		...
		// << 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
		ПараметрыФормы.Вставить("КР_ДетализироватьДоКоллекций", КР_ДетализироватьДоКоллекций);  
		// >> 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
...
Процедура УстановитьУсловноеОформление()
	...
	ЦеныПредприятияЗаполнениеСервер.УстановитьУсловноеОформлениеЦенаВключаетНДС(ЭтаФорма, "РасходыСуммаСНДС");
	...
	УчетПрослеживаемыхТоваровЛокализация.УстановитьУсловноеОформлениеКоличестваПоРНПТ(ЭтаФорма,"РасходыКоличествоПоРНПТ","Объект.Расходы.ВедетсяУчетПоРНПТ");
...
Функция ПоместитьВыделенныеСтрокиВХранилище(Знач ВыделенныеСтроки
	...
	// << 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
	КР_ДетализироватьДоКоллекций = Ложь;
	// >> 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
	...
	Для Каждого ТекСтрока Из ВыделенныеСтроки Цикл
		...
		Если СтрокаТаблицы = Неопределено    
			...
			// << 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
			И Не ЗначениеЗаполнено(СтрокаТаблицы.КР_СтатьяРасходовБазовая)
			// >> 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918    
		...
		// << 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918   
		Если ЗначениеЗаполнено(СтрокаТаблицы.КР_СтатьяРасходовБазовая) Тогда   
			КР_ДетализироватьДоКоллекций = Истина;
			НоваяСтрока.СтатьяРасходов = СтрокаТаблицы.КР_СтатьяРасходовБазовая;  
			// КОСТЫЛЬ                       
			// Устанавливаем тип аналитики так как она можен быть не верной
			ПриобретениеТоваровУслугПустаяСсылка = Документы.ПриобретениеТоваровУслуг.ПустаяСсылка();   
			НоваяСтрока.АналитикаРасходов = ПриобретениеТоваровУслугПустаяСсылка;
		КонецЕсли;	
		// >> 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918    
	КонецЦикла;
...
Функция ТоварПрослеживается(НомерГТД, КодТНВЭД)
	...
	Если ЗначениеЗаполнено(НомерГТД) Тогда
		РНПТ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НомерГТД,"РНПТ");
	Иначе 
		РНПТ = Ложь;
	КонецЕсли;
	...
	Если РНПТ И ПрослеживаемыйТовар Тогда
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "РасходыНомерГТД" ИЛИ Элемент.Имя = "РасходыКодТНВЭД" Тогда 
		РеквизитыПрослеживаемостиПриИзмененииНаСервере();
	Иначе
...
&НаСервере
Процедура РеквизитыПрослеживаемостиПриИзмененииНаСервере()
	
	Объект.Расходы[Элементы.Расходы.ТекущаяСтрока].ВедетсяУчетПоРНПТ = ТоварПрослеживается(Объект.Расходы[Элементы.Расходы.ТекущаяСтрока].НомерГТД, Объект.Расходы[Элементы.Расходы.ТекущаяСтрока].КодТНВЭД);
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормыРасходыСтатьяРасходов = Элементы.РасходыСтатьяРасходов;
	
	// Расходы.КР_СтатьяРасходовБазовая
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Расходы.КР_СтатьяРасходовБазовая", , ЭлементФормыРасходыСтатьяРасходов); 
	
	// Скопируем и расширим параметы выбора из типового элемента Формы
	ЭлементФормыПараметрыВыбора = Новый Массив(ЭлементФормыРасходыСтатьяРасходов.ПараметрыВыбора);
	ЭлементФормыПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.КР_БазоваяСтатья",
		ПланыВидовХарактеристик.СтатьиРасходов.ПустаяСсылка()));
	ЭлементФормы.ПараметрыВыбора = Новый ФиксированныйМассив(ЭлементФормыПараметрыВыбора);
	
	// Расходы.КР_Коллекция
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Расходы.КР_Коллекция", , ЭлементФормыРасходыСтатьяРасходов);
	// >> 01.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
