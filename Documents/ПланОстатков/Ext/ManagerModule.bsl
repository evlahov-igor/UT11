...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.ПланОстатков.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.3.10";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("65eaf3dc-9950-402e-a071-467bce30fb35");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ПланОстатков.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Выполняет замещение планов остатков:
	...
	Изменяемые.Добавить(Метаданные.Документы.ПланОстатков.ПолноеИмя());
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ЗамещениеПланов.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ПланОстатков.ПолноеИмя());
...
Функция ТекстЗапросаДанныеКОбработке(ВыбратьДляРегистрацииДвиженийОборотыДвижений = Ложь) Экспорт
	...
	|	ПланОстатков.Статус КАК Статус,
	...
	|	ПланыОстатков.Статус КАК СтатусЗамещения
	...
	|			И (ПланыОстатков.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.НаУтверждении)
	|					И ПланыОстатков1.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Утвержден)
	|				ИЛИ ПланыОстатков.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Утвержден)
	|					И ПланыОстатков1.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Утвержден))
	...
	|ПОМЕСТИТЬ ПланыОстатковКОбработке
	|ИЗ
	...
	|			И ЗамещающиеИЗамещенныеПланыОстатков.СтатусЗамещения = ЗамещениеПланов.СтатусЗамещения
	|ГДЕ
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПланыОстатков.Ссылка
	|ИЗ
	|	ПланыОстатков КАК ПланыОстатков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗамещениеПланов КАК ЗамещениеПланов
	|		ПО (ЗамещениеПланов.ЗамещенныйПлан = ПланыОстатков.Ссылка)
	|ГДЕ
	|	ПланыОстатков.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПланов.Отменен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПланыОстатковКОбработке.Ссылка КАК Ссылка
	|ИЗ
	|	ПланыОстатковКОбработке КАК ПланыОстатковКОбработке
	|ГДЕ
	|	&УсловиеОтбораДляРегистрацииДвиженийОборотыБюджетов";
	
	Если ВыбратьДляРегистрацииДвиженийОборотыДвижений Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбораДляРегистрацииДвиженийОборотыБюджетов", "ПланыОстатковКОбработке.Ссылка.ОтражаетсяВБюджетировании");
	Иначе
