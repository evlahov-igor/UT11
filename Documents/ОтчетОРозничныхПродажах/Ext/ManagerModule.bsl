...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	МеханизмыДокумента.Добавить("КР_РозничныеПродажи");
	МеханизмыДокумента.Добавить("КР_РозничныеОплатыБезналичные");
	// >> 13.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166   
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// << 13.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
		КР_ТекстЗапросаТаблицаРозничныеПродажи(Запрос, ТекстыЗапроса, Регистры);
		КР_ТекстЗапросаТаблицаРозничныеОплатыБезналичные(Запрос, ТекстыЗапроса, Регистры);
		// >> 13.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	Запрос.УстановитьПараметр("КР_БазаЯвляетсяЦентральной", КР_ДополнительныеНастройкиПовтИсп.БазаЯвляетсяЦентральной());  
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725   
...
Функция ТекстЗапросаТаблицаРасчетыПоЭквайрингу(Запрос, ТекстыЗапроса, Регистры)
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ ТаблицаПлатежей.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ ТаблицаСерии.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаСерии.Номенклатура,
...
Процедура СформироватьСуммыДокументаВВалютахУчета(Запрос, ТекстыЗапроса, Регистры = Неопределено) Экспорт
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ ТаблицаДокумента.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры)
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ ТаблицаОплатаПлатежнымиКартами.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ ТаблицаТовары.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаВтАгентскиеУслуги(Запрос, ТекстыЗапроса)
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ КОформлению.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|";
...
Функция ТекстЗапросаВтСуммаПлатежнымиКартами(Запрос, ТекстыЗапроса) Экспорт
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ &КР_БазаЯвляетсяЦентральной 
	|		ИЛИ ТабличнаяЧасть.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|";
...
Процедура ОформитьОтгрузкуВРозницу(Запрос, ТекстыЗапроса, Регистры)
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константа.КР_БазаЯвляетсяЦентральной КАК КР_БазаЯвляетсяЦентральной
	|		ПО (ИСТИНА)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|ГДЕ
	...
	// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
	|	И (НЕ КР_БазаЯвляетсяЦентральной.Значение 
	|		ИЛИ ТоварыДокумента.КР_НомерЗаказаИнтернетМагазина = """"
	|	)
	// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|ГДЕ
		// << 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
		|	(НЕ КР_БазаЯвляетсяЦентральной.Значение 
		|		ИЛИ ТабЧасть.КР_НомерЗаказаИнтернетМагазина = """"
		|	) И
		// >> 19.01.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"". Заполняет реквизит ""Идентификатор документа"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ ОтчетОРозничныхПродажах.ИдентификаторДокумента = """"";
...
// << 13.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
Функция КР_ТекстЗапросаТаблицаРозничныеПродажи(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "КР_РозничныеПродажи";
	...
	"ВЫБРАТЬ
	|	ТаблицаДокумента.Дата КАК Период,
	|	ТаблицаТовары.КР_ИдентификаторЧека КАК ИдентификаторЧека,
	|	ТаблицаТовары.КР_НомерЧека КАК НомерЧека,
	|	ТаблицаТовары.КР_ДатаЧека КАК ДатаЧека,
	|	ТаблицаТовары.КР_НомерЗаказаИнтернетМагазина КАК НомерЗаказаИнтернетМагазина,
	|	ТаблицаТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаТовары.Характеристика КАК Характеристика,
	|	ТаблицаТовары.КР_КартаЛояльности КАК КартаЛояльности,
	|	ТаблицаДокумента.КассаККМ КАК КассаККМ,
	|	ТаблицаТовары.КоличествоУпаковок КАК Количество,
	// << 07.08.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2072
	|	ТаблицаТовары.Сумма - ТаблицаТовары.СуммаНДС КАК СуммаБезНДС,
	// >> 07.08.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2072
	|	ТаблицаТовары.Сумма КАК Сумма
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Товары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ТаблицаДокумента
...
КонецФункции // >> 13.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166

// << 13.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
Функция КР_ТекстЗапросаТаблицаРозничныеОплатыБезналичные(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "КР_РозничныеОплатыБезналичные";
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаДокумента.Дата КАК Период,
	|	ТаблицаОплаты.КР_НомерЧека КАК НомерЧека,
	|	ТаблицаОплаты.КР_ДатаЧека КАК ДатаЧека,
	|	ТаблицаОплаты.КР_ИдентификаторЧека КАК ИдентификаторЧека,
	|	ТаблицаОплаты.КР_НомерЗаказаИнтернетМагазина КАК НомерЗаказаИнтернетМагазина,
	|	ТаблицаДокумента.КассаККМ КАК КассаККМ,
	|	ТаблицаОплаты.Сумма КАК Сумма,
	|	ТаблицаОплаты.НомерПлатежнойКарты КАК НомерПлатежнойКарты
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.ОплатаПлатежнымиКартами КАК ТаблицаОплаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ТаблицаДокумента
	|		ПО ТаблицаОплаты.Ссылка = ТаблицаДокумента.Ссылка
	|ГДЕ
...
Функция КР_ТекстЗапросаТаблицаТоварыКОформлениюОптовыхПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И НЕ ТаблицаТовары.КР_НомерЗаказаИнтернетМагазина ПОДОБНО """"";
