...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Команда = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект,
		"КР_ЗагрузкаОперативныхПродажИзКассовогоПО",
		"Загрузка опер. продаж из кассового ПО");
		
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект,
		Команда, ,
		Элементы.ПодменюОтчеты);
	
КонецПроцедуры // >> 07.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150

// << 07.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаКлиенте
Процедура КР_ЗагрузкаОперативныхПродажИзКассовогоПО(Команда)
	
	Результат = КР_ЗагрузкаОперативныхПродажИзКассовогоПОНаСервере();
...
// << 07.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
&НаСервере
Функция КР_ЗагрузкаОперативныхПродажИзКассовогоПОНаСервере()
	
	Возврат КР_ИнтеграцияСВнешнимиСистемами.ЗагрузкаПродажArtix();
