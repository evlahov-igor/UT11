...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// << 23.05.2023 Федотов А.М., КРОК, Jira№A2105505-1688
...
Процедура ТоварыЦенаПриИзменении(Элемент)
	...
	ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	РассчитатьИтоговыеПоказателиОтчетаОРозничныхПродажах(ЭтаФорма);
	КР_ЗаполнитьПоказателиДокумента();		// #4045.. Фомин Д.Ю. 19.06.2024.
...
Процедура ТоварыСуммаПриИзменении(Элемент)
	...
	// #4045.. Фомин Д.Ю. 19.06.2024.
	РассчитатьИтоговыеПоказателиОтчетаОРозничныхПродажах(ЭтаФорма);
	КР_ЗаполнитьПоказателиДокумента();
	// ..#4045
...
Процедура КР_ПриЧтенииСозданииНаСервере()
	...
	// << 27.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_КартаЛояльности", Элементы.Товары);
	// >> 27.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	
	// Товары.КР_НомерЧека 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_НомерЧека", Элементы.Товары);
	
	// Товары.КР_ДатаЧека 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_ДатаЧека", Элементы.Товары);
	
	// Товары.КР_ИдентификаторЧека
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_ИдентификаторЧека", Элементы.Товары);
	
	// Товары.КР_НомерЗаказаИнтернетМагазина 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_НомерЗаказаИнтернетМагазина", Элементы.Товары);
	ЭлементФормы.ТолькоПросмотр = Истина;
	
	// ОплатаПлатежнымиКартами.КР_НомерЧека 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.ОплатаПлатежнымиКартами.КР_НомерЧека", Элементы.ОплатаПлатежнымиКартами);
	
	// ОплатаПлатежнымиКартами.КР_ДатаЧека 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.ОплатаПлатежнымиКартами.КР_ДатаЧека", Элементы.ОплатаПлатежнымиКартами);
	
	// ОплатаПлатежнымиКартами.КР_ИдентификаторЧека
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.ОплатаПлатежнымиКартами.КР_ИдентификаторЧека", Элементы.ОплатаПлатежнымиКартами);
	
	// ОплатаПлатежнымиКартами.КР_НомерЗаказаИнтернетМагазина 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.ОплатаПлатежнымиКартами.КР_НомерЗаказаИнтернетМагазина", Элементы.ОплатаПлатежнымиКартами); 
	ЭлементФормы.ТолькоПросмотр = Истина;
	
	// << 14.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_Закрыт", Элементы.ГруппаДатаНомер);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.Товары.ИтогКоличествоУпаковок", , Элементы.СуммаСкидки);
	ЭлементФормы.Заголовок = НСтр("ru = 'Продано товаров, шт'");
	ЭлементФормы.КнопкаВыбора = Ложь;
	
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ИтогоНаличными", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2), , НСтр("ru = 'Оплачено наличными, руб'"));
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, РеквизитФормы, , Элементы.СуммаСкидки);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.КнопкаВыбора = Ложь;
	
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ИтогоПлатежнымиКартами", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2), , НСтр("ru = 'Оплачено платежными картами, руб'"));
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, РеквизитФормы, , Элементы.СуммаСкидки);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.КнопкаВыбора = Ложь;
	
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ИтогоСБП", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2), , НСтр("ru = 'Оплачено СБП, руб'"));
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, РеквизитФормы, , Элементы.СуммаСкидки);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.КнопкаВыбора = Ложь;
	
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_КоличествоПосетителей", ОбщегоНазначения.ОписаниеТипаЧисло(10, 0), , НСтр("ru = 'Количество посетителей, чел'"));
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, РеквизитФормы, , Элементы.СуммаСкидки);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.КнопкаВыбора = Ложь;
...
// << 14.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
&НаСервере
Процедура КР_ЗаполнитьПоказателиДокумента()
	
	ОплатыПК = Объект.ОплатаПлатежнымиКартами.Выгрузить(, "ЭквайринговыйТерминал, Сумма");
	ОплатыПК.Колонки.Добавить("ЭтоТерминалСБП", Новый ОписаниеТипов("Булево"));
	
	МассивТерминалов = ОбщегоНазначения.ВыгрузитьКолонку(ОплатыПК, "ЭквайринговыйТерминал", Истина);
	ТерминалыСБП = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивТерминалов, "КР_СистемаБыстрыхПлатежей");
	
	Для Каждого СтрокаОплатыПК Из ОплатыПК Цикл
		СтрокаОплатыПК.ЭтоТерминалСБП = ТерминалыСБП.Получить(СтрокаОплатыПК.ЭквайринговыйТерминал);
	КонецЦикла;
	
	ОплатыПК.Свернуть("ЭтоТерминалСБП", "Сумма");
	
	СтрокаОплатыПК = ОплатыПК.Найти(Ложь, "ЭтоТерминалСБП");
	Если Не СтрокаОплатыПК = Неопределено Тогда
		ЭтотОбъект.КР_ИтогоПлатежнымиКартами = СтрокаОплатыПК.Сумма;
	КонецЕсли;
	
	СтрокаОплатыПК = ОплатыПК.Найти(Истина, "ЭтоТерминалСБП");
	Если Не СтрокаОплатыПК = Неопределено Тогда
		ЭтотОбъект.КР_ИтогоСБП = СтрокаОплатыПК.Сумма;
	КонецЕсли;
	
	ЭтотОбъект.КР_ИтогоНаличными = Объект.Товары.Итог("Сумма") - Объект.ОплатаПлатежнымиКартами.Итог("Сумма");
	
	РегистрыСведений.КР_ПосещаемостьМагазина.ЕстьПодсчетПосетителейНаДату(
		Объект.Склад, Объект.Дата, ЭтотОбъект.КР_КоличествоПосетителей);
