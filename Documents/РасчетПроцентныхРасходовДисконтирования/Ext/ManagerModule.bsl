...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если НЕ ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		РасчетПроцентныхРасходовДисконтирования(Запрос, Документ);
...
Процедура Использование_РасчетПроцентныхРасходовДисконтирования(ПараметрыОбработчика) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.ПроцентныеРасходыДисконтирования.Остатки(&ГраницаКонецПериода, АналитикаУчетаПоПартнерам.Организация В (&МассивОрганизаций)) КАК ПроцентныеРасходыДисконтированияОстатки";
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	ЕстьСписаниеПроцентныхРасходов = Не РезультатЗапроса[0].Пустой();
	НетПроцентовКСписанию = РезультатЗапроса[1].Пустой();
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		Пока НачалоРасчета <= ПараметрыОбработчика.ПараметрыРасчета.КонецПериода Цикл
			...
			Запрос.Текст = 
				...
				|ГДЕ
				|	РасчетПроцентныхРасходов.Проведен
				|	И РасчетПроцентныхРасходов.Дата = &ОкончаниеПериода
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			ВыборкаПоОрганизациям = РезультатЗапроса.Выбрать();
			
			ОрганизацииКСписаниюПроцентовВСледующемПериоде = Новый Массив;
			
			Пока ВыборкаПоОрганизациям.Следующий() Цикл
				
				Если НЕ ВыборкаПоОрганизациям.РасчетПроцентныхРасходов.Пустая() Тогда
					ДокументЗакрытияМесяца = ВыборкаПоОрганизациям.РасчетПроцентныхРасходов.ПолучитьОбъект();
				Иначе
...
Функция ТекстЗапросаТаблицаДвиженияКонтрагентДоходыРасходы(ТекстыЗапроса, Регистры = Неопределено)
	...
	|ИЗ
	|	ТаблицаДвиженияКонтрагентДоходыРасходы КАК Таблица
	|
	|УПОРЯДОЧИТЬ ПО
	|	Таблица.Период";
...
Процедура РасчетПроцентныхРасходовДисконтирования(Запрос, ДокументСсылка)
	...
	|	Данные.ОбъектРасчетов.Организация КАК Организация,
	|	Данные.ОбъектРасчетов.Подразделение КАК Подразделение,
	|	Данные.ОбъектРасчетов.Партнер КАК Партнер,
	|	Данные.ОбъектРасчетов.Контрагент КАК Контрагент,
	|	Данные.ОбъектРасчетов.Договор КАК Договор,
	|	Данные.ОбъектРасчетов.НаправлениеДеятельности КАК НаправлениеДеятельности,
	...
	|ИТОГИ
	|	МАКСИМУМ(Данные.Валюта),
	|	СУММА(Данные.ДолгОстаток),
	|	МАКСИМУМ(Данные.СуммаДисконтированияОстаток),
	|	МАКСИМУМ(Данные.ДисконтированиеПрекращено),
	|	МАКСИМУМ(Данные.ЗадолженностьЗакрыта),
	|	МАКСИМУМ(Данные.СтавкаДисконтирования),
	|	МАКСИМУМ(Данные.СуммаБезНДС),
	|	МАКСИМУМ(Данные.СуммаСНДС),
	|	МАКСИМУМ(Данные.СтавкаНДС)
	|ПО
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоОбъектуРасчетов.Следующий() Цикл
			...
			Если ВыборкаПоОбъектуРасчетов.ДолгОстаток = 0 Тогда
				Если 	ВыборкаПоОбъектуРасчетов.СуммаДисконтированияОстаток > 0 
