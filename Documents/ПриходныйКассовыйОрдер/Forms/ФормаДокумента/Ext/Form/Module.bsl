...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// #4385.. Фомин Д.Ю. 09.07.2024.
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПодотчетника") Тогда
		Объект.Основание = НСтр("ru = 'Возврат подотчетной суммы'");		
	КонецЕсли;	
	// ..#4385
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если Объект.ХозяйственнаяОперация <> 
		...
		Объект.КР_ПоступлениеВКассуККМ = Ложь;
	КонецЕсли;	
	// >> 21.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-725
	
	// --> Евлахов Игорь Николаевич (Начало) 25.09.2023
	// Задача #3084
	Если ЗначениеЗаполнено(Объект.ДокументОснование)
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") 
		И Объект.ХозяйственнаяОперация <> ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыККМ") Тогда
		Объект.ДокументОснование = Неопределено;	
	КонецЕсли;
	
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПодотчетника") Тогда
		// #4385.. Фомин Д.Ю. 09.07.2024.
		//Объект.Основание = НСтр("ru = 'Возврат от подотчетного лица'");		
		Объект.Основание = НСтр("ru = 'Возврат подотчетной суммы'");		
		// ..#4385
	КонецЕсли;	 
	// <-- Евлахов Игорь Николаевич (Конец) 25.09.2023
	
	ИнициализироватьРасшифровкуПлатежа();
	
	// --> Евлахов Игорь Николаевич (Начало) 25.09.2023
	// Задача #3084
	// Необходима инициализированная расшифровка платежа
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств") Тогда
		...
		Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
			...
			ТекстШаблонаПринятоОт = НСтр("ru = 'По заявлению от %1'");
			ТекстПринятоОт = СтрШаблон(ТекстШаблонаПринятоОт, ДанныеКонтрагента.Представление);
			
			Объект.ПринятоОт = ТекстПринятоОт;
		КонецЕсли;
		
		ПодразделениеКассы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Касса, "Подразделение");
		
		Если ЗначениеЗаполнено(ПодразделениеКассы) Тогда
			Для каждого СтрокаРасшифровки Из Объект.РасшифровкаПлатежа Цикл
				СтрокаРасшифровки.Подразделение = ПодразделениеКассы;
			КонецЦикла;
	    КонецЕсли;
	КонецЕсли;
	// <-- Евлахов Игорь Николаевич (Конец) 25.09.2023
...
Процедура КассаПриИзменении(Элемент)
	...
	// #4385.. Фомин Д.Ю. 26.07.2024.
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств")
		И Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
		Объект.РасшифровкаПлатежа[0].Подразделение = СтруктураРеквизитовКассы.Подразделение;
	КонецЕсли;	
	// ..#4385
...
Процедура КонтрагентПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		// --> Евлахов Игорь Николаевич (Начало) 26.09.2023
		// Задача #3084
		...
		Иначе	
			...
			КонецЕсли;		
		КонецЕсли;
	    // <-- Евлахов Игорь Николаевич (Конец) 26.09.2023
...
Процедура КассирПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Кассир)
		И Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыККМ") Тогда
		КассирПредставление = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Кассир, "Наименование");
		
		ТекстПринятоОт = "";
		Если ЗначениеЗаполнено(КассирПредставление) Тогда
			ТекстПринятоОт = СокрЛП(КассирПредставление);	
	    КонецЕсли;
		Объект.ПринятоОт = ТекстПринятоОт;	
	КонецЕсли;
...
// << 21.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-725
&НаКлиенте
Процедура КР_ПоступлениеВКассуККМПриИзменении(Элемент)
...
Процедура Подключаемый_ВыборФизЛица(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("злПослеВыбораСотрудника", ЭтотОбъект);
	ПоказатьВыборИзСписка(Оповещение, СписокСотрудников, Элементы.ПринятоОт);

КонецПроцедуры 

&НаКлиенте
Процедура злПослеВыбораСотрудника(ВыбранноеЗначение, Параметры) Экспорт
	
	Если ВыбранноеЗначение <> Неопределено Тогда
		Объект.ПринятоОт = СокрЛП(ВыбранноеЗначение.Представление);
		
		Объект.Кассир = злПолучитьОтветственноеЛицоОрганизацийКассир(
								Объект.Организация, 
...
Процедура КР_ПриЧтенииСозданииНаСервереДополнительно()
	...
	// ГруппаХозОперация
	ГруппаХозОперация = Неопределено;
	Если КР_МетодыМодификацииФорм.ЕстьЭлементФормы(ЭтотОбъект, "КР_ГруппаХозОперация", ГруппаХозОперация) Тогда
		// При перечитывании выходим так как "все уже создано до нас"
		Возврат;
	КонецЕсли;
	...
	// КР_ПоступлениеВКассуККМ
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ПоступлениеВКассуККМ", ГруппаХозОперация);
	ЭлементФормы.Заголовок = НСтр("ru = 'В кассу ККМ'");
	ЭлементФормы.ВидФлажка = ВидФлажка.Тумблер;
    ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ПоступлениеВКассуККМПриИзменении");
	
	// --> Евлахов Игорь Николаевич (Начало) 25.09.2023
	// Задача #3084
	ИмяКоманды 	= "КомандаВыбратьФизЛицо";
	Заголовок 	= НСтр("ru = 'Выбрать физ. лицо'");
	Действие 	= "Подключаемый_ВыборФизЛица";
	
	НоваяКоманда = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект, ИмяКоманды, Заголовок, Действие);
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, 
																НоваяКоманда, 
																Элементы.ГруппаПринятоОтКассир,
																, 
																"КнопкаФормы");
	
	// Для элемента Подотчетное лицо, организуем отбор по подразделению
	ЭлементПодотчетноеЛицо = Элементы.Найти("ПодотчетноеЛицо");
	Если ЭлементПодотчетноеЛицо <> Неопределено Тогда 
		ЭлементПодотчетноеЛицо.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый Массив);
	    
		НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.КР_Подразделение", "Объект.Подразделение");
		
		НовыйМассив = Новый Массив();
	    НовыйМассив.Добавить(НоваяСвязь);
	    
	    ЭлементПодотчетноеЛицо.СвязиПараметровВыбора = Новый ФиксированныйМассив(НовыйМассив);
	КонецЕсли;
	// <-- Евлахов Игорь Николаевич (Конец) 25.09.2023
...
&НаСервереБезКонтекста
Функция злПолучитьОтветственноеЛицоОрганизацийКассир(Организация, ФизическоеЛицо, ОтборПоДате)

	ОтветственноеЛицо = ПредопределенноеЗначение("Справочник.ОтветственныеЛицаОрганизаций.ПустаяСсылка");
	...
	Запрос.УстановитьПараметр("ОтборПоДате", ОтборПоДате);
	
	Результат 	= Запрос.Выполнить();
	Выборка 	= Результат.Выбрать();
	
	Пока Выборка.Следующий() Цикл	
		ОтветственноеЛицо = Выборка.Ссылка;	
	КонецЦикла;
