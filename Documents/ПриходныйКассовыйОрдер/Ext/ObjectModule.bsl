...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	// Перед обращением по индексу, нужно проверить, что тип поддерживает такой функционал
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		И ДанныеЗаполнения.Свойство("ДокументОснование")
		И НЕ Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ДанныеЗаполнения.ДокументОснование))
		И ТипЗнч(ДанныеЗаполнения.ДокументОснование[0]) = Тип("ДокументСсылка.РаспоряжениеНаПеремещениеДенежныхСредств") Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// << 15.02.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента Тогда 
		Если КР_ПоступлениеВКассуККМ Тогда 
	...
	ПроверяемыеРеквизиты.Добавить("Кассир");	// #4383.. Фомин Д.Ю. 10.07.2024.
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыККМ Тогда
		ПроверяемыеРеквизиты.Добавить("ПринятоОт");
		//ПроверяемыеРеквизиты.Добавить("Кассир");		// #4383.. Фомин Д.Ю. 10.07.2024. Вынес за условие.  
						
		ПредставлениеКассира = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Кассир, "Наименование");
		
		Если ПредставлениеКассира <> СокрЛП(ПринятоОт) Тогда
			ТекстСообщения = НСтр("ru = 'Реквизит ""Принято от"" должен содержать текст равный значению реквизита ""Кассир"".'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ПринятоОт", , Отказ);	
		КонецЕсли;		
	КонецЕсли;	
	// <-- Евлахов Игорь Николаевич (Конец) 02.05.2024
	
	// #4385.. Фомин Д.Ю. 15.07.2024.
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств
		...
		Если РасшифровкаПлатежа[0].Подразделение <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Касса, "Подразделение") Тогда
			ТекстСообщения = НСтр("ru = 'Подразделение документа должно быть равно подразделению кассы.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "РасшифровкаПлатежа[0].Подразделение", , Отказ);	
		КонецЕсли;	
	КонецЕсли;		
	// ..#4385
...
КонецПроцедуры // >> 28.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-725

// << 15.02.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
Процедура КР_ПередЗаписьюДополнительно(Отказ, РежимЗаписи, РежимПроведения)
	
	ПравоТолькоДобавление =
		УправлениеДоступом.ЕстьРоль("КР_ДобавлениеПриходныхКассовыхОрдеров")
		И Не УправлениеДоступом.ЕстьРоль("КР_ДобавлениеИзменениеКассовыхДокументовЛюбойДатой");
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ПравоТолькоДобавление Тогда
		ТекущыйДень = НачалоДня(ТекущаяДатаСеанса());
		ПредыдущийДень = НачалоДня(ТекущыйДень - 1);
		ДокументДень = НачалоДня(Дата);
		
		Если Не (ДокументДень = ТекущыйДень
			Или ДокументДень = ПредыдущийДень) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Запрещено проведение документа указанной датой'"), , , , Отказ);
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры // >> 15.02.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
