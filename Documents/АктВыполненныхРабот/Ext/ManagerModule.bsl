...
//	ЕстьКорректировки       - Булево - Признак, что по акту есть корректировки
//	СостояниеРасчетов       - ФормаКлиентскогоПриложения - Форма, в реквизиты которой будет помещено рассчитанное состояние.
//
Процедура РассчитатьСостояние(Знач АктВыполненныхРабот, Знач ЕстьКорректировки, СостояниеРасчетов) Экспорт
	...
	Если  ЗначениеЗаполнено(АктВыполненныхРабот) И ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСКлиентами) Тогда
		...
		ДанныеРасчетаПараметров.Вставить("АктПоЗаказам", АктВыполненныхРабот.АктПоЗаказам);
		ДанныеРасчетаПараметров.Вставить("ЕстьКорректировки", ЕстьКорректировки);
		ДанныеРасчетаПараметров.Вставить("Ссылка", АктВыполненныхРабот);
		
		ДанныеПоСостоянию = ВзаиморасчетыСервер.СостояниеВзаиморасчетов(АктВыполненныхРабот, ПараметрыВзаиморасчеты(ДанныеРасчетаПараметров));
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	ВЫБОР КОГДА ТаблицаУслуги.Номенклатура.ТипНоменклатуры В (&СписокТиповСАналитикойПодразделение) 
	|		ТОГДА ТаблицаУслуги.Подразделение
	|	ИНАЧЕ
	|		&Подразделение
	|	КОНЕЦ КАК Подразделение,
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Очищает поле ""Назначение в ТЧ ""Услуги"" для строк без обособленного обеспечения в заказе клиента.
	|Заполняет реквизит ""Объект расчетов""
	|Заполняет реквизит ""Подразделение"" в табличной части ""Услуги"" для номенклатур типа ""Работа""
	|Заполняет варианты отсчета графика и заполняет сдвиг для варианта оплаты ""Кредит (плановый)"".
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7089e502-f645-4a46-808b-47605498bdfc");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.АктВыполненныхРабот.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИЛИ АктВыполненныхРаботУслуги.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|					И АктВыполненныхРаботУслуги.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|					И НЕ АктВыполненныхРабот.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)))
	|	ИЛИ НЕ АктВыполненныхРабот.ОплатаВВалюте
	|			И (АктВыполненныхРабот.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ АктВыполненныхРабот.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	...
	|																КОГДА РАЗНОСТЬДАТ(АктВыполненныхРабот.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ) < 0
	|																ТОГДА 0
	|																ИНАЧЕ РАЗНОСТЬДАТ(АктВыполненныхРабот.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ)
	|															КОНЕЦ))
	|	ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	...
	|	ИЛИ НЕ АктВыполненныхРабот.Проведен И (ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.АктВыполненныхРабот.РасшифровкаПлатежа КАК ТЧРасшифровкаПлатежа
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			ДокументОбъект = Документ.Ссылка.ПолучитьОбъект(); // ДокументОбъект.АктВыполненныхРабот
			...
			Иначе
				...
				Пока ВыборкаСтрокТЧ.Следующий() Цикл
					...
					ОбъектИзменен = Макс(ОбъектИзменен, НекорректныеСтроки.Количество());
					
				КонецЦикла;
			
				СкладыСервер.ЗаполнитьКолонкуТЧПодразделение(ДокументОбъект, ОбъектИзменен, "Услуги");
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|ГДЕ
		|	(НЕ ТабЧасть.Ссылка.АктПоЗаказам ИЛИ ТабЧасть.КодСтроки = 0 ИЛИ НЕ &ИспользоватьРасширенныеВозможностиЗаказаКлиента)
