...
Функция СформироватьПечатнуюФормуЛистаКассовойКниги(МассивОбъектов, ОбъектыПечати, ПараметрыПечати = Неопределено)
	...
	// ГалкинАН #4137 (06.06.2024) galkin.an {
	|	ДокументКассоваяКнига.КР_Кассир.ФизическоеЛицо КАК Кассир,
	// #4137 }
			...
			Если ДатаЛиста >= ДатаНач Тогда
				...
				ОбластьПодвалОтчет.Параметры.Кассир = 
					ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(
						...
						// Стало
						ВыборкаПоДокументамКассоваяКнига.Кассир,
						// #4137 }
...
#Область КР_ИнициализаицияИЗаполнение

// << 01.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
Функция КР_ПолучитьКассовуюКнигуКассы(Знач КассоваяКнига) Экспорт
	
	Результат = Справочники.КассовыеКниги.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(КассоваяКнига) Тогда
		
		Результат = КассоваяКнига;
		
	Иначе
		
		Запрос = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Кассы.КассоваяКнига КАК КассоваяКнига
		|ИЗ
		|	Справочник.Пользователи КАК Пользователи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Кассы КАК Кассы
		|		ПО Пользователи.Подразделение = Кассы.Подразделение
		|ГДЕ
		...
		Если Выборка.Следующий() Тогда
			Результат = Выборка.КассоваяКнига;
		КонецЕсли;
...
КонецФункции // >> 01.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166

// << 01.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
Процедура КР_ЗаполнитьТипЛистаКассовойКниги(КассоваяКнига, Дата, ТипЛиста) Экспорт
	...
	|		ПО Кассы.Подразделение = Склады.Подразделение
	|ГДЕ
	|	Кассы.КассоваяКнига = &КассоваяКнига");
	Запрос.УстановитьПараметр("КассоваяКнига", КассоваяКнига);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И ЗначениеЗаполнено(Выборка.Календарь) Тогда
		ТекущийКалендарь = Выборка.Календарь;
	Иначе
		ТекущийКалендарь = Константы.ОсновнойКалендарьПредприятия.Получить();
	КонецЕсли;
	
	ДатаПроверки = НачалоДня(?(ЗначениеЗаполнено(Дата), Дата, ТекущаяДатаСеанса()));
	
	// Проверяем дату документа и следующий день
	НачальныеДаты = Новый Массив;
	НачальныеДаты.Добавить(ДатаПроверки);
	НачальныеДаты.Добавить(КонецДня(ДатаПроверки) + 1);
	
	ПараметрыРабочихДат = ГрафикиРаботы.ПараметрыПолученияБлижайшихДатПоГрафику();
	ПараметрыРабочихДат.ВызыватьИсключение = Ложь;
	ПараметрыРабочихДат.ИгнорироватьНезаполненностьГрафика = Истина;
	
	БлижайшиеДаты = ГрафикиРаботы.БлижайшиеДатыВключенныеВГрафик(ТекущийКалендарь, НачальныеДаты, ПараметрыРабочихДат);
	
	Если БлижайшиеДаты = Неопределено Тогда
		ТипЛиста = Перечисления.ТипыЛистовКассовойКниги.Обычный;
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(
					НСтр("ru = 'Невозможно определить тип листа кассовой книги для даты %1, 
						 |возможно, график работы не заполнен.'"), 
					Формат(Выборка.Дата, "ДЛФ=D")));
		Возврат;
	КонецЕсли;
	
	Для Каждого БлижайшаяДата Из БлижайшиеДаты Цикл
		Если БлижайшаяДата.Ключ = ДатаПроверки
			И БлижайшаяДата.Ключ = БлижайшаяДата.Значение Тогда
			Продолжить;
		КонецЕсли;
		
		Если Год(ДатаПроверки) <> Год(БлижайшаяДата.Значение) Тогда
			ТипЛиста = Перечисления.ТипыЛистовКассовойКниги.ПоследнийВГоду;
		ИначеЕсли Месяц(ДатаПроверки) <> Месяц(БлижайшаяДата.Значение) Тогда
			ТипЛиста = Перечисления.ТипыЛистовКассовойКниги.ПоследнийВМесяце;
		Иначе
			ТипЛиста = Перечисления.ТипыЛистовКассовойКниги.Обычный;
		КонецЕсли;
...
КонецПроцедуры // >> 01.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
