...
Процедура ПолучитьДанныеПакета()
	...
	СостоянияДокументовЭДО = ЭлектронныеДокументыЭДО.СостоянияДокументовПакета(ИдентификаторПакета);
	СостояниеПакета = ЭлектронныеДокументыЭДО.СостояниеПакета(СостоянияДокументовЭДО); 
...
&НаСервере
Процедура ПриИзмененииВидаОсновногоДокумента() Экспорт
...
Процедура ПодготовитьОтражениеВУчете()
	...
	Если ЭлементСхемы.ТипДокумента <> Перечисления.ТипыДокументовЭДО.Прикладной 
		И Не ЭлементСхемы.Распознан Тогда
	...
	Иначе			
		Элементы.КомандаСоздатьДокументУчета.Заголовок = "создать...";
	КонецЕсли;
...
Процедура НастроитьЭлементыОтраженияВУчете()
	...
	Если ЕстьПравоОбработкиДокумента Тогда
		Элементы.ГруппаСоздатьДокументУчета.Видимость = Не ЕстьОдинДУ 
			И (ЭлементСхемы.ФормированиеПоОбъектуУчета Или ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.Прикладной);
...
Процедура ПроверитьВозможностьОтраженияВУчете()
	...
	Если ЭлементСхемы = Неопределено И ЭлементСхемы.РасширениеФайла <> "xml"  Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеФайла = РаботаСФайлами.ДвоичныеДанныеФайла(ЭлементСхемы.ПрисоединенныйФайл);
	...
	Если ЭлементСхемыИнформацияОтправителя(ЭтотОбъект).Распознан Тогда
		...
		Описание = "";
		Если Не ИнтеграцияЭДО.ВозможноСозданиеУчетногоДокумента(ДанныеФайла, Описание) Тогда
