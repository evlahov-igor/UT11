...
Процедура ДоговорПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Договор) 
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеНедостачЗаСчетКомитента Тогда
			Если Объект.Валюта <> Договор.ВалютаВзаиморасчетов Тогда
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// ГалкинАН #4186 (30.05.2024) galkin.an {
	
	злПриЧтенииСозданииНаСервере();
...
#Область злДополнительныеПроцедурыИФункции

#Область злСлужебныеПроцедурыИФункции

&НаСервере
Процедура злПриЧтенииСозданииНаСервере()

	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.злЗакрытиеОтрицательныхОстатков", Элементы.ГруппаРеквизитыПраво);
