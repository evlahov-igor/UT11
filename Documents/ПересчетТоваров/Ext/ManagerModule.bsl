...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ПересчетТоваров";
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	// В режиме работы со складом вида магазин и не заполненным помещением
	// анализ "Помещение.ИспользоватьАдресноеХранение" и "Помещение.ДатаНачалаАдресногоХраненияОстатков" не требуется
	|					ИЛИ (ПересчетТоваров.Помещение = Значение(Справочник.СкладскиеПомещения.ПустаяСсылка) И
	|						ПересчетТоваров.Склад.КР_ВидСклада = Значение(Перечисление.КР_ВидыСкладов.Магазин))
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	|		ИНАЧЕ ВЫБОР
	...
	|	ПересчетТоваров.КР_НеФормироватьДвижения КАК КР_НеФормироватьДвижения,
	// >> 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
	// << 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
	|	ПересчетТоваров.КР_УпорядочиваниеТоваровВЯчейках КАК КР_УпорядочиваниеТоваровВЯчейках,
	// >> 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	|	ПересчетТоваров.Помещение = Значение(Справочник.СкладскиеПомещения.ПустаяСсылка) И
	|		ПересчетТоваров.Склад.КР_ВидСклада = Значение(Перечисление.КР_ВидыСкладов.Магазин) КАК КР_СкладВидаМагазин,
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	...
	// << 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
	Запрос.УстановитьПараметр("КР_НеФормироватьДвижения", Реквизиты.КР_НеФормироватьДвижения);
	// >> 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	Запрос.УстановитьПараметр("КР_СкладВидаМагазин", Реквизиты.КР_СкладВидаМагазин);
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	// << 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
	Запрос.УстановитьПараметр("КР_УпорядочиваниеТоваровВЯчейках", Реквизиты.КР_УпорядочиваниеТоваровВЯчейках);
	// >> 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюИзлишковНедостач(Запрос, ТекстыЗапроса, Регистры)
	...
	// << 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
	|	И НЕ &КР_НеФормироватьДвижения
	// >> 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
	// << 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
	|	И НЕ &КР_УпорядочиваниеТоваровВЯчейках
	// >> 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
	|";
...
Функция ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Период,
	|	&Склад,
	|	ПересчетТоваровТовары.Ячейка.Помещение
	|ИЗ
	|	Документ.ПересчетТоваров.Товары КАК ПересчетТоваровТовары
	|ГДЕ
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		// << 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
		|		И НЕ ТабЧасть.Ссылка.КР_НеФормироватьДвижения
		// >> 29.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-723
		// << 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
		|		И НЕ ТабЧасть.Ссылка.КР_УпорядочиваниеТоваровВЯчейках
		// >> 12.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1449
		|";
	...
	ТекстЗапросаТабЧасть =
		...
		|ГДЕ
		|	ТИПЗНАЧЕНИЯ(ТабЧасть.Ссылка) = ТИП(Документ.ПересчетТоваров)"; // нужно чтобы не задваивалось в обновлении ИБ.
...
Функция ПечатьЗадания(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		ИначеЕсли ВыборкаПоДокументам.Статус = Перечисления.СтатусыПересчетовТоваров.Подготовлено Тогда
			...
			// Конец было
			ТекстСообщения = НСтр("ru = 'Документ ""%ПредставлениеДокумента%"" находится в статусе ""%Статус%"". Печать заданий на пересчет возможна в статусах ""%ВРаботе%"", ""%КПроверке%"" и ""%Выполнено%"".'");
			...
			// Конец было
			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%КПроверке%", Перечисления.СтатусыПересчетовТоваров.злКПроверке);
			// <-- Евлахов Игорь Николаевич (Конец) 25.09.2024
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	ШаблонПредставления = НСтр("ru='Пересчет товаров сводный %1 от %2'");
	Представление		= СтрШаблон(ШаблонПредставления, Данные.Номер, Данные.Дата);

КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 25.04.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1449
Процедура КР_ПроверитьДублиЗональныхПересчетовВСтрокахТЧ(ТаблицаЗональныеПересчеты, Отказ) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаПересчеты", ТаблицаЗональныеПересчеты);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаПересчеты.НомерСтроки КАК НомерСтроки,
	|	ТаблицаПересчеты.ЗональныйПересчет КАК ЗональныйПересчет
	|ПОМЕСТИТЬ врт_ТаблицаПересчеты
	|ИЗ
	|	&ТаблицаПересчеты КАК ТаблицаПересчеты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	врт_ТаблицаПересчеты.НомерСтроки КАК НомерСтроки,
	|	ЗональныйПересчетТоваров.ЗонаПересчета КАК ЗонаПересчета
	|ИЗ
	|	врт_ТаблицаПересчеты КАК врт_ТаблицаПересчеты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КР_ЗональныйПересчетТоваров КАК ЗональныйПересчетТоваров
	|		ПО врт_ТаблицаПересчеты.ЗональныйПересчет = ЗональныйПересчетТоваров.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|ИТОГИ ПО
	...
	ШаблонСообщения = НСтр("ru = 'В строках %1 таблицы ""Зональные пересчеты"" дублируется зона пересчета ""%2""'");
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтрШаблон(ШаблонСообщения, СтрСоединить(МассивНомеровСтрок, ", "), Выборка.ЗонаПересчета);
...
Процедура КР_ПроверитьДатыЗональныхПересчетовВСтрокахТЧ(ПриказНаИнвентаризациюТоваров, ЗональныеПересчеты, Отказ) Экспорт
	
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаПересчеты", ЗональныеПересчеты.Выгрузить());
	Запрос.УстановитьПараметр("ПриказНаИнвентаризациюТоваров", ПриказНаИнвентаризациюТоваров);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаПересчеты.НомерСтроки КАК НомерСтроки,
	|	ТаблицаПересчеты.ЗональныйПересчет КАК ЗональныйПересчет
	...
	|ВЫБРАТЬ
	|	врт_ТаблицаПересчеты.НомерСтроки КАК НомерСтроки,
	|	ЗональныйПересчетТоваров.Ссылка КАК ЗональныйПересчетТоваров
	|ИЗ
	|	врт_ТаблицаПересчеты КАК врт_ТаблицаПересчеты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КР_ЗональныйПересчетТоваров КАК ЗональныйПересчетТоваров
	|		ПО врт_ТаблицаПересчеты.ЗональныйПересчет = ЗональныйПересчетТоваров.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИнвентаризационнаяОпись КАК ИнвентаризационнаяОпись
	|		ПО (ИнвентаризационнаяОпись.Ссылка = &ПриказНаИнвентаризациюТоваров)
	|			И (НЕ(ЗональныйПересчетТоваров.Дата МЕЖДУ ИнвентаризационнаяОпись.ДатаНачала И ИнвентаризационнаяОпись.ДатаОкончания))
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	
	ШаблонСообщения = НСтр("ru = 'В строке %1 таблицы ""Зональные пересчеты"" дата документа ""%2"" не входит в период инвентаризации'");
	
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока Выборка.Следующий() Цикл
			мсвСтроки = ЗональныеПересчеты.НайтиСтроки(Новый Структура("НомерСтроки",Выборка.НомерСтроки));
			Если мсвСтроки.Количество() > 0 Тогда
				ТекстСообщения = СтрШаблон(ШаблонСообщения, Выборка.НомерСтроки, Выборка.ЗональныйПересчетТоваров);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , СтрШаблон("Объект.КР_ЗональныеПересчеты[%1].ЗональныйПересчет",ЗональныеПересчеты.Индекс(мсвСтроки[0])),,Отказ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
