...
Процедура КР_ПриСозданииНаСервереДополнительно(Отказ, СтандартнаяОбработка)
	...
	КР_ПриказНаИнвентаризациюТоваров = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "СписокПересчетов.КР_ПриказНаИнвентаризациюТоваров", Элементы.СписокПересчетов);
	
	// << 10.05.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1449
	ЭтотОбъект.Заголовок = НСтр("ru = 'Сводные пересчеты товаров'");
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "СписокПересчетов.КР_УпорядочиваниеТоваровВЯчейках", Элементы.СписокПересчетов);
	// >> 10.05.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1449
