...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// << 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
		КР_ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры);
		...
		КР_ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры);
	...
	// << 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	КР_ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры);
	// >> 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751      
...
Процедура ОтобратьТовары(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И ТаблицаТовары.Ячейка.Помещение.НастройкаАдресногоХранения = ЗНАЧЕНИЕ(Перечисление.НастройкиАдресногоХранения.ЯчейкиОстатки) 
	// >> 18.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	|	И ТаблицаТовары.КоличествоУпаковокОтобрано > 0
	|	И ТаблицаТовары.Ссылка.Статус В(ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.ВыполненоБезОшибок),
	|									ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.ВыполненоСОшибками))
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Обработчик = "УправлениеПечатьюУТКлиент.ПечатьЭтикетокИЦенников";
	КомандаПечати.МенеджерПечати = "Документ.ОтборРазмещениеТоваров";
	КомандаПечати.Идентификатор = "ЦенникиЭтикетки";
	КомандаПечати.Представление = НСтр("ru = 'Ценники и этикетки'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	// >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
...
// << 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
Функция КР_ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Склад КАК Склад,
	|	ДанныеШапки.Распоряжение.СкладОтправитель КАК Отправитель,
	...
	|	ВЫБОР
	|		КОГДА ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятКороб)
	// << 18.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2616
	|				И НЕ ДанныеШапки.КР_ПотоварнаяПриемкаБезВскрытияКороба		
	// >> 18.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2616
	|			ТОГДА 0
	|		ИНАЧЕ Товары.КоличествоРазмещено
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
	|		ПО (ДанныеШапки.Ссылка = Товары.Ссылка)
	|ГДЕ
	|	Товары.Ссылка = &Ссылка  
	|	И НЕ ДанныеШапки.Распоряжение ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|	И (ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятКороб)
	|			ИЛИ ДанныеШапки.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятТовар), 
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Товары.Назначение,
	|	&Склад,
	|	ДанныеШапки.Распоряжение.СкладОтправитель,
	...
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|	И (ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятКороб)
	|				И ДанныеШапки.КР_ПотоварнаяПриемкаБезВскрытияКороба
	|			ИЛИ ДанныеШапки.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятТовар), 
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Товары.Назначение КАК Назначение,
	|	&Склад КАК Склад,
	|	ДанныеШапки.Распоряжение.Партнер КАК Отправитель,
	|	ДанныеШапки.Распоряжение.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	0 КАК КОформлениюНакладныхПоРаспоряжению,
	|	0 КАК КОформлениюПоступленийПоРаспоряжению,
	|	0 КАК КОформлениюПоступленийПоНакладным,
	|	Товары.КоличествоРазмещено КАК КОформлениюПоступленийПоОрдерам,
	|	0 КАК КОформлениюОрдеров,
	|	0 КАК Принимается
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыРазмещение КАК Товары
	...
	|ГДЕ
	|	Товары.Ссылка = &Ссылка  
	|	И ДанныеШапки.Распоряжение ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция КР_ТекстЗапросаТаблицаКоробаНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеШапки.КР_Короб КАК Короб,
	|	&Склад КАК Склад,
	|	1 КАК ВНаличии
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
	|ГДЕ
	|	ДанныеШапки.Ссылка = &Ссылка
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_УчетОстатковКоробов
	|	И ДанныеШапки.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ДанныеШапки.Склад.КР_УчетОстатковКоробов
	|	И ДанныеШапки.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
	|	И ДанныеШапки.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятТовар), 
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ГДЕ
	|	ДанныеШапки.Ссылка = &Ссылка
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Перемещение)
	|	И ДанныеШапки.Склад.КР_УчетОстатковКоробов
	|	И ДанныеШапки.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)";      
...
Функция КР_ТекстЗапросаТаблицаКоробаКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	ДанныеШапки.Ссылка = &Ссылка
	|	И ТИПЗНАЧЕНИЯ(&Распоряжение) <> ТИП(Документ.ПриобретениеТоваровУслуг)
	|	И ДанныеШапки.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)      
	|
	// A2105505-1519	
...
// << 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
Функция КР_ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("КР_ВТКПеремещениюМеждуПомещениями", ТекстыЗапроса) Тогда
		КР_ТекстЗапросаВтКПеремещениюМеждуПомещениями(Запрос, ТекстыЗапроса);
	КонецЕсли; 
	...
	|	&Склад КАК Склад,
	|	ВЫБОР
	|		КОГДА Товары.Ячейка <> ЗНАЧЕНИЕ(Справочник.СкладскиеЯчейки.ПустаяСсылка)
	|			ТОГДА Товары.Ячейка.Помещение
	|		ИНАЧЕ ДанныеШапки.Помещение
	|	КОНЕЦ КАК Помещение,
	|	ВЫБОР
	|		КОГДА Товары.СтатусУказанияСерий В (6, 8, 10, 14)
	|			ТОГДА Товары.Серия
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Серия,
	|	Товары.КоличествоРазмещено КАК ВНаличии,
	|	0 КАК КОтгрузке
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыРазмещение КАК Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
	...
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И Товары.КоличествоРазмещено <> 0
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|	И (ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятКороб)
	|				И ДанныеШапки.КР_ПотоварнаяПриемкаБезВскрытияКороба
	|			ИЛИ ДанныеШапки.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятТовар), ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И Товары.КоличествоОтобрано <> 0
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Отбор)
	|	И ДанныеШапки.Склад.КР_АдресныйСкладБезордернойОтгрузки
	|	И Товары.Ячейка.Помещение <> ДанныеШапки.Помещение
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	0
	|ИЗ
	|	КР_ВТКПеремещениюМеждуПомещениями КАК Товары";
...
Функция КР_ТекстЗапросаТаблицаТоварыВЯчейках(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ КАК Серия,
	|	Товары.КоличествоУпаковокРазмещено КАК ВНаличии,
	|	0 КАК КОтбору,
	|	0 КАК КРазмещению
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыРазмещение КАК Товары
	...
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И Товары.КоличествоУпаковокРазмещено <> 0  
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|	И ДанныеШапки.Склад.НастройкаАдресногоХранения = ЗНАЧЕНИЕ(Перечисление.НастройкиАдресногоХранения.ОпределяетсяНастройкамиПомещения)
	|	И (ДанныеШапки.Помещение.НастройкаАдресногоХранения = ЗНАЧЕНИЕ(Перечисление.НастройкиАдресногоХранения.ЯчейкиОстатки)
	|		ИЛИ ДанныеШапки.Склад.НастройкаАдресногоХранения  = ЗНАЧЕНИЕ(Перечисление.НастройкиАдресногоХранения.ЯчейкиОстатки))
	|	И (ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятКороб)
	|				И ДанныеШапки.КР_ПотоварнаяПриемкаБезВскрытияКороба
	|			ИЛИ ДанныеШапки.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятТовар), 
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен)))
	...
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|	И Товары.Ячейка.Помещение.НастройкаАдресногоХранения <> ЗНАЧЕНИЕ(Перечисление.НастройкиАдресногоХранения.ЯчейкиОстатки)
	|	И ДанныеШапки.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ПринятТовар), 
	|		ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение)
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
	|	И ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтборовРазмещенийТоваров.КР_ТоварРазмещен)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ									КАК Серия,
	|	Товары.КоличествоУпаковокОтобрано КАК ВНаличии,
	|	0										КАК КОтбору,
	|	0										КАК КРазмещению
	|
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыОтбор КАК Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
	|		ПО Товары.Ссылка = ДанныеШапки.Ссылка
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И ДанныеШапки.ЗонаОтгрузки.Помещение.НастройкаАдресногоХранения = ЗНАЧЕНИЕ(Перечисление.НастройкиАдресногоХранения.ЯчейкиОстатки)
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Отбор)
	|	И ДанныеШапки.Склад.КР_АдресныйСкладБезордернойОтгрузки
	|	И ТИПЗНАЧЕНИЯ(ДанныеШапки.Распоряжение) = ТИП(Документ.ЗаказНаПеремещение)
	|	И Товары.КоличествоУпаковокОтобрано <> 0"; 
...
// << 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
Процедура КР_ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Товары.Назначение КАК Назначение,
	|	Товары.КоличествоРазмещено КАК Количество,
	|	ИСТИНА КАК ПоГрафику,
	|	ДанныеШапки.Распоряжение КАК РаспоряжениеВГрафике
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыРазмещение КАК Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
	|		ПО (ДанныеШапки.Ссылка = Товары.Ссылка)
...
КонецПроцедуры // >> 07.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
...
Функция КР_ТекстЗапросаТаблицаТоварыКОтбору(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.КоличествоУпаковокОтобрано	КАК КОтбору,
	|	0										КАК Отбирается,
	|	ТаблицаТовары.КоличествоУпаковокОтобрано	КАК Отобрано
	|
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыОтбор КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
...
КонецФункции // >> 09.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
...
Функция КР_ТекстЗапросаТаблицаТоварыКОтгрузке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеШапки.Склад						КАК Склад,
	|	ВЫРАЗИТЬ(ДанныеШапки.Распоряжение КАК Документ.ЗаказНаПеремещение).СкладПолучатель 
	|											КАК Получатель,
	|	ДанныеШапки.Распоряжение				КАК ДокументОтгрузки,
	|	ТаблицаТовары.Номенклатура				КАК Номенклатура,
	|	ТаблицаТовары.Характеристика			КАК Характеристика,
	|	ТаблицаТовары.Назначение				КАК Назначение,
	|	ВЫБОР
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	КОНЕЦ									КАК Серия,
	|	ТаблицаТовары.КоличествоОтобрано		КАК КСборке,
	|	0										КАК Собрано
	|
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыОтбор КАК ТаблицаТовары
	...
	|	И ДанныеШапки.Склад.КР_АдресныйСкладБезордернойОтгрузки
	|	И ТИПЗНАЧЕНИЯ(ДанныеШапки.Распоряжение) = ТИП(Документ.ЗаказНаПеремещение)
	|	И ТаблицаТовары.КоличествоОтобрано <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	КОНЕЦ									КАК Серия,
	|	0										КАК КСборке,
	|	ТаблицаТовары.КоличествоОтобрано		КАК Собрано
	|
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыОтбор КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
...
Функция КР_ТекстЗапросаТаблицаКоробаКОтгрузке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НАЧАЛОПЕРИОДА(&Период, ДЕНЬ) КАК Период,
	|	&Склад КАК Склад,
	|	ВЫРАЗИТЬ(ДанныеШапки.Распоряжение КАК Документ.ЗаказНаПеремещение).СкладПолучатель КАК Получатель,
	...
	|	И ДанныеШапки.Склад.КР_АдресныйСкладБезордернойОтгрузки
	|	И ТИПЗНАЧЕНИЯ(ДанныеШапки.Распоряжение) = ТИП(Документ.ЗаказНаПеремещение)
	|	И ДанныеШапки.КР_ИспользоватьКороб
	|	И ДанныеШапки.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Отбор) 
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция КР_ТекстЗапросаВтКПеремещениюМеждуПомещениями(Запрос, ТекстыЗапроса)
	
	ИмяРегистра = "КР_ВТКПеремещениюМеждуПомещениями";
	
	ТекстЗапросаВТКПеремещениюМеждуПомещениямиИсходныеДанные = 
	"ВЫБРАТЬ
	|	Товары.Номенклатура КАК Номенклатура,
	|	Товары.Характеристика КАК Характеристика,
	|	Товары.Назначение КАК Назначение,
	|	Товары.Ячейка.Помещение КАК Помещение,
	|	Товары.Серия КАК Серия,
	|	Товары.СтатусУказанияСерий КАК СтатусУказанияСерий,
	|	Товары.КоличествоОтобрано КАК КоличествоОтобрано,
	|	0 КАК КоличествоРазмещено
	|ПОМЕСТИТЬ КР_ВТКПеремещениюМеждуПомещениямиИсходныеДанные
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыОтбор КАК Товары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтборРазмещениеТоваров КАК ДанныеШапки
	|		ПО (ДанныеШапки.Ссылка = Товары.Ссылка)
	...
	|	И Товары.КоличествоОтобрано <> 0
	|	И ДанныеШапки.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Перемещение)
	|	И ДанныеШапки.Склад.КР_ВидСклада = ЗНАЧЕНИЕ(Перечисление.КР_ВидыСкладов.Магазин)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ДанныеШапки.Склад.КР_ВидСклада = ЗНАЧЕНИЕ(Перечисление.КР_ВидыСкладов.Магазин)
	|";
	ТекстыЗапроса.Добавить(ТекстЗапросаВТКПеремещениюМеждуПомещениямиИсходныеДанные, ИмяРегистра);
	...
	|	Данные.Помещение КАК Помещение,
	|	Данные.Серия КАК Серия,
	|	Данные.СтатусУказанияСерий КАК СтатусУказанияСерий,
	|	ВЫБОР
	|		КОГДА СУММА(Данные.КоличествоОтобрано) > СУММА(Данные.КоличествоРазмещено)
	|			ТОГДА СУММА(Данные.КоличествоОтобрано) - СУММА(Данные.КоличествоРазмещено)
	|		ИНАЧЕ СУММА(Данные.КоличествоРазмещено) - СУММА(Данные.КоличествоОтобрано)
	|	КОНЕЦ КАК Количество,
	|	СУММА(Данные.КоличествоОтобрано) > СУММА(Данные.КоличествоРазмещено) КАК ЭтоРасход
	|ПОМЕСТИТЬ КР_ВТКПеремещениюМеждуПомещениями
	|ИЗ
	|	КР_ВТКПеремещениюМеждуПомещениямиИсходныеДанные КАК Данные
	|
	|СГРУППИРОВАТЬ ПО
	|	Данные.Назначение,
	|	Данные.Номенклатура,
	...
	|	Данные.Помещение,
	|	Данные.Серия,
	|	Данные.СтатусУказанияСерий
	|
	|ИМЕЮЩИЕ
	|	СУММА(Данные.КоличествоОтобрано) <> СУММА(Данные.КоличествоРазмещено)";
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Отбор") Тогда
		Заголовок = НСтр("ru = 'Отбор товаров'");	
	ИначеЕсли Данные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Размещение") Тогда
		Заголовок = НСтр("ru = 'Размещение товаров'");	
	ИначеЕсли Данные.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОтбораРазмещенияТоваров.Перемещение") Тогда
		Заголовок = НСтр("ru = 'Внутрискладское перемещение'");	
    Иначе
		Возврат;
	КонецЕсли;                       
	...
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления, 
		Заголовок, Данные.Номер, Данные.Дата);    
...
//  Строка - Адрес временного хранилища, куда помещены данные для печати.
//
Функция КР_ДанныеДляПечатиЭтикетокЦенников(МассивДокументов) Экспорт
	...
	|	МИНИМУМ(ТоварыРазмещение.НомерСтроки) КАК Порядок,
	|	СУММА(ТоварыРазмещение.Количество) КАК Количество
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров.ТоварыРазмещение КАК ТоварыРазмещение
	|ГДЕ
	|	ТоварыРазмещение.Ссылка В(&МассивДокументов)
	// ГалкинАН #4228 (13.06.2024) galkin.an {
	// Было
	//|	И ТоварыРазмещение.Ячейка.Помещение.КР_ТипСкладскогоПомещения = ЗНАЧЕНИЕ(Перечисление.КР_ТипыСкладскихПомещений.ТорговыйЗал)
	// #4228 }	
	|
	|СГРУППИРОВАТЬ ПО
	|	ТоварыРазмещение.Номенклатура,
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОтборРазмещениеТоваров.Склад КАК Склад,
	|	ОтборРазмещениеТоваров.Склад.РозничныйВидЦены КАК ВидЦены
	|ИЗ
	|	Документ.ОтборРазмещениеТоваров КАК ОтборРазмещениеТоваров
	|ГДЕ
	|	ОтборРазмещениеТоваров.Ссылка В(&МассивДокументов)";
...
КонецФункции // >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028

// << 18.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751  
&НаСервере
Процедура КР_УстановитьПомещениеПоУмолчанию(Объект) Экспорт 
	...
	Если (АдресныйСкладБезордернойОтгрузки И (ЭтоОтбор Или ЭтоПеремещение))
		Или (ИспользоватьРазмещениеКакПриходныйОрдер И (ЭтоРазмещение Или ЭтоПеремещение)) Тогда 
		Объект.Помещение = Справочники.СкладскиеПомещения.КР_СкладскоеПомещениеСкладМагазина(Склад);
	Иначе
