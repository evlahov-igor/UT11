...
Процедура СоздатьЗаданиеНаПеремещение(Команда)
	...
	// << 07.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
	Если Не ЗначениеЗаполнено(Помещение) Тогда 
		КР_УстановитьПомещениеПоУмолчанию(ПараметрыЗаполнения);	
	КонецЕсли;	
	// >> 07.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044   
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	Элементы.СкладОтбор.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
	
	// A2105505-1736
	Элементы.ПомещениеОтбор.АвтоОтметкаНезаполненного = Ложь;
	//
	
	// << 22.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955
	// КР_ОприходованиеМежскладскихПеремещенийТоваровСТСД
	КомандаФормы = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект,
		"КР_ОприходованиеМежскладскихПеремещенийТоваровСТСД", 
		НСтр("ru = 'Оприходование межскладских перемещений товаров с ТСД'"),
		"КР_ОприходованиеМежскладскихПеремещенийТоваровСТСД"
		);
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		КомандаФормы, Элементы.КоманднаяПанельФормы);
	// >> 22.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955
		
КонецПроцедуры // >> 16.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751

// << 22.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955
&НаКлиенте
Процедура КР_ОприходованиеМежскладскихПеремещенийТоваровСТСД(Команда)

	ОткрытьФорму("Обработка.КР_ОприходованиеМежскладскихПеремещенийТоваровСТСД.Форма.Форма", , ЭтотОбъект);

КонецПроцедуры // >> 22.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955

// << 07.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044
&НаСервере
Процедура КР_УстановитьПомещениеПоУмолчанию(ПараметрыЗаполнения)
	Документы.ОтборРазмещениеТоваров.КР_УстановитьПомещениеПоУмолчанию(ПараметрыЗаполнения);	
КонецПроцедуры // >> 07.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2044   
