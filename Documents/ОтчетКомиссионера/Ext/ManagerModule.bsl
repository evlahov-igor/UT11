...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ОтчетКомиссионера.ОбъектРасчетовВознаграждение.УникальныйИдентификатор        КАК ИдентификаторФинЗаписи,
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Заполняет варианты отсчета графика и сдвиг для варианта оплаты ""Кредит (после отгрузки)"".
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("67398efb-ab3a-4398-adba-bd2c3cef5392");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетКомиссионера.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ НЕ ОтчетКомиссионера.ОплатаВВалюте
	|			И (ОтчетКомиссионера.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ОтчетКомиссионера.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|					ИСТИНА
	|				ИЗ
	|					Документ.ОтчетКомиссионера.ЭтапыГрафикаОплаты КАК ТЧЭтапыГрафикаОплаты
	|				ГДЕ
	|					ТЧЭтапыГрафикаОплаты.Ссылка = ОтчетКомиссионера.Ссылка
	|					И ( ТЧЭтапыГрафикаОплаты.ВариантОтсчета = ЗНАЧЕНИЕ(Перечисление.ВариантыОтсчетаДатыПлатежа.ПустаяСсылка)
	|						ИЛИ ТЧЭтапыГрафикаОплаты.ВариантОплаты = ЗНАЧЕНИЕ(Перечисление.ВариантыКонтроляОплатыКлиентом.ПустаяСсылка)
	...
	|																КОГДА РАЗНОСТЬДАТ(ОтчетКомиссионера.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ) < 0
	|																ТОГДА 0
	|																ИНАЧЕ РАЗНОСТЬДАТ(ОтчетКомиссионера.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ)
	|															КОНЕЦ
	|						)
	|				)
	...
	|	ИЛИ НЕ ОтчетКомиссионера.Проведен И (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.ОтчетКомиссионера.РасшифровкаПлатежаСКлиентом КАК ТЧРасшифровкаПлатежаСКлиентом
		...
		Попытка
			...
			Для Каждого Стр Из ДокументОбъект.ЭтапыГрафикаОплаты Цикл
				...
				Если НЕ ЗначениеЗаполнено(Стр.ВариантОплаты) Тогда
					Стр.ВариантОплаты = Перечисления.ВариантыКонтроляОплатыКлиентом.КредитСдвиг;
					Стр.Сдвиг = Сдвиг;
					ОбъектИзменен = Истина;
				КонецЕсли;
