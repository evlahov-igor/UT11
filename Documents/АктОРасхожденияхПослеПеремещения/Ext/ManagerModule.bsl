...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	МеханизмыДокумента.Добавить("ВнутреннееТовародвижение"); // для РН.ЗаказыНаПеремещение
	МеханизмыДокумента.Добавить("КР_КоробаКПоступлению");   
	МеханизмыДокумента.Добавить("КР_РасхожденияПоКоробам");   
	// >> 20.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	// << 20.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	КР_ТекстЗапросаТаблицаЗаказыНаПеремещение(Запрос, ТекстыЗапроса, Регистры);
	КР_ТекстЗапросаТаблицаРасхожденияПоКоробам(Запрос, ТекстыЗапроса, Регистры);
	КР_ТекстЗапросаТаблицаКоробаКПоступлению(Запрос, ТекстыЗапроса, Регистры);
	// >> 20.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	
	// << 07.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2158  
	...
	// << 18.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2612
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.АктОРасхожденияхПослеПеремещения) Тогда
		...
		// Обернем в структуру типовой параметр команды
		// Наличие свойства "КР_ЗагружатьКороба" будет нам говорить о том что 
		//	требуется загружать короба. Значением свойства будет являться параметр команды (типовое поведение)
		КомандаСоздатьНаОсновании.ИмяПараметраФормы = "Основание.КР_ЗагружатьКороба";
		// >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
//   Строка - текст запроса
//
Функция ТекстЗапросаПоОснованиюПеремещения(
	// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	КР_ВыбиратьДанныеПоКоробам = Ложь
	// >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	) Экспорт
	...
	// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	// Текст запроса выборки по коробам должен быть всегда в конце
	//	
	Если КР_ВыбиратьДанныеПоКоробам Тогда 
		
		ТекстЗапроса = ТекстЗапроса + ";
		|
		|////////////////////////////////////////////////////////////////////////////////
		|// 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702 
		|// Выборка по коробам. Должна быть всегда в конце пакета
		|
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	Т.КР_Короб 							КАК Короб,
		|	Т.Ссылка 							КАК ДокументОснование,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Т.КР_Короб) 	КАК КоличествоПоДокументу
		|ИЗ
		|	Документ.ПеремещениеТоваров.Товары КАК Т
		|ГДЕ
		|	Т.Ссылка В (&Основания)
		|	И Т.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	Т.КР_Короб,
		|	Т.Ссылка";
...
ТекстЗапросаДокумента =
	...
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("ТоварыНаСкладах", Регистры) Тогда
		СкладыСервер.ОформитьИзлишкиНедостачиНаСкладеАкты(Запрос, ТекстыЗапроса, "ТоварыНаСкладах", ТекстЗапросаДокумента);
	КонецЕсли;
...
// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Функция КР_ПолучитьСкладРазбораРасхождений(Склад) Экспорт   
	
	Если Склад.КР_ВидСклада = Перечисления.КР_ВидыСкладов.ЦентральныйСклад Тогда 
		ВидСкладаДляПоиска = Перечисления.КР_ВидыСкладов.СкладРазбораРасхожденийЦентральный;  
	// << 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595
	ИначеЕсли Склад.КР_ВидСклада = Перечисления.КР_ВидыСкладов.СкладФулфилмента Тогда 
		ВидСкладаДляПоиска = Перечисления.КР_ВидыСкладов.СкладРазбораРасхожденийФулфилмента;  	
	// >> 27.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2595	
	Иначе 
		ВидСкладаДляПоиска = Перечисления.КР_ВидыСкладов.СкладРазбораРасхожденийМагазинов;
	КонецЕсли;	
	
	Запрос = Новый Запрос(КР_ТекстЗапросаПервыйНайденныйСкладПоВидуСклада());
	Запрос.УстановитьПараметр("ВидСкладаДляПоиска", ВидСкладаДляПоиска);       

	ВыборкаДетальныеЗаписи = Запрос.Выполнить().Выбрать();
	Если Не ВыборкаДетальныеЗаписи.Следующий() Тогда 
		ТекстИсключения = НСтр("ru = 'Не удалось определить склад разбора расхождений'");
		ВызватьИсключение ТекстИсключения;   
	КонецЕсли;	

	Возврат ВыборкаДетальныеЗаписи.Ссылка;
	
КонецФункции // >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Функция КР_ТекстЗапросаТаблицаЗаказыНаПеремещение(Запрос, ТекстыЗапроса, Регистры)
	...
	// 19.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2592
	// Изменения в запросе согласно задаче
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)  КАК ВидДвижения,
	|	НачалоПериода(&Период, День) 			КАК Период,
	|	ТаблицаТовары.Заказ         			КАК ЗаказНаПеремещение,
	|	ТаблицаТовары.Номенклатура   			КАК Номенклатура,
	|	ТаблицаТовары.Характеристика 			КАК Характеристика,
	|	ТаблицаТовары.Серия          			КАК Серия,
	|	ТаблицаТовары.КодСтроки      			КАК КодСтроки,
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу	КАК КОформлению,
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу 	КАК Заказано
	|ИЗ
	|	Документ.АктОРасхожденияхПослеПеремещения.Товары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка 
	|	И ТаблицаТовары.КодСтроки <> 0
	|	И ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу <> 0
...
// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Функция КР_ТекстЗапросаТаблицаРасхожденияПоКоробам(Запрос, ТекстыЗапроса, Регистры)
	ИмяРегистра = "КР_РасхожденияПоКоробам";
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ТаблицаКороба.КоличествоПоДокументу > ТаблицаКороба.Количество
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) 
	|	КОНЕЦ КАК ВидДвижения,
	|	НачалоПериода(&Период, День) 			КАК Период,
	|	ТаблицаКороба.Короб         			КАК Короб,
	|	ВЫБОР
	|		КОГДА ТаблицаКороба.КоличествоПоДокументу > ТаблицаКороба.Количество
	|			ТОГДА ТаблицаКороба.КоличествоПоДокументу - ТаблицаКороба.Количество
	|		ИНАЧЕ ТаблицаКороба.Количество - ТаблицаКороба.КоличествоПоДокументу 
	|	КОНЕЦ 									КАК Количество
	|ИЗ
	|	Документ.АктОРасхожденияхПослеПеремещения.КР_Короба КАК ТаблицаКороба
	|ГДЕ
	|	ТаблицаКороба.Ссылка = &Ссылка 
	|	И ТаблицаКороба.КоличествоПоДокументу <> ТаблицаКороба.Количество";      
...
Функция КР_ТекстЗапросаТаблицаКоробаКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НачалоПериода(&Период, День) 			КАК Период,
	|	ТаблицаКороба.ДокументОснование         КАК ДокументПоступления,
	|	ТаблицаКороба.Короб         			КАК Короб,
	|	&СкладПолучатель	         			КАК Склад,
	|	&СкладОтправитель	         			КАК Отправитель,
	|	ТаблицаКороба.Количество 
	|	 - ТаблицаКороба.КоличествоПоДокументу	КАК КОформлениюОрдеров
	|ИЗ
	|	Документ.АктОРасхожденияхПослеПеремещения.КР_Короба КАК ТаблицаКороба
	|ГДЕ
	|	ТаблицаКороба.Ссылка = &Ссылка 
...
Функция КР_ТекстЗапросаТаблицаТоварыКОтгрузке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.Серия          			КАК Серия,
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу	КАК КОтгрузке,
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу	КАК КОформлению,
	|	ВЫБОР 
	|		КОГДА ТаблицаТовары.Ссылка.СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки
...
КонецФункции // >> 07.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2158
...
Функция КР_ТекстЗапросаТаблицаТоварыКОтбору(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу	КАК КОтбору
	|ИЗ
	|	Документ.АктОРасхожденияхПослеПеремещения.Товары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка 
	...
	|	И  ТаблицаТовары.Действие =
    |		ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеОтгрузки.КР_ПереместитьНаСкладРазбора)	
	|	И ТаблицаТовары.Ссылка.СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки
	|";      
...
Функция КР_ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.Серия          			КАК Серия,
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу	КАК КОформлениюНакладныхПоРаспоряжению,
	|	ТаблицаТовары.Количество
	|		-ТаблицаТовары.КоличествоПоДокументу	КАК КОформлениюПоступленийПоРаспоряжению
	|ИЗ
	|	Документ.АктОРасхожденияхПослеПеремещения.Товары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка 
...
КонецФункции // >> 18.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2612

#КонецОбласти

// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Процедура КР_ЗаполнитьПоПриемкеСервер(
	Объект, ПараметрыУказанияСерий, ИспользоватьОрдернуюСхемуСкладПолучатель) Экспорт 

	// Текст медода перенесен из ФормаДокумента.ЗаполнитьПоПриемкеСервер
	// В нем есть подробное описание причины и объема перенесенного кода
	...
	// Кастом. Адаптация кода под модуль менеджера
	//	см. описание в КР_ДополнитьТаблицуСпособомСоединения 
	ТаблицаОрдера = КР_ДополнитьТаблицуСпособомСоединения(ТаблицаОрдера, ПараметрыУказанияСерий, Объект.СкладОтправитель, 
	...
	// Кастом. Адаптация кода под модуль менеджера
	//	см. описание в КР_ДополнитьТаблицуСпособомСоединения 
	ТаблицаНакладная = КР_ДополнитьТаблицуСпособомСоединения(ТаблицаНакладная, ПараметрыУказанияСерий, Объект.СкладОтправитель, 
							Объект.СкладПолучатель, ОрдернаяСхемаПриОтгрузке, ИспользоватьОрдернуюСхемуСкладПолучатель);
	...
	Для Каждого КлючЗначение Из ДействияСПолученнымиТаблицами Цикл
		...
		// << 16.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-702
		// Ошибка поставщика (1С)
		//ПараметрыОтбора = Новый Структура("Заказ", Документы.ЗаказНаПеремещение.ПустаяСсылка());
		ПараметрыОтбора = Новый Структура("ДокументОснование", Документы.ПеремещениеТоваров.ПустаяСсылка());
		// >> 16.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-702
		...
		Для Каждого Строка Из СтрокиСПустымЗаказом Цикл
			...
			// << 16.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-702
			// ??? Ошибка поставщика (1С)
			ИначеЕсли Не ЗначениеЗаполнено(Строка.Распоряжение) 
				И ЗначениеЗаполнено(Строка.Заказ) Тогда
				СтрокаСДокументомОснованием = ТаблицаНакладнойПоДействию.Найти(Строка.Заказ, "Заказ");
				Если СтрокаСДокументомОснованием <> Неопределено Тогда
					Строка.ДокументОснование = СтрокаСДокументомОснованием.ДокументОснование;
				КонецЕсли;
			// >> 16.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-702
	...
	// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702   
	КР_ЗаполнитьПоПриемкеСерверЗавершение(Объект);
	// >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	
КонецПроцедуры // >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702

// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Функция КР_ДополнитьТаблицуСпособомСоединения(Таблица, 
	ПараметрыУказанияСерий, СкладОтправитель, СкладПолучатель, ОрдернаяСхемаПриОтгрузке, ОрдернаяСхемаПриПриемке)

	// Текст медода перенесен из ФормаДокумента.ДополнитьТаблицуСпособомСоединения 
	// так как используется в КР_ЗаполнитьПоПриемкеСервер (ранее ФормаДокумента.ЗаполнитьПоПриемкеСервер)
	// В нем есть подробное описание причины и объема перенесенного кода
...
// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Процедура КР_ЗаполнитьПоПриемкеСерверЗавершение(Объект) 

	Запрос = Новый Запрос(КР_ТекстЗапросаКоробаПоПриемке());   
	ДокументыОснования = Объект.Товары.Выгрузить().ВыгрузитьКолонку("ДокументОснование");
	ДокументыОснования = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДокументыОснования);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		...
		Короб = ВыборкаДетальныеЗаписи.Короб;             
		СтрокаСКоробом = Объект.КР_Короба.НайтиСтроки(Новый Структура("Короб", Короб));
		Если ЗначениеЗаполнено(СтрокаСКоробом) Тогда   
			СтрокаСКоробом = СтрокаСКоробом[0];     
		Иначе	
			СтрокаСКоробом = Объект.КР_Короба.Добавить();
			СтрокаСКоробом.Короб = Короб;
			СтрокаСКоробом.ДокументОснование = ВыборкаДетальныеЗаписи.ДокументОснование; 
		КонецЕсли;                                                               
		
		СтрокаСКоробом.Количество = ВыборкаДетальныеЗаписи.Количество;
...
// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Функция КР_ТекстЗапросаПервыйНайденныйСкладПоВидуСклада()
	...
	|ГДЕ
	|	Склады.КР_ВидСклада = &ВидСкладаДляПоиска
	|	И НЕ Склады.ПометкаУдаления";
	
КонецФункции // >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	
// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
Функция КР_ТекстЗапросаКоробаПоПриемке()
	
	Возврат
	"ВЫБРАТЬ
	|	Т.ДокументПоступления КАК ДокументОснование,
	|	Т.Короб КАК Короб,
	|	Т.КОформлениюОрдеров КАК Количество
	|ИЗ
	|	РегистрНакопления.КР_КоробаКПоступлению КАК Т
	|ГДЕ
	|	Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И Т.ДокументПоступления В(&ДокументыОснования)";
