...
Процедура ЗаполнитьПоОснованиям(ВариантЗаполнения = "Вариант1")
	...
	Запрос.Текст = Документы.АктОРасхожденияхПослеПеремещения.ТекстЗапросаПоОснованиюПеремещения(
		// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
		ЭтотОбъект.КР_ИспользуютсяКороба
		// >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	);
	...
	// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	Объект.КР_Короба.Очистить();
	Если ЭтотОбъект.КР_ИспользуютсяКороба Тогда 
		Объект.КР_Короба.Загрузить(РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить());  
	КонецЕсли;	
	// >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище, КэшированныеЗначения)
	...
	//<< 20.02.2021 Фомичев А.Е. КРОК, JIRA№A2105505-2639
	//РасхожденияСервер.ПолучитьЗагруженныеТоварыИзХранилища(Объект.Товары, ЭтаФорма.ДокументыОснования, АдресТоваровВХранилище);
	КР_ОбщегоНазначениеСервер.ПолучитьЗагруженныеТоварыИзХранилища(Объект.Товары, ЭтаФорма.ДокументыОснования, АдресТоваровВХранилище);  
	//>> 20.02.2021 Фомичев А.Е. КРОК, JIRA№A2105505-2639
...
Процедура ЗаполнитьПоПриемкеСервер()
	...
	// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	// По ФДР этот метод должен вызываться для ДокументОбъект (из набора записей регистра)
	// Для выполнения данного требования метод частично вынесен в модуль менеджера текущего документа
	// Код, который вынесен никак не изменен.
	// Для отсутсвия дублирования логики та часть кода, которая вынесена в модуль менеджера
	//	"выключена" здесь через условие препроцессова "Не Сервер" чтоб избежать затраты на компиляцию  
	// В случае появления различий при обновлении на следующий релиз нужно то что в этом условии 
	//	перенести в метод Документы.АктОРасхожденияхПослеПеремещения.КР_ЗаполнитьПоПриемкеСервер
	
	Документы.АктОРасхожденияхПослеПеремещения.
		КР_ЗаполнитьПоПриемкеСервер(Объект, ПараметрыУказанияСерий, ИспользоватьОрдернуюСхемуСкладПолучатель);
...
Функция ДополнитьТаблицуСпособомСоединения(Таблица, ПараметрыУказанияСерий, СкладОтправитель, СкладПолучатель, ОрдернаяСхемаПриОтгрузке, ОрдернаяСхемаПриПриемке)
	...
	// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702 
	// Данный метод перенесен в молуль менеджера текущего документа (КР_ДополнитьТаблицуСпособомСоединения)
	// 	так как теперь вызывается из Документы.АктОРасхожденияхПослеПеремещения.КР_ЗаполнитьПоПриемкеСервер
	// Подробное описание причины смотри в ЗаполнитьПоПриемкеСервер()
	// В случае появления различий при обновлении на следующий релиз 
	//	нужно все тело метода перенести в Документы.АктОРасхожденияхПослеПеремещения.КР_ДополнитьТаблицуСпособомСоединения
	// >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
Процедура КР_ПриЧтенииСозданииНаСервере()
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_СкладРазбораРасхождений", , Элементы.НадписьРасхождения);
	ЭлементФормы.Заголовок = НСтр("ru = 'Склад разбора'");                
	...
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ИспользуютсяКороба", Новый ОписаниеТипов("Булево"));
	
	ЭтотОбъект.КР_ИспользуютсяКороба = Объект.КР_Короба.Количество();
	
	КР_РазместитьСтраницуКороба();   
	
	КР_КоробаЗаполнитьРеквизитыФормы();
...
КонецПроцедуры // >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702

// << 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаСервере
Процедура КР_КоробаЗаполнитьРеквизитыФормы()  
	
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПризнакНаличиеКомментария");

	ОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.КР_Короба, СтруктураДействий, Неопределено);
	
КонецПроцедуры // >> 19.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702

// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаСервере
Процедура КР_РазместитьСтраницуКороба()
	
	// Страница Короба
	СтраницаКороба = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_СтраницаКороба", , Элементы.СтраницаДополнительно);
	СтраницаКороба.Вид = ВидГруппыФормы.Страница;
	СтраницаКороба.Заголовок = НСтр("ru = 'Короба'");      
	// При повторном вызове метода, так как элемент уже создан, 
	//	ругается на установку ПутьКДаннымЗаголовка
	Если Не ЗначениеЗаполнено(СтраницаКороба.ПутьКДаннымЗаголовка) Тогда 
		СтраницаКороба.ПутьКДаннымЗаголовка = "Объект.КР_Короба.КоличествоСтрок"; 
	КонецЕсли;	
	СтраницаКороба.Видимость = ЭтотОбъект.КР_ИспользуютсяКороба;
	
	#Область ТаблицаКороба

	// КР_Короба
	ТаблицаКороба = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба", СтраницаКороба); 
	ТаблицаКороба.УстановитьДействие("Выбор", "КР_КоробаВыбор");
	
	// КР_Короба.НомерСтроки
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба.НомерСтроки", ТаблицаКороба);
	
	// КР_Короба.Короб
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба.Короб", ТаблицаКороба);
	
	// КоробаГруппаКоличество
	КоробаГруппаКоличество = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_КоробаГруппаКоличество", ТаблицаКороба);   
	КоробаГруппаКоличество.ОтображатьВШапке = Истина;
	КоробаГруппаКоличество.Группировка = ГруппировкаКолонок.Горизонтальная;
	КоробаГруппаКоличество.Заголовок = НСтр("ru = 'Количество'");
	
	// КР_Короба.КоличествоПоДокументу
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба.КоличествоПоДокументу", КоробаГруппаКоличество);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.Заголовок = НСтр("ru = 'По документу'");
	
	// КР_Короба.Количество
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба.Количество", КоробаГруппаКоличество);  
	ЭлементФормы.Заголовок = НСтр("ru = 'Факт'");
	
	// КоробаНаличиеКомментария
	КоробаНаличиеКомментария = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_КоробаНаличиеКомментария", ТаблицаКороба);   
	КоробаНаличиеКомментария.ОтображатьВШапке = Истина;
	КоробаНаличиеКомментария.Группировка = ГруппировкаКолонок.Горизонтальная;
	КоробаНаличиеКомментария.Заголовок = НСтр("ru = 'Комментарий'");
	
	// ЕстьКомментарийПолучателя
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"ЕстьКомментарийПолучателя", Новый ОписаниеТипов("Булево"),
		"Объект.КР_Короба", 
		НСтр("ru = 'получателя'"));
		
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		РеквизитФормы, КоробаНаличиеКомментария);
	ЭлементФормы.Вид = ВидПоляФормы.ПолеКартинки;	
	ЭлементФормы.КартинкаЗначений = БиблиотекаКартинок.Комментарий;	 
	
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"ЕстьКомментарийМенеджера", Новый ОписаниеТипов("Булево"),
		"Объект.КР_Короба", 
		НСтр("ru = 'менеджера'"));
	...
	#Область ПодвалСтраницы

	// КоробаГруппаКомментарии
	КоробаГруппаКомментарии = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_КоробаГруппаКомментарии", СтраницаКороба);   
	КоробаГруппаКомментарии.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	КоробаГруппаКомментарии.ОтображатьЗаголовок = Ложь;
	КоробаГруппаКомментарии.Отображение = ОтображениеОбычнойГруппы.Нет;
	
	// КР_Короба.КомментарийПолучателя
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба.КомментарийПолучателя", КоробаГруппаКомментарии);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МногострочныйРежим = Истина;
	ЭлементФормы.КнопкаВыбора = Истина;
	ЭлементФормы.УстановитьДействие("НачалоВыбора", "КР_КоробаКомментарийПолучателяНачалоВыбора");
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_КоробаКомментарийПолучателяПриИзменении");
	
	// КР_Короба.КомментарийМенеджера
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Короба.КомментарийМенеджера", КоробаГруппаКомментарии);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МногострочныйРежим = Истина;
	ЭлементФормы.КнопкаВыбора = Истина;     
	ЭлементФормы.УстановитьДействие("НачалоВыбора", "КР_КоробаКомментарийМенеджераНачалоВыбора");
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_КоробаКомментарийМенеджераПриИзменении");
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_КоробаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ТекущаяСтрока = Элементы.КР_Короба.ТекущиеДанные;

	Если Поле = Элементы.КР_КоробаЕстьКомментарийПолучателя И ПравоНаРедактирование Тогда
		
		КР_КомментарийНачалоВыбора(
			"КомментарийПолучателя", "ЕстьКомментарийПолучателя", "КР_КоробаКомментарийПолучателя");
		
	ИначеЕсли Поле = Элементы.КР_КоробаЕстьКомментарийМенеджера И ПравоНаРедактирование Тогда
		
		КР_КомментарийНачалоВыбора(
			"КомментарийМенеджера", "ЕстьКомментарийМенеджера", "КР_КоробаКомментарийМенеджера");
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_КоробаКомментарийПолучателяНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_КоробаКомментарийПолучателяПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.КР_Короба.ТекущиеДанные;
	ТекущиеДанные.ЕстьКомментарийПолучателя = Не ПустаяСтрока(ТекущиеДанные.КомментарийПолучателя);
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_КоробаКомментарийМенеджераНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_КоробаКомментарийМенеджераПриИзменении(Элемент)
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_КомментарийНачалоВыбора(ИмяПоляКомментарий, ИмяПоляЕстьКомментарий, ИмяПоляРедактирования)
	
	// Код взят из КомментарийМенеджераНачалоВыбора() -> РасхожденияКлиент.КомментарийНачалоВыбора
	//	так как вызов не универсальный
	...
	ОписаниеОповещенияЗавершение = Новый ОписаниеОповещения(
		"РедактированиеКомментарияЗавершение", РасхожденияКлиент, ДополнительныеПараметры);
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(ОписаниеОповещенияЗавершение, 
		Элементы[ИмяПоляРедактирования].ТекстРедактирования);
