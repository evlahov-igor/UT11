...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Не ПодключенаОбработкаКодовМаркировки Тогда
		СобытияФормИСМП.ПодключитьОбработкуКодовМаркировки(ЭтотОбъект,,"КоличествоПотребительскихУпаковок");
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Если Форма.Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ВыводИзОборотаЭкспортВСтраныЕАЭС") Тогда
		Форма.Элементы.СтранаНазначения.Видимость = Истина;
	Иначе
		Форма.Элементы.СтранаНазначения.Видимость = Ложь;
	КонецЕсли;
