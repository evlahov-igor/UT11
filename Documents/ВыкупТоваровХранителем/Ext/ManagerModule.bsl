...
//
Процедура РассчитатьСостояние(Знач ДокументСсылка, СостояниеРасчетов) Экспорт
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Обработчик = Документы.ВыкупТоваровХранителем.ОбработчикДействий(Данные.ХозяйственнаяОперация);
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажиПереданныеПартнерам)
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыкупТоваровПереработчиком)
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("62ef3ffe-f7b3-44e0-b531-28418f0db8e4");
	...
	Обработчик.Комментарий = НСтр("ru = 'В выкупах товаров хранителем в табличной части Товары заполняет номер ГТД. В случае если одной строке из табличной части Товары соответствует несколько номеров ГТД из табличной части ""Виды запасов"", то разбивает строки в табличной части Товары.
	...
	|До окончания выполнения обработчика редактирование документов Выкупов товаров хранителем будет недоступно.'");
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.ВыкупТоваровХранителем.СгенерироватьОбъектыРасчетов";
	Обработчик.Версия = "11.5.6.189";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c5f4d096-df06-4918-9859-897f9ac28f60");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ВыкупТоваровХранителем.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ВыкупТоваровХранителем";
	...
	|					ИСТИНА
	|				ИЗ
	|					Документ.ВыкупТоваровХранителем.Товары КАК ВыкупТоваровХранителемТовары
	|				ГДЕ
	|					ВыкупТоваровХранителем.Ссылка = ВыкупТоваровХранителемТовары.Ссылка
	|					И ВыкупТоваровХранителемТовары.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|					И ВыкупТоваровХранителемТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|			ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ВыкупТоваровХранителем.ВидыЗапасов КАК ВыкупТоваровХранителемВидыЗапасов
	|				ГДЕ
	|					ВыкупТоваровХранителем.Ссылка = ВыкупТоваровХранителемВидыЗапасов.Ссылка
	|					И ВыкупТоваровХранителемВидыЗапасов.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|					И ВыкупТоваровХранителемВидыЗапасов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)))
	|	ИЛИ НЕ ВыкупТоваровХранителем.ОплатаВВалюте
	|			И (ВыкупТоваровХранителем.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ВыкупТоваровХранителем.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ (ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ВыкупТоваровХранителем.РасшифровкаПлатежа КАК ТЧРасшифровкаПлатежа
	|				ГДЕ
	|					ТЧРасшифровкаПлатежа.Ссылка = ВыкупТоваровХранителем.Ссылка
	...
	|	ИЛИ (ВыкупТоваровХранителем.Проведен
	|			И ВыкупТоваровХранителем.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка))
	|	ИЛИ НЕ ВыкупТоваровХранителем.Проведен И (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.ВыкупТоваровХранителем.РасшифровкаПлатежа КАК ТЧРасшифровкаПлатежа
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ОбновляемыеДанные.Количество() > 0 Тогда
		...
		|	ТоварыДокумента.НомерГТД                   КАК НомерГТД,
		|	-ТоварыДокумента.Количество                КАК Количество
		|ПОМЕСТИТЬ ВТКЗаполнениюНомеровГТД
		|ИЗ
		...
		|	КЗаполнениюНомеровГТД.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	КЗаполнениюНомеровГТД.СтавкаНДС                  КАК СтавкаНДС,
		|	КЗаполнениюНомеровГТД.НомерГТД                   КАК НомерГТД,
		|	СУММА(КЗаполнениюНомеровГТД.Количество)          КАК Количество,
		|	СУММА(ЕСТЬNULL(КЗаполнениюНомеровГТД.Количество, 0)) > 0 КАК ТребуетсяУказатьНомераГТД
		|ИЗ
		|	ВТСсылкиДляОбработки КАК СсылкиДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКЗаполнениюНомеровГТД КАК КЗаполнениюНомеровГТД
		|		ПО СсылкиДляОбработки.Ссылка = КЗаполнениюНомеровГТД.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	СсылкиДляОбработки.Ссылка,
		|	КЗаполнениюНомеровГТД.АналитикаУчетаНоменклатуры,
		|	КЗаполнениюНомеровГТД.СтавкаНДС,
		|	КЗаполнениюНомеровГТД.НомерГТД
		|
		|УПОРЯДОЧИТЬ ПО
		|	СсылкиДляОбработки.Ссылка
		|
		...
		Пока ВыборкаПоДокументам.Следующий() Цикл
			...
			Попытка
				...
				Пока ВыборкаПоТоварам.Следующий() Цикл
					...
					Если Не ВыборкаПоТоварам.ТребуетсяУказатьНомераГТД Тогда
						Продолжить;
					КонецЕсли;
					
					КоличествоРаспределить = ВыборкаПоТоварам.Количество;
					
					ОтборПоТоварам = Новый Структура("АналитикаУчетаНоменклатуры, УдалитьСтавкаНДС, НомерГТД");
