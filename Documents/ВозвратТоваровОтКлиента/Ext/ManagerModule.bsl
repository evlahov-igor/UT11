...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// << 28.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-751         
		КР_ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры);
		// >> 28.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-751         
		
		// << 11.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1942
...
Функция КР_ТекстЗапросаТаблицаТоварыКОформлениюОптовыхПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	&Период КАК Период,
	|	ШапкаДокумента.КР_РозничныйВозврат КАК ИсточникДанных,
	|	ТаблицаТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаТовары.Характеристика КАК Характеристика,
	...
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента.Товары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента КАК ШапкаДокумента
	|		ПО ТаблицаТовары.Ссылка = ШапкаДокумента.Ссылка
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ШапкаДокумента.КР_РозничныйВозврат <> ЗНАЧЕНИЕ(Документ.ОтчетОРозничныхПродажах.ПустаяСсылка)
	|	И ТаблицаТовары.Количество > 0";
...
Функция КР_ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Склад 									КАК Склад,
	|	ДанныеШапки.КР_Помещение 				КАК Помещение,
	|	ВЫБОР
	|		КОГДА Т.СтатусУказанияСерий В (6, 8, 10, 14)
	|			ТОГДА Т.Серия
	...
	|		ПО (ДанныеШапки.Ссылка = Т.Ссылка)
	|ГДЕ
	|	Т.Ссылка = &Ссылка
	|	И Т.Количество <> 0
	|	И ДанныеШапки.Склад.КР_СкладСПомещениямиБезордернойОтгрузки
	|	И ДанныеШапки.Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер // A2105505-1942";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса, ИмяРегистра);
	
	Возврат ТекстЗапроса;
	
КонецФункции // >> 28.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-751
...
Функция КР_ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	0 КОформлениюПоступленийПоОрдерам,
	|	Т.Количество КАК КОформлениюОрдеров,
	|	0 КАК Принимается
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента.Товары КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента КАК ДанныеШапки
	...
	|	0 КАК КОформлениюПоступленийПоРаспоряжению,
	|	0 КАК КОформлениюПоступленийПоНакладным,
	|	Т.Количество КОформлениюПоступленийПоОрдерам,
...
КонецФункции // >> 11.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1942
