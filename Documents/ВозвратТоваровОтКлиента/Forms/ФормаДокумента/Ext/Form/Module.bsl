...
Процедура ЗагрузитьИзВнешнегоФайлаЗавершение(АдресЗагруженныхДанных, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(АдресЗагруженныхДанных) Тогда
		...
		ОценкаПроизводительностиКлиент.ЗамерВремени(
			"Документ.ВозвратТоваровОтКлиента.ФормаДокумента.Команда.ЗагрузитьИзВнешнегоФайла");
		// <-- Евлахов Игорь Николаевич (Конец) 23.08.2023
...
Процедура СкладПриИзмененииСервер()
	...
	Объект.КР_Помещение = Справочники.СкладскиеПомещения.КР_СкладскоеПомещениеИнтернетТорговля(Объект.Склад);
...
Процедура РассчитатьИтоговыеПоказателиВозврата(Форма)
	...
	// << 29.03.2023, Федоров Д.Е., КРОК, JIRA№A2105505-1469
	Форма.КР_КоличествоТоварыИтого = Форма.Объект.Товары.Итог("Количество");
	// >> 29.03.2023, Федоров Д.Е., КРОК, JIRA№A2105505-1469
...
Процедура ЗаполнитьНомераГТДСервер(ВариантПолученияДанных, ИмяПоляРаспоряжение)
	...
	// ГалкинАН #4129 (10.06.2024) galkin.an {
	ИначеЕсли ВариантПолученияДанных = "злПоДатамПоступленияТоваровОрганизаций" Тогда
		злЗаполнитьНомераГТДПоДатамПоступленияТоваров();
		Возврат;
	// #4129 }
	Иначе
...
// << 19.02.2022 Марченко С.Н., КРОК, JIRA№A2105505-725
&НаСервере
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_РозничныйВозврат", Элементы.ГруппаПараметрыПраво);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 28;
	ЭлементФормы.ТолькоПросмотр = Истина;
	
	// КР_ЗаказИнтернетМагазина
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ЗаказИнтернетМагазина", Элементы.ГруппаПараметрыПраво);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 28;
	ЭлементФормы.ТолькоПросмотр = Истина;
	...
	// КР_Помещение
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Помещение", , Элементы.СпособКомпенсации);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 28;
	...
	// << 17.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1418
	КомандаФормы = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_ЗаполнитьРеализации", НСтр("ru = 'Заполнить документы продажи'")); 
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КомандаФормы, , Элементы.ТоварыЗаполнитьСебестоимость);
	// >> 17.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1418
	
	// ГалкинАН #4129 (30.05.2024) galkin.an {
	ИмяКоманды = "злЗаполнитьГТДТоваров";
	ЗаголовокКоманды = НСтр("ru = 'Заполнить ГТД товаров'");
	ДействиеКоманды = "Подключаемый_ЗаполнитьНомераГТД";
	
	Команда = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект, 
															ИмяКоманды, 
															ЗаголовокКоманды, 
															ДействиеКоманды);
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, 
																Команда, 
																Элементы.ГруппаТоварыПодобратьТовары,
																, 
																"КнопкаФормы");  
	...
	КР_МетодыМодификацииФорм.ДобавитьЭлементФормыКоличествоТоварыИтого(ЭтотОбъект,
		Элементы.ГруппаИтого, Элементы.ГруппаСтраницыНДС);
	// >> 29.03.2023, Федоров Д.Е., КРОК, JIRA№A2105505-1469
	
КонецПроцедуры // >> 19.02.2022 Марченко С.Н., КРОК, JIRA№A2105505-725
...
Процедура КР_УстановитьВидимостьДоступность()
	...
	СкладСПомещениямиБезордернойОтгрузки = Склад.КР_СкладСПомещениямиБезордернойОтгрузки
	    // A2105505-1942
		И Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер
		//
		;
	Элементы.КР_Помещение.Видимость = СкладСПомещениямиБезордернойОтгрузки;
	
КонецПроцедуры // >> 10.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751

// << 17.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1418
&НаКлиенте
Процедура КР_ЗаполнитьРеализации(Команда)
	
	Если Объект["Товары"].Количество() = 0 Тогда
		
		ТекстПредупреждения = НСтр("ru = 'В документе не заполнен список Товары. Документы продажи не могут быть заполнены.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
		
	КонецЕсли;                     
	
	// << 06.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1927
	Если Модифицированность Тогда 
		
		ТекстСообщения = НСтр("ru = 'Документ изменен.
        	|Запишите документ и повторите попытку.'");   
	...
	КР_ЗаполнитьРеализацииСервер();
	
КонецПроцедуры // >> 17.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1418

// << 17.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1418
&НаСервере
Процедура КР_ЗаполнитьРеализацииСервер()
	
	КР_ЗаполнитьРеализацииНаСервере();
	ФинансыКлиентСервер.ПересчитатьСуммыВСтрокеРасшифровкиПлатежа(Объект, СуммаВсего);
...
// << 17.03.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1418
&НаСервере
Процедура КР_ЗаполнитьРеализацииНаСервере()
	
	// Прим. разработчика
	// Чтобы не дублировать типовой функционал модуля,
	// выгрузим таблицу товаров, заполним типовым способом,
	// после чего вернем на место цены из исходной таблицы
	
	ТаблицаДоИзменений = Объект.Товары.Выгрузить();
	
	// << 25.06.2024 Вдовенко С.А., КРОК, A2105505-2676
	//ПродажиСервер.ЗаполнитьРеализацииИЦены(Объект, "Товары");
	ДополнительныеФильтры = Новый Структура;
	ДополнительныеФильтры.Вставить("Склад", Объект.Склад);
	ПродажиСервер.ЗаполнитьРеализацииИЦены(Объект, "Товары", ДополнительныеФильтры);
	// >> 25.06.2024 Вдовенко С.А., КРОК, A2105505-2676
	
	Для Каждого ТекСтрока Из Объект.Товары Цикл
		СтрокаТаблицы = ТаблицаДоИзменений.Найти(ТекСтрока.ИдентификаторСтроки, "ИдентификаторСтроки");
		Если Не ЗначениеЗаполнено(ТекСтрока.ИдентификаторСтроки)
			Или СтрокаТаблицы = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ТекСтрока.Цена = СтрокаТаблицы.Цена;    
		
		// << 09.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2358
		ТекСтрока.СтавкаНДС = СтрокаТаблицы.СтавкаНДС;    
		// >> 09.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2358
...
&НаКлиенте
Процедура Подключаемый_ЗаполнитьНомераГТД(Команда)

	ЗаполнитьНомераГТД("злПоДатамПоступленияТоваровОрганизаций");
...
&НаСервере
Процедура злЗаполнитьНомераГТДПоДатамПоступленияТоваров()

	ТоварыКЗаполнению = ПолучитьТоварыКЗаполнению(Объект.Товары, Объект.ВидыЗапасов);
	...
	//	|	ТоварыКЗаполнению.ВидЗапасов КАК ВидЗапасов
	...
	//	|	ВТ_НоменклатураДокумента.Характеристика КАК Характеристика,
	//	|	ВТ_НоменклатураДокумента.ВидЗапасов КАК ВидЗапасов,
	//	|	МАКСИМУМ(ДатыПоступленияТоваровОрганизаций.ДатаПоступления) КАК ДатаПоступления
	//	|ПОМЕСТИТЬ ВТ_АктуальныеДатыСреза
	//	|ИЗ
	...
	//	|		ПО ВТ_НоменклатураДокумента.Номенклатура = ДатыПоступленияТоваровОрганизаций.Номенклатура
	//	|			И ВТ_НоменклатураДокумента.Характеристика = ДатыПоступленияТоваровОрганизаций.Характеристика
	//	|			И ВТ_НоменклатураДокумента.ВидЗапасов = ДатыПоступленияТоваровОрганизаций.ВидЗапасов
	//	|ГДЕ
	//	|	ДатыПоступленияТоваровОрганизаций.ДатаПоступления <= &ДатаСреза
	//	|
	//	|СГРУППИРОВАТЬ ПО
	//	|	ВТ_НоменклатураДокумента.Номенклатура,
	//	|	ВТ_НоменклатураДокумента.Характеристика,
	//	|	ВТ_НоменклатураДокумента.ВидЗапасов
	//	|
	//	|ИНДЕКСИРОВАТЬ ПО
	//	|	Номенклатура,
	//	|	Характеристика,
	...
	//	|ИЗ
	//	|	ВТ_НоменклатураДокумента КАК ВТ_НоменклатураДокумента
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_АктуальныеДатыСреза КАК ВТ_АктуальныеДатыСреза
	//	|		ПО ВТ_НоменклатураДокумента.Номенклатура = ВТ_АктуальныеДатыСреза.Номенклатура
	//	|			И ВТ_НоменклатураДокумента.Характеристика = ВТ_АктуальныеДатыСреза.Характеристика
	//	|			И (ВТ_АктуальныеДатыСреза.ВидЗапасов = ВТ_НоменклатураДокумента.ВидЗапасов)
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступленияТоваровОрганизаций
	//	|		ПО (ВТ_АктуальныеДатыСреза.Номенклатура = ДатыПоступленияТоваровОрганизаций.Номенклатура)
	//	|			И (ВТ_АктуальныеДатыСреза.Характеристика = ДатыПоступленияТоваровОрганизаций.Характеристика)
	//	|			И (ВТ_АктуальныеДатыСреза.ВидЗапасов = ДатыПоступленияТоваровОрганизаций.ВидЗапасов)
	//	|			И (ВТ_АктуальныеДатыСреза.ДатаПоступления = ДатыПоступленияТоваровОрганизаций.ДатаПоступления)";
	// Конец было
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ВТНоменклатураДокумента.ВидЗапасов КАК ВидЗапасов,
	               |	МАКСИМУМ(ДатыПоступленияТоваровОрганизаций.ДатаПоступления) КАК ДатаПоступления
	               |ПОМЕСТИТЬ ВТАктуальныеДатыСреза
	               |ИЗ
	               ...
	               |		ПО ВТНоменклатураДокумента.Номенклатура = ДатыПоступленияТоваровОрганизаций.Номенклатура
	               |			И ВТНоменклатураДокумента.Характеристика = ДатыПоступленияТоваровОрганизаций.Характеристика
	               |			И ВТНоменклатураДокумента.ВидЗапасов = ДатыПоступленияТоваровОрганизаций.ВидЗапасов
	               |ГДЕ
	               |	ДатыПоступленияТоваровОрганизаций.ДатаПоступления <= &ДатаСреза
	               |	И ДатыПоступленияТоваровОрганизаций.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВТНоменклатураДокумента.Номенклатура,
	               |	ВТНоменклатураДокумента.Характеристика,
	               |	ВТНоменклатураДокумента.ВидЗапасов
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Номенклатура,
	               |	Характеристика,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ДатыПоступленияТоваровОрганизаций.Номенклатура КАК Номенклатура,
	               |	ДатыПоступленияТоваровОрганизаций.Характеристика КАК Характеристика,
	               |	МАКСИМУМ(ДатыПоступленияТоваровОрганизаций.НомерГТД) КАК НомерГТД
	               |ИЗ
	               |	ВТАктуальныеДатыСреза КАК ВТАктуальныеДатыСреза
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступленияТоваровОрганизаций
	               |		ПО ВТАктуальныеДатыСреза.Номенклатура = ДатыПоступленияТоваровОрганизаций.Номенклатура
	               |			И ВТАктуальныеДатыСреза.Характеристика = ДатыПоступленияТоваровОрганизаций.Характеристика
	               |			И ВТАктуальныеДатыСреза.ВидЗапасов = ДатыПоступленияТоваровОрганизаций.ВидЗапасов
	               |			И ВТАктуальныеДатыСреза.ДатаПоступления = ДатыПоступленияТоваровОрганизаций.ДатаПоступления
	               |
	               |СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ТоварыКЗаполнению", ТоварыКЗаполнению);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для каждого СтрокаТовары Из МассивСтрок Цикл
			...
			// Задача #4386
			// Начало было
			// СтрокаТовары.НомерГТД = ВыборкаДетальныеЗаписи.НомерГТД;
			// Конец было
			Если Не ЗначениеЗаполнено(СтрокаТовары.НомерГТД) Тогда
				СтрокаТовары.НомерГТД = ВыборкаДетальныеЗаписи.НомерГТД;	
			КонецЕсли;
			// Задача #4386
			// <-- Евлахов Игорь Николаевич (Конец) 12.07.2024			
...
&НаСервере
Функция ПолучитьТоварыКЗаполнению(ТаблицаТоваров, ТаблицаВидыЗапасов)
	
	ВидЗапасовПоУмолчанию = ПолучитьВидЗапасовПоУмолчанию();
	
	ВидыЗапасов = ТаблицаВидыЗапасов.Выгрузить(, "АналитикаУчетаНоменклатуры,ВидЗапасов");
	
	ТоварыКЗаполнению = ТаблицаТоваров.Выгрузить(, "АналитикаУчетаНоменклатуры,Номенклатура,Характеристика");
	ТоварыКЗаполнению.Колонки.Добавить("ВидЗапасов", Новый ОписаниеТипов("СправочникСсылка.ВидыЗапасов"));

	Для каждого СтрокаТовары Из ТоварыКЗаполнению Цикл
		
		СтрокаВидаЗапасов = ВидыЗапасов.Найти(СтрокаТовары.АналитикаУчетаНоменклатуры, "АналитикаУчетаНоменклатуры");
		Если СтрокаВидаЗапасов <> Неопределено Тогда
			СтрокаТовары.ВидЗапасов = СтрокаВидаЗапасов.ВидЗапасов;
		Иначе
			СтрокаТовары.ВидЗапасов = ВидЗапасовПоУмолчанию;
		КонецЕсли;
		
	КонецЦикла;
	
	ТоварыКЗаполнению.Колонки.Удалить("АналитикаУчетаНоменклатуры");	
	ТоварыКЗаполнению.Свернуть("Номенклатура,Характеристика,ВидЗапасов");
...
&НаСервереБезКонтекста
Функция ПолучитьВидЗапасовПоУмолчанию()
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		ВидЗапасовПоУмолчанию = ВыборкаДетальныеЗаписи.ВидЗапасов;
	КонецЕсли;
	
	Возврат ВидЗапасовПоУмолчанию;
