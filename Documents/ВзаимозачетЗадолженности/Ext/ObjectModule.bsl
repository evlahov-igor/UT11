...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВзаимозачетКомиссионный Тогда
		Если ДатаВходящегоДокумента < НачалоДня(Дата)-4*86400
			Или ДатаВходящегоДокумента > НачалоДня(Дата) Тогда
				ТекстСообщения = НСтр("ru = 'Дата платежного документа может быть меньше даты счета-фактуры комиссионера на 5 дней и не может быть больше даты счета-фактуры'");
				ОбщегоНазначения.СообщитьПользователю(
					ТекстСообщения,
					,
					"Элементы.ДатаВходящегоДокумента",
					,
					Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации <> Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Бартер
		...
		Если Приемник.Количество() > 0 Тогда
			...
			Если СуммыВзаиморасчетов <> Неопределено И СуммыВзаиморасчетов.Количество() > 0 Тогда
