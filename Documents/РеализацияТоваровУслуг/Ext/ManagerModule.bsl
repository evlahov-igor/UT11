...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	МеханизмыДокумента.Добавить("КР_ТоварыКОформлениюОптовыхПродаж");
	// >> 22.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// << 22.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
		КР_ТекстЗапросаТаблицаТоварыКОформлениюОптовыхПродаж(Запрос, ТекстыЗапроса, Регистры);
		// >> 22.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	// << 10.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	ДоступноИзменениеТоварыНаСкладах = КР_ПроведениеДокументовПереопределяемый.ДоступноИзменениеШаблоновПроведения(
	...
	Если ДоступноИзменениеТоварыНаСкладах Тогда 	
		...
		|	ВЫБОР 
		|		КОГДА ДанныеДокумента.Склад.КР_СкладСПомещениямиБезордернойОтгрузки
...
Функция КР_ТекстЗапросаТаблицаКоробаНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	ТаблицаТовары.Ссылка.Дата              КАК Период,
	|	ТаблицаТовары.КР_Короб     			   КАК Короб,
	...
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ТаблицаТовары.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
	|	И ТаблицаТовары.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	|	И ТаблицаТовары.Ссылка.Склад.КР_УчетОстатковКоробов";      
...
// << 22.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
Функция КР_ТекстЗапросаТаблицаТоварыКОформлениюОптовыхПродаж(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "КР_ТоварыКОформлениюОптовыхПродаж";
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
	|	&Период КАК Период,
	|	ШапкаДокумента.КР_РозничнаяПродажа КАК ИсточникДанных,
	|	ТаблицаТовары.Номенклатура КАК Номенклатура,
	|	ТаблицаТовары.Характеристика КАК Характеристика,
	|	ШапкаДокумента.КР_ЗаказИнтернетМагазина.КР_НомерЗаказаИнтернетМагазина КАК НомерЗаказаИнтернетМагазина,
	|	&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ТаблицаТовары.Количество КАК Количество,
	...
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ШапкаДокумента.КР_РозничнаяПродажа <> ЗНАЧЕНИЕ(Документ.ОтчетОРозничныхПродажах.ПустаяСсылка)
	|	И ТаблицаТовары.Количество > 0";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса, ИмяРегистра);
	Возврат ТекстЗапроса;

КонецФункции // >> 22.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-725
