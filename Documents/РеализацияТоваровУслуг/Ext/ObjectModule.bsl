...
Процедура ЗаполнитьУсловияПродаж(Знач УсловияПродаж) Экспорт
	...
	Если УсловияПродаж.ИспользуютсяДоговорыКонтрагентов <> Неопределено И УсловияПродаж.ИспользуютсяДоговорыКонтрагентов Тогда
		...
		КонецЕсли;
		ВариантВыбытияМаркируемойПродукции = Договор.ВариантВыбытияМаркируемойПродукции;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет Тогда
			...
			ПараметрыВзаиморасчетовУпр = ОбщегоНазначения.СкопироватьРекурсивно(ДополненныеПараметры.МассивПараметров[0]); 
			ПараметрыВзаиморасчетовУпр.Организация = Справочники.Организации.УправленческаяОрганизация;
			ПараметрыВзаиморасчетовУпр.Договор = "";
			
			Если ЭтотОбъект.ЭтоНовый() Тогда
				СсылкаНового = ЭтотОбъект.ПолучитьСсылкуНового();
				Если Не ЗначениеЗаполнено(СсылкаНового) Тогда
					СсылкаНового = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ЭтотОбъект.Ссылка).ПолучитьСсылку();
					ЭтотОбъект.УстановитьСсылкуНового(СсылкаНового);
				КонецЕсли;
				ПараметрыВзаиморасчетовУпр.Вставить("СсылкаНового", СсылкаНового);
			КонецЕсли;
			
			РеквизитыОбъектаРасчетов = ОбъектыРасчетовСервер.РеквизитыОбъектаРасчетовПоСтруктуре(ЭтотОбъект,Ссылка,ПараметрыВзаиморасчетовУпр, РежимЗаписи);
			...
			Для Каждого Стр Из РасшифровкаПлатежа Цикл
				Если НЕ ЗначениеЗаполнено(Стр.ОбъектРасчетов) Тогда
					Стр.ОбъектРасчетов = ОбъектРасчетовУпр;
				КонецЕсли;
			КонецЦикла;
			
		ИначеЕсли Организация <> Справочники.Организации.УправленческаяОрганизация Тогда
			ОбъектРасчетовУпр = ОбъектыРасчетовСервер.ПолучитьОбъектРасчетовПоСсылке(Ссылка, 
			...
			Если ЗначениеЗаполнено(ОбъектРасчетовУпр) Тогда
				ОбъектыРасчетовСервер.ПроверитьУдалитьОбъектРасчетов(ОбъектРасчетовУпр, Отказ, Истина);
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьДаннымиПоПереданнымНаКомиссиюТоварам(Знач ДанныеЗаполнения)
	...
	Если ЗначениеЗаполнено(Договор) Тогда
		...
		//Если ЗначениеЗаполнено(Договор.ВидЦенПродажи) Тогда
		//	ЦенаВключаетНДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор.ВидЦенПродажи, "ЦенаВключаетНДС");
		//КонецЕсли;
	КонецЕсли;
	ПараметрыЗаполнения = Документы.РеализацияТоваровУслуг.ПараметрыЗаполненияНалогообложенияНДСПродажи(ЭтотОбъект);
	УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(НалогообложениеНДС, ПараметрыЗаполнения);
...
Процедура КР_ПриКопировании(ОбъектКопирования)
 	...
 	КР_РозничнаяПродажа = Неопределено;
 	КР_ЗаказИнтернетМагазина = Неопределено;
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Склад.КР_СкладСПомещениямиБезордернойОтгрузки Тогда  
		МассивНепроверяемыхРеквизитов.Добавить("КР_Помещение");
	КонецЕсли;	
