...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-725
...
Процедура ПриОткрытии(Отказ)
	...
	// Задача #4320
	злОбновитьДоступностьВидимостьЭлементов();
...
Процедура ДоговорПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт 
	...
	Если НЕ РезультатВопроса = КодВозвратаДиалога.Отмена И РезультатВопроса Тогда
		...
		Если ЦеныПредприятияЗаполнениеКлиент.НеобходимоЗаполнениеЦенПоВиду(Объект, ВидЦеныДоговора) Тогда
			...
			ЦеныРассчитаны = ЗаполнитьЦеныПоСоглашениюСервер(Истина, Ложь);
			ПродажиКлиент.ОповеститьОбОкончанииЗаполненияЦенПоСоглашению(ЦеныРассчитаны);
...
Процедура ЗагрузитьИзВнешнегоФайлаЗавершение(АдресЗагруженныхДанных, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(АдресЗагруженныхДанных) Тогда
		...
		ОценкаПроизводительностиКлиент.ЗамерВремени(
			"Документ.РеализацияТоваровУслуг.ФормаДокумента.Команда.ЗагрузитьИзВнешнегоФайла");
		// <-- Евлахов Игорь Николаевич (Конец) 23.08.2023
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ДанныеПоФоновомуЗаданию = ДоставкаТоваров.ПриЧтенииСозданииРаспоряженийНаСервере(Элементы, Объект,
		Объект.РеализацияПоЗаказам Или РеализацияЧерезКомиссионера,,,Истина);
...
Процедура СкладПриИзмененииСервер(ОчищатьСтроки = Истина, ОтвязатьВсеСтрокиОтЗаказа = Ложь)
	...
	КР_УстановитьВидимостьПомещенияИКороба();
...
Процедура РассчитатьСкидкиБезПримененияКОбъекту()
	...
	СтруктураПараметры.УправляемыеСкидки				 = УправляемыеСкидки.ВыгрузитьЗначения();
...
Функция ПредварительноРассчитанныеСкидкиНаценкиПоЗаказу()
	...
	СкидкиНаценкиЗаказа = ДанныеСкидкиНаценкиПоЗаказу.СкидкиНаценкиЗаказа;
	ТаблицаТоваровЗаказа = ДанныеСкидкиНаценкиПоЗаказу.ТаблицаТоваров;
	Для Каждого СтрокаТоваровЗаказа Из ТаблицаТоваровЗаказа Цикл
		...
		Если СтрокаТаблицы.КлючСвязи <> 0 Тогда
			...
			НайденныеСтроки = СкидкиНаценкиЗаказа.НайтиСтроки(
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		СписокСвойств = "НоменклатураНабора, ХарактеристикаНабора, Номенклатура, Характеристика, Упаковка, ВидЦены,
			| Цена, КоличествоУпаковок, СрокПоставки, ПроцентРучнойСкидки, Серия, Обособленно";
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	Элементы.СтраницаДоставка.Видимость = Не РеализацияЧерезКомиссионера;
...
Процедура РассчитатьИтоговыеПоказатели()
	...
	// << 29.03.2023, Федоров Д.Е., КРОК, JIRA№A2105505-1469
	ЭтотОбъект["КР_КоличествоТоварыИтого"] = Объект.Товары.Итог("Количество");
	// >> 29.03.2023, Федоров Д.Е., КРОК, JIRA№A2105505-1469
...
Процедура КлиентДоговорПриИзмененииНаСервере()
	...
	ВзаиморасчетыСервер.ФормаПриИзмененииРеквизитов(ЭтаФорма, "КлиентДоговор");
...
&НаКлиенте
Процедура Подключаемый_злЗакрывающийДокументУПДПриИзменении(Элемент)
	
	злОбновитьДоступностьВидимостьЭлементов();	
	злОбработатьЗакрывающийДокументНомерСчетФактуры();
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_злЗакрывающийДокументНомерПриИзменении(Элемент)
...
&НаКлиенте
Процедура злОбработатьЗакрывающийДокументНомерСчетФактуры()

	Если Объект.злЗакрывающийДокументУПД Тогда
		Объект.злЗакрывающийДокументНомерСчетФактуры = Объект.злЗакрывающийДокументНомер;	
	КонецЕсли;
			
КонецПроцедуры

&НаКлиенте
Процедура злОбновитьДоступностьВидимостьЭлементов()

	Если Объект.злЗакрывающийДокументУПД Тогда
		Элементы["злЗакрывающийДокументНомерСчетФактуры"].ТолькоПросмотр = Истина;
	Иначе
		Элементы["злЗакрывающийДокументНомерСчетФактуры"].ТолькоПросмотр = Ложь;
	КонецЕсли;
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_РозничнаяПродажа", Элементы.ГруппаПараметрыПраво);
	...
	// КР_Помещение
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Помещение", Элементы.ГруппаСклад);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 29;
	
	// Изменим группировку группы склад на вертикальную (как в типовых документах)
	...
	// --> Евлахов Игорь Николаевич (Начало) 10.07.2024
	// Задача #4320
	ГруппаРеквизитыЗакрывающихДокументов = Элементы.Вставить("ГруппаРеквизитыЗакрывающихДокументов", Тип("ГруппаФормы"), Элементы.ГруппаШапкаЛево);
	ГруппаРеквизитыЗакрывающихДокументов.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаРеквизитыЗакрывающихДокументов.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
	ГруппаРеквизитыЗакрывающихДокументов.Заголовок = НСтр("ru = 'Реквизиты закрывающих документов'"); 
	ГруппаРеквизитыЗакрывающихДокументов.ОтображениеУправления = ОтображениеУправленияОбычнойГруппы.Картинка;
	ГруппаРеквизитыЗакрывающихДокументов.Отображение = ОтображениеОбычнойГруппы.Нет;
	ГруппаРеквизитыЗакрывающихДокументов.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаРеквизитыЗакрывающихДокументов.СквозноеВыравнивание = СквозноеВыравнивание.Использовать;
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злЗакрывающийДокументНомер", ГруппаРеквизитыЗакрывающихДокументов);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 29;
	ЭлементФормы.УстановитьДействие("ПриИзменении", "Подключаемый_злЗакрывающийДокументНомерПриИзменении");
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злЗакрывающийДокументНомерСчетФактуры", ГруппаРеквизитыЗакрывающихДокументов);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 29;
	
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злЗакрывающийДокументДата", ГруппаРеквизитыЗакрывающихДокументов);
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злЗакрывающийДокументУПД", ГруппаРеквизитыЗакрывающихДокументов);
	ЭлементФормы.УстановитьДействие("ПриИзменении", "Подключаемый_злЗакрывающийДокументУПДПриИзменении");
	// Задача #4320
	// <-- Евлахов Игорь Николаевич (Конец) 10.07.2024
	
КонецПроцедуры // >> 28.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-725
...
// << 10.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
&НаСервере
Процедура КР_УстановитьВидимостьПомещенияИКороба()
	...
	УчетОстатковКоробов = Склад.КР_УчетОстатковКоробов;
	Элементы.ТоварыКР_Короб.Видимость = УчетОстатковКоробов;
