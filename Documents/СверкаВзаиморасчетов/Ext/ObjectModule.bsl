...
Процедура УстановитьСвойстваИзмененияРеквизитов(Объект, ДополнительныеСвойства)
	...
	Если Объект.Проведен Тогда
		...
		Если ИзмененияДокумента.Свойство("Реквизиты") Тогда
			...
			Для каждого Реквизит Из ИзмененияДокумента.Реквизиты Цикл
				Если Реквизит.Имя <> "Статус" Тогда
					ИзменилисьДругиеРеквизиты = Истина;
					Прервать;
				ИначеЕсли Реквизит.Имя = "Статус" Тогда
					Если	(Реквизит.НовоеЗначение = Перечисления.СтатусыСверокВзаиморасчетов.Создана
					 		ИЛИ Реквизит.СтароеЗначение = Перечисления.СтатусыСверокВзаиморасчетов.Создана) Тогда
						ИзменилсяСтатусСоздана = Истина;
						Прервать;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		...
		ИзменилсяТолькоСтатусСвереноНаСверке = ИзменилсяСтатус И Не (ИзменилисьДругиеРеквизиты Или ИзменилсяСтатусСоздана);
...
Процедура УдалитьЗависшиеПодчиенныеСтроки(тчГруппировки, тчДетальныеЗаписи, ПолеСвязи)
	...
	ОбъектыГруппировки = ЭтотОбъект[тчГруппировки].Выгрузить(); //ТаблицаЗначений
	ОбъектыГруппировки.Свернуть(ПолеСвязи);
	ОбъектыГруппировки.Индексы.Добавить(ПолеСвязи);
	
	ОбъектыДетальныеЗаписи = ЭтотОбъект[тчДетальныеЗаписи].Выгрузить(); //ТаблицаЗначений
	...
	Для Каждого Запись Из ОбъектыДетальныеЗаписи Цикл
		...
		Если Владелец = Неопределено Тогда
			...
			НайденныеСтроки = ЭтотОбъект[тчДетальныеЗаписи].НайтиСтроки(Отбор);
			Для Каждого Строка Из НайденныеСтроки Цикл
				ЭтотОбъект[тчДетальныеЗаписи].Удалить(Строка);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
