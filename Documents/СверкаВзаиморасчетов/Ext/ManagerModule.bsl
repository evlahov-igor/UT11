...
Функция ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	|				(ТИП(Документ.ВыкупВозвратнойТарыУПоставщика),
	|				ТИП(Документ.ВыкупВозвратнойТарыКлиентом))
	|			ТОГДА ЕСТЬNULL(ВЫБОР КОГДА РеестрДокументов.ДатаПервичногоДокумента = ДАТАВРЕМЯ(1,1,1) 
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "Партнер");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ГруппировкиРасчеты.УдалитьОбъектРасчетов");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ГруппировкиРасчеты.Партнер");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ДетальныеЗаписиРасчеты.Партнер");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ДетальныеЗаписиРасчеты.УдалитьОбъектРасчетов");
	...
	|			ГДЕ
	|				ТЧДетальныеЗаписиРасчеты.Ссылка = СверкаВзаиморасчетов.Ссылка
	|				И ТЧДетальныеЗаписиРасчеты.ОбъектРасчетов.Договор <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|				И ТЧДетальныеЗаписиРасчеты.ОбъектРасчетов.Договор <> НЕОПРЕДЕЛЕНО
	|				И ТЧДетальныеЗаписиРасчеты.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)))";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	#Область ЗаполнениеОбъектаРасчетов
	Запрос = Новый Запрос();
	...
	|			ПО ТЧГруппировкиРасчеты.Договор = Договоры.Ссылка
	...
	|
	|				И (ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(ТЧГруппировкиРасчеты.УдалитьОбъектРасчетов) = ТИП(Справочник.ДоговорыКонтрагентов)
	|							И Договоры.РазрешенаРаботаСДочернимиПартнерами
	|						ТОГДА Договоры.Партнер
	|					ИНАЧЕ ТЧГруппировкиРасчеты.Партнер
	...
	|				И ТЧГруппировкиРасчеты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|ГДЕ
	|	НЕ ТЧГруппировкиРасчеты.Ссылка ЕСТЬ NULL
	|ИТОГИ
	|ПО
	|	Ссылка,
	|	ВерсияДанных";
			...
			Попытка
					...
					Если Не НомерСтроки = Null Тогда
						...
						ИначеЕсли ЗначениеЗаполнено(СтрокаТЧ.Партнер) 
							...
							Если ОбъектыРасчетовСервер.ОбъектГотовКЗаполнению(
								...
								СтрокаТЧ.УдалитьОбъектРасчетов,
				...
				Для Каждого ДетальнаяЗаписьРасчеты Из ДокументОбъект.ДетальныеЗаписиРасчеты Цикл
					Если Не ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.ОбъектРасчетов) Тогда
						...
						Если ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.Партнер) Тогда
							СтруктураПоиска.Вставить("Партнер", ДетальнаяЗаписьРасчеты.Партнер);
						КонецЕсли;
						
						Если ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.ТипРасчетов) Тогда
							...
							Иначе
								...
								Иначе
									Если ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов) Тогда
										...
										ДопКритерииПоиска.Партнер = ?(ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.Партнер), ДетальнаяЗаписьРасчеты.Партнер, Неопределено);
										ДопКритерииПоиска.ОбновлениеИБ = Истина;
										ДопКритерииПоиска.ВернутьПервый = Ложь;
										ДопКритерииПоиска.НеГенерироватьИсключение = Истина;
										
										Если ПоискДополнительныйБезВалюты.Количество() <> 1 
											И ДопКритерииПоиска.Партнер = Неопределено Тогда
											ДопКритерииПоиска.Вставить("ТолькоОстатки", Ложь);
										КонецЕсли;
										...
										ИначеЕсли ДокументОбъект.ГруппировкиРасчеты.Количество() 
												И Не ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.Партнер) 
												И Не ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.ТипРасчетов) Тогда
		
											ДопКритерииПоиска = ОбъектыРасчетовСервер.ДополнительныеКритерииПоиска();
											ДопКритерииПоиска.ОбновлениеИБ = Истина;
											ДопКритерииПоиска.ВернутьПервый = Истина;
											ДопКритерииПоиска.НеГенерироватьИсключение = Истина;
											...
											Иначе
												Если ОбъектыРасчетовСервер.ОбъектГотовКЗаполнению(
													ДетальнаяЗаписьРасчеты.ОбъектРасчетов,
													ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов,
													ГотовКЗаполнению) Тогда
													ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
															НСтр("ru = 'Не удалось заполнить объект расчетов в документе: %1, табличная часть ДетальнаяЗаписьРасчеты, строка №%2, 
															|по источнику данных %3"),
																Ссылка,
																ДетальнаяЗаписьРасчеты.НомерСтроки,
																ДетальнаяЗаписьРасчеты.УдалитьОбъектРасчетов);
											...
											КонецЕсли;
										ИначеЕсли ДокументОбъект.ГруппировкиРасчеты.Количество() 
											И (ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.Партнер) 
												Или ЗначениеЗаполнено(ДетальнаяЗаписьРасчеты.ТипРасчетов)) Тогда
	...
	#Область ЗаполнениеДоговораВТЧДетальныеЗаписиРасчеты
	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОбновляемыеДанные.Ссылка
	...
	|	ОбновляемыеДанные.Ссылка КАК Ссылка,
	|	СверкаВзаиморасчетов.ВерсияДанных  КАК ВерсияДанных,
	|	ТЧДетальныеЗаписиРасчеты.НомерСтроки  КАК НомерСтроки,
	|	ТЧДетальныеЗаписиРасчеты.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ТЧДетальныеЗаписиРасчеты.ОбъектРасчетов.Договор КАК Договор
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов.ДетальныеЗаписиРасчеты КАК ТЧДетальныеЗаписиРасчеты
	|			ПО ОбновляемыеДанные.Ссылка = ТЧДетальныеЗаписиРасчеты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СверкаВзаиморасчетов КАК СверкаВзаиморасчетов
	|			ПО ОбновляемыеДанные.Ссылка = СверкаВзаиморасчетов.Ссылка
	|ГДЕ
	|	ТЧДетальныеЗаписиРасчеты.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|	
	|ИТОГИ
	|	МАКСИМУМ(ВерсияДанных)
	|ПО
	|	Ссылка";
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Попытка
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				СтрокаТЧ = ДокументОбъект.ДетальныеЗаписиРасчеты[ВыборкаДетальныеЗаписи.НомерСтроки-1];
				Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Договор) И ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Договор) Тогда
