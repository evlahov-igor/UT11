...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента")
		...
		И ЗначениеЗаполнено(Договор) И Не Договор.КомиссионныеПродажи25 Тогда
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
		...
		Если Не Отказ Тогда
			...
			// << 11.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1950
			КР_ЗаполнитьУказанныеГТДВВидахЗапасов();
			// >> 11.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1950
...
Процедура ПроверитьЗаполнениеНомеровГТДВВидахЗапасов(Отказ)
	...
	Если Не УчетПрослеживаемыхТоваровЛокализация.ЭтоПрослеживаемыйДокумент(Товары, Дата)
		И Не ПолучитьФункциональнуюОпцию("ЗапретитьПоступлениеТоваровБезНомеровГТД") Тогда
		Возврат;
	КонецЕсли;
	...
	|	И ВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	|СГРУППИРОВАТЬ ПО
	|	ВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура,
	|	ВидыЗапасов.АналитикаУчетаНоменклатуры.Характеристика
	|";
	
	Запрос.УстановитьПараметр("ВидыЗапасов", ВидыЗапасов.Выгрузить(, "АналитикаУчетаНоменклатуры, НомерГТД"));
...
// << 11.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1950
Процедура КР_ЗаполнитьУказанныеГТДВВидахЗапасов()

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Т.НомерСтроки - 1 КАК ИндексСтроки,
	|	Т.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры, Т.НомерГТД
	...
	|ВЫБРАТЬ
	|	Т.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	МАКСИМУМ(Т.НомерГТД) КАК Ссылка
	|ПОМЕСТИТЬ НомераГТД
	|ИЗ
	|	Товары КАК Т
	|
	...
	|		ПО Т.АналитикаУчетаНоменклатуры = НомераГТД.АналитикаУчетаНоменклатуры
	|ГДЕ
	|	Т.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|	И Т.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД");   
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		...
		СтрокаДанных = ВидыЗапасов[ВыборкаДетальныеЗаписи.ИндексСтроки];
		СтрокаДанных.НомерГТД = ВыборкаДетальныеЗаписи.НомерГТД;
		
	КонецЦикла;	
	
КонецПроцедуры // >> 11.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-1950
