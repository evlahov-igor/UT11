...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействий(Данные.ХозяйственнаяОперация);
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.КорТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	...
	|			КОНЕЦ
	|	КОНЕЦ                  КАК РазделУчета,
	|	ВидыЗапасов.КорВидЗапасов КАК ВидЗапасов,
	|	ВидыЗапасов.Количество КАК Количество,
	|	0                      КАК Стоимость,
	|	0                      КАК КорСтоимость,
	|	0                      КАК СтоимостьРегл,
...
Процедура ЗапланироватьПоступлениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ИсточникДанных.Ссылка.ХозяйственнаяОперация =
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|	ТабЧасть.Ссылка.ПоРаспоряжениям
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'В поступлениях товаров от хранителя:
	|	- В шапке:
	|		- Устанавливает флаг ""По распоряжениям"".
	|		- Устанавливает пустые значения ""Распоряжение"" в Неопределено
	|	- В табличной части ""Товары"":
	|		- Заполняет ""Номер ГТД"". В случае если одной строке из табличной части ""Товары"" соответствует несколько строк табличной части ""Виды запасов"", то разбивает строки в табличной части ""Товары"".
	|		- Заполняет ""Распоряжение"".
	|	- В табличной части ""Виды запасов"":
	|		- Заполняет ""Кор. вид запасов"".
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Заполнение кор. вида запасов
	|	ВЫБРАТЬ
	|		ПоступлениеТоваровОтХранителя.Ссылка КАК Ссылка
	|	ИЗ
	|		Документ.ПоступлениеТоваровОтХранителя КАК ПоступлениеТоваровОтХранителя
	|	ГДЕ
	|		ПоступлениеТоваровОтХранителя.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтХранителя)
	|		И ПоступлениеТоваровОтХранителя.Проведен
	|		И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.ПоступлениеТоваровОтХранителя.ВидыЗапасов КАК ТабличнаяЧастьВидыЗапасов
	|			ГДЕ
	|				ТабличнаяЧастьВидыЗапасов.Ссылка = ПоступлениеТоваровОтХранителя.Ссылка
	|				И ТабличнаяЧастьВидыЗапасов.ВидЗапасов <> ТабличнаяЧастьВидыЗапасов.КорВидЗапасов)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Установка флага По распоряжениям
	|	ВЫБРАТЬ
	|		ПоступлениеТоваровОтХранителя.Ссылка КАК Ссылка
	|	ИЗ
	|		Документ.ПоступлениеТоваровОтХранителя КАК ПоступлениеТоваровОтХранителя
	|	ГДЕ
	|		НЕ ПоступлениеТоваровОтХранителя.ПоРаспоряжениям
	|		И НЕ ПоступлениеТоваровОтХранителя.Распоряжение В
	|			(ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
	|			НЕОПРЕДЕЛЕНО)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Заполнение Распоряжение в таблице Товары
	|	ВЫБРАТЬ
	|		ПоступлениеТоваровОтХранителя.Ссылка КАК Ссылка
	|	ИЗ
	|		Документ.ПоступлениеТоваровОтХранителя.Товары КАК ПоступлениеТоваровОтХранителя
	|	ГДЕ
	|		НЕ ПоступлениеТоваровОтХранителя.Ссылка.Распоряжение В
	|			(ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
	|			НЕОПРЕДЕЛЕНО)
	|		И ПоступлениеТоваровОтХранителя.Распоряжение В
	|			(ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
	|			НЕОПРЕДЕЛЕНО)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Установка пустых значений Распоряжения в шапке документа
	|	ВЫБРАТЬ
	|		ПоступлениеТоваровОтХранителя.Ссылка КАК Ссылка
	|	ИЗ
	|		Документ.ПоступлениеТоваровОтХранителя КАК ПоступлениеТоваровОтХранителя
	|	ГДЕ
	|		ПоступлениеТоваровОтХранителя.Распоряжение В
	|			(
	|			ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка)
	|			)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Установка пустых значений Распоряжения в таблице Товары
	|	ВЫБРАТЬ
	|		ПоступлениеТоваровОтХранителя.Ссылка КАК Ссылка
	|	ИЗ
	|		Документ.ПоступлениеТоваровОтХранителя.Товары КАК ПоступлениеТоваровОтХранителя
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// Заполнение кор. вида запасов
	|ВЫБРАТЬ
	|	&ДействиеУстановитьЗначениеКорВидЗапасов      КАК Действие,
	|	ТабличнаяЧастьВидыЗапасов.НомерСтроки         КАК НомерСтроки,
	|	ТабличнаяЧастьВидыЗапасов.Ссылка              КАК Ссылка,
	|	ТабличнаяЧастьВидыЗапасов.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	NULL                                          КАК АналитикаУчетаНоменклатуры,
	|	NULL                                          КАК АналитикаУчетаНаборов,
	|	NULL                                          КАК УдалитьСтавкаНДС,
	|	NULL                                          КАК НомерГТД,
	...
	|	&ВТДокументыДляОбработки КАК ДокументДляОбработки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровОтХранителя.ВидыЗапасов КАК ТабличнаяЧастьВидыЗапасов
	|	ПО ДокументДляОбработки.Ссылка = ТабличнаяЧастьВидыЗапасов.Ссылка
	|
	|ГДЕ
	|	ТабличнаяЧастьВидыЗапасов.Ссылка.Проведен
	|	И ТабличнаяЧастьВидыЗапасов.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтХранителя)
	|	И ТабличнаяЧастьВидыЗапасов.ВидЗапасов <> ТабличнаяЧастьВидыЗапасов.КорВидЗапасов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Установка флага По распоряжениям
	|ВЫБРАТЬ
	|	&ДействиеУстановитьФлагПоРаспоряжениям  КАК Действие,
	|	0                                       КАК НомерСтроки,
	|	ДокументПоступления.Ссылка              КАК Ссылка,
	|	ДокументПоступления.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	NULL                                    КАК АналитикаУчетаНоменклатуры,
	...
	|
	|ГДЕ
	|	НЕ ДокументПоступления.ПоРаспоряжениям
	|	И НЕ ДокументПоступления.Распоряжение В
	|		(ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
	|		НЕОПРЕДЕЛЕНО)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	// Заполнение Распоряжение в таблице Товары
	|ВЫБРАТЬ
	|	&ДействиеЗаполнитьРаспоряжениеТЧ        КАК Действие,
	|	ДокументПоступления.НомерСтроки         КАК НомерСтроки,
	|	ДокументПоступления.Ссылка              КАК Ссылка,
	|	ДокументПоступления.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	NULL                                    КАК АналитикаУчетаНоменклатуры,
	|	NULL                                    КАК АналитикаУчетаНаборов,
	...
	|	&ВТДокументыДляОбработки КАК ДокументДляОбработки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровОтХранителя.Товары КАК ДокументПоступления
	|	ПО ДокументДляОбработки.Ссылка = ДокументПоступления.Ссылка
	|
	|ГДЕ
	|	НЕ ДокументПоступления.Ссылка.Распоряжение В
	|		(ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
	|		НЕОПРЕДЕЛЕНО)
	|	И ДокументПоступления.Распоряжение В
	|		(ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка),
	|		НЕОПРЕДЕЛЕНО)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение пустого значения Распоряжение в шапке документа
	|ВЫБРАТЬ
	|	&ДействиеУстановитьРаспоряжениеШапкиВНеопределено КАК Действие,
	|	0                                       КАК НомерСтроки,
	|	ДокументПоступления.Ссылка              КАК Ссылка,
	|	ДокументПоступления.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	NULL                                    КАК АналитикаУчетаНоменклатуры,
	...
	|
	|ГДЕ
	|	ДокументПоступления.Распоряжение В
	|		(
	|		ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка)
	|		)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение пустого значения Распоряжение в ТЧ документа
	|ВЫБРАТЬ
	|	&ДействиеУстановитьРаспоряжениеТЧВНеопределено КАК Действие,
	|	ДокументПоступления.НомерСтроки         КАК НомерСтроки,
	|	ДокументПоступления.Ссылка              КАК Ссылка,
	|	ДокументПоступления.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	NULL                                    КАК АналитикаУчетаНоменклатуры,
	...
	ДействиеПоУмолчанию                     = "";
	ДействиеУстановитьЗначениеКорВидЗапасов = "Установить значение КорВидЗапасов";
	ДействиеУстановитьФлагПоРаспоряжениям   = "Установить флаг По распоряжениям";
	ДействиеЗаполнитьРаспоряжениеТЧ         = "Заполнить Распоряжение ТЧ";
	ДействиеУстановитьРаспоряжениеШапкиВНеопределено = "Установить Распоряжение шапки в Неопределено";
	ДействиеУстановитьРаспоряжениеТЧВНеопределено    = "Установить Распоряжение ТЧ в Неопределено";
	
	Запрос.УстановитьПараметр("ДействиеПоУмолчанию",                     ДействиеПоУмолчанию);
	Запрос.УстановитьПараметр("ДействиеУстановитьЗначениеКорВидЗапасов", ДействиеУстановитьЗначениеКорВидЗапасов);
	Запрос.УстановитьПараметр("ДействиеУстановитьФлагПоРаспоряжениям",   ДействиеУстановитьФлагПоРаспоряжениям);
	Запрос.УстановитьПараметр("ДействиеЗаполнитьРаспоряжениеТЧ",         ДействиеЗаполнитьРаспоряжениеТЧ);
	Запрос.УстановитьПараметр("ДействиеУстановитьРаспоряжениеШапкиВНеопределено", ДействиеУстановитьРаспоряжениеШапкиВНеопределено);
	Запрос.УстановитьПараметр("ДействиеУстановитьРаспоряжениеТЧВНеопределено",    ДействиеУстановитьРаспоряжениеТЧВНеопределено);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Попытка
			...
			ВыборкаПоДействиям = ВыборкаПоДокументам.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаПоДействиям.Следующий() Цикл
			
				Если ВыборкаПоДействиям.Действие = ДействиеПоУмолчанию Тогда
			
					ВыборкаПоТоварам = ВыборкаПоДействиям.Выбрать();
					...
					Пока ВыборкаПоТоварам.Следующий() Цикл
						...
						ОтборПоТоварам = Новый Структура("АналитикаУчетаНоменклатуры, АналитикаУчетаНаборов, НомерГТД");
						...
						Для Каждого СтрокаТЧ Из СтрокиТоваров Цикл
							...
							Иначе
								...
								ЗаполняемыеКолонки = Новый Структура("Сумма");
						
								НоваяСтрока = ДокументОбъект.Товары.Добавить();
								ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЧ);
								
								Если ЗначениеЗаполнено(ДокументОбъект.Распоряжение) Тогда
									НоваяСтрока.Распоряжение = ДокументОбъект.Распоряжение;
								Иначе
				...
				ИначеЕсли ВыборкаПоДействиям.Действие = ДействиеУстановитьЗначениеКорВидЗапасов Тогда
					
					ТабличнаяЧастьВидыЗапасов = ДокументОбъект.ВидыЗапасов;
					
					ВыборкаПоТоварам = ВыборкаПоДействиям.Выбрать();
					Пока ВыборкаПоТоварам.Следующий() Цикл
						СтрокаТабличнойЧасти = ТабличнаяЧастьВидыЗапасов[ВыборкаПоТоварам.НомерСтроки - 1];
						СтрокаТабличнойЧасти.КорВидЗапасов = СтрокаТабличнойЧасти.ВидЗапасов;
					КонецЦикла;
					
				ИначеЕсли ВыборкаПоДействиям.Действие = ДействиеУстановитьФлагПоРаспоряжениям Тогда
					
					ДокументОбъект.ПоРаспоряжениям = ЗначениеЗаполнено(ДокументОбъект.Распоряжение);
					
				ИначеЕсли ВыборкаПоДействиям.Действие = ДействиеЗаполнитьРаспоряжениеТЧ Тогда
					
					ВыборкаПоТоварам = ВыборкаПоДействиям.Выбрать();
					Пока ВыборкаПоТоварам.Следующий() Цикл
						ДокументОбъект.Товары[ВыборкаПоТоварам.НомерСтроки - 1].Распоряжение = ДокументОбъект.Распоряжение;
					КонецЦикла;
					
				ИначеЕсли ВыборкаПоДействиям.Действие = ДействиеУстановитьРаспоряжениеШапкиВНеопределено Тогда
					ДокументОбъект.Распоряжение = Неопределено;
				ИначеЕсли ВыборкаПоДействиям.Действие = ДействиеУстановитьРаспоряжениеТЧВНеопределено Тогда
					ВыборкаПоТоварам = ВыборкаПоДействиям.Выбрать();
					Пока ВыборкаПоТоварам.Следующий() Цикл
						ДокументОбъект.Товары[ВыборкаПоТоварам.НомерСтроки - 1].Распоряжение = Неопределено;
					КонецЦикла;
				КонецЕсли;
