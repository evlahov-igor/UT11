...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора)
	...
	ИначеЕсли НоменклатураКлиент.ЭтоУказаниеНомераГТД(ИсточникВыбора) Тогда
		...
		НоменклатураКлиент.ОбработатьУказаниеНомераГТД(ЭтотОбъект, РезультатВыбора);
...
Процедура ПодобратьТовары(Команда)
	...
	ИначеЕсли ХозяйственныеОперацииРаздельнойЗакупки.Найти(Объект.ХозяйственнаяОперация) <> Неопределено Тогда
		ОтборПоТипуНоменклатуры = Новый ФиксированныйМассив(НоменклатураКлиентСервер.ОтборПоТоваруМногооборотнойТареУслуге(Ложь));
...
Процедура ЗаполнитьНомераГТД(Команда)
	...
	ОткрытьФорму("Справочник.НомераГТД.ФормаВыбора",,,,,, Новый ОписаниеОповещения("ЗаполнитьНомераГТДЗавершение", ЭтотОбъект, Новый Структура("ЗаполненыНомераГТД", Неопределено)), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗагрузитьИзВнешнегоФайлаЗавершение(АдресЗагруженныхДанных, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(АдресЗагруженныхДанных) Тогда
		...
		ОценкаПроизводительностиКлиент.ЗамерВремени(
			"Документ.ПриобретениеТоваровУслуг.ФормаДокумента.Команда.ЗагрузитьИзВнешнегоФайла");
		// <-- Евлахов Игорь Николаевич (Конец) 23.08.2023
...
Процедура УстановитьУсловноеОформлениеДляСтатейИАналитик()
	...
	ГруппаОтбораИЛИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
		...
		ГруппаОтбораИ3.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
			...
			СписокЗначений.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭСФактуровкаПоставки"));
			СписокЗначений.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаФактуровкаПоставки"));
			ОтборЭлемента.ПравоеЗначение = СписокЗначений;
...
Процедура УстановитьПараметрыСтруктурыДействийПриПоискеПоШтрихкоду(СтруктураДействий, ХозяйственнаяОперация)
	...
	Если Не СписокОперацийБезРабот.НайтиПоЗначению(ХозяйственнаяОперация) = Неопределено 
		ИЛИ ХозяйственныеОперацииРаздельнойЗакупки.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
...
Процедура ПолучитьТоварыИзХранилища(АдресТоваровВХранилище)
	...
	СтруктураДействийСПересчетомСумм.Вставить("ЗаполнитьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект));
	...
	СтруктураДействийБезПересчетаСумм.Вставить("ЗаполнитьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект));
...
Функция ПривязатьСтрокиПоступленияКЗаказамПоставщикам(ОтобратьПоЗаказу)
	...
	Если Объект.Договор.Пустая() И Объект.Соглашение.Пустая() Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ЗаказПоставщику.ПорядокРасчетов = &ПорядокРасчетов", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ЗаказыДвижения.ЗаказПоставщику.ПорядокРасчетов = &ПорядокРасчетов", "");
	КонецЕсли;
...
Процедура ЗаполнитьНомераГТДПоПоследнимПоступлениямСервер(ЗаполненыНомераГТД)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, Выборка, "НомерГТД, СтранаПроисхождения");
	КонецЦикла;
...
Процедура КР_ПриЧтенииСозданииНаСервереДополнительно()
	...
	КР_ДобавлениеЭлементовНаФорму();
	
КонецПроцедуры // >> 14.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-742

// << 18.01.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1015
&НаСервере
Процедура КР_ДобавлениеЭлементовНаФорму()
	
	КР_ГруппаВесБрутто = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "КР_ГруппаВесБрутто", Элементы.ГруппаИтого, Элементы.ГруппаСтраницыНДС);
	КР_ГруппаВесБрутто.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	КР_ГруппаВесБрутто.Отображение = ОтображениеОбычнойГруппы.Нет;
	КР_ГруппаВесБрутто.ОтображатьЗаголовок = Ложь;
	
	КР_ВесБруттоШапкаЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ВесБруттоДокумента", КР_ГруппаВесБрутто);
	КР_ВесБруттоШапкаЭлемент.Заголовок = НСтр("ru = 'Вес, брутто'");
	КР_ВесБруттоШапкаЭлемент.ТолькоПросмотр = Истина;
	КР_ВесБруттоШапкаЭлемент.КнопкаВыбора = Ложь;
	
	КР_ЕдИзмВесРеквизит = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(
		ЭтотОбъект, "КР_ЕдиницаИзмеренияВес", Новый ОписаниеТипов("СправочникСсылка.УпаковкиЕдиницыИзмерения"));
	КР_ВесБруттоШапкаДекорация = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_ЕдИзмВесРеквизит, КР_ГруппаВесБрутто);
	КР_ВесБруттоШапкаДекорация.Вид = ВидПоляФормы.ПолеНадписи;
	КР_ВесБруттоШапкаДекорация.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	КР_ВесБруттоШапкаДекорация.АвтоМаксимальнаяШирина = Ложь;
	КР_ВесБруттоШапкаДекорация.МаксимальнаяШирина = 2;
	ЭтотОбъект.КР_ЕдиницаИзмеренияВес = Константы.ЕдиницаИзмеренияВеса.Получить();
	
	КР_ВесБруттоЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.Товары.КР_ВесБрутто", Элементы.Товары);
	КР_ВесБруттоЭлемент.ТолькоПросмотр = Истина;
	
	КР_ВесНеттоЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.Товары.КР_ВесНетто", Элементы.Товары);
	КР_ВесНеттоЭлемент.ТолькоПросмотр = Истина;
	
	КР_КонтейнерЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_Контейнер", Элементы.ШапкаПраво);
	КР_КонтейнерЭлемент.АвтоМаксимальнаяШирина = Ложь;
	КР_КонтейнерЭлемент.МаксимальнаяШирина = 32;

	// << 29.05.2024 Петухов А.В., Фактор, #4155
	злОбратныйВыкупЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.злОбратныйВыкуп", Элементы.ГруппаПараметрыПраво);
	// >> 29.05.2024 Петухов А.В., Фактор, #4155
	
КонецПроцедуры // >> 18.01.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1015
