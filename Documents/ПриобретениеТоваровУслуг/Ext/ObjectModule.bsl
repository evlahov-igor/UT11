...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// << 24.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-742
	ПередЗаписьюДополнительно(Отказ, РежимЗаписи, РежимПроведения);
	// >> 24.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-742
...
Процедура ЗаполнитьАналитикиУчетаНоменклатурыВТабличныхЧастяхТоваров(ТаблицыДокумента = Неопределено)
	...
	Если Склад.ЭтоГруппа
		И ОперацииВПутиИФактуровка.Найти(ХозяйственнаяОперация) = Неопределено
		И Склад.ВыборГруппы = Перечисления.ВыборГруппыСкладов.РазрешитьВЗаказахИНакладных Тогда
...
// << 24.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-742
Процедура ПередЗаписьюДополнительно(Отказ, РежимЗаписи, РежимПроведения)
	
	КР_ВесБруттоДокумента = Товары.Итог("КР_ВесБрутто");
	
КонецПроцедуры // >> 24.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-742
