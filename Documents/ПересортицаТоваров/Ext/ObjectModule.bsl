...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// << 11.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1624
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
    ...
    СкладПодразделения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение,"КР_Склад");
	Склад = СкладПодразделения;
	Если ТипЗнч(ДанныеЗаполнения) = Тип ("Структура") Тогда
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов)
	...
	Если ТекущийВидСкладаМагазин И Не ЗначениеЗаполнено(ПересчетТоваров) Тогда
		ПроверяемыеРеквизиты.Добавить("Товары.КР_ЯчейкаСписание");
		ПроверяемыеРеквизиты.Добавить("Товары.КР_ЯчейкаОприходование");
	КонецЕсли;
...
Процедура КР_ПередЗаписью(Отказ, РежимЗаписи,РежимПроведения)
	...
	КР_ПроверитьПраваНаПроведениеРасширеннойПересортицы(Отказ, РежимЗаписи);
	// << 10.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1624
	КР_ЗаполнитьНомераГТД(Отказ, РежимЗаписи);
...
// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
Процедура КР_ПроверитьПраваНаПроведениеРасширеннойПересортицы(Отказ, РежимЗаписи)
	
	Если РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
		// В режимах проведения и отмены проведения проверка требуется
		Возврат;
	КонецЕсли;
	
	ТребуетсяПроверкаПрав = Ложь;
	НомераСтрок = "";
	ШаблонНомеровСтрок = "%1%2%3";
	Для Каждого СтрокаТовары Из Товары Цикл
		Если СтрокаТовары.Номенклатура <> СтрокаТовары.НоменклатураОприходование Тогда
			ТребуетсяПроверкаПрав = Истина;
			НомераСтрок = СтрШаблон(ШаблонНомеровСтрок, НомераСтрок, СтрокаТовары.НомерСтроки, ", ");
		КонецЕсли;
	КонецЦикла;
	
	Если Не ТребуетсяПроверкаПрав Тогда
		Возврат;
	КонецЕсли;

	// Удалим последние два символа: ", "
	НомераСтрок = Лев(НомераСтрок, СтрДлина(НомераСтрок) - 2);
	ЕстьПравоНаПроведениеРасширеннойПересортицы = Пользователи.РолиДоступны("КР_РасширеннаяПересортицаТоваров");
	Если Не ЕстьПравоНаПроведениеРасширеннойПересортицы Тогда
		ШаблонСообщения = НСтр("ru='В строках [№%1] документа %2 номенклатура оприходования и списания не совпадают.
		|Для отражение такой пересортицы необходимо наличие роли ""Расширенная пересортица товаров""'");
		Сообщение = СтрШаблон(ШаблонСообщения, НомераСтрок, Ссылка);
		ОбщегоНазначения.СообщитьПользователю(Сообщение, Ссылка, , , Отказ);
	КонецЕсли;
	
КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 10.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1624
Процедура КР_ЗаполнитьНомераГТД(Отказ, РежимЗаписи)
	...
	Если Отказ Или ЭтоОтменаПроведения Или Не ИспользоватьИмпортныеТовары Тогда
		Возврат;
	КонецЕсли;
	
	Ключ = "Номенклатура, Характеристика, Назначение, Серия";
	Выгрузка = Товары.Выгрузить( , Ключ);
	ДанныеГТД = КР_АвтоматическоеСозданиеСкладскихАктов.ПолучитьБлижайшиеНомераГТД(Выгрузка, Дата);
...
КонецПроцедуры // >> 10.05.2023, Гашников А.П., КРОК, Jira№ A2105505-1624
