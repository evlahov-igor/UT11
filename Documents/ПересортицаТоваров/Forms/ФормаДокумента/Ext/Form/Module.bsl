...
Процедура КР_ПриСозданииНаСервереДополнительно(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТоварыХарактеристикаСписание.УстановитьДействие(
		"ПриИзменении", "КР_ТоварыХарактеристикаСписаниеПриИзменении");
	Элементы.ТоварыХарактеристикаОприходование.УстановитьДействие(
		"ПриИзменении", "КР_ТоварыХарактеристикаОприходованиеПриИзменении");
	
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	// Товары.КР_ЯчейкаСписание
	КР_ЯчейкаСписание = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_ЯчейкаСписание",
		Элементы.ТоварыГруппаСписываемыйТовар);
	
	// Товары.КР_ЯчейкаОприходование
	КР_ЯчейкаОприходование = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_ЯчейкаОприходование",
		Элементы.ТоварыГруппаПриходуемыйТовар);
	
	// << 07.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1448
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ЗакрытиеОтрицательныхОстатков", Элементы.ШапкаПраво);
...
КонецПроцедуры // >> 30.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-619

// << 30.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-619
&НаКлиенте
Процедура КР_ТоварыХарактеристикаСписаниеПриИзменении(Элемент)
		
КонецПроцедуры // >> 30.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-619

// << 04.10.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-619
&НаКлиенте
Процедура КР_ТоварыХарактеристикаОприходованиеПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
		
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	СтруктураДанных = Новый Структура;
	СтруктураДанных.Вставить("Номенклатура",	ТекущаяСтрока.НоменклатураОприходование);
	СтруктураДанных.Вставить("Характеристика",	ТекущаяСтрока.ХарактеристикаОприходование);
	СтруктураДанных.Вставить("Серия",			ТекущаяСтрока.СерияОприходование);
	СтруктураДанных.Вставить("Назначение",		ТекущаяСтрока.НазначениеОприходование);
	СтруктураДанных.Вставить("Дата",			Объект.Дата);
	НомерГТД = КР_ПолучитьНомерГТД(СтруктураДанных);
	Если НомерГТД <> Неопределено Тогда
		ТекущаяСтрока.НомерГТД = НомерГТД;
	КонецЕсли;
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	
КонецПроцедуры // >> 04.10.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-619
...
Процедура КР_УстановитьВидимостьДоступность()
	...
	КР_ДействияЗависящиеОтСклада();
	КР_ДоступностьПриходоватьТоварыПоСебестоимостиСписания();
	//<< 31.10.2023 Фомичев А.Е., КРОК, JIRA№A2105505-2467
	КР_УстановитьДоступностьСклада();
	//>> 31.10.2023 Фомичев А.Е., КРОК, JIRA№A2105505-2467
...
// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Процедура КР_ДействияЗависящиеОтСклада()
	
	ВидСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Склад, "КР_ВидСклада");
	ТекущийВидСкладаМагазин = (ВидСклада = Перечисления.КР_ВидыСкладов.Магазин);
	КР_УстановитьВидимостьЯчеекПоСкладу(ТекущийВидСкладаМагазин);
	КР_УстановитьРеквизитыДляСкладаМагазина(ТекущийВидСкладаМагазин);
		
КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Процедура КР_УстановитьВидимостьЯчеекПоСкладу(ТекущийВидСкладаМагазин)

	ВидимостьЯчеек = (ТекущийВидСкладаМагазин И Не ЗначениеЗаполнено(Объект.ПересчетТоваров));
	Элементы["ТоварыКР_ЯчейкаСписание"].Видимость		= ВидимостьЯчеек;
	Элементы["ТоварыКР_ЯчейкаОприходование"].Видимость	= ВидимостьЯчеек;
	
КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Процедура КР_УстановитьРеквизитыДляСкладаМагазина(ТекущийВидСкладаМагазин)
	
	Если ТекущийВидСкладаМагазин И Не ЗначениеЗаполнено(Объект.ПересчетТоваров) Тогда
		
		Если Не Объект.ПриходоватьТоварыПоСебестоимостиСписания Тогда
			Объект.ПриходоватьТоварыПоСебестоимостиСписания = Истина;
			// Ряд типовых действий в связи с установкой "ПриходоватьТоварыПоСебестоимостиСписания" = Истина
			Объект.ВидЦены = Неопределено;
			ОчиститьЦены(ЭтаФорма);
			ПриходоватьТоварыПоСебестоимостиСписанияПриИзмененииСервер();
		КонецЕсли;
		
		Объект.ВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка();
...
// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервере
Процедура КР_ДоступностьПриходоватьТоварыПоСебестоимостиСписания()
	
	ЕстьПравоНаРедактирование = Пользователи.РолиДоступны("ДобавлениеИзменениеСкладскихАктов, КР_РасширеннаяПересортицаТоваров");
	Элементы.ПриходоватьТоварыПоСебестоимостиСписания.ТолькоПросмотр = Не ЕстьПравоНаРедактирование;
	
КонецПроцедуры // >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860

// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
&НаСервереБезКонтекста
Функция КР_ПолучитьНомерГТД(СтруктураДанных)
	
	ВестиУчетПоГТД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанных.Номенклатура, "ВестиУчетПоГТД");
	...
	Данные.Колонки.Добавить("Характеристика",		Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
	Данные.Колонки.Добавить("Назначение",			Новый ОписаниеТипов("СправочникСсылка.Назначения"));
	Данные.Колонки.Добавить("Серия",				Новый ОписаниеТипов("СправочникСсылка.СерииНоменклатуры"));
	СтрокаДанных = Данные.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаДанных, СтруктураДанных);
	
	ДанныеГТД = КР_АвтоматическоеСозданиеСкладскихАктов.ПолучитьБлижайшиеНомераГТД(Данные, СтруктураДанных.Дата);
...
//<< 31.10.2023 Фомичев А.Е., КРОК, JIRA№A2105505-2467
Процедура КР_УстановитьДоступностьСклада(); 
	
	ПолныеПрава = Пользователи.РолиДоступны("ПолныеПрава");
	БазаЯвляетсяЦентральной = КР_ОбменRabbitОбщиеМеханизмыПовтИсп.ТекущаяИБЯвляетсяЦентральной();
	Если БазаЯвляетсяЦентральной Тогда  //в ЦБ изменение Склада доступно всем пользователям
		Элементы.Склад.Доступность = Истина
	ИначеЕсли ПолныеПрава И НЕ БазаЯвляетсяЦентральной Тогда //В базе магазина изменение Склада доступно только с полными правами
		Элементы.Склад.Доступность = Истина
	Иначе
		Элементы.Склад.Доступность = Ложь
	КонецЕсли;
				
КонецПроцедуры 	//>> 31.10.2023 Фомичев А.Е., КРОК, JIRA№A2105505-2467
