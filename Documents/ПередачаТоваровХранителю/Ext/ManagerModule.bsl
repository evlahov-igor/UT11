...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора, Товары, СкладПередачи, МассивЗаказов, ПараметрыЗаполнения) Экспорт
	...
	РезультатЗапросаПоОстаткамЗаказов = ПолучитьРезультатЗапросаПоОстаткамЗаказов(
											ДанныеОтбора, СкладПередачи, МассивЗаказов);
	...
	Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(ДанныеОтбора.ХозяйственнаяОперация);
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	// << 16.02.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1264
	// Исправлена ошибка вендора - обращение из НаКлиенте к серверному методу
	//Обработчик = Документы.ПередачаТоваровХранителю.ОбработчикДействий(Данные.ХозяйственнаяОперация);
	//
	//Представление = СтрШаблон(НСтр("ru='%1 %2 от %3'"), Обработчик.ПредставлениеОбъекта(), Данные.Номер, Данные.Дата);
	Если Данные.ХозяйственнаяОперация =
			ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию") Тогда
...
Функция ТекстЗапросаТаблицаЗаказыКлиентов(Запрос, ТекстыЗапроса, Регистры)
	...
	ПодставитьТекстИспользоватьРасширенныеВозможностиЗаказаКлиента(ТекстЗапроса, "ТаблицаТовары.Ссылка");
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПереработчику2_5)
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ПодставитьТекстИспользоватьРасширенныеВозможностиЗаказаКлиента(ТекстЗапросаТабЧасть, "ТабЧасть.Ссылка");
...
Функция ТекстЗапросаВТТоварыРаспоряжениеНакладнаяОтгружено()
	...
	ПодставитьТекстИспользоватьРасширенныеВозможностиЗаказаКлиента(ТекстЗапроса, "ДокументТовары.Ссылка");
...
//
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(
			ДанныеОтбора, СкладПередачи = Неопределено, МассивЗаказов = Неопределено)
	...
	|				И &ИспользоватьРасширенныеВозможностиЗаказаКлиента
	|				И (НЕ &ИспользоватьОтборПоСкладуПередачи
	|					ИЛИ Склад В(ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), &СкладПередачи))
	|			) КАК ЗаказыОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		И (НЕ &ИспользоватьОтборПоСкладуПередачи
	|			ИЛИ ЗаказыДвижения.Склад В(ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), &СкладПередачи))
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		И (НЕ &ИспользоватьОтборПоСкладуПередачи
	|			ИЛИ ТоварыЗаказа.Склад В(ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), &СкладПередачи))
...
Функция ТекстЗапросаТоварыДокумента(Отбор, ТекстСоединения, ТекстПроверкиСоединения = "") Экспорт
	...
	|	СУММА(Таблица.Количество)    КАК Количество,
	|	ВЫБОР
	|		КОГДА АналитикаЗапасов.КОформлению > АналитикаЗапасов.КоличествоВДокументе
	|			ТОГДА АналитикаЗапасов.КоличествоВДокументе
	|		ИНАЧЕ АналитикаЗапасов.КОформлению
	|	КОНЕЦ                        КАК КОформлению,
	|	Таблица.Цена                 КАК Цена,
	|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) КАК СтавкаНДС,
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	КонецЕсли;
		...
		|			ГДЕ
		|				РеестрДокументов.Ссылка = ЗаказыКлиентов.ЗаказКлиента
		...
		|				РегистрСведений.РеестрДокументов КАК РеестрДокументов
		|			ГДЕ
		|				РеестрДокументов.Ссылка = ТоварыКОтгрузке.ДокументОтгрузки
		|				И РеестрДокументов.ХозяйственнаяОперация В(&ХозяйственныеОперации)
		|				И &ИспользоватьРасширенныеВозможностиЗаказаКлиента
		|				И РеестрДокументов.Ответственный = &Пользователь
		|			)
...
Функция ЗаголовокФормыАкта() Экспорт
	...
	Возврат НСтр("ru = 'Акты о расхождениях после передачи товаров хранителю'");
...
Процедура ПодставитьТекстИспользоватьРасширенныеВозможностиЗаказаКлиента(
			ТекстЗапроса, ИмяТаблицы, Параметр = "&ИспользоватьРасширенныеВозможностиЗаказаКлиента") Экспорт
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'В передаче товаров хранителю:
	| - заполняет пустые значения поля ""Заказ клиента"" шапки документа, табличных частей ""Товары"", ""Виды запасов"" значением Неопределено;
	| - заполняет поле ""Назначение отправителя"" в табличных частях ""Товары"", ""Серии"".'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	// Назначение отправителя в ТЧ Товары
	|	ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ПередачаТоваровХранителю.Товары КАК ТабличнаяЧастьТовары
	|		ГДЕ
	|			ТабличнаяЧастьТовары.Ссылка = ПередачаТоваровХранителю.Ссылка
	|			И ПередачаТоваровХранителю.ХозяйственнаяОперация =
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи)
	|			И НЕ ТабличнаяЧастьТовары.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|			И НЕ ТабличнаяЧастьТовары.НазначениеОтправителя = ТабличнаяЧастьТовары.Назначение
	|		)
	// Назначение отправителя в ТЧ Серии
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ПередачаТоваровХранителю.Серии КАК ТабличнаяЧастьТовары
	|		ГДЕ
	|			ТабличнаяЧастьТовары.Ссылка = ПередачаТоваровХранителю.Ссылка
	|			И ПередачаТоваровХранителю.ХозяйственнаяОперация =
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи)
	...
	|			Документ.ПередачаТоваровХранителю КАК ТаблицаДокумента
	|		ГДЕ
	|			ТаблицаДокумента.Ссылка = ПередачаТоваровХранителю.Ссылка
	|			И ТаблицаДокумента.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|		)
	// Заказ клиента в ТЧ Товары
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			ТабличнаяЧастьТовары.Ссылка = ПередачаТоваровХранителю.Ссылка
	|			И ТабличнаяЧастьТовары.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|		)
	// Заказ клиента ТЧ Виды запасов
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ПередачаТоваровХранителю.ВидыЗапасов КАК ТабличнаяЧастьВидыЗапасов
	|		ГДЕ
	|			ТабличнаяЧастьВидыЗапасов.Ссылка = ПередачаТоваровХранителю.Ссылка
	|			И ТабличнаяЧастьВидыЗапасов.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|		)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	#Область ТекстыЗапросаЗаполнитьНазначениеОтправителяТовары
	...
	|ПОМЕСТИТЬ ЗаполнитьНазначениеОтправителяТовары
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|
	|ГДЕ
	...
	|			Документ.ПередачаТоваровХранителю КАК ТаблицаДокумента
	|		ГДЕ
	|			ОбновляемыеДанные.Ссылка = ТаблицаДокумента.Ссылка
	|			И ТаблицаДокумента.ХозяйственнаяОперация =
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаХранениеСПравомПродажи)
	|		)
	|	И ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|			Документ.ПередачаТоваровХранителю.Товары КАК ТабличнаяЧастьТовары
	|		ГДЕ
	|			ОбновляемыеДанные.Ссылка = ТабличнаяЧастьТовары.Ссылка
	|			И НЕ ТабличнаяЧастьТовары.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|			И НЕ ТабличнаяЧастьТовары.НазначениеОтправителя = ТабличнаяЧастьТовары.Назначение
	|		)
	|
	...
	#Область ТекстыЗапросаЗаполнитьНазначениеОтправителяСерии
	...
	|	ОбновляемыеДанные.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	ИСТИНА                                КАК Обработать
	|ПОМЕСТИТЬ ЗаполнитьНазначениеОтправителяСерии
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|
	|ГДЕ
	...
	#Область ТекстыЗапросаЗаполнитьЗаказКлиентаТовары
	...
	|	ОбновляемыеДанные.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	ИСТИНА                                КАК Обработать
	|ПОМЕСТИТЬ ЗаполнитьЗаказКлиентаТовары
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|
	|ГДЕ
	...
	#Область ТекстыЗапросаЗаполнитьЗаказКлиентаВидыЗапасов
	...
	|	ОбновляемыеДанные.Ссылка.ВерсияДанных КАК ВерсияДанных,
	|	ИСТИНА                                КАК Обработать
	|ПОМЕСТИТЬ ЗаполнитьЗаказКлиентаВидыЗапасов
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|
	|ГДЕ
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.ПередачаТоваровХранителю.ВидыЗапасов КАК ТабличнаяЧастьТовары
	|		ГДЕ
	|			ОбновляемыеДанные.Ссылка = ТабличнаяЧастьТовары.Ссылка
	|			И ТабличнаяЧастьТовары.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)
	|		)
	...
	"ВЫБРАТЬ
	|	ОбновляемыеДанные.Ссылка										КАК Ссылка,
	|	ЕСТЬNULL(ЗаполнитьНазначениеОтправителяТовары.Обработать, ЛОЖЬ)	КАК ЗаполнитьНазначениеОтправителяТовары,
	|	ЕСТЬNULL(ЗаполнитьНазначениеОтправителяСерии.Обработать, ЛОЖЬ)	КАК ЗаполнитьНазначениеОтправителяСерии,
	|	ЕСТЬNULL(ЗаполнитьЗаказКлиентаТовары.Обработать, ЛОЖЬ)			КАК ЗаполнитьЗаказКлиентаТовары,
	|	ЕСТЬNULL(ЗаполнитьЗаказКлиентаВидыЗапасов.Обработать, ЛОЖЬ)		КАК ЗаполнитьЗаказКлиентаВидыЗапасов,
	|	ВЫБОР
	|		КОГДА ЗаполнитьНазначениеОтправителяТовары.ВерсияДанных ЕСТЬ НЕ NULL
	|			ТОГДА ЗаполнитьНазначениеОтправителяТовары.ВерсияДанных
	|		КОГДА ЗаполнитьНазначениеОтправителяСерии.ВерсияДанных ЕСТЬ НЕ NULL
	|			ТОГДА ЗаполнитьНазначениеОтправителяСерии.ВерсияДанных
	|		КОГДА ЗаполнитьЗаказКлиентаТовары.ВерсияДанных ЕСТЬ НЕ NULL
	|			ТОГДА ЗаполнитьЗаказКлиентаТовары.ВерсияДанных
	|		КОГДА ЗаполнитьЗаказКлиентаВидыЗапасов.ВерсияДанных ЕСТЬ НЕ NULL
	|			ТОГДА ЗаполнитьЗаказКлиентаВидыЗапасов.ВерсияДанных
	|		ИНАЧЕ ОбновляемыеДанные.Ссылка.ВерсияДанных
	|	КОНЕЦ															КАК ВерсияДанных
	|ИЗ
	|	ОбновляемыеДанные КАК ОбновляемыеДанные
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьНазначениеОтправителяТовары КАК ЗаполнитьНазначениеОтправителяТовары
	|	ПО ЗаполнитьНазначениеОтправителяТовары.Ссылка = ОбновляемыеДанные.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьНазначениеОтправителяСерии КАК ЗаполнитьНазначениеОтправителяСерии
	|	ПО ЗаполнитьНазначениеОтправителяСерии.Ссылка = ОбновляемыеДанные.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьЗаказКлиентаТовары КАК ЗаполнитьЗаказКлиентаТовары
	|	ПО ЗаполнитьЗаказКлиентаТовары.Ссылка = ОбновляемыеДанные.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьЗаказКлиентаВидыЗапасов КАК ЗаполнитьЗаказКлиентаВидыЗапасов
	|	ПО ЗаполнитьЗаказКлиентаВидыЗапасов.Ссылка = ОбновляемыеДанные.Ссылка
	|");
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ДокументОбъект.ЗаказКлиента)
				И Не ДокументОбъект.ЗаказКлиента = Неопределено Тогда
				ДокументОбъект.ЗаказКлиента = Неопределено;
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			Если ВыборкаДокументов.ЗаполнитьНазначениеОтправителяТовары
				Или ВыборкаДокументов.ЗаполнитьЗаказКлиентаТовары Тогда
				
				Для Каждого ТекущиеДанные Из ДокументОбъект.Товары Цикл
					
					Если ВыборкаДокументов.ЗаполнитьЗаказКлиентаТовары
						И ТекущиеДанные.ЗаказКлиента = Документы.ЗаказКлиента.ПустаяСсылка() Тогда
						ТекущиеДанные.ЗаказКлиента = Неопределено;
					КонецЕсли;
					
					Если ВыборкаДокументов.ЗаполнитьНазначениеОтправителяТовары
						И ЗначениеЗаполнено(ТекущиеДанные.Назначение)
						И Не ТекущиеДанные.НазначениеОтправителя = ТекущиеДанные.Назначение Тогда
			...
			Если ВыборкаДокументов.ЗаполнитьНазначениеОтправителяСерии Тогда
				
				Для Каждого ТекущиеДанные Из ДокументОбъект.Серии Цикл
			...
			Если ВыборкаДокументов.ЗаполнитьЗаказКлиентаВидыЗапасов Тогда
				
				Для Каждого ТекущиеДанные Из ДокументОбъект.ВидыЗапасов Цикл
					Если ТекущиеДанные.ЗаказКлиента = Документы.ЗаказКлиента.ПустаяСсылка() Тогда
						ТекущиеДанные.ЗаказКлиента = Неопределено;
					КонецЕсли;
				КонецЦикла;
