...
Процедура СформироватьНачальноеЗаполнениеЧекККМТовары()
	...
	ТЧТоварыЧекаККМ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ЧекККМ, "Товары");
	
	Если ТЧТоварыЧекаККМ <> Неопределено Тогда
		//.Выгрузить();
	
		Для Каждого СтрокаТЧ Из ТЧТоварыЧекаККМ.Выгрузить() Цикл
...
Функция ПараметрыОперацииФискализацииЧекаНаСервере()
	...
	ФормированиеПараметровФискальногоЧекаСервер.ПараметрыФискальногоЧекаЗаполнитьПараметрыКоррекции(
		ПараметрыОперацииФискализацииЧека,
		Объект.ЧекККМ,
		Перечисления.ВидыЧековКоррекции.ИсправлениеОшибки,
		ДанныеКоррекции
	);
...
Процедура ФискальнаяОперацияОбновитьСтатус()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ФискальнаяОперацияДанныеЖурнала = МенеджерОборудованияВызовСервера.СписокФискальныхОпераций(Объект.Ссылка);
	Иначе
