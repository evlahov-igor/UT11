...
Функция ТекстЗапросаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОстаткиПоНазначениям(ИмяВременнойТаблицы = Неопределено) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ",ИмяВременнойТаблицы", ТекстДляЗамены);
...
Функция ТекстЗапросаТаблицаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупках(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.ДокументПриобретения,
...
Функция ПолучитьДанныеДляПечатнойФормыОтчетПоЗакупкам(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	СчетФактураВыданныйТовары.Ссылка КАК СчетФактураВыданный,
	...
	|		ПО СчетФактураВыданныйТовары.Ссылка = СчетФактураВыданныйДокументыОснования.Ссылка
	|	ГДЕ
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОтчетКомитентуОЗакупкахТовары.Ссылка КАК Ссылка,
	|		ОтчетКомитентуОЗакупкахТовары.Номенклатура КАК Номенклатура,
	|		ВЫБОР
	|			КОГДА ОтчетКомитентуОЗакупкахТовары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|				ТОГДА ОтчетКомитентуОЗакупкахТовары.Номенклатура.ЕдиницаИзмерения
	|			ИНАЧЕ ОтчетКомитентуОЗакупкахТовары.Упаковка
	|		КОНЕЦ КАК ЕдиницаИзмерения,
	|		ОтчетКомитентуОЗакупкахТовары.Характеристика КАК Характеристика,
	|		ОтчетКомитентуОЗакупкахТовары.Упаковка КАК Упаковка,
	|		ОтчетКомитентуОЗакупкахТовары.КоличествоУпаковок КАК Количество,
	|		ОтчетКомитентуОЗакупкахТовары.Цена КАК Цена,
	|		ВЫБОР КОГДА ОтчетКомитентуОЗакупкахТовары.Ссылка.ЦенаВключаетНДС ТОГДА
	|			ОтчетКомитентуОЗакупкахТовары.СуммаСНДС
	|		ИНАЧЕ
	|			ОтчетКомитентуОЗакупкахТовары.Сумма
	|		КОНЕЦ КАК СуммаКомитента,
	|		ОтчетКомитентуОЗакупкахТовары.СуммаВознаграждения КАК СуммаВознаграждения,
	|		ОтчетКомитентуОЗакупкахТовары.НомерСтроки КАК НомерСтроки,
	|		ОтчетКомитентуОЗакупкахТовары.ДатаСчетаФактуры КАК ДатаСчетаФактуры,
	|		ОтчетКомитентуОЗакупкахТовары.СуммаСНДС КАК СуммаСНДС,
	//++ Локализация
	|		ОтчетКомитентуОЗакупкахТовары.КоличествоПоРНПТ КАК КоличествоПоРНПТ,
	...
	|		ОтчетКомитентуОЗакупкахТовары.Поставщик КАК Поставщик,
	|		ВЫБОР
	|			КОГДА ОтчетКомитентуОЗакупкахТовары.СуммаВознаграждения <> 0
	|				ТОГДА ОтчетКомитентуОЗакупкахТовары.СуммаВознаграждения/(ОтчетКомитентуОЗакупкахТовары.СуммаСНДС/100)
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК ПроцентВознаграждения
	//++ Локализация
	|		,СчетФактураВыданныйТовары.СчетФактураВыданный.Дата КАК СчетФактураВыданныйДата,
	|		СчетФактураВыданныйТовары.СчетФактураВыданный.ПредставлениеНомера КАК СчетФактураВыданныйПредставлениеНомера
	//-- Локализация
	|	ИЗ
	|		Документ.ОтчетКомитентуОЗакупках.Товары КАК ОтчетКомитентуОЗакупкахТовары
	//++ Локализация
	|			ЛЕВОЕ СОЕДИНЕНИЕ СчетФактураВыданныйТовары КАК СчетФактураВыданныйТовары
	|			ПО ОтчетКомитентуОЗакупкахТовары.Ссылка = СчетФактураВыданныйТовары.ДокументОснование
	|				И ОтчетКомитентуОЗакупкахТовары.Номенклатура = СчетФактураВыданныйТовары.Номенклатура
	|				И ОтчетКомитентуОЗакупкахТовары.Характеристика = СчетФактураВыданныйТовары.Характеристика
	|				И ОтчетКомитентуОЗакупкахТовары.Количество = СчетФактураВыданныйТовары.Количество
	|				И ОтчетКомитентуОЗакупкахТовары.СуммаСНДС = СчетФактураВыданныйТовары.Сумма
	|				И ОтчетКомитентуОЗакупкахТовары.СуммаНДС = СчетФактураВыданныйТовары.СуммаНДС
	|				И ОтчетКомитентуОЗакупкахТовары.НомерГТД = СчетФактураВыданныйТовары.НомерГТД
	//-- Локализация
	|	ГДЕ
	|		ОтчетКомитентуОЗакупкахТовары.Ссылка В(&МассивДокументов)
	|		И ОтчетКомитентуОЗакупкахТовары.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.ОтчетКомитентуОЗакупках.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "11.5.8.88";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("39bca5bb-fff0-443f-bad6-9de302a5c9b5");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетКомитентуОЗакупках.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Обновляет сумму НДС вознаграждения в строках документа ""Отчет комитенту о закупках"".
	...
	|Очищает служебную табличную часть ""Расшифровка платежа с клиентом"" в непроведенных документах.'");
	...
	Изменяемые.Добавить(Метаданные.Документы.ОтчетКомитентуОЗакупках.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ОтчетКомитентуОЗакупках";
	...
	|					Документ.ОтчетКомитентуОЗакупках.Товары КАК ОтчетКомитентуОЗакупкахТовары
	|				ГДЕ
	|					ОтчетКомитентуОЗакупках.Ссылка = ОтчетКомитентуОЗакупкахТовары.Ссылка
	|					И НЕ ОтчетКомитентуОЗакупкахТовары.СуммаНДСВознаграждения = ВЫРАЗИТЬ(ОтчетКомитентуОЗакупкахТовары.СуммаВознаграждения
	|						* ЕСТЬNULL(ОтчетКомитентуОЗакупках.СтавкаНДСВознаграждения.Ставка,0)
	|						/ (100 + ЕСТЬNULL(ОтчетКомитентуОЗакупках.СтавкаНДСВознаграждения.Ставка, 0)) КАК ЧИСЛО(31,2))
	|				)
	|	)
	|	ИЛИ ОтчетКомитентуОЗакупках.ИдентификаторДокумента = """"
	|	ИЛИ  ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ОтчетКомитентуОЗакупках.ЭтапыГрафикаОплаты КАК ТЧЭтапыГрафикаОплаты
	|				ГДЕ
	|					ТЧЭтапыГрафикаОплаты.Ссылка = ОтчетКомитентуОЗакупках.Ссылка
	|					И (ТЧЭтапыГрафикаОплаты.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|						ИЛИ ТЧЭтапыГрафикаОплаты.ВариантОтсчета = ЗНАЧЕНИЕ(Перечисление.ВариантыОтсчетаДатыПлатежа.ПустаяСсылка)
	...
	|																КОГДА РАЗНОСТЬДАТ(ОтчетКомитентуОЗакупках.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ) < 0
	|																ТОГДА 0
	|																ИНАЧЕ РАЗНОСТЬДАТ(ОтчетКомитентуОЗакупках.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ)
	|															КОНЕЦ)
	|				)
	|	ИЛИ НЕ ОтчетКомитентуОЗакупках.Проведен И (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.ОтчетКомитентуОЗакупках.РасшифровкаПлатежаСКлиентом КАК ТЧРасшифровкаПлатежаСКлиентом
	|			ГДЕ
	|				ТЧРасшифровкаПлатежаСКлиентом.Ссылка = ОтчетКомитентуОЗакупках.Ссылка))";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(ДокументОбъект.СтавкаНДСВознаграждения) Тогда
					СтавкаНДС = ДокументОбъект.СтавкаНДСВознаграждения.Ставка;
					Товары = ДокументОбъект.Товары;
					Для каждого Стр Из Товары Цикл
						Если Стр.СуммаНДСВознаграждения = 0 Тогда
							Стр.СуммаНДСВознаграждения = Стр.СуммаВознаграждения * СтавкаНДС / (100 + СтавкаНДС);
