...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли НоменклатураКлиент.ЭтоУказаниеНомераГТД(ИсточникВыбора)
		...
		НоменклатураКлиент.ОбработатьУказаниеНомераГТД(ЭтотОбъект, ВыбранноеЗначение, Неопределено, ТекущиеДанные);
...
Процедура ПереданныеПереработчикамТоварыЗаказПереработчикуПриИзменении(Элемент)
	...
	Если ЗаказПереработчикуДоИзменения = Объект.ЗаказПереработчику Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПереданныеПереработчикамТоварыЗаказПереработчикуПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
    ...
    Если РезультатВопроса = КодВозвратаДиалога.Нет Тогда
        Объект.ЗаказПереработчику = ЗаказПереработчикуДоИзменения;
        Возврат;
    Иначе
...
Процедура УстановитьУсловноеОформление()
	...
	УчетПрослеживаемыхТоваровЛокализация.УстановитьУсловноеОформлениеКоличестваПоРНПТ(ЭтаФорма, "ТоварыКоличествоПоРНПТ");
...
Процедура ЗаполнитьПоСоглашениюСервер()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковТоваровПолученныхНаКомиссию Тогда
		...
		ЗаполнитьЗначенияСвойств(Объект, Реквизиты,, "Склад, Валюта");
...
Процедура ПриЧтенииСозданииНаСервере(ДанныеОбъекта)
	...
	ЗаказПереработчикуДоИзменения = ДанныеОбъекта.ЗаказПереработчику;
	
	Элементы.ГруппаПоЗаказу.Видимость =
		ЭтоОстаткиПереданныхПереработчикамТоваров
...
Процедура УстановитьВидимость()
	...
	Элементы.ГруппаВводОстатковПо.Видимость                         = Истина;
	Элементы.ГруппаРеглУчет.Видимость                               = ВидимостьГруппУчета;
...
Процедура УстановитьПараметрыВыбораПартнера()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковМатериаловПереданныхПереработчикам
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковМатериаловПереданныхПереработчикам2_5 Тогда
		МассивПараметров = Новый Массив;
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(Объект));
	ОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Объект.ДетализацияПартий, СтруктураДействий, Неопределено);
	Для Каждого Строка Из Объект.ДетализацияПартий Цикл
