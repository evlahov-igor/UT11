...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	// << 28.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-933
	КР_ТекстЗапросаТаблицаКоробаНаСкладах(ТекстыЗапроса, Регистры);
	// >> 28.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-933
...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|		ВЫБРАТЬ ОбъектыРасчетов.ОбъектРасчетов ИЗ ОбъектыРасчетов)) КАК РасчетыСКлиентамиОстатки
		|ПО
...
Процедура ВременныеТаблицыТоваровЗаказа(МенеджерВременныхТаблиц, Заказ, Склад = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Склад) Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &Склад", "И Товары.Склад В (&Склад)");
...
Функция ПараметрыДокументаДляДействийОбеспечения(Объект, ДатаОтгрузки = Неопределено) Экспорт
	...
	Иначе
		ВариантОбеспеченияИФлагОбособленно = ПродажиСервер.ВариантОбеспеченияИФлагОбособленноПоУмолчанию(Объект.Соглашение, Объект.Статус);
	КонецЕсли;
...
Процедура ПослеЗаполненияОбеспечения(Объект, Изменения, Режим, ПараметрыЗаполнения) Экспорт
	...
	Если ЕстьИзмененияДатыОтгрузки Тогда
		Для Каждого Стр Из Объект.ЭтапыГрафикаОплаты Цикл
			Если Стр.ВариантОтсчета = Перечисления.ВариантыОтсчетаДатыПлатежа.ДоДатыОтгрузки
				ИЛИ Стр.ВариантОтсчета = Перечисления.ВариантыОтсчетаДатыПлатежа.ОтДатыОтгрузки
				ИЛИ Стр.ВариантОтсчета = Перечисления.ВариантыОтсчетаДатыПлатежа.ОтДатыПереходаПраваСобственности Тогда
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеШапки.Договор.КомиссионныеПродажи25 КАК КомиссионныеПродажи25
	|
	|ИЗ
...
Функция ПолучитьДанныеДляПечатнойФормыЗаказаНаТоварыУслуги(МассивОбъектов, ПараметрыПечати) Экспорт
	...
	|			ИЛИ Документы.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка) ТОГДА
	|		ЛОЖЬ
	|	ИНАЧЕ
	|		ИСТИНА
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Заполняет реквизит ""Вариант обеспечения"", признак ""Обособленно"".
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("96a664a9-56b1-4ef6-9088-9652d8c8df45");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаказКлиента.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИЛИ ЗаказКлиентаТовары.ВариантОбеспечения В(
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОбособленно),
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно))
	|				ИЛИ ЗаказКлиентаТовары.ИдентификаторСтроки = """"
	|				ИЛИ ЗаказКлиентаТовары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|					И ЗаказКлиентаТовары.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|					И НЕ ЗаказКлиента.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)))
	|	ИЛИ ИСТИНА В(
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			ДокументыДляЗамены.Ссылка = ЗаказКлиента.Ссылка)
	|	ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ЗаказКлиента.ЭтапыГрафикаОплаты КАК ТЧЭтапыГрафикаОплаты
	|						ЛЕВОЕ СОЕДИНЕНИЕ 
	|							(ВЫБРАТЬ
	|								Товары.Ссылка КАК Ссылка,
	...
	|				ГДЕ
	|					ТЧЭтапыГрафикаОплаты.Ссылка = ЗаказКлиента.Ссылка
	...
	|	ИЛИ НЕ ЗаказКлиента.ОплатаВВалюте
	|			И (ЗаказКлиента.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ЗаказКлиента.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ (ЗаказКлиента.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|		И ЗаказКлиента.Проведен
	|		И (ЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
	|		ИЛИ ЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|		ИЛИ ЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамКонтрагентов)
	|		ИЛИ ЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамНакладным)))
	|";
	
	Запрос.Текст = ОбеспечениеВДокументахСервер.ДобавитьТекстВременнойТаблицыДокументыДляЗаменыКОбеспечениюНаТребуется(
		Запрос.Текст, Документы.ЗаказКлиента);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказКлиента"; ИмяТЧ = "Товары";
	...
	Текст = ОбеспечениеВДокументахСервер.ТекстЗапросаВременнойТаблицыДокументыДляЗаменыКОбеспечениюНаТребуется(Документы.ЗаказКлиента);
	ТекстыЗапроса.Добавить(Текст);
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ОбщегоНазначенияУТ.ЗаполнитьИдентификаторыДокумента(ДокументОбъект, Метаданные.Документы.ЗаказКлиента.ТабличныеЧасти.Товары.Имя);
				#КонецОбласти
				...
				Для Каждого СтрТовары Из ДокументОбъект.Товары Цикл
					ДатаОтгрузки = Макс(СтрТовары.ДатаОтгрузки, ДатаОтгрузки);
				КонецЦикла;
...
Функция ТекстЗапросаДокументыДляЗаменыКОбеспечениюНаТребуется() Экспорт
	Возврат ОбеспечениеВДокументахСервер.ТекстЗапросаДокументыДляЗаменыКОбеспечениюНаТребуется(
		"Документ.ЗаказКлиента", "Товары", Истина);
КонецФункции
...
Функция КР_СтатусыТекущейСистемы() Экспорт 

	СтатусыЗаказовКлиентов = Перечисления.СтатусыЗаказовКлиентов;
	МассивСтатусов = Новый Массив;
	
	МассивСтатусов.Добавить(СтатусыЗаказовКлиентов.НеСогласован);
	МассивСтатусов.Добавить(СтатусыЗаказовКлиентов.КОбеспечению);
	МассивСтатусов.Добавить(СтатусыЗаказовКлиентов.КОтгрузке);
	
	Возврат МассивСтатусов;
	
КонецФункции // >> 09.09.2022 Марченко С.Н., КРОК, JIRA№A2105505-416

// << 09.09.2022 Марченко С.Н., КРОК, JIRA№A2105505-416
Функция КР_СтатусыВнешнихСистем() Экспорт 

	СтатусыЗаказовКлиентов = Перечисления.СтатусыЗаказовКлиентов;
	МассивСтатусов = Новый Массив;
	
	МассивСтатусов.Добавить(СтатусыЗаказовКлиентов.Закрыт);
...
// Параметры: 
// РеквизитыДокумента - Структура, Объект - нечто к чему можно через точку получить реквизиты указанные 
//											в КР_РеквизитыПроверкиРазрешенияРедактирования(). 
// Отказ 			  - Булево - флаг возводимый в истину, если нет доступа к редактированию данного документа.
//
Процедура КР_ПроверитьРазрешениеРедактирования(РеквизитыДокумента, Отказ) Экспорт
	
	Если  Пользователи.ЭтоПолноправныйПользователь() Тогда
		Возврат;
	КонецЕсли;
	
	Если  Пользователи.РолиДоступны("КР_ДобавлениеИзменениеЗаказовКлиентовРасширенный") Тогда
		Возврат;
	КонецЕсли;
	
	ЗакрытыеСтатусы = Новый Массив;
	ЗакрытыеСтатусы.Добавить(Перечисления.СтатусыЗаказовКлиентов.Закрыт);
	ЗакрытыеСтатусы.Добавить(Перечисления.СтатусыЗаказовКлиентов.КОтгрузке);
	
	ЗапретРедактирования = РеквизитыДокумента.Проведен 
	И РеквизитыДокумента.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссиюВДругуюОрганизацию
	И ЗакрытыеСтатусы.Найти(РеквизитыДокумента.Статус) <> Неопределено;   
	
	Если ЗапретРедактирования  Тогда  
		ТекстОшибки = НСтр("ru='Документ в статусе ""%1"" запрещен к редактированию. Обратитесь к администратору.'");
		ТекстОшибки = СтрШаблон(ТекстОшибки, Строка(РеквизитыДокумента.Статус));
...
// Создан: 22.08.2022, Мельников А.А., КРОК, Jira№ A2105505-384 
// Возвращает пустую структуру содержащую реквизиты которые следует заполнить,
//  чтоб вызвать "КР_ПроверитьРазрешениеРедактирования" без ошибок
// Возвращаемое значение:
// Структура
Функция КР_РеквизитыПроверкиРазрешенияРедактирования() Экспорт
	
	Возврат Новый Структура("Статус, ХозяйственнаяОперация, Проведен");	  
	
КонецФункции // >> 22.08.2022, Мельников А.А., КРОК, Jira№ A2105505-384 

// << 28.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-933
// Добавляет запрос движения по регистру накопления "Короба на складах"
//
// Параметры:
//  ТекстыЗапроса - Массив - Тексты запросов, для проведения документа.
//  Регистры - Структура - Список имен регистров, для которых необходимо получить таблицы
// 
// Возвращаемое значение:
//  Строка - Текст запроса.
//
Функция КР_ТекстЗапросаТаблицаКоробаНаСкладах(ТекстыЗапроса, Регистры)
	...
	// Строки с одинаковым коробом сворачиваются в одну, при этом значение <К отгрузке> всегда = 1.
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	ТоварыДокумента.Ссылка.Дата КАК Период,
	|	ТоварыДокумента.Склад КАК Склад,
	|	ТоварыДокумента.КР_Короб КАК Короб,
	|	0 КАК ВНаличии,
	|	1 КАК КОтгрузке
	|ИЗ
	|	Документ.ЗаказКлиента.Товары КАК ТоварыДокумента
	...
	|	И ТоварыДокумента.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовКлиентов.КОтгрузке)
	|	И ТоварыДокумента.Ссылка.Склад.КР_УчетОстатковКоробов
	|	И ТоварыДокумента.КР_Короб <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)
