...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	// << 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
	Если ЭтотОбъект["КР_ЭтоПользовательМагазина"] Тогда 
		
		ТекущиеДанные = Элемент.ТекущиеДанные;
		ТекущиеДанные.РольАссортимента = ПредопределенноеЗначение("Перечисление.РолиАссортимента.ДополнительныйАссортимент");
		ТекущиеДанные.ВидЦены = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(
			Объект.ОбъектПланирования, "РозничныйВидЦены");
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	// << 26.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-948
	КР_ОбработкаВыбораПодборНаСервереАльтернативно(ВыбранноеЗначение);
	Возврат;
	// >> 26.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-948
...
Процедура УстановитьДоступностьКомандБуфераОбмена()
	...
	// << 26.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-948
	МассивЭлементов.Добавить("КР_ВставитьСтрокиСХарактеристикой");
	// >> 26.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-948
...
Процедура ЭтапПриИзмененииНаСервере()
	...
	// << 26.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-948
	КР_ЭтапПриИзмененииНаСервереАльтернативно();
	Возврат;
	// >> 26.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-948
...
Процедура КР_ПриСозданииНаСервереДополнительно(Отказ, СтандартнаяОбработка)
	...
	ЭтоПользовательМагазина = Справочники.Пользователи.КР_ЭтоПользовательМагазина(ТекущийПользователь);
	ЭтотОбъект["КР_ЭтоПользовательМагазина"] = ЭтоПользовательМагазина;
	Если ЭтоПользовательМагазина Тогда 
		Объект.ОбъектПланирования = ТекущийПользователь.Подразделение.КР_Склад;
	КонецЕсли;	       
	
	Элементы.КР_ТоварыСХарактеристикой.УстановитьДействие(
		"ПриНачалеРедактирования", "КР_ТоварыСХарактеристикойПриНачалеРедактирования");
	// >> 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057

	КР_ПереопределитьСписокЭтапов();
...
&НаКлиенте
Процедура КР_ЗагрузитьТоварыСХарактеристикойИзФайла(Команда)
	
	ПараметрыЗагрузки = ЗагрузкаДанныхИзФайлаКлиент.ПараметрыЗагрузкиДанных();
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "ИзменениеАссортимента.КР_ТоварыСХарактеристикой";
	ПараметрыЗагрузки.Заголовок = НСтр("ru = 'Загрузка списка товаров из файла'");
	ПараметрыЗагрузки.ИмяМакетаСШаблоном = "КР_ЗагрузкаИзФайлаСХарактеристикой";
	ПараметрыЗагрузки.ДополнительныеПараметры =  Новый Структура;
	ПараметрыЗагрузки.ДополнительныеПараметры.Вставить("КоллекцияНоменклатуры", Объект.КоллекцияНоменклатуры);
	
	Оповещение = Новый ОписаниеОповещения("КР_ЗагрузитьТоварыСХарактеристикойИзФайлаЗавершение", ЭтотОбъект);
	ЗагрузкаДанныхИзФайлаКлиент.ПоказатьФормуЗагрузки(ПараметрыЗагрузки, Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура КР_СкопироватьСтрокиСХарактеристикой(Команда)
	
	Если РаботаСТабличнымиЧастямиКлиент.ВыбранаСтрокаДляВыполненияКоманды(Элементы.КР_ТоварыСХарактеристикой) Тогда
		КР_СкопироватьСтрокиСХарактеристикойНаСервере();
		РаботаСТабличнымиЧастямиКлиент.ОповеститьПользователяОКопированииСтрок(
			Элементы.КР_ТоварыСХарактеристикой.ВыделенныеСтроки.Количество());
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КР_ВставитьСтрокиСХарактеристикой(Команда)
	
	КоличествоТоваровДоВставки = Объект.КР_ТоварыСХарактеристикой.Количество();
	
	КР_ПолучитьСтрокиСХарактеристикойИзБуфераОбмена();
	
	КоличествоВставленных = Объект.КР_ТоварыСХарактеристикой.Количество() - КоличествоТоваровДоВставки;
	РаботаСТабличнымиЧастямиКлиент.ОповеститьПользователяОВставкеСтрок(КоличествоВставленных);
	
КонецПроцедуры

&НаКлиенте
Процедура КР_ОткрытьПодборСХарактеристикой(Команда)
...
&НаКлиенте
Процедура КР_УстановитьРольДляВсехТоваровСХарактеристикой(Команда)
	
	Если Не Объект.КР_ТоварыСХарактеристикой.Количество() Тогда
		ТекстПредупреждения = НСтр("ru = 'В документе не заполнена таблица ""Товары с характеристикой"". Роль не может быть заполнена'");
		ПоказатьПредупреждение(Неопределено, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	ОткрытьФорму("Перечисление.РолиАссортимента.ФормаВыбора", ,
		ЭтотОбъект, , , , 
		Новый ОписаниеОповещения("КР_УстановитьРольДляВсехТоваровСХарактеристикойЗавершение", ЭтотОбъект),
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура КР_ДобавитьТоварыСХарактеристикойПоОтбору(Команда)
	...
	ПараметрыФормы = Новый Структура("УникальныйИдентификатор", УникальныйИдентификатор);
	ПараметрыФормы.Вставить("ОтключитьХарактеристики", Ложь);
	Если ИспользоватьУправлениеКоллекциями 
		И Объект.Операция = ПредопределенноеЗначение("Перечисление.ОперацииИзмененияАссортимента.УправлениеКоллекцией") Тогда
		ПараметрыФормы.Вставить("КоллекцияНоменклатуры", Объект.КоллекцияНоменклатуры);
	КонецЕсли;
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("КР_ДобавитьТоварыСХарактеристикойПоОтборуЗавершение", ЭтотОбъект);
...
Процедура КР_ДобавитьЭлементыНаФорму()
	...
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Контейнер", Элементы.ШапкаЛево);
	
	// << 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ДатаЗагрузки", Элементы.ШапкаЛево);
	// >> 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309 
	
	// #3371.. Фомин Д.Ю. 17.11.2023.
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.злДатаЗавершения", Элементы.ШапкаЛево);
	// ..#3371
	
	КР_ГруппаТоварыСХарактеристикой = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "КР_ГруппаТоварыСХарактеристикой", , Элементы.ГруппаКомментарийОтветственный);
	КР_ГруппаТоварыСХарактеристикой.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	КР_ГруппаТоварыСХарактеристикой.Отображение = ОтображениеОбычнойГруппы.Нет;
	КР_ГруппаТоварыСХарактеристикой.ОтображатьЗаголовок = Ложь;
	
	КР_ТоварыСХарактеристикойЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ТоварыСХарактеристикой", КР_ГруппаТоварыСХарактеристикой);
	
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.НомерСтроки", КР_ТоварыСХарактеристикойЭлемент);
	НовыйЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.Номенклатура.Артикул", КР_ТоварыСХарактеристикойЭлемент);
	НовыйЭлемент.Ширина = 10;
	НовыйЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.Номенклатура", КР_ТоварыСХарактеристикойЭлемент);
	НовыйЭлемент.Ширина = 30;
	НовыйЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.Характеристика", КР_ТоварыСХарактеристикойЭлемент);
	НовыйЭлемент.Ширина = 30;
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.РольАссортимента", КР_ТоварыСХарактеристикойЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.Номенклатура.ТоварнаяКатегория", КР_ТоварыСХарактеристикойЭлемент);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТоварыСХарактеристикой.НормаПотребления", КР_ТоварыСХарактеристикойЭлемент);
	
	КР_КомандаСкопироватьСтроки = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_СкопироватьСтрокиСХарактеристикой",
		НСтр("ru = 'Скопировать строки'"), "КР_СкопироватьСтрокиСХарактеристикой");
	КР_КомандаСкопироватьСтроки.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.НеИспользует;
	КР_КомандаСкопироватьСтроки.Картинка = БиблиотекаКартинок.КопированиеСтрок;
	
	КР_КомандаВставитьСтроки = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_ВставитьСтрокиСХарактеристикой",
		НСтр("ru = 'Вставить строки'"), "КР_ВставитьСтрокиСХарактеристикой");
	КР_КомандаВставитьСтроки.ИзменяетСохраняемыеДанные = Истина;
	КР_КомандаВставитьСтроки.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.НеИспользует;
	КР_КомандаВставитьСтроки.Картинка = БиблиотекаКартинок.ВставкаСтрок;
	
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаСкопироватьСтроки, Элементы.КР_ТоварыСХарактеристикойКоманднаяПанель);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаВставитьСтроки, Элементы.КР_ТоварыСХарактеристикойКоманднаяПанель);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаСкопироватьСтроки, Элементы.КР_ТоварыСХарактеристикойКонтекстноеМеню);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаВставитьСтроки, Элементы.КР_ТоварыСХарактеристикойКонтекстноеМеню);
	
	КР_ГруппаЗаполнитьСХарактеристикой = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(
		ЭтотОбъект, "КР_ЗаполнитьСХарактеристикой", Элементы.КР_ТоварыСХарактеристикойКоманднаяПанель);
	КР_ГруппаЗаполнитьСХарактеристикой.Вид = ВидГруппыФормы.Подменю;
	КР_ГруппаЗаполнитьСХарактеристикой.Заголовок = НСтр("ru = 'Заполнить товары'");
	
	КР_КомандаПодборСХарактеристикой = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_ОткрытьПодборСХарактеристикой",
		НСтр("ru = 'Подобрать товары'"), "КР_ОткрытьПодборСХарактеристикой");
	КР_КомандаПодборСХарактеристикой.ИзменяетСохраняемыеДанные = Истина;
	КР_КомандаПодборСХарактеристикой.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.НеИспользует;
	КР_КомандаПодборСХарактеристикой.Картинка = БиблиотекаКартинок.ПодобратьТовары;
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаПодборСХарактеристикой, КР_ГруппаЗаполнитьСХарактеристикой);
	
	КР_КомандаДобавитьТоварыСХарактеристикойПоОтбору = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_ДобавитьТоварыСХарактеристикойПоОтбору",
		НСтр("ru = 'Добавить товары по отбору'"), "КР_ДобавитьТоварыСХарактеристикойПоОтбору");
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаДобавитьТоварыСХарактеристикойПоОтбору, КР_ГруппаЗаполнитьСХарактеристикой);
	
	КР_КомандаУстановитьРольСХарактеристикой = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_УстановитьРольДляВсехТоваровСХарактеристикой",
		НСтр("ru = 'Заполнить роль для всех товаров...'"), "КР_УстановитьРольДляВсехТоваровСХарактеристикой");
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаУстановитьРольСХарактеристикой, КР_ГруппаЗаполнитьСХарактеристикой);
	
	КР_КомандаЗагрузитьИзФайла = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
		ЭтотОбъект, "КР_ЗагрузитьТоварыСХарактеристикойИзФайла",
		НСтр("ru = 'Загрузить из файла...'"), "КР_ЗагрузитьТоварыСХарактеристикойИзФайла");
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КР_КомандаЗагрузитьИзФайла, Элементы.КР_ТоварыСХарактеристикойКоманднаяПанель);
		
	// << 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		КомандаФормы, Элементы.Товары.КоманднаяПанель, Элементы.Заполнить);
	// >> 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
	
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	ИмяЭлемента = "КР_ЗагрузитьДанныеСХарактеристикамиИзТСД";
	ЭлементФормы = Элементы.Вставить(ИмяЭлемента,
		Тип("КнопкаФормы"),
		Элементы.КР_ТоварыСХарактеристикой.КоманднаяПанель,
		Элементы.КР_ЗаполнитьСХарактеристикой);
	ЭлементФормы.ИмяКоманды = КомандаФормы.Имя;
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553

	// << 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057  
	// КР_ЭтоПользовательМагазина
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ЭтоПользовательМагазина", Новый ОписаниеТипов("Булево"));
	// >> 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
...
Процедура КР_УстановитьВидимостиДоступности()
	...
	// << 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	Этапы = Перечисления.ЭтапыИзмененияАссортимента;
	
	ЭтоПереводОтменаСтрогойКонсервации =
		Объект.Этап = Этапы.КР_ПереводНаСтрогуюКонсервацию Или
		Объект.Этап = Этапы.КР_ОтменаСтрогойКонсервацииПоФормату;
	// >> 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	
	ЭтоВводВАссортиментЗапретОтгрузок =
		Объект.Этап = Этапы.ВводВАссортимент Или
		Объект.Этап = Этапы.КР_ЗапретОтгрузок;
		
	ЭтоПриостановкаЗаморозка =
		Объект.Этап = Этапы.КР_Заморозка Или
		Объект.Этап = Этапы.КР_Приостановка;
		
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	ЭтоВременнаяКонсервация =
		Объект.Этап = Этапы.КР_РазблокировкаВременнойКонсервации Или
		Объект.Этап = Этапы.КР_ПереводНаВременнуюКонсервацию;
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
		
	Элементы.КР_Контейнер.Видимость = ЭтоВводВАссортиментЗапретОтгрузок;
	
	// << 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
	ЭтоСнятиеЗаморозки = (Объект.Этап = Этапы.КР_СнятиеЗаморозки);	
	// >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
	
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	Элементы.КР_ГруппаТоварыСХарактеристикой.Видимость = ЭтоПриостановкаЗаморозка Или ЭтоВременнаяКонсервация
		// << 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
		Или ЭтоСнятиеЗаморозки	
		// >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
	;
		
	Элементы.ГруппаТовары.Видимость =
		Не (ЭтоПриостановкаЗаморозка Или ЭтоВременнаяКонсервация
			// << 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
			Или ЭтоСнятиеЗаморозки	
			// >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
		)
		Или Не ЗначениеЗаполнено(Объект.Этап)
		;
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	
	// << 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
	// Установка видимости команды "КР_ЗагрузитьДанныеИзТСД"
	Элементы.КР_ЗагрузитьДанныеИзТСД.Видимость = Объект.Этап = Этапы.КР_РазблокировкаСтрогойКонсервации;
	// >> 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
	
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	Элементы.КР_ЗагрузитьДанныеСХарактеристикамиИзТСД.Видимость = ЭтоВременнаяКонсервация;
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	
	// << 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	Элементы.КР_ДатаЗагрузки.Видимость = ЭтоПереводОтменаСтрогойКонсервации;
	// >> 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	
	Элементы.злДатаЗавершения.Видимость = ЭтоПереводОтменаСтрогойКонсервации;	// #3371.. Фомин Д.Ю. 17.11.2023.
	
КонецПроцедуры

&НаСервере
Процедура КР_ПереопределитьСписокЭтапов()
	
	Этапы = Перечисления.ЭтапыИзмененияАссортимента;
	
	СписокЭтапов = Элементы.Этап.СписокВыбора;
	
	СписокЭтапов.Очистить();   
	
	// << 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
	Если ЭтотОбъект["КР_ЭтоПользовательМагазина"] Тогда   
		СписокЭтапов.Добавить(Этапы.КР_РазблокировкаСтрогойКонсервации, 
			НСтр("ru = 'Разблокировка строгой консервации (разрешены закупки и продажи)'"));
		СписокЭтапов.Добавить(Этапы.КР_РазблокировкаВременнойКонсервации,
			НСтр("ru = 'Разблокировка временной консервации (разрешены закупки и продажи)'")); 
		// << 27.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2426
		СписокЭтапов.Добавить(Этапы.КР_ПереводНаВременнуюКонсервацию); 
		// >> 27.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2426	
		Возврат;
	КонецЕсли;	
	// >> 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
	
	СписокЭтапов.Добавить(Этапы.КР_Приостановка                     , НСтр("ru = 'Приостановка (разрешены только продажи)'"));
	СписокЭтапов.Добавить(Этапы.КР_ЗапретОтгрузок                   , НСтр("ru = 'Запрет отгрузок (запрещены закупки и продажи)'"));
	СписокЭтапов.Добавить(Этапы.КР_Заморозка                        , НСтр("ru = 'Заморозка (разрешены закупки и продажи)'"));
	СписокЭтапов.Добавить(Этапы.КР_ПереводНаСтрогуюКонсервацию      , НСтр("ru = 'Перевод на строгую консервацию (разрешены только закупки)'"));
	СписокЭтапов.Добавить(Этапы.КР_РазблокировкаСтрогойКонсервации  , НСтр("ru = 'Разблокировка строгой консервации (разрешены закупки и продажи)'"));
	СписокЭтапов.Добавить(Этапы.КР_НастройкаСпецраспределения       , НСтр("ru = 'Настройка спецраспределения (разрешены закупки и продажи)'"));
	СписокЭтапов.Добавить(Этапы.ВводВАссортимент                    , НСтр("ru = 'Ввод в ассортимент (разрешены закупки и продажи)'"));
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	СписокЭтапов.Добавить(Этапы.КР_РазблокировкаВременнойКонсервации, НСтр("ru = 'Разблокировка временной консервации (разрешены закупки и продажи)'"));
	СписокЭтапов.Добавить(Этапы.КР_ПереводНаВременнуюКонсервацию    , НСтр("ru = 'Перевод на временную консервацию (разрешены только закупки)'"));
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	// << 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	СписокЭтапов.Добавить(Этапы.КР_ОтменаСтрогойКонсервацииПоФормату, НСтр("ru = 'Отмена строгой консервации по формату (разрешены закупки и продажи)'"));
	// >> 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	
	// << 26.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
	СписокЭтапов.Добавить(Этапы.КР_СнятиеЗаморозки);
	// >> 26.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403

КонецПроцедуры

&НаСервере
Процедура КР_ОбработкаВыбораПодборНаСервереАльтернативно(ВыбранноеЗначение)
	
	Если ЗначениеЗаполнено(ВыбранноеЗначение.АдресТоваровВХранилище) Тогда
		Если Объект.Этап = Перечисления.ЭтапыИзмененияАссортимента.КР_Приостановка
			Или Объект.Этап = Перечисления.ЭтапыИзмененияАссортимента.КР_Заморозка Тогда
			// Для ТЧ КР_ТоварыСХарактеристикой
			КР_ДобавитьТоварыСХарактеристикойИзВременногоХранилищаНаСервере(
				ВыбранноеЗначение.АдресТоваровВХранилище, Истина);
		Иначе
			ДобавитьТоварыИзВременногоХранилищаНаСервере(ВыбранноеЗначение.АдресТоваровВХранилище, Истина);
		КонецЕсли;
	КонецЕсли;
...
#Область КР_РаботаСБуферомОбмена

&НаСервере
Процедура КР_СкопироватьСтрокиСХарактеристикойНаСервере()
	
	РаботаСТабличнымиЧастями.СкопироватьСтрокиВБуферОбмена(
		Объект.КР_ТоварыСХарактеристикой, Элементы.КР_ТоварыСХарактеристикой.ВыделенныеСтроки);
	
КонецПроцедуры

&НаСервере
Процедура КР_ПолучитьСтрокиСХарактеристикойИзБуфераОбмена()
	
	Колонки = "Номенклатура,Характеристика,РольАссортимента";
	
	ТаблицаТоваров = РаботаСТабличнымиЧастями.СтрокиИзБуфераОбмена(, Колонки);
	
	Если Не ЗначениеЗаполнено(ТаблицаТоваров)
		Или ТаблицаТоваров.Колонки.Найти("Номенклатура") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		ПараметрыОтбора = Новый Структура("Номенклатура, Характеристика",
			СтрокаТовара.Номенклатура, СтрокаТовара.Характеристика);
		НайденныеСтроки = Объект.КР_ТоварыСХарактеристикой.НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() = 0 Тогда
			ТекущаяСтрока = Объект.КР_ТоварыСХарактеристикой.Добавить();
			ЗаполнитьЗначенияСвойств(ТекущаяСтрока, СтрокаТовара);
...
&НаСервере
Процедура КР_ЭтапПриИзмененииНаСервереАльтернативно()
	
	СтадииПоЭтапам = КР_СтадииПоЭтапам();
	ТекущаяСтадия = СтадииПоЭтапам.Получить(Объект.Этап);
	Если ТекущаяСтадия = Неопределено Тогда
...
&НаСервереБезКонтекста
Функция КР_СтадииПоЭтапам()
	
	Этапы  = Перечисления.ЭтапыИзмененияАссортимента;
	Стадии = Перечисления.СтадииАссортимента;
	
	СтадииПоЭтапам = Новый Соответствие;
	
	СтадииПоЭтапам.Вставить(Этапы.ВводВАссортимент                      , Стадии.РазрешеныЗакупкиИПродажи);
	СтадииПоЭтапам.Вставить(Этапы.КР_Заморозка                          , Стадии.РазрешеныЗакупкиИПродажи);
	СтадииПоЭтапам.Вставить(Этапы.КР_НастройкаСпецраспределения         , Стадии.РазрешеныЗакупкиИПродажи);
	СтадииПоЭтапам.Вставить(Этапы.КР_РазблокировкаСтрогойКонсервации    , Стадии.РазрешеныЗакупкиИПродажи);
	СтадииПоЭтапам.Вставить(Этапы.ИзменениеСостоянияАссортимента        , Стадии.РазрешеныЗакупкиИПродажи);
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	СтадииПоЭтапам.Вставить(Этапы.КР_РазблокировкаВременнойКонсервации  , Стадии.РазрешеныЗакупкиИПродажи);
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	// << 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	СтадииПоЭтапам.Вставить(Этапы.КР_ОтменаСтрогойКонсервацииПоФормату  , Стадии.РазрешеныЗакупкиИПродажи);
	// >> 26.09.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-2309
	
	СтадииПоЭтапам.Вставить(Этапы.ВводВАссортиментСОтсрочкойНачалаПродаж, Стадии.РазрешеныТолькоЗакупки);
	СтадииПоЭтапам.Вставить(Этапы.КР_ПереводНаСтрогуюКонсервацию        , Стадии.РазрешеныТолькоЗакупки);
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	СтадииПоЭтапам.Вставить(Этапы.КР_ПереводНаВременнуюКонсервацию      , Стадии.РазрешеныТолькоЗакупки);
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	
	СтадииПоЭтапам.Вставить(Этапы.ВыводИзАссортимента                   , Стадии.РазрешеныТолькоПродажи);
	СтадииПоЭтапам.Вставить(Этапы.КР_Приостановка                       , Стадии.РазрешеныТолькоПродажи);
	
	СтадииПоЭтапам.Вставить(Этапы.ПереносВАрхивОтзыв                    , Стадии.ЗапрещеныЗакупкиИПродажи);
	СтадииПоЭтапам.Вставить(Этапы.КР_ЗапретОтгрузок                     , Стадии.ЗапрещеныЗакупкиИПродажи);
	
	// << 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
	СтадииПоЭтапам.Вставить(Этапы.КР_СнятиеЗаморозки					, Стадии.РазрешеныЗакупкиИПродажи);
	// >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2403
	
	Возврат СтадииПоЭтапам;
	
КонецФункции

&НаКлиенте
Процедура КР_УстановитьРольДляВсехТоваровСХарактеристикойЗавершение(
    ...
    Если ВыбраннаяРоль = Неопределено Тогда
        Возврат;
    КонецЕсли;
    КР_УстановитьЗначениеРеквизитаВсемСтрокамСХарактеристикой("РольАссортимента", ВыбраннаяРоль);

КонецПроцедуры

&НаСервере
Процедура КР_УстановитьЗначениеРеквизитаВсемСтрокамСХарактеристикой(ИмяРеквизита, УстанавливаемоеЗначение)
	
	Для Каждого СтрокаТоваров Из Объект.КР_ТоварыСХарактеристикой Цикл
		СтрокаТоваров[ИмяРеквизита] = УстанавливаемоеЗначение;
	КонецЦикла;
...
&НаКлиенте
Процедура КР_ЗагрузитьТоварыСХарактеристикойИзФайлаЗавершение(
			АдресЗагруженныхДанных, ДополнительныеПараметры) Экспорт
	
	Если АдресЗагруженныхДанных = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	КР_ЗагрузитьТоварыСХарактеристикойИзФайлаНаСервере(АдресЗагруженныхДанных);
	
КонецПроцедуры

&НаСервере
Процедура КР_ЗагрузитьТоварыСХарактеристикойИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		НоваяСтрокаТовары = Объект.КР_ТоварыСХарактеристикой.Добавить();
		НоваяСтрокаТовары.Номенклатура = СтрокаТаблицы.Номенклатура;
...
&НаКлиенте
Процедура КР_ДобавитьТоварыСХарактеристикойПоОтборуЗавершение(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		КР_ДобавитьТоварыСХарактеристикойИзВременногоХранилищаНаСервере(Результат, Истина);
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура КР_ДобавитьТоварыСХарактеристикойИзВременногоХранилищаНаСервере(АдресВоВременномХранилище, ДополнятьТаблицуТоваров)
	
	Если ДополнятьТаблицуТоваров Тогда
		ТаблицаТоваров = ПолучитьИзВременногоХранилища(АдресВоВременномХранилище);
		ТаблицаТоваровДополненная = КР_ТаблицаТоваровСХарактеристикойДополненнаяАссортиментомОбъекта(ТаблицаТоваров);
	Иначе
		ТаблицаТоваровДополненная = ПолучитьИзВременногоХранилища(АдресВоВременномХранилище);
	КонецЕсли;
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваровДополненная Цикл
		...
		НайденныеСтроки = Объект.Товары.НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() = 0 Тогда
			НоваяСтрока = Объект.КР_ТоварыСХарактеристикой.Добавить();
...
// Функция дополняет таблицу товаров с характеристикой данными регистра "Ассортимент"
// для текущего формата магазинов установленными на дату начала действия
//
// Параметры: 
//  	ТаблицаТоваров - ТаблицаЗначений - таблица подобранных товаров.
//
&НаСервере
Функция КР_ТаблицаТоваровСХарактеристикойДополненнаяАссортиментомОбъекта(ТаблицаТоваров)
	
	РозничныйВидЦены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ОбъектПланирования, "РозничныйВидЦены");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаТоваров", ТаблицаТоваров);
	Запрос.УстановитьПараметр("НаДату", КонецДня(Объект.ДатаНачалаДействия));
	Запрос.УстановитьПараметр("ОбъектПланирования", Объект.ОбъектПланирования);
	Запрос.УстановитьПараметр("ВидЦенФормата", РозничныйВидЦены);
	Запрос.Текст =
	"ВЫБРАТЬ
...
Процедура КР_ЗагрузитьДанныеИзТСД(Команда)
	...
	Если ДаннныеТСДДляРежимаОтладки <> Неопределено Тогда
		...
		КР_ОбработатьШтрихкоды(ДаннныеТСДДляРежимаОтладки);
		Возврат;
	КонецЕсли;
	
	МенеджерОборудованияКлиент.НачатьЗагрузкуДанныеИзТСД(
		Новый ОписаниеОповещения("КР_ЗагрузитьИзТСДЗавершение", ЭтотОбъект),
		УникальныйИдентификатор);

КонецПроцедуры // >> 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260

// << 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
&НаКлиенте
Процедура КР_ЗагрузитьИзТСДЗавершение(РезультатВыполнения, Параметры) Экспорт
	
	Если РезультатВыполнения.Результат Тогда
		КР_ОбработатьШтрихкоды(РезультатВыполнения.ТаблицаТоваров);
	Иначе
...
// << 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
&НаКлиенте
Процедура КР_ОбработатьШтрихкоды(ДанныеШтрихкодов)
	
	КР_ОбработатьШтрихкодыНаСервере(ДанныеШтрихкодов);
	
КонецПроцедуры // >> 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260

// << 27.03.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
&НаСервере
Процедура КР_ОбработатьШтрихкодыНаСервере(ДанныеШтрихкодов)
	
	// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	ЭтоВременнаяКонсервация =
		Объект.Этап = Перечисления.ЭтапыИзмененияАссортимента.КР_РазблокировкаВременнойКонсервации
		Или Объект.Этап = Перечисления.ЭтапыИзмененияАссортимента.КР_ПереводНаВременнуюКонсервацию;
		
	ПоХарактеристикам = ЭтоВременнаяКонсервация;
	ИмяТЧ = ?(ПоХарактеристикам, "КР_ТоварыСХарактеристикой", "Товары");
	// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
	
	СоответствиеНоменклатурыШтрихкодам = КР_РаботаСТСД.ПолучитьСоответствиеЭлементовШтрихкодам(ДанныеШтрихкодов,
		"Номенклатура");
	
	Если Объект.Этап = Перечисления.ЭтапыИзмененияАссортимента.КР_РазблокировкаСтрогойКонсервации
		// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
		Или ЭтоВременнаяКонсервация
		// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
		Тогда
		РольАссортимента = Перечисления.РолиАссортимента.ДополнительныйАссортимент;
	Иначе
		РольАссортимента = Перечисления.РолиАссортимента.ПостоянныйАссортимент;
	КонецЕсли;

	ВидЦены = КР_ДополнительныеНастройкиПовтИсп.Значение("ВидЦеныРозничный");
	
	Для Каждого ТекущееСоответствие Из СоответствиеНоменклатурыШтрихкодам Цикл
		
		Если ТекущееСоответствие.Значение.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка") Тогда
			Продолжить;
		КонецЕсли;
		
		ОтборСтрок = Новый Структура;
		ОтборСтрок.Вставить("Номенклатура", ТекущееСоответствие.Значение.Номенклатура);
		// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
		Если ПоХарактеристикам Тогда
			ОтборСтрок.Вставить("Характеристика", ТекущееСоответствие.Значение.Характеристика);
		КонецЕсли;
		// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
		
		// Добавим строки номенклатуры, если её нет в табличной части
		// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
			НоваяСтрока.Номенклатура 		= ТекущееСоответствие.Значение.Номенклатура;
			НоваяСтрока.РольАссортимента 	= РольАссортимента;
			// << 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
			//НоваяСтрока.ВидЦены 			= ВидЦены;
			Если ПоХарактеристикам Тогда
				НоваяСтрока.Характеристика 	= ТекущееСоответствие.Значение.Характеристика;
			Иначе
				НоваяСтрока.ВидЦены 		= ВидЦены;
			КонецЕсли;
			// >> 25.04.2023, Гашников А.П., КРОК, Jira№ A2105505-1553
...
// << 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
&НаКлиенте
Процедура КР_ТоварыСХарактеристикойПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
...
КонецПроцедуры // >> 13.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2057
