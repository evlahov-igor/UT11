...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	СУММА(ДанныеТабличнойЧастиТовары.Сумма) + СУММА(ДанныеТабличнойЧастиКассы.Сумма) +
	|		СУММА(ДанныеТабличнойЧастиБанковскиеСчета.Сумма) + СУММА(ДанныеТабличнойЧастиКассыККМ.Сумма) +
	|		СУММА(ДанныеТабличнойЧастиРасчетыМеждуОрганизациями.Сумма) + СУММА(ДанныеТабличнойЧастиРасчетыСПартнерами.Сумма) +
	|		СУММА(ДанныеТабличнойЧастиРасчетыСПодотчетниками.Сумма) + СУММА(ДанныеТабличнойЧастиРасчетыПоЭквайрингу.Сумма) +
	|		СУММА(ДанныеТабличнойЧастиПрочиеАктивыПассивы.Сумма) +
	|		СУММА(ДанныеТабличнойЧастиПодарочныеСертификаты.СуммаВВалютеСертификата) +
	|		СУММА(ДанныеТабличнойЧастиРасчетыПоФинансовымИнструментам.Сумма) + СУММА(ДанныеТабличнойЧастиТМЦВЭксплуатации.Сумма)
	|		+ СУММА(ДанныеТабличнойЧастиПрочиеРасходы.Сумма) +
	|		СУММА(ДанныеТабличнойЧастиФинансовыйРезультатРасходы.СуммаРасходов) +
	|		СУММА(ДанныеТабличнойЧастиФинансовыйРезультатДоходы.СуммаДоходов) + СУММА(ДанныеТабличнойЧастиОптовыеПродажи.Сумма)
	|		+ СУММА(ДанныеТабличнойЧастиРозничныеПродажи.Сумма) + СУММА(ДанныеТабличнойЧастиНДСПредъявленный.СуммаБезНДС) +
	|		СУММА(ДанныеТабличнойЧастиНДСПредъявленный.НДС) + СУММА(ДанныеТабличнойЧастиРасходыПриУСН.СуммаРасходов) КАК Сумма,
	|	ВЫРАЗИТЬ(ДанныеДокумента.Комментарий КАК СТРОКА(100)) КАК Комментарий,
...
Процедура ЗаполнитьОбъектыРасчетов(Объект, РежимЗаписи, ГотовКЗаполнению = Ложь) Экспорт
			...
			Если ЗначениеЗаполнено(ИсточникОбъектаРасчетов) Тогда
				...
				Если ТипЗнч(ИсточникОбъектаРасчетов) = Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациями") 
					...
					Если ОтчетКомитент = Объект.Организация И ОтчетКомиссионер = Объект.ОрганизацияПолучатель Тогда
						...
						ОбъектыРасчетовСервер.ОбъектГотовКЗаполнению(
							СтрокаТЧ.ОбъектРасчетовОтправитель,
							ИсточникОбъектаРасчетов,
							ГотовКЗаполнению);
						...
						ОбъектыРасчетовСервер.ОбъектГотовКЗаполнению(
							СтрокаТЧ.ОбъектРасчетовПолучатель,
							ИсточникОбъектаРасчетов,
							ГотовКЗаполнению);
				...
				ИначеЕсли ТипЗнч(ИсточникОбъектаРасчетов) = Тип("ПеречислениеСсылка.ТипыДоговоровМеждуОрганизациями") Тогда
					Если Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийВводаОстатков.ОстаткиРасчетовМеждуОрганизациямиПоАвансам
						...
						Если НеобходимоСоздатьЭлемент Тогда
							...
							ДоговорМеждуОрганизациями.ОбменДанными.Загрузка = Истина;							
							Попытка
	...
	Для Каждого СтрокаТЧ Из Объект.РасчетыСПартнерами Цикл
		...
		Если СтрокаТЧ.ОбъектРасчетовОтправитель = Справочники.ОбъектыРасчетов.ПустаяСсылка() Тогда
			...
			Если ЗначениеЗаполнено(ИсточникОбъектаРасчетов)
				...
				И Не ОбъектыРасчетовСервер.ЕстьБитыеСсылкиВИсточникеОбъектаРасчетов(ИсточникОбъектаРасчетов) Тогда
				Если Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийВводаОстатков.ОстаткиЗадолженностиПередПоставщиками
					Или Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам Тогда
						ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком;
				ИначеЕсли Объект.УдалитьТипОперации = Перечисления.УдалитьТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов 
			...
			ИначеЕсли Не ОбъектыРасчетовСервер.ЕстьБитыеСсылкиВИсточникеОбъектаРасчетов(ИсточникОбъектаРасчетов)
				И (Не ЗначениеЗаполнено(ИсточникОбъектаРасчетов)
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет документы ""Ввод начальных остатков"":
	|- заполняет реквизиты ""Объект расчетов (отправитель)"" и ""Объект расчетов (получатель)"".
	|- заполняет реквизиты ""Хозяйственная операция"" и ""Ставка НДС"".
	|- заполняет реквизит ""Договор эквайринга"".
	|- заполняет реквизит ""Партия"" для расходов по УСН.
	|- заполняет реквизит ""Партия"" для ТМЦ в эксплуатации.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "РасчетыСПартнерами.ОбъектРасчетов");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "РасчетыСПартнерами.Партнер");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "РасчетыСПартнерами.Контрагент");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "РасчетыСПартнерами.Договор");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "РасчетыМеждуОрганизациями.ОбъектРасчетов");
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.ВводОстатков.НДСПредъявленный КАК ВводОстатковНДСПредъявленный
	|		ГДЕ
	|			ВводОстатков.Ссылка = ВводОстатковНДСПредъявленный.Ссылка
	|			И ВводОстатковНДСПредъявленный.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ВводОстатковНДСПредъявленный.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|ГДЕ
	|	РасчетыПоЭквайрингу.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковДенежныхСредствКПоступлениюОтЭквайера)
	|	И РасчетыПоЭквайрингу.ДоговорЭквайринга = ЗНАЧЕНИЕ(Справочник.ДоговорыЭквайринга.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Документ.ВводОстатков КАК ДанныеДокумента
	|ГДЕ
	|	(ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковТМЦВЭксплуатации)
	|		ИЛИ ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
	|			И ДанныеДокумента.УдалитьТипОперации = ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийВводаОстатков.ОстаткиТМЦВЭксплуатации))
	|
	|	ИЛИ ИСТИНА В (
	|			ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатков.ТМЦВЭксплуатации КАК ТабличнаяЧасть
	...
	Выборка = Результат.Выбрать();
 	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект(); // ДокументОбъект.ВводОстатков
			ОбъектИзменен = Ложь;
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				|	И ВводОстатков.ХозяйственнаяОперация В (
				|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковСобственныхТоваров),
				|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковТоваровПереданныхНаКомиссию),
				|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковВозвратнойТарыПринятойОтПоставщиков),
				|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковМатериаловПереданныхВПроизводство),
				...
				МассивТЧ.Добавить("НДСПредъявленный");
				МассивТЧ.Добавить("ДетализацияПартий");
				...
				Если ДокументОбъект.Проведен Тогда
					...
					ЗаполнитьОбъектыРасчетов(ДокументОбъект, РежимЗаписиДокумента.Запись, ГотовКЗаполнению);
...
Функция ТекстыЗапросаРаспределениеЗапасовДвиженияДляОбновленияИБ() Экспорт
	...
	ТекстЗапросаТабЧасть =
		...
		|ГДЕ
		|	ТабЧасть.Ссылка.ОтражатьВОперативномУчете
		|		И ТабЧасть.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка)
		|		И ТабЧасть.Ссылка.УдалитьТипОперации В(
		|			ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров),
		|			ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийВводаОстатков.ОстаткиТоваровПолученныхНаКомиссию),
		|			ЗНАЧЕНИЕ(Перечисление.УдалитьТипыОперацийВводаОстатков.ОстаткиВозвратнойТарыПринятойОтПоставщиков))";
