...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"" ссылкой на справочник ""Ставки НДС""
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|					ИСТИНА
	|				ИЗ
	|					Документ.ЗаданиеТорговомуПредставителю.ЭтапыГрафикаОплаты КАК ТЧЭтапыГрафикаОплаты
	|				ГДЕ
	|					ЗаданиеТорговомуПредставителю.Ссылка = ТЧЭтапыГрафикаОплаты.Ссылка
	...
	|							И ТЧЭтапыГрафикаОплаты.Сдвиг <>
	|							ВЫБОР КОГДА ЗаданиеТорговомуПредставителю.ЖелаемаяДатаОтгрузки = ДАТАВРЕМЯ(1,1,1)
	|									ТОГДА 
	|										ВЫБОР КОГДА РАЗНОСТЬДАТ(ЗаданиеТорговомуПредставителю.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ) < 0
	|											ТОГДА 0
	|											ИНАЧЕ РАЗНОСТЬДАТ(ЗаданиеТорговомуПредставителю.Дата, ТЧЭтапыГрафикаОплаты.ДатаПлатежа, ДЕНЬ)
	|										КОНЕЦ
	|									ИНАЧЕ ВЫБОР КОГДА РАЗНОСТЬДАТ(ЗаданиеТорговомуПредставителю.ЖелаемаяДатаОтгрузки, ТЧЭтапыГрафикаОплаты.ДатаПлатежа,ДЕНЬ) < 0
	|											ТОГДА 0
	|											ИНАЧЕ РАЗНОСТЬДАТ(ЗаданиеТорговомуПредставителю.ЖелаемаяДатаОтгрузки, ТЧЭтапыГрафикаОплаты.ДатаПлатежа,ДЕНЬ)
	|										КОНЕЦ
	|							КОНЕЦ))";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ДокументОбъект.ЖелаемаяДатаОтгрузки) Тогда
					ДатаОтгрузки = ДокументОбъект.ЖелаемаяДатаОтгрузки;
				Иначе
