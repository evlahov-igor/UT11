...
Процедура ОбъектРасчетовБезРазбиенияОчистка(Элемент, СтандартнаяОбработка)
	...
	Элементы.ВалютаВзаиморасчетовБезРазбиения.ТолькоПросмотр = Ложь;
...
Процедура РасходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		СтрокаТаблицы.Валюта = Объект.ДоходыРасходыАктивыПассивы[0].Валюта;
	КонецЕсли;
	Если НоваяСтрока И НЕ Копирование Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ЗадолженностьВалютаВзаиморасчетов.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ЗадолженностьПартнер.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Задолженность.ОбъектРасчетов");
...
Процедура ЗадолженностьОбъектРасчетовОбработкаВыбораНаСеревере(ОбъектРасчетов, НомерТекущейСтроки)
	...
	Элементы.ВалютаВзаиморасчетовБезРазбиения.ТолькоПросмотр = Истина;
...
Процедура ЗаполнитьДанныеВыбораОбъектаРасчетов(Текст, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		НастройкиПодбора.Вставить("Контрагент", Объект.Контрагент);
		НастройкиПодбора.Вставить("Организация", Объект.Организация);
		НастройкиПодбора.Вставить("Текст", Текст);
...
Функция ОбновлениеНеЗавершено()
	...
	Возврат НЕ Константы.ОтложенноеОбновлениеЗавершеноУспешно.Получить();
