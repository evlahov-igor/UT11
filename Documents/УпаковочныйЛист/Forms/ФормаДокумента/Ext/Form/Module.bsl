...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Дополнительная обработка после записи на сервере
...
Процедура ВидПриИзмененииСервер()
	...
	// << 08.08.2022, Глухов А.Н., КРОК, Jira№ A2105505-314
	// Настройка формы при изменении вида документа
...
// << 08.08.2022, Глухов А.Н., КРОК, Jira№ A2105505-314
// Дополнительная обработка при чтении на сервере
&НаСервере
Процедура КР_ПриЧтенииСозданииНаСервереДополнительно()
	
	// 22.09.2022 Марченко С.Н., КРОК, JIRA№A2105505-487
	// Рефакторинг
	
	// КР_Партнер
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Партнер", Элементы.ШапкаЛево);
	
	// КР_ДокументОснование
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ДокументОснование", Элементы.ШапкаЛево);
	
	// КР_ТипНаполнения
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ТипНаполнения", Элементы.ШапкаПраво);
	
	Элементы.ТоварыХарактеристика.УстановитьДействие("ПриИзменении", "КР_ТоварыХарактеристикаПриИзменении");
	
	// КР_Штрихкод                            
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200), "Объект.Товары", НСтр("ru = 'Штрихкод'"));
	...
	// КР_ШтрихкодКороба
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Штрихкод", Элементы.ШапкаПраво);
	// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
	...
	// злНомерОтбора
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
						"Объект.злНомерОтбора", Элементы.ГруппаКодДата);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.РастягиватьПоГоризонтали = Ложь;
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.Ширина = 12;
	ЭлементФормы.Подсказка = НСтр("ru = 'Номер получен из документа ""Отбор (размещение) товаров""'");
...
#Область КР_ОбработчикиТаблицыФормыТовары

&НаКлиенте
Процедура КР_ТоварыХарактеристикаПриИзменении(Элемент)
...
Процедура КР_УправлениеВидимостьюДополнительно()
	...
	ЭтоВходящийВидДокумента = (Объект.Вид = Перечисления.ВидыУпаковочныхЛистов.Входящий);
	
	Элементы.КР_Партнер.Видимость = ЭтоВходящийВидДокумента;
	Элементы.КР_Партнер.ОтметкаНезаполненного = ЭтоВходящийВидДокумента;
    Элементы.КР_ТипНаполнения.Видимость = ЭтоВходящийВидДокумента;
	Элементы.КР_ТипНаполнения.ОтметкаНезаполненного = ЭтоВходящийВидДокумента;
	Элементы.СкладУпаковки.ОтметкаНезаполненного = Не ЭтоВходящийВидДокумента;
	
	// << 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
	// Реквиит видимт только для Исходящих документов
	Элементы.КР_Штрихкод.Видимость = Не ЭтоВходящийВидДокумента;
	// >> 18.05.2023, Федоров Д.Е., КРОК, Jira№A2105505-1664
