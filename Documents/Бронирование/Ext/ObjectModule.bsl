...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ОбработкаТабличнойЧастиСервер.ОбработатьТЧ(Операции, СтруктураДействий, КэшированныеЗначения);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КонецЕсли;
	Если Операции.Найти(Перечисления.ТипыОперацийСБилетами.Возврат, "ТипОперации") <> Неопределено Тогда
...
Процедура ПроверитьЗапретИзмененияДанных(Отказ)
	...
	Если ДатыЗапретаИзменения.НайденЗапретИзмененияДанных(ДанныеДляПроверки, ОписаниеДанных, ОписаниеОшибки) Тогда
		ОбщегоНазначения.СообщитьПользователю(ОписаниеОшибки, ЭтотОбъект,,, Отказ);
	КонецЕсли;
...
Процедура ПроверитьДатыПоездки(Отказ)
	...
	Если ДатаПрибытия < ДатаОтправления Тогда
		...
		ТекстСообщения = СтрШаблон(Нстр("ru = 'Значение в поле ""%1"" меньше значения ""%2""'"),
