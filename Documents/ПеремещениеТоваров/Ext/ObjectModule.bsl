...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КР_ДополнительныеДействияВМагазине.ПроверитьЗаблокироватьЗаписьДокументаВМагазине(ЭтотОбъект, Отказ, "СкладОтправитель");
	// >> 16.05.2023 Федотов А.М., КРОК, Jira№A2105505-1657
...
Процедура ПриЗаписи(Отказ)
	...
	// << 12.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1947  
	КР_ПриЗаписи(Отказ);
	// >> 12.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1947
...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	// << 17.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	КР_ЗаполнитьПоЗаказу(СтруктураЗаполнения);
	// >> 17.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
...
Процедура ЗаполнитьДокументНаОснованииАктаОРасхожденияхПослеПеремещения(Знач ДанныеЗаполнения)
	...
	|	ПеремещениеТоваров.УдалитьЕстьАлкогольнаяПродукцияДляРеализацииЕГАИС,
...
Процедура ЗаполнитьНазначениеВидовЗапасов()
	Экспорт // << 19.09.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1448
...
Функция ПараметрыЗаполненияВидовЗапасов(ИмяПоляОрганизация)
	Экспорт // << 21.09.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1448
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ОснованиеАктОРасхожденияхПослеПеремещения = 
		(ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения")); 
	
	Если Не СкладОтправитель.КР_СкладСПомещениямиБезордернойОтгрузки 
		Или ОснованиеАктОРасхожденияхПослеПеремещения Тогда    
		МассивНепроверяемыхРеквизитов.Добавить("КР_ПомещениеОтправителя");
	КонецЕсли;

	Если Не СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки 
		Или ОснованиеАктОРасхожденияхПослеПеремещения Тогда    
		МассивНепроверяемыхРеквизитов.Добавить("КР_ЗонаОтгрузки");
	КонецЕсли;
...
Процедура КР_ПриЗаписи(Отказ)
	...
	// Удалим не актуальную запись в РС приемка перемещений товаров
	Если Не Проведен Тогда 
		Запись = РегистрыСведений.КР_ПриемкаПеремещенийТоваров.СоздатьМенеджерЗаписи();
		Запись.Накладная = Ссылка;
		Запись.Прочитать();
...
КонецПроцедуры // >> 12.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1947

// << 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
Процедура КР_ЗаполнитьПоЗаказу(СтруктураЗаполнения) 

	Если СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки
		И ТипЗнч(ДокументОснование) <> Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения") Тогда    
		
		КР_ПомещениеОтправителя = Справочники.СкладскиеПомещения.КР_СкладскоеПомещениеСкладМагазина(
			СкладОтправитель);
		
		КР_ЗонаОтгрузки = Справочники.СкладскиеЯчейки.КР_СкладскаяЯчейкаОтгрузкиПоУмолчанию(
			СкладОтправитель, КР_ПомещениеОтправителя);
		
	КонецЕсли;
	
	Если СкладОтправитель.КР_ВидСклада = Перечисления.КР_ВидыСкладов.Магазин Тогда 
		Товары.Очистить();
	КонецЕсли;
