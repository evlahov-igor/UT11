...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
Процедура ПриОткрытии(Отказ)
	...
	// --> Евлахов Игорь Николаевич (Начало)
	// Задача #3407
	злПриОткрытииДополнительно(Отказ);
		
	// Задача #3069
	злСобытияФормОценкаПроизводительностиКлиент.ПриОткрытии(ЭтотОбъект, Отказ);
	// <-- Евлахов Игорь Николаевич (Конец)
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	КР_ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи);
	// >> 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
...
Процедура СкладОтправительПриИзменении(Элемент)
	...
	// << 09.08.2022, Глухов А.Н., КРОК, Jira№ A2105505-352
	КР_УправлениеВидимостьюДополнительно();
	// >> 09.08.2022, Глухов А.Н., КРОК, Jira№ A2105505-352
	
	// << 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	КР_ЗаполнитьПомещениеОтправителя();
	// >> 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751  
...
Процедура ОткрытьПодборТоваровПоЗаказамОрдерам()
	...
	// Адрес заполним чуть ниже так как возможно ТЧ нужно будет очищать
	...
	// << 05.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-751  
	Если КР_ЭтоАдресныйСкладБезордернойОтгрузки(Объект.СкладОтправитель) Тогда 
		
		// Не обязательный параметр, но должен в значении иметь тип Булево
		ПараметрыФормы.Вставить("КР_ЭтоАдресныйСкладБезордернойОтгрузки", Истина);
		...
		Если ЗначениеЗаполнено(Объект.Товары) Тогда 
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("КР_ОткрытьПодборТоваровПоЗаказамОрдерамЗавершение",
	...
	// Типовое заполнение которое закомментированно выше
	ПараметрыФормы.Вставить("АдресТовары", АдресТоварыНакладной());
	// >> 05.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
...
Функция АдресТоварыНакладной()
	...
	// << 05.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	// Ошибка 1С. Значение хранилища теряется пока передается с клиента в ОткрытьФорму...
	// Ошибка не стабльная и понятно почему
	// По хорошему надо бы реквизит формы создать с адресом временного хранилища и использовать его 
	//	но лучше чем специалисты 1С кодить нельзя
	, УникальныйИдентификатор	
	// >> 05.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	);
...
Процедура КР_ПриСозданииНаСервереДополнительно()
	...
	// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	Элементы.КР_ГруппаДатаПотоварнойПриемки.Видимость = ЗначениеЗаполнено(Объект.Ссылка);
	// >> 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	
	// << 15.05.2023 Марченко С.Н., КРОК, JIRA№A2105505-1651     
	
	// КР_ИдентификаторСистемыИсточника
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ИдентификаторСистемыИсточника", , Элементы.ГруппаДополнительныеРеквизиты);
	
	// КР_НомерВходящегоДокумента
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_НомерВходящегоДокумента", , Элементы.ГруппаДополнительныеРеквизиты);
	// >> 15.05.2023 Марченко С.Н., КРОК, JIRA№A2105505-1651
			
КонецПроцедуры // >> 09.08.2022, Глухов А.Н., КРОК, Jira№ A2105505-352
...
Процедура КР_ПриЧтенииСозданииНаСервере()   
	...
	// --> Евлахов Игорь Николаевич (Начало) 06.12.2023
	// Задача #3407
	КомандаАнонсПоставки = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(
						ЭтотОбъект, 
						"злКомандаАнонсПоставки", 
						НСтр("ru = 'Анонс поставки'"), 
						"Подключаемый_злАнонсПоставкиEsolutions");
	// <-- Евлахов Игорь Николаевич (Конец) 06.12.2023
	...
	// КР_ГруппаДатаПотоварнойПриемки	
	ГруппаДатаПотоварнойПриемки = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект,
		"КР_ГруппаДатаПотоварнойПриемки", Элементы.СтраницаДополнительноПраво);     
	КР_МетодыМодификацииФорм.УстановитьСвойстваОбычнаяГруппаГоризонтальноБезОтображения(
		ГруппаДатаПотоварнойПриемки);
	ГруппаДатаПотоварнойПриемки.РазрешитьИзменениеСостава = Ложь;
	
	// КР_ДатаПотоварнойПриемки 
	РеквизитФормы = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект,
		"КР_ДатаПотоварнойПриемки", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя), ,
		НСтр("ru = 'Потоварная приемка завершена'"));
		             
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		РеквизитФормы, ГруппаДатаПотоварнойПриемки);   
	ЭлементФормы.ТолькоПросмотр = Истина;  
	
	// КР_УстановитьДатуПотоварнойПриемки
	КомандаФормы = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект,
		"КР_УстановитьДатуПотоварнойПриемки", 
		НСтр("ru = 'Установить'"), "КР_УстановитьДатуПотоварнойПриемки");
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		КомандаФормы, ГруппаДатаПотоварнойПриемки);   
			
	КР_ДатаПотоварнойПриемкиОбновить();	  
	
	// << 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	
	// ДокументОснование
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.ДокументОснование", , Элементы.Шапка); 
	...
	ЭлементФормы.МаксимальнаяШирина = 60; 
	
	// КР_ПомещениеОтправителя
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ПомещениеОтправителя", , Элементы.СкладПолучатель);   
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 27; 
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ПомещениеОтправителяПриИзменении");
	
	// КР_ЗонаОтгрузки
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ЗонаОтгрузки", , Элементы.СкладПолучатель);
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 27; 
	
	Если ЗначениеЗаполнено(Объект.СкладОтправитель) 
		И Не ЗначениеЗаполнено(Объект.КР_ПомещениеОтправителя) Тогда 
		КР_ЗаполнитьПомещениеОтправителя();
	КонецЕсли;	
	// >> 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, "Объект.КР_ЗакрытиеОтрицательныхОстатков", , Элементы.Шапка);
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	// >> 07.04.2023, Маскаев П.Ю., КРОК, JIRA№ A2105505-1448
	
	// --> Евлахов Игорь Николаевич (Начало) 06.12.2023
	// Задача #3407 
	ТипРеквизита = Новый ОписаниеТипов("СправочникСсылка.Склады");
	НовыйРеквизит = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект, "злСкладEsolutions", ТипРеквизита);
	
	// Заполним значением новый реквизит формы
	ЭтотОбъект.злСкладEsolutions = КР_ДополнительныеНастройкиПовтИсп.СкладEsolutions();
	
	ТипРеквизита = Новый ОписаниеТипов("ПеречислениеСсылка.КР_СтатусыАнонсаПоставкиДляСистемыEsolutions"); 
	НовыйРеквизит = КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(
						ЭтотОбъект, 
						"злСтатусEsolutions", 
						ТипРеквизита,, 
						НСтр("ru = 'Статус eSolutions'"));
																				
	НовыйЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
						ЭтотОбъект, 
						НовыйРеквизит, 
	...
	НовыйЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
						ЭтотОбъект, 
						КомандаАнонсПоставки, 
						Элементы.ФормаГруппаКомандыФормы);
	// <-- Евлахов Игорь Николаевич (Конец) 06.12.2023
    #КонецОбласти
	
КонецПроцедуры // >> 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702

// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаСервере
Процедура КР_ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	
	Элементы.КР_ГруппаДатаПотоварнойПриемки.Видимость = Истина; 
	КР_ДатаПотоварнойПриемкиОбновить();
...
// Задача #3407
&НаКлиенте
Процедура злПриОткрытииДополнительно(Отказ)
	
	ПодключитьОбработчикОжидания("злЗаполнитьСтатусEsolutions", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура злПослеЗаписиДополнительно()
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_ПомещениеОтправителяПриИзменении(Элемент) 
	
	КР_ПомещениеОтправителяПриИзмененииНаСервере();	
...
// Задача #3407	
&НаКлиенте
Процедура Подключаемый_злАнонсПоставкиEsolutions(Команда)
		
	ВходящиеДанные = злПроверитьВозможностьФормированияАнонсаПоставки();
	...
	Если Модифицированность Или Объект.Ссылка.Пустая() Тогда
		ПоказатьВопрос(
			Новый ОписаниеОповещения("злАнонсПоставкиEsolutionsПродолжение", ЭтотОбъект),
	...
	злАнонсПоставкиEsolutionsНаСервере(Объект.Ссылка);
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаКлиенте
Процедура КР_УстановитьДатуПотоварнойПриемки(Команда) 
	
	КР_УстановитьДатуПотоварнойПриемкиНаСервере();
...
// Задача #3407
&НаКлиенте
Процедура злАнонсПоставкиEsolutionsПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	
	ОписаниеОповещения = Новый ОписаниеОповещения("злАнонсПоставкиEsolutionsЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура злАнонсПоставкиEsolutionsЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
// << 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
&НаКлиенте
Процедура КР_ОткрытьПодборТоваровПоЗаказамОрдерамЗавершение(Результат, ПараметрыФормы) Экспорт 
...
КонецПроцедуры // >> 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751  
...
// Задача #3407
&НаКлиенте
Процедура злЗаполнитьСтатусEsolutions()
	
	Если Не Объект.Ссылка.Пустая() И Объект.СкладПолучатель = ЭтотОбъект.злСкладEsolutions Тогда
		ЭтотОбъект.злСтатусEsolutions = злПолучитьСтатусEsolutionsНаСервере(Объект.Ссылка);
	КонецЕсли;
	
	Если ЭтотОбъект.злСтатусEsolutions = ПредопределенноеЗначение(
			"Перечисление.КР_СтатусыАнонсаПоставкиДляСистемыEsolutions.Отправлен") Тогда
		ПодключитьОбработчикОжидания("злЗаполнитьСтатусEsolutions", 600, Истина);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура злАнонсПоставкиEsolutionsНаСервере(ДокументСсылка)
	
	// --> Евлахов Игорь Николаевич (Начало) 20.12.2023
	// Задача #3407	
	ОбработкаСсылка = КР_ДополнительныеНастройкиПовтИсп.Значение("ВнешняяОбработкаESolutionsSecureFTP", Ложь);
	...
	Если ЗначениеЗаполнено(ОбработкаСсылка) Тогда
		...
		ОбработкаОбъект.АнонсЗаказаНаСкладEsolution(ДокументСсылка);
	Иначе
...
&НаСервереБезКонтекста
Функция злПолучитьСтатусEsolutionsНаСервере(Заказ)
...
&НаСервере
Функция злПроверитьВозможностьФормированияАнонсаПоставки()
	...
	Если Объект.ПеремещениеПоЗаказам Тогда		
		ТЗЗаказыНаПеремещение = Объект.Товары.Выгрузить(,"ЗаказНаПеремещение");
		ТЗЗаказыНаПеремещение.Свернуть("ЗаказНаПеремещение");
		...
		ТекстЗапроса = "ВЫБРАТЬ
		               |	Данные.ЗаказНаПеремещение КАК ЗаказНаПеремещение
		               |ПОМЕСТИТЬ ВТЗаказыНаПеремещение
		               |ИЗ
		               |	&ЗаказыНаПеремещение КАК Данные
		               |;
		               |
		               |////////////////////////////////////////////////////////////////////////////////
		               |ВЫБРАТЬ
		               ...
		               |			Объект В
		               |				(ВЫБРАТЬ
		               |					ВТЗаказыНаПеремещение.ЗаказНаПеремещение КАК ЗаказНаПеремещение
		               |				ИЗ
		               |					ВТЗаказыНаПеремещение КАК ВТЗаказыНаПеремещение)) КАК СтатусыПоставки
		               |ГДЕ
		...
		Запрос.УстановитьПараметр("ЗаказыНаПеремещение", ТЗЗаказыНаПеремещение);
		
		Результат 	= Запрос.Выполнить();
		Выборка 	= Результат.Выбрать();
		
		ТекстОшибки = НСтр("ru = 'Запрещено выполнять операцию ""Анонс поставки"".
									|В документе используются ""Заказы на перемещение"", в статусе eSolutions:"
...
// << 09.08.2022, Глухов А.Н., КРОК, Jira№ A2105505-352
// Проверка условия необходимости заполнения КР_Короб ТЧ Товары
&НаСервере
Процедура КР_УправлениеВидимостьюДополнительно()
	
	// << 14.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-352
	СкладыДокумента = Новый Массив;
	СкладыДокумента.Добавить(Объект.СкладОтправитель);
	СкладыДокумента.Добавить(Объект.СкладПолучатель);
	ВедетсяУчетОстатковКоробов = Документы.ПеремещениеТоваров.КР_ВедетсяУчетОстатковКоробовНаСкладах(СкладыДокумента);
	Элементы.ТоварыКР_Короб.Видимость = ВедетсяУчетОстатковКоробов;
	// >> 14.11.2022 Маскаев П.Ю., КРОК, JIRA№ A2105505-352

	// << 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751    
	СкладОтправительСПомещениямиБезордернойОтгрузки = Объект.СкладОтправитель.КР_СкладСПомещениямиБезордернойОтгрузки; 
	ОснованиеНеАктОРасхожденияхПослеПеремещения = 
		(ТипЗнч(Объект.ДокументОснование) <> Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения")); 
		
	Элементы.КР_ПомещениеОтправителя.Видимость = СкладОтправительСПомещениямиБезордернойОтгрузки 
		И ОснованиеНеАктОРасхожденияхПослеПеремещения;
	Элементы.КР_ЗонаОтгрузки.Видимость = СкладОтправительСПомещениямиБезордернойОтгрузки
		И ОснованиеНеАктОРасхожденияхПослеПеремещения;  
		
	Элементы.КР_ДатаПотоварнойПриемки.Видимость = ОснованиеНеАктОРасхожденияхПослеПеремещения;
	Элементы.КР_УстановитьДатуПотоварнойПриемки.Видимость = ОснованиеНеАктОРасхожденияхПослеПеремещения;
	
	//
	Элементы.ДокументОснование.Видимость = Не ОснованиеНеАктОРасхожденияхПослеПеремещения;
	Элементы.СтраницыЗаказ.Видимость = ОснованиеНеАктОРасхожденияхПослеПеремещения;
	...
	Элементы.злСтатусEsolutions.Видимость 	= ЭтоСкладEsolutions;
	Элементы.злКомандаАнонсПоставки.Видимость 	= ЭтоСкладEsolutions;
	// <-- Евлахов Игорь Николаевич (Конец) 07.12.2023 
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаСервере
Процедура КР_ДатаПотоварнойПриемкиОбновить()   
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Т.ДатаПотоварнойПриемки КАК ДатаПотоварнойПриемки
	|ИЗ
	|	РегистрСведений.КР_ПриемкаПеремещенийТоваров КАК Т
	|ГДЕ
	|	Т.Накладная = &Накладная"); 
	Запрос.УстановитьПараметр("Накладная", Объект.Ссылка);
	
	ВыборкаДетальныеЗаписи = Запрос.Выполнить().Выбрать();
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда 
		ЭтотОбъект.КР_ДатаПотоварнойПриемки = ВыборкаДетальныеЗаписи.ДатаПотоварнойПриемки;   
		Элементы.КР_УстановитьДатуПотоварнойПриемки.Заголовок = 
			НСтр("ru = 'Обновить акт расхождения'");
	Иначе
		ЭтотОбъект.КР_ДатаПотоварнойПриемки = Неопределено;
	КонецЕсли;	
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаСервере
Процедура КР_ПомещениеОтправителяПриИзмененииНаСервере() 
...
// << 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
&НаСервере
Процедура КР_УстановитьДатуПотоварнойПриемкиНаСервере() 
	
    Запись = РегистрыСведений.КР_ПриемкаПеремещенийТоваров.СоздатьМенеджерЗаписи();
	Запись.Активность = Истина;
	Запись.Накладная = Объект.Ссылка;    
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.КР_ДатаПотоварнойПриемки) Тогда 
		Запись.ДатаПотоварнойПриемки = ЭтотОбъект.КР_ДатаПотоварнойПриемки;
	Иначе	
		Запись.ДатаПотоварнойПриемки = ТекущаяДатаСеанса();  
	КонецЕсли;
	
	Запись.Записать();
	
	// Обновим без перечитывания
	ЭтотОбъект.КР_ДатаПотоварнойПриемки = Запись.ДатаПотоварнойПриемки;
	
КонецПроцедуры // >> 18.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702

// << 02.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-751
&НаСервере
Процедура КР_ЗаполнитьПомещениеОтправителя()
	
	Объект.КР_ПомещениеОтправителя = Неопределено;
	Объект.КР_ЗонаОтгрузки = Неопределено;

	ОснованиеНеАктОРасхожденияхПослеПеремещения = 
		(ТипЗнч(Объект.ДокументОснование) <> Тип("ДокументСсылка.АктОРасхожденияхПослеПеремещения")); 
	
	Если ЗначениеЗаполнено(Объект.СкладОтправитель)
		И Объект.СкладОтправитель.КР_АдресныйСкладБезордернойОтгрузки
		И ОснованиеНеАктОРасхожденияхПослеПеремещения Тогда 

		Объект.КР_ПомещениеОтправителя = 
			Справочники.СкладскиеПомещения.КР_СкладскоеПомещениеСкладМагазина(Объект.СкладОтправитель);	 
