...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 25.04.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 30.08.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1448
	// Рефакторинг
	Список.ПроизвольныйЗапрос = Истина;
	Список.ТекстЗапроса =
	"ВЫБРАТЬ
	|	ДокументПеремещениеТоваров.*
	|ИЗ
	|	Документ.ПеремещениеТоваров КАК ДокументПеремещениеТоваров
	|ГДЕ
	|	НЕ(&КР_ИсключитьАктОРасхожденияхПослеПеремещения
	|				И ДокументОснование ССЫЛКА Документ.АктОРасхожденияхПослеПеремещения
	|				И НЕ ДокументОснование = ЗНАЧЕНИЕ(Документ.АктОРасхожденияхПослеПеремещения.ПустаяСсылка))";
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Список,
		"КР_ИсключитьАктОРасхожденияхПослеПеремещения",
		Параметры.Свойство("КР_ИсключитьАктОРасхожденияхПослеПеремещения"));
	// >> 30.08.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1448
	
КонецПроцедуры // >> 25.04.2023 Федоров Д.Е., КРОК, JIRA№A2105505-1260
