...
Функция ЗаписатьИзмененияЦенНаСервере(Форма, Комментарий = "", ВариантПримененияЦен = 0) Экспорт
			...
			Для каждого СтрокаТЗ Из ТаблицаОбработкиПериодовЦен Цикл
				Если СтрокаТЗ.УдалитьКортеж Тогда
					Если ЗначениеЗаполнено(СтрокаТЗ.ДокументНачалаПериодаДействияЦенБывший) Тогда
						ДокументНаУдаление = СтрокаТЗ.ДокументНачалаПериодаДействияЦенБывший.ПолучитьОбъект();
					...
					Если ЗначениеЗаполнено(СтрокаТЗ.ДокументОкончанияПериодаДействияЦен) Тогда
						ДокументНаУдаление = СтрокаТЗ.ДокументОкончанияПериодаДействияЦен.ПолучитьОбъект();
						Если ДокументНаУдаление <> Неопределено Тогда
							ДокументНаУдаление.Удалить();
						КонецЕсли; 
					КонецЕсли; 
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Ошибка при установке цен.'") + ИнформацияОбОшибке().Описание;
...
Функция ТекстЗапросаПереоценкиТоваровВРозницеБД2_5()
	...
	|			И ЦеныНоменклатурыТовары.ХарактеристикаЦО <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК СерииНоменклатуры
	|		ПО ТаблицаНоменклатуры.ПоСерии
...
Функция ТекстЗапросаПереоценкиТоваровВРозницеБДПоАссортименту2_5()
	...
	|			И УстановкаЦенТоварыАссортимент.ХарактеристикаЦО <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК СерииНоменклатуры
	|		ПО ТаблицаНоменклатуры.ПоСерии
...
Функция СформироватьПечатнуюФормуПереоценкиТоваровВРозницеБД(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого УстановкаЦенНоменклатуры Из МассивОбъектов Цикл
		...
		ИспользуетсяЦенообразование25 = ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25(ГраницаКонецПредыдущегоДня.Значение);
...
Функция СформироватьПечатнуюФормуПереоценкиТоваровВРозницеВТ(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ИспользуетсяЦенообразование25 = ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25(ДанныеДляПечати.ДатаДействующихЦен);
...
Функция ДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		...
		|	Документ.УстановкаЦенНоменклатуры.Товары2_5 КАК УстановкаЦенНоменклатурыТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
		|			ПО УстановкаЦенНоменклатурыТовары.ХарактеристикаЦО = ХарактеристикиНоменклатуры.ХарактеристикаНоменклатурыДляЦенообразования
		|			И УстановкаЦенНоменклатурыТовары.ХарактеристикаЦО <> ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры КАК СерииНоменклатуры
...
Функция ДанныеДляПечатиЦенников(МассивДокументов) Экспорт
	...
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		...
		|	, УстановкаЦенНоменклатуры.Ссылка.КР_НеПечататьСтарыеЦены КАК КР_НеПечататьСтарыеЦены
		// >> 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
		|ИЗ
		|	Документ.УстановкаЦенНоменклатуры.ВидыЦен КАК УстановкаЦенНоменклатуры
		|ГДЕ
	...
	МассивВидовЦен = ТаблицаРеквизитыДокументов.ВыгрузитьКолонку(0);
	// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	МассивНеПечататьСтарыеЦены = ОбщегоНазначенияУТ.УникальныеЗначенияИзКолонкиТаблицы(
	...
	//СтруктураДействий.Вставить("ЗаполнитьСклад",       Неопределено);
	Склад = КР_СкладПользователя();
	СтруктураДействий.Вставить("ЗаполнитьСклад", Склад);
	// >> 14.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	СтруктураДействий.Вставить("ЗаполнитьВидЦены",     ?(МассивВидовЦен.Количество() = 1, МассивВидовЦен[0], Неопределено));
	СтруктураДействий.Вставить("ПоказыватьКолонкуКоличествоВДокументе", Ложь);
	СтруктураДействий.Вставить("УстановитьРежимПечатиИзДокумента");
	...
	СтруктураДействий.Вставить("ЗаполнитьТаблицуТоваров");
	// << 13.02.2023, Гашников А.П., КРОК, Jira№ A2105505-1028
	СтруктураДействий.Вставить("КР_ТолькоСОстаткамиНаСкладе");
	СтруктураДействий.Вставить("КР_Источник", "Документ.УстановкаЦенНоменклатуры");
	СтруктураДействий.Вставить("КР_НеПечататьСтарыеЦены", 
		?(МассивНеПечататьСтарыеЦены.Количество() = 1, МассивНеПечататьСтарыеЦены[0], Неопределено));
