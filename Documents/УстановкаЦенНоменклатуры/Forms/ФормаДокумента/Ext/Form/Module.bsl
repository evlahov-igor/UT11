...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И Объект.Товары.Количество() Тогда
	...
	ЗафиксироватьКолонкуНоменклатуры(Истина);
...
Процедура ДеревоЦенПередУдалением(Элемент, Отказ)
	Если Элемент.ТекущиеДанные = Неопределено
		Или Элемент.ТекущиеДанные.ИндексКартинки <> 0 Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПерейтиКУстановкеЦен(Команда)
	...
	Если Не ЭтаФорма.ТолькоПросмотр 
		И УстановкаЦенКлиентСервер.НеВыбраныЗависимыеЦены(ЭтаФорма) Тогда
...
Процедура ПерейтиКУстановкеЦенФрагмент()
	...
	Если Не ЭтаФорма.ТолькоПросмотр 
		И Не УстановкаЦенКлиент.ЗаполненыОбязательныеПараметрыСхемКомпоновкиДанных(ЭтаФорма) Тогда
		Возврат;
	КонецЕсли;
...
Функция ДобавитьНоменклатуруПоШтрихкодамВДеревоЦен(ДанныеШтрихкодов)
	...
	Если ИспользуетсяЦенообразование25 Тогда
		...
		ТаблицаНоменклатуры = УстановкаЦенСервер.СоздатьТаблицуНоменклатуры(ЭтаФорма);
	...
	УстановкаЦенСервер.ДобавитьТоварыПоОтбору(ЭтаФорма, Неопределено, ТаблицаНоменклатура);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	УстановкаЦенСервер.ДобавитьТоварыПоОтбору(ЭтаФорма, ВыбранноеЗначение.АдресТоваровВХранилище, ТаблицаНоменклатуры);
...
Процедура ДеревоЦенНоменклатураПриИзмененииСервер(ИдентификаторСтроки, РассчитатьЦены = Истина)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		...
		ТаблицаНоменклатуры = УстановкаЦенСервер.ДобавитьТовары(ЭтаФорма, ТаблицаНоменклатуры, КэшДанных);	
	...
	Если РассчитатьЦены Тогда
		Если ИспользуетсяЦенообразование25 Тогда
			УстановкаЦенСервер.РассчитатьЦеныПоИдентификаторуСтроки2_5(ЭтаФорма, ИдентификаторСтроки, Истина,,КэшДанных);
		Иначе	
			УстановкаЦенСервер.РассчитатьЦеныПоИдентификаторуСтроки(ЭтаФорма, ИдентификаторСтроки, Истина,,КэшДанных);
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаВидЦены Из ВыбранныеЦены Цикл
		...
		Если СтрокаВидЦены.Выбрана ИЛИ СтрокаВидЦены.Влияет Тогда
			...
			Если ИспользуетсяЦенообразование25 Тогда
				...
				НоваяСтрока.СерияЦОДляСвязи   = ?(ТекущаяСтрока.СерияЦО.Предопределенный, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), ТекущаяСтрока.СерияЦО);
			КонецЕсли;			
...
Процедура ДеревоЦенХарактеристикаПриИзмененииСервер(ИдентификаторСтроки, РассчитатьЦены = Истина)
	...
	УстановкаЦенСервер.ДеревоЦенХарактеристикаПриИзменении(ЭтаФорма, ИдентификаторСтроки, РассчитатьЦены);
...
Процедура ДобавитьНоменклатуруЦеновойГруппыНаСервере(ЦеновыеГруппы)
	...
	ТаблицаТовары = УстановкаЦенСервер.ДобавитьТовары(ЭтаФорма, ТаблицаНоменклатуры, КэшДанных);	

	УстановкаЦенСервер.ОбновитьСтарыеЦеныНоменклатуры(ЭтаФорма, КэшДанных, ТаблицаТовары);
	...
	Если РассчитыватьАвтоматически Тогда
		...
		Если ЗагружатьЦеныИзИБ Тогда
			УстановкаЦенСервер.ЗагрузитьЗначенияБазовыхЦен(ЭтаФорма, ТаблицаТовары, КэшДанных);
			УстановкаЦенСервер.ВычислитьЦеныПоДаннымИБ(ЭтаФорма, ТаблицаТовары,,,КэшДанных);
		КонецЕсли;
		
		УстановкаЦенСервер.РассчитатьВычисляемыеЦены(
			ЭтаФорма, ТаблицаТовары, КэшДанных);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.ДокументОснование)
				...
				Если УстановкаЦенКлиентСервер.ВыбранныеВидыЦен(ЭтотОбъект).Количество() > 0 Тогда
					...
					ТаблицаНоменклатуры = УстановкаЦенСервер.СоздатьТаблицуНоменклатурыПоДеревуЦен(ЭтаФорма, Ложь);
...
Процедура ДобавитьТоварыПоОтборамСКДНаСервере()
	...
	УстановкаЦенСервер.ДобавитьТоварыПоОтборамСКД(ЭтаФорма);
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	Иначе
		Если СтрНайти(Элемент.Имя, "ДеревоЦен" + "ВидЦены") > 0
			ИЛИ СтрНайти(Элемент.Имя, "ДеревоЦен" + "ПроцентИзменения" + "ВидЦены") > 0 Тогда
...
Процедура ПолучитьСтрокиИзБуфераОбмена(ЗаполняемыйВидЦен)
	...
	Если ИспользуетсяЦенообразование25 Тогда
		Колонки = "Номенклатура,ХарактеристикаЦО,УпаковкаЦО,СерияЦО,Упаковка,Цена";
	Иначе
...
Процедура ПереформироватьТаблицуЦен(ПересчитатьЦены)
	УстановкаЦенСервер.ПереформироватьТаблицуЦен(ЭтаФорма, ПересчитатьЦены);
КонецПроцедуры
...
Процедура ДеревоЦенЦенаПриИзмененииКлиент(Элемент)
	...
	Если СтрНайти(ИмяТекущейКолонки, "Формула" + "ВидЦены") Тогда
		...
		ИмяКолонкиУпаковка = СтрЗаменить(СтрЗаменить(Элемент.Имя, "Формула" + "ВидЦены", "ВидЦены"), "ДеревоЦен" + "ВидЦены", "Упаковка" + "ВидЦены");
	КонецЕсли;
...
Процедура ИзменитьЦеныНаПроцентНаСервере(ПараметрыРасчета)
	УстановкаЦенСервер.ИзменитьЦеныНаПроцент(ЭтаФорма, ПараметрыРасчета);
КонецПроцедуры

&НаСервере
Процедура ДобавитьТоварыПоОтборуНаСервере(АдресВоВременномХранилище)
	
	УстановкаЦенСервер.ДобавитьТоварыПоОтбору(ЭтаФорма, АдресВоВременномХранилище); 
	
КонецПроцедуры

&НаСервере
Процедура РассчитатьЦеныНаСервере(ПараметрыРасчета)
	УстановкаЦенСервер.РассчитатьЦены(ЭтаФорма, ПараметрыРасчета);
КонецПроцедуры

&НаСервере
Процедура РассчитатьЦеныПоИдентификаторуСтрокиНаСервере(ИдентификаторСтроки, ИзмененноеПоле)
	Если ИспользуетсяЦенообразование25 Тогда
		УстановкаЦенСервер.РассчитатьЦеныПоИдентификаторуСтроки2_5(ЭтаФорма, ИдентификаторСтроки,,ИзмененноеПоле);
	Иначе	
		УстановкаЦенСервер.РассчитатьЦеныПоИдентификаторуСтроки(ЭтаФорма, ИдентификаторСтроки,,ИзмененноеПоле);
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьВидимостьКолонокРедактирования(Форма, ИспользоватьРежимРедактирования)
	...
	Форма.Элементы.ДеревоЦенХарактеристика.Видимость 	= Не Форма.ИспользуетсяЦенообразование25;

	Форма.Элементы.ДеревоЦенХарактеристикаЦО.Видимость 	= Форма.ИспользуетсяЦенообразование25;
	Форма.Элементы.ДеревоЦенСерияЦО.Видимость 			= Форма.ИспользуетсяЦенообразование25;
	Форма.Элементы.ДеревоЦенУпаковкаЦО.Видимость 		= Форма.ИспользуетсяЦенообразование25;
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект,
		"Объект.КР_НеПечататьСтарыеЦены", ,
		Элементы.Состояние);
		
	ЭлементФормы.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
	
	// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
	КомандаФормы = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект,
		"КР_ЗагрузкаЦенИзExcel", НСтр("ru = 'Загрузить из Excel'"));
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(
		ЭтотОбъект, КомандаФормы, Элементы.ДеревоЦен.КоманднаяПанель);
	ЭлементФормы.Заголовок = НСтр("ru = 'Загрузить из шаблона'");
	// >> 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
...
// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаКлиенте
Процедура КР_ЗагрузкаЦенИзExcel(Команда)
	
	Если Модифицированность Или Объект.Ссылка.Пустая() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("КР_ВопросОНеобходимостиЗаписиПослеЗавершения", ЭтотОбъект);
		
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ru = 'Для загрузки цен документ должен быть записан. Продолжить?'"),
			РежимДиалогаВопрос.ДаНет, ,
	...
	КР_ЗагрузкаЦенИзExcelПродолжение();
	
КонецПроцедуры // >> 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048

// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаКлиенте
Процедура КР_ЗагрузкаЦенИзExcelПродолжение()
	
	Если Не Объект.ВидыЦен.Количество() Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Не выбраны виды цен'"));
		Возврат;
	КонецЕсли;
	
	Если ИспользуетсяЦенообразование25 Тогда
		ИмяТабличнойЧасти = "Товары2_5";
	Иначе
		ИмяТабличнойЧасти = "Товары";
	КонецЕсли;
	
	Если Объект[ИмяТабличнойЧасти].Количество() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("КР_ПерезаполнитьТабличнуюЧастьЦен", ЭтотОбъект);
		
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ru = 'Дерево цен будет перезаполнено. Продолжить?'"),
			РежимДиалогаВопрос.ДаНет, ,
			КодВозвратаДиалога.Нет);
	...
	КР_ОткрытьФормуЗагрузкиЦенИзExcel();
	
КонецПроцедуры // >> 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048

// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаКлиенте
Процедура КР_ВопросОНеобходимостиЗаписиПослеЗавершения(Результат, ДополнительныеПараметры) Экспорт
...
// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаКлиенте
Процедура КР_ПерезаполнитьТабличнуюЧастьЦен(Ответ, ДополнительныеПараметры) Экспорт
...
// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаКлиенте
Процедура КР_ОткрытьФормуЗагрузкиЦенИзExcel()
	
	МассивВидовЦен = Новый Массив;
	Для Каждого ТекСтрока Из Объект.ВидыЦен Цикл
		МассивВидовЦен.Добавить(ТекСтрока.ВидЦены);
	КонецЦикла;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("КР_ПослеЗагрузкиЦенНоменклатурыИзExcel", ЭтотОбъект);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ВидыЦен", МассивВидовЦен);
	
	ОткрытьФорму(
		"Документ.УстановкаЦенНоменклатуры.Форма.КР_ЗагрузкаЦенИзExcel",
		ПараметрыФормы,
		ЭтотОбъект, , , ,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаКлиенте
Процедура КР_ПослеЗагрузкиЦенНоменклатурыИзExcel(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КР_ЗагрузитьТаблицуЦенНаСервере(Результат);
...
// << 03.03.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-1048
&НаСервере
Процедура КР_ЗагрузитьТаблицуЦенНаСервере(АдресХранилища)
	
	СтруктураТаблицЦен = ПолучитьИзВременногоХранилища(АдресХранилища);
	...
	Объект[ИмяТабличнойЧасти].Загрузить(СтруктураТаблицЦен[ИмяТабличнойЧасти]);
