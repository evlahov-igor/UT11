...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 22.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862 
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	КР_ЗаполнитьПризнакМаркируемаяПродукция();
	// >> 30.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	// << 30.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
	СтруктураДействий.Вставить("КР_ЗаполнитьПризнакМаркируемаяПродукция", ТекущаяСтрока.Характеристика);
	// >> 30.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
	...
	КР_ТоварыЗаполнитьЗначенияПолей(Элементы.Товары.ТекущиеДанные.ПолучитьИдентификатор());
	// >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-487 
...
Процедура ТоварыСтатьяРасходовПриИзменении(Элемент)
	...
	// << 06.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
	КР_СтатьяРасходовПриИзмененииНаСервере(Элементы.Товары.ТекущиеДанные.ПолучитьИдентификатор());
	// >> 06.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
...
Процедура УстановитьУсловноеОформление()
	...
	// << 21.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	// << 16.01.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-862
	КР_УстановитьВидимостьДоступность();
	// >> 16.01.2023, Маскаев П.Ю., КРОК, Jira№ A2105505-862
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	// << 21.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
	Если КР_ОбработатьШтрихкодыДополнительно(ДанныеШтрихкодов) Тогда
		Возврат;
	КонецЕсли;
	// >> 21.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
	
	// #3840.. Фомин Д.Ю. 23.05.2024.
	...
	Если СтруктураДействий.ТекущаяСтрока <> Неопределено Тогда
		...
		// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-487
		КР_ТоварыЗаполнитьЗначенияПолей(СтруктураДействий.ТекущаяСтрока); 
		// >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-487    
...
Процедура ОбработкаВыбораПодборНаКлиенте(ВыбранноеЗначение)
	...
	Для Каждого СтрокаДанных Из Объект.Товары Цикл 	
		КР_ТоварыЗаполнитьЗначенияПолей(СтрокаДанных.ПолучитьИдентификатор()); 
	КонецЦикла;
	// >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-487 
...
// Задача #3848
&НаКлиенте
Процедура Подключаемый_ЗаполнитьДатуВыдачи(Команда)
	
	ОчиститьСообщения();
	
	ДатаВыдачи	= НачалоДня(ТекущаяДата());
	Подсказка	= НСтр("ru = 'Дата выдачи форменной одежды сотруднику'");  
	ЧастьДаты	= ЧастиДаты.Дата;
	Оповещение	= Новый ОписаниеОповещения("злПослеВводаДатыВыдачи", ЭтотОбъект);
	
	ПоказатьВводДаты(Оповещение, ДатаВыдачи, Подсказка, ЧастьДаты);
	
КонецПроцедуры 

&НаКлиенте
Процедура Подключаемый_ЗаполнитьДатуВыдачиДляВыделенныхСтрок(Команда)
	...
	Подсказка	= НСтр("ru = 'Дата выдачи форменной одежды сотруднику'");  
	ЧастьДаты	= ЧастиДаты.Дата;
	Оповещение	= Новый ОписаниеОповещения("злПослеВводаДатыВыдачиДляВыделенныхСтрок", ЭтотОбъект);
...
// Задача #3848
&НаКлиенте
Процедура злПослеВводаДатыВыдачи(ДатаВыдачи, ДополнительныеПараметры) Экспорт
	
	Если ДатаВыдачи <> Неопределено И ЗначениеЗаполнено(ДатаВыдачи) Тогда		
...
&НаКлиенте
Процедура злПослеВводаДатыВыдачиДляВыделенныхСтрок(ДатаВыдачи, ДополнительныеПараметры) Экспорт
...
Процедура КР_УстановитьУсловноеОформлениеДополнительно()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыКР_КодМаркировкиИСМП.Имя);
	
	ГруппаОтбора = Элемент.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	ГруппаОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
	
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.КР_МаркируемаяПродукция");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
	
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.КР_ПричинаОтсутствияКодаМаркировкиИСМП");
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыКР_ПричинаОтсутствияКодаМаркировкиИСМП.Имя);
	...
	ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.КР_КодМаркировкиИСМП");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
...
Процедура КР_ДобавитьЭлементыНаФорму()
	...
	// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
	// Задача #3848
	// Заполнение для всей табличной части
	ИмяКоманды = "злКомандаЗаполнитьДатуВыдачи";
	ЗаголовокЗаполнитьДатуВыдачи = НСтр("ru = 'Заполнить дату выдачи'");
	ДействиеЗаполнитьДатуВыдачи = "Подключаемый_ЗаполнитьДатуВыдачи";
	
	КомандаЗаполнитьДатуВыдачи = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект, 
																			ИмяКоманды, 
																			ЗаголовокЗаполнитьДатуВыдачи, 
																			ДействиеЗаполнитьДатуВыдачи);
	
	КомандаЗаполнитьДатуВыдачи.Картинка	= БиблиотекаКартинок.Календарь;
	КомандаЗаполнитьДатуВыдачи.Отображение = ОтображениеКнопки.КартинкаИТекст;
	КомандаЗаполнитьДатуВыдачи.ИзменяетСохраняемыеДанные = Истина; 
	КомандаЗаполнитьДатуВыдачи.Подсказка = ЗаголовокЗаполнитьДатуВыдачи;
	
	// Заполнение для выделенных строк
	ИмяКоманды = "злКомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок";
	ЗаголовокЗаполнитьДатуВыдачиДляВыделенныхСтрок = НСтр("ru = 'Заполнить дату выдачи (для выделенных строк)'");
	ДействиеЗаполнитьДатуВыдачиДляВыделенныхСтрок = "Подключаемый_ЗаполнитьДатуВыдачиДляВыделенныхСтрок";
	
	КомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок = КР_МетодыМодификацииФорм.ДобавитьКомандуФормы(ЭтотОбъект, 
																			ИмяКоманды, 
																			ЗаголовокЗаполнитьДатуВыдачиДляВыделенныхСтрок, 
																			ДействиеЗаполнитьДатуВыдачиДляВыделенныхСтрок);
	
	КомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок.Картинка = БиблиотекаКартинок.Календарь;
	КомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок.Отображение = ОтображениеКнопки.КартинкаИТекст;
	КомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок.ИзменяетСохраняемыеДанные = Истина; 
	КомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок.Подсказка = ЗаголовокЗаполнитьДатуВыдачиДляВыделенныхСтрок;
	// <-- Евлахов Игорь Николаевич (Конец) 05.02.2024
	
	#КонецОбласти
	
	#Область злДобавлениеРеквизитовФормы
	
	КР_МетодыМодификацииФорм.ДобавитьРеквизитФормы(ЭтотОбъект, "КР_МаркируемаяПродукция",
		Новый ОписаниеТипов("Булево"), "Объект.Товары", НСтр("ru = 'Маркируемая продукция'"));
	...
	// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
	// Задача #3848
	// Элемент формы "Заполнить дату выдачи"
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, 
																КомандаЗаполнитьДатуВыдачи, 
																Элементы.ТоварыГруппаДатаОтгрузки,
																, 
																"КнопкаФормы");   
	// Элемент формы "Заполнить дату выдачи (для выделенных строк)"															
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, 
																КомандаЗаполнитьДатуВыдачиДляВыделенныхСтрок, 
																Элементы.ТоварыГруппаДатаОтгрузки,
																, 
																"КнопкаФормы");													
	// <-- Евлахов Игорь Николаевич (Конец) 05.02.2024
	...
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_ВидБрака", , Элементы.ТоварыОтменено);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_КодМаркировкиИСМП", , Элементы.ТоварыОтменено);
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.КР_ПричинаОтсутствияКодаМаркировкиИСМП", , Элементы.ТоварыОтменено);
	
	// --> Евлахов Игорь Николаевич (Начало) 11.03.2024
	// Задача #3848
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.Товары.злДатаВыдачи", , Элементы.ТоварыОтменено);
	// <-- Евлахов Игорь Николаевич (Конец) 11.03.2024
	
	// #2858..
	НоваяСтраница = КР_МетодыМодификацииФорм.ВставитьГруппуФормы(ЭтотОбъект, "ГруппаПодписанты", Элементы.ГруппаСтраницы);
	НоваяСтраница.Заголовок = нСтр("ru='Члены комиссии'", "ru");                                                       
	НоваяСтраница.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злОтветственныйЗаДокумент", НоваяСтраница);  
	
	НовоеТабПоле = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злПодписанты", НоваяСтраница);
	НовоеТабПоле.УстановитьДействие("ПриНачалеРедактирования", "злПриНачалеРедактированияПодписанты"); // #2912
	
	КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злПодписанты.ЧленКомиссии", НовоеТабПоле);
	
	// #2912..
	НовыйЭлемент = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект, "Объект.злПодписанты.Председатель", НовоеТабПоле);
	НовыйЭлемент.РежимРедактирования = РежимРедактированияКолонки.Непосредственно;
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "злПриИзмененииФлагаПредседатель");
	// ..#2912
	// ..#2858
...
&НаСервере
Процедура КР_УстановитьВидимостьДоступность()
	
	ЭтоСписаниеТоваровПоТребованию = // Списание на расходы
		(Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию);
		
	Элементы.ТоварыКР_ВидБрака.Видимость = ЭтоСписаниеТоваровПоТребованию;
	Элементы.ТоварыКР_КодМаркировкиИСМП.Видимость = ЭтоСписаниеТоваровПоТребованию;
	Элементы.ТоварыКР_ПричинаОтсутствияКодаМаркировкиИСМП.Видимость = ЭтоСписаниеТоваровПоТребованию;
	
    // << 02.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
	РазрешениеПередачиВЭксплуатацию = Пользователи.РолиДоступны("КР_РасширенноеЗаполнениеПередачаВЭксплуатацию");
	РасширениеСписанияНаРасходы = Пользователи.РолиДоступны("КР_РасширенноеЗаполнениеСписаниеНаРасходы");
	ВидСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Склад, "КР_ВидСклада");
	ЭтоМагазин = (ВидСклада = Перечисления.КР_ВидыСкладов.Магазин); 
	ЭтоПередачаВЭксплуатацию = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию);
	
	Если ЭтоМагазин Тогда 
		Элементы.ТоварыКатегорияЭксплуатации.Доступность = РазрешениеПередачиВЭксплуатацию;
		Элементы.ТоварыСтатьяРасходов.Доступность = РазрешениеПередачиВЭксплуатацию И ЭтоПередачаВЭксплуатацию
			Или РасширениеСписанияНаРасходы И ЭтоСписаниеТоваровПоТребованию;
	КонецЕсли;	
	// >> 02.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
...
// << 21.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
&НаКлиенте
Функция КР_ОбработатьШтрихкодыДополнительно(ДанныеШтрихкодов)
	
	Если ДанныеШтрихкодов = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;

	// #3840.. Фомин Д.Ю. 23.05.2024. Перенес проверку в процедуру ОбработатьШтрихкоды
	//Если Не ШтрихкодированиеНоменклатурыКлиент.ШтрихкодыВалидны(ДанныеШтрихкодов) Тогда
	//	Возврат Ложь;
	//КонецЕсли;
	// ..#3840
	
	// --> Евлахов Игорь Николаевич (Начало) 05.09.2024
	// Задача #4532
	// Начало было
	//Если ДанныеШтрихкодов.Количество() = 1 Тогда
	//	ДанныеШтрихкода = ДанныеШтрихкодов[0];
	//Иначе
	//	Возврат Ложь;
	//КонецЕсли;
	//	
	...
	//ВидыМаркируемойПродукции.Добавить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ЛегкаяПромышленность"));
	//ВидыМаркируемойПродукции.Добавить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Обувь"));
	//
	//ПримечаниеКРазборуШтрихкода = Неопределено;
	//ПараметрыРазбораКМ = РазборКодаМаркировкиИССлужебныйКлиентСервер.ПользовательскиеПараметрыРазбораКодаМаркировки();
	//НастройкиРазбораКМ = РазборКодаМаркировкиИССлужебныйКлиентПовтИсп.НастройкиРазбораКодаМаркировки();
	//
	//ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(
	//	ДанныеШтрихкода.Штрихкод, ВидыМаркируемойПродукции, ПримечаниеКРазборуШтрихкода, НастройкиРазбораКМ, ПараметрыРазбораКМ);
	//
	//Если ТипЗнч(ДанныеРазбора) = Тип("Структура") И ДанныеРазбора.Свойство("ТипШтрихкода")
	//	И ДанныеРазбора.ТипШтрихкода = ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.GS1_DataMatrix") Тогда
	//	КР_ДополнитьКодМаркировки(ДанныеРазбора);
	//	КР_ОбработатьКодМаркировки(ДанныеРазбора);
	//	Возврат Истина;
	//Иначе
	//	Возврат Ложь;
	//КонецЕсли;
	...
	Пока Индекс < МассивШтрихкодов.Количество() Цикл
		СтруктураШтрихкода = МассивШтрихкодов[Индекс];
		
		ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
		ДанныеРазбора = РазборКодаМаркировкиИССлужебныйКлиент.РазобратьКодМаркировки(
			СтруктураШтрихкода.Штрихкод, ВидыМаркируемойПродукции, ПримечаниеКРазборуШтрихкода, НастройкиРазбораКМ, ПараметрыРазбораКМ);
		...
		Если ТипЗнч(ДанныеРазбора) = Тип("Структура") И ДанныеРазбора.Свойство("ТипШтрихкода")
			...
			МассивШтрихкодов.Удалить(Индекс);
		Иначе
			Индекс = Индекс + 1;
		КонецЕсли;		
	КонецЦикла;	
	...
	КонецЕсли; 
	// Задача #4532
	// <-- Евлахов Игорь Николаевич (Конец) 05.09.2024
	
КонецФункции

&НаКлиенте
Процедура КР_ОбработатьКодМаркировки(ДанныеРазбораКодаМаркировки)
	
	НоваяСтрока = Объект.Товары.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеРазбораКодаМаркировки.СоставКодаМаркировки);
	НоваяСтрока.КоличествоУпаковок = 1;
	НоваяСтрока.КР_КодМаркировкиИСМП = ДанныеРазбораКодаМаркировки.НормализованныйКодМаркировки;
	...
	// --> Евлахов Игорь Николаевич (Начало) 05.09.2024
	// Задача #4532
	КР_ТоварыЗаполнитьЗначенияПолей(Элементы.Товары.ТекущаяСтрока);
	// <-- Евлахов Игорь Николаевич (Конец) 05.09.2024
...
Процедура КР_ДополнитьКодМаркировки(ДанныеРазбораКодаМаркировки)
	
	Если ДанныеРазбораКодаМаркировки.Свойство("СоставКодаМаркировки")
		И ДанныеРазбораКодаМаркировки.СоставКодаМаркировки.Свойство("EAN") Тогда
		Штрихкод = ДанныеРазбораКодаМаркировки.СоставКодаМаркировки.EAN;
	Иначе
	...
	ДанныеРазбораКодаМаркировки.СоставКодаМаркировки.Вставить("Номенклатура");
	ДанныеРазбораКодаМаркировки.СоставКодаМаркировки.Вставить("Характеристика");
	ДанныеРазбораКодаМаркировки.СоставКодаМаркировки.Вставить("Упаковка");
	
	ЗаполнитьЗначенияСвойств(ДанныеРазбораКодаМаркировки.СоставКодаМаркировки, КэшированныеЗначения.Штрихкоды[Штрихкод]);
	
КонецПроцедуры // >> 21.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862

// << 30.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-862
&НаСервере
Процедура КР_ЗаполнитьПризнакМаркируемаяПродукция()
	
	КэшированныеЗначения = Неопределено;
	
	СтруктураДействий = Новый Структура;	
	
	СтруктураДействий.Вставить("КР_ЗаполнитьПризнакМаркируемаяПродукция");
...
// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-487
&НаСервере
Процедура КР_ТоварыЗаполнитьЗначенияПолей(ИдентификаторСтроки)
	
	ДанныеСтроки = Объект.Товары.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	ВидСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Склад, "КР_ВидСклада");
	ЭтоМагазин = (ВидСклада = ПредопределенноеЗначение("Перечисление.КР_ВидыСкладов.Магазин"));	 
	
	Если ЭтоМагазин Тогда 
		
		ДанныеСтроки.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить; 
		ДанныеСтроки.ДатаОтгрузки = Объект.Дата; // A2105505-2036
		
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию Тогда   
		
			Категория = КР_ДополнительныеНастройкиПовтИсп.Значение("КатегорияЭксплуатацииФорменнойОдежды");  
			СтатьяРасходов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Категория, "СтатьяРасходов");   
				
			ДанныеСтроки.КатегорияЭксплуатации = Категория;
		...
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию Тогда   
			...
			ДанныеСтроки.СтатьяРасходов = КР_ДополнительныеНастройкиПовтИсп.Значение("СтатьяРасходовСписаниеБрак");  
			
		КонецЕсли;
		
		КР_СтатьяРасходовПриИзмененииНаСервере(ДанныеСтроки);   
		
	КонецЕсли;
	
	// --> Евлахов Игорь Николаевич (Начало) 11.03.2024
	// Задача #3848
	Если Не ЗначениеЗаполнено(ДанныеСтроки.злДатаВыдачи) 
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию Тогда
		ДанныеСтроки.злДатаВыдачи = НачалоДня(ТекущаяДатаСеанса());
	КонецЕсли;
	// <-- Евлахов Игорь Николаевич (Конец) 11.03.2024
	
КонецПроцедуры // >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-487 

// << 06.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
&НаСервере
Процедура КР_СтатьяРасходовПриИзмененииНаСервере(ИдентификаторДанныеСтроки)
	
	Если ТипЗнч(ИдентификаторДанныеСтроки) = Тип("Число") Тогда  
		ДанныеСтроки = Объект.Товары.НайтиПоИдентификатору(ИдентификаторДанныеСтроки);
	Иначе
		ДанныеСтроки = ИдентификаторДанныеСтроки;
	КонецЕсли;
	
	Если ДанныеСтроки.СтатьяРасходов.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Номенклатура")) Тогда 
		ДанныеСтроки.АналитикаРасходов = ДанныеСтроки.Номенклатура;  
	Иначе 
		ДанныеСтроки.АналитикаРасходов = Неопределено;  
	КонецЕсли;  
	
КонецПроцедуры // >> 06.03.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188

// #2912..
&НаКлиенте
Процедура злПриИзмененииФлагаПредседатель(Элемент)
	
	// Флаг "Председатель" может быть установлен только у одной строки,
	// т.е. работает, как переключатель.
	// Просто взять и снять флаг нельзя.
	
	ТабПоле = Элементы.злПодписанты;
	
	УстановленноеЗначение = ТабПоле.ТекущиеДанные.Председатель;
	
	Если УстановленноеЗначение Тогда
		Для Каждого ТекСтрока Из Объект.злПодписанты Цикл
			ТекСтрока.Председатель = Ложь;
		КонецЦикла;
	КонецЕсли;

	ТабПоле.ТекущиеДанные.Председатель = Истина;
	
КонецПроцедуры// ..#2912

// #2912..
&НаКлиенте
Процедура злПриНачалеРедактированияПодписанты(Элемент, НоваяСтрока, Копирование)
