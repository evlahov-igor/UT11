...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеШапки.ОбъектРасчетовСКомиссионеромВознаграждение.УникальныйИдентификатор КАК ИдентификаторФинЗаписи,
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Очищает служебные табличные части ""Расшифровка платежа с клиентом"", ""Расшифровка платежа с поставщиком"",
	|""Расшифровка платежа с клиентом вознаграждение"" и ""Расшифровка платежа с поставщиком вознаграждение"" в непроведенных документах.'");
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("1d87f896-5e7d-4a63-bfee-6be046a4f999");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетПоКомиссииМеждуОрганизациями.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ НЕ ДанныеДокумента.ОплатаВВалюте
	|			И (ДанныеДокумента.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ДанныеДокумента.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|	ИЛИ ДанныеДокумента.ИдентификаторДокумента = """"
	|	ИЛИ НЕ ДанныеДокумента.Проведен И (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.ОтчетПоКомиссииМеждуОрганизациями.РасшифровкаПлатежаСКлиентом КАК ТЧРасшифровкаПлатежаСКлиентом
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Не ДокументОбъект.Проведен И ДокументОбъект.РасшифровкаПлатежаСКлиентомВознаграждение.Количество() > 0 Тогда
					ДокументОбъект.РасшифровкаПлатежаСКлиентомВознаграждение.Очистить();
					ОбъектИзменен = Истина;
				КонецЕсли;
				
				Если Не ДокументОбъект.Проведен И ДокументОбъект.РасшифровкаПлатежаСПоставщикомВознаграждение.Количество() > 0 Тогда
					ДокументОбъект.РасшифровкаПлатежаСПоставщикомВознаграждение.Очистить();
					ОбъектИзменен = Истина;
				КонецЕсли;
