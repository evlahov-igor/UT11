...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// << 30.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-749
...
Процедура ЗаполнитьДокументНаОснованииЗаказа(Знач ЗаказКлиента)
	...
	Если ДокументОснование.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоставкаПодПринципала Тогда
		...
		НалогообложениеНДС = ДокументОснование.НалогообложениеНДС;
		ЗаполнитьЦеныПоЗаказуПринципала();
	КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииДоговора(Знач СправочникОснование)
	...
	Запрос = Новый Запрос("
		...
		// Ошибка типового механизма 1С
		//|	Договор.Ссылка = &СправочникОснование
		|	Договор.Ссылка = &ДоговорОснование
		// >> 28.11.2022 Марченко С.Н., КРОК, JIRA№A2105505-918	
		|");
...
// << 30.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-749
// Расширение типового события "Перед записью"
//
// Параметры:
...
Процедура КР_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	КР_КонтрольУникальностиНомераЗаказаПоДаннымПоставщика(Отказ, РежимЗаписи);
	
КонецПроцедуры // >> 30.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-749
...
// << 30.11.2022,  Федоров Д.Е.,  КРОК,  Jira№A2105505-749
// Проверка уникальности реквизита "Номера заказа по данным поставщика"
//
// Параметры:
//  Отказ - Булево - Признак отказа от записи.
//  РежимЗаписи - РежимЗаписиДокумента - Текущий режим записи документа.
//
Процедура КР_КонтрольУникальностиНомераЗаказаПоДаннымПоставщика(Отказ, РежимЗаписи)

	Если Отказ Тогда
		Возврат;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(НомерПоДаннымПоставщика) Тогда
		// Проверка уникальности для заказа с пустым номером не требуется
		Возврат;
	КонецЕсли;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ШаблонСообщения	= НСтр("ru='Значение %1 «НомерПоДаннымПоставщика» указанное в документе не уникально.'");
		ТекстСообщения 	= СтрШаблон(ШаблонСообщения, НомерПоДаннымПоставщика);
