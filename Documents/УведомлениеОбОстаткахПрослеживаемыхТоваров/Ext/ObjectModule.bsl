...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ЗначениеЗаполнено(РНПТ) Тогда
		...
		УчетПрослеживаемыхТоваровЛокализация.ЗаполнитьНомера(ЭтотОбъект, ОсновныеСредства.Выгрузить(, "НомерСтроки,СтранаПроисхождения,НомерГТД"), "ОсновныеСредства", "НомерГТД");
	КонецЕсли;
