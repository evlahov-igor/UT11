...
Процедура ПриЗаписи(Отказ)
	...
	// << 08.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// << 02.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
	КР_ОбработкаПроведенияДополнительно(Отказ, РежимПроведения);
	// >> 02.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	КР_ИнициализироватьДокументДополнительно(ДанныеЗаполнения);
	// >> 27.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
...
Процедура КР_ПередЗаписьюДополнительно(Отказ, РежимЗаписи, РежимПроведения)
	...
	ВсеОтгружено  = Маршрут.Количество() И (Маршрут.НайтиСтроки(Новый Структура("КР_Отгружено", Ложь)).Количество() = 0);
	ВсеДоставлено = Маршрут.Количество() И (Маршрут.НайтиСтроки(Новый Структура("Доставлено", Ложь)).Количество() = 0);
	
	Если ВсеОтгружено Тогда
		Статус = Перечисления.СтатусыЗаданийНаПеревозку.Отправлено;
	КонецЕсли;
	Если ВсеДоставлено Тогда
		Статус = Перечисления.СтатусыЗаданийНаПеревозку.Закрыто;
	КонецЕсли;
...
КонецПроцедуры // >> 08.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
...
Процедура КР_ОбработкаПроверкиЗаполненияДополнительно(Отказ, ПроверяемыеРеквизиты)
	...
	МассивНепроверяемых.Добавить("Перевозчик");
	
	Если Статус = Перечисления.СтатусыЗаданийНаПеревозку.Формируется Тогда
		МассивНепроверяемых.Добавить("КР_ЭкспедиторПартнер");
		МассивНепроверяемых.Добавить("КР_ЭкспедиторКонтрагент");
		МассивНепроверяемых.Добавить("Маршрут.КР_Перевозчик");
		МассивНепроверяемых.Добавить("Маршрут.КР_Водитель");
		МассивНепроверяемых.Добавить("Маршрут.КР_ТранспортноеСредство");
	// << 15.11.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
	ИначеЕсли Статус = Перечисления.СтатусыЗаданийНаПеревозку.Отправлено
		Или Статус = Перечисления.СтатусыЗаданийНаПеревозку.КПогрузке
		Или Статус = Перечисления.СтатусыЗаданийНаПеревозку.Закрыто Тогда
		МассивНепроверяемых.Добавить("Распоряжения");
		МассивНепроверяемых.Добавить("ТранспортноеСредство");
	// >> 15.11.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
	КонецЕсли;
	
	// << 02.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
	МассивПроверяемых.Добавить("Маршрут.Адрес");
	МассивПроверяемых.Добавить("Маршрут.ВремяС");
	МассивПроверяемых.Добавить("Маршрут.ВремяПо");
	// >> 02.08.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
	
	// << 19.10.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
	СоответствиеТСИТиповВладения = Новый Соответствие;
	Для Каждого СтрТабл Из Маршрут Цикл
		АдресОшибки = " " + НСтр("ru = 'в строке %НомерСтроки% списка ""Маршруты""'");
		АдресОшибки = СтрЗаменить(АдресОшибки, "%НомерСтроки%", СтрТабл.НомерСтроки);
		ПолеТипВладенияТС = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Маршрут", СтрТабл.НомерСтроки, "КР_ТипВладенияТС");
		
		Если ЗначениеЗаполнено(СтрТабл.КР_ТранспортноеСредство)
			И Не ЗначениеЗаполнено(СтрТабл.КР_ТипВладенияТС) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Тип владения ТС""'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки + АдресОшибки, ЭтотОбъект, ПолеТипВладенияТС,, Отказ);
		КонецЕсли;
		
		ТекТипВладения = СоответствиеТСИТиповВладения.Получить(СтрТабл.КР_ТранспортноеСредство);
		Если ТекТипВладения = Неопределено Тогда
			СоответствиеТСИТиповВладения.Вставить(СтрТабл.КР_ТранспортноеСредство, СтрТабл.КР_ТипВладенияТС);
		Иначе
			Если Не ТекТипВладения = СтрТабл.КР_ТипВладенияТС Тогда
				ТекстОшибки = НСтр("ru = 'Есть расхождение в значении колонки ""Тип владения ТС""'");
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки + АдресОшибки, ЭтотОбъект, ПолеТипВладенияТС,, Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	ОбщегоНазначения.УдалитьНепроверяемыеРеквизитыИзМассива(ПроверяемыеРеквизиты, МассивНепроверяемых);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПроверяемыеРеквизиты, МассивПроверяемых, Истина);
	
КонецПроцедуры // >> 24.08.2022, Мельников А.А., КРОК, Jira№ A2105505-376

// << 02.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
Процедура КР_ОбработкаПроведенияДополнительно(Отказ, РежимПроведения)
	
	ПроведениеДокументов.ОбработкаПроведенияДокумента(ЭтотОбъект, Отказ);
	
КонецПроцедуры // >> 02.09.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
...
// << 27.12.2022, Маскаев П.Ю., КРОК, Jira№ A2105505-376
Процедура КР_ИнициализироватьДокументДополнительно(ДанныеЗаполнения = Неопределено)
