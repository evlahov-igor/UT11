...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	МеханизмыДокумента.Добавить("КР_РасходыНаПеревозку");
	МеханизмыДокумента.Добавить("КР_ЗаданияНаПеревозку");
	МеханизмыДокумента.Добавить("КР_ВладениеТранспортнымСредством");
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		КР_ЗаполнитьПараметрыИнициализации(Запрос, Документ);
		
		////////////////////////////////////////////////////////////////////////////
		// Сформируем текст запроса
		КР_ТекстЗапросаТаблицаРасходыНаПеревозку(Запрос, ТекстыЗапроса, Регистры);
		КР_ТекстЗапросаТаблицаВладениеТранспортнымСредством(Запрос, ТекстыЗапроса, Регистры);
		КР_ТекстЗапросаТаблицаЗаданияНаПеревозку(Запрос, ТекстыЗапроса, Регистры);
	КонецЕсли;
...
Процедура КР_ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|ИЗ
	|	Документ.ЗаданиеНаПеревозку КАК ДанныеДокумента
	|	
	|ГДЕ
...
Функция КР_ТекстЗапросаТаблицаРасходыНаПеревозку(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "КР_РасходыНаПеревозку";
	...
	СтатусыПодтверждения.Добавить(Перечисления.СтатусыЗаданийНаПеревозку.Отправлено);
	СтатусыПодтверждения.Добавить(Перечисления.СтатусыЗаданийНаПеревозку.Закрыто);
	...
	Запрос.УстановитьПараметр("ПрямаяПеревозка"     , Перечисления.КР_ТипЗаданияНаПеревозку.ПеревозкаТуда);
	Запрос.УстановитьПараметр("ОбратнаяПеревозка"   , Перечисления.КР_ТипЗаданияНаПеревозку.ПеревозкаОбратно);
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаДокумента.Дата                                      КАК Период,
	|	ТаблицаМаршруты.КР_НомерСчета                              КАК НомерСчета,
	|	ТаблицаДокумента.КР_ЭкспедиторПартнер                      КАК Экспедитор,
	|	ВЫБОР
	|		КОГДА ТаблицаМаршруты.КР_ТипЗадания = &ОбратнаяПеревозка
	|			ТОГДА ТаблицаМаршруты.КР_ПолучательОтправитель
	|		ИНАЧЕ ТаблицаДокумента.Склад
	|	КОНЕЦ                                                      КАК Отправитель,
	|	ТаблицаМаршруты.КР_ДатаСчета                               КАК ДатаСчета,
	|	ТаблицаМаршруты.КР_Подтверждено                            КАК Подтверждено,
	|	ВЫБОР
	|		КОГДА ТаблицаМаршруты.КР_ТипЗадания = &ОбратнаяПеревозка
	|			ТОГДА ТаблицаДокумента.Склад
	|		ИНАЧЕ ТаблицаМаршруты.КР_ПолучательОтправитель
	|	КОНЕЦ                                                      КАК Получатель,
	|	ТаблицаМаршруты.КР_ТранспортноеСредство                    КАК ТранспортноеСредство,
	|	ТаблицаМаршруты.КлючСвязи                                  КАК ИдентификаторСтроки,
	|	ТаблицаМаршруты.КР_Маршрут                                 КАК Маршрут,
	|
	|	ТаблицаМаршруты.КР_ТарифСтоимость                          КАК СтоимостьБезНДС,
	|	ТаблицаМаршруты.КР_ПРРСтоимость                            КАК СтоимостьПРРБезНДС,
	|	ТаблицаМаршруты.КР_ДополнительноСтоимость                  КАК СтоимостьДополнительноБезНДС,
	|	ТаблицаМаршруты.КР_СтоимостьДоставки                       КАК СтоимостьИтогоБезНДС,
	|	ТаблицаМаршруты.КР_ОбщаяСтоимостьДоставкиСНДС              КАК СтоимостьИтогоСНДС,
	|	ВЫБОР
	|		КОГДА Маршруты.ВидТарифа = &ВидТарифаПаллеты
	|			ТОГДА 0
	|		ИНАЧЕ ТаблицаМаршруты.Объем
	|	КОНЕЦ                                                      КАК Объем,
	|	ВЫБОР
	|		КОГДА Маршруты.ВидТарифа = &ВидТарифаПаллеты
	|			ТОГДА ТаблицаМаршруты.КР_КоличествоПаллет
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                      КАК КоличествоПаллет,
	|
	|	ТаблицаМаршруты.КР_СтавкаНДС                               КАК СтавкаНДС,
	|	ТаблицаМаршруты.КР_ТарифСтоимостьСНДС                      КАК СтоимостьСНДС,
	|	ТаблицаМаршруты.КР_ПРРСтоимостьСНДС                        КАК СтоимостьПРРСНДС,
	|
	|	ТаблицаДокумента.ДатаВремяРейсаПланС                       КАК ДатаОтгрузки,
	|	ТаблицаМаршруты.КР_ФактическаяДатаДоставки                 КАК ДатаДоставки,
	|	ТаблицаМаршруты.КР_ТарифСтавка                             КАК Тариф
	|ИЗ
	|	Документ.ЗаданиеНаПеревозку.Маршрут КАК ТаблицаМаршруты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаданиеНаПеревозку КАК ТаблицаДокумента
	|		ПО ТаблицаМаршруты.Ссылка = ТаблицаДокумента.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КР_Маршруты КАК Маршруты
	|		ПО ТаблицаМаршруты.КР_Маршрут = Маршруты.Ссылка
	|ГДЕ
	|	ТаблицаМаршруты.Ссылка = &Ссылка
	|	И ТаблицаДокумента.Статус В(&СтатусыПодтверждения)";
...
Функция КР_ТекстЗапросаТаблицаВладениеТранспортнымСредством(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "КР_ВладениеТранспортнымСредством";
	...
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	тч_Маршруты.Ссылка.КР_ЭкспедиторПартнер КАК Экспедитор,
	|	тч_Маршруты.КР_ТранспортноеСредство КАК ТранспортноеСредство,
	|	тч_Маршруты.КР_ТипВладенияТС КАК ТипВладенияТС
	|ИЗ
	|	Документ.ЗаданиеНаПеревозку.Маршрут КАК тч_Маршруты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КР_ВладениеТранспортнымСредством КАК КР_ВладениеТранспортнымСредством
	|		ПО тч_Маршруты.Ссылка.КР_ЭкспедиторПартнер = КР_ВладениеТранспортнымСредством.Экспедитор
	|			И тч_Маршруты.КР_ТранспортноеСредство = КР_ВладениеТранспортнымСредством.ТранспортноеСредство
	|			И тч_Маршруты.КР_ТипВладенияТС = КР_ВладениеТранспортнымСредством.ТипВладенияТС
	|ГДЕ
	|	тч_Маршруты.Ссылка = &Ссылка
	|	И НЕ тч_Маршруты.КР_ТранспортноеСредство = ЗНАЧЕНИЕ(Справочник.ТранспортныеСредства.ПустаяСсылка)
	|	И КР_ВладениеТранспортнымСредством.Регистратор ЕСТЬ NULL";
...
Функция КР_ТекстЗапросаТаблицаЗаданияНаПеревозку(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "КР_ЗаданияНаПеревозку";
	...
	|	ТаблицаДокумента.Дата КАК Период,
	|	ТаблицаМаршруты.Ссылка КАК ЗаданиеНаПеревозку,
	|	ВЫБОР
	|		КОГДА ТаблицаМаршруты.КР_ТипЗадания = ЗНАЧЕНИЕ(Перечисление.КР_ТипЗаданияНаПеревозку.ПеревозкаОбратно)
	|			ТОГДА ТаблицаМаршруты.КР_ПолучательОтправитель
	|		ИНАЧЕ ТаблицаДокумента.Склад
	|	КОНЕЦ КАК Отправитель,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Получатель,
	|	ТаблицаМаршруты.КлючСвязи КАК ИдентификаторСтроки,
	|	НАЧАЛОПЕРИОДА(ТаблицаДокумента.ДатаВремяРейсаПланС, ДЕНЬ) КАК ДатаОтгрузки,
	|	1 КАК КВыполнению
	|ИЗ
	|	Документ.ЗаданиеНаПеревозку.Маршрут КАК ТаблицаМаршруты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаданиеНаПеревозку КАК ТаблицаДокумента
