...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		ЭтоРасчетыСКлиентами = РеквизитыДоговора.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем
...
Процедура УстановитьВидимостьЭлементов()
	...
	Если ЭтоРасчетыСКлиентами Тогда
		...
		Элементы.ЭтапыГрафикаИсполненияДоговора.Заголовок = НСтр("ru='График отгрузки'");
	...
	Иначе
		...
		Элементы.ЭтапыГрафикаИсполненияДоговора.Заголовок = НСтр("ru='График поставок'");
	...
	Если Форма.СуммаДоговора <> 0 Тогда
		...
		Для каждого ЭтапГрафика Из Форма.Объект.ЭтапыГрафикаОплаты Цикл
			...
			Если ЭтапГрафика.СуммаПлатежа <> 0 Тогда
				ЭтапГрафика.ПроцентПлатежа = ЭтапГрафика.СуммаПлатежа * 100 / Форма.СуммаДоговора;
			КонецЕсли;
		...
		Иначе
			...
			Для Каждого Стр Из Объект.ЭтапыГрафикаИсполненияДоговора Цикл
				...
				Если сч <> Объект.ЭтапыГрафикаИсполненияДоговора.Количество() Тогда
					НовСтр.СуммаПлатежа = Стр.СуммаИсполнения/СуммаДоговора * Результат.СуммаПлатежа;
					НовСтр.ПроцентПлатежа = Стр.СуммаИсполнения/СуммаДоговора * Результат.ПроцентПлатежа;
...
Процедура СвернутьЭтапыИсполнения()
	ТЗ = Объект.ЭтапыГрафикаИсполненияДоговора.Выгрузить();
	ТЗ.Свернуть("ДатаПоГрафику,Комментарий,ДатаОтгрузки","ПроцентИсполнения,СуммаИсполнения");
