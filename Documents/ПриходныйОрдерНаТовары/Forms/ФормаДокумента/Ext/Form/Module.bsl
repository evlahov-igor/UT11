...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << 07.09.2022 Марченко С.Н., КРОК, JIRA№A2105505-376
...
Процедура Печать(Команда)
	...
	Если Не ЕстьОшибки Тогда
		...
		ОткрытьФорму(ЦенообразованиеКлиент.ИмяФормыПечатьЭтикетокИЦенников(), ПараметрыПечати, , ИдентификаторУникальности);
	КонецЕсли;
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ПриемкаПоОрдеру", Элементы.ИнформацияШапкаПраво); 
	...
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_ИдентификаторСистемыИсточника", Элементы.ИнформацияШапкаПраво); 
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.АвтомаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 28;
		
КонецПроцедуры // >> 07.09.2022 Марченко С.Н., КРОК, JIRA№A2105505-376

// << 03.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-487
&НаКлиентеНаСервереБезКонтекста
Процедура КР_ПоказатьПредставлениеРаспоряжения(РаспоряжениеПредставление, Распоряжение)
...
КонецПроцедуры // >> 03.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-487
