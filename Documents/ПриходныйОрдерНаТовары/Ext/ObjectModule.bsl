...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ПеремещениеТоваров")
			...
			ТекстСообщения = НСтр("ru = 'Для ввода документа оприходования товаров по складу %1 используйте команду " +
				"""Создать на основании > Приемка и размещение товаров""'"); 
...
Процедура КР_ПриКопировании(ОбъектКопирования)    
	...
	КР_ПриемкаПоОрдеру = Неопределено;
...
Процедура КР_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)    
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ПеремещениеТоваров")
		И Склад.КР_ИспользоватьРазмещениеКакПриходныйОрдер Тогда 
		ТекстСообщения = НСтр("ru = 'Для склада %1 в качестве приходного ордера на товары " +
			"используйте документ ""Отбор (размещение) товаров""'"); 
		ТекстСообщения = СтрШаблон(ТекстСообщения, Склад);
		ВызватьИсключение ТекстСообщения;   
	КонецЕсли;	
