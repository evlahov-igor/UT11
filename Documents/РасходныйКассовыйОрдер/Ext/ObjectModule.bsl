...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	// Задача #3091
	// Выполним переопределение значения для заполнения поля ХозяйственнаяОперация, 
	// которое настроено в реквизите документа вместо ОплатаПоставщику,
	// будем использовать одну из доступных в компании хоз.операцию ИнкассацияДенежныхСредствВБанк
	Если ДанныеЗаполнения = Неопределено Тогда
		ДанныеЗаполнения = Новый Структура;
		ДанныеЗаполнения.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ИнкассацияДенежныхСредствВБанк);	
	КонецЕсли;
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту Тогда 
		Если КР_ВозвратИзКассыККМ Тогда 
	...
	// --> Евлахов Игорь Николаевич (Начало) 09.02.2024
	// Задача #3681
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВКассуККМ Тогда
		...
		ПроверяемыеРеквизиты.Добавить("Выдать");
		// --> Евлахов Игорь Николаевич (Начало) 02.05.2024
		// Задача #3985
		// Начало было
		//ПроверяемыеРеквизиты.Добавить("ПоДокументу");
		// Конец было
		...
		Если ПредставлениеКассира <> СокрЛП(Выдать) Тогда
			ТекстСообщения = НСтр("ru = 'Реквизит ""Выдать"" должен содержать текст равный значению реквизита ""Кассир"".'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "Выдать", , Отказ);	
		КонецЕсли;		
		// ..#4384
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ИнкассацияДенежныхСредствВБанк Тогда
		ПроверяемыеРеквизиты.Добавить("Выдать");
	КонецЕсли;
	// <-- Евлахов Игорь Николаевич (Конец) 09.02.2024
