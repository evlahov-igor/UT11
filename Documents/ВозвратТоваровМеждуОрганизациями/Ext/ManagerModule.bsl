...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ИнформацияПоОрганизации = НСтр("ru = 'Возврат из ""%Организация%""'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1) Заполняет реквизит ""Ставка НДС""
	|2) В возвратах товаров между организациями в табличной части Товары заполняет способ определения себестоимости и документ передачи. Для строк с заполненным документом передачи устанавливается способ указания себестоимости ""Из документа передачи"", для остальных — ""Из текущего документа"". 
	|3) В возвратах товаров между организациями в табличной части Товары заполняет номер ГТД. В случае если одной строке из табличной части Товары соответствует несколько номеров ГТД из табличной части ""Виды запасов"", то разбивает строки в табличной части Товары.
	|4) Заполняет реквизит ""Объекты расчетов"" и ""Объекты расчетов получателя"".
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("acac065e-8b9d-41af-8471-194533955500");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ВозвратТоваровМеждуОрганизациями.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ ВозвратТоваровМеждуОрганизациями.Проведен
	|			И ВозвратТоваровМеждуОрганизациями.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	...
	|	ИЛИ ВозвратТоваровМеждуОрганизациями.Проведен
	|			И ВозвратТоваровМеждуОрганизациями.ОбъектРасчетовПолучателя = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|			И ВозвратТоваровМеждуОрганизациями.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровМеждуОрганизациями)
	|	";
