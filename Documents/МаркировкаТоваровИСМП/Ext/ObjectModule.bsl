...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		Если ЭтотОбъект.ВидПродукции = Перечисления.ВидыПродукцииИС.УпакованнаяВода
			И (ОперацииМаркировки.ЭтоПроизводствоРФ
			Или ОперацииМаркировки.ЭтоКонтрактноеПроизводство) Тогда
			
			ШаблонОшибки = НСтр("ru = 'В строке %1 для номенклатурной позиции %2 должны быть указаны данные сертификации'");
			
			Для Каждого СтрокаТЧ Из ЭтотОбъект.Товары Цикл
				
				Если (Не ЗначениеЗаполнено(СтрокаТЧ.ВидДокументаСертификации) И ЗначениеЗаполнено(СтрокаТЧ.НомерСкважины))
					Или (ЗначениеЗаполнено(СтрокаТЧ.ВидДокументаСертификации) И Не ЗначениеЗаполнено(СтрокаТЧ.НомерСкважины)) Тогда
					
					ТекстОшибки = СтрШаблон(ШаблонОшибки, СтрокаТЧ.НомерСтроки, СтрокаТЧ.Номенклатура);
					ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТЧ.НомерСтроки, "Сертификация");
					ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, ПутьКТабличнойЧасти,, Отказ);
...
Функция ХешСуммаДокумента() Экспорт
	...
	Пока ДанныеШтрихкодов.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеШтрихкодов.ЗначениеШтрихкода) Тогда
			...
			ДанныеХешСуммыСтроки.Добавить(ДанныеШтрихкодов.ХешСуммаНормализации);
		КонецЕсли;
