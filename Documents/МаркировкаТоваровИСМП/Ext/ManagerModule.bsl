...
Функция СтатусПослеПолученияДанных(ДокументСсылка, Операция, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Операция = Перечисления.ВидыОперацийИСМП.ПолучениеРезультатаОбработкиДокумента
		...
		Иначе
			Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				ДокументСсылка,
				"ВидПродукции, Операция, Организация, ДокументОснование, Контрагент, ОперацияНанесения, ОтчетПроизводственнойЛинии, ВариантЗаполненияДекларации");
			Если Реквизиты.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотИмпортСФТС Тогда
				Если Реквизиты.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.АТК Тогда
					Статусы.ПринятДействия.Добавить(Перечисления.ДальнейшиеДействияПоВзаимодействиюИСМП.ВыполнитеФормированиеАТК);
				ИначеЕсли Реквизиты.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.КодыУпаковок Тогда
...
#Область ПолучениеДанныхДокумента
	...
	ТребуетсяАгрегация   = ТребуетсяАгрегация(ДокументСсылка, Шапка);
	...
	Если ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(Шапка.ВидПродукции, Ложь, Истина)
		И Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотИмпортСФТС
...
#Область РазборКодовМаркировки
	...
	Если Шапка.ОтчетПроизводственнойЛинии Тогда
		...
		Если СоздаватьШтрихкодыУпаковок Тогда
			...
			ИнформацияПоШтрихкодамEAN = ШтрихкодированиеИС.ДанныеПоШтрихкодамEAN(ДанныеEAN, ПараметрыСканирования, GTINПотребительскихУпаковок);
			...
			Для Каждого СтрокаДанныхОтчета Из ИсточникДанных Цикл
				...
				Иначе
					...
					Если НайденныеСтроки.Количество() = 1 Тогда
						...
						Если СтрокаДерева.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Логистическая Тогда
							ДанныеПоEANGTIN = Новый Структура("Количество, Коэффициент, КоличествоПотребительскихУпаковок, ПроизвольнаяЕдиницаУчета, ТребуетВзвешивания");
					...
					ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.GTIN)
						...
						ШтрихкодированиеИСМПСлужебный.ЗаполнитьСтрокиТаблицыДанныхПоГрупповымУпаковкам(ДанныеПоШтрихкодам, Истина);
				...
				Если ЭтоПродукцияМОТП Тогда
					СтрокаДерева.ХешСуммаНормализации    = Справочники.ШтрихкодыУпаковокТоваров.ХэшСуммаСтроки(СтрокаДанныхОтчета.Штрихкод);
				КонецЕсли;
...
#КонецОбласти
	...
	ИначеЕсли ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(Шапка.ВидПродукции)
		...
		РеквизитыОрганизации = ИнтеграцияГИСМВызовСервера.ИННКППGLNОрганизации(Шапка.Организация, Шапка.ПроизводственныйОбъект);
		...
		Если ЭтоПиво Тогда
			...
			СведенияОбАдресе = РаботаСАдресами.СведенияОбАдресе(
				РезультатОпределенияСУЗ.НастройкаОбмена.ПроизводственныйОбъектАдрес,
				ДополнительныеПараметрыСведенийОбАдресе);
			...
			Иначе
				ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
					СообщениеJSON,
					СтрШаблон(
						НСтр("ru = 'Не удалось определить адрес производственного объекта для станции %1'"),
		...
		ПараметрыЗапроса.Вставить("ТребуетсяВводВОборот", ТребуетсяВводВОборот(Шапка));
		...
		Если Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотИмпортСФТС Тогда
			...
			ИначеЕсли Шапка.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.АТК Тогда
				ПараметрыЗапроса.Вставить("ТребуетсяФормированиеАТК", Истина);
			ИначеЕсли Шапка.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.КодыУпаковок Тогда
				// Следующее действие должно быть "Выполните агрегацию"
				ПараметрыЗапроса.Вставить("ТребуетсяВводВОборот", Ложь);
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого ДанныеСообщения Из ГруппыКодов Цикл
			...
			Если ЭтоПиво Тогда
				...
				ТелоЗапроса.Вставить("productionDate", ИнтеграцияИС.ДатаСЧасовымПоясомЦРПТ(Шапка.ДатаПроизводства, ЧасовойПоясСеанса()));
				Если ЗначениеЗаполнено(РеквизитыОрганизации.КПП) Тогда
					ТелоЗапроса.Вставить("kpp", РеквизитыОрганизации.КПП);
				КонецЕсли;
			КонецЕсли;
			
			Если ЭтоПиво И ЗначениеЗаполнено(ИдентификаторАдреса) Тогда
				ТелоЗапроса.Вставить("fiasId", ИдентификаторАдреса);
			КонецЕсли;
...
Функция АгрегацияJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Если ВидПродукцииИспользуетОтчетыОНанесении
		...
		Если Шапка.Операция = Перечисления.ВидыОперацийИСМП.ВводВОборотИмпортСФТС 
			И Шапка.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.КодыУпаковок Тогда
				ПараметрыЗапроса.Вставить("ТребуетсяВводВОборот", Истина);
		Иначе
			ПараметрыЗапроса.Вставить("ТребуетсяВводВОборот", Ложь);
		КонецЕсли;
...
Процедура ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыЗаполнения)
	...
	КонецЕсли;
	Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукции) И ТребуетсяСрокГодности Тогда
...
Процедура ЗаполнитьМаркируемуюПродукциюДокумента(ТаблицаМаркируемойПродукции, ДокументСсылка) Экспорт
	...
	|	МаркировкаТоваровИСМПТовары.КоличествоПотребительскихУпаковок,
...
Функция ТребуетсяАгрегация(ДокументСсылка, Реквизиты)
	...
	Если Реквизиты.ОтчетПроизводственнойЛинии Тогда
		...
		Запрос = Новый Запрос(
			...
			|	Или Т.ЗначениеШтрихкодаУпаковки <> """")");
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из ДанныеПоШтрихкодам.МаркированныеТовары Цикл
		...
		Если Настройки.УказыватьСертификацию
			...
			Если Настройки.ДоступноУказаниеНомерСкважины
				И Шапка.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.УпакованнаяВода") Тогда
...
Процедура ЗаполнитьСведенияОТоварахПоДаннымОтчета(
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из ДанныеОтчета Цикл
		...
		РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
			СтрокаТЧМаркированныеТовары.НормализованноеЗначениеШтрихкода, Шапка.ВидПродукции, ПользовательскиеПараметрыРазбораКодаМаркировки);
		...
		// Сертификация
		Если Настройки.ДоступноУказаниеНомерСкважины
			И ЗначениеЗаполнено(Шапка.НомерСкважины)
			И Шапка.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.УпакованнаяВода") Тогда
			
			СтрокаТЧ.Вставить("certificate_document_data", Новый Массив);
			
			ДанныеСертификации = Новый Структура;
			ДанныеСертификации.Вставить("certificate_type",   ИнтерфейсИСМПСлужебный.ВидДокументаСертификации(Шапка.ВидДокументаСертификации));
			ДанныеСертификации.Вставить("certificate_number", Шапка.НомерДокументаСертификации);
			ДанныеСертификации.Вставить("certificate_date",   ИнтеграцияИС.ДатаUTC(Шапка.ДатаДокументаСертификации));
			ДанныеСертификации.Вставить("well_number",        Шапка.НомерСкважины);
	...
	Для Каждого СтрокаКодМаркировки Из ПолныеКодыМаркировки Цикл
		...
		Иначе
			...
			Если Лев(Прав(ПолныйКодМаркировки, 6), 2) = "93"
				...
				ПолныйКодМаркировки = ПолныйКодМаркировки + Символ(29) + "3103" + СтрЗаменить(Формат(СтрокаКодМаркировки.Количество, "ЧЦ=6; ЧДЦ=3; ЧН=000000; ЧВН=; ЧГ=0;"), ",", "");
			КонецЕсли;
...
Функция РассчитатьКоличествоВложенныхШтрихкодовПоДаннымДерева(ДеревоУпаковок, СообщениеJSON, СообщенияJSON)
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		ЕстьОшибки = ЕстьОшибки Или РассчитатьКоличествоВложенныхШтрихкодовСтрокиДерева(СтрокаДерева, СообщениеJSON, Итоги);
...
Функция РассчитатьКоличествоВложенныхШтрихкодовСтрокиДерева(СтрокаДерева, СообщениеJSON, Итоги)
	...
	Если СтрокаДерева.Строки.Количество() > 0 Тогда
		...
		Для Каждого ВложеннаяСтрока Из СтрокаДерева.Строки Цикл
			...
			ЕстьОшибки = ЕстьОшибки Или РассчитатьКоличествоВложенныхШтрихкодовСтрокиДерева(ВложеннаяСтрока, СообщениеJSON, Итоги);
	...
	ИначеЕсли  СтрокаДерева.КоличествоПотребительскихУпаковок = 0 И СтрокаДерева.Количество = 0 Тогда
		...
		ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(НСтр("ru = 'Не удалось определить плановое количество потребительских упаковок для %1.'"), СтрокаДерева.НормализованныйШтрихкод));
		ЕстьОшибки = Истина;
