...
Процедура СертификацияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИспользоватьНомерСкважины = ИспользоватьНомерСкважины(ЭтотОбъект);
...
Процедура ЗаполнитьКоличествоПоДаннымОтчета(Команда)
	ПараметрыСканирования = ШтрихкодированиеИСКлиент.ПараметрыСканирования(ЭтотОбъект);
	РезультатОбработки = ЗаполнитьКоличествоПоДаннымОтчетаНаСервере(ПараметрыСканирования);
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТоварыСертификация.Имя);
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.ВидПродукции");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.УпакованнаяВода");
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение =  Новый ПолеКомпоновкиДанных("Объект.Товары.ВидДокументаСертификации");
	ОтборЭлемента.ВидСравнения  = ВидСравненияКомпоновкиДанных.Заполнено;
	
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение =  Новый ПолеКомпоновкиДанных("Объект.Товары.НомерСкважины");
	ОтборЭлемента.ВидСравнения  = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Если ТребуетсяУказатьДанныеДекларации(Форма) Тогда
	...
	ДоступенОтчетПроизводственнойЛинии =
			...
			И Не ЭтоИмпортСФТС
			И (ЭтоПродукцияМОТП Или Не ОперацииМаркировки.ЭтоОперацияНанесения))
	...
	Элементы.ДатаПроизводства.Видимость = (ЭтоКонтрактноеПроизводство
		...
		Или (ЭтоОперацияНанесения И ЭтоПиво));
	...
	Элементы.Сертификация.Видимость                      = ОтчетПроизводственнойЛинии
	                                                        И ЭтоУпакованнаяВода
...
Процедура ЗаполнитьСертификациюНоменклатурыПоСтроке(Форма, ДанныеДляЗаполнения)
	...
	Для Каждого СтрокаДанныхЗаполнения Из МассивДанныхЗаполнения Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			ДанныеСтроки.НомерСкважины              = СтрокаСертифкации.НомерСкважины;
			
			ДанныеЗаполненияСертификации = ИнициализироватьДанныеЗаполненияСертификации();
			ЗаполнитьЗначенияСвойств(ДанныеЗаполненияСертификации, СтрокаСертифкации);
			ДанныеЗаполненияСертификации.ИспользоватьНомерСкважины = ИспользоватьНомерСкважины;
			
			ДанныеСтроки.Сертификация = ПредставлениеСертификации(ДанныеЗаполненияСертификации);
...
Функция ИнициализироватьДанныеЗаполненияСертификации()
	...
	Результат.Вставить("ВидСертификации");
	Результат.Вставить("НомерСертификации");
	Результат.Вставить("ДатаСертификации");
	Результат.Вставить("НомерСкважины");
	Результат.Вставить("ИспользоватьНомерСкважины");
...
Процедура ОткрытьФормуРедактированияСертификации(ИдентификаторыСтрок)
	...
	Если ИдентификаторыСтрок.Количество() = 1 Тогда
		...
		ПараметрыОткрытия.Вставить("НомерСкважины",     ВыбраннаяСтрока.НомерСкважины);
		ПараметрыОткрытия.Вставить("ИспользоватьНомерСкважины", ИспользоватьНомерСкважины);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПредставлениеСертификации(ДанныеСертификации)
	
	Если ДанныеСертификации.ИспользоватьНомерСкважины Тогда
		
		ШаблонПредставлениеСертификата = "%1 №%2 от %3, номер скважины: %4";
...
Процедура СоздатьКэшСертификацииСервер()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		ДанныеЗаполненияСертификации = ИнициализироватьДанныеЗаполненияСертификации();
		ДанныеЗаполненияСертификации.ВидСертификации   = СтрокаТовары.ВидДокументаСертификации;
		ДанныеЗаполненияСертификации.ДатаСертификации  = СтрокаТовары.ДатаДокументаСертификации;
		ДанныеЗаполненияСертификации.НомерСертификации = СтрокаТовары.НомерДокументаСертификации;
		ДанныеЗаполненияСертификации.НомерСкважины     = СтрокаТовары.НомерСкважины;
		ДанныеЗаполненияСертификации.ИспользоватьНомерСкважины = ИспользоватьНомерСкважины;
		
		ОбновитьКэшСертификацииКлиентСервер(ЭтотОбъект, ДанныеЗаполненияСертификации);
		
		СтрокаТовары.Сертификация = ПредставлениеСертификации(ДанныеЗаполненияСертификации);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьКэшСертификацииКлиентСервер(Форма, ДанныеЗаполненияСертификации, Номенклатура = Неопределено)
	...
	Если ДанныеЗаполненияСертификации.ИспользоватьНомерСкважины Тогда
		ПараметрыПоиска = Новый Структура("ВидСертификации, НомерСертификации, ДатаСертификации, НомерСкважины");
	Иначе
...
Процедура ЗаполнитьПредставлениеСертификации()
	...
	Если Объект.ОтчетПроизводственнойЛинии И ИспользоватьНомерСкважины Тогда
		
		Если Не ЗначениеЗаполнено(Объект.ВидДокументаСертификации) Тогда
			Возврат;
		КонецЕсли;
		...
		Сертификация = ПредставлениеСертификации(ДанныеЗаполненияСертификации);
	...
	Иначе
		...
		Для Каждого Строка Из Объект.Товары Цикл
			...
			ДанныеЗаполненияСертификации = ИнициализироватьДанныеЗаполненияСертификации();
			ДанныеЗаполненияСертификации.ВидСертификации   = Строка.ВидДокументаСертификации;
			ДанныеЗаполненияСертификации.ДатаСертификации  = Строка.ДатаДокументаСертификации;
			ДанныеЗаполненияСертификации.НомерСертификации = Строка.НомерДокументаСертификации;
			ДанныеЗаполненияСертификации.НомерСкважины     = Строка.НомерСкважины;
			ДанныеЗаполненияСертификации.ИспользоватьНомерСкважины = ИспользоватьНомерСкважины;
			
			Строка.Сертификация = ПредставлениеСертификации(ДанныеЗаполненияСертификации);
...
Процедура ЗаполнениеСертификацииЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ДанныеЗаполненияСертификации.ИспользоватьНомерСкважины = ИспользоватьНомерСкважины(ЭтотОбъект);
...
Процедура ЗаполнитьДоступныеОперации()
	...
	Если ДоступныОперацииНанесения
			И(ИспользуетсяПодсистемаВЕТИС Или Не ЭтоМолочнаяПродукция) Тогда
	...
	ВидыПродукцииОперацииИмпортСФТС.Вставить(Перечисления.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС, Истина);
	ВидыПродукцииОперацииИмпортСФТС.Вставить(Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС, Истина);
	ВидыПродукцииОперацииИмпортСФТС.Вставить(Перечисления.ВидыПродукцииИС.УпакованнаяВода, Истина);
...
&НаСервере
Функция ЗаполнитьКоличествоПоДаннымОтчетаНаСервере(ПараметрыСканирования)
	...
	ПараметрыРазбора.Вставить("УпрощеннаяОбработкаКодовМаркировки",     Истина);
	...
	ПараметрыРазбора.Вставить("ЗаполнитьДанныеРазбора",                 Ложь);
	...
	Результат.КоличествоПоДокументу = КоличествоПотребительскихУпаковок * ОписаниеНоменклатуры.КоличествоВПотребительскойУпаковке;
...
&НаКлиентеНаСервереБезКонтекста
Функция ИспользоватьНомерСкважины(Форма)
	ОперацииМаркировки = ИнтеграцияИСМПСлужебныйКлиентСервер.ОперацииМаркировки(Форма.Объект.Операция);
	ВидПродукции = Форма.Объект.ВидПродукции;
