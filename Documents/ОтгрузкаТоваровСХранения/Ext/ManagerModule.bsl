...
Функция МакетФормыВыбораНазначений() Экспорт
	...
	ШаблонНазначенияБезАгентскойЗакупки.УсловиеИспользования = "Объект.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоставкаПодПринципала)";
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		|ГДЕ
		|	ТабЧасть.ЗаказКлиента = ЗНАЧЕНИЕ(Документ.ЗаказКлиента.ПустаяСсылка)";
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		СтруктураДанныхШапки.Вставить("ТекстЗаголовка",
										ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
											ДанныеПечати,
											НСтр("ru='Отгрузка товаров с хранения'", ОбщегоНазначения.КодОсновногоЯзыка())));
		...
		СтруктураДанныхИтоговаяСтрока.Вставить("СуммаПрописью",
												РаботаСКурсамиВалютУТ.СформироватьСуммуПрописью(СуммаКПрописи, ДанныеПечати.Валюта));
	...
	Для каждого СтрокаЦена Из ДанныеПоЦенам Цикл
		...
		Для каждого СтрокаОстаток Из СтрокиПоОстаткам Цикл
			Если СтрокаОстаток.КоличествоОстаток > СтрокаЦена.КоличествоОстаток 
				...
				СтрокаОстаток.Сумма = ?(ПараметрыДанных.ЦенаВключаетНДС, СтрокаОстаток.СуммаСНДС, СтрокаОстаток.Цена * СтрокаЦена.КоличествоОстаток);
			...
			Иначе 
				...
				СтрокаОстаток.СуммаСНДС = СтрокаЦена.СуммаСНДСОстаток;
				СтрокаОстаток.СуммаНДС = СтрокаЦена.СуммаНДСОстаток;
				СтрокаОстаток.Сумма = ?(ПараметрыДанных.ЦенаВключаетНДС, СтрокаОстаток.СуммаСНДС,
					СтрокаОстаток.СуммаСНДС - СтрокаОстаток.СуммаНДС);
	...
	Для каждого СтрокаПоЗаказуИОтгрузки Из ДанныеПоЗаказуИОтгрузке Цикл
		...
		Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
			...
			Если КоличествоОстаток > 0 Тогда
				...
				НоваяСтрокаОтгрузки.СуммаСНДС = СтрокаОстатка.Сумма;
		...
		Для каждого СтрокаОстаткаПоОрганизации Из СтрокиСОстаткамиПоОрганизации Цикл
			...
			Если КоличествоОстаток = СтрокаПоЗаказуИОтгрузки.КоличествоВЗаказе Тогда
				...
				НоваяСтрокаОтгрузки.СуммаСНДС = СтрокаОстаткаПоОрганизации.СуммаСНДС;
				НоваяСтрокаОтгрузки.СуммаНДС = СтрокаОстаткаПоОрганизации.СуммаНДС;
			Иначе                                                                                          
				ПроцентНДС = УчетНДСУПКлиентСервер.ЗначениеСтавкиНДС(НоваяСтрокаОтгрузки.СтавкаНДС);
				Если ПараметрыДанных.ЦенаВключаетНДС Тогда
					НоваяСтрокаОтгрузки.Сумма = НоваяСтрокаОтгрузки.СуммаСНДС;
					НоваяСтрокаОтгрузки.СуммаНДС = ?(ПроцентНДС > 0, НоваяСтрокаОтгрузки.СуммаСНДС / (ПроцентНДС + 100) * ПроцентНДС, 0);
				Иначе
					НоваяСтрокаОтгрузки.Сумма = ?(ПроцентНДС > 0, НоваяСтрокаОтгрузки.СуммаСНДС / (ПроцентНДС + 100) * 100, НоваяСтрокаОтгрузки.СуммаСНДС);
					НоваяСтрокаОтгрузки.СуммаНДС = НоваяСтрокаОтгрузки.СуммаСНДС - НоваяСтрокаОтгрузки.Сумма;
				КонецЕсли;
			КонецЕсли;
			НоваяСтрокаОтгрузки.Цена = ?(НоваяСтрокаОтгрузки.КоличествоУпаковок = 0, 0,
										?(ПараметрыДанных.ЦенаВключаетНДС, НоваяСтрокаОтгрузки.СуммаСНДС / НоваяСтрокаОтгрузки.КоличествоУпаковок,
										НоваяСтрокаОтгрузки.Сумма / НоваяСтрокаОтгрузки.КоличествоУпаковок));
...
Запрос.Текст = "ВЫБРАТЬ
   ...
   |	ВтТоварыРеализации.СуммаСНДС КАК СуммаСНДС,
   |	ВтТоварыРеализации.СуммаНДС КАК СуммаНДС
   ...
   |	ВтТоварыРеализации.СуммаСНДС,
   |	ВтТоварыРеализации.СуммаНДС
   |;
   |
   |////////////////////////////////////////////////////////////////////////////////
   |ВЫБРАТЬ
   ...
   |		ТоварыЗаказаУпаковка.СуммаСНДС,
   |		ТоварыЗаказаУпаковка.СуммаНДС
   |	ИЗ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("9c1cec40-d5a7-45e1-ae21-3425f3fe9fb1");
	...
	Изменяемые.Добавить(Метаданные.Документы.ОтгрузкаТоваровСХранения.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ОтгрузкаТоваровСХранения";
	...
	|		ГДЕ
	|			ОтгрузкаТоваровСХранения.Ссылка = ОтгрузкаТоваровСХраненияТовары.Ссылка
	|			И ОтгрузкаТоваровСХраненияТовары.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ОтгрузкаТоваровСХраненияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ОтгрузкаТоваровСХранения.ВидыЗапасов КАК ОтгрузкаТоваровСХраненияВидыЗапасов
	|		ГДЕ
	|			ОтгрузкаТоваровСХранения.Ссылка = ОтгрузкаТоваровСХраненияВидыЗапасов.Ссылка
	|			И ОтгрузкаТоваровСХраненияВидыЗапасов.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И ОтгрузкаТоваровСХраненияВидыЗапасов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)))";
