...
Процедура ЗаполнитьТаблицуТовары()
	...
	Для каждого СтрокаОтгрузки Из ДанныеПоОтгрузке Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТовара.СтавкаНДС) Тогда
			СтрукутраОбъект = Новый Структура("Дата, Организация", Дата, Организация);
			СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", ОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияСтавкиНДС(СтрукутраОбъект));
		КонецЕсли;
