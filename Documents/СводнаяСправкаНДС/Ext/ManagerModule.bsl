...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.СводнаяСправкаНДС";
	...
	|			Документ.СводнаяСправкаНДС.Продажи КАК СводнаяСправкаНДСПродажи
	|		ГДЕ
	|			СводнаяСправкаНДС.Ссылка = СводнаяСправкаНДСПродажи.Ссылка
	|			И СводнаяСправкаНДСПродажи.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|			И СводнаяСправкаНДСПродажи.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)))
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.СводнаяСправкаНДС";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Для Каждого СтрокаПродаж Из ДокументОбъект.Продажи Цикл
				Если Не ЗначениеЗаполнено(СтрокаПродаж.СтавкаНДС) Тогда
					СтрокаПродаж.СтавкаНДС = УчетНДСЛокализация.СтавкаНДСПоПеречислению(СтрокаПродаж.УдалитьСтавкаНДС);
