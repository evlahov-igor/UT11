...
Процедура ХозяйственнаяОперацияПриИзменении(Элемент)
	...
	// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188  
	КР_ХозяйственнаяОперацияПриИзмененииНаСервере();
	// >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
...
Процедура Подключаемый_ПриВыполненииКоманды(Команда)
	...
	ВнутреннееПотреблениеТоваровКлиентЛокализация.ПриВыполненииКоманды(Команда, ЭтаФорма);
...
Процедура СкладПриИзмененииСервер()
	...
	// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
	Если Объект.Склад.КР_СкладСПомещениямиБезордернойОтгрузки Тогда 
		
		Объект.КР_Помещение = Справочники.СкладскиеПомещения.КР_СкладскоеПомещениеСкладМагазина(Объект.Склад); 
		
		Объект.КР_Ячейка = Справочники.СкладскиеЯчейки.КР_СкладскаяЯчейкаПоХозяйственнойОперация(
			Объект.Склад, Объект.ХозяйственнаяОперация);
...
Процедура ПриВыполненииКомандыЗавершениеНаСервере(Знач ИмяКоманды, Знач ДополнительныеПараметры)
	...
	ВнутреннееПотреблениеТоваровЛокализация.ПриВыполненииКоманды(ИмяКоманды, ЭтотОбъект);
...
Процедура СтатусПриИзмененииСервер(ЗаполнитьСерииПоОтгрузке)
	...
	Если ЗаполнитьСерииПоОтгрузке Тогда
		Документы.ВнутреннееПотреблениеТоваров.ЗаполнитьСерииПоОтгрузке(Объект);
	КонецЕсли;
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	// Модифицируем группы размещения 
	//	так как при добавлении элементов в правую все "плывет" 
	Элементы.ШапкаПраво.Объединенная = Истина;
	Элементы.ШапкаЛево.Объединенная = Истина;
	
	// КР_Помещение
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Помещение", Элементы.ШапкаПраво);	
	...
	// A2105505-2314
	ЭлементФормы.УстановитьДействие("ПриИзменении", "КР_ПомещениеПриИзменении");
	//
	
	// КР_Ячейка
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Ячейка", Элементы.ШапкаПраво);	
	ЭлементФормы.АвтоМаксимальнаяШирина = Ложь;
	ЭлементФормы.МаксимальнаяШирина = 28;
	ЭлементФормы.ТолькоПросмотр = Не ЭтоПолноправныйПользователь; 
...
КонецПроцедуры // >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188

// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
&НаСервере
Процедура КР_УстановитьВидимостьДоступностьЭлементов() 
	
	// A2105505-2314 Рефакторинг, доработка
	Элементы.КР_Помещение.Видимость = Объект.Склад.КР_СкладСПомещениямиБезордернойОтгрузки;
	Элементы.КР_Ячейка.Видимость = (Объект.КР_Помещение.НастройкаАдресногоХранения 
		= Перечисления.НастройкиАдресногоХранения.ЯчейкиОстатки);
	
КонецПроцедуры // >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188

// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
&НаСервере
Процедура КР_ХозяйственнаяОперацияПриИзмененииНаСервере() 
...
// << 28.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2314
&НаКлиенте
Процедура КР_ПомещениеПриИзменении(Элемент)  
	КР_УстановитьВидимостьДоступностьЭлементов();	
КонецПроцедуры // >> 28.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-2314   
