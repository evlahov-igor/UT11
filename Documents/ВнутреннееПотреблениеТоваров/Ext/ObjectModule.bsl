...
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.ВнутреннееПотреблениеТоваров - документ, для которого выполняется инициализация параметров.
...
//	ТаблицыДокумента - см. Документы.ВнутреннееПотреблениеТоваров.КоллекцияТабличныхЧастейТоваров.
//
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	КР_ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка);
	// >> 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
...
Процедура ЗаполнитьАналитикиУчетаНоменклатурыВТабличныхЧастяхТоваров(ТаблицыДокумента = Неопределено)
	...
	Если ТаблицыДокумента = Неопределено Тогда
		ТаблицыДокумента = Документы.ВнутреннееПотреблениеТоваров.КоллекцияТабличныхЧастейТоваров();
...
// << 03.02.2023 Марченко С.Н., КРОК, JIRA№A2105505-1188
Процедура КР_ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка); 
	
	// << 06.05.2024 Петухов А.В., Фактор, #3938
	//ЭтоМагазин = (Склад.КР_ВидСклада = Перечисления.КР_ВидыСкладов.Магазин);
	//
	//ИндексПервого = 0;
	//Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") 
	//	И ДанныеЗаполнения.Свойство("МассивЗаказов") Тогда
	//    ЗаказНаВнутреннееПотребление = ДанныеЗаполнения.МассивЗаказов[ИндексПервого];
	...
	//	И ЭтоМагазин Тогда 
	//	
	//	Дата = ЗаказНаВнутреннееПотребление.Дата;

	//КонецЕсли;  
	// >> 06.05.2024 Петухов А.В., Фактор, #3938
	
	Если Склад.КР_СкладСПомещениямиБезордернойОтгрузки Тогда 
		КР_Помещение = Справочники.СкладскиеПомещения.КР_СкладскоеПомещениеСкладМагазина(Склад);
	КонецЕсли;	     
	
	КР_Ячейка = Справочники.СкладскиеЯчейки.КР_СкладскаяЯчейкаПоХозяйственнойОперация(
		Склад, ХозяйственнаяОперация);
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов)   
	...
	// A2105505-2314 доработка
	Если КР_Помещение.НастройкаАдресногоХранения <> Перечисления.НастройкиАдресногоХранения.ЯчейкиОстатки Тогда 
		МассивНепроверяемыхРеквизитов.Добавить("КР_Ячейка");   
	КонецЕсли;	
