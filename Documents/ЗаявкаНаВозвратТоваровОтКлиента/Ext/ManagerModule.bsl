...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|ПО
		|	ВтРезервПревышаетОстатки.Ссылка = ДокументЗаявка.Ссылка
		|ЛЕВОЕ СОЕДИНЕНИЕ
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'от комиссионера'");
	ИначеЕсли Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтХранителя Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'от хранителя'");
	Иначе
		ПостфиксИмениДокумента = НСтр("ru = 'от клиента'");
	КонецЕсли;
	
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Заявка на возврат товаров %1 %2 от %3'"),
...
Процедура ЗаполнитьТабличныйДокументЗаявкиНаВозврат(ТабличныйДокумент, Запрос, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ЕстьТоварыНаЗамену Тогда
			ВидДокумента = НСтр("ru = 'Заявка на замену'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			ВидДокумента = НСтр("ru = 'Заявка на возврат'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
		...
		ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(
			...
			ИмяКолонкиКодов,
			НСтр("ru = 'Возвращаемый товар'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ДанныеПечати,
			ПараметрыВывода
		);
		...
		Если ЕстьТоварыНаЗамену Тогда
			...
			ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(
				...
				ЗаменяющиеТоварыТЧ,
				ИмяКолонкиКодов,
				НСтр("ru = 'Заменяющий товар'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ДанныеПечати,
				ПараметрыВывода
			);
...
Процедура ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(ТабличныйДокумент,
	...
	Если ПараметрыВывода.ТолькоКоличество Тогда
		ШапкаТаблицы = "ШапкаТаблицыБезСумм";
	...
	Иначе
		Если ПараметрыВывода.УчитыватьНДС И ПоказыватьНДС И ПараметрыВывода.ЕстьСкидки Тогда
			ШапкаТаблицы = "ШапкаТаблицыСНДССоСкидкой";
	...
	Если ПараметрыВывода.ВыводитьКоды Тогда
		СтруктураДанныхКоды = Новый Структура("ИмяКолонкиКодов", ПараметрыВывода.ПредставлениеКолонкиКодов);
	...
	Для Каждого Строка Из ТЧ Цикл
		...
		Если  ИмяКолонкиТоваров = НСтр("ru = 'Заменяющий товар'", ОбщегоНазначения.КодОсновногоЯзыка()) Тогда
	...
	Если ПараметрыВывода.ВыводитьПодвалНДС И НЕ ПараметрыВывода.ТолькоКоличество Тогда
		...
		Иначе
			...
			КонецЕсли;
			СтруктураДанныхНДС = Новый Структура;
			СтруктураДанныхНДС.Вставить("НДС", НСтр("ru = 'Без налога (НДС)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ПараметрыВывода.ТолькоКоличество Тогда
		ИтоговаяСтрока = НСтр("ru = 'Всего наименований %Количество%'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		...
		СуммаПрописью = РаботаСКурсамиВалютУТ.СформироватьСуммуПрописью(СуммаКПрописи, Валюта);
	КонецЕсли;
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Заполняет реквизит ""Подразделение"" в табличной части ""Заменяющие товары"" для номенклатур типа ""Работа"".
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("e6a1e994-eee5-4e8a-9d18-ff57a50ea4b8");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаявкаНаВозвратТоваровОтКлиента.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.ВариантОбеспечения В(
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОбособленно),
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно))
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.ИдентификаторСтроки = """"
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|				И ЗаявкаНаВозвратТоваровОтКлиентаЗаменяющиеТовары.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|				И НЕ ЗаявкаНаВозвратТоваровОтКлиента.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)))
	|	ИЛИ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ЗаявкаНаВозвратТоваровОтКлиента.ЭтапыГрафикаОплаты КАК ТЧЭтапыГрафикаОплаты
	|				ГДЕ
	|					ТЧЭтапыГрафикаОплаты.Ссылка = ЗаявкаНаВозвратТоваровОтКлиента.Ссылка
	...
	|							И ТЧЭтапыГрафикаОплаты.ДатаПлатежа <> НАЧАЛОПЕРИОДА(ЗаявкаНаВозвратТоваровОтКлиента.Дата,ДЕНЬ)))
	|	ИЛИ ИСТИНА В(
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			ДокументыДляЗамены.Ссылка = ЗаявкаНаВозвратТоваровОтКлиента.Ссылка)
	|	ИЛИ НЕ ЗаявкаНаВозвратТоваровОтКлиента.ОплатаВВалюте
	|			И (ЗаявкаНаВозвратТоваровОтКлиента.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	...
	|		И ЗаявкаНаВозвратТоваровОтКлиента.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|		И ЗаявкаНаВозвратТоваровОтКлиента.Проведен
	|		И ((ЗаявкаНаВозвратТоваровОтКлиента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтКомиссионера)
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратОтХранителя))
	|		И ЗаявкаНаВозвратТоваровОтКлиента.СпособКомпенсации = ЗНАЧЕНИЕ(Перечисление.СпособыКомпенсацииВозвратовТоваров.ЗаменитьТовары)
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.СпособКомпенсации = ЗНАЧЕНИЕ(Перечисление.СпособыКомпенсацииВозвратовТоваров.ВернутьДенежныеСредства))
	|		И (ЗаявкаНаВозвратТоваровОтКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам)
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным)
	|			ИЛИ ЗаявкаНаВозвратТоваровОтКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоДоговорамНакладным)))";
	
	Запрос.УстановитьПараметр("ИспользоватьРасширенныеВозможностиЗаказаКлиента",
		Константы.ИспользоватьРасширенныеВозможностиЗаказаКлиента.Получить());

	Запрос.Текст = ОбеспечениеВДокументахСервер.ДобавитьТекстВременнойТаблицыДокументыДляЗаменыКОбеспечениюНаТребуется(
		Запрос.Текст, Документы.ЗаявкаНаВозвратТоваровОтКлиента);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаявкаНаВозвратТоваровОтКлиента"; ИмяТЧ = "ЗаменяющиеТовары";
	...
	Текст = ОбеспечениеВДокументахСервер.ТекстЗапросаВременнойТаблицыДокументыДляЗаменыКОбеспечениюНаТребуется(Документы.ЗаявкаНаВозвратТоваровОтКлиента);
	ТекстыЗапроса.Добавить(Текст);
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				СкладыСервер.ЗаполнитьКолонкуТЧПодразделение(ДокументОбъект, ОбъектИзменен, "ЗаменяющиеТовары");
				#КонецОбласти
				...
				Иначе
					Для Каждого СтрТовары Из ДокументОбъект.ЗаменяющиеТовары Цикл
				...
				ОбеспечениеВДокументахСервер.ОбработатьНеиспользуемыеВариантыОбеспечения(
					ДокументОбъект.ЗаменяющиеТовары,
					ОбъектИзменен,
...
Функция ТекстЗапросаДокументыДляЗаменыКОбеспечениюНаТребуется() Экспорт
	Возврат ОбеспечениеВДокументахСервер.ТекстЗапросаДокументыДляЗаменыКОбеспечениюНаТребуется(
		"Документ.ЗаявкаНаВозвратТоваровОтКлиента", "ЗаменяющиеТовары", Истина);
КонецФункции
