...
Процедура КР_ПриЗаписи(Отказ)
	...
	Если Проведен Тогда  
		КР_ЗапуститьФоновоеЗаданиеОбновленияСтраныПроисхождения();
	КонецЕсли;
	
КонецПроцедуры // >> 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918

// << 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
Процедура КР_ЗапуститьФоновоеЗаданиеОбновленияСтраныПроисхождения()
	
	НаименованиеЗадания = НСтр("ru = 'Обновление страны происхождения'");   
	ИмяПроцедуры = "Документы.ТаможеннаяДекларацияИмпорт.КР_ОбновитьСтраныПроисходения";
	...
	Запрос = Новый Запрос(КР_ТекстЗапросаДанныеДляОбновленияСтаныПроисходжения());
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	
	Данные = Запрос.Выполнить().Выгрузить();
	
	ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, ИмяПроцедуры, Данные);
...
// << 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
Функция КР_ТекстЗапросаДанныеДляОбновленияСтаныПроисходжения()
	...
	|ГДЕ
	|	Т.Ссылка = &Ссылка
	|	И Т.Номенклатура.СтранаПроисхождения <> Т.СтранаПроисхождения
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Номенклатура,
	|	Т.СтранаПроисхождения";

КонецФункции // >> 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
