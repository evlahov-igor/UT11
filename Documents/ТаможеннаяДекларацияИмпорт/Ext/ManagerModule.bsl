...
// << 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
Процедура КР_ОбновитьСтраныПроисходения(Данные) Экспорт 
	
	Для Каждого СтрокаДанных Из Данные Цикл 
		
		НоменклатураОбъект = СтрокаДанных.Номенклатура.ПолучитьОбъект();
		НоменклатураОбъект.СтранаПроисхождения = СтрокаДанных.СтранаПроисхождения;
		НоменклатураОбъект.Записать();
		
	КонецЦикла;	  
	
КонецПроцедуры // >> 20.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918	    
