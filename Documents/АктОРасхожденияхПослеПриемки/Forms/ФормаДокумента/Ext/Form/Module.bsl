...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли НоменклатураКлиент.ЭтоУказаниеНомераГТД(ИсточникВыбора)
		...
		НоменклатураКлиент.ОбработатьУказаниеНомераГТД(ЭтаФорма, ВыбранноеЗначение);
...
Процедура КонтрагентПриИзмененииСервер()
	...
	Если Объект.ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратОтКомиссионера
		Или Объект.ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратТоваровОтКлиента Тогда 
		Объект.КР_СкладРазбораРасхождений = 
			Документы.АктОРасхожденияхПослеПриемки.КР_ПолучитьСкладРазбораРасхождений(Объект.Контрагент);  
	Иначе 
		Объект.КР_СкладРазбораРасхождений = Неопределено;
	КонецЕсли;	
	// >> 17.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	// << 17.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
	ХозяйственныеОперации = Перечисления.ХозяйственныеОперации;
	Элементы.КР_СкладРазбораРасхождений.Видимость = (
		Объект.ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратОтКомиссионера
		Или Объект.ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратТоваровОтКлиента
	);
	// >> 17.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
...
Функция ЗаполнитьПоПриемкеСервер()
	...
	КолонкаЗаказ           = ТаблицаЗаказы.Колонки.ЗаказПоставщику; // КолонкаТаблицыЗначений
	...
	Для Каждого КлючЗначение Из ДействияСПолученнымиТаблицами Цикл
		...
		Для Проход = 1 По Проходов Цикл
			...
			Иначе
				...
				// Есть накладные и заказы
				Условие = "[КоличествоМаксимум], [КоличествоВЗаказе], ПО [КоличествоВЗаказе]";
				НакладныеСервер.РаспределитьКоличествоИЗаполнить(ТаблицаОрдераПоДействию, ТаблицаНакладнойПоДействию, "КоличествоВОрдере", Ключ, Условие, Ложь, "Серия");
				...
				// Есть накладные, нет заказов
				Условие = "[КоличествоМаксимум], НЕ [КоличествоВЗаказе], ПО [КоличествоМаксимум]";
				НакладныеСервер.РаспределитьКоличествоИЗаполнить(ТаблицаОрдераПоДействию, ТаблицаНакладнойПоДействию, "КоличествоВОрдере", Ключ, Условие, Истина, "Серия");
