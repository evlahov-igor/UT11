...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// << 22.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
		КР_ТекстЗапросаТаблицаЗаявкиНаВозвратТоваровОтКлиентов(Запрос, ТекстыЗапроса, Регистры);	
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	ИначеЕсли РасхожденияКлиентСервер.ТипОснованияПоступлениеТоваровОтХранителя(Данные.ТипОснованияАктаОРасхождении) Тогда
		Обработчик = Документы.ПоступлениеТоваровОтХранителя.ОбработчикДействийПоТипуОснованияАкта(Данные.ТипОснованияАктаОРасхождении);
		ПостфиксИмениДокумента = Обработчик.ПостфиксИмениАкта();
	ИначеЕсли Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ВозвратОтКомиссионера Тогда
		ПостфиксИмениДокумента = НСтр("ru = 'после возврата от комиссионера'");
	Иначе
...
Процедура ОформитьИзлишкиНедостачиНаСкладе(Запрос, ТекстыЗапроса, Регистры)
	...
	Если ПроведениеДокументов.ТребуетсяТаблицаДляДвижений("ТоварыКОформлениюИзлишковНедостач", Регистры) Тогда
		СкладыСервер.ОформитьИзлишкиНедостачиНаСкладеАкты(Запрос, ТекстыЗапроса, "ТоварыКОформлениюИзлишковНедостач", ТекстЗапросаДокумента);
	КонецЕсли;
...
Процедура ОформитьПоступлениеТоваровПоОдноходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ТОГДА
	|				ТаблицаТовары.ДокументОснование.ПоступлениеПоЗаказам
	|			ИНАЧЕ
	|				ЛОЖЬ
	|		КОНЕЦ
	|	КАК ПоступлениеПоЗаказам,
...
Процедура ПринятьТоварНаСклад(Запрос, ТекстыЗапроса, Регистры)
	...
	|			КОГДА НЕ ТаблицаТовары.ДокументОснование.ПоступлениеПоЗаказам
	...
	|		ЗНАЧЕНИЕ(Перечисление.СпособыОтраженияАктовОРасхожденияПослеПоступления.ИсправлениеПервичныхДокументов)
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И (
	|		(
	|		ТаблицаТовары.Ссылка.СпособОтраженияРасхождений = ЗНАЧЕНИЕ(Перечисление.СпособыОтраженияАктовОРасхожденияПослеПоступления.ИсправлениеПервичныхДокументов)
	|		И ТаблицаТовары.Действие В(
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ОформитьНедостачуИОжидатьДопоставку)
	|			)
	|		)
	...
	СкладыСервер.ПринятьТоварНаСклад(Запрос, ТекстыЗапроса, РегистрыПоступления, ТекстЗапросаДокумента, "ТаблицаТовары");
...
Процедура ОформитьПоступлениеТоваровПоДвухходовке(Запрос, ТекстыЗапроса, Регистры)
	...
	Если ЗначениеЗаполнено(Регистры) Тогда
		СодержитРегистрТоварыКПоступлению = Ложь;
		Если ТипЗнч(Регистры) = Тип("Строка")
			И Регистры = "ТоварыКПоступлению" Тогда
			СодержитРегистрТоварыКПоступлению = Истина;
		ИначеЕсли ТипЗнч(Регистры) = Тип("Структура")
			И Регистры.Свойство("ТоварыКПоступлению") Тогда
			СодержитРегистрТоварыКПоступлению = Истина;
		КонецЕсли;
		Если Не СодержитРегистрТоварыКПоступлению Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		// Сторно документа "Корректировка поступления", так как в приходном ордере уже отразили излишек/недостачу на складе.
		"ВЫБРАТЬ
		|	ТабЧасть.Ссылка                                      КАК Ссылка,
		|	ТабЧасть.Ссылка.Дата                                 КАК Период,
		|	ТабЧасть.Номенклатура                                КАК Номенклатура,
	...
	ТекстЗапросаТабЧасть =
		// При способе отражения посредством оформления корректировок акт закрывает график поступления.
		"ВЫБРАТЬ
		|	ТабЧасть.Ссылка                                         КАК Ссылка,
		|	ТабЧасть.Ссылка.Дата                                    КАК Период,
		|	ТабЧасть.Номенклатура                                   КАК Номенклатура,
...
//
// Параметры:
//  Реквизиты - ДеревоЗначений - список реквизитов шаблона, содержит:
//     * Имя            - Строка - Уникальное имя общего реквизита.
//     * Представление  - Строка - Представление общего реквизита.
//     * Тип            - Тип    - Тип реквизита. По умолчанию строка.
//     * Формат         - Строка - Формат вывода значения для чисел, дат, строк и булевых значений.
//  Вложения                - ТаблицаЗначений - печатные формы и вложения, содержит:
...
//
// Параметры:
//  Сообщение - Структура - структура с ключами, содержит:
...
// << 22.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
Функция КР_ПолучитьСкладРазбораРасхождений(Клиент) Экспорт   
	
	ВидСкладаДляПоиска = Перечисления.КР_ВидыСкладов.СкладРазбораРасхожденийМаркетплейса;  	
	...
	Запрос.УстановитьПараметр("МаркетплейсСкладаРасхождений", Клиент);       
...
КонецФункции // >> 22.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
...
// << 22.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
Функция КР_ТекстЗапросаТаблицаЗаявкиНаВозвратТоваровОтКлиентов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаТовары.ЗаказПоставщику         	КАК ЗаявкаНаВозвратТоваровОтКлиента,
	|	ТаблицаТовары.Номенклатура   			КАК Номенклатура,
	|	ТаблицаТовары.Характеристика 			КАК Характеристика,
	|	ТаблицаТовары.Ссылка.Договор          	КАК Договор,
	|	ТаблицаТовары.КодСтроки      			КАК КодСтроки,
    ...
    |		ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьНаСкладРазбора)	
	|";      
...
Функция КР_ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НачалоПериода(&Период, День) 			КАК Период,
	|	ВЫБОР 
	|		КОГДА ТаблицаТовары.ЗаказПоставщику НЕ В (
	|				НЕОПРЕДЕЛЕНО,
	|				ЗНАЧЕНИЕ(Документ.ЗаказПоставщику.ПустаяСсылка),
	|				ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка))
	|			ТОГДА ТаблицаТовары.ЗаказПоставщику
	|		ИНАЧЕ ТаблицаТовары.ДокументОснование	
...
Функция КР_ТекстЗапросаПервыйНайденныйСкладПоВидуСклада()
	...
	|	И Склады.КР_ВидСклада = &ВидСкладаДляПоиска
	|	И Склады.КР_МаркетплейсСкладаРасхождений = &МаркетплейсСкладаРасхождений";
