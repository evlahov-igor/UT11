...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратОтКомиссионера
		Или ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратТоваровОтКлиента Тогда 
		
		КР_СкладРазбораРасхождений = 
			Документы.АктОРасхожденияхПослеПриемки.КР_ПолучитьСкладРазбораРасхождений(Контрагент);
...
Процедура КР_ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ХозяйственныеОперации = Перечисления.ХозяйственныеОперации; 
	ВариантыДействийПоРасхождениямВАктеПослеПриемки = Перечисления.ВариантыДействийПоРасхождениямВАктеПослеПриемки;
	Если ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратОтКомиссионера
		Или ХозяйственнаяОперация = ХозяйственныеОперации.ВозвратТоваровОтКлиента Тогда 
	
		ОтборСтрокПереместитьНаСкладРазбора = Новый Структура("Действие", 
			ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьНаСкладРазбора);
		ОтборСтрокПереместитьСоСкладаРазбора = Новый Структура("Действие", 
			ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьСоСкладаРазбора);
...
КонецПроцедуры // >> 22.01.2024 Марченко С.Н., КРОК, JIRA№A2105505-2597
...
Функция КР_ТекстЗапросаДанныеЗаполненияТехническихПеремещений()  
	...
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьСоСкладаРазбора), 
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьНаСкладРазбора)
	|	)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.ДокументОснование.Склад,
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьНаСкладРазбора)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.КР_ПереместитьСоСкладаРазбора)
	|	КОНЕЦ КАК Действие,
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.РазницаКоличества > 0
