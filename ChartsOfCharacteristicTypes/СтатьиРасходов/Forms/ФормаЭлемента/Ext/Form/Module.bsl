...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Элементы.ГруппаСтраницаРеглУчет.Заголовок = НСтр("ru = 'Налоги и учетные системы'");
	...
	Тогда
		...
		Элементы.ГруппаКлассификацияРасходовДляРеглУчета.Видимость = Ложь;
	КонецЕсли;
	...
	Если ЭтоУТ Тогда
		...
		КонецЕсли;
		Элементы.ПереключательИспользоватьПолныеВозможности.Видимость = Ложь;
	КонецЕсли;
...
Процедура КР_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	СвязанСБазовойСтатьей = ЗначениеЗаполнено(Объект.КР_БазоваяСтатья);
	Если СвязанСБазовойСтатьей Тогда 
		ТолькоПросмотр = Истина;
	КонецЕсли;
	
	// КР_БазоваяСтатья
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_БазоваяСтатья", , Элементы.Родитель);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.Видимость = СвязанСБазовойСтатьей;
	
	// КР_Коллекция
	ЭлементФормы = КР_МетодыМодификацииФорм.ВставитьЭлементФормы(ЭтотОбъект,
		"Объект.КР_Коллекция", , Элементы.Родитель);
	ЭлементФормы.ТолькоПросмотр = Истина;
	ЭлементФормы.Видимость = СвязанСБазовойСтатьей;
			
КонецПроцедуры // >> 19.12.2022 Марченко С.Н., КРОК, JIRA№A2105505-918
