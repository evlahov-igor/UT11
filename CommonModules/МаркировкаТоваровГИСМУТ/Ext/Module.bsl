...
Функция ДанныеДокументаДляПроведенияПеремаркировкаТоваров(Документ, Регистры, ДопПараметры)
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		ЗаполнитьПараметрыИнициализацииПеремаркировкаТоваров(Запрос, Документ);
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегиструПеремаркировкаТоваров(ИмяРегистра) Экспорт
	...
	ИначеЕсли ИмяРегистра = "РаспределениеЗапасовДвижения" Тогда
		...
		ТекстыЗапроса = Новый СписокЗначений();
		СинонимТаблицыДокумента = "ТабЧасть";
		ОтразитьРаспределениеЗапасовДвиженияПеремаркировкаТоваров(Запрос, ТекстыЗапроса, ИмяРегистра);
		ТекстЗапроса = СтрСоединить(ТекстыЗапроса.ВыгрузитьЗначения(), ОбщегоНазначенияУТ.РазделительЗапросовВОбъединении());
