...
Процедура ПостроитьДеревоЦен(Форма) Экспорт
	...
	Если ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыДерево") Или ТипЗнч(Форма.ДеревоЦен) = Тип("ДанныеФормыКоллекция") Тогда
		...
		Для Каждого ВидЦены Из Форма.ВыбранныеЦены Цикл
			...
			Если ВидЦены.Выбрана ИЛИ ПоказыватьВлияющуюЦену Тогда
				...
				Если Не ЭтоРегистрацияЦенПоставщика Тогда
					Если Справочники.ВидыЦен.РазрешитьИзменятьФормулы(ВидЦены.СпособЗаданияЦены) Тогда
...
Функция ЗагрузитьСправочникВидовЦен(Форма, НастройкиКомпоновкиДанных, МассивВидовЦен = Неопределено, ТаблицаНастроекВидовЦен = Неопределено) Экспорт
	...
	|			
	|ГДЕ
	|	НЕ ВидыЦен.ЭтоГруппа И НЕ ВидыЦен.ПометкаУдаления 
	|УПОРЯДОЧИТЬ ПО
	...
	Для Каждого СтрокаВидаЦен Из ТаблицаВидовЦен Цикл
		...
		Если СтрокаВидаЦен.ПометкаУдаления ИЛИ СтрокаВидаЦен.ЗапрещенныйВидЦены ИЛИ НЕ СтрокаВидаЦен.ВидЦеныВМассиве Тогда
			...
			Если СтрокаВидаЦен.ЗависимыеЦены.Количество() <> 0 Тогда
				Для Каждого ВидЦены Из СтрокаВидаЦен.ЗависимыеЦены Цикл
					...
					Если НайденноеЗначение <> Неопределено Тогда
						Если Не НайденноеЗначение.ПометкаУдаления ИЛИ СтрокаВидаЦен.ВидЦеныВМассиве Тогда
							УдалитьВидЦен = Ложь;
							Прервать;
						КонецЕсли;
					КонецЕсли;
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Иначе
			...
			Если ИерархическоеДерево Тогда
				...
				НайденнаяИерархия = НайтиСтрокуДереваЦен(Форма, ПараметрыОтбора, КэшДанных);
		...
		ЗаполнитьФормулу(ВыбранныеВидыЦен, ЭлементНоменклатура, КэшДанных);
			...
			Пока ВыборкаХарактеристики.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(ВыборкаХарактеристики.Характеристика) Тогда
					...
					Иначе
						...
						ЗаполнитьФормулу(ВыбранныеВидыЦен, ЭлементХарактеристика, КэшДанных);
					КонецЕсли;
...
//  ТаблицаЗначений - соответствие номенклатуры и характеристики с добавленной строкой дерева цен.
//
Функция ДобавитьТоварыПредприятия2_5(Форма, Знач ТаблицаТовары, КэшДанных) Экспорт
	...
	Параметры.Вставить("ВыбранныеВидыЦен",		ВыбранныеВидыЦен);
				...
				Если ТекущаяЦена = 0 Или Не ТолькоНезаполненные Тогда
					...
					Если ЗначениеЗаполнено(Формула) Тогда
						...
						Для Каждого БазоваяЦена Из ВидЦены.ВлияющиеЦены Цикл
							...
							ВалютаЦены            =  ?(Значениезаполнено(СтрокаТаблицыЦен["Валюта" + ВидЦены.ИмяКолонки]), СтрокаТаблицыЦен["Валюта" + ВидЦены.ИмяКолонки], ВидЦены.Валюта);
						...
						Попытка
							...
							СтрокаТаблицыЦен["Формула" + ВидЦены.ИмяКолонки] = ПредставлениеФормулы;
	...
	Для Каждого ВидЦены Из ВидыЦен Цикл
		...
		Для Каждого СтрокаВидЦены Из МассивСтрокЗависимыхЦен Цикл
			Если ЗависимыеЦены.Найти(СтрокаВидЦены) = Неопределено
				И ВидыЦен.Найти(СтрокаВидЦены) = Неопределено 
				И СтрокаВидЦены.Редактировать Тогда
...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПредприятия2_5(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекущиеДанные.ВидЦен <> Выборка.ВидЦены Тогда
			...
			ИмяКолонки = НайтиСтрокуДереваЦен(Форма, Выборка, КэшДанных);;
		КонецЕсли;
		...
		НайденнаяСтрока["СтараяЦена"           + ИмяКолонки] = Выборка.ДействующаяЦена;
		...
		Если Выборка.ЗапретРедактирования И Выборка.ДействующаяЦена <> 0 Тогда
...
Процедура ЗагрузитьСтарыеЦеныНоменклатуры(Форма, ТаблицаНоменклатуры, КэшДанных) Экспорт
	...
	Иначе
		ТаблицаНоменклатурыЗапрещеннойКРедактированию = УстановкаЦенСервер.ПолучитьЗапрещеннуюКРедактированиюНоменклатуру(Форма, ТаблицаНоменклатуры);
		Если Форма.ИспользуетсяЦенообразование25 Тогда
		...
		Пока ВыборкаХарактеристики.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаХарактеристики.Характеристика) Тогда
				...
				Пока ВыборкаВидыЦен.Следующий() Цикл
					...
					Если ЗначениеЗаполнено(ВыборкаВидыЦен.ФормулаДействующая) Тогда
						
						ЭлементХарактеристика["Формула" 		 		+ ИмяКолонки] = ВыборкаВидыЦен.ФормулаДействующая;
						ЭлементХарактеристика["ФормулаИзмененаВручную" 	+ ИмяКолонки] = Истина;
						
					ИначеЕсли ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаДругойВидЦен Тогда
						
						БазовыйВидЦены = ВыборкаВидыЦен.БазовыйВидЦены; // СправочникСсылка.ВидыЦен
						ЭлементХарактеристика["Формула" 		 + ИмяКолонки] = "[" + БазовыйВидЦены.Идентификатор + "]*" + Формат(((ВыборкаВидыЦен.Наценка/100)+1),"ЧРД=.; ЧГ=0");
						
					ИначеЕсли ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен 
					 ИЛИ ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаЦенуПоступления 
					 ИЛИ ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаЦенуВводаОстатков
					 ИЛИ ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоКонкурентам 
					 ИЛИ ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоПоставщикам 
					 ИЛИ ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоСебестоимости Тогда 
						
						ЭлементХарактеристика["Формула" 		 + ИмяКолонки] = ВыборкаВидыЦен.Формула;
			...
			Иначе
				...
				Пока ВыборкаВидыЦен.Следующий() Цикл
					...
					Если ЗначениеЗаполнено(ВыборкаВидыЦен.ФормулаДействующая) Тогда
						...
						ЭлементНоменклатура["Формула" 		 		+ ИмяКолонки] = ВыборкаВидыЦен.ФормулаДействующая;						
						ЭлементНоменклатура["ФормулаИзмененаВручную" 	+ ИмяКолонки] = Истина;						
						
					ИначеЕсли ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаДругойВидЦен Тогда
						
						БазовыйВидЦены = ВыборкаВидыЦен.БазовыйВидЦены; // СправочникСсылка.ВидыЦен
						ЭлементНоменклатура["Формула" 		 + ИмяКолонки] = "[" + БазовыйВидЦены.Идентификатор + "]*" + Формат(((ВыборкаВидыЦен.Наценка/100)+1),"ЧРД=.; ЧГ=0");
					...
					ИначеЕсли ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен 
					 ...
					 ИЛИ ВыборкаВидыЦен.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоСебестоимости Тогда 
						...
						ЭлементНоменклатура["Формула" 		 + ИмяКолонки] = ВыборкаВидыЦен.Формула;
...
Процедура ЗагрузитьСтарыеЦеныНоменклатурыПрайсЛист2_5(Форма, ТаблицаНоменклатуры, КэшДанных, ТаблицаНоменклатурыЗапрещеннойКРедактированию = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НайденнаяСтрока["СтараяЦена"           + ИмяКолонки] = Выборка.ДействующаяЦена;
		НайденнаяСтрока["ПроцентИзменения"     + ИмяКолонки] = Выборка.Процент;
		НайденнаяСтрока["СуммаИзменения"       + ИмяКолонки] = Выборка.Цена - Выборка.ДействующаяЦена;
		НайденнаяСтрока["ЗапретРедактирования" + ИмяКолонки] = Выборка.ЗапретРедактирования;
		...
		Если ЗначениеЗаполнено(Выборка.ФормулаДействующая) Тогда
			НайденнаяСтрока["Формула"                + ИмяКолонки] = Выборка.ФормулаДействующая;
			НайденнаяСтрока["ФормулаИзмененаВручную" + ИмяКолонки] = Истина;
		ИначеЕсли Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаДругойВидЦен Тогда
			БазовыйВидЦены = Выборка.БазовыйВидЦены; // СправочникСсылка.ВидыЦен
			НайденнаяСтрока["Формула" + ИмяКолонки] = "[" + БазовыйВидЦены.Идентификатор + "]*" + Формат(((Выборка.Наценка/100)+1),"ЧРД=.; ЧГ=0");
		ИначеЕсли Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен 
		 ИЛИ Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаЦенуПоступления 
		 ИЛИ Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.НаценкаНаЦенуВводаОстатков
		 ИЛИ Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоКонкурентам 
		 ИЛИ Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоПоставщикам 
		 ИЛИ Выборка.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоСебестоимости Тогда 
			НайденнаяСтрока["Формула" + ИмяКолонки] = Выборка.Формула;
		КонецЕсли;
	...
	Для Каждого СтруктураТовар Из МассивСтруктурТовары Цикл
		...
		Попытка
			...
			Если ИспользуетсяЦенообразование25 Тогда
				...
				НоваяСтрока.СерияЦОДляСвязи    = ?(НоваяСтрока.СерияЦО.Предопределенный, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), НоваяСтрока.СерияЦО);
...
Процедура ЗагрузитьТабличнуюЧастьТовары(Форма, КэшДанных) Экспорт
	...
	Если Не ЭтоРегистрацияЦенПоставщика(Форма, КэшДанных) Тогда
		...
		Если Форма.ИспользуетсяЦенообразование25 Тогда
			...
			Для Каждого Строка Из ТаблицаТоваров Цикл
				Строка.СерияЦОДляСвязи   = ?(Строка.СерияЦО.Предопределенный, Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка(), Строка.СерияЦО);
	...
	Если Не ЭтоРегистрацияЦенПоставщика(Форма, КэшДанных) Тогда
		ТаблицаНоменклатурыЗапрещеннойКРедактированию = УстановкаЦенСервер.ПолучитьЗапрещеннуюКРедактированиюНоменклатуру(Форма, ТаблицаТоваров);
		Запрос.УстановитьПараметр("ТоварыЗапрещенныеКРедактированию", ТаблицаНоменклатурыЗапрещеннойКРедактированию);
	КонецЕсли;
...
//
Функция НайтиСтрокуДереваЦен(Форма, ВходящиеДанныеПоиска, КэшДанных) Экспорт
...
//
Процедура ДеревоЦенУпаковкаПриИзменении(Форма, ИдентификаторСтроки, ИмяТекущейКолонки) Экспорт
	...
	// Загрузка старых цен и процента изменения
	ТаблицаЗначений = СоздатьТаблицуТовары(ЭтоРегистрацияЦенПоставщика(Форма), Форма.ИспользуетсяЦенообразование25);
	Для Каждого СтрокаВидЦены Из Форма.ВыбранныеЦены Цикл
...
Процедура ДобавитьТоварыПоОтборамСКД(Форма) Экспорт
...
Процедура ДобавитьТоварыДляОбнуленияЦенПоОтборамСКД(Форма) Экспорт
	...
	Для Каждого СтрокаВидЦены Из УстановкаЦенКлиентСервер.ВыбранныеСтрокиТаблицыВидовЦен(Форма) Цикл
		...
		Если НастройкиКомпоновкиДанных.Отбор.Элементы.Количество() > 0 Тогда
			...
			ГруппаЭлементовОтбора = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(КомпоновщикНастроек.Настройки.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаНе);
...
Процедура УстановитьЦены(Знач МассивВидовЦен, Знач СтруктураФормы, Знач ВидОбновленияЦен = "Обновление")
	...
	Если ВидОбновленияЦен = "Обновление" Тогда
		...
		УстановкаЦенСервер.ДобавитьТоварыПоОтборамСКД(СтруктураФормы);
...
Функция СостояниеПереходаНаЦенообразованиеВерсии2_5() Экспорт 
	...
	Для Каждого КлючИЗначение Из Выборка.ПараметрыШага1.Получить() Цикл
		СостояниеПерехода.ПараметрыШага1.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из Выборка.ПараметрыШага2.Получить() Цикл
		СостояниеПерехода.ПараметрыШага2.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из Выборка.ПараметрыШага3.Получить() Цикл
...
Процедура ДобавитьНоменклатуруПоЦеновымГруппамИТоварам(Форма, ВидЦены, ЦеновыеГруппы, Товары)
	...
	Если ЦеновыеГруппы.количество() Тогда
		...
		УстановкаЦенСервер.ДобавитьТоварыПоОтборамСКД(Форма);
	...
	Если ТаблицаНоменклатуры.Количество() > 0 Тогда
		...
		Если СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
			...
			Если ПроверкаСКДПройдена2_5 Тогда
				...
				ВнешниеНаборыДанных = Новый Структура;
				//ВнешниеНаборыДанных.Вставить("ТаблицаНоменклатуры", ТаблицаНоменклатуры.Скопировать(, "Номенклатура, ХарактеристикаЦО, СерияЦО, УпаковкаЦО"));
...
Процедура ЗагрузитьТоварыВДеревоЦен(Форма, ВыборкаНоменклатура, КэшДанных)
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Если Форма.ИспользоватьУпаковкиНоменклатуры Тогда
			...
			Если ЭлементНоменклатура.Свойство("УпаковкаЗаказа" + ИмяКолонки) Тогда
		...
		Если ЭлементНоменклатура.Свойство("МинимальнаяПартияПоставки" + ИмяКолонки) Тогда
			ЭлементНоменклатура["МинимальнаяПартияПоставки"  + ИмяКолонки] = ВыборкаНоменклатура.МинимальнаяПартияПоставки;
		КонецЕсли;
		
		Если ЭлементНоменклатура.Свойство("УстановкаУсловий" + ИмяКолонки) Тогда
			ЭлементНоменклатура["УстановкаУсловий"  + ИмяКолонки] = ВыборкаНоменклатура.УстановкаУсловий;
		КонецЕсли;
		
		Если Не ЭтоРегистрацияЦенПоставщика Тогда

			ЭлементНоменклатура["ЗапретРедактирования"  + ИмяКолонки] = ВыборкаНоменклатура.ЗапретРедактирования;
			ЭлементНоменклатура["Формула"               + ИмяКолонки] = ВыборкаНоменклатура.Формула;
		...
		ИначеЕсли Параметры.Операция = "ИзменитьЦены" Тогда
			Для Каждого ВидЦен Из Параметры.ВидыЦен Цикл
				...
				Если ЭлементДереваЦенИсточник["СтараяЦена" + ИмяКолонки] <> 0 Тогда
					ПроцентИзменения = Окр(100 * (НоваяЦена - ЭлементДереваЦенИсточник["СтараяЦена" + ИмяКолонки]) / ЭлементДереваЦенИсточник["СтараяЦена" + ИмяКолонки], 10, 2);
				Иначе
					ПроцентИзменения = 0;
				КонецЕсли;
					
				ЭлементДереваЦенИсточник["ПроцентИзменения" + ИмяКолонки] = ПроцентИзменения;
				ЭлементДереваЦенИсточник["СуммаИзменения"   + ИмяКолонки] = ЭлементДереваЦенИсточник[ИмяКолонки] - ЭлементДереваЦенИсточник["СтараяЦена"+ИмяКолонки];
				
				ЭлементДереваЦенИсточник["ИзмененаВручную" + ИмяКолонки] = Истина;
			...
			Для Каждого ВидЦен Из Параметры.ВидыЦен Цикл
				...
				Если ЭлементДереваЦенИсточник["СтараяЦена"+ИмяКолонки] <> 0 Тогда
					...
					Если Параметры.ИзменятьПоОтношениюКСтаройЦене Тогда
						...
						ЭлементДереваЦенИсточник[ИмяКолонки] = Окр(ЭлементДереваЦенИсточник["СтараяЦена"+ИмяКолонки]*((ЭлементДереваЦенИсточник["ПроцентИзменения"+ИмяКолонки]/100)+1), 2);
						ЭлементДереваЦенИсточник["СуммаИзменения"   + ИмяКолонки] = ЭлементДереваЦенИсточник[ИмяКолонки] - ЭлементДереваЦенИсточник["СтараяЦена"+ИмяКолонки];
					Иначе
						ЭлементДереваЦенИсточник[ИмяКолонки] = Окр(ЭлементДереваЦенИсточник[ИмяКолонки]*((ВидЦен.ПроцентИзменения/100)+1), 2);
						ЭлементДереваЦенИсточник["ПроцентИзменения" + ИмяКолонки] = (ЭлементДереваЦенИсточник[ИмяКолонки] / ЭлементДереваЦенИсточник["СтараяЦена" + ИмяКолонки] - 1) * 100; 
						ЭлементДереваЦенИсточник["СуммаИзменения"   + ИмяКолонки] = ЭлементДереваЦенИсточник[ИмяКолонки] - ЭлементДереваЦенИсточник["СтараяЦена"+ИмяКолонки];
					КонецЕсли;
...
//    * ВыбранныеВидыЦен - Массив - выбранных цен.
...
Процедура ДобавитьТоварПредприятияФрагмент(Форма, КэшДанных, ТекущийЭлемент, ВыборкаСтрок, Параметры)
	...
	ЗаполнитьФормулу(Параметры.ВыбранныеВидыЦен, ТекущийЭлемент, КэшДанных);
...
Процедура ЗаполнитьФормулу(Знач ВыбранныеВидыЦен, Элемент, КэшДанных)
				...
				Если Справочники.ВидыЦен.РазрешитьИзменятьФормулы(ВидЦены.СпособЗаданияЦены) Тогда
						...
						Если ЦеновыеГруппыАлгоритмов.Количество() > 0 Тогда
							Если ВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен Тогда
								Формула = ЦеновыеГруппыАлгоритмов[0].Формула;
							Иначе
								Формула = "[" + ЦеновыеГруппыАлгоритмов[0].БазовыйВидЦены.Идентификатор + "]*" + Формат(((ЦеновыеГруппыАлгоритмов[0].Наценка/100)+1),"ЧРД=.; ЧГ=0");
							КонецЕсли;
						Иначе
							Если ВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.РассчитыватьПоФормуламОтДругихВидовЦен Тогда
								Формула = ВидЦены.Формула;
							Иначе
								Формула = "[" + ВидЦены.БазовыйВидЦены.Идентификатор + "]*" + Формат(((ВидЦены.Наценка/100)+1),"ЧРД=.; ЧГ=0");
							КонецЕсли;
						КонецЕсли;
