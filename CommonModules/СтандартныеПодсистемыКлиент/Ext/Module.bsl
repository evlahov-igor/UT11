...
Процедура ПриНачалеРаботыСистемы(Знач ОповещениеЗавершения = Неопределено, НепрерывноеВыполнение = Истина) Экспорт
	...
	// << 11.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-991
	Если КР_ИспользуетсяРежимЗагрузкиДанных() Тогда    
		ОткрытьФорму("Обработка.КР_МенеджерЗагрузкиДанных.Форма");
	КонецЕсли;	
	// >> 11.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-991

	//Петухов А.В.  24.10.2023  #3269   {
	злОбновлениеКонфигурацииКлиент.ПриНачалеРаботыСистемы();
	//}	
...
Функция ОтключенаЛогикаНачалаРаботыСистемы() Экспорт
	Возврат СтрНайти(ПараметрЗапуска, "ОтключитьЛогикуНачалаРаботыСистемы") > 0
		...
		Или КР_ИспользуетсяРежимЗагрузкиДанных()	
		// >> 08.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-991
	;
КонецФункции
...
// << 08.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-991
// Проверяет, запущена ли база в режиме загрузки данных
//
// Возвращаемое значение:
//  Булево
//
Функция КР_ИспользуетсяРежимЗагрузкиДанных() Экспорт
	Возврат СтрНайти(ПараметрЗапуска, "КР_РежимЗагрузкиДанных") > 0;
КонецФункции // >> 08.09.2023 Марченко С.Н., КРОК, JIRA№A2105505-991
