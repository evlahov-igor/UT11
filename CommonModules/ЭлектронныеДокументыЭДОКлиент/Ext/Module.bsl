...
Процедура ПроверитьПодписиДокумента(ОповещениеОЗавершении, ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	ОбработатьРезультатПроверкиПодпсейСервера(ОповещениеОЗавершении, РезультатПроверки, КонтекстДиагностики);
...
// 
// Возвращаемое значение:
//  См. ЭлектронныеДокументыЭДОВызовСервера.ПроверитьПодписиПоМЧДПриРегистрацииСертификатов
Функция ПроверитьПодписиПоМЧДПриРегистрацииСертификатов(НаборПараметровПроверок, КонтекстДиагностики) Экспорт
	Возврат ЭлектронныеДокументыЭДОВызовСервера.ПроверитьПодписиПоМЧДПриРегистрацииСертификатов(
...
Процедура Подписать(ПараметрыПодписания)
	...
	Отказ = Ложь;
	ПередПодписанием(ОписаниеНабораДанных, ПараметрыПодписания, ОтборСертификатов, Отказ);
	...
	Если ОписаниеНабораДанных.ТребуетсяЗаполнитьПодписантов ИЛИ ОписаниеНабораДанных.ЗаполнениеДанныхДоверенностейДоступно Тогда
		...
		Для Каждого ДанныеСообщения Из ОписаниеНабораДанных.НаборДанных Цикл
			ДанныеСообщения.Вставить("ИсходныеДвоичныеДанные", ДанныеСообщения.Данные);
			Если ДанныеСообщения.ТребуетсяЗаполнитьПодписанта ИЛИ ДанныеСообщения.ЗаполнениеДанныхДоверенностиДоступно Тогда
	...
	Оповещение = Новый ОписаниеОповещения("ОбработатьРезультатПодписания", ЭлектронныеДокументыЭДОКлиент, ПараметрыПодписания);
	Если Отказ Тогда
...
Процедура ЗаполнитьПодписантовПослеВыбораСертификата(Результат, ПараметрыПодписания) Экспорт
	...
	ПараметрыПодписания.ТребуетсяДоверенность = ОписаниеНабораДанных.СертификатыТребующиеДоверенность[Сертификат] <> Неопределено;
	Если ОписаниеНабораДанных.ЗаполнениеДанныхДоверенностейДоступно
		И ПараметрыПодписания.ТребуетсяДоверенность И ОписаниеНабораДанных.НаборДанных.Количество() Тогда
		
		ПараметрыВыбора = Новый Структура;
		ПараметрыВыбора.Вставить("Сертификат", Сертификат);
		ПараметрыВыбора.Вставить("Организация", ОписаниеНабораДанных.НаборДанных[0].Организация);
		
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьПодписантовИДоверенностиПослеУточненияМЧД",
			ЭтотОбъект, ПараметрыПодписания);
		ОткрытьФорму("Справочник.МашиночитаемыеДоверенностиОрганизаций.Форма.ВыборДоверенности",
...
Процедура ЗаполнитьПодписантовИДоверенностиПослеУточненияМЧД(РезультатУточненияМЧД, ПараметрыПодписания) Экспорт
	...
	Для Каждого ДанныеСообщения Из ОписаниеНабораДанных.НаборДанных Цикл
		...
		ПараметрыЗаполнения.Вставить("ТребуетсяЗаполнитьДоверенность", ПараметрыПодписания.ТребуетсяДоверенность);
		...
		Если ПараметрыЗаполнения.ТребуетсяЗаполнитьПодписанта ИЛИ ПараметрыЗаполнения.ТребуетсяЗаполнитьДоверенность Тогда
	...
	ИндексыДляУдаления = Новый Массив; // Массив из Число
	ИндексДанных = 0;
	
	Для Каждого ДанныеСообщения Из ОписаниеНабораДанных.НаборДанных Цикл
		
		Если ИсходныеДвоичныеДанные = Неопределено Тогда
			ИсходныеДвоичныеДанные = ДанныеСообщения.ИсходныеДвоичныеДанные;
		КонецЕсли;
	...
	Если ДанныеДляПодписания = Неопределено Тогда
		ДанныеДляПодписания = ИсходныеДвоичныеДанные;
	КонецЕсли;
...
Процедура ОбработатьРезультатПодписания(РезультатПодписания, ПараметрыПодписания) Экспорт
	...
	Иначе
		...
		Если ПараметрыПодписания.ВыбраннаяМЧД = Неопределено И ПараметрыПодписания.ТребуетсяДоверенность Тогда
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьДействияПоЭДОПослеПодписания(ПараметрыПодписания)
	...
	Если ЗначениеЗаполнено(ПараметрыПодписания.ВыбраннаяМЧД) Тогда
		Для Каждого ПодписанноеСообщение Из ПараметрыПодписания.ПодписанныеСообщения Цикл
			ПодписанноеСообщение.ВыбраннаяМЧД = ПараметрыПодписания.ВыбраннаяМЧД;
		КонецЦикла;
	КонецЕсли;
...
Процедура ПередПодписанием(ОписаниеНабораДанных, ПараметрыПодписания, ОтборСертификатов, Отказ)
	
	ОтборСертификатов = Новый Массив;
	Если ОписаниеНабораДанных.ЗаполнениеДанныхДоверенностейДоступно Тогда
		ДокументыКоторыеНельзяПодписатьПоДоверенности = Новый Массив;
		Если ОписаниеНабораДанных.СертификатыТребующиеДоверенность.Количество() = 0
			Или НаборОднороденПоЗаполнениюДанныхДоверенностей(ОписаниеНабораДанных,
				ДокументыКоторыеНельзяПодписатьПоДоверенности) Тогда
			ОтборСертификатов = ОписаниеНабораДанных.Сертификаты;
		Иначе
			Отказ = Истина;
			ТекстОшибки = НСтр("ru = 'Не удалось подписать документы:'");
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ СтрСоединить(ДокументыКоторыеНельзяПодписатьПоДоверенности, Символы.ПС);
			Ошибка = ОбработкаНеисправностейБЭДКлиент.НоваяОшибка(НСтр("ru = 'Подписание электронных документов'"),
				ВидОшибкиЕстьДокументыКоторыеМожноПодписатьПоДоверенности(), ТекстОшибки, ТекстОшибки);
			ОбработкаНеисправностейБЭДКлиент.ДобавитьОшибку(ПараметрыПодписания.КонтекстДиагностики, Ошибка,
	...
	Иначе
		Для Каждого Сертификат Из ОписаниеНабораДанных.Сертификаты Цикл
			Если ОписаниеНабораДанных.СертификатыТребующиеДоверенность[Сертификат] = Неопределено Тогда
				ОтборСертификатов.Добавить(Сертификат);
			КонецЕсли;
		КонецЦикла;
		Если ОтборСертификатов.Количество() = 0 И ОписаниеНабораДанных.Сертификаты.Количество() > 0 Тогда
			Отказ = Истина;
			
			ДокументыКоторыеМожноПодписатьТолькоБезДоверенности = Новый Массив;
			Для Каждого ЭлементНабора Из ОписаниеНабораДанных.НаборДанных Цикл
				ДобавитьПредставлениеЭлементаНабора(ДокументыКоторыеМожноПодписатьТолькоБезДоверенности, ЭлементНабора);
			КонецЦикла;
			
			ТекстОшибки = НСтр("ru = 'Не удалось подписать документы:'");
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ СтрСоединить(ДокументыКоторыеМожноПодписатьТолькоБезДоверенности, Символы.ПС);
			ВидОшибки = ВидОшибкиЕстьДокументыКоторыеМожноПодписатьТолькоБезДоверенности();
			Ошибка = ОбработкаНеисправностейБЭДКлиент.НоваяОшибка(НСтр("ru = 'Подписание электронных документов'"),
...
Функция НаборОднороденПоЗаполнениюДанныхДоверенностей(ОписаниеНабораДанных, ДокументыКоторыеНельзяПодписатьПоДоверенности) 
	
	НаборОднороденПоЗаполнениюДанныхДоверенностей = Истина;
	ДокументыКоторыеНельзяПодписатьПоДоверенности = Новый Массив;
	Для Каждого ЭлементНабора Из ОписаниеНабораДанных.НаборДанных Цикл
		Если Не ЭлементНабора.ЗаполнениеДанныхДоверенностиДоступно Тогда
			НаборОднороденПоЗаполнениюДанныхДоверенностей = Ложь;
			ДобавитьПредставлениеЭлементаНабора(ДокументыКоторыеНельзяПодписатьПоДоверенности, ЭлементНабора);
		КонецЕсли;
	КонецЦикла;
	
	Возврат НаборОднороденПоЗаполнениюДанныхДоверенностей;
	
КонецФункции

Функция ВидОшибкиЕстьДокументыКоторыеМожноПодписатьТолькоБезДоверенности()
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "ОшибкаПодписанияПоДоверенности";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Не удалось подписать документы'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Среди подписываемых документов есть документы которые можно подписать только сертификатом организации, на которую была зарегистрирована учетная запись'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Добавьте сертификат организации в учетную запись, подпишите документы отдельно'");
	
	Возврат ВидОшибки;
	
КонецФункции

Функция ВидОшибкиЕстьДокументыКоторыеМожноПодписатьПоДоверенности()
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "ОшибкаПодписанияПоДоверенности";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Не удалось подписать документы'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Выбрано несколько документов. Часть из них может быть подписана только сертификатом организации, часть - еще и другим сертификатом по доверенности.
		|Но пока программа не умеет подписывать такие документы вместе.'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Выберите однотипные документы и повторите подписание'");
	
	Возврат ВидОшибки;
	
КонецФункции

Процедура ДобавитьПредставлениеЭлементаНабора(Коллекция, ЭлементНабора)
	
	Если ЭлементНабора.Свойство("Представление") Тогда
		Если ТипЗнч(ЭлементНабора.Представление) = Тип("Структура") Тогда
			Коллекция.Добавить(ЭлементНабора.Представление.Представление);
		Иначе
			Коллекция.Добавить(ЭлементНабора.Представление);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьРезультатПроверкиПодпсейСервера(ОповещениеОЗавершении, РезультатПроверки, КонтекстДиагностики)
