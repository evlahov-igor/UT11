...
Процедура ОбработкаПолученияПредставленияВходящегоДокумента(Данные, Представление, СтандартнаяОбработка, ИмяИлиИдентификаторДокумента) Экспорт
	...
	Если СпособПредставленияДокумента = ПредопределенноеЗначение("Перечисление.СпособыПредставленияВходящихДокументов.Полное") Тогда
		...
		СтандартноеПредставление = СтандартноеПредставлениеДокумента(
									Представление, ИмяИлиИдентификаторДокумента, Данные.Номер, Данные.Дата);
...
Функция ДанныеПоЧастямПути(Объект, ЧастиПутиКДанным, ИдентификаторСтроки)
	...
	Если ТипЗнч(Объект) <> Тип("ФормаКлиентскогоПриложения") // "Объект.Реквизит", "Объект.ТабличнаяЧасть", "Объект.ТабличнаяЧасть.Реквизит"
		И ЧастиПутиКДанным[0] = "Объект" Тогда
		ЧастиПутиКДанным.Удалить(0);
	ИначеЕсли ТипЗнч(Объект) = Тип("ФормаКлиентскогоПриложения") // "Форма.РеквизитФормы", "Форма.ТаблицаФормы", "Форма.ТаблицаФормы.Колонка"
	...
	Если ЧастиПутиКДанным.Количество() > 0 Тогда
		...
		Если СтрНачинаетсяС(Строка(Данные), "ДокументТабличнаяЧасть")
			ИЛИ СтрНачинаетсяС(Строка(Данные), "СправочникТабличнаяЧасть")
			ИЛИ ТипЗнч(Данные) = Тип("ДанныеФормыКоллекция") Тогда // Данные - это таблица
		...
		ИначеЕсли ЧастиПутиКДанным.Количество() = 2 Тогда 
			Если ТипЗнч(Данные) = Тип("ДанныеФормыСтруктура") 
...
Функция СтандартноеПредставлениеДокумента(Представление, ИмяИлиИдентификаторДокумента, Номер, Дата)
