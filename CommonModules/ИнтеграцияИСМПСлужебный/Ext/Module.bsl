	...
	Для Каждого СтрокаМаркированногоТовара Из МаркированныеТовары Цикл
		Если ЗначениеЗаполнено(СтрокаМаркированногоТовара.РодительскаяУпаковка) Тогда
			...
			Иначе
				...
				РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
					СтрокаМаркированногоТовара.Штрихкод, ОбщийЭлементОчереди.ВидПродукции, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Процедура ЗагрузитьРезультатОбработкиАгрегацияОтчетОбИспользованииКМРасчетСтатуса(ЭлементОчереди, ПараметрыОбмена)
	...
	Иначе
		...
		ИначеЕсли Не ЕстьОшибки Тогда
			...
			ИначеЕсли ОперацияДокумента = Перечисления.ВидыОперацийИСМП.ВводВОборотИмпортСФТС
				И ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЭлементОчереди.РеквизитыИсходящегоСообщения.ПараметрыЗапроса, "ТребуетсяВводВОборот", Ложь)  Тогда
				Статус = Перечисления.СтатусыОбработкиМаркировкиТоваровИСМП.КодыМаркировкиАгрегированы;
			Иначе
...
Процедура ДополнитьТабличныеЧастиПоДаннымИСМП(ДокументПриемка, ДанныеШтрихкодов, РезультатЗапроса)
	...
	МаркированныеТовары = РезультатПолученияДанных.ВложенныеШтрихкоды.МаркированныеТовары.Скопировать(,
		"GTIN, Номенклатура, Характеристика, Серия, КоличествоПотребительскихУпаковок");
	МаркированныеТовары.Свернуть(
		"GTIN, Номенклатура, Характеристика, Серия",
		"КоличествоПотребительскихУпаковок");
	МаркированныеТовары.Колонки.КоличествоПотребительскихУпаковок.Имя = "КоличествоУпаковок";
	...
	ДанныеУпаковокСКоличеством = ЗаполнитьКоличествоПотребительскихУпаковок(РезультатПолученияДанных.ВложенныеШтрихкоды.ДеревоУпаковок);
...
Функция ЗаполнитьКоличествоПотребительскихУпаковок(ДеревоУпаковок)
...
Процедура ПодготовитьСообщенияПоАбстрактнойОперацииВводВОборотИмпортСФТС(ЭлементОчереди, ПараметрыОбмена, НаборЗаписей)
	...
	Настройки = Новый Структура(
		"ЗапрашиватьДанныеСервисаИСМП, СоздаватьШтрихкодУпаковки, СсылкаНаОбъект",
		Истина, Ложь, ЭлементОчереди.Документ);
	...
	ТелоЗапроса = Новый Структура;
	ТелоЗапроса.Вставить("trade_participant_inn", РеквизитыОрганизации.ИНН);
	ТелоЗапроса.Вставить("declaration_date",      Формат(ДанныеШапки.ДатаДекларации,"ДФ=yyyy-MM-dd"));
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ДанныеШапки.ВидПродукции) Тогда
	...
	Если ДанныеШапки.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.КодыМаркировки Тогда
		...
		ЗаполнитьСведенияОТоварахИмпортСФТСПоШтрихкодамУпаковок(СообщениеJSON, ЭлементОчереди, ТелоЗапроса,
			РезультатПоиска, Товары, ПараметрыНормализацииПрочее, Настройки);
	...
	Иначе
		Если ДанныеШапки.ВариантЗаполненияДекларации = Перечисления.ВариантыЗаполненияДекларацииИСМП.КодыУпаковок Тогда
			...
			ТребуетсяАгрегация = Ложь;
			ДеревоУпаковок = РезультатПоиска.ДеревоУпаковок;
		Иначе
			...
			ДеревоУпаковок = РезультатПоискаАТК.ДеревоУпаковок;
		КонецЕсли;
		
		ЗаполнитьСведенияОТоварахИмпортФТС(СообщениеJSON, ЭлементОчереди, ТелоЗапроса,
			ДеревоУпаковок, Товары, ПараметрыНормализацииПрочее, Настройки);
...
Процедура ЗаполнитьСведенияОТоварахИмпортФТС(СообщениеJSON, ОбщийЭлементОчереди, ТелоЗапроса, ИсточникДанных, Товары,
	ПараметрыНормализацииПрочее, Настройки = Неопределено)
	...
	Для Каждого СтрокаШтрихкодаУпаковки Из ИсточникДанных.Строки Цикл
		...
		Если СтрокаШтрихкодаУпаковки.Строки.Количество() Тогда
			...
			НастройкиВложенныхУпаковок.ИмяРеквизитаТовары = "children";
			...
			ЗаполнитьСведенияОТоварахИмпортФТС(
				СообщениеJSON, ОбщийЭлементОчереди, СтрокаТЧ, СтрокаШтрихкодаУпаковки, Товары, ПараметрыНормализацииПрочее,
				НастройкиВложенныхУпаковок);
...
Процедура ЗаполнитьСведенияОТоварахИмпортСФТСПоШтрихкодамУпаковок(
	СообщениеJSON, ОбщийЭлементОчереди, ТелоЗапроса, ДанныеПоШтрихкодам, Товары, ПараметрыНормализацииПрочее, Настройки = Неопределено)
...
Функция ЗаполнитьДанныеСтрокиВводВОборотИмпортФТС(СообщениеJSON, ОбщийЭлементОчереди, СтрокаИсточникаДанных, Товары,
	Настройки, ПараметрыНормализацииПрочее, ПользовательскиеПараметрыРазбораКодаМаркировки)
	...
	РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
		СтрокаИсточникаДанных.Штрихкод, ОбщийЭлементОчереди.ВидПродукции, ПользовательскиеПараметрыРазбораКодаМаркировки);
