	...
	Иначе
		Если Форма.ЭтоКомпания = 0 Тогда
			Если Форма.УказыватьЮридическиеРеквизиты И (Форма.ВидКомпании = 0 Или Форма.ВидКомпании = 3) Тогда
				Если ЗначениеЗаполнено(Форма.НаименованиеКонтрагента) Или ЗначениеЗаполнено(Форма.ИНН) Или ЗначениеЗаполнено(Форма.КПП) Или ЗначениеЗаполнено(Форма.КодПоОКПО) Тогда
					НеобходимостьВопроса = Истина;
				КонецЕсли;
			ИначеЕсли Форма.УказыватьЮридическиеРеквизиты И Форма.ВидКомпании = 2 Тогда
				Если ЗначениеЗаполнено(Форма.ИНН) Или ЗначениеЗаполнено(Форма.КодПоОКПО) Тогда
					НеобходимостьВопроса = Истина;
				КонецЕсли;
			КонецЕсли;
		Иначе
...
//
Процедура ПроверитьИНН(Форма) Экспорт
	//++ Локализация
	...
	Если РеквизитыКонтрагента.ЭтоФормаПомощника Тогда
		ИННПриИзменении_ПомощникНового(Форма);
	Иначе
		ИННПриИзменении(Форма, РеквизитыКонтрагента);
	КонецЕсли;
	//-- Локализация
КонецПроцедуры
...
Процедура ЗаполнитьРеквизитыКонтрагентаПоПолученнымДанным(РеквизитыКонтрагента, Форма)
	...
	Если РеквизитыФормыКонтрагента.ЭтоФормаПомощника Тогда
		...
		Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаГоловнойКонтрагент Тогда
			...
			Форма.НаименованиеГоловногоКонтрагента      = РеквизитыКонтрагента.Наименование;
...
Функция ДанныеОбъектаДляСравнения(Форма)
	...
	Если РеквизитыКонтрагента.ЭтоФормаПомощника Тогда
		...
		Иначе
			ДанныеДляСравнения.Наименование            = Форма.НаименованиеГоловногоКонтрагента;
...
//++ Локализация
Процедура ИННПриИзменении(Форма, РеквизитыКонтрагента)
...
Процедура ИННПриИзменении_ПомощникНового(Форма)
	...
	Если ИННЗаполненКорректно И Форма.НастройкиПодключенияКСервисуИППЗаданы И Не Форма.ВидКомпании = 3 Тогда
