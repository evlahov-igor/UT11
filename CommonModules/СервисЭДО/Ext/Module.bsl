...
Функция Авторизоваться(КлючСинхронизации, КонтекстДиагностики,
	...
	Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
		...
		Если ЗначениеЗаполнено(КлючСинхронизации.СпособОбмена) Тогда
		...
		Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
			...
			ПредставлениеОперации = НСтр("ru = 'Аутентификации в сервисе 1С-ЭДО'");
			Если КлючСинхронизации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском Тогда
				ПредставлениеОперации = НСтр("ru = 'Аутентификации в сервисе 1С-Такском'");
			КонецЕсли;
			...
			Результат = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики, ПовторноеПолучение);
		...
		Если РезультатОперации.Успех Тогда
			...
			Для каждого Абонент Из СтруктураОтвета.abonents Цикл
				...
				Если Абонент.Свойство("error") Тогда
					...
					ВидОшибки = ВидОшибкиПоТексту(ПодробныйТекстОшибки);
...
// 	КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 	ПовторнаяПопытка  - Булево - необходимо установить в Истину, если запрос выполняется повторно в рамках одной
// 	                    операции синхронизации.
// 	
// Возвращаемое значение:
// 	ТаблицаЗначений - см. ПриглашенияЭДО.НоваяТаблицаПриглашений
//
Функция ПолучитьПриглашения(КлючСинхронизации, ДатаПоследнегоЗапроса, КонтекстДиагностики, ПовторнаяПопытка = Ложь) Экспорт
	...
	РезультатОперации = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики, ПовторнаяПопытка);
...
// 	КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 	ИстекСрокДействияМаркера - Булево - выходной параметр
// Возвращаемое значение:
// 	Структура:
//   * Успех - Булево
//   * Ошибка - см. СинхронизацияЭДО.НоваяОшибкаПередачи
Функция ОтправитьКонтейнер(КлючСинхронизации, ОписаниеКонтейнера, КонтекстДиагностики, ИстекСрокДействияМаркера = Неопределено) Экспорт
	...
	РезультатВыполненияОперации = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики, Ложь);
	
	Успех = РезультатВыполненияОперации.Успех;
	Если Не РезультатВыполненияОперации.Успех Тогда
		ОшибкаОператора = РезультатВыполненияОперации.ОшибкаОператора;
		Если ОшибкаОператора <> Неопределено И ОшибкаОператора.ИстекСрокДействияМаркера Тогда
			ИстекСрокДействияМаркера = Истина;
			Успех = Ложь;
		КонецЕсли;
	КонецЕсли;
...
// 	ДатаПоследнегоЗапроса - Дата - дата последнего запроса к сервису
// 	КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 	ПовторноеПолучение - Булево - необходимо установить в Истину, если запрос выполняется повторно в рамках одной
// 	                    операции синхронизации.
// Возвращаемое значение:
// 	см. НовыйРезультатПолученияСпискаДокументов
//
Функция ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, КонтекстДиагностики,
	...
	РезультатОперации = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики, ПовторноеПолучение);
...
Функция ПолучитьДокумент(КлючСинхронизации, Документ, КонтекстДиагностики, ПовторноеПолучение = Ложь) Экспорт
...
// Отправляет запрос оператору электронного документооборота об спешном получении сообщения.
// 
// Параметры:
// 	КлючСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыйКлючСинхронизации
...
Функция ПодтвердитьПолучениеСообщения(КлючСинхронизации, ИдентификаторСообщения, КонтекстДиагностики) Экспорт
...
Функция ВыполнитьОперацию(ПараметрыОперации, КонтекстДиагностики,
	...
	Если Не ЭтоОперацияPing(ПараметрыОперации)
		...
		И Не ЕстьДоступКВебСервису1CЭДО(ПараметрыОперации, КонтекстДиагностики) Тогда
	...
	РезультатВыполненияЗапроса = ИнтернетСоединениеБЭД.ВызватьHTTPМетод(ОписаниеСоединения, HTTPЗапрос,
		ПараметрыОперации.Метод, ПараметрыОперации.ПредставлениеОперации, КонтекстДиагностики);
	...
	Если Не РезультатВыполненияОперации.Успех Тогда
		...
		Если ЭтоКодОшибкиПовторнойОтправки(ОшибкаОператора.Код)
			Или ОшибкаОператора.ИстекСрокДействияМаркера Тогда
			СтандартнаяОбработкаОшибки = Ложь;
			Если ОшибкаОператора.ИстекСрокДействияМаркера Тогда
...
Процедура ОбработатьОшибкуСервиса(ПараметрыОперации, КонтекстДиагностики, ОшибкаОператора,
	...
	ВидОшибки = ВидОшибкиПоТексту(ОшибкаОператора.ДополнительныеДанные);
	
	Если ВидОшибки.Идентификатор <> "НеизвестнаяОшибка" Тогда 
		ПодробныйТекст = ОшибкаОператора.Текст + Символы.ПС + ОшибкаОператора.ДополнительныеДанные;
	Иначе 
		ВидОшибки = ВидОшибкиПоТексту(ОшибкаОператора.Детали);
		Если ВидОшибки.Идентификатор <> "НеизвестнаяОшибка" Тогда
			ПодробныйТекст = ОшибкаОператора.Текст + Символы.ПС + ОшибкаОператора.Детали;
		Иначе 
			ПодробныйТекст = ОшибкаОператора.Текст;
		КонецЕсли;
	КонецЕсли;
...
Функция ВидОшибкиПоТексту(ТекстОшибки) 
	
	Если СтрНайти(ВРег(ТекстОшибки), "АСТРАЛ-М") Тогда
		ВидОшибки = ВидОшибкиСертификатНедействителенУЦНеПрошелАккредитацию(ТекстОшибки);
	Иначе 
...
Функция ВидОшибкиСертификатНедействителенУЦНеПрошелАккредитацию(ТекстОшибки)

	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = "СертификатНедействителенУЦНеПрошелАккредитацию";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Сертификат недействителен'");
	ВидОшибки.ОписаниеРешения = ТекстОшибки;
...
//
Функция ЕстьДоступКВебСервису1CЭДО(ПараметрыОперации, КонтекстДиагностики)
	
	ПовторноеПолучение = Ложь;
	
	ПараметрыВыполненияОперации = НовыеПараметрыВыполненияОперацииСервиса();
	ПараметрыВыполненияОперации.АдресРесурса = АдресРесурсаPing();
	ПараметрыВыполненияОперации.ТребуетсяМаркер = Ложь;
	ПараметрыВыполненияОперации.ПредставлениеОперации = НСтр("ru = 'Проверка доступа к сервису 1С-ЭДО'");
	ПараметрыВыполненияОперации.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().GET;
	
	ПараметрыВыполненияОперации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;
	ПараметрыВыполненияОперации.Таймаут = 7;
	ПараметрыВыполненияОперации.ИдентификаторУчетнойЗаписи = ПараметрыОперации.ИдентификаторУчетнойЗаписи;
	
	РезультатОперации = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики, ПовторноеПолучение);
	
	Возврат РезультатОперации.Успех;
