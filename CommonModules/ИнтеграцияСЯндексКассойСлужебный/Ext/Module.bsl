...
Функция АдресКастомизацииСтраницыОплаты(Знач ИдентификаторМагазина) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Результат.URLСтраницы = СтрШаблон("%1/shops/%2?freshSubscriberCode=%3", 
	...
	Иначе
		...
		Если ДанныеАутентификации = Неопределено Тогда
			Результат.Ошибка = "НеЗаполненыДанныеАутентификации";
		Иначе
			Результат.URLСтраницы = СтрШаблон("%1/shops/%2?boxModeLogin=%3", 
				ХостПредоставленияДоступа(),
				Формат(ИдентификаторМагазина, "ЧГ=0"),
				КодироватьСтроку(ДанныеАутентификации.Логин, СпособКодированияСтроки.КодировкаURL));
...
Процедура ОбновитьНастройкуЯндексКассы(НастройкаСсылка, Параметры, Отказ)
	...
	Попытка
		...
		ОбщегоНазначенияБЭД.УстановитьУправляемуюБлокировкуПоСсылке(НастройкаСсылка);
...
Функция НастройкиСервиса()
	...
	ПараметрыСоединения.Прокси = ИнтернетСоединениеБЭД.СформироватьПрокси(ПараметрыСоединения.Протокол);
...
Функция ПараметрыСервиса(Знач ИмяКоманды, Отказ = Ложь) 
	...
	Если Не ОбщегоНазначенияБЭДПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьИнтеграциюСЯндексКассой") Тогда
...
Процедура ОбработатьПараметрыСообщенияОбОшибке(Знач ИдентификаторОшибки, Знач ОбязательныеПараметры, Параметры) 
	...
	Если ЗначениеЗаполнено(МассивОтсутствующихПараметров) Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ИнтеграцияСЯндексКассой);
