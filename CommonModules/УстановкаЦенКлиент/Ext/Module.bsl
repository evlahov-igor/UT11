...
Функция ПолучитьИнформационнуюСтрокуДляЯчейки(Элементы,
	...
	Если ПозицияПодстроки <> 0 Тогда
		...
		Если ЗначениеЗаполнено(ТекущееМаркетинговоеМероприятие) Тогда
			ФорматированнаяСтрокаМаркетинговоеМероприятие = Новый ФорматированнаяСтрока(НСтр("ru = 'по акции'"),Новый Шрифт(,,Истина),WebЦвета.Коралловый,,ПолучитьНавигационнуюСсылку(ТекущееМаркетинговоеМероприятие));
			ФорматированнаяСтрокаМаркетинговоеМероприятие = Новый ФорматированнаяСтрока(ФорматированнаяСтрокаОтступ, ФорматированнаяСтрокаМаркетинговоеМероприятие);
		Иначе
...
Функция ПараметрыИзмененияВЭлементеДереваЦен(ВариантВыполнения) Экспорт
	...
	Если ВариантВыполнения = "ЗаполнитьУпаковкиНоменклатуры" Тогда
		ПараметрыВызова.Вставить("МассивСтруктурУпаковокНоменклатуры");
	ИначеЕсли ВариантВыполнения = "ИзменитьУпаковкуВСтрокеДереваЦен" Тогда
		ПараметрыВызова.Вставить("ТекущееЗначение");
		ПараметрыВызова.Вставить("ИмяТекущейКолонки");
	ИначеЕсли ВариантВыполнения = "ИзменитьДанныеВСтрокеДереваЦен" Тогда
		ПараметрыВызова.Вставить("СоответствиеКоэффициентовУпаковокНоменклатуры");
		ПараметрыВызова.Вставить("ТекущееЗначениеУпаковки");
		ПараметрыВызова.Вставить("ТекущееЗначение");
		ПараметрыВызова.Вставить("КоэффициентУпаковкиЦеныИсточника");
		ПараметрыВызова.Вставить("ИмяТекущейКолонки");
		ПараметрыВызова.Вставить("НоваяФормула");
		ПараметрыВызова.Вставить("ПроцентИзменения");
		ПараметрыВызова.Вставить("СуммаИзменения");
		ПараметрыВызова.Вставить("ИзмененПроцент");
		ПараметрыВызова.Вставить("ИзмененаФормула");
		ПараметрыВызова.Вставить("ИзмененаСумма");
		ПараметрыВызова.Вставить("Номенклатура");
		ПараметрыВызова.Вставить("ИспользоватьУпаковкиНоменклатуры");
	КонецЕсли;	
...
Процедура ПрименитьИзмененияВДеревеЦен(ЭлементДереваЦен, ПараметрыВызова) Экспорт
	...
	Для Каждого ЭлементДереваЦенТекущий Из ЭлементыДереваЦенТекущие Цикл
		...
		ИначеЕсли ПараметрыВызова.ВариантВыполнения = "ИзменитьУпаковкуВСтрокеДереваЦен" Тогда
	...
	Если ПараметрыВызова.ИспользоватьУпаковкиНоменклатуры Тогда
		...
		Если ПараметрыВызова.ТекущееЗначениеУпаковки = ЗначениеКолонкиУпаковка
			...
			ИначеЕсли ПараметрыВызова.ИзмененаСумма Тогда
				...
				ТекущийЭлементДереваЦен["СуммаИзменения"+ИмяТекущейКолонки] = ПараметрыВызова.СуммаИзменения;
				ТекущийЭлементДереваЦен[ИмяТекущейКолонки] = ТекущийЭлементДереваЦен["СтараяЦена" + ИмяТекущейКолонки] +ТекущийЭлементДереваЦен["СуммаИзменения"+ИмяТекущейКолонки];
			...
			ТекущийЭлементДереваЦен["ИзмененаВручную"       + ИмяТекущейКолонки] = Истина;
