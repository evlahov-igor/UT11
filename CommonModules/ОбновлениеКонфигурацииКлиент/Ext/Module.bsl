...
Функция ПоддерживаетсяУстановкаОбновлений() Экспорт
	...
	////Нужно так, но это клиент
	//Если РолиДоступны("ПолныеПрава,КР_БазовыеПраваZolla") Тогда 
	//	Результат.ОписаниеОшибки = НСтр("ru = 'Для установки обновления требуются полные права или базовые права Zolla.'");
	//	Возврат Результат;
	//КонецЕсли;
	// >> 11.08.2024 Петухов А.В., Фактор, #4342
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	ВставитьПараметрСкрипта("ВыполнятьСжатиеТаблицИБ"           , Ложь                            , Ложь  , ОбластьПараметров);
	//}	
...
Процедура ЗапуститьСкриптОбновления(Параметры, ПараметрыАдминистрирования)
	...
	Если ТекущееСостояние <> Неопределено Тогда
		злОбновлениеКонфигурацииВызовСервера.ИзменитьСостояниеОбновления(ТекущееСостояние.Версия, 
			ПредопределенноеЗначение("Перечисление.злСтатусыОбновленияКонфигурации.ВыполняетсяОбновление"),
			"ЗапуститьСкриптОбновления",
			ИмяГлавногоФайлаСкрипта
		);
	КонецЕсли;
	// >> 24.07.2024 Петухов А.В., Фактор, #4316
