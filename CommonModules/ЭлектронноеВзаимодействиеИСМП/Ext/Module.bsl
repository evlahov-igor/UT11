...
Функция ДанныеДокументаСоответствуютДаннымУпаковок(ПроверяемыйДокумент) Экспорт
	...
	ИнтеграцияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(Запрос.Текст, "ОписаниеИС", "ДанныеДокумента.Номенклатура");
...
Функция ЧастичноеСодержимое(ТаблицаУпаковок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Результат Цикл
		...
		РезультатРазбора = ШтрихкодированиеИС.НоваяСтруктураОбработкиШтрихкода(
			СтрокаТаблицы.ЗначениеШтрихкода, СтрокаТаблицы.ВидПродукции, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
//   * ЧастичноеВыбытиеВариантУчета - ПеречислениеСсылка.ВариантыУчетаЧастичногоВыбытияИСМП - Вариант частичного выбытия.
Функция Содержимое(Знач ШтрихкодыУпаковок, Знач ВидыПродукцииИС = Неопределено) Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВариантыУчетаЧастичногоВыбытияИСМП.ПустаяСсылка) КАК ЧастичноеВыбытиеВариантУчета
	|ИЗ
	|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковок
	...
	Результат.Свернуть("Номенклатура, Характеристика, Серия", "КоличествоНеУказано, КоличествоПотребительскихУпаковок, ЧастичноеВыбытиеВариантУчета, Количество");
...
Процедура ЗаполнитьСведенияОМаркировке(Приемник, Источник, ДанныеШтрихкодовУпаковок, ОбъектыСОграничениемДлины) Экспорт
	...
	Для Каждого СтрокаУпаковки Из СтрокиУпаковок Цикл
		...
		Если Не СтрокаУпаковки.ТребуетВзвешивания Тогда
...
Процедура ОбработкаСтрокиЧастичногоВыбытия(СтрокаТаблицы, КэшВходящихСтрок)
	...
	СтрокаКэша = КэшВходящихСтрок.Получить(СтрокаТаблицы.ШтрихкодУпаковки);
	...
	Если СтрокаКэша <> Неопределено
		...
		Если СтрокаКэша.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура Тогда
