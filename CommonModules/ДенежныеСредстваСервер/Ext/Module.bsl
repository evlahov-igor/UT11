...
Процедура УстановитьПараметрыВыбораКассы(Объект, ЭлементКасса) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		// Задача #3091
		Если  Объект.Свойство("Подразделение")
			И ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
			НуженОтборПоПодразделению = Истина;
		Иначе
			НуженОтборПоПодразделению = Ложь;	
		КонецЕсли;
		...
		// --> Евлахов Игорь Николаевич (Начало) 27.09.2023
		// Задача #3091
		Если НуженОтборПоПодразделению Тогда
...
// Процедура устанавливает параметры выбора для кассы ККМ.
//
// Параметры:
//	Объект - ДанныеФормыСтруктура - Текущий объект
//	ЭлементКассаККМ - ПолеФормы - Поле для ввода кассы.
//
Процедура злУстановитьПараметрыВыбораКассыККМ(Объект, ЭлементКассаККМ) Экспорт
	...
	ЭлементКассаККМ.СвязиПараметровВыбора = Новый ФиксированныйМассив(МассивПараметров);
...
Функция ПолучитьСписокХозяйственныхОперацийПродажДляДокумента(ТипДокумента) Экспорт
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
		...
		// Задача #3463
		МассивХозяйственныхОпераций.Добавить(Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзКассыККМ);
