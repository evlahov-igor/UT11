		...
		Если СокрЛП(ОписаниеОбъекта.ИмяОбъекта) = "ВариантыОтчетов" Тогда
			...
			Если Не НастройкиПользователя = Неопределено Тогда
				...
				ОбъектОбменаСтрокаТаблицы.ПредставлениеНастройки =
					НСтр("ru = 'Текущие пользовательские настройки'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ДосьеПартнера(Партнер, ВыгружатьКонтактныеТелефоны = Истина)
	...
	Запрос.УстановитьПараметр("Ответственный", НСтр("ru = 'Ответственный менеджер'", КодОсновногоЯзыка));
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	ЗаголовокТелефон = НСтр("ru = 'тел.:'", КодОсновногоЯзыка)+" ";
	...
	Пока Выборка.Следующий() Цикл
		...
		ПредставлениеКонтакта = СтрЗаменить(ПредставлениеКонтакта,
			НСтр("ru = 'Телефон'", КодОсновногоЯзыка), ЗаголовокТелефон);
		ПредставлениеКонтакта = СтрЗаменить(ПредставлениеКонтакта,
			НСтр("ru = 'телефон'", КодОсновногоЯзыка), ЗаголовокТелефон);
...
Процедура ЗаписатьКонтактнуюИнформацию(Фабрика, ИнформацияПоОбъекту, Знач СсылкаНаОбъект, Знач ИмяСправочника)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СправочникИмяСправочникаКонтактнаяИнформация", "Справочник." + ИмяСправочника + ".КонтактнаяИнформация");
