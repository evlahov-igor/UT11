...
Функция СериализоватьОбъект(Источник) Экспорт
	...
	ТекОбъектЗаписи = ПолучитьОбъектПотоковойЗаписи();
...
Функция ДесериализоватьОбъект(Источник) Экспорт
	...
	потокЧтение = ПолучитьОбъектПотоковогоЧтения(Источник);
...
Функция КонвертироватьСтруктуруВJSONОбъект(Структура) Экспорт
...
Функция СериализоватьXDTOВJSON(ОбъектXDTO, Фабрика = Неопределено, ПараметрыЗаписи = Неопределено) Экспорт
	
	xdtoФабрика = ?(Фабрика = Неопределено, ФабрикаXDTO, Фабрика);	
	
	НовыйЗаписьJSON = ПолучитьОбъектПотоковойЗаписи(ПараметрыЗаписи);		
	
	xdtoФабрика.ЗаписатьJSON(НовыйЗаписьJSON, ОбъектXDTO, НазначениеТипаXML.Явное); 
	
	Возврат НовыйЗаписьJSON.Закрыть();
	
КонецФункции

Функция ДесериализоватьXDTOИзJSON(СтрокаXML, Фабрика = Неопределено) Экспорт
	
	xdtoФабрика = ?(Фабрика = Неопределено, ФабрикаXDTO, Фабрика);
	
	НовыйЧтениеXML = ПолучитьОбъектПотоковогоЧтения(СтрокаXML);	
	
	ОбъектXDTO = xdtoФабрика.ПрочитатьJSON(НовыйЧтениеXML);
