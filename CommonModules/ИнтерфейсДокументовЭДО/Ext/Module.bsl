...
Процедура ПриОпределенииСпискаКомандЭДО(Знач СоставКоманд, Знач ПолноеИмя, Знач НаправлениеЭД, КомандыЭДО) Экспорт
	...
	Если СоставКоманд.Организации.Найти(ПолноеИмя) <> Неопределено
		...
		КомандаЭДО.МестоРазмещения = "КомандыЭДОВажное";
		КомандаЭДО.УправлениеВидимостью.Вставить("Использовать", Истина);
		КомандаЭДО.УправлениеВидимостью.Вставить("ИмяАлгоритмаПроверкиУсловия", "МашиночитаемыеДоверенности.ИспользуютсяМЧД");
		КомандаЭДО.УправлениеВидимостью.Вставить("ЗначениеУсловия", Истина);
		КомандаЭДО.Недоступна    = Не ЕстьПравоЧтенияНастройкиОбмена;
	...
	Для Каждого СтрокаПодписи Из Подписи Цикл 
		...
		Если СтрокаПодписи.ЭтоПодписьПоДоверенности Тогда 
			Если ДоверенностьОпределена Тогда 
				...
				ДоверенностьДействует = ДоверенностьДействительна(СтрокаПодписи); 
			...
			Иначе
				...
				СтрокаПодписи.ПодписьВернаПредставление = НСтр("ru = 'Доверенность недействительна'")
				+ Символы.ПС;
			КонецЕсли; 
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаПодписи.ДатаПроверкиПодписи) Тогда
			СтрокаПодписи.ПодписьВернаПредставление = 
			?(СтрокаПодписи.ПодписьВерна, НСтр("ru = 'Подпись верна'"), НСтр("ru = 'Подпись неверна'")) 
			+ Символы.ПС
			+ СтрокаПодписи.ПодписьВернаПредставление;
			
			СтрокаПодписи.ПодписьВернаПредставление = СтрокаПодписи.ПодписьВернаПредставление 
			+ Формат(СтрокаПодписи.ДатаПроверкиПодписи, "ДФ='dd.MM.yyyy HH:mm'");
		Иначе
...
Функция СтатусыЭлектронныхДокументовОбъектаУчета(ОписанияЭлектронныхДокументовОбъектаУчета, УчитыватьВсеАктуальные = Ложь) Экспорт
	...
	Для каждого ОписаниеЭлектронногоДокументаОбъектаУчета Из ОписанияЭлектронныхДокументовОбъектаУчета Цикл
	...
	Пока ВыборкаНастроек.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаНастроек.Контрагент) Тогда
			ОтборАктуальныхДокументовОбъекта = Новый Структура("ОбъектУчета, Организация, Контрагент, ДоговорКонтрагента");
		Иначе
...
Функция НастройкиПодбораУчетногоДокумента(ЭлектронныйДокумент, СпособОбработки = "") Экспорт
	...
	ИнфрмацияОтправителя = ДанныеЭлементовСхемыРегламента.Найти(
		Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя, "ТипЭлементаРегламента");
		
	Если ИнфрмацияОтправителя <> Неопределено Тогда
	
		ПрисоединенныйФайл = ИнфрмацияОтправителя.ПрисоединенныйФайл;
		...
		Если СодержаниеСообщения <> Неопределено Тогда
			...
			Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП("Контрагенты", Отправитель.ИНН, Отправитель.КПП);
		КонецЕсли;
...
// 	* ПредварительныйПросмотр - Неопределено, Структура - параметры предварительного просмотра, если электронный документ не сформирован.
// 	** РезультатПроверкиГотовности - см. ПроверитьГотовностьОбъектаУчетаКДокументообороту
// 	** ПараметрыПросмотраСтатусовЭлектронныхДокументов - см. ПараметрыПросмотраСтатусовЭлектронныхДокументов
...
// 	* ПросмотрСтатусовЭлектронныхДокументов - см. ПараметрыПредварительногоПросмотраЭлектронногоДокумента
//
...
//
Функция ПараметрыПросмотраЭлектронногоДокументаПоОбъектуУчета(ОбъектУчета, ДополнительныеДанные = Неопределено) Экспорт
	
	ПараметрыПросмотра = НовыеПараметрыПросмотраЭлектронногоДокумента();
	ПараметрыПросмотра.ОбъектУчета = ОбъектУчета;
	
	ОписанияЭлектронныхДокументовОбъектаУчета = ИнтеграцияЭДО.ОписаниеОбъектаУчета(ОбъектУчета);
	
	СтатусыЭлектронныхДокументов = СтатусыЭлектронныхДокументовОбъектаУчета(
		ОписанияЭлектронныхДокументовОбъектаУчета, Истина);
	
	ЗаполнитьПараметрыПросмотраПоСтатусамЭлектронныхДокументов(ПараметрыПросмотра,
		СтатусыЭлектронныхДокументов, ОписанияЭлектронныхДокументовОбъектаУчета, ДополнительныеДанные);
...
// 	см. НовыеПараметрыПросмотраЭлектронногоДокумента
//
Функция ПараметрыПросмотраЭлектронногоДокументаПоОписанию(ОписаниеЭлектронногоДокументаОбъектаУчета, ДополнительныеДанные = Неопределено) Экспорт
...
Процедура ЗаполнитьПараметрыПросмотраПоСтатусамЭлектронныхДокументов(ПараметрыПросмотра, СтатусыЭлектронныхДокументов, ОписанияЭлектронныхДокументовОбъектаУчета, ДополнительныеДанные = Неопределено)
	...
	ИначеЕсли ЗначениеЗаполнено(СтатусыЭлектронныхДокументов[0].ЭлектронныйДокумент) Тогда
	...
	Иначе
		...
		ОписаниеЭлектронногоДокумента = НайтиОписаниеПоСтатусуЭлектронногоДокумента(
			ОписанияЭлектронныхДокументовОбъектаУчета, СтатусыЭлектронныхДокументов[0]);
		...
		Если ЗначениеЗаполнено(ОписаниеЭлектронногоДокумента) Тогда
			ПараметрыПросмотра.ПредварительныйПросмотр = ПараметрыПредварительногоПросмотраЭлектронногоДокумента(
				ОписаниеЭлектронногоДокумента, ДополнительныеДанные);
		КонецЕсли;
...
Функция НайтиОписаниеПоСтатусуЭлектронногоДокумента(ОписанияЭлектронныхДокументовОбъектаУчета, СтатусЭлектроннгоДокумента)
	
	Результат = Неопределено;
	
	Если СтатусЭлектроннгоДокумента.ОжидаемоеДействие <> Перечисления.ДействияПоЭДО.Сформировать Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(СтатусЭлектроннгоДокумента.ВидДокумента) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Отбор = Новый Структура("ОбъектУчета, Организация, Контрагент, Договор");
	ЗаполнитьЗначенияСвойств(Отбор, СтатусЭлектроннгоДокумента);
	Отбор.Договор = СтатусЭлектроннгоДокумента.ДоговорКонтрагента;
	
	ЗначенияРеквизитовВидаДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		СтатусЭлектроннгоДокумента.ВидДокумента, "ТипДокумента, ПрикладнойТипДокумента");
...
Функция ПараметрыПредварительногоПросмотраЭлектронногоДокумента(ОписаниеЭлектронногоДокументаОбъектаУчета, ДополнительныеДанные = Неопределено)
	...
	ПредварительныйПросмотр.РезультатПоискаНастроек = НастройкиФормированияЭлектронногоДокументаОбъектаУчета(
		ОписаниеЭлектронногоДокументаОбъектаУчета, ДополнительныеДанные);
...
Процедура СкорректироватьДатыПодписиНаЧасовойПоясКлиента(Подпись)
...
Функция ДоверенностьДействительна(Подпись)   
	
	ДоверенностьДействует = Подпись.ПодписьВерна;
	Если ЗначениеЗаполнено(Подпись.РезультатПроверкиПоМЧД) Тогда 
		РезультатПроверкиПоМЧД = Подпись.РезультатПроверкиПоМЧД;  
		ДоверенностьДействует = РезультатПроверкиПоМЧД.ПодписьВерна;
		Если ЗначениеЗаполнено(РезультатПроверкиПоМЧД.ПротоколПроверки) Тогда 
			...
			Если ЗначениеЗаполнено(ПротоколПроверки.ПроверкаОператором) Тогда
				ДоверенностьДействует = 
				ПротоколПроверки.ПроверкаДоверенности["ДоверенностьДействительна"]
				И ПротоколПроверки.ПроверкаОператором["ДоверенностьДействительна"];	
			Иначе  
				ДоверенностьДействует = 
				ПротоколПроверки.ПроверкаДоверенности["ДоверенностьДействительна"];
			КонецЕсли;
