...
//  Булево - Истина, если требуется обработка, ложь - в противном случае.
//
Функция НужноОбновитьДоступностьПослеЗаписи(Форма, ИмяОбработчика) Экспорт
	
	СлужебныеПараметры = Форма.ДоступныеОстаткиПараметрыВстраивания.СлужебныеПараметры;
	СлужебныеПараметры.ИнтервалЗапускаОбработчика = СлужебныеПараметры.ИнтервалЗапускаОбработчика * 1.2;
	...
	Если Не НужноОбновить Тогда
		Если СлужебныеПараметры.ИнтервалЗапускаОбработчика > 15 Тогда
			ПоказатьПредупреждение(
				,
				НСтр("ru = 'Превышено время ожидания распределения запасов по заказу. Доступный остаток не актуализирован'"),
				,
...
//
// Параметры:
//  Ответ - КодВозвратаДиалога - ответ на необходимость снимать флаг отгружать одной датой в форме заказа.
//
// Возвращаемое значение:
//  Структура - структура с полями:
...
//  ИмяПроцедурыОповещения - Строка - процедура оповещения в форме заказа в которую будет передано управление после ответа.
//
Процедура ПоказатьВопросСнятьФлагОтгружатьОднойДатойПриУказанииВариантаОбеспечения(Форма, ВариантОбеспеченияВыбранноеЗначение, ИмяПроцедурыОповещения) Экспорт
	...
	Если ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить") Тогда
...
//
Процедура ПоказатьВопросСнятьФлагОтгружатьОднойДатойДляСпискаЗаказов(Заказы, КодДействия, ПараметрыДат, Форма) Экспорт
...
//
// Возвращаемое значение:
//  Структура - структура выбора варианта обеспечения.
//
Функция ВариантОбеспеченияПриИзменении(Форма, ТекущиеДанные) Экспорт
	
	СлужебныеПараметры = Форма.ДоступныеОстаткиПараметрыВстраивания.СлужебныеПараметры;
	Если СлужебныеПараметры.ВариантОбеспеченияВыбранноеЗначение = Неопределено Тогда
		Значение = Новый Структура();
		Значение.Вставить("ВариантОбеспечения", ТекущиеДанные.ВариантОбеспечения);
	...
	Иначе
		Если СлужебныеПараметры.ВариантОбеспеченияВыбранноеЗначение.ОткрытьФормуВыбораСкладаИСерий Тогда
			ТекущиеДанные.ВариантОбеспечения = СлужебныеПараметры.ВариантОбеспеченияДоИзменения;
		КонецЕсли;
	КонецЕсли;
	Возврат СлужебныеПараметры.ВариантОбеспеченияВыбранноеЗначение;
...
Процедура ПоказатьВопросОбОтгрузкеОднойДатой(ОписаниеОповещения, ДатаОтгрузкиВсехСтрокОднойДатой, ДатаОтгрузкиНовыхСтрок, ОднаДата, ЗаголовокВопроса, ШаблонТекстаВопроса = Неопределено, ШаблонКнопкиДа = Неопределено)
	...
	КонецЕсли;
	СписокКнопок.Добавить(КодВозвратаДиалога.Да, ШаблонКнопкиДа);
...
Процедура ВыполнитьДействиеВСпискеЗаказовРазныхТиповПослеВопроса(Ответ, СтруктураПараметров) Экспорт
	...
	РезультатВыполненияДействий.Вставить("СнятьФлагОтгружатьОднойДатой",    Ответ = КодВозвратаДиалога.Да);
	РезультатВыполненияДействий.Вставить("ЗаполнитьДатуОтгрузкиОднойДатой", Ответ = КодВозвратаДиалога.Нет);
