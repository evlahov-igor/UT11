...
//		* ПоДатеОтгрузки - Булево - Признак того, что ставки необходимо получать на дату отгрузки
//
Функция ПараметрыЗаполненияСтавкиНДС(Объект = Неопределено, ПоДатеОтгрузки = Ложь) Экспорт
...
Функция ПараметрыЗаполненияСтраныПроисхождения() Экспорт
	...
	ПараметрыЗаполненияСтраныПроисхождения.Вставить("Номенклатура");
	ПараметрыЗаполненияСтраныПроисхождения.Вставить("НомерГТД");
...
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//
Процедура ЗаполнитьВидЦеныВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий) Экспорт
...
Процедура ПересчитатьСуммуСуффиксВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуСуффикс", Суффикс) Тогда
		...
		Если ТекущаяСтрока <> Неопределено Тогда
			ТекущаяСтрока[СуммаССуффиксом(Суффикс).Сумма] = ТекущаяСтрока.Цена * ТекущаяСтрока[КоличествоССуффиксом(Суффикс).КоличествоУпаковок];
		КонецЕсли;
	КонецЕсли;
...
Функция СуммаССуффиксом(Суффикс) Экспорт
	...
	СтруктураВозврата = Новый Структура("Сумма, СуммаНДС, СуммаСНДС");
...
Функция КоличествоССуффиксом(Суффикс) Экспорт
	...
	СтруктураВозврата = Новый Структура("Количество, КоличествоУпаковок");
