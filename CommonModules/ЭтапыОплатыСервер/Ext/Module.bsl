...
Процедура ЗаполнитьПроцентыПоСуммам(ЭтапыГрафикаОплаты) Экспорт
	...
	Сч = 0;
	// << 13.06.2024 Вдовенко С.А., КРОК, A2105505-2658
	// проблема алгоритма в том, что на больших числах теряются остатки от округления, н-р: от 0,524 останет 0,52, а 0,004 потеряется
	// и если строк много - эти потери становятся значимыми и мы получаем строки с нулевыми процентами
	// для исправления ошибки данный остаток будем сохранять и прибавлять к следующему числу
	// н-р: 
	// 0,524 -> 0,52 (0,004 запомнили)
	// 0,524 + 0,04 = 0,528 -> 0,53 (-0,002 запомнили)
	// и т.д.
	ПотериНаОкруглении = 0;
	// >> 13.06.2024 Вдовенко С.А., КРОК, A2105505-2658
	Пока Сч < ЭтапыГрафикаОплаты.Количество() Цикл
		...
		Если СуммаПлатежей <> 0 Тогда
			...
			ТекущийПроцент = ЭтапыГрафикаОплаты[Сч].СуммаПлатежа / СуммаПлатежей * 100 + ПотериНаОкруглении;
			// >> 13.06.2024 Вдовенко С.А., КРОК, A2105505-2658
			...
			// << 13.06.2024 Вдовенко С.А., КРОК, A2105505-2658
			ПотериНаОкруглении = ТекущийПроцент - ЭтапыГрафикаОплаты[Сч].ПроцентПлатежа;
			// >> 13.06.2024 Вдовенко С.А., КРОК, A2105505-2658
			СуммаПроцентов = СуммаПроцентов - ЭтапыГрафикаОплаты[Сч].ПроцентПлатежа;
		КонецЕсли;
	
		Если ЕстьТара Тогда
			Если ЭтапыГрафикаОплаты[Сч].СуммаЗалогаЗаТару <> 0 Тогда
