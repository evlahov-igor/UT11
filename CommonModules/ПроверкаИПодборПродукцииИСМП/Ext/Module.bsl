...
Процедура ДополнитьТаблицуШтрихкодовКолонкамиЧастичногоВыбытия(Таблица) Экспорт
	...
	Таблица.Колонки.Добавить("ЧастичноеВыбытиеВариантУчета",   Новый ОписаниеТипов("ПеречислениеСсылка.ВариантыУчетаЧастичногоВыбытияИСМП"));
...
Процедура ПриОчисткиСопоставленияМаркированнойПродукции(Форма, ИдентификаторыДереваМаркированнойПродукции, КэшированныеЗначения) Экспорт
...
Функция СохраненнаяДетализацияСтруктурыХранения(ПолноеИмяФормы, РежимПодбораСуществующихУпаковок) Экспорт
...
Функция ПрерватьОбработкуЗагрузкиДанных(ДанныеДокумента, ПараметрыСканирования, ДанныеНоменклатурыПоДаннымУПД, ДеревоУпаковок, ВосстановлениеДетализацииТребуетОбращенияКСервису) Экспорт
	...
	Для Каждого ВложеннаяСтрокаДерева Из СтрокаДерева.Строки Цикл
		...
		Если ЗначениеЗаполнено(ВложеннаяСтрокаДерева.GTIN)
			...
			Если ДанныеУточнения = Неопределено Тогда
				...
				ИначеЕсли ВложеннаяСтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая
		...
		Если ВложеннаяСтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
			Или ВложеннаяСтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
			ПроверитьКоэффициентыУпаковок(
...
Процедура ОбработкаСтрокиЧастичногоВыбытияПриВосстановленииДанных(СтрокаПриемника, СтрокаИсточника, ПараметрыСканирования, ВидПродукции) Экспорт
	...
	Если ШтрихкодированиеИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(
		...
		Если СтрокаИсточника.ЧастичноеВыбытие Тогда
			...
			Если СтрокаИсточника.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура Тогда
...
Процедура ДополнитьТаблицуПодобраннойМаркируемойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ВидПродукции, ПараметрыСканирования) Экспорт
	...
	ДополнительныеПараметры.Вставить("ЭтоЗагрузкаДанныхДокументаПриОткрытииФормы", Истина);
	ДополнительныеПараметры.Вставить("УточнитьКоличествоИС",                       Не ДанныеДокумента.РежимПодбораСуществующихУпаковок);
...
Функция СформироватьТаблицуИзмененийПодобраннойПродукции(ТаблицаМаркируемойПродукции, ДанныеДокумента, ПараметрыСканирования, ВидПродукции)
	...
	Если ПоддерживаетсяГрупповаяУпаковка Тогда
		ПрисутствуетПотребительскаяУпаковка = ТаблицаМаркируемойПродукции.Найти(Перечисления.ВидыУпаковокИС.Потребительская, "ВидУпаковки") <> Неопределено;
	Иначе
	...
	Для Каждого СтрокаИсточника Из ТаблицаМаркируемойПродукции Цикл
		...
		ЭтоГрупповаяТоварнаяУпаковка = (СтрокаИсточника.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая);
		...
		Иначе
			КоличествоИзменение = СтрокаИсточника.КоличествоПотребительскихУпаковок; // или КоличествоПотребительскихУпаковокПоДаннымИБ
		КонецЕсли;
