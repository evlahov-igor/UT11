...
Процедура ОбработатьУточнениеДанных(ДанныеШтрихкода, ДанныеВыбора, ПараметрыСканирования, РезультатОбработки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеШтрихкода.ВидУпаковки) Тогда
		Если ДанныеВыбора.КоличествоПотребительскихУпаковок > 1 Тогда
			...
			ДанныеШтрихкода.Коэффициент = КоличествоВПотребительскойУпаковке * ДанныеВыбора.КоличествоПотребительскихУпаковок;
		Иначе
			...
			ДанныеШтрихкода.КоличествоГрупповыхУпаковок = 0;
		КонецЕсли;
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеШтрихкода.КоличествоПотребительскихУпаковок) Тогда
		...
		Иначе
			ДанныеШтрихкода.КоличествоПотребительскихУпаковок = ДанныеВыбора.КоличествоПотребительскихУпаковок;
			ДанныеШтрихкода.Коэффициент = КоличествоВПотребительскойУпаковке * ДанныеВыбора.КоличествоПотребительскихУпаковок;
			ДанныеШтрихкода.КоличествоГрупповыхУпаковок = 1;
		КонецЕсли;
	КонецЕсли;
	...
	Если РезультатПроверки.СостояниеСеройЗоны <> Неопределено Тогда
		...
		Для Каждого ЭлементДанных Из КандидатыВСеруюЗону Цикл
			...
			Если СостояниеСеройЗоны.СодержитСерыеКоды
				...
				КоличествоПотребительскихУпаковокВСерыхБлоках = 0;
				Если СостояниеСеройЗоны.КоличествоГрупповыхУпаковок > 0 Тогда
					GTINБлока = GTINВложеннойГрупповойУпаковки(СтрокаДерева);
					Если GTINБлока <> Неопределено Тогда
						КоличествоПотребительскихУпаковокВБлоке = СтандартнаяВложенностьУпаковок[GTINБлока];
						Если ЗначениеЗаполнено(КоличествоПотребительскихУпаковокВБлоке) Тогда
							КоличествоПотребительскихУпаковокВСерыхБлоках = КоличествоПотребительскихУпаковокВБлоке * СостояниеСеройЗоны.КоличествоГрупповыхУпаковок;
						КонецЕсли;
					ИначеЕсли СостояниеСеройЗоны.КоличествоПотребительскихУпаковок > 0 Тогда
						ПлановоеКоличествоВложенныхПачек = КоэффициентУпаковки(СтрокаДерева, СтандартнаяВложенностьУпаковок);
						Если ЗначениеЗаполнено(ПлановоеКоличествоВложенныхПачек) Тогда
							РасчетноеКоличествоПотребительскихУпаковокВБлоке = ПлановоеКоличествоВложенныхПачек / СостояниеСеройЗоны.КоличествоПотребительскихУпаковок;
							Если РасчетноеКоличествоПотребительскихУпаковокВБлоке = Цел(РасчетноеКоличествоПотребительскихУпаковокВБлоке) Тогда
								КоличествоПотребительскихУпаковокВСерыхБлоках = РасчетноеКоличествоПотребительскихУпаковокВБлоке * СостояниеСеройЗоны.КоличествоГрупповыхУпаковок;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
...
Функция КоэффициентУпаковки(СтрокаДерева, СтандартнаяВложенностьУпаковок)
	...
	Если СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
		...
		СтрокаДерева.СоставКодаМаркировки.Свойство("КоличествоПотребительскихУпаковок", КоличествоПотребительскихУпаковокGTIN02);
	КонецЕсли;
	
	Если КоличествоПотребительскихУпаковокGTIN02 <> Неопределено Тогда
		КоэффициентУпаковки = КоличествоПотребительскихУпаковокGTIN02;
	Иначе
...
Процедура РассчитатьХэшСуммуНормализации(СтрокаДанных, ДанныеРазбора) Экспорт
	...
	Если ШтрихкодированиеИС.ПрисутствуетТабачнаяПродукция(ДанныеРазбора.ВидыПродукции) Тогда
		Если ДанныеРазбора.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
			...
			СтрокаДанных.ХешСуммаНормализации = Справочники.ШтрихкодыУпаковокТоваров.ХэшСуммаСтроки(СтрокаДанных.НормализованныйШтрихкод);
...
Процедура ДополнитьДанныеПоШтрихкодамСпецификойБезЗапросаСтатусовМОТП(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Если ДанныеПоШтрихкодам.Свойство("ВложенныеШтрихкоды")
		...
		ШтрихкодированиеИСМПСлужебный.ЗаполнитьСтрокиТаблицыДанныхПоГрупповымУпаковкам(СтрокиТаблицыДанных);
...
Процедура ОпределитьУдаляемыеСтрокиДереваПоДетализации(СтрокиДерева, Детализация, МассивУдаляемыхСтрок, МаркированныеТовары, ДанныеКодовМаркировки) Экспорт
...
Процедура ПроверитьНаОшибкиПоПравилам(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки)
	...
	Если ДанныеПоШтрихкодам.ВложенныеШтрихкоды <> Неопределено Тогда
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтролироватьСоставГрупповыхУпаковок") Тогда
			...
			Если ПравилаПроверкиНаОшибки.КонтролироватьСоставГрупповыхУпаковок.Количество() Тогда
