			...
			Если ПризнакиСпособаРасчетаАвто.Количество() > 0 Тогда
				...
				Для Каждого ПризнакСпособаРасчетаАвто Из ПризнакиСпособаРасчетаАвто Цикл
					...
					Если ВзаиморасчетыПоОбъектуПредоплата(ПризнакСпособаРасчетаАвто.ОбъектРасчета, ПризнакСпособаРасчетаАвто.ДокументРасчета) Тогда
						...
						ОбъектРасчетовСПризнакомСпособаРасчетов = ОбъектыРасчетовСПризнакамиСпособаРасчетов.Добавить();
						ОбъектРасчетовСПризнакомСпособаРасчетов.ОбъектРасчетов 		  = ПризнакСпособаРасчетаАвто.ОбъектРасчета;
						ОбъектРасчетовСПризнакомСпособаРасчетов.ПризнакСпособаРасчета = ОпределитьВидПредоплатыПоВзаиморасчетам(ПризнакСпособаРасчетаАвто.ОбъектРасчета, ПризнакСпособаРасчетаАвто.Оплата);
						ОбъектРасчетовСПризнакомСпособаРасчетов.Сумма				  = ПризнакСпособаРасчетаАвто.Оплата;
...
Функция ОбновитьМакетЧека(ПараметрыОперацииФискализацииЧека, ВерсияФФД) Экспорт
	...
	ТекстНефискальногоЧека = МенеджерОборудованияКлиентСервер.СформироватьТекстНефискальногоЧека(ПараметрыОперацииФискализацииЧека, 34, ВерсияФФД);
	//-- Локализация
	...
	Иначе
		...
		Если ДокументОплатыСВозможностьюПробитияЧеков(ДокументСсылка) Тогда
			...
			Иначе
				...
				ИначеЕсли ДатаОплатыМеньшеДатыПоставки(ДокументСсылка, ОбъектыРасчетов)
					ИЛИ ПоДокументуОплатыПробитЧек(ДокументСсылка)
					ИЛИ НЕ ПоВсемОбъектамРасчетовНеобходимоПробитьЧек(ОбъектыРасчетов)
					ИЛИ ОбъектыРасчетовРТУКПредоплате(ОбъектыРасчетов) Тогда
...
Функция МаксимальнаяВерсияФФД() Экспорт
	...
	//++ Локализация
	ПоддерживаемыеФорматыФФД = МенеджерОборудованияКлиентСерверПереопределяемый.ПолучитьПоддерживаемыйФорматыФФД();
...
// 	ИмяКомандыПробитияЧека - Строка - Имя команды пробития чека
// Возвращаемое значение:
// 	ТаблицаЗначений - Описание: таблицу значений, содержащую значения ставок НДС и сумм НДС по оплачиваемому объекту расчетов
Функция СтавкиНДСПоОбъектуРасчетовПоДокументуОплаты(ДокументОплаты, ОбъектРасчета, СистемаНалогообложения, ИмяКомандыПробитияЧека) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДокументОплаты)
		ИЛИ НЕ ДокументОплатыСВозможностьюПробитияЧеков(ДокументОплаты) Тогда
...
Процедура ТаблицаТоваровСРаспределениемКодовМаркировкиИСМП(Товары, ДанныеДляИСМП) Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		Для каждого СтрокаДляДобавленияВТовары из МассивСтрокДляДобавленияВТовары Цикл
			...
			Если СтрокаДляДобавленияВТовары.РезультатРаспределения.ЧастичноеВыбытие
				И СтрокаТЧ.КоличествоУпаковок <> СтрокаТЧ.Количество Тогда
...
Процедура ОбновитьВзаиморасчеты(ПараметрыОперации, ВзаиморасчетыОбновлены)
	...
	Если ВзаиморасчетыОбновлены
		...
		РассчитатьВзаиморасчеты(ПараметрыОперации);
	КонецЕсли;
	//-- Локализация
...
Функция ОпределитьВидПредоплатыПоВзаиморасчетам(ОбъектРасчета, Оплата)
	
	Попытка
		СуммаПоставки = ФормированиеПараметровФискальногоЧекаСервер.ПозицииНоменклатурыПоДокументу(ОбъектРасчета).Итог("Сумма");
		пПризнакСпособаРасчета = ?(Оплата = СуммаПоставки, Перечисления.ПризнакиСпособаРасчета.ПредоплатаПолная, Перечисления.ПризнакиСпособаРасчета.ПредоплатаЧастичная);
	Исключение
		пПризнакСпособаРасчета = Перечисления.ПризнакиСпособаРасчета.ПредоплатаПолная;
	КонецПопытки;
	
	Возврат пПризнакСпособаРасчета;
	
КонецФункции

Функция РассчитатьПоВзаиморасчетамПризнакСпособаРасчетаДокументаПоставки(ПризнакиСпособаРасчетаАвто)
	
	ПризнакСпособаРасчетаПоставки = Неопределено;
	
	СуммаПредоплаты = СуммаПредоплаты(ПризнакиСпособаРасчетаАвто);
...
Функция СуммаПредоплаты(ПризнакиСпособаРасчетаАвто)
	...
	Для Каждого СтрокаПризнакаСпособаРасчетаАвто Из ПризнакиСпособаРасчетаАвто Цикл
		Если СтрокаПризнакаСпособаРасчетаАвто.ОбъектРасчета = СтрокаПризнакаСпособаРасчетаАвто.ДокументРасчета
			И ДокументОплатыСВозможностьюПробитияЧеков(СтрокаПризнакаСпособаРасчетаАвто.ОбъектРасчета) Тогда
		...
		Иначе
			...
			Если НЕ ПараметрыОперацииЧека.Свойство("ЗаданныйПризнакСпособаРасчета")
				...
				ФормированиеПараметровФискальногоЧекаСервер.ОбновитьПараметрыФискальногоЧекаТаблицыОплатПоДокументуПоставки(
					ПараметрыОперацииФискализацииЧека,
					РассчитатьСтруктуруОплатДокументаПоставки(ПараметрыОперацииЧека.ПризнакиСпособаРасчетаАвто, СуммаПоставки));
	...
	Если ТаблицаДокументовЕдиногоЧека.Количество() > 0 Тогда
		Для Каждого СтрокаОбъектыРасчетов Из ОбъектыРасчетов Цикл
			Если ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(СтрокаОбъектыРасчетов.Заказ) Тогда
				...
				СтрокаДокументЕдиногоЧека.Сумма = СтрокаОбъектыРасчетов.Сумма;
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;
...
Функция ДокументыВзаимозачета(ДокументыПоставки)
	...
	|ГДЕ
	|	ВзаимозачетЗадолженностиДебиторскаяЗадолженность.Ссылка.ВидОперации = &ВидОперации
	...
	Запрос.УстановитьПараметр("ВидОперации", Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПереносАвансаКлиентаОрганизацияКонтрагент);
...
Функция ДоступнаВозможностьПробитияЕдиногоЧека(ДокументОплаты, ОбъектыРасчетов)
	...
	Если СуммаОплатыБольшеСуммыПоставок(ДокументОплаты, ОбъектыРасчетов) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ДокументПоставкиСВозможностьюПробитияЕдиногоЧека(ДокументСсылка)
	...
	|	И РеализацияУслугПрочихАктивов.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту)
...
Функция ПоДокументуОплатыПробитЧек(ДокументОплаты)
	...
	Если МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(ДокументОплаты) <> Неопределено Тогда
...
Функция СуммаОплатыБольшеСуммыПоставок(ДокументОплаты, ОбъектыРасчетов)
	...
	СуммаПоставки = 0;
	Для Каждого СтрокаОбъектыРасчетов Из ОбъектыРасчетов Цикл
		СуммаПоставки = СуммаПоставки + ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаОбъектыРасчетов.Заказ, "СуммаДокумента");
	КонецЦикла;
...
Процедура РассчитатьВзаиморасчеты(ПараметрыОперации)
...
Функция ДокументПоставкиСВозможностьюПробитияЧеков(ДокументСсылка)
