...
Функция ТаблицаОстатковКЗаказу(ДокументСсылка, Параметры) Экспорт
	...
	УстановитьОтбор = ЗначениеЗаполнено(Отбор);
	...
	Запрос.Текст =
		...
		|	КОНЕЦ КАК Упаковка,
		|	Товары.Заказано КАК Количество,
		|	Товары.Заказано / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) КАК КоличествоУпаковок,
		...
		|ИЗ
		|	ВТНоменклатураЗаказа КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК ВсеУпаковки
		|		ПО ВсеУпаковки.Ссылка = Товары.Упаковка1
		|			И Товары.Упаковка1 = Товары.Упаковка2
...
Процедура ДобавитьВременныеТаблицыОстатковРасходаИВозвратов(Запрос)
	...
	Запрос.Текст =
		...
		|		ТоварыНаСкладах.Характеристика КАК Характеристика,
		|		ТоварыНаСкладах.Склад          КАК Склад,
		// << 04.12.2023 Марченко С.Н., КРОК, JIRA№A2105505-2342
		...
		|			&ПериодКлассификацииДатаНачала,
		|			&ПериодКлассификацииДатаОкончания,
		// << 09.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2342
		//|			ДЕНЬ,
		|			Регистратор,
		// >> 09.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2342
		|			Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		...
		// << 09.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2342
		|	ГДЕ
		|		ТИПЗНАЧЕНИЯ(ТоварыНаСкладах.Регистратор) <> ТИП(Документ.ОтборРазмещениеТоваров) 
		// >> 09.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2342
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
