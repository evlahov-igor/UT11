	...
	Для Каждого Эл Из СоответствиеОбъектов Цикл
		Если Эл.Значение Тогда
			ТекстЗапроса = "ОБЪЕДИНИТЬ ВСЕ
							...
							|ИЗ &СправочникЭлКлючИзменения
							|ГДЕ 
							|	Узел = &Узел";
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СправочникЭлКлючИзменения", "Справочник." + Эл.Ключ + ".Изменения");
		КонецЕсли;
	КонецЦикла;
	
	ТекстЗапроса = "(" + СокрЛП(Сред(ТекстЗапроса, 15)) + ")";
	ТекстИтоговогоЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|Количество(ИзмСсылка) КАК КоличествоИзм
					|ИЗ &ТекстЗапроса КАК Изменения";
	ТекстИтоговогоЗапроса = СтрЗаменить(ТекстИтоговогоЗапроса, "&ТекстЗапроса", ТекстЗапроса);
	
	Запрос = Новый Запрос(ТекстИтоговогоЗапроса);
