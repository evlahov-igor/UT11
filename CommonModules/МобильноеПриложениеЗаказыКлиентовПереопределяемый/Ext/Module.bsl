...
Процедура ВыгрузитьОбъектыСсылочногоТипа(ЗаписьXML, ВыгруженныеОбъекты, СтрокаПрайсЛиста, УзелОбмена)
	...
	Если ЗначениеЗаполнено(СтрокаПрайсЛиста.УпаковкаСсылка)
		...
		СтруктураУпаковки		= МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураУпаковки();
...
Функция ГруппаСкладовДляВыбораЗаказе(ИдентификаторСклада, СкладРодитель, Выгружать)
	...
	Для Каждого Строка Из ИерархияСклада Цикл
		...
		Если Выгружать И (НЕ Строка.СкладСсылка = СкладРодитель) Тогда
			СтруктураСклада = МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураСклада(Истина);
...
Процедура ЗаписатьЗаданиеВПакетОбмена(ЗаписьXML, Данные, ИспользоватьСоглашенияИлиДоговоры)
	...
	// Данные ТЧ "Задачи".
	Если Данные.Задачи.Количество() > 0 Тогда //@NON-NLS-1
		Для Каждого ЭлементСписка Из Данные.Задачи Цикл //@NON-NLS-1
	...
	Если Данные.ДолгиПоЗаказам.Количество() > 0 Тогда //@NON-NLS-1
		Для Каждого СтрокаТЧ Из Данные.ДолгиПоЗаказам Цикл //@NON-NLS-1
	...
	Если Данные.РасшифровкаДебиторскойЗадолженности.Количество() > 0 Тогда //@NON-NLS-1
		Для Каждого СтрокаТЧ Из Данные.РасшифровкаДебиторскойЗадолженности Цикл //@NON-NLS-1
...
Процедура ЗаписатьИнформациюПоСтатусуЗаданияТорговомуПредставителю(Сообщение, Знач ЗаданиеТорговомуПредставителю)
	...
	Структура = МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураСтатусаЗаданияТорговомуПредставителю();
	Структура.ref = СокрЛП(ЗаданиеТорговомуПредставителю.Ссылка.УникальныйИдентификатор());
...
Процедура ЗаписатьИнформациюПоСтатусуЗаказаКлиента(Сообщение, Знач ЗаказКлиента, Знач Изменен = Ложь, Ошибка = Ложь)
	...
	ИнформацияПоСтатусуЗаказа = МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураСтатусаЗаказаКлиента();
...
Процедура ЗаписатьИнформациюПоСтатусуЗаявкиНаВозврат(Сообщение, Знач ЗаявкаНаВозвратТоваров, Знач Изменен = Ложь, Ошибка = Ложь)
	...
	ИнформацияПоСтатусу = МобильноеПриложениеЗаказыКлиентовПовтИсп.СтруктураСтатусаЗаявкиНаВозврат();
...
Функция ИнформацияОКонтрагенте(Знач СтрокаТаблицы)
	...
	СтруктураКонтрагента["ПредставлениеКонтрагента"] = СтрокаТаблицы.КонтрагентНаименованиеПолное//@NON-NLS-1
		+ " (" +СтруктураКонтрагента["ИННКПП"] + ")";//@NON-NLS-1
