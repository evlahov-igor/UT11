...
//  Количество - Число
//  Заголовок                - Строка             - переопределяемый заголовок.
Процедура ПоказатьВводШтрихкодаСписком(ОповещениеУспешногоВвода, Количество = Неопределено, Заголовок = "") Экспорт 
	
	Если НЕ ЗначениеЗаполнено(Заголовок) Тогда
		Заголовок = НСтр("ru = 'Введите штрихкоды'");
	КонецЕсли;
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПоказатьВводШтрихкодаСпискомЗавершение",
		ЭтотОбъект,
		ДополнительныеПараметры);
	
	ПоказатьВводСтроки(Оповещение, "", Заголовок,, Истина);
...
// --> Евлахов Игорь Николаевич (Начало) 05.02.2024
// Задача #3617
Процедура ПоказатьВводШтрихкодаСпискомЗавершение(Штрихкоды, ДополнительныеПараметры) Экспорт
	
	ОповещениеУспешногоВвода = ДополнительныеПараметры.ОповещениеУспешногоВвода;
	Количество = ДополнительныеПараметры.Количество;
	
	Если (Штрихкоды <> Неопределено) Тогда
		Если Не ПустаяСтрока(Штрихкоды) Тогда
			Если Количество = Неопределено Тогда
				Количество = 1;
			КонецЕсли;		
			
			Штрихкоды = СокрЛП(Штрихкоды);
			Штрихкоды = СтрЗаменить(Штрихкоды, " ", "");	// Удалим пробелы из строки 
			
			МассивШтрихкодов = Новый Массив();
			МассивДанных = СтрРазделить(Штрихкоды, Символы.ПС);
			
			Для каждого Штрихкод Из МассивДанных Цикл
				Если СтрДлина(Штрихкод) > 200 Тогда
					ТекстСообщения = НСтр("ru='Длина штрихкода не должна быть больше 200 символов.'");
					ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
				...
				Если ЗначениеЗаполнено(Штрихкод) Тогда
					МассивШтрихкодов.Добавить(СтруктураДанныхШтрихкода(СокрЛП(Штрихкод), Количество));
				КонецЕсли;
			КонецЦикла;
			
			Если МассивШтрихкодов.Количество() > 0 Тогда
				ВыполнитьОбработкуОповещения(ОповещениеУспешногоВвода, МассивШтрихкодов);
			КонецЕсли;				
		КонецЕсли;
	КонецЕсли;
