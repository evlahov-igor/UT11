...
Функция ИмяПрикладногоРеквизита(Представление, Обязательное = Истина) Экспорт
	...
	ИмяРеквизита = ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(Представление);
...
Процедура ПрочитатьТикетСбербанка(НастройкаОбмена, Тикет,
	...
	Если Тикет.Info = Неопределено
		...
		Запрос.УстановитьПараметр("УникальныйИдентификатор", УникальныйИДВнешний);
	ИначеЕсли ЗначениеЗаполнено(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "Info.docExtId")) Тогда
		УникальныйИД = Тикет.Info.docExtId;
		Запрос.УстановитьПараметр("УникальныйИдентификатор", УникальныйИД);
	ИначеЕсли ЗначениеЗаполнено(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "Info.MsgFromBank.Message"))
	...
	ИначеЕсли ВРег(Тикет.Info.statusStateCode) = "RESPONSE_DIVISION" Тогда
		...
		Возврат;
	ИначеЕсли ЗначениеЗаполнено(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "Info.MsgFromBank.Message")) Тогда
	...
	Если Выборка.Следующий() Тогда
		...
		Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "Info") <> Неопределено Тогда
		...
		Если НЕ ЗначениеЗаполнено(СтатусСББОЛ)
			...
			Если ЗначениеЗаполнено(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "createTime")) Тогда
				ДатаСоздания = XMLЗначение(Тип("Дата"), Тикет.createTime);
			...
			СтруктураВладельца.Вставить("ВнешнийИдентификатор", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "docId"));
			...
			Если ИспользуетсяКриптография Тогда
				...
				Если НЕ РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "Sign") = Неопределено Тогда
			...
			ПричинаОтклонения = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Тикет, "Info.MsgFromBank.Message");
...
Процедура ОбработатьЭлектронныеДокументы(Параметры, АдресРезультата) Экспорт
	...
	Если НЕ ОбщегоНазначенияБЭДПовтИсп.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
		ТекстСообщения = НастройкиБЭДКлиентСервер.ТекстСообщенияОНеобходимостиНастройкиСистемы(
			"РАБОТАСБАНКАМИ");
	...
	Если МассивОтпечатковКлиент <> Неопределено
		И (ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Подписать")
			ИЛИ ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Отправить")) Тогда
	...
	Если ТипЗнч(СообщениеОбмена) <> Тип("Массив")
			...
			И (СообщениеОбмена.Направление = Перечисления.НаправленияЭДО.Входящий)) Тогда
	...
	Сертификаты = КриптографияБЭД.НайтиСертификаты(МассивОтпечатковКлиент);
	СертификатыСПаролями = КриптографияБЭД.ПаролиСертификатов(Сертификаты);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ПодчиненноеСообщениеОбмена(ИсходноеСообщение, ВидЭД = Неопределено, ВыдаватьОшибку = Истина) Экспорт
	...
	Иначе
		Если ВидЭД = Перечисления.ВидыЭДОбменСБанками.ДополнительныеДанные
			...
			ПараметрыСообщения.Вставить("Направление", Перечисления.НаправленияЭДО.Исходящий);
...
Процедура ПолучитьНовыеДокументыИзБанка(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	Результат = ИнтернетСоединениеБЭД.СкачатьФайлВоВременноеХранилище(АдресСервера
...
Процедура Пауза(КоличествоСекунд) Экспорт
	...
	Если ТипЗнч(КоличествоСекунд) <> Тип("Число") ИЛИ КоличествоСекунд < 1 ИЛИ КоличествоСекунд > 15 Тогда
		Возврат
	КонецЕсли;
	...
	Если ФоновоеЗадание = Неопределено Тогда
		...
		ИмяМетода = "ОбменСБанкамиСлужебный.Пауза";
...
Функция ОтправкаПакетовВБанк(МассивПакетов, ИдентификаторСессии = Неопределено, ТекстСообщения = "") Экспорт
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетов Цикл
		...
		Попытка
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ПакетОбменСБанками, Перечисления.СтатусыТранспортныхСообщенийБЭД.Отправлен, СтруктураЭД);
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				ОбменСБанкамиСлужебныйВызовСервера.УстановитьСтатусПакета(ПакетОбменСБанками, Перечисления.СтатусыТранспортныхСообщенийБЭД.Отменен);
		...
		Исключение
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ПакетОбменСБанками, Перечисления.СтатусыТранспортныхСообщенийБЭД.Отменен, СтруктураРеквизитов);
		КонецПопытки;
...
Процедура СформироватьИОтправитьПакетыВБанк(Параметры, АдресХранилища) Экспорт
	...
	СтруктураРезультата.Вставить("ПодтверждениеПлатежейВБанке", СтруктураПодтвержденияПлатежа());
	...
	Попытка
		...
		Если НЕ ДанныеВозврата.ЕстьОшибка Тогда
			ОбменСБанкамиСлужебныйВызовСервера.УстановитьСтатусПакета(
				ПакетОбменСБанками, Перечисления.СтатусыТранспортныхСообщенийБЭД.Распакован);
		КонецЕсли;
...
Процедура СохранитьПолученныйПакет(
	...
	Если ЗначениеСвойстваXDTO(ResultBank, "Success.GetPacketResponse") <> Неопределено Тогда
		...
		СтруктураПараметров.Вставить("Статус", Перечисления.СтатусыТранспортныхСообщенийБЭД.КРаспаковке);
		СтруктураПараметров.Вставить("Направление", Перечисления.НаправленияЭДО.Входящий);
...
Функция ГотовыеКРаспаковкеПакетыЭД(Знач ПакетыЭД) Экспорт
	...
	Пока ВыборкаПакеты.Следующий() Цикл
		...
		Если ВыборкаПакеты.Статус <> Перечисления.СтатусыТранспортныхСообщенийБЭД.КРаспаковке Тогда
...
Процедура ПрочитатьИдентификаторыПакетов(НастройкаОбмена, Путь,
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	И ПакетЭД.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Входящий)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура СоздатьПакетОбменСБанками(НастройкаОбмена, МассивСообщений, ПакетОбменСБанками) Экспорт
	...
	ПакетОбменСБанкамиОбъект.Направление = Перечисления.НаправленияЭДО.Исходящий;
	...
	ПакетОбменСБанкамиОбъект.Статус = Перечисления.СтатусыТранспортныхСообщенийБЭД.ПодготовленКОтправке;
...
//
// Параметры:
//  Конверт  - ДокументСсылка.ПакетОбменСБанками - ссылка на пакет;
...
Процедура УстановитьМаршрутПодписанияПоследнихПлатежныхДокументов(НастройкаОбмена) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПараметрыОбмена.ТребуетсяПодпись
			...
			ВесМаршрута = 0;
			// Сформируем маршрут
			МаршрутыПодписанияБЭД.СформироватьМаршрутПодписанияЭД(Выборка.Ссылка, ПараметрыОбмена.МаршрутПодписания, , , ВесМаршрута);
...
// Возвращает описание запроса для валидации маршрутов 
// 
// Параметры:
// 	ИмяВременнойТаблицы - Строка - название временной таблицы для помещения результата
// 	МаршрутПодписанияИмяПараметра - Неопределено - маршрут не задан
// 								  - Строка - параметр запроса, содержащий массив маршрутов.
// Возвращаемое значение:
// 	Структура - Описание:
...
Функция ЗапросНастроекДляВалидацииМаршрутов(
	ИмяВременнойТаблицы,
	МаршрутПодписанияИмяПараметра = Неопределено) Экспорт
	...
	|ИЗ
	|	Справочник.НастройкиОбменСБанками.ИсходящиеДокументы КАК НастройкиОбменСБанкамиИсходящиеДокументы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиОбменСБанками.СертификатыПодписейОрганизации КАК
	...
	|ГДЕ
	|	НастройкиОбменСБанкамиИсходящиеДокументы.МаршрутПодписания <> ЗНАЧЕНИЕ(Справочник.МаршрутыПодписания.ПустаяСсылка)";
	
	ПоляУсловия = Новый Массив;

	Если ЗначениеЗаполнено(МаршрутПодписанияИмяПараметра) Тогда
		ПоляУсловия.Добавить(СтрШаблон("НастройкиОбменСБанкамиИсходящиеДокументы.МаршрутПодписания В(%1)",
		 МаршрутПодписанияИмяПараметра));
	КонецЕсли;
	
	ОписаниеЗапроса = ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса();
	ОписаниеЗапроса.Текст = ОбщегоНазначенияБЭД.ТекстЗапросаИзШаблона(
		ТекстЗапроса, ИмяВременнойТаблицы, "", ПоляУсловия);
...
Процедура ДобавитьОтпечаткиСертификатовНаСервере(МассивСертификатов) Экспорт
	...
	Если ВыполнятьКриптооперацииНаСервере Тогда
		Попытка
			МассивОтпечатковСертификатовНаСервере = КриптографияБЭД.ПолучитьОтпечаткиСертификатовНаСервере(Истина).Отпечатки;
		Исключение
...
Процедура СформироватьСправкуОПодтверждающихДокументах(
	...
	ДеревоЭлектронногоДокументаБЭД.ПроверитьЗаполнениеРеквизитовДереваДанныхРекурсивно(ДеревоДокумента, Ошибки);
...
Процедура ОбменСБанкамиВладелецЭДПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ЭлектронноеВзаимодействиеПереопределяемый.ПередЗаписьюВладельцаЭлектронногоДокумента(Источник, ПризнакИзменения, Состояние, ПризнакУчастияВОбмене, Истина, Отказ);
...
Процедура УстановитьСостояниеПриЗаписиОбъектаИБ(Источник, Отказ) Экспорт
	...
	Если НЕ ОбщегоНазначенияБЭД.ЗначениеФункциональнойОпции("ИспользоватьОбменСБанками") Тогда
		Возврат;
	КонецЕсли;
...
Функция МассивСтатусовЭД(НастройкиОбмена) Экспорт
	...
	Если НастройкиОбмена.Направление = Перечисления.НаправленияЭДО.Исходящий Тогда
...
//         "ПользовательОповещенОПлатформе8_3_18" - выведено сообщение о необходимости обновления платформы.
...
Функция ПолучитьМакетИдентификатораДокумента() Экспорт
	...
	Возврат Обработки.ОбменСБанками.ПолучитьМакет(
...
Процедура ДобавитьВыпискуXML(
	...
	Попытка
		...
		Иначе // тип указан в явном виде
			ТипClientBankExchange = ТипЗначенияCML(ФабрикаXDTO, "urn:1C.ru:ClientBankExchange", "ClientBankExchange");
...
Процедура ДобавитьВыпискуJSON(СообщениеОбмена, ДвоичныеДанныеФайла, ТаблицаВыписок) Экспорт
	...
	СтруктураДанных = ДанныеИзJSON(ДанныеСтрокой);
...
//
Функция МассивЗапросовВыпискиАсинхронныйОбмен(
	НастройкаОбмена,
	ДатаНачала,
	ДатаОкончания,
	МассивБанковскихСчетов,
	СозданАвтоматически = Ложь) Экспорт
...
//  ПараметрыЖурналирования - Структура - данные, переданные в метод ИнтернетСоединениеБЭДВызовСервера.СкачатьФайлВоВременноеХранилище.
//
...
Процедура ПолучитьВыпискуБанкаFintech(Параметры, Адрес) Экспорт
	...
	ОбменСБанкамиФинтехСлужебный.ПолучитьВыписку(Параметры.НастройкаОбмена, ТокенАутентификацииBase64,
		Параметры.НомерСчета, Дата, ДатаОкончания, ДанныеВыписок, ТребуетсяАутентификация);
...
Процедура ПробноеПолучениеВыпискиFintech(Параметры, Адрес) Экспорт
	...
	ОбменСБанкамиФинтехСлужебный.ПолучитьВыписку(Параметры.НастройкаОбмена, ТокенАутентификацииBase64,
		НомерСчета, ТекущаяДатаСеанса(), ТекущаяДатаСеанса(), ДанныеВыписок, ТребуетсяАутентификация);
...
Процедура ДобавитьЗначениеВДерево(ДеревоДанных, ИмяРеквизита,
	...
	ДеревоЭлектронногоДокументаБЭД.ДобавитьЗначениеВДерево(ДеревоДанных, ИмяРеквизита, ЗначениеРеквизита);
...
Функция НайтиСоздатьСтрокуВДеревеРазбора(СтрокаТипаВДереве, ИдОбъекта,
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		...
		Если НайденнаяСтрока = Неопределено Тогда
			...
			НайденнаяСтрока.ОписаниеТипа = ДеревоЭлектронногоДокументаБЭД.ТипОбъекта(СтрокаТипаВДереве.ТипОбъекта);
			НайденнаяСтрока.ИндексСтроки = СтрокаТипаВДереве.ИндексСтроки + "_"
				+ Строка(СтрокаТипаВДереве.Строки.Индекс(НайденнаяСтрока));
			ДеревоЭлектронногоДокументаБЭД.ДобавитьДопРеквизиты(НайденнаяСтрока, ДопРеквизиты, ДеревоРазбора);
		Иначе
			ДеревоЭлектронногоДокументаБЭД.ДобавитьДопРеквизиты(НайденнаяСтрока, ДопРеквизиты, ДеревоРазбора);
		КонецЕсли;
...
Процедура ЗаполнитьОтправителяИПолучателя(
	...
	СведенияОбОрганизации = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(Организация);
...
Функция ТипОбъектаCML(Фабрика, URIПространстваИмен, Путь)
	...
	МассивПути = РаботаСФайламиБЭД.ЭлементыПутиТипаXDTO(Путь, ".");
...
Функция СформироватьФайлыДокументов(МассивОбъектовДляВыгрузки, НастройкиОбъектов, ДопПараметры = "")
	...
	Для Каждого ТекЭлемент Из СтруктураВидовЭД Цикл
		...
		Данные = Новый Массив;
		Для Каждого ОбъектДляВыгрузки Из ТекЭлемент.Значение Цикл
			ДеревоДокумента = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента(Макет);
...
Процедура ПрочитатьВыпискуXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	Если НЕ ЭД.СекцияРасчСчет = Неопределено Тогда
		...
		Если ЭД.СекцияРасчСчет.РасчСчета.Количество() > 0 Тогда
			...
			Для Каждого РеквизитыРасчСчета Из ЭД.СекцияРасчСчет.РасчСчета Цикл
				...
				НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "БанковскийСчет");
				РасчетныйСчет = ОбщегоНазначенияБЭД.НайтиСсылку("БанковскиеСчетаОрганизаций", РеквизитыРасчСчета.РасчСчет);
				...
				ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитыТЧОбъекта(НовыйЭД, "БанковскиеСчетаОрганизаций", СписокТЧ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ЭлементПлатежа Из ЭД.СекцияПлатежногоДокумента Цикл
		...
		ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитыТЧОбъекта(НовыйЭД, "СтрокаТЧ", РеквизитыПлатежа);
	КонецЦикла;
...
Процедура ПрочитатьЗапросВыпискиXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	Для Каждого Элемент Из ЗапросВыписки.РасчСчет Цикл
		...
		ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитыТЧОбъекта(НовыйЭД, "СтрокаТЧ", РеквизитыСчета);
	КонецЦикла;
...
Процедура ПрочитатьВыпискуAsyncXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "БанковскийСчет");
	РасчетныйСчет = ОбщегоНазначенияБЭД.НайтиСсылку("БанковскиеСчетаОрганизаций", ПараметрыСчета.РасчСчет);
	...
	Если Stamp <> Неопределено Тогда
		...
		НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "Штамп");
...
Процедура ДобавитьРеквизитШапкиОбъекта(СтрокаЭлементов, ИмяРеквизита,
	...
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(СтрокаЭлементов, ИмяРеквизита, ЗначениеРеквизита, СсылкаНаОбъект);
...
Процедура ОбновитьИнтервалОжидания(КоличествоСекунд)
	...
	КоличествоСекунд = КоличествоСекунд * 1.4;
	Если КоличествоСекунд > 15 Тогда
		КоличествоСекунд = 15;
	КонецЕсли;
...
Функция НайтиПодготовленныеКОтправкеПакеты(МассивСообщенийОбмена)
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	И ПакетЭДСообщения.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыТранспортныхСообщенийБЭД.ПодготовленКОтправке)";
...
Функция НайтиПакетОбменСБанками(НастройкаОбмена, ВнешнийИдентификатор)
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	И ПакетОбменСБанками.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Исходящий)";
	Запрос.УстановитьПараметр("НастройкаОбмена", НастройкаОбмена);
...
Функция НовыйМаршрутПоСертификатамНастройки(НастройкаОбмена, ВидДокумента,
	...
	Если НастройкаСодержитСертификаты Тогда
		...
		ДеревоТребований = МаршрутыПодписанияБЭД.ПустоеДеревоТребованийКПодписанию();
		...
		МаршрутыПодписанияБЭД.ЗаполнитьТаблицуТребованийКПодписаниюПоДереву(МаршрутОбъект.ТаблицаТребований, ДеревоТребований);
...
Функция НайтиСоздатьМаршрутПодписания(НастройкаОбменаОбъект, ВидЭД, ДеревоТребований)
	...
	МаршрутыПодписанияБЭД.ЗаполнитьТаблицуТребованийКПодписаниюПоДереву(
...
//
Функция СтруктураПодтвержденияПлатежа()
...
Процедура СформироватьЭлектронныеДокументы(
	...
	Если СообщениеОбмена = Неопределено
			И ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Сформировать") Тогда
...
Процедура ПодготовитьКПодписаниюЭлектронныеДокументы(
	...
	Замер = ОценкаПроизводительности.НачатьЗамерДлительнойОперации(
		"ОбщийМодуль.ОбменСБанкамиСлужебный.ПодготовитьКПодписаниюЭлектронныеДокументы");
	
	Если НЕ ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		Возврат
	КонецЕсли;
	...
	ОценкаПроизводительности.ЗакончитьЗамерДлительнойОперации(Замер, МассивСсылокНаОбъект.Количество());
...
Процедура ОтправитьЭлектронныеДокументы(
	...
	Если НЕ ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьДокументыЧерезСервисFintech(Тз_Эд_КОтправке, СессииОбменСБанками, СтруктураВозврата)
	...
	Для Каждого НастройкаОбмена Из МассивНастроекОбмена Цикл
		...
		МассивСообщенийОбмена = Новый Массив;
		Для Каждого ЭлементВыборки Из ВременнаяТЗПоНастройкеОбмена Цикл
			// Если платежное поручение отправлено в банк и находится на подписи,
			// то повторно передавать в банк его не требуется (возникнет коллизия).
			Если ЭлементВыборки.Статус <> Перечисления.СтатусыОбменСБанками.НеПодтвержден Тогда
				МассивСообщенийОбмена.Добавить(ЭлементВыборки.СсылкаНаСообщениеОбмена);
			КонецЕсли;
		КонецЦикла;
...
Функция НастройкиОбъектов(Действия, МассивСсылокНаОбъект, Фрод, СтруктураВозврата, АдресРезультата)
	...
	Для Сч = -МассивСсылокНаОбъект.Количество() + 1 По 0 Цикл
		...
		Фрод.Вставить("ЕстьОтправка", ОбменСБанкамиКлиентСервер.ЕстьДействие(Действия, "Отправить"));
...
Процедура ПроверитьВозможностьПередачиДанныхПлатежногоПоручения(
	...
	Если ЗначениеЗаполнено(КодВидаДохода) Тогда
		Если (ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
			...
			ДеревоЭлектронногоДокументаБЭД.ДобавитьОшибкуПроверкиДереваДанных(Ошибки, СтрокаДерева, ТекстОшибки, Ложь);
...
Процедура ПроверитьДанныеПлатежногоПоручения(СсылкаНаОбъект, ДеревоДанных, ПрограммаБанка, Ошибки)
	...
	Если ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн
		Или ПрограммаБанка = Перечисления.ПрограммыБанка.FintechIntegration Тогда
...
Функция ИзмененыРеквизитыДокумента(СообщениеОбмена)
	...
	ДеревьяДанных.Добавить(ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента(Макет));
	...
	Если НЕ StatusPacketNotice.Result.Error = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ОтправленныйПакетЭД) Тогда
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(
				ОтправленныйПакетЭД, Перечисления.СтатусыТранспортныхСообщенийБЭД.Отменен, СтруктураЭД, СообщенияОбмена);
			Для Каждого СообщениеОбмена Из СообщенияОбмена Цикл
		...
		Если ЗначениеЗаполнено(ОтправленныйПакетОбменСБанками) Тогда
			Если StatusPacketNotice.Result.Status.Code = "01" Тогда
				...
				ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ОтправленныйПакетОбменСБанками,
					Перечисления.СтатусыТранспортныхСообщенийБЭД.Доставлен, СтруктураЭД, СообщенияОбмена);
				Для Каждого СообщениеОбмена Из СообщенияОбмена Цикл
					ДокументУчета = ОбменСБанкамиСлужебныйВызовСервера.ДокументУчета(СообщениеОбмена);
					Если ЗначениеЗаполнено(ДокументУчета) Тогда
						СостояниеЭДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СообщениеОбмена, "Состояние");
...
Процедура ЗагрузитьТипыПисем(НастройкаОбмена, Data)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ТипПисьмаОбъект.Наименование = Выборка.НаименованиеИзБанка;
			ТипПисьмаОбъект.Направление = Перечисления.НаправленияЭДО.Исходящий;
...
Функция ТекстЗапросаТиповПисем()
	...
	|ГДЕ
	|	ТипыПисемОбменСБанками.Владелец = &Владелец
	|	И ТипыПисемОбменСБанками.Направление = ЗНАЧЕНИЕ(Перечисление.НаправленияЭДО.Исходящий)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
		...
		Если Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			...
			Для Каждого ГруппаПодписи Из ЭД.Data.CryptoParameters.CustomerSignature.GroupSignatures Цикл
				...
				Для Каждого Сертификат Из ГруппаПодписи.Certificate Цикл
					...
					Сертификат = КриптографияБЭД.НайтиСоздатьСертификатЭП(
...
Функция ПараметрыАвторизацииНаСервереБанка(
	...
	ИспользоватьЭП = ОбщегоНазначенияБЭДПовтИсп.ЗначениеФункциональнойОпции(
	...
	Если ТипЗнч(СтМассивовСтруктурСертификатов) = Тип("Структура") Тогда
		...
		Если ВыполнятьКриптооперацииНаСервере
			...
			МассивОтпечатковСервер = КриптографияБЭД.ПолучитьОтпечаткиСертификатовНаСервере(Ложь).Отпечатки;
		КонецЕсли;
	КонецЕсли;
	...
	Если (МассивОтпечатковСервер <> Неопределено
		...
		УстановитьПривилегированныйРежим(Истина);
		Сертификаты = КриптографияБЭД.НайтиСертификаты(МассивОтпечатковДляАвторизации);
		СертификатыСПаролями = КриптографияБЭД.ПаролиСертификатов(Сертификаты);
	...
	Если ПоддерживаетсяПередачаФрод(ВерсияФормата) Тогда
		Если ЗначениеЗаполнено(Фрод) Тогда
			...
			Если Фрод.Свойство("MAC", MACАдресаКлиента) И ЗначениеЗаполнено(MACАдресаКлиента) Тогда
				...
				КонецЦикла;
			КонецЕсли;
			IPАдресаКлиента = Неопределено;
			Если Фрод.Свойство("IP", IPАдресаКлиента) И ЗначениеЗаполнено(IPАдресаКлиента) Тогда
				Для Каждого IP Из IPАдресаКлиента Цикл
					SenderFootprint.IP.Добавить(IP);
				КонецЦикла;
			КонецЕсли;
...
Процедура ПолучитьСтатусыДокументовСбербанк(
	НастройкаОбмена,
	ИдентификаторСессии, КоличествоПолучено,
	МассивСообщенийОбмена = Неопределено,
	ВыводитьПрогрессВыполнения = Истина)
	...
	Пока Истина Цикл
		...
		Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
			...
			Данные.Добавить(ТекстЗапроса);
			ОтправитьДанныеВСбербанк(
				НастройкаОбмена, ИдентификаторСессии, Данные, МассивТикетов, , , ВыводитьПрогрессВыполнения);
...
Процедура ПрочитатьВыпискуСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД)
	...
	ПараметрыСчета.Вставить("КонечныйОстаток", КонечныйОстаток);
	НайденныйТипВДереве = ДеревоЭлектронногоДокументаБЭД.НайтиСоздатьТипОбъектаВДеревеРазбора(ДеревоРазбора, "БанковскийСчет");
	РасчетныйСчет = ОбщегоНазначенияБЭД.НайтиСсылку("БанковскиеСчетаОрганизаций", ЭД.Acc);
...
Процедура ПрочитатьОперациюВыпискиСбербанк(Документ, НовыйЭД)
	...
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитыТЧОбъекта(НовыйЭД, "СтрокаТЧ", РеквизитыДокумента);
...
Процедура ПрочитатьПоручениеНаПереводВалютыСбербанк(ЭД, ДеревоРазбора,
	...
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ИдентификаторДокумента", ЭД.docExtId);

	ДеревоДанных = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента("ОбменСБанками.ПоручениеНаПереводВалюты");
	...
	ДеревоЭлектронногоДокументаБЭД.ДобавитьРеквизитШапкиОбъекта(НовыйЭД, "ДеревоДанных", ДеревоДанных);
...
Процедура ПрочитатьПоручениеНаПокупкуВалютыСбербанк(ЭД, ДеревоРазбора,
	...
	ДеревоДанных = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента("ОбменСБанками.ПоручениеНаПокупкуВалюты");
...
Процедура ПрочитатьПоручениеНаПродажуВалютыСбербанк(ЭД, ДеревоРазбора,
	...
	ДеревоДанных = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента("ОбменСБанками.ПоручениеНаПродажуВалюты");
...
Процедура ПрочитатьРаспоряжениеНаОбязательнуюПродажуВалютыСбербанк(ЭД,
	...
	ДеревоДанных = ДеревоЭлектронногоДокументаБЭД.ДеревоЭлектронногоДокумента("ОбменСБанками.РаспоряжениеНаОбязательнуюПродажуВалюты");
...
Функция ДайджестВыпискиСбербанк(ДеревоРазбора, СтрокаОбъекта)
	...
	НаименованиеОрганизации = ЗначениеРеквизитаСтрокиДереваРазбора(
	...
	Для Каждого СтрокаТЧ Из СтрокиТЧ Цикл
		...
		ШифрДокументаКартотека = ЗначениеРеквизитаСтрокиДереваРазбора(
		...
		СтрокаВозврата = СтрокаВозврата + Символ(10) + "#" + ?(Индекс <> ВсегоЗаписей, Символ(10), "");
	...
	Если ЭД.VoSumInfo <> Неопределено И ЭД.VoSumInfo.VoInfo.Количество() Тогда
		...
		Для Каждого VoInfo Из ЭД.VoSumInfo.VoInfo Цикл
			...
			Если VoInfo.ContractSum <> Неопределено Тогда
				СуммаКонтрактаСтрокой = Формат(VoInfo.ContractSum.docSum, "ЧДЦ=2; ЧРД=.; ЧГ=");
				ДанныеЗаполненияСтроки.Добавить(СуммаКонтрактаСтрокой, "ContractSum.DocSum");
			...
			КонецЕсли;
			ДобавитьЗаписиВДайджест(ДанныеЗаполненияСтроки, СтрокаВозврата);
			СтрокаВозврата = СтрокаВозврата + Символ(10) + "#" + Символ(10);
		КонецЦикла;
...
Функция ДайджестПисьмаСбербанк(НастройкаОбмена, ДвоичныеДанныеФайла)
	...
	Если ЭД.Attachments <> Неопределено
		...
		Для Каждого Attachment Из ЭД.Attachments.Attachment Цикл
			...
			ДанныеЗаполненияСтроки.Добавить(Attachment.AttachmentName, "Attachment.AttachmentName");
			ДанныеЗаполненияСтроки.Добавить(Attachment.Description, "Attachment.Description");
			СтрокаБезПереводаСтрок = СтрЗаменить(Base64Строка(Attachment.Body), Символы.ПС, "");
			СтрокаДвоичныхДанных = СтрЗаменить(СтрокаБезПереводаСтрок, Символы.ВК, "");
...
Процедура ДобавитьЗаписиВДайджест(ДанныеЗаполненияСтроки, СтрокаВозврата)
	...
	Для каждого Элемент Из ДанныеЗаполненияСтроки Цикл
		Если ЗначениеЗаполнено(Элемент.Значение) Тогда
			МассивСтрок.Добавить(Элемент.Представление + "=" + Элемент.Значение);
		КонецЕсли;
	КонецЦикла;
	
	СтрокаВозврата = СтрокаВозврата + СтрСоединить(МассивСтрок, Символ(10));
...
Процедура ОбработатьОшибкиФормированияЭД(Ошибки, ВидЭД, СсылкаНаОбъект = Неопределено)
	...
	Если ЗначениеЗаполнено(Ошибки) Тогда
		...
		ОбработатьОшибку(ВидОперации, ПодробноеОписаниеОшибки, ПодробноеОписаниеОшибки, СсылкаНаОбъект);
	КонецЕсли;
...
Процедура ПрочитатьРеквизитыОрганизацииИБанкаИзJSON(СтрокаВыписки, Организация, Банк)
	...
	РеквизитНаименованиеОрганизации = ИмяПрикладногоРеквизита("ПолноеНаименованиеОрганизации");
	РеквизитИННОрганизации = ИмяПрикладногоРеквизита("ИННОрганизации");
	...
	РеквизитБИКБанка = ИмяПрикладногоРеквизита("Банк.БИК");
	РеквизитНаименованиеБанка = ИмяПрикладногоРеквизита("Банк.Наименование");
...
Процедура ОтметитьИсполненныеПлатежныеДокументы(НастройкаОбмена, ПрограммаБанка, МассивВнешнихИдентификаторов, НомерСчета=Неопределено)
	...
	Запрос = ЗапросНеисполненныхПлатежныхПоручений(
		НастройкаОбмена, ПрограммаБанка, МассивВнешнихИдентификаторов, НомерСчета);
...
Функция ЗапросНеисполненныхПлатежныхПоручений(
	НастройкаОбмена,
	ПрограммаБанка,
	МассивВнешнихИдентификаторов,
	НомерСчета)
...
Процедура ПолучитьПоследниеВыпискиFintech(НастройкаОбмена, ТекущаяСессия, КоличествоПолучено)
	...
	Для Каждого КлючЗначение Из ТекущаяСессия.ДоступныеСчета Цикл
		...
		ОбменСБанкамиФинтехСлужебный.ПолучитьВыписку(НастройкаОбмена, ТокенАутентификацииBase64,
			КлючЗначение.Ключ, ДатаНачала, ДатаОкончания, ДанныеВыписок, ТребуетсяАутентификация);
...
Функция ОтправитьПлатежныеПорученияFintech(НастройкаОбмена, ТекущаяСессия, МассивСообщенийОбмена)
	...
	Для Каждого КлючЗначение Из БанковскиеСчета Цикл
		...
		ОбменСБанкамиФинтехСлужебный.ОтправитьПлатежныйДокумент(
			НастройкаОбмена, ТокенАутентификацииBase64, ДанныеЭД, Результат, ТребуетсяАутентификация);
