...
// Заполняет валюту цены в справочнике виды цен при при первом создании валюты,
//
// Параметры:
//   Валюта - СправочникСсылка.Валюты - ссылка на создававаемую валюту.
//
Процедура ЗаполнитьВалютуЦеныВВидахЦенПриСозданииВалюты(Валюта) Экспорт
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВалют") И Справочники.Валюты.КоличествоВалют() = 1 Тогда
		ВыборкаВидыЦен = Справочники.ВидыЦен.Выбрать();
		Пока ВыборкаВидыЦен.Следующий() Цикл
			Если Не ВыборкаВидыЦен.ПометкаУдаления И Не ЗначениеЗаполнено(ВыборкаВидыЦен.ВалютаЦены) Тогда
				ОбъектВидЦены = ВыборкаВидыЦен.ПолучитьОбъект();
				ОбъектВидЦены.ВалютаЦены = Валюта;
				ОбъектВидЦены.Записать();
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
