...
Функция ТекстЗапросаМаркируемойПродукции(ДокументСсылка,
	...
	Если ЕстьТабличнаяЧастьСерии Тогда
		...
		ИнтеграцияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ТекстЗапросаШаблон, "ОписаниеИС", "ТаблицаТовары.Номенклатура");
	...
	Иначе
		...
		ИнтеграцияИС.ОбновитьТекстЗапросаСРегистромОписаниеНоменклатурыИС(ТекстЗапросаШаблон, "ОписаниеИС", "Товары.Номенклатура");
...
Процедура ЗаполнитьТабличныеЧастиОтгрузкиТоваровИСМП(
	...
	ФильтрВидПродукции = ИнтеграцияИСМПКлиентСерверПовтИсп.УчитываемыеВидыМаркируемойПродукции(Истина, Ложь);
	ОсобенностьУчета   = Новый Массив;
	Для Каждого ВидПродукции Из ФильтрВидПродукции Цикл
		Если ВидПродукции <> Перечисления.ВидыПродукцииИС.Пиво
				И Не ИнтеграцияИСКлиентСервер.ЭтоМолочнаяПродукцияИСМП(ВидПродукции) Тогда
			ОсобенностьУчета.Добавить(ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(ВидПродукции));
		КонецЕсли;
	КонецЦикла;
	Если ЗначениеЗаполнено(ДокументОбъект.ВидПродукции) Тогда
...
Процедура ПроверкаЗаполненияПоВидамУпаковок(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если ЭтоНоменклатура
		И ИнтеграцияИСМПУТКлиентСервер.ИспользуетсяОбщийНаборУпаковок(Объект) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Форма.ИспользуетсяЧастичноеВыбытиеИСМП Тогда
		...
		Если ЗначениеЗаполнено(Форма.УпаковкаЧастичногоВыбытияИСМП) Тогда
			...
			Если Не ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИСМП)
				И Форма.ЭтоМернаяЕдиницаИСМП Тогда
				ПроверяемыеРеквизиты.Добавить("ПотребительскаяУпаковкаИСМП");
			ИначеЕсли Форма.ПотребительскаяУпаковкаИСМП = Форма.УпаковкаЧастичногоВыбытияИСМП Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Упаковка частичного выбытия не может совпадать с потребительской'"),,,
					"УпаковкаЧастичногоВыбытияИСМП",
					Отказ);
		...
		Иначе
			...
			ПроверяемыеРеквизиты.Добавить("УпаковкаЧастичногоВыбытияИСМП");
...
Функция КоэффициентыПоВидамУпаковок(Форма)
	...
	МассивУпаковок.Добавить(Форма.ПотребительскаяУпаковкаИСМП);
	МассивУпаковок.Добавить(Форма.УпаковкаЧастичногоВыбытияИСМП);

	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивУпаковок, "КоличествоУпаковок, Числитель, Знаменатель");
	
	Если ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИСМП)
		И Форма.ПотребительскаяУпаковкаИСМП <> Форма.Объект.ЕдиницаИзмерения Тогда
		РеквизитыПотребительской = ЗначенияРеквизитов.Получить(Форма.ПотребительскаяУпаковкаИСМП);
	...
	Если ЗначениеЗаполнено(Форма.УпаковкаЧастичногоВыбытияИСМП)
		И Форма.УпаковкаЧастичногоВыбытияИСМП <> Форма.Объект.ЕдиницаИзмерения Тогда
		РеквизитыЧастичногоВыбытия = ЗначенияРеквизитов.Получить(Форма.УпаковкаЧастичногоВыбытияИСМП);
		Если ЗначениеЗаполнено(Форма.УпаковкаЧастичногоВыбытияИСМП) Тогда
...
Процедура ЗаполнитьЗначениеПолейПоВидамУпаковок(Форма) Экспорт
	...
	Форма.ПотребительскаяУпаковкаИСМП      = ДанныеНастройкиВидаУпаковки.ПотребительскаяУпаковка;
	Форма.УпаковкаЧастичногоВыбытияИСМП    = ДанныеНастройкиВидаУпаковки.УпаковкаЧастичногоВыбытия;
	Форма.ИспользуетсяЧастичноеВыбытиеИСМП = ДанныеНастройкиВидаУпаковки.ЧастичноеВыбытиеИспользуется;
	
	Если Не ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИСМП)
		И Не ДанныеНастройкиВидаУпаковки.МернаяЕдиница Тогда
		Форма.ПотребительскаяУпаковкаИСМП = Объект.ЕдиницаИзмерения;
	КонецЕсли;
...
Функция ДанныеНастройкиВидаУпаковки(Форма)
	...
	ИсточникДанных              = ИнтеграцияИСМПУТКлиентСервер.ИсточникДанныхПоВидамУпаковок(Форма);
	ЭтоНоменклатура             = ТипЗнч(Объект.Ссылка) = Тип("СправочникСсылка.Номенклатура");
	...
	Если ЗначениеЗаполнено(ИсточникДанных) Тогда
		ДанныеОписания = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(ИсточникДанных)[ИсточникДанных];
...
Процедура ЗаписатьДанныеПоВидамУпаковок(Форма, ТекущийОбъект, ПараметрыЗаписи, Отказ) Экспорт
	...
	ИсточникОписания = ИнтеграцияИСМПУТКлиентСервер.ИсточникДанныхПоВидамУпаковок(ТекущийОбъект);
	...
	Если Не ЗначениеЗаполнено(ДанныеНастройки.ПотребительскаяУпаковка)
		...
		Если (Не ЗначениеЗаполнено(Форма.ПотребительскаяУпаковкаИСМП)
			И Не ЗначениеЗаполнено(Форма.УпаковкаЧастичногоВыбытияИСМП))
			Или (Форма.ПотребительскаяУпаковкаИСМП = ТекущийОбъект.ЕдиницаИзмерения
				И Не Форма.ИспользуетсяЧастичноеВыбытиеИСМП) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		ДанныеОписания.ПотребительскаяУпаковка             = Форма.ПотребительскаяУпаковкаИСМП;
		ДанныеОписания.УпаковкаЧастичногоВыбытия           = Форма.УпаковкаЧастичногоВыбытияИСМП;
		ДанныеОписания.ВариантИспользованияЕдиницыХранения = Перечисления.ВариантыИспользованияЕдиницыХраненияИСМП.ЗаданУпаковками;
		...
		Если Форма.ИспользуетсяЧастичноеВыбытиеИСМП Тогда
			ДанныеОписания.ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.ТекущаяНоменклатура;
			...
			КонецЕсли;
		Иначе
			ДанныеОписания.ВариантЧастичногоВыбытия = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.ПустаяСсылка();
		КонецЕсли;
...
Процедура ДобавитьЭлементыНастройкиВидаУпаковокПриСоздании(Форма) Экспорт
	...
	ГруппаОбщая.Заголовок   = НСтр("ru = 'Маркируемая продукция ГИС МТ'");
	...
	ГруппаГИСМТ = Форма.Элементы.Добавить("ГруппаВидыУпаковокЭлементыИСМП", Тип("ГруппаФормы"), ГруппаОбщая);
	ГруппаГИСМТ.Вид                 = ВидГруппыФормы.ОбычнаяГруппа;
	ГруппаГИСМТ.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаГИСМТ.Отображение         = ОтображениеОбычнойГруппы.Нет;
	ГруппаГИСМТ.ОтображатьЗаголовок = Ложь;
	ГруппаГИСМТ.Заголовок           = НСтр("ru = 'Маркируемая продукция'");
	
	Если ЭтоНоменклатура Тогда
		НовыйЭлемент = Форма.Элементы.Добавить("ТипНоменклатурыИСМП", Тип("ДекорацияФормы"), ГруппаГИСМТ);
	...
	НовыйЭлемент = Форма.Элементы.Добавить("ПотребительскаяУпаковкаИСМП", Тип("ПолеФормы"), ГруппаГИСМТ);
	...
	ГруппаУпаковкаЧастичногоВыбытия = Форма.Элементы.Добавить(
		"ГруппаУпаковкаЧастичногоВыбытияИСМП",
		Тип("ГруппаФормы"),
		ГруппаГИСМТ);
	...
	Если ЭтоНоменклатура Тогда
		...
		ТекстПодсказки = Новый ФорматированнаяСтрока(
			СтрШаблон(
				"%1: %2",
				ИнтеграцияИСМПВызовСервера.ПредставлениеЧастичногоВыбытияПоВидуПродукции(ВидПодукции),
				НСтр("ru = 'не используется, для настройки необходимо использовать упаковки.'")),,
			ЦветаСтиля.ЦветТекстаНеТребуетВниманияГосИС);
		
		НовыйЭлемент = Форма.Элементы.Добавить("НадписьЧастичноеВыбытиеИСМП", Тип("ДекорацияФормы"), ГруппаГИСМТ);
		НовыйЭлемент.АвтомаксимальнаяШирина = Ложь;
	...
	Если ЭтоНоменклатура Тогда
		...
		ИмяКоманды = "КомандаЗаполнитьУпаковкуЧастичногоВыбытияИСМП";
...
Процедура ЗаполнитьПредставлениеТипаНоменклатуры(Форма) Экспорт
	...
	Элементы.ТипНоменклатурыИСМП.Заголовок = Новый ФорматированнаяСтрока(МассивТекстов);
...
Процедура ИспользованиеЭлементов(ВидимостьЭлементов, Объект, РежимВидимостиПоказатьТолькоВажные, РежимПроверкиИспользованияРеквизитов) Экспорт
	...
	Если Не РежимПроверкиИспользованияРеквизитов Тогда
		...
		ВидПродукции             = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(Объект.ОсобенностьУчета);
		ЭтоПродукцияИСМП         = ИнтеграцияИСПовтИсп.ЭтоПродукцияИСМП(ВидПродукции, Истина);
		ДоступноЧастичноеВыбытие = ИнтеграцияИСКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(ВидПродукции);
		ИспользутсяУпаковки      = (ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры") И Объект.ИспользоватьУпаковки);
		
		ВидимостьЭлементов.Вставить("ТипНоменклатурыИСМП",              ЭтоПродукцияИСМП);
		ВидимостьЭлементов.Вставить("ПотребительскаяУпаковкаИСМП",      ЭтоПродукцияИСМП);
		ВидимостьЭлементов.Вставить("ИспользуетсяЧастичноеВыбытиеИСМП", (ДоступноЧастичноеВыбытие И ИспользутсяУпаковки));
		ВидимостьЭлементов.Вставить("УпаковкаЧастичногоВыбытияИСМП",    (ДоступноЧастичноеВыбытие И ИспользутсяУпаковки));
		ВидимостьЭлементов.Вставить("НадписьЧастичноеВыбытиеИСМП",      (ДоступноЧастичноеВыбытие И Не ИспользутсяУпаковки));
...
Процедура ЗаголовкиЭлементов(ЗаголовкиЭлементов, Объект, ВидимостьЭлементов) Экспорт
	...
	ВидПродукции = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(Объект.ОсобенностьУчета);
	Если ВидимостьЭлементов["ИспользуетсяЧастичноеВыбытиеИСМП"] Тогда
