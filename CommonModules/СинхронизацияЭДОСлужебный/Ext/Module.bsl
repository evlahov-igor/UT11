...
Функция ПараметрыСинхронизации(ПараметрыПолучения) Экспорт
	...
	УдалитьСертификатыНедействительныхУЦ();
...
Процедура УдалитьСертификатыНедействительныхУЦ() Экспорт
	
	Если ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ОбменСКонтрагентами_EF_00_00427661", "СертификатыУдалены", Ложь) Тогда
		Возврат;
	КонецЕсли;
	...
	Пока ВыборкаДетальныеЗаписи.СледующийПоЗначениюПоля("Сертификат") Цикл
		Сертификаты.Добавить(ВыборкаДетальныеЗаписи.Сертификат);
	КонецЦикла;
	
	ВыборкаДетальныеЗаписи.Сбросить();
	
	СвойстваИздателейСертификатов = СвойстваИздателейСертификатовПоСсылке(Сертификаты);
	
	Для каждого КлючИЗначение Из СвойстваИздателейСертификатов Цикл
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(КлючИЗначение.Значение, "ИНН", "") <> "007720623379" Тогда
			Продолжить;
		КонецЕсли;
		
		Пока ВыборкаДетальныеЗаписи.НайтиСледующий(КлючИЗначение.Ключ, "Сертификат") Цикл
			
			НаборЗаписей = РегистрыСведений.СертификатыУчетныхЗаписейЭДО.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.ИдентификаторЭДО.Установить(ВыборкаДетальныеЗаписи.ИдентификаторЭДО);
			НаборЗаписей.Отбор.Сертификат.Установить(ВыборкаДетальныеЗаписи.Сертификат);
			
			Попытка
				НаборЗаписей.Записать();
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Удаление недействительных сертификатов ЭДО'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ОбменСКонтрагентами_EF_00_00427661", "СертификатыУдалены", Истина);
	
КонецПроцедуры

Функция СвойстваИздателейСертификатовПоСсылке(Сертификаты)
	...
	Для Каждого КлючИЗначение Из КриптографияБЭД.СвойстваСертификатов(Сертификаты) Цикл
		...
		СвойстваИздателя = ЭлектроннаяПодпись.СвойстваИздателяСертификата(СертификатКриптографии);
		Результат.Вставить(КлючИЗначение.Ключ, СвойстваИздателя);
	КонецЦикла;
			...
			Для Каждого ИдентификаторПакета Из ИдентификаторыПакетов Цикл
				...
				Попытка
					ПолучитьДокументы(ТранспортныеКонтейнерыПакета, КонтекстДиагностики,
						ИстекСрокДействияМаркера, Результат, РезультатыОтправкиПолучения, ПараметрыПолучения, Истина);
...
Процедура ПолучитьДокументы(ЗагружаемыеДокументы, КонтекстДиагностики, ИстекСрокДействияМаркера,
	Результат, РезультатыОтправкиПолучения, ПараметрыПолучения, ВызыватьИсключение = Ложь)
