...
// << 03.11.2023 Петухов А.В., Фактор, #3226
	// Для исключения обратной отправки
	Если ПараметрыФайла.Свойство(КР_УТ10_УТ11_ЗагрузкаДанныхСервер.СвойствоОбъектБылЗагружен()) Тогда
		ПрисоединенныйФайл.ДополнительныеСвойства.Вставить(КР_УТ10_УТ11_ЗагрузкаДанныхСервер.СвойствоОбъектБылЗагружен());
	КонецЕсли;
// >> 03.11.2023 Петухов А.В., Фактор, #3226
	МенеджерФайла = РаботаСФайламиСлужебный.МенеджерФайлов(ПрисоединенныйФайл);
	
	// << 08.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2136
	// В Перед обновлением, если хранение в томах, то происходит запись данных в РС Двоичные данные
	// Нужно все делать в транзакции
	...
	Попытка
		...
		// << 08.10.2023 Марченко С.Н., КРОК, JIRA№A2105505-2136  
		// Для интеграции ЦБ-Магазин важно чтоб в выгружаемых РС не было битых ссылок
		// Запишем ПрисоединенныйФайл в режиме загрузка=Истина 
		Если ПрисоединенныйФайл.ЭтоНовый() Тогда 
			ПредыдущийРежимЗагрузки = ПрисоединенныйФайл.ОбменДанными.Загрузка;   
			ПрисоединенныйФайл.ОбменДанными.Загрузка = Истина;
			ПрисоединенныйФайл.Записать(); 
			ПрисоединенныйФайл.ОбменДанными.Загрузка = ПредыдущийРежимЗагрузки;
		КонецЕсли;	
