...
// 		*** КоличествоПотребительскихУпаковок - Строка - Идентификатор продукции.
...
Функция ЗапроситьСтатусыКодовМаркировки(ДанныеДляЗапроса, Организация = Неопределено) Экспорт
	...
	НастройкиРазбора = РазборКодаМаркировкиИССлужебный.НастройкиРазбораКодаМаркировки(
	...
	Для Каждого ПакетКодовМаркировки Из ПакетыКодовМаркировки Цикл
		...
		РезультатЗапросаСтатусовКодовМаркировок = ЗапроситьСтатусыКодовМаркировкиПакетно(
			ПакетКодовМаркировки, СтатусыКодовМаркировки, Организация,, НастройкиРазбора);
...
Функция ЗапроситьСтатусыКодовМаркировкиПакетно(МассивИсходныхСтрок, СтатусыКодовМаркировкиКеш = Неопределено, Организация = Неопределено, ПолеИсточник = Неопределено, НастройкиРазбора = Неопределено) Экспорт
	...
	Для Каждого ЭлементМассива Из МассивИсходныхСтрок Цикл
		...
		Если ПараметрыЗапроса = Неопределено Тогда
			...
			Если СтрокаТаблицы.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				Для Каждого СтрокаДанных Из ДанныеОбработки Цикл
					...
					СтрокаПродукции.Вставить("КоличествоПотребительскихУпаковок", СтрокаДанных.innerUnitCount);
...
Функция ДобавлятьУпаковкуВДерево(СтрокаКодаМаркировки, Детализация, ДанныеВерхнегоУровня)
	...
	Если СтрокаКодаМаркировки.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
		...
		ИначеЕсли Детализация = Перечисления.ДетализацияСтруктурыХраненияИС.КоробаСГрупповымиУпаковками Тогда
			...
			Если ДанныеВерхнегоУровня.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
				Добавлять = Ложь;
			КонецЕсли;
