...
Функция СформироватьПечатнуюФормуКО2(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Кассир = ЗНАЧЕНИЕ(Справочник.ОтветственныеЛицаОрганизаций.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.Автор.ФизическоеЛицо
	|		ИНАЧЕ ДанныеДокумента.Кассир.ФизическоеЛицо
	|	КОНЕЦ КАК Кассир,
	...
	Пока Выборка.Следующий() Цикл
		...
		//	Истина);
		// Конец было
		ОбластьМакета.Параметры.НомерДокумента = Выборка.Номер;
...
Процедура ЗаполнитьВидыФискальныхЧековПоДокументу(ВидыЧеков, Операция, ИмяКомандыПробитияЧека) Экспорт
	...
	Если Операция = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
		ИЛИ Операция = Перечисления.ХозяйственныеОперации.ПрочиеРасходы Тогда
