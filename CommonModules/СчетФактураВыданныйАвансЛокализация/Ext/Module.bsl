...
Процедура ОбработкаЗаполнения(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	|	ЕСТЬNULL(ИсторияКППКонтрагентов.КПП, """")                                КАК КПП,
...
//	Булево
//
Функция ЭтоРеализацияВПутиПоРасчетнымДокументам(ОбъектРасчетов) Экспорт
	
	Значение = Ложь;
	//++ Локализация
	Если ЗначениеЗаполнено(ОбъектРасчетов)
		И ТипЗнч(ОбъектРасчетов.Объект) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ОбъектРасчетов.Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности
		И ОбъектРасчетов.Объект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным Тогда
		Значение = Истина;
	КонецЕсли;
	//-- Локализация
	Возврат Значение;
