...
Процедура ВывестиИзображения(Изображения, ОбластьКартинка, ТабличныйДокумент, ДанныеИзображений, ОтносительныеАдресаКартинок)
	...
	ДанныеИзображенийПоСсылкам(Изображения, ДанныеИзображений, ОтносительныеАдресаКартинок);
...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|	ПрайсЛисты.ОбновлятьОстаткиИЦены КАК ОбновлятьОстаткиИЦены,
...
// * Характеристика - ОпределяемыйТип.ХарактеристикаНоменклатурыБЭД - характиристика товара торгового предложения.
...
Процедура ОбработатьТорговыеПредложения(ПрайсЛист, ЧастичнаяПубликация, КоличествоОпубликованных, Отказ)
	...
	Для Каждого СтрокаТовары Из ПрайсЛист.Товары Цикл
		...
		Если ПрайсЛист.ОбновлятьОстаткиИЦены Тогда
...
Процедура ЗаполнитьСкидкиЗаОбъемПродаж(ПрайсЛист, ДанныеПредложения)
	...
	Если ВариантПубликацииСкидкиЗаОпт = Перечисления.ВариантыПубликацииСкидок.ВидыСкидок Тогда
		СкидкиЗаОбъемПродаж = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПрайсЛист, "СкидкиЗаОбъемПродаж");
	ИначеЕсли ВариантПубликацииСкидкиЗаОпт = Перечисления.ВариантыПубликацииСкидок.ВидыЦен Тогда
		СкидкиЗаОбъемПродаж = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПрайсЛист, "СкидкиЗаОбъемПродажВидыЦен");
	КонецЕсли;
...
Процедура ЗаполнитьСкидкиПроцентСумма(ПрайсЛист, ДанныеПредложения)
	...
	ИначеЕсли ВариантПубликацииСкидки = Перечисления.ВариантыПубликацииСкидок.ВидыЦен Тогда
		СкидкиПроцентСумма = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПрайсЛист, "СкидкиПроцентСуммаВидыЦен");
	КонецЕсли;
...
// Получает двоичные данные картинок по массиву ссылок.
// 
// Параметры:
//  Изображения - Массив - Изображения
//  ДанныеИзображений - СписокЗначений, Массив - Данные изображений
//  ОтносительныеАдресаКартинок - Булево - Относительные адреса картинок
//
Процедура ДанныеИзображенийПоСсылкам(Знач Изображения, ДанныеИзображений, Знач ОтносительныеАдресаКартинок) Экспорт
...
Функция НовыйПараметрыСоединения()
	...
	ПараметрыСоединения.Вставить("Протокол");
	ПараметрыСоединения.Вставить("ИмяСервера");
	ПараметрыСоединения.Вставить("АдресРесурса");
	ПараметрыСоединения.Вставить("Порт");
	ПараметрыСоединения.Вставить("Таймаут");
	ПараметрыСоединения.Вставить("ЗащищенноеСоединение");
	ПараметрыСоединения.Вставить("Прокси");
...
Процедура ПроверитьЗаписиТорговыеПредложенияТорговойПлощадки(Знач Отбор, Знач АдресРезультата = Неопределено) Экспорт
	...
	// Собираем текст запроса
	СведенияОТорговомПредложении = ПолучитьДанныеПубликации("ТоварыСлужебный, ПрайсЛистыСлужебный, Контакты, 
	|	СвойстваСлужебный, Штрихкоды, Контрагенты, Изображения, СкидкиЗаОбъемПродажВидыЦен, СкидкиПроцентСуммаВидыЦен", 
		Отбор);
	...
	РезультатЗапросаСкидкиЗаОбъемПродажВидыЦен = СведенияОТорговомПредложении.СкидкиЗаОбъемПродажВидыЦен;
	РезультатЗапросаСкидкиПроцентСуммаВидыЦен = СведенияОТорговомПредложении.СкидкиПроцентСуммаВидыЦен;
	...
	ТаблицаСкидкиЗаОбъемПродажВидыЦен = РезультатЗапросаСкидкиЗаОбъемПродажВидыЦен.Выгрузить();
	ТаблицаСкидкиЗаОбъемПродажВидыЦен.Индексы.Добавить("Номенклатура");
	
	ТаблицаСкидкиПроцентСуммаВидыЦен = РезультатЗапросаСкидкиПроцентСуммаВидыЦен.Выгрузить();
	ТаблицаСкидкиПроцентСуммаВидыЦен.Индексы.Добавить("Номенклатура");
	...
	Для каждого СтрокаТаблицаТоваров Из ТаблицаТоваров Цикл
		...
		ДанныеПозиции.Вставить("ВыборкаПрайсЛисты", ВыборкаПрайсЛисты);
		ДанныеПозиции.Вставить("СтрокаТаблицаТоваров", СтрокаТаблицаТоваров);
		ДанныеПозиции.Вставить("ТаблицаСвойств", ТаблицаСвойств);
		ДанныеПозиции.Вставить("ТаблицаШтрихкодов", ТаблицаШтрихкодов);
		ДанныеПозиции.Вставить("ТаблицаИзображения", ТаблицаИзображения);
		ДанныеПозиции.Вставить("ТаблицаСкидкиЗаОбъемПродажВидыЦен", ТаблицаСкидкиЗаОбъемПродажВидыЦен);
		ДанныеПозиции.Вставить("ТаблицаСкидкиПроцентСуммаВидыЦен", ТаблицаСкидкиПроцентСуммаВидыЦен);
		ДанныеПозиции.Вставить("ХешНовый", Неопределено);
		
		ДанныеПозицииТорговогоПредложенияИзменены = ДанныеПозицииТорговогоПредложенияИзменены(ДанныеПозиции);
...
Функция ДанныеПозицииТорговогоПредложенияИзменены(ДанныеПозиции)
	...
	ВыборкаПрайсЛисты = ДанныеПозиции.ВыборкаПрайсЛисты;
	...
	// Скидки за объем продаж ВидыЦен
	Если ВыборкаПрайсЛисты.ПубликоватьСкидкиЗаРазовыйОбъемПродаж Тогда
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Номенклатура", СтрокаТаблицаТоваров.Номенклатура);
		СтрокиСкидки = ДанныеПозиции.ТаблицаСкидкиЗаОбъемПродажВидыЦен.НайтиСтроки(СтруктураПоиска);
		Для Каждого СтрокаСкидки Из СтрокиСкидки Цикл
			НоваяСтрока = ДанныеПредложения.СкидкиЗаОбъемПродаж.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСкидки);
		КонецЦикла;
	...
	Если ВыборкаПрайсЛисты.ПубликоватьЦенуДоСкидки Тогда
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Номенклатура", СтрокаТаблицаТоваров.Номенклатура);
		СтрокиСкидки = ДанныеПозиции.ТаблицаСкидкиПроцентСуммаВидыЦен.НайтиСтроки(СтруктураПоиска);
		Для Каждого СтрокаСкидки Из СтрокиСкидки Цикл
			НоваяСтрока = ДанныеПредложения.СкидкиПроцентСумма.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСкидки);
		КонецЦикла;
	...
	Если ПустаяСтрока(ДанныеПредложения.ИдентификаторНоменклатурыСервиса) Тогда
		...
		ЗапросСервиса = ЗапросСервисаСозданиеОбновлениеНоменклатурыПоставщика(
			ВыборкаПрайсЛисты.Организация, ДанныеНоменклатуры, Неопределено, Отказ);
...
Функция ДанныеПрайсЛиста(ПрайсЛист, КонтактныеДанные, Контрагенты)
	...
	Если КонтактныеДанные <> Неопределено Тогда
		...
		Если ПустаяСтрока(Результат.ЭлектроннаяПочта) Тогда
