			...
			Если НЕ РасчетСебестоимостиПовтИсп.ВозможенРасчетСебестоимости(ПериодРасчета) Тогда
				...
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='В периоде %1 учет себестоимости не ведется.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
//	Запрос - Запрос - запрос для установки параметров
//	Реквизиты - Произвольный - реквизиты документа для определения параметров партионного учета
// 	Периоды - Массив - периоды, в которых документ формирует движения;
...
Процедура ЗаполнитьПараметрыИнициализацииПоПериодам(Запрос, Реквизиты, Периоды = Неопределено, ИмяТаблицыПериодов = "") Экспорт
	...
	Если Не ТипыКолонок.Свойство("Период") Тогда
		ТипыКолонок.Вставить("Период", "Дата");
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметровЗапросаДляПроведения(Реквизиты, Период)
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Реквизиты, "Организация")
	 И ЗначениеЗаполнено(Реквизиты.Организация) Тогда
		...
		СтруктураПараметров.Вставить("ДетализироватьДополнительныеРасходыВСебестоимостиТоваров",
			(ПараметрыУчетнойПолитики <> Неопределено
				И ПараметрыУчетнойПолитики.ДетализироватьДополнительныеРасходыВСебестоимостиТоваров));
	...
	Иначе
		...
		СтруктураПараметров.Вставить("ДетализироватьДополнительныеРасходыВСебестоимостиТоваров", Ложь);
...
Процедура ЗаполнитьРасчетнуюПартию(ПараметрыРасчета, Контекст, РасчетнаяПартия, Расход, Приход) Экспорт
	...
	Если НЕ ПартияЗаполнена Тогда
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для контекста %1 не предусмотрено заполнение полей расчетной партии'", ОбщегоНазначения.КодОсновногоЯзыка()),
			Контекст);
...
Процедура ДобавитьОписаниеИсточника(ОписаниеЦепочек, Приемник, Источник, ПоляИсточника) Экспорт
	...
	Если ПоляПриемника.Количество() <> ПоляСвязи.Количество() Тогда
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Различное количество полей связи источника ""%1"" и приемника ""%2"".'", ОбщегоНазначения.КодОсновногоЯзыка()),
			Источник,
			Приемник);
...
Процедура ПостроитьЦепочкиДвижений(ПараметрыРасчета, ОптимизацияДанных = Ложь) Экспорт
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, "Источники", ?(ОптимизацияДанных, НСтр("ru='После оптимизации'", ОбщегоНазначения.КодОсновногоЯзыка()), ""));
	...
	Если НЕ ОптимизацияДанных Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			ОписаниеПроблемы = НСтр("ru='В выбранных данных есть типы записей, отсутствующие в описании цепочек.'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Пока Выборка.Следующий() Цикл
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип записи: ""%1"", источник: ""%2"", организация ""%3"", регистратор: ""%4""'", ОбщегоНазначения.КодОсновногоЯзыка()),
				...
				ЗарегистрироватьПроблемуВыполненияРасчета(
					ПараметрыРасчета,
					Выборка.Организация,
					НСтр("ru='При формировании движений по партиям диагностированы ошибки'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Для Каждого ТекущийТипЗаписи Из ТипыЗаписейПриемников Цикл
			...
			Если РасчетСебестоимостиПовтИсп.ЭтоНепересчитываемыйТипЗаписи(ТекущийТипЗаписи) Тогда
				...
				ТекстДляПротокола = НСтр("ru='В описании цепочек в качестве приемников не могут использоваться непересчитываемые типы записей'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Пока Выборка.Следующий() Цикл
		...
		РасчетСебестоимостиУниверсальныеАлгоритмы.ДополнитьСтроку(
			...
			+ ?(ЗначениеЗаполнено(Выборка.ТипЗаписи),
				...
				"<"	+ НСтр("ru='Тип записи не указан'", ОбщегоНазначения.КодОсновногоЯзыка()) + ">")
	...
	Если КоличествоДанных > 0 Тогда
		...
		ТекстОписания = НСтр("ru='Описание структуры данных для расчета%1'", ОбщегоНазначения.КодОсновногоЯзыка()) + ":" + Символы.ПС
			+ НСтр("ru='Всего данных: %2%3'", ОбщегоНазначения.КодОсновногоЯзыка()) + Символы.ПС
			+ НСтр("ru='Всего связей: %4%5'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		ТекстОписания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОписания,
			?(ОптимизацияДанных, " (" + НСтр("ru='после оптимизации'", ОбщегоНазначения.КодОсновногоЯзыка()) + ")", ""),
			СокрЛП(КоличествоДанных),
			?(КоличествоДанных > 0, ", " + НСтр("ru='в том числе'", ОбщегоНазначения.КодОсновногоЯзыка()) + ":" + Символы.ПС + ТекстДляПротоколаДанные, ""),
			СокрЛП(КоличествоСвязей),
			?(КоличествоСвязей > 0, ", " + НСтр("ru='в том числе'", ОбщегоНазначения.КодОсновногоЯзыка()) + ":" + Символы.ПС + ТекстДляПротоколаСвязи, ""));
...
Процедура ОптимизироватьДанныеВЦепочках(ПараметрыРасчета)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(
		ПараметрыРасчета,
		"Данные",
		НСтр("ru = 'Оптимизация исходных данных'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|	размер таблиц Источники и Приемники (до/после) - %4 / %5'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ОптимизироватьНумерациюВЦепочках(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.Отладка.НеВыполнятьОптимизациюЦепочек Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			ПараметрыРасчета,
			НСтр("ru='Оптимизация нумерации узлов графа перед расчетом партий по цепочкам пропущена для целей отладки.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		
		Возврат;
		
	КонецЕсли;
	
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(ПараметрыРасчета, "ВТНовыеНомераУзлов", НСтр("ru='Оптимизация нумерации исходных данных'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|	выполнено итераций поиска подграфов - %09'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура СформироватьУжеИзвестныеРасчетныеПартии(ПараметрыРасчета, ОптимизацииДанныхНеБудет)
	...
	|	всего %1, в т.ч. не участвующих в дальнейшем расчете %2'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Если ТолькоУдалениеУзловБезСвязей Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос,,,,
					НСтр("ru='(перенумерация таблиц связей)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если НЕ ПараметрыРасчета.ИнформационнаяБазаФайловая
	 ...
	 И ПараметрыРасчета.РаспределениеПартий.КоличествоСтрокДанных > ПараметрыРасчета.ОграниченияВыборки.ПорцияРасчетаПартий Тогда
		...
		РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(
			ПараметрыРасчета,
			"ОписаниеПодграфов",
			НСтр("ru='(разделение на порции для фоновых заданий)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Пока СледующийЭлементКоллекции(ВыборкаГруппаПодграфов) Цикл
		...
		Пока Данные.ПараметрыОбхода.ИндексСтроки < ВыборкаГруппаПодграфов.МаксимальныйНомерУзла Цикл
			...
			Если НЕ СледующийЭлементКоллекции(ВыборкаДанных) ИЛИ ВыборкаДанных.К <> Данные.ПараметрыОбхода.ИндексСтроки Тогда
				...
				ТекстДляПротокола = НСтр("ru='Ошибка нумерации данных для расчета партий'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция РассчитатьПартиюРекурсивно(ПараметрыРасчета, Данные, ИндексРасхода)
	...
	Если ТребуетсяСортировка Тогда
		СортироватьИсточникиПоФИФО_ЛИФО(Источники, ОписаниеДвижений.ПолеПорядка, Данные.Приходы, Данные.Расходы, ЭтоСторно И НЕ ЭтоИсправление);
		СортироватьИсточникиПоЗначениямПолей(Источники, ЗначенияПолей, Данные.Приходы, Данные.Расходы, Расход.Регистратор, ЭтоСторно И Не ЭтоИсправление, ЕстьИсправления, ЭтоСторно);
	КонецЕсли;
	...
	Пока Счетчик <= ВГраница Цикл
		...
		Если ЭтоСторно И НЕ ЭтоИсправление Тогда
...
Процедура ЗапуститьРасчетПартийВФоне(ПараметрыРасчета, ПараметрыФоновогоРасчета,
	...
	Пока СледующийЭлементКоллекции(ВыборкаГруппаПодграфов) Цикл
		...
		ПоместитьТаблицуЗначенийВоВременнуюТаблицу(Запрос, "ВТОставшиесяУзлы", ТаблицаУзлы, "%1Ключ, %1КоличествоСвязей");
		...
		Пока ПродолжатьВыгрузку Цикл
			...
			ОписаниеЗамера.КэшированиеУзлов_Время = 
				ЗаписатьКэшДанныхРасчетаПартийСЗамеромВремени(
					ПараметрыРасчета,
					МинимальныйНомерЗаданияРасчета,
					ИдентификаторРасчета,
			...
			Если ТаблицаКэша.Количество() > 0 Тогда
				...
				ОписаниеЗамера.КэшированиеСвязей_Время = 
					ЗаписатьКэшДанныхРасчетаПартийСЗамеромВремени(
						ПараметрыРасчета,
						МинимальныйНомерЗаданияРасчета,
						ИдентификаторРасчета,
	...
	КлючЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='ФЗ №%1 - %2 %3 (%4)'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СокрЛП(МинимальныйНомерЗаданияРасчета - 1),
		НСтр("ru='Фоновый расчет партий'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	НовоеЗадание = ФоновыеЗадания.Выполнить(
		...
		КлючЗадания,
		НСтр("ru='Партионный учет версии 2.2: расчет порции данных.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ТекстНачало = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Запуск фонового расчета партий №%1: %2, в т.ч.
			...
			|	* подготовка данных графов: %8 (%9)'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СокрЛП(МинимальныйНомерЗаданияРасчета - 1),
	...
	ТекстПродолжение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='
			...
			|	* кэширование параметров расчета: %9'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Пока ПараметрыРасчета.ТаблицаЗаданийРасчета.Количество() > 0 Цикл
		...
		Для Каждого СтрокаОписанияЗадания Из ПараметрыРасчета.ТаблицаЗаданийРасчета Цикл
			...
			Если Задание = Неопределено Тогда
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Фоновое задание %1 не найдено. Вероятно задание завершено аварийно, см. Журнал регистрации.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			...
			ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно
			 ИЛИ Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					"""" + СокрЛП(Задание.Ключ) + """",
					?(Задание.Состояние = СостояниеФоновогоЗадания.Отменено, НСтр("ru='отменено'", ОбщегоНазначения.КодОсновногоЯзыка()), НСтр("ru='завершено аварийно'", ОбщегоНазначения.КодОсновногоЯзыка())),
...
Процедура ЗаписатьКэшДанныхРасчетаПартий(ИдентификаторРасчета, ТипДанных, Данные,
	...
	МенеджерЗаписи.Комментарий			 = НСтр("ru='Расчет партий'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПрочитатьКэшДанныхРасчетаПартий(ПараметрыРасчета, ИдентификаторРасчета, ТипДанных, НомерПорцииДанных = 0,
	...
	Если НЕ МенеджерЗаписи.Выбран() Тогда
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|Тип данных: %2
			|Номер порции: %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
			СокрЛП(ИдентификаторРасчета),
			СокрЛП(ТипДанных),
...
Процедура ЗавершитьЗамерРаспределенияПартий(ПараметрыРасчета)
	...
	ОписаниеРасчета = НСтр("ru='Описание результатов обхода цепочек (графа):'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ШаблонОписаниеГруппыПодграфов = "
		|	" + НСтр("ru='%1. Подграфов: %2, узлов: %3, глубина обхода: %4, сформировано записей: %5, в т.ч. не рассчитано %6.
		|	%7
		|	Результаты расчета цепочек: %8'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Для Каждого ТекущаяСтрока Из ПараметрыРасчета.РаспределениеПартий.СтатистикаРасчета Цикл
		...
		ОписаниеВремени = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Начало расчета: %1, длительность расчета: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ОписаниеРасчета = ОписаниеРасчета
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				ОписаниеВремени,
				?(ОписаниеРезультатов = "", НСтр("ru='не выполнялся (нет цепочек)'", ОбщегоНазначения.КодОсновногоЯзыка()), ОписаниеРезультатов));
		...
		Если НЕ ПараметрыРасчета.РаспределениеПартий.ЕстьПереполнениеСтека Тогда
			...
			ТекстДляПротокола =
				НСтр("ru='При расчете цепочек движения партий сработало ограничение на максимально допустимую длину цепочки.
					...
					|Для некоторых документов партии могут быть не подобраны или может быть нарушен принцип подбора партий по ФИФО.'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			ЗарегистрироватьПроблемуВыполненияРасчета(
				ПараметрыРасчета,
				,
				НСтр("ru='При расчете партий диагностированы проблемы'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ТекстДляПротокола,
				,
...
Процедура ИнициализироватьСвойстваЗапроса(Запрос, ПараметрыРасчета, ИспользоватьОбщийМенеджерВТ = Истина) Экспорт
	...
	// Для WE параметр "ОрганизацииСУчетомПартийНДСВерсии24" заполняется по данным учетной политики финансового учета.
	Если Не Запрос.Параметры.Свойство("ОрганизацииСУчетомПартийНДСВерсии24") Тогда
		Запрос.УстановитьПараметр("ОрганизацииСУчетомПартийНДСВерсии22", Новый Массив);
		Запрос.УстановитьПараметр("ОрганизацииСУчетомПартийНДСВерсии24", ПараметрыРасчета.ОрганизацииСДетализациейДополнительныхРасходов);
	КонецЕсли;
...
Функция ИнициализироватьОбщиеПараметрыРасчета(ПараметрыИнициализации, ПараметрыРасчета, ПараметрыОтладки = Неопределено) Экспорт
	...
	ПараметрыРасчета.Вставить("МестоВызоваРасчета",
		?(ПараметрыИнициализации.Свойство("МестоВызоваРасчета"),
			ПараметрыИнициализации.МестоВызоваРасчета, НСтр("ru='Не указано'", ОбщегоНазначения.КодОсновногоЯзыка()))); // откуда вызвали расчет (для протокола)
	...
	Для Каждого КлючИЗначение Из Новый Структура(ИменаФО) Цикл
		...
		Иначе
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найдена функциональная опция или константа ""%1""'", ОбщегоНазначения.КодОсновногоЯзыка()),
				КлючИЗначение.Ключ);
			...
			ЗарегистрироватьПроблемуВыполненияРасчета(
				...
				НСтр("ru='При запуске расчета диагностированы ошибки'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ТекстДляПротокола);
	...
	ПараметрыРасчета.Вставить("ОрганизацииСДетализациейДополнительныхРасходов", Новый Массив);
	...
	|	ЕСТЬNULL(ВТФинУчет.ДетализироватьДополнительныеРасходыВСебестоимостиТоваров,
	|		ЛОЖЬ) КАК ДетализироватьДополнительныеРасходыВСебестоимостиТоваров
	|ИЗ
	...
	Для Каждого ПараметрыУчетнойПолитики Из УчетныеПолитики Цикл
		...
		Если ПараметрыУчетнойПолитики.ДетализироватьДополнительныеРасходыВСебестоимостиТоваров Тогда
			ПараметрыРасчета.ОрганизацииСДетализациейДополнительныхРасходов.Добавить(ПараметрыУчетнойПолитики.Организация);
		КонецЕсли;
...
Процедура ПроверитьКорректностьОстатковВРегистреСебестоимостьТоваровПослеРасчета(ПараметрыРасчета)
	...
	Для Каждого ТекущаяПроверка Из ВыполняемыеПроверки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Проверка) Тогда
			...
			ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найдена проверка ""%1"".'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ТекущаяПроверка.Ключ);
...
//
Процедура ПроверитьКорректностьСформированныхДвижений(ПараметрыРасчета)
	...
	Для Каждого КлючИЗначение Из ПараметрыРасчета.Движения Цикл
		...
		РасчетСебестоимостиПротоколРасчета.ЗафиксироватьОшибкиВСформированныхДвижениях(
			...
			НСтр("ru='Сформированы движения с некорректной организацией'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если ПараметрыРасчета.ПартионныйУчетВерсии22
	 ...
	 И ПараметрыРасчета.Движения.Свойство(Метаданные.РегистрыНакопления.СебестоимостьТоваров.Имя) Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ЗафиксироватьОшибкиВСформированныхДвижениях(
			...
			НСтр("ru='Сформированы движения по непересчитываемому типу записи'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		РасчетСебестоимостиПротоколРасчета.ЗафиксироватьОшибкиВСформированныхДвижениях(
			...
			НСтр("ru='Изменилось количество в первичных движениях'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		РасчетСебестоимостиПротоколРасчета.ЗафиксироватьОшибкиВСформированныхДвижениях(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Изменился реквизит ""%1"" в движениях'", ОбщегоНазначения.КодОсновногоЯзыка()),
				"ДокументИсточник"));
		//-- Локализация
...
Функция ИнициализироватьКорректировкуОстатковРегистра(ПараметрыРасчета, ИмяРегистра) Экспорт
	...
	Если РегистрыСКорректировкамиНачальныхОстатков().Найти(ИмяРегистра) = Неопределено Тогда
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Регистр ""%1"" должен быть указан в %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ИмяРегистра,
...
// Параметры:
//	ПараметрыРасчета - Структура - параметры расчета
//	ПараметрыКорректировки - Структура - см. РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьКорректировкуОстатковРегистра()
//
Процедура СформироватьТаблицуРасчетныхНачальныхОстатковРегистра(ПараметрыРасчета, ПараметрыКорректировки)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(ПараметрыРасчета,
		ПараметрыКорректировки.ИмяТаблицыОстатков, НСтр("ru='Вспомогательная'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОчиститьКорректировкиРегистровВСледующихПериодах(ПараметрыРасчета, ПараметрыКорректировки)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			ПараметрыРасчета,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Очищено движений корректировки по регистру ""%1"": %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОчиститьУстаревшиеКорректировкиРегистров(ПараметрыРасчета, ПараметрыКорректировки)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			ПараметрыРасчета,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Очищено движений устаревших корректировки по регистру ""%1"": %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ПараметрыКорректировки.ПредставлениеРегистра,
				РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(Выборка.Количество())));
...
Процедура ИнициализироватьОписаниеОрганизацийДляВыполненияКорректировки(ПараметрыРасчета, ПараметрыКорректировки) Экспорт
	...
	Если ОписаниеКорректировки = Неопределено Тогда
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдено описание корректировки ""%1"" в %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Иначе // есть РодительскаяКорректировка
			...
			ИначеЕсли ОписаниеРодительскойКорректировки.ВозможноМногократноеВыполнение
			 ИЛИ НЕ ЗначениеЗаполнено(ОписаниеРодительскойКорректировки.ИмяКонстантыДатыНачалаДействия) Тогда
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Некорректное описание корректировки ""%1"" в %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
					ОписаниеКорректировки.РодительскаяКорректировка,
					"РасчетСебестоимостиПрикладныеАлгоритмы.ОписаниеКорректировокНачальныхОстатков");
...
Процедура НачалоЗаписиДвижений(ПараметрыРасчета)
	...
	ВыборкаЗаписываемыхРегистров = ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос,,,, НСтр("ru='Упорядочивание таблиц для записи'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Пока ВыборкаЗаписываемыхРегистров.Следующий() Цикл
		...
		Если ОписаниеРегистра.ПодчиненРегистратору И КоличествоДокументов > 0 Тогда
			...
			Иначе
				...
				ПараметрыЗаписиРегистра = ИнициализироватьПараметрыЗаписи(
					ПараметрыРасчета.РасчетныйПериод.НачалоПериода,
					ПараметрыРасчета.НомерЗаданияДоРасчета,
					Истина);
				...
				Если НЕ РезультатЗапроса.Пустой() Тогда
					...
					КэшироватьПорциюДанныхДляЗаписи(ПараметрыРасчета, ПараметрыЗаписиРегистра, РезультатЗапроса, Истина);
				...
				Для НомерПорции = 1 По МаксНомерПорции Цикл
					...
					Если РазмерВременнойТаблицы(Запрос, "ВТРегистраторыТекущейПорции") = 1
					 И КоличествоДвижений > ПараметрыРасчета.ОграниченияФЗ.КоличествоДвиженийВФЗЗаписи Тогда
						...
						Для НомерНабора = 1 По КоличествоНаборов Цикл
							...
							КэшироватьПорциюДанныхДляЗаписи(ПараметрыРасчета, ПараметрыЗаписиРегистра, ТаблицаДвижений,, НомерНабора > 1);
					...
					КэшироватьПорциюДанныхДляЗаписи(ПараметрыРасчета, ПараметрыЗаписиРегистра, ТаблицаДвижений);
				...
				РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
					ПараметрыРасчета,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Время формирования таблиц ""%1"" регистра ""%2"": %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
						"ВТДвиженияТекущейПорции",
						ОписаниеРегистра.ИмяРегистра,
				...
				РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
					ПараметрыРасчета,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Время выгрузки результатов запроса в таблицы значений, регистр ""%1"": %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
						ОписаниеРегистра.ИмяРегистра,
...
Процедура ЗавершениеЗаписиДвижений(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.Отладка.РегистраторыВФЗ[0].Количество > 1 Тогда
		...
		ТекстДляПротокола =	НСтр("ru = 'Некорректно выполнено разделение движений на порции для многопоточной записи. Движения могут быть некорректны.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
//	Структура -
//
Функция ИнициализироватьПараметрыЗаписи(ПериодРасчета, НомерЗаданияДоРасчета = -1, ВФоновомЗадании = Ложь, ДополнитьНаборЗаписей = Ложь) Экспорт
...
Процедура ЗаписатьДвиженияПоРегиструФоновымЗаданием(ПараметрыЗаписиРегистра) Экспорт
	...
	ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТекстСтатистика = НСтр("ru='Начало'") + ": " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DT");
	ТекстОшибки = "";
	...
	Для НомерПорции = ПараметрыЗаписиРегистра.НомерПервойПорции По ПараметрыЗаписиРегистра.КоличествоПорций Цикл
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(Неопределено, НСтр("ru='Запись движений'", ОбщегоНазначения.КодОсновногоЯзыка())),
			...
			Если НЕ ЗаписьРегистра.Выбран() Тогда
				...
				ТекстОшибки = ТекстОшибки + Символы.ПС +
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдена порция данных №%1'"), НомерПорции);
	...
	ТекстСтатистика = ТекстСтатистика + "; "
		+ НСтр("ru='окончание'") + ": " + Формат(ТекущаяДатаСеанса(), "ДЛФ=DT") + "; "
		+ НСтр("ru='длительность'") + ": " + РасчетСебестоимостиПротоколРасчета.ПредставлениеВремени((ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала) / 1000) + "; "
		+ НСтр("ru='порций'") + ": " + СокрЛП(ПараметрыЗаписиРегистра.КоличествоПорций - ПараметрыЗаписиРегистра.НомерПервойПорции + 1) + "; "
		+ НСтр("ru='наборов'") + ": " + СокрЛП(ПараметрыЗаписиРегистра.КоличествоНаборов) + "; "
		+ НСтр("ru='записей'") + ": " + СокрЛП(ПараметрыЗаписиРегистра.КоличествоЗаписей);
	...
	ЗаписьРегистра.Комментарий			 = НСтр("ru='Запись движений'", ОбщегоНазначения.КодОсновногоЯзыка()) + " - " + ПараметрыЗаписиРегистра.ИмяРегистра;
...
Процедура ЗаписатьПорциюДвиженийНезависимогоРегистра(ИсходныеДанные, МенеджерРегистра, ПараметрыЗаписи) Экспорт
	...
	ИмяПоляНоменклатура = ?(ПериодическийРегистр, "АналитикаУчетаНоменклатуры", "Номенклатура");
	...
	Пока ПолучитьСледующуюПорциюИзИсточникаДанных(ИсточникДанных, ПорцияДанныхИсточника) Цикл
		...
		Если НомерНабора <> ПорцияДанныхИсточника.НомерНабора Тогда
			...
			НаборЗаписей.Отбор[ИмяПоляНоменклатура].Установить(ПорцияДанныхИсточника[ИмяПоляНоменклатура]);
...
Функция РегистраторИзмененПриРасчете(Регистратор, НаборЗаписей, ОписаниеОшибокЗаписи, ПараметрыЗаписи)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Во время расчета изменены движения документа ""%1"" по регистру накопления ""%2""'", ОбщегоНазначения.КодОсновногоЯзыка()),
			СокрЛП(Регистратор),
			НаборЗаписей.Метаданные().Имя);
		...
		ЗарегистрироватьОшибкуЗаписиДвижений(
			Регистратор,
			ПараметрыЗаписи,
			НСтр("ru='Во время расчета были внесены изменения в рассчитываемый документ'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстДляПротокола,
			ОписаниеОшибокЗаписи,
			Выборка.Организация);
...
Функция ЗаписатьНезависимыйРегистрСведений(ПараметрыРасчета, ОписаниеРегистра, ИменаТаблицДляЗаписи, ЗаписьКорректировочныхДвижений)
	...
	Иначе
		...
		ОбъединитьВременныеТаблицы(
			...
			"Организация, АналитикаУчетаНоменклатуры",
			Истина);
	...
	Если НЕ ОписаниеРегистра.ПериодическийРегистр Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Т.АналитикаУчетаНоменклатуры,",
			"Т.Номенклатура,");
	...
	ПараметрыНумерации = СформироватьПараметрыНумерацииСтрокВременнойТаблицы(
		"",
		"",
		?(ОписаниеРегистра.ПериодическийРегистр,
			"Период, Организация, АналитикаУчетаНоменклатуры, ТипЗаписи",
			"Период, Организация, Номенклатура, ТипЗаписи"),
		"НомерНабора");
	...
	ПараметрыЗаписи.Вставить("ИмяРегистра", 		  ОписаниеРегистра.ИмяРегистра);
	ПараметрыЗаписи.Вставить("МенеджерРегистра", 	  ОписаниеРегистра.МенеджерРегистра);
	ПараметрыЗаписи.Вставить("ИдентификаторРасчета",  Новый УникальныйИдентификатор);
	ПараметрыЗаписи.Вставить("НомерПервойПорции", 	  1);
	ПараметрыЗаписи.Вставить("КоличествоПорций", 	  0);
	ПараметрыЗаписи.Вставить("ПорцииОчистки", 		  Новый Массив); // номера порций с данными для очистки движений
	ПараметрыЗаписи.Вставить("ПорцииДополнения", 	  Новый Массив); // номер порций с движениями "больших" наборов, для записи с параметром Замещать = Ложь
	ПараметрыЗаписи.Вставить("ВремяПодготовкиДанных", 0);
	ПараметрыЗаписи.Вставить("КоличествоЗаписей", 	  0);
	ПараметрыЗаписи.Вставить("ЭтоНезависимыйРегистр", Истина);
	ПараметрыЗаписи.Вставить("НеВызыватьИсключениеПриОшибке", Истина);
	...
	ИмяПоляНоменклатура = ?(ОписаниеРегистра.ПериодическийРегистр, "АналитикаУчетаНоменклатуры", "Номенклатура");
	
	Пока Выборка.Следующий() Цикл
		
		НаборЗаписей.Отбор[ИмяПоляПериод].Установить(Выборка.Период);
		НаборЗаписей.Отбор.Организация.Установить(Выборка.Организация);
		НаборЗаписей.Отбор[ИмяПоляНоменклатура].Установить(Выборка[ИмяПоляНоменклатура]);
	...
	Если НЕ ЗаписьКорректировочныхДвижений И КоличествоФЗ > 1 Тогда
		...
		Если НЕ ОписаниеРегистра.ПериодическийРегистр Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "Т.АналитикаУчетаНоменклатуры = Отбор.АналитикаУчетаНоменклатуры",
				"Т.Номенклатура = Отбор.Номенклатура");
		КонецЕсли;
		...
		ПараметрыЗаписиРегистра = Неопределено;
		
		Для НомерПорции = 1 По МаксНомерПорции Цикл
			
			Если Цел((НомерПорции-1) / КоличествоПорцийВЗадании) = (НомерПорции-1) / КоличествоПорцийВЗадании Тогда
				
				// Формируем новую порцию записи.
				Если МассивПараметровЗаписи.Количество() > 0 Тогда
					
					// Ожидаем возможности запустить новое задание.
					ЕстьМестоВСтекеЗаданий = ОжидатьЗавершенияЗаданийЗаписи(ПараметрыРасчета, Ложь, Ложь);
					
					Если ЕстьМестоВСтекеЗаданий Тогда
						// Если есть место в стеке - запустим запись порции сразу, если нет - продолжим формирование порций чтобы не тратить время на ожидание
						ЗапуститьЗаданиеЗаписиРегистра(ПараметрыРасчета, МассивПараметровЗаписи[0], ЗаписьКорректировочныхДвижений);
						МассивПараметровЗаписи.Удалить(0);
					КонецЕсли;
				...
				ПараметрыЗаписиРегистра.Вставить("ВремяПодготовкиДанных", 0);
				ПараметрыЗаписиРегистра.Вставить("КоличествоЗаписей", 	  0);
				ПараметрыЗаписиРегистра.Вставить("ЭтоНезависимыйРегистр", Истина);
				
				МассивПараметровЗаписи.Добавить(ПараметрыЗаписиРегистра);
			...
			КэшироватьПорциюДанныхДляЗаписи(ПараметрыРасчета, ПараметрыЗаписиРегистра, РезультатЗапроса);
		...
		Пока МассивПараметровЗаписи.Количество() > 0 Цикл
			
			ОжидатьЗавершенияЗаданийЗаписи(ПараметрыРасчета, Ложь);
...
Процедура СформироватьТаблицуСтарыхДокументов(ПараметрыРасчета, ОписаниеРегистра, Запрос, ДляОчисткиДвижений = Ложь)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(
		...
		НСтр("ru='Регистр'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": " + ОписаниеРегистра.ИмяРегистра);
...
Процедура СформироватьТаблицуИзмененныхДокументов(ПараметрыРасчета, ОписаниеРегистра, Запрос)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(
		...
		НСтр("ru='Регистр'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": " + ОписаниеРегистра.ИмяРегистра,
		"ВТКэшСтарыеДокументы, " + ОписаниеРегистра.ИмяТаблицыКэшаРегистра);
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Т.Регистратор КАК Регистратор
	|ПОМЕСТИТЬ ВТМоментыВремениРегистраторов
	|ИЗ
	...
	СформироватьТаблицуМоментовВремени(
		ПараметрыРасчета,
		"ВТМоментыВремениРегистраторов",
		,
		"ВТКэшИзмененныеДокументы");
		
	УничтожитьВременныеТаблицы(ПараметрыРасчета, "ВТДокументыРБП, ВТМоментыВремениРегистраторов");
...
Процедура СформироватьТаблицуОчищаемыхДокументов(ПараметрыРасчета, ОписаниеРегистра, Запрос)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(
		...
		НСтр("ru='Регистр'", ОбщегоНазначения.КодОсновногоЯзыка()) + ": "  + ОписаниеРегистра.ИмяРегистра,
		"ВТКэшИзмененныеДокументы, " + ОписаниеРегистра.ИмяТаблицыКэшаРегистра + ", ВТКэшДвиженияДругихПериодовИлиОрганизаций");
...
//
Процедура ЗапуститьЗаданиеЗаписиРегистра(ПараметрыРасчета, ПараметрыЗаписиРегистра, ЗаписьКорректировочныхДвижений = Ложь, ЗаписьДвиженийВнеРасчета = Ложь) Экспорт
	...
	ОписаниеЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1, запись регистра %2.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура КэшироватьПорциюДанныхДляЗаписи(ПараметрыРасчета, ПараметрыЗаписиРегистра, Данные, ЭтоПорцияОчистки = Ложь, ЭтоПорцияДополнения = Ложь)
	...
	Если НЕ ЭтоПорцияОчистки Тогда
		...
		// Проверим корректность формирования порций записи.
		Если ПараметрыРасчета.Отладка.КонтролироватьРегистраторыВФЗ И НЕ ЭтоПорцияДополнения Тогда
	 ...
	 ИЛИ ПараметрыРасчета.СтекЗаданийЗаписи.Количество() >= МаксимальноеКоличествоЗаданийЗаписи(ПараметрыРасчета) Тогда
		...
		Пока НомерЗадания < ПараметрыРасчета.СтекЗаданийЗаписи.Количество() Цикл
			...
			Если НЕ ЗавершениеВсехЗаданий И НЕ ЗаписьДвиженийВнеРасчета Тогда
				...
				ЗаданиеЗавершено = ОжидатьЗавершенияФоновогоЗаданияЗаписи(ПараметрыРасчета, НомерЗадания, 0.5);
				
				Если ЗаданиеЗавершено ИЛИ ПараметрыРасчета.СтекЗаданийЗаписи.Количество() = 0 Тогда
					Возврат Истина; // освободилось место в стеке заданий
				КонецЕсли;
				...
				Если НомерЗадания >= ПараметрыРасчета.СтекЗаданийЗаписи.Количество() Тогда
					...
					Если НЕ ОжидатьОсвобожденияМестаВСтеке Тогда
						Возврат Ложь; // проверили - стек все еще заполнен
					КонецЕсли;
...
Функция ОжидатьЗавершенияФоновогоЗаданияЗаписи(ПараметрыРасчета, НомерЗадания, Таймаут = Неопределено, ЗаписьДвиженийВнеРасчета = Ложь)
	...
	Если СтрокаЗадания <> Неопределено Тогда
		...
		Если НЕ ЗаписьРегистра.Выбран() Тогда
			...
			Результат = Новый Структура;
			Результат.Вставить("ТекстСтатистика", НСтр("ru = 'Задание завершилось с ошибками (см. ниже)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Иначе
		...
		Результат.Вставить("ТекстОшибки", НСтр("ru = 'Информация о задании записи отсутствует'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПолучитьФоновоеЗаданиеЗаписи(ПараметрыРасчета, НомерЗадания)
	...
	Если Задание = Неопределено Тогда
		...
		Если СтрокаЗадания = Неопределено Тогда
			...
			КлючЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='ФЗ №? (%1)'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ИнициализироватьПараметрыОтраженияВУчете(ПараметрыРасчета, ПредставлениеВидаУчета, ИмяМетода,
			ИмяВременнойТаблицы, ДопустимаТаблицаЗначений = Истина) Экспорт
...
Процедура ОжидатьЗавершенияФоновогоЗаданияОтраженияВУчете(ПараметрыРасчета, НомерЗадания, ПараметрыОтраженияВУчете)
	...
	Если ЗначениеЗаполнено(ТекстДляПротокола)Тогда
		...
		ЗарегистрироватьПроблемуВыполненияРасчета(
			...
			,
			ПараметрыОтраженияВУчете.ПредставлениеВидаУчета + ": "
				+ НСтр("ru = 'обнаружены ошибки'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстДляПротокола);
...
Процедура ПерепровестиДокументыПоОтдельнымРегистрамНакопления(ПараметрыРасчета, Запрос, РасшифровкаКодовОшибок, ПричинаПерепроведения, 
		...
		РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Обнаружены некорректные движения по регистру накопления ""%1"".
					|	  Причина: %2
					|	  Перепроведено документов: %3'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Если НЕ РезультатПроведения.ВыполненоУспешно Тогда
		...
		РасчетСебестоимостиПротоколРасчета.ЗафиксироватьОшибкуРасчета(
			...
			РасчетСебестоимостиПротоколРасчета.СлужебныйСимвол_БезНумерации()
				+ НСтр("ru='После перепроведения остались документы с некорректными движениями:'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Для Каждого РасшифровкаОшибок Из РезультатПроведения.РасшифровкаОшибок Цикл
		...
		РасчетСебестоимостиПротоколРасчета.ЗафиксироватьОшибкуРасчета(
			ПараметрыРасчета,
			Перечисления.ТипыОшибокПартионногоУчетаИСебестоимости.ОшибкаВосстановленияДвиженийДокументов,
			"#- " + НСтр("ru='ошибка'", ОбщегоНазначения.КодОсновногоЯзыка())  + " """ + РасшифровкаОшибок.ТипОшибки + """:");
	...
	РасчетСебестоимостиПротоколРасчета.ДополнительнаяИнформация(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружены некорректные движения документов.
				|	  Причина: %1
				|	  Перепроведено документов: %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ПричинаПерепроведения,
...
Процедура ИнициализироватьДанныеРегистра(ПараметрыРасчета, МетаданныеРегистра) Экспорт
	...
	Если ПараметрыРасчета.Движения.Свойство(МетаРегистр.Имя) Тогда
		...
		ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка инициализации расчета: регистр %1 уже инициализирован'", ОбщегоНазначения.КодОсновногоЯзыка()),
			МетаРегистр.Имя);
...
#Область ИменаВременныхТаблицРегистра
	...
	Иначе
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для регистра ""%1"" не заданы имена служебных временных таблиц'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ОписаниеРегистра.ПолноеИмяРегистра);
				...
				Для Каждого ДанныеДвижения Из НекорректныеСтроки Цикл
					...
					Если НекорректныеРегистраторы.Получить(ДанныеДвижения.Регистратор) = Неопределено
					 И ОписаниеПоля.Ключ <> "Регистратор" Тогда
						...
						ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'По документу ""%1"" сформированы движения по регистру ""%2"" с незаполненными обязательными полями'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Если ОкончаниеЭтапа Тогда
			...
			Если ВременнаяТаблицаСуществует(ПараметрыРасчета, ИмяПромежуточнойТаблицы) Тогда
				...
				КэшироватьСформированныеДвиженияИзВременныхТаблиц(
					...
					Истина,
					Ложь,
					НЕ ПараметрыРасчета.ЗаполняютсяПартииВСебестоимости);
...
Процедура КэшироватьСформированныеДвиженияИзВременныхТаблиц(ПараметрыРасчета, СоответствиеВременныхТаблицДвижений,
	...
	РасчетСебестоимостиПротоколРасчета.НачалоВыполненияФрагментаКода(
		ПараметрыРасчета,
		"КэшироватьСформированныеДвиженияИзВременныхТаблиц",
		НСтр("ru = 'Кэширование движений из временных таблиц'", ОбщегоНазначения.КодОсновногоЯзыка()));
			...
			Если ОписаниеРегистра.ЭтоОписаниеРегистра И ОписаниеРегистра.ПодчиненРегистратору Тогда
				...
				Пока ДанныеДвижения.Следующий() Цикл
					...
					Если ОписаниеРегистра.ПодчиненРегистратору И НЕ ЗначениеЗаполнено(ДанныеДвижения.Регистратор) Тогда
						...
						ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Не заполнено свойство %1 для движения по регистру ""%2"" (период %3, организация ""%4"")'", ОбщегоНазначения.КодОсновногоЯзыка()),
							"ДокументДвижения",
							ТаблицаПриемник,
					...
					ИначеЕсли ОписаниеРегистра.ПодчиненРегистратору Тогда
						...
						ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Документ ""%1"" не может иметь движений по регистру ""%2"" (период %3, организация ""%4"")'", ОбщегоНазначения.КодОсновногоЯзыка()),
							СокрЛП(ДанныеДвижения.Регистратор),
							ТаблицаПриемник,
							РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ПараметрыРасчета),
							ОрганизацияСПроблемой);
...
Процедура ОбновитьРасчетныеОборотыРегистра(ПараметрыРасчета, ОписаниеРегистра, Инициализация = Ложь) Экспорт
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(ПараметрыРасчета,
		ОписаниеРегистра.ИмяТаблицыРасчетныхОборотовРегистра, НСтр("ru='Вспомогательная'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОбновитьРасчетныеОстаткиРегистра(ПараметрыРасчета, ОписаниеРегистра, Инициализация = Ложь) Экспорт
	...
	РасчетСебестоимостиПротоколРасчета.НачалоФормированиеВременнойТаблицы(ПараметрыРасчета,
		ОписаниеРегистра.ИмяТаблицыРасчетныхОстатковРегистра, НСтр("ru='Вспомогательная'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ВернутьСостояниеИтоговРегистровФоновымЗаданием(СостояниеИтоговДоРасчета) Экспорт
	...
	ФоновыеЗадания.Выполнить(
		...
		ПараметрыОбработчикаЗадания,
		,
		НСтр("ru='Расчет партий и себестоимости, включение итогов.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//	
//	Возвращаемое значение:
//		Массив, РезультатЗапроса, ВыборкаИзРезультатаЗапроса -
...
Функция ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, УвеличиватьСчетчикЗамераПроизводительности = Ложь,
	...
	Если ВестиПротоколВыполненияЗапросаВЖурналеРегистрации Тогда
		...
		ТекстДляПротокола = НСтр("ru = 'Размеры существующих временных таблиц'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если НЕ СхемаРазобрана Тогда
		...
		Если НЕ ЗначениеЗаполнено(Пояснение) И ИмяТаблицы = "Данные" Тогда
			ПояснениеКТаблице = НСтр("ru = 'Формирование'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
		...
		Если ВестиПротоколВыполненияЗапросаВЖурналеРегистрации Тогда
			...
			ТекстОписаниеТаблицы = НСтр("ru = 'Текст запроса'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + ИмяТаблицы + ":" + Символы.ПС + Запрос.Текст;
	...
	Для Каждого ЗапросВыбора Из СхемаЗапроса.ПакетЗапросов Цикл
		...
		ИмяПодзапроса = НСтр("ru = 'Выборка'", ОбщегоНазначения.КодОсновногоЯзыка()) + ?(КоличествоВыборок = 1, "", "_" + СокрЛП(КоличествоВыборок));
		...
		Если ВедетсяПротоколРасчета Тогда
			...
			Если ВестиПротоколВыполненияЗапросаВЖурналеРегистрации Тогда
				...
				ТекстОписаниеТаблицы = НСтр("ru = 'Текст запроса'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
...
Процедура ПоместитьТаблицуЗначенийВоВременнуюТаблицу(КонтейнерМенеджераВТ, ИмяПриемника, Таблица, ПоляТаблицы, ПоляИндексирования = "") Экспорт
	...
	ВыполнитьЗапросСЗамеромДляПротокола(
		...
		Запрос,,,,
		НСтр("ru = 'Создание временной таблицы из таблицы значений'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОбъединитьВременныеТаблицы(КонтейнерМенеджераВТ, ИменаИсточников, ИмяПриемника, ПоляТаблицы, РесурсыТаблицы, ПоляИндексирования = "", УдалятьИсточники = Ложь) Экспорт
	...
	Для НомерЗапроса = 1 По КоличествоЗапросов Цикл
		...
		ВыполнитьЗапросСЗамеромДляПротокола(
			...
			Запрос,,
			ИменаТаблицДляПротокола,,
			НСтр("ru = 'Объединение временных таблиц'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ДобавитьНедостающиеКолонкиВременнойТаблицы(КонтейнерМенеджераВТ, ИмяТаблицы, ИмяОбразца, ПоляИндексирования = "") Экспорт
	...
	ВыполнитьЗапросСЗамеромДляПротокола(
		КонтейнерМенеджераВТ,
		Запрос,,,,
		НСтр("ru = 'Добавление колонок временной таблицы'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура СкопироватьВременнуюТаблицу(КонтейнерМенеджераВТ, ИмяИсточника, ИмяПриемника, ПоляИндексирования = "") Экспорт
	...
	ВыполнитьЗапросСЗамеромДляПротокола(
		КонтейнерМенеджераВТ,
		Запрос,,,,
		НСтр("ru = 'Копирование временной таблицы'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ЗаполнитьНомераСтрокВременнойТаблицы(ПараметрыРасчета, ПараметрыНумерации, ИмяТаблицы, ИмяПронумерованнойТаблицы = Неопределено, Пояснение = "") Экспорт
	...
	РасчетСебестоимостиПротоколРасчета.НачалоВыполненияФрагментаКода(
		ПараметрыРасчета,
		ИмяТаблицы,
		НСтр("ru = 'Нумерация строк временной таблицы'", ОбщегоНазначения.КодОсновногоЯзыка())
			+ ?(Пояснение = "", "", " (" + Пояснение + ")"));
...
Функция РазделитьВременнуюТаблицуНаПорции(ПараметрыРасчета, ПараметрыРазделения, ИмяТаблицы, Пояснение = "") Экспорт
	...
	РасчетСебестоимостиПротоколРасчета.НачалоВыполненияФрагментаКода(
		ПараметрыРасчета,
		ИмяТаблицы,
		НСтр("ru = 'Деление временной таблицы на порции'", ОбщегоНазначения.КодОсновногоЯзыка())
			+ ?(Пояснение = "", "", " (" + Пояснение + ")"));
...
Процедура СформироватьТаблицуМоментовВремени(КонтейнерМенеджераВТ, ИмяИсточника, ИмяПоляРегистратор = "Регистратор",
	...
	Пока Выборка.Следующий() Цикл
			...
			Если НЕ ДопустимыПустыеРегистраторы Тогда
				...
				ТекстДляПротокола = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При обработке регистраторов из временной таблицы ""%1"" возникла ошибка:
						|некорректный тип регистратора ""%2""'", ОбщегоНазначения.КодОсновногоЯзыка()),
					ИмяИсточника,
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|") + СтрЗаменить(ШаблонЗапроса, "ПОМЕСТИТЬ ИмяПриемника", ?(ЭтоПервыйПодзапрос, "ПОМЕСТИТЬ ИмяПриемника", ""));
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "КорректировкаРегистров", ИмяТипаРегистратора);
...
//	Строка -
//
Функция ПолучитьИменаКолонокСтрокой(ИсточникКолонок, ИсключаемыеКолонки = "") Экспорт
...
Функция ИмяСобытияЖурналаРегистрации(ПараметрыРасчета, Уточнение) Экспорт
	...
	Если НЕ ПараметрыРасчета = Неопределено
	 ...
	 И ПараметрыРасчета.ЗапущенРасчетПартий Тогда
		ИмяСобытия = НСтр("ru = 'Расчет партий и себестоимости%1'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		ИмяСобытия = НСтр("ru = 'Расчет себестоимости%1'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ИмяСобытияОшибкиДляЖурналаРегистрации(ПараметрыРасчета = Неопределено) Экспорт
	...
	Возврат ИмяСобытияЖурналаРегистрации(ПараметрыРасчета, НСтр("ru='Ошибка'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ИмяСобытияПротоколДляЖурналаРегистрации(ПараметрыРасчета = Неопределено) Экспорт
	...
	Возврат ИмяСобытияЖурналаРегистрации(ПараметрыРасчета, НСтр("ru='Протокол расчета'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ИмяСобытияДопИнформацииДляЖурналаРегистрации(ПараметрыРасчета = Неопределено) Экспорт
	...
	Возврат ИмяСобытияЖурналаРегистрации(ПараметрыРасчета, НСтр("ru='Описание расчета'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ИмяСобытияПротоколВыполненияЗапросаДляЖурналаРегистрации(ПараметрыРасчета = Неопределено) Экспорт
	...
	Возврат ИмяСобытияЖурналаРегистрации(ПараметрыРасчета,
		НСтр("ru = 'Протокол выполнения запросов'", ОбщегоНазначения.КодОсновногоЯзыка())  + "." + ПараметрыРасчета.ПредставлениеЭтапа);
...
Функция ТекстОшибкиНекорректногоЗавершенияРасчетаСебестоимости() Экспорт
	Возврат СлужебныйСимвол_ПроблемаУжеЗарегистрирована()
		+ НСтр("ru='При расчете себестоимости возникла ошибка. Подробности см. в Журнале регистрации.'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции
...
Функция ПредставлениеОрганизаций(МассивОрганизаций, Разделитель = Неопределено, ТекстДляЗаписиВИБ = Ложь) Экспорт
	...
	Если МассивОрганизаций = Неопределено Тогда
		Если ТекстДляЗаписиВИБ Тогда
			Возврат НСтр("ru='<Все организации>'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
	...
	Если ЕстьФИФОВзвешеннаяОценка И ЕстьДругиеМетодыОценки Тогда // проверка для партионного учета версии 2.2
		...
		ТекстОшибки = 
			НСтр("ru = 'Недопустимое сочетание методов оценки стоимости запасов для организаций, работающих по схеме Интеркампани:
						...
						|Настройка метода оценки стоимости выполняется в учетной политике организаций.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ИнициализироватьДокументыРасчетаСебестоимости(ПараметрыРасчета, ПредварительныйРасчет, РегламентноеЗадание)
		...
		Если НЕ ВозможенРасчет Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Расчет предварительной себестоимости за %1 по %2 регламентным заданием не выполнялся,
					|т.к. уже выполнен расчет фактической себестоимости'", ОбщегоНазначения.КодОсновногоЯзыка()),
				РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ПараметрыРасчета),
	...
	Попытка
		...
		Для Каждого КлючИЗначение Из ДокументыРасчетаПоОрганизациям Цикл
			...
			Если НЕ ЗначениеЗаполнено(ПараметрыРасчета.РежимЗакрытияМесяца) Тогда
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность исходных данных для расчета себестоимости'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Проверяются соблюдение следующих условий:
			...
			|- должны быть оформлены поступления работ в подразделения.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Проверка превышения остатков товаров в пути'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Поступление товаров в пути не должно превышать остатки этих товаров.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Соответствие остатков по количеству в оперативном учете и учете себестоимости'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='В оперативном учете и учете себестоимости остатки по количеству должны быть идентичны.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Соответствие остатков сумм и остатков по количеству в регистре ""Себестоимость товаров""'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		|- разных знаков у остатков по суммам (упр. и регл.) и остатка по количеству.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Наличие остатков затрат, не распределенных на себестоимость.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='В регистре ""Партии прочих расходов"" не должно быть остатков по статьям расходов с вариантом распределения расходов ""На себестоимость товаров"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ОписаниеПроверки.ВозможноИзменениеВажности = Истина;
	 ...
	 Шаблон1 = НСтр("ru='В регистрах, обслуживаемых механизмом партионного учета, для всех расчетных движений должна быть подобрана партия.
			...
			|Универсального способа исправления данной проблемы нет - в каждом случае надо анализировать причины в контексте конкретной информационной базы.'", ОбщегоНазначения.КодОсновногоЯзыка());

	 Шаблон1 = СтрЗаменить(Шаблон1, "%Парам1%", "РасчетЗавершен");
	 Шаблон1 = СтрЗаменить(Шаблон1, "%Парам2%", "РасчетНеЗавершен");
	 
	 ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Наличие движений, для которых не удалось подобрать партии.'", ОбщегоНазначения.КодОсновногоЯзыка()), Шаблон1);
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Наличие остатков партий с отрицательным количеством.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='В регистре себестоимости не должно быть отрицательных остатков в разрезе партий.
			...
			|анализируются ""детальные"" остатки в разрезе всех измерений регистра себестоимости.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Наличие аналитик с нерассчитанной стоимостью.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Стоимость некоторых аналитик номенклатуры не может быть рассчитана при решении СЛУ. 
			|Возможные причины: превышение расходов над приходами и начальными остатками или зацикливание движений.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаКорректностиИсходныхДанныхДляРасчетаСебестоимости(ПараметрыПроверки) Экспорт
	...
	|		ГДЕ
	|			ЗаданияКРасчету.Месяц < &КонецПериода
	|			И ЗаданияКРасчету.Организация = ДД.Организация)
	|	
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ 
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru='Обнаружены отрицательные остатки по количеству в регистре себестоимости по организации ""%1"" на конец периода %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru = 'Не отражены поступления работ в подразделения организации ""%1"" за период %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru = 'Фактуровки превышают неотфактурованные поставки в организации ""%1"" за период %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
...
Процедура ПроверкаСоответствияСуммИКоличества(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Количество",					НСтр("ru='Количество'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ПолнаяСтоимость",				НСтр("ru='Полная стоимость'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ПолнаяСтоимостьБезНДС",		НСтр("ru='Полная стоимость без НДС'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ПолнаяСтоимостьРегл",			НСтр("ru='Полная стоимость (регл.)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ПолнаяСтоимостьУпр",			НСтр("ru='Полная стоимость (упр.)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТОстаткиБезКоличества",
		НСтр("ru='Обнаружены ненулевые остатки по суммам при нулевом остатке по количеству в регистре себестоимости по организации ""%1"" на конец периода %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru='Обнаружены разные знаки у остатков по суммам и остатка по количеству в регистре себестоимости по организации ""%1"" на конец периода %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
...
Процедура ПроверкаПоступленийТоваровВПути(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Поступления из товаров в пути превышают остатки товаров в пути в организации ""%1"" за период %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
...
Процедура ПроверкаЗатратНеРаспределенныхНаСебестоимость(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Сумма",						НСтр("ru='Сумма'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаБезНДС",					НСтр("ru='Сумма без НДС'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("СуммаРегл",					НСтр("ru='Сумма (регл.)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТОстаткиНераспределенныхРасходов",
		НСтр("ru='Обнаружены не распределенные на себестоимость остатки расходов по организации ""%1"" на конец периода %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"", // ИмяПоляОбъекта
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru='Обнаружены остатки расходов по организации ""%1"" на конец периода %2 по аналитикам, в которых указаны непроведенные документы'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"", // ИмяПоляОбъекта
		Метаданные.РегистрыНакопления.ПрочиеРасходы.ПолноеИмя());
...
Процедура ПроверкаЗаписейСПризнакомРасчетНеЗавершен(ПараметрыПроверки) Экспорт
	...
	СписокПолей = Новый СписокЗначений;
	СписокПолей.Добавить("Организация", 		НСтр("ru = 'Организация'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ИмяРегистра", 		НСтр("ru = 'Имя регистра'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("Регистратор", 		НСтр("ru = 'Документ'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("КоличествоДвижений", 	НСтр("ru = 'Количество движений'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТДокументыСНезаполненнойПартией",
		НСтр("ru = 'Обнаружены неподобранные партии в движениях документов организации ""%1"" за период %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Регистратор");
...
Процедура ПроверкаРезультатаРасчетаПоКоличеству(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТОтрицательныеОстатки",
		НСтр("ru='Обнаружены отрицательные остатки партий в регистре себестоимости по организации ""%1"" на конец периода %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
...
Процедура ПроверкаСоответствияОперативногоУчетаИСебестоимости(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Организация", 				НСтр("ru='Организация'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры",	НСтр("ru='Аналитика номенклатуры'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("КоличествоСебестоимость",		НСтр("ru='Количество (учет себестоимости)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("КоличествоОперативное",		НСтр("ru='Количество (оперативный учет)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВТРасхожденияСебестоимостиИТоваровОрганизаций",
		НСтр("ru='Обнаружены расхождения по количеству между остатками регистра себестоимости и остатками оперативных регистров по организации ""%1"" на конец периода %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
...
Процедура ПроверкаАналитикСПереполнениемПолей(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Партия",						НСтр("ru='Партия расходов'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидДеятельностиНДС",			НСтр("ru='Вид деятельности НДС'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		"ВтАналитикиСПереполнениемПолей",
		НСтр("ru='Стоимость некоторых аналитик номенклатуры не может быть рассчитана при решении СЛУ по организации ""%1"" за период %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		,
		Метаданные.РегистрыНакопления.СебестоимостьТоваров.ПолноеИмя());
...
Процедура ЗарегистрироватьПроблемуВыполненияРасчета(ПараметрыРасчета, МассивОрганизаций = Неопределено,
	...
	ПолныйТекстПроблемы = ТекстПроблемы
		+ ?(ОрганизацииДляПротокола.Количество() = 1, " " + НСтр("ru='по организации ""%1"" за период ""%2""'", ОбщегоНазначения.КодОсновногоЯзыка()), "");
...
Функция ВыполнитьОперацииЗакрытияМесяцаДляПодготовкиКРасчетуЭтапа(СтрокаСхемыРасчета, ИдентификаторРасчета) Экспорт
	...
	АктивныеРасчеты = РегистрыСведений.ВыполнениеОперацийЗакрытияМесяца.УстановитьПризнакЗапускаРасчета(
		...
		ИдентификаторРасчета,
		НСтр("ru='Выполняется актуализация данных партионного учета при формировании отчета'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ОписаниеГруппПараметровОперацииЗакрытияМесяца() Экспорт
	...
	ОписаниеГруппы.Наименование = НСтр("ru='Решение СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, влияющие на результат решения системы линейных уравнений, с помощью которой выполняется расчет стоимости номенклатуры.'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "ПогрешностиРешенияСЛУ";
	ОписаниеГруппы.Наименование = НСтр("ru='Погрешности решения СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		...
		|Параметры этой группы также используются в операции ""Настройка распределения расходов"".'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "ОграниченияВыборки";
	ОписаниеГруппы.Наименование = НСтр("ru='Ограничения на размер различных структур данных'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, ограничивающие максимальный размер различных типов данных, используемых механизмом расчета.
		|Необходимы для предотвращения ошибок нехватки памяти.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ОписаниеГруппы.Наименование = НСтр("ru='Управление итогами регистров'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, влияющие на скорость запись движений по регистрам, сформированным механизмом расчета.'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "НачальныеОстатки";
	ОписаниеГруппы.Наименование = НСтр("ru='Переход на партионный учет версии 2.2'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, влияющие на формирование начальных остатков партий в регистре ""Себестоимость товаров"".'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "Проверки";
	ОписаниеГруппы.Наименование = НСтр("ru='Проверки данных'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, используемые в процедурах проверки данных до и после расчета.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "ВыгрузкаДанныхРасчета";
	ОписаниеГруппы.Наименование = НСтр("ru='Выгрузка промежуточных данных расчета'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, позволяющие сохранить промежуточные данные расчета партий и себестоимости.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "ВозможностиРазныхВерсийПлатформы";
	ОписаниеГруппы.Наименование = НСтр("ru='Использование возможностей Платформы'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Скрыть		= НЕ ВерсияПлатформыНеНиже8_3_14;
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, позволяющие задействовать или отключить возможности, предоставляемые разными версиями Платформы.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "Отладка";
	ОписаниеГруппы.Наименование = НСтр("ru='Отладка расчета'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Скрыть		= НЕ РежимОтладки;
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры, используемые для отладки механизмов расчета.
		|Изменять данные параметры рекомендуется только в тестовых информационных базах!'", ОбщегоНазначения.КодОсновногоЯзыка());
		
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "ГруппаАвтоматическоеТестирование";
	ОписаниеГруппы.Наименование = НСтр("ru='Автоматическое тестирование'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Скрыть		= НЕ РежимОтладки;
	ОписаниеГруппы.Описание 	=
	...
	ОписаниеГруппы = ОписаниеГруппПараметров.Добавить();
	ОписаниеГруппы.Имя 			= "РасширенныеНастройкиРасчетаСебестоимости";
	ОписаниеГруппы.Наименование = НСтр("ru='Расширенные настройки расчета себестоимости'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеГруппы.Описание 	=
		НСтр("ru='Параметры тонкой настройки алгоритмов расчета себестоимости, формирования баз распределения, а также производительность отдельных механизмов закрытия месяца.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ОписаниеПараметровОперацииЗакрытияМесяца() Экспорт
	...
	#Область РешениеСЛУ
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Точность решения СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Описание =
			...
			|но тем большее количество итераций решения СЛУ требуется, чтобы достигнуть этой точности.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// МаксимальноеКоличествоИтераций
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "МаксимальноеКоличествоИтераций";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество итераций поиска решения СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Описание =
			...
			|Необходимо для того, чтобы принудительно закончить поиск подходящего решения СЛУ в случае, если не удается достичь требуемой точности решения СЛУ.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Разрядность числовых полей, используемых при решении СЛУ.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Описание =
			...
			|Подробнее про разрядность см. https://its.1c.ru/db/metod8dev#content:2665:hdoc'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// ЗащитаОтПереполненияПоля
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "ЗащитаОтПереполненияПоля";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное значение, допустимое для суммового поля запроса'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный));
		...
		ОписаниеПараметра.Описание =
			...
			|Значение подобрано опытным путем.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область ПогрешностиРешенияСЛУ
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальная себестоимость (регл.), считаемая погрешностью'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Описание =
			...
			|Подробнее см. описание группы параметров.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальная себестоимость (упр.), считаемая погрешностью'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(8, 2, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 0;
		ОписаниеПараметра.ЗначениеПоУмолчанию = 12;
		ОписаниеПараметра.Описание =
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальная сумма расходов (регл.), считаемая погрешностью'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(8, 2, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 0;
		ОписаниеПараметра.ЗначениеПоУмолчанию = 12;
		ОписаниеПараметра.Описание =
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальная сумма расходов (упр.), считаемая погрешностью'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(8, 2, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 0;
		ОписаниеПараметра.ЗначениеПоУмолчанию = 12;
		ОписаниеПараметра.Описание =
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество, считаемое погрешностью'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область ОграниченияВыборки
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество строк в таблице значений'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Описание =
			НСтр("ru='Максимальное количество строк, которое может быть помещено в таблицу значений.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// КоличествоЗаписейВНЗ
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "КоличествоЗаписейВНЗ";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество строк в наборе записей'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(10, 0, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 0;
		...
		ОписаниеПараметра.Описание =
			...
			|т.к. при этом не будут записаны начальные остатки в разрезе партий и последующий расчет будет некорректным.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// КоличествоРегистраторовВТранзакцииЗаписи
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "КоличествоРегистраторовВТранзакцииЗаписи";
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Количество регистраторов в транзакции записи'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(10, 0, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 0;
		ОписаниеПараметра.ЗначениеПоУмолчанию = 0;
		ОписаниеПараметра.Описание =
			...
			|но увеличивает время блокировки записываемой порции регистраторов, делая их недоступными для изменения пользователями во время расчета.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// ПорцияРасчетаПартий
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "ПорцияРасчетаПартий";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальная порция выборки данных при расчете партий'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(10, 0, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 1;
		ОписаниеПараметра.ЗначениеПоУмолчанию = 10000;
		ОписаниеПараметра.Описание =
			НСтр("ru='Максимальное количество узлов, обсчитываемых за один проход в механизме распределения партий версии 2.2.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// МаксимальнаяДлинаЦепочки
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "МаксимальнаяДлинаЦепочки";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальная длина цепочки движения партии'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(4, 0, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 1;
	...
	#Область КонфликтыБлокировок
		...
		ОписаниеПараметра.Скрыть			  = Истина; // пока не используется
	...
	#Область ОграниченияФЗ
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество одновременно выполняемых потоков расчета партий'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Описание =
			...
			|Если значение параметра равно 0, то расчет партий будет выполняться только в основном сеансе, без использования фоновых заданий.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество одновременно выполняемых заданий записи'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(3, 0, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 0;
		...
		ОписаниеПараметра.Описание =
			...
			|Одно фоновое задание записи обслуживает один регистр.
			...
			|Если значение параметра меньше 2, то запись движений будет выполняться последовательно, без использования фоновых заданий.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// КоличествоДвиженийВФЗЗаписи
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "КоличествоДвиженийВФЗЗаписи";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Максимальное количество движений в порции данных для записи'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(10, 0, ДопустимыйЗнак.Неотрицательный));
		ОписаниеПараметра.ДиапазонС     	  = 1;
		ОписаниеПараметра.ЗначениеПоУмолчанию = 100000;
		ОписаниеПараметра.Описание =
			...
			|При увеличении размера порции возможно незначительное увеличение скорости записи, но также возможно аварийное завершение расчета в случае нехватки памяти для большой порции данных.'", ОбщегоНазначения.КодОсновногоЯзыка());
	
		// МаксимальноеКоличествоФЗОтраженияВУчете
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "МаксимальноеКоличествоФЗОтраженияВУчете";
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Максимальное количество одновременно выполняемых заданий отражения документов в учете'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Число",,, Новый КвалификаторыЧисла(3, 0, ДопустимыйЗнак.Неотрицательный));
		...
		ОписаниеПараметра.Описание =
			...
			|Если значение параметра меньше 2, то отражение в учете выполняться последовательно, без использования фоновых заданий.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область УправлениеИтогами
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Отключать итоги регистра перед записью движений'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Истина;
		ОписаниеПараметра.Описание =
			...
			|Это позволяет ускорить запись движений, но по окончании расчета тратится некоторое время на пересчет итогов.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область НачальныеОстатки
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Взять остатки себестоимости из партионных регистров'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		...
		ОписаниеПараметра.Описание =
			...
			|- потом выполняется приход партий по данным партионных регистров.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Распределять расхождения в суммах регистра себестоимости и регистров партий'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Истина;
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
			...
			|Данный параметр имеет смысл если не установлен предыдущий параметр.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область Проверки
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Проверять подбор партий в сформированных движениях'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		...
		ОписаниеПараметра.Описание =
			...
			|Проверка по умолчанию отключена, т.к. еще находится в режиме тестирования.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// ПроверятьСоответствиеСебестоимостиИПартийНДС
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "ПроверятьСоответствиеСебестоимостиИПартийНДС";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Проверять соответствие остатков в регистрах себестоимости и партий НДС (средняя)'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь; // проверка используется для автоматического тестирования
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Проверять соответствие остатков в регистрах себестоимости и партий НДС (ФИФО скользящая)'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь; // проверка используется для автоматического тестирования
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Проверять соответствие оперативного учета и себестоимости'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь; // проверка используется для автоматического тестирования
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
			|остатки по оперативным регистрам должны соответствовать остаткам по регистру ""Себестоимость товаров"".
			|Проверка по умолчанию отключена, т.к. находится в режиме тестирования.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Проверять соответствие регистров Прочие расходы и Партии прочих расходов'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
	...
	#Область ВыгрузкаДанныхРасчета
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Каталог для сохранения данных расчета партий'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Строка");
		ОписаниеПараметра.ЗначениеПоУмолчанию = "";
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
			|в указанный каталог будут сохранены данные, используемые при расчете одного из этапов расчета партий (см. описание следующего параметра).'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// ЭтапПартионногоУчета22ДляВыгрузкиВФайлы
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "ЭтапПартионногоУчета22ДляВыгрузкиВФайлы";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Сохраняемый этап партионного учета 2.2'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Строка");
		ОписаниеПараметра.ЗначениеПоУмолчанию = "";
		ОписаниеПараметра.СписокВыбора 		  = РасчетСебестоимости.ЭтапыСРаспределениемПартий();
		ОписаниеПараметра.СписокВыбора.Вставить(0, "", НСтр("ru='<не выгружать>'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
			...
			|Следует иметь в виду, что при попытке выгрузить большой объем данных может возникнуть ошибка нехватки памяти и расчет прервется.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Каталог для сохранения СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Строка");
		ОписаниеПараметра.ЗначениеПоУмолчанию = "";
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме расчета себестоимости:
			|в указанный каталог будут сохранены данные, используемые для составления СЛУ.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// ФорматСохраненияДанныхСЛУ
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "ФорматСохраненияДанныхСЛУ";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Формат сохранения СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Строка");
	...
	#Область ВозможностиРазныхВерсийПлатформы
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Использовать платформенный механизм решения СЛУ'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		...
		ОписаниеПараметра.Описание =
			...
			|см. http://downloads.v8.1c.ru/content//Platform/8_3_14_1565/1cv8upd_8_3_14_1565.htm'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Отключить редуцирование графа при решении системы линейных уравнений'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		...
		ОписаниеПараметра.Описание =
			...
			|см. http://downloads.v8.1c.ru/content//Platform/8_3_17_1851/1cv8upd_8_3_17_1851.htm'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Отключить редуцирование графа при решении системы линейных уравнений этапа расчета партий НДС'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.Скрыть			  = НЕ ВерсияПлатформыНеНиже8_3_17;
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Описание =
	...
	#Область Отладка
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Перед расчетом очищать старые расчетные движения'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		...
		ОписаниеПараметра.Описание =
			...
			|Аналогичен следующему параметру, но используется для всего расчета.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// НеСохранятьРасчетныеДвижения
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "НеСохранятьРасчетныеДвижения";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='При проведении документа не сохранять старые расчетные движения'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|%Парам1%.
			|Аналогичен предыдущему параметру, но используется для конкретного проводимого документа.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru = 'Вести протокол выполнения запросов этапа в журнале регистрации'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Строка");
		ОписаниеПараметра.ЗначениеПоУмолчанию = "";
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|Последний запрос, который будет записан в журнал регистрации будет тем самым проблеммным запросом,
			|из-за которого происходит аварийное завершение работы программмы.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// КонтролироватьРегистраторыВФЗ
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "КонтролироватьРегистраторыВФЗ";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Контролировать корректность разбиения на порции'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|а в худшем - затрет движения другого задания по этому регистру и регистратору.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Игнорировать некорректные первичные движения документов'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			НСтр("ru='Используется в механизме партионного учета версии 2.2:
			|позволяет не выполнять перепроведение документов, имеющих ошибки в первичных движениях по регистрам.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// НеВыполнятьОптимизациюДанных
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "НеВыполнятьОптимизациюДанных";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Не выполнять оптимизацию данных при расчете партий'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|Оптимизация уменьшает объем обрабатываемых данных за счет удаления узлов, которые заведомо не будут рассчитаны.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// НеВыполнятьОптимизациюЦепочек
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "НеВыполнятьОптимизациюЦепочек";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Не выполнять оптимизацию цепочек при расчете партий'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|Расчет цепочек будет выполняться как в партионном учете версии 2.1.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// НеВыполнятьРасчетСебестоимости
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "НеВыполнятьРасчетСебестоимости";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Не выполнять расчет себестоимости'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|Фактическая себестоимость и так не оказывает влияния на другие этапы (внутри одного периода), т.к. рассчитывается в последнюю очередь.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// НеСдвигатьПериодПоОкончанииРасчета
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "НеСдвигатьПериодПоОкончанииРасчета";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Не сдвигать период расчета по окончании расчета'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|Полезно при отладке расчета какого-то конкретного периода - каждый раз не надо вручную возвращать задания к расчету.'", ОбщегоНазначения.КодОсновногоЯзыка());
		
		// РасчетОдногоЭтапа
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "РасчетОдногоЭтапа";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Расчет только одного этапа'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		...
		ОписаниеПараметра.Описание =
			...
			|(куда были бы помещены данные предыдущих этапов при нормальном ходе расчета партий), а из данных ИБ.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Исправлять пустой регистратор в сформированных движениях'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Скрыть			  = НЕ РежимОтладки;
		ОписаниеПараметра.Описание =
			...
			|При этом запись движений будет выполнена успешно.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	#Область РасширенныеНастройкиРасчетаСебестоимости
		
		Родитель = "РасширенныеНастройкиРасчетаСебестоимости";
		
		// РаспределятьРасходыНаПродажуНаВсеРеализацииЗаказа
		ОписаниеПараметра = ОписаниеПараметров.Добавить();
		ОписаниеПараметра.Родитель 			  = Родитель;
		ОписаниеПараметра.Имя 				  = "РаспределятьРасходыНаПродажуНаВсеРеализацииЗаказа";
		ОписаниеПараметра.Наименование 		  = НСтр("ru='Распределять расходы на продажу на все реализации в рамках заказа'", ОбщегоНазначения.КодОсновногоЯзыка());
		ОписаниеПараметра.ТипЗначения   	  = Новый ОписаниеТипов("Булево");
		ОписаниеПараметра.ЗначениеПоУмолчанию = Ложь;
		ОписаниеПараметра.Описание =
			НСтр("ru='Влияет на определение базы для варианта ""На себестоимость продаж"" с аналитикой ""Заказ клиента"".
			|При отключенной опции распределение будет происходить на реализации в рамках заказа только текущего месяца.
			|При включенной опции распределение будет происходить на все реализации заказа текущего и предшествующих периодов.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПериодыИзмененияТабличныеЧастиДокумента(ДокументОбъект, ОписаниеТабличныхЧастей) Экспорт
	...
	Для Каждого КлючИЗначение Из ОписаниеТабличныхЧастей Цикл
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаДокумента", СтрокаТаблицаДокумента);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПоляСвернутые", 	  СтрокаПоляСвернутые);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ГДЕ &Группировка",  СтрокаПоляГруппировка);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &УсловиеОтбора",  ?(СтрокаРесурсыОтбор = "", "", "ИМЕЮЩИЕ" + Символы.ПС + СтрокаРесурсыОтбор));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Дата", 			  СтрокаДата);
