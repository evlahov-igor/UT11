...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	СУММА(РеализацияУслугПрочихАктивовПрослеживаемыеТовары.КоличествоПоРНПТ) КАК КоличествоПоРНПТ
	|ИЗ
	...
	|ГДЕ
	|	РеализацияУслугПрочихАктивовПрослеживаемыеТовары.НомерГТД.РНПТ
	|
	|СГРУППИРОВАТЬ ПО
	...
	СтруктураДанныхДляПечати 	= Новый Структура("РезультатПоШапке, РезультатПоТабличнойЧасти, Прослеживаемость",
	                                               РезультатПоШапке, РезультатПоТабличнойЧасти, Прослеживаемость);
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка) Экспорт
	...
	ДанныеДокументаДляЧека.Товары = Запрос.Выполнить().Выгрузить();
