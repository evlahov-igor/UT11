...
Процедура ЗаполнитьОписаниеГруппЭтаповЗакрытияМесяца(ТаблицаГруппЭтапов) Экспорт
	...
	НоваяСтрока = ТаблицаГруппЭтапов.Добавить();
	НоваяСтрока.Код = ИдентификаторГруппыКонтрольФинансовыхРегистров();
	НоваяСтрока.Наименование = НСтр("ru='Контроль финансовых регистров'");
...
Функция ИдентификаторГруппыКонтрольФинансовыхРегистров() Экспорт
	Возврат "КонтрольФинансовыхРегистров";
КонецФункции
...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	#Область Группа_КонтрольФинансовыхРегистров
	
	ТекущийРодитель = ИдентификаторГруппыКонтрольФинансовыхРегистров();
...
#Область Группа_ПодготовкаДанных
...
Функция ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, ВыполнятьССамогоРаннего,
	...
	Если НЕ ЗначениеЗаполнено(ИмяРегистраЗаданий) Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ТекстУсловиеОтбора", "И Задания.Операция = &РассчитываемыйЭтап");
	...
	Иначе
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ТекстУсловиеОтбора", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТОрганизации", ?(ЗначениеЗаполнено(ИмяТаблицыОтбораОрганизаций), ИмяТаблицыОтбораОрганизаций, "ВТОрганизации"));
