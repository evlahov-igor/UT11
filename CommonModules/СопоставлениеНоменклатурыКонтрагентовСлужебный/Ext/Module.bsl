...
Процедура ЗаполнитьСловарьНоменклатурыБЭДПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("ОтключитьЗаполнениеСловаряСопоставленияНоменклатурыБЭД") 
		И Источник.ДополнительныеСвойства.ОтключитьЗаполнениеСловаряСопоставленияНоменклатурыБЭД Тогда
		Возврат;
	КонецЕсли;
...
Процедура УдалитьСловаИзСловаряСопоставленияНоменклатурыБЭД(Знач СсылкаНаОбъект, Отказ)
	...
	Исключение
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстОшибки, ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().СопоставлениеНоменклатурыКонтрагентов,, ПараметрыЗаписи);
	КонецПопытки;
