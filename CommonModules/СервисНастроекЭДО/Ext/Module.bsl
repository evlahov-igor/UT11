...
Функция ОбновитьИнформациюОбАктивностиУчетнойЗаписи(ИдентификаторУчетнойЗаписи, КонтекстДиагностики) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Тикет = ИнтеграцияБИПБЭД.БилетНаСайтПоддержки(ИнтернетПоддержкаПользователейПодключена, Ложь, Ложь, КонтекстДиагностики);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ПараметрыКомандыОтправкаДанныхАбонентаВСервисНастроек(ПараметрыЗапроса, Отказ,
	...
	БилетПоддержки = ИнтеграцияБИПБЭД.БилетНаСайтПоддержки(ПоддержкаПодключена,,, КонтекстДиагностики);
...
Функция ВыполнитьКомандуСервиса(ПараметрыКоманды, Отказ, КонтекстДиагностики)
	...
	РезультатВызова = ИнтернетСоединениеБЭД.ВызватьHTTPМетод(ОписаниеСоединения, Запрос, ПараметрыКоманды.Метод, ВидОперации, КонтекстДиагностики);
	...
	Результат = ОбработатьРезультатОтветаСервиса(ПараметрыКоманды, РезультатВызова.Ответ, Отказ, КонтекстДиагностики);
...
Функция ОбработатьДанныеОтветаСервиса(Знач ПараметрыКоманды, ДанныеСервиса, Отказ, КонтекстДиагностики)
	...
	Если ЗначениеЗаполнено(ПараметрыКоманды.ОбработкаРезультата) Тогда
		...
		Если ПараметрыКоманды.ОбработкаРезультата = "КаталогФорматовЭД" Тогда
			...
			Результат = ОбработкаРезультатаКаталогФорматовЭД(ДанныеСервиса, Отказ, КонтекстДиагностики);
		...
		ИначеЕсли ПараметрыКоманды.ОбработкаРезультата = "СвязьТиповИФорматовЭД" Тогда
			...
			Результат = ОбработкаРезультатаСвязьТиповИФорматовЭД(ДанныеСервиса, Отказ, КонтекстДиагностики);
