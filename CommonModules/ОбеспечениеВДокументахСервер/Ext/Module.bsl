...
Функция ПодстановкиОсновногоСпособаОбеспечения(Номенклатура, Характеристика, Склад, УсловиеПолучения) Экспорт
	...
	ТекстЗапроса =
		...
		// << 17.03.2023 Федотов А.М., КРОК, Jira№A2105505-1417
		// {
		//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияИзмененияФорматовМагазинов.СрезПоследних(, ) КАК ФорматовМагазинов
		// }
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ТабРС.Склад КАК Склад,
		|			МАКСИМУМ(ТабРС.ФорматМагазина) КАК ФорматМагазина,
		|			МАКСИМУМ(ТабРС.КонтролироватьАссортимент) КАК КонтролироватьАссортимент,
		|			МАКСИМУМ(ТабРС.РозничныеЦеныИзФорматаМагазина) КАК РозничныеЦеныИзФорматаМагазина
		|		ИЗ
		|			РегистрСведений.ИсторияИзмененияФорматовМагазинов.СрезПоследних(, ) КАК ТабРС
		|	
		|		СГРУППИРОВАТЬ ПО
		|			ТабРС.Склад) КАК ФорматовМагазинов
		// >> 17.03.2023 Федотов А.М., КРОК, Jira№A2105505-1417
		|		ПО (ВедущаяТаблица.УсловиеПолучения)
		|			И (ВариантыОбеспеченияТоварами.Номенклатура ЕСТЬ NULL)
