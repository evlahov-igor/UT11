...
Процедура ОбработатьУточнениеДанных(ДанныеШтрихкода, ДанныеВыбора, ПараметрыСканирования, РезультатОбработки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДанныеШтрихкода.ВидУпаковки) Тогда
		Если ДанныеВыбора.КоличествоПотребительскихУпаковок > 1 Тогда
			...
			ДанныеШтрихкода.Коэффициент = ДанныеВыбора.КоличествоПотребительскихУпаковок * КоличествоВПотребительскойУпаковке;
		Иначе
		...
		КонецЕсли;
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеШтрихкода.КоличествоПотребительскихУпаковок) Тогда
		Если ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская Тогда
			...
			ДанныеШтрихкода.Коэффициент = КоличествоВПотребительскойУпаковке;
		Иначе
			ДанныеШтрихкода.КоличествоПотребительскихУпаковок = ДанныеВыбора.КоличествоПотребительскихУпаковок;
			ДанныеШтрихкода.Коэффициент = ДанныеВыбора.КоличествоПотребительскихУпаковок * КоличествоВПотребительскойУпаковке;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДополнитьДанныеПоШтрихкодамПараметрамиДополнения(ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыДополнения) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		СтрокиДляЗаполнения.Вставить(СтрокаДанных, ДанныеЗаполненияПоЧастичномуВыбытию);
	...
	ШаблонныйЭлементДляЗаполнения = ШтрихкодированиеИС.НовыйЭлементДополненияВложенныхШтрихкодовУпаковокЧастичноеВыбытие();
	
	Для Каждого КлючИЗначение Из ШаблонныйЭлементДляЗаполнения Цикл
		ШаблонныйЭлементДляЗаполнения[КлючИЗначение.Ключ] = КлючИЗначение.Ключ;
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из СтрокиДляЗаполнения Цикл
		
		СтрокаДанных                        = КлючИЗначение.Ключ;
		ДанныеЗаполненияПоЧастичномуВыбытию = КлючИЗначение.Значение;
		
		Если ЗначениеЗаполнено(ДанныеЗаполненияПоЧастичномуВыбытию[ШаблонныйЭлементДляЗаполнения.Количество]) Тогда
			СтрокаДанных.ЧастичноеВыбытие = Истина;
...
Процедура ЗаполнитьСтрокиТаблицыДанныхПоГрупповымУпаковкам(ДанныеКодовМаркировки, ЗаполнятьКоличествоПотребительскихУпаковок = Истина) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанных.GTIN) Тогда
			...
			Если ЗаполнятьКоличествоПотребительскихУпаковок Тогда
	...
	Для Каждого СтрокаДанных Из НетКоэффициента Цикл
		...
		ПлановоеКоличествоПотребительскихУпаковок = ПлановоеКоличествоПотребительскихУпаковок(СтрокаДанных, ОписаниеGTIN);
...
Функция ПлановоеКоличествоПотребительскихУпаковок(СтрокаДанных, ОписаниеGTIN)
	...
	Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая
		...
		СтрокаДанных.СоставКодаМаркировки.Свойство("КоличествоПотребительскихУпаковок", КоличествоПотребительскихУпаковокGTIN02);
	КонецЕсли;
...
Функция ШтрихкодыУпаковокИзДокумента(Знач ДокументСсылка, Знач ПараметрыСканирования) Экспорт
	...
	ВыбыраемыеПоля = Новый Массив();
	ВыбыраемыеПоля.Добавить(СтрШаблон("Штрихкоды.%1 КАК ШтрихкодУпаковки", ИмяКолонки));
	Если ЕстьКолонкаЗначениеШтрихкода Тогда
		ВыбыраемыеПоля.Добавить("Штрихкоды.ЗначениеШтрихкода КАК Штрихкод");
	Иначе
		ВыбыраемыеПоля.Добавить(СтрШаблон("ЕСТЬNULL(Штрихкоды.%1.ЗначениеШтрихкода, """") КАК Штрихкод", ИмяКолонки));
	КонецЕсли;
	...
	ТекстЗапроса = СтрШаблон(
		...
		ИмяДокумента,
		ИмяТабличнойЧасти,
		СтрСоединить(ВыбыраемыеПоля, ", " + Символы.ПС));
...
Функция ВложенныеШтрихкодыУпаковокПоДокументу(ДокументСсылка, Детализация, ПараметрыСканирования, СортироватьДанные = Ложь) Экспорт
	...
	Возврат ВложенныеШтрихкодыУпаковок(ШтрихкодыУпаковок, Детализация, ПараметрыСканирования, СортироватьДанные);
...
// Возвращаемое значение:
//  (См. ИнициализацияВложенныхШтрихкодов).
Функция ВложенныеШтрихкодыУпаковок(ШтрихкодыУпаковок, Детализация, ПараметрыСканирования, СортироватьДанные = Ложь) Экспорт
		...
		Если ДляУпаковокТребоватьПодключениеМОТП Тогда
			...
			Для Каждого СтрокаШтрихкода Из ДанныеШтрихкодовСписок Цикл
				...
				Если ДанныеРазбора = Неопределено
					...
					ТекстОшибки = НСтр(
					"ru = 'Обработка логистических упаковок и групповых упаковок
					      |невозможна, так как выключена опция
					      |""Запрашивать данные сервиса ГИС МТ"".
					      |Включите опцию в панели администрирования ""Интеграция с ИС МП (обувь, одежда, табак...)""
					      |в разделе ""Настройки сканирования кодов маркировки"".'");
...
Функция ВложенныеШтрихкодыПоДаннымИСМП(ДанныеШтрихкодовСписок, ПараметрыСканирования, НормализоватьВложенныеШтрихкоды = Истина, ДляУпаковокТребоватьПодключениеМОТП = Ложь) Экспорт
...
Процедура ДобавитьКолонкиТаблицыДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования = Неопределено, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ПараметрыСканирования <> Неопределено Тогда
		...
		ШтрихкодированиеМОТП.КоллекцияДополнительныхКолонокДереваУпаковокИСМП(ПараметрыДерева, ОбязательныеКолонки,,"КоличествоПотребительскихУпаковок");
...
Процедура ЗаполнитьПараметрыДополненияВложенныхШтрихкодов(ПараметрыДополнения, ШтрихкодыУпаковок, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ШтрихкодыУпаковок Цикл
		...
		Если ЕстьВидПродукцииВСтроке Тогда
			Если Не ШтрихкодированиеИСКлиентСервер.ЭтоСтрокаДанныхЧастичногоВыбытия(СтрокаТаблицы, ПараметрыСканирования,, Истина) Тогда
				Продолжить;
			КонецЕсли;
		Иначе
		...
		Если Не ЗначениеЗаполнено(ЭлементДополнения.Количество) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьСтрокиТаблицыДанныхДаннымиЧастичногоВыбытия(ДанныеПоШтрихкодам, ДанныеЗаполнения, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаНастройки Из Настройки Цикл
		...
		Для Каждого СтрокаТаблицы Из ИсходнаяСтрока.СтрокиПоНоменклатуре Цикл
			...
			Если СтрокаТаблицы.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура Тогда
...
Процедура ДобавитьСтрокуДляЗаполненияДаннымиЧастичногоВыбытия(СтрокаДанных, ПараметрыСканирования, ДанныеЗаполнения) Экспорт
	...
	Если Не ИнтеграцияИСМПКлиентСерверПовтИсп.ПоддерживаетсяЧастичноеВыбытие(
		...
		Или СтрокаДанных.ЧастичноеВыбытиеВариантУчета <> Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура
...
Функция СтрокаСодержитОшибкиЧастичногоВыбытия(СтрокаДанных, ПараметрыСканирования, ПравилаПроверкиНаОшибки, КэшМаркируемойПродукции) Экспорт
	...
	Если ПараметрыСканирования.ТребуетсяЧастичноеВыбытие
		...
		ИначеЕсли СтрокаДанных.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура
...
Процедура ОписаниеОшибокКонтроляЧастичногоВыбытия(ПравилаПроверкиНаОшибки, ПараметрыСканирования, ОбработкаСтрокиТаблицы = Ложь) Экспорт
	...
	Если ПравилаПроверкиНаОшибки.Свойство("ОшибкаНастройкиПараметровЧастичногоВыбытия")
		...
		Для Каждого СтрокаДанных Из ПравилаПроверкиНаОшибки.ОшибкаНастройкиПараметровЧастичногоВыбытия Цикл
			...
			Если СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак Тогда
				ОписаниеПроблемы = НСтр("ru = 'Не указана номенклатура для поштучной продажи сигары (сигарилл, ...).'");
			ИначеЕсли СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Духи Тогда
...
Функция НастройкиЧастичногоВыбытияПоТаблицеНоменклатуры(ТаблицаНоменклатуры)
	...
	Запрос.Текст =
		...
		|	НастройкиЧастичногоВыбытияТоваровИСМП.НоменклатураЧастичногоВыбытия   КАК НастроеннаяНоменклатура,
		|	НастройкиЧастичногоВыбытияТоваровИСМП.ХарактеристикаЧастичногоВыбытия КАК НастроеннаяХарактеристика,
		...
		|		ПО ИсходнаяТаблицаВременная.Номенклатура = НастройкиЧастичногоВыбытияТоваровИСМП.Номенклатура
		|		И ИсходнаяТаблицаВременная.Характеристика = НастройкиЧастичногоВыбытияТоваровИСМП.Характеристика";
...
Процедура ЗаполнитьРезультатОбработкиШтрихкодаЧастичноеВыбытие(ДанныеШтрихкода, РезультатОбработки, ПараметрыСканирования) Экспорт
	...
	Если Не ПараметрыСканирования.ЭтоПроверкаКодовМаркировкиИСМП
		...
		Если ДанныеШтрихкода.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.ТекущаяНоменклатура
			Или (ДанныеШтрихкода.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура
...
Процедура ЗаполнитьРезультатОбработкиШтрихкодаПроверкаСредствамиККТ(ДанныеШтрихкода, РезультатОбработки, ПараметрыСканирования) Экспорт
	...
	Если ДанныеШтрихкода <> Неопределено
		...
		И Не ДанныеШтрихкода.ТребуетсяУточнениеЧастичногоВыбытия Тогда
	...
	Если ДанныеПоШтрихкодам.ВложенныеШтрихкоды <> Неопределено Тогда
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтролироватьРасхождениеСоставаУпаковокМеждуИБиИСМП") Тогда
			...
			ШтрихкодированиеИС.ВыявитьРасхожденияВСоставахДеревьев(
				...
				ЕстьРасхождения, ПараметрыСканирования.ДетализацияСтруктурыХранения);
...
Функция ТребуетсяВыборСерии(ДанныеШтрихкода, ПараметрыСканирования)
	...
	ИначеЕсли ПараметрыСканирования.ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияИС.ПалетыСМонотоварнымиКоробами
		И ЗначениеЗаполнено(ДанныеШтрихкода.GTIN) Тогда
		ВидУпаковкиПодходит = Истина;
	ИначеЕсли (ПараметрыСканирования.ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияИС.КоробаСГрупповымиУпаковками
		Или ПараметрыСканирования.ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияИС.ГрупповыеУпаковки)
		И (ДанныеШтрихкода.ВидУпаковки = Перечисления.ВидыУпаковокИС.Потребительская
...
Функция СтрокиТаблицыДанныхДляПолученияСпецифики(ДанныеКодовМаркировки, СпецификаШтрихкодовГрупповыхИЛогистическихУпаковок, ПараметрыСканирования)
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ВозможнаПродукцияИСМП(СтрокаДанных)
			...
			И (СтрокаДанных.Входящий Или Не ЗначениеЗаполнено(СтрокаДанных.Статус)) Тогда
...
Функция ПараметрыСлиянияДанных(ПараметрыЗаполнения, Приемник, Источник) Экспорт
...
Функция ЗаполнитьСтрокиДерева(СтрокаВерхнегоУровня, ПараметрыЗаполнения,
	...
	ОчиститьКоличествоПотребительскихУпаковок = Истина;
	
	Для Каждого СтрокаДерева Из СтрокаВерхнегоУровня.Строки Цикл
		
		Если ОчиститьКоличествоПотребительскихУпаковок Тогда
			Если ТипЗнч(СтрокаВерхнегоУровня) = Тип("СтрокаДереваЗначений") Тогда
				...
				ОчиститьКоличествоПотребительскихУпаковок = Ложь;
			КонецЕсли;
		КонецЕсли;
		...
		РассчитатьКоличественныеПоказателиСтрокиДерева(СтрокаДерева, СтрокаДанных);
...
Процедура ЗаполнитьСтрокуДереваПоДаннымТаблицыКодыМаркировок(СтрокаДерева, СтрокаДанных, ПараметрыЗаполнения)
	...
	Если ЗаполняемыеКолонки.ЗаполнятьНоменклатуру Тогда
		Если ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
		...
		ИначеЕсли ЕстьРодитель И
			Не ЗначениеЗаполнено(СтрокаДерева.Номенклатура)
...
Процедура РассчитатьКоличественныеПоказателиСтрокиДерева(СтрокаДерева, СтрокаДанных)
...
Процедура ОбработатьДеревоПолученноеИзИСМП(ДеревоУпаковок, ДанныеПоШтрихкодам, ПараметрыСканирования, ПараметрыОбработки)
	...
	Изменения = Новый Структура(
		"ДобавленныеСтроки, ИзмененныеСтроки",
	...
	ШтрихкодированиеИСМПСлужебный.ЗаполнитьСтрокиТаблицыДанныхПоГрупповымУпаковкам(Изменения.ИзмененныеСтроки, Ложь);
...
Процедура ДополнитьВложенныеШтрихкодыДаннымиМолочнойПродукции(ВложенныеШтрихкоды, ПараметрыСканирования)
	...
	ИначеЕсли ПараметрыСканирования.Свойство("ЗаполнятьСрокГодности") И ПараметрыСканирования.ЗаполнятьСрокГодности Тогда
...
Функция НормализоватьСвойстваКоллекцииВложенныхШтрихкодов(Коллекция, ПараметрыСканирования = Неопределено) Экспорт
	...
	ШтрихкодированиеМОТП.КоллекцияДополнительныхКолонокДереваУпаковокИСМП(ПараметрыДерева, КоллекцияОбязательныхКолонок,, "КоличествоПотребительскихУпаковок");
...
Функция ОбработатьКодыМаркировкиДляОтчетаОНанесении(ДанныеОтчетаПроизводственнойЛинии, ВидПродукции, ПараметрыОбработки) Экспорт
	...
	ПараметрыРазбора.Вставить("ПользовательскиеПараметрыРазбораКодаМаркировки", ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Функция ОбработатьКодМаркировкиДляОтчетаОНанесении(КодМаркировки, ПараметрыРазбора)
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
		...
		ПараметрыРазбора.ПользовательскиеПараметрыРазбораКодаМаркировки);
	...
	Если ПараметрыРазбора.УпрощеннаяОбработкаКодовМаркировки Тогда
		...
		Если ПараметрыРазбора.ВидПродукции = Перечисления.ВидыПродукцииИС.Табак Тогда
			...
			ВключатьМРЦ = ПараметрыРазбора.ПараметрыНормализации.ВключатьМРЦ
		...
		ИначеЕсли ПараметрыРазбора.ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
			...
			СкоропортящаясяПродукция = ?(ВключатьСрокГодности, ДанныеРазбора.СоставКодаМаркировки.СкоропортящаясяПродукция, Ложь);
			
			ПараметрыШаблона.Вставить("ВключатьСрокГодности",     ВключатьСрокГодности);
			ПараметрыШаблона.Вставить("СкоропортящаясяПродукция", СкоропортящаясяПродукция);
...
Функция УпрощеннаяОбработкаКодовМаркировки(КодМаркировки, ПараметрыРазбора, ДлинаКодаМаркировки, СодержитРазделительGS, НачинаетсяСоСкобки, ХешСуммаСвойствКодаМаркировки)
	...
	Для Каждого ПараметрыШаблона Из ДопустимыеШаблоны Цикл
		...
		Если ПараметрыРазбора.ПользовательскиеПараметрыРазбораКодаМаркировки.ПроверятьАлфавитЭлементов Тогда
...
Функция НормализоватьКодМаркировкиПоУпрощенномуРазбору(ЗначенияЭлементовКМ, ВидПродукции, ПараметрыОписанияКодаМаркировки, ПараметрыНормализации, ПараметрыШаблона)
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.УпакованнаяВода
	...
	ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС Тогда
		...
		Если ПараметрыШаблона.ВключатьСрокГодности Тогда
			...
			Если ПараметрыШаблона.СкоропортящаясяПродукция Тогда
