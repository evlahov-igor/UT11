...
Функция ОтправитьПриглашения(Приглашения, КлючиСинхронизации, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого Приглашение Из Приглашения Цикл
		...
		КонецЕсли;
		МассивКонтрагентов.Добавить(Приглашение.Контрагент);
		МассивИдентификаторовОрганизации.Добавить(Приглашение.ИдентификаторОрганизации);
	КонецЦикла;
	
	ЗапросКонтрагентов = ИнтеграцияЭДО.ЗапросКонтрагентов("Контрагенты", "МассивКонтрагентов");
	...
	ИтоговыйЗапрос.УстановитьПараметр("МассивКонтрагентов", МассивКонтрагентов);
	...
	Пока ВыборкаКонтрагентов.Следующий() Цикл
		...
		ЗначенияРеквизитов.Вставить("ВнешнийКод");
	...
	ВыборкаУчетныхЗаписей = РезультатыЗапроса[3].Выбрать();
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Для Каждого СтрокаПриглашенияНаИдОрганизации Из ПриглашенияНаИдентификаторОрганизации Цикл
			...
			ПараметрыКонтрагента = ЗначениеРеквизитовПоКонтрагентам.Получить(Приглашение.Контрагент);
			Если ЗначениеЗаполнено(Приглашение.ПолучательИНН) Тогда
				ПараметрыКонтрагента["ИНН"] = Приглашение.ПолучательИНН;
			КонецЕсли;
			Если ЗначениеЗаполнено(Приглашение.ПолучательКПП) Тогда
				ПараметрыКонтрагента["КПП"] = Приглашение.ПолучательКПП;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ПараметрыКонтрагента["ИНН"]) Тогда
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'У контрагента ''%1'' не указан ИНН'"),
					Приглашение.Контрагент));
				Продолжить;
			КонецЕсли;
			...
			НоваяСтрока.Наименование = ПараметрыКонтрагента["Наименование"];
			НоваяСтрока.ИНН = ПараметрыКонтрагента["ИНН"];
			Если СтрДлина(НоваяСтрока.ИНН) <> 12 Тогда
				НоваяСтрока.КПП = ПараметрыКонтрагента["КПП"];
			КонецЕсли;
	...
	Для Каждого Приглашение Из Приглашения Цикл
		...
		Попытка
			...
			Если МенеджерЗаписи.Выбран() Тогда
				...
				Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП("Контрагенты", РеквизитыКонтрагента.ИНН, РеквизитыКонтрагента.КПП);
				Если Не ЗначениеЗаполнено(Контрагент) Тогда
					Если СоздаватьКонтрагентовАвтоматическиБЭД
						Или ВыборкаДетальныеЗаписи.СоздатьКонтрагентаБезусловно Тогда
...
Контрагент = Неопределено, ТекстПриглашения = Неопределено, ОписаниеОшибки = Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(КлючПриглашения) Тогда
		Результат.ОписаниеОшибки = "С данными натуральными ключами уже есть приглашение.";
