...
Процедура НастроитьБыстрыеТоварыИГорячиеКлавиши(Форма, Выборка) Экспорт
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Форма, "БыстрыеТовары")
		...
		БыстрыеТовары = Выборка.БыстрыеТовары.Получить(); // ТаблицаЗначений - см. Справочники.ГорячиеКлавиши.ФормаЭлемента.БыстрыеТовары
	...
	КомандыФормы = Выборка.КомандыФормы.Получить(); // ТаблицаЗначений - см. Справочники.ГорячиеКлавиши.ФормаЭлемента.КомандыФормы
...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		ЭтоПопыткаЗаписиДокумента = Ложь;
		Если Не ТолькоЗаполнить Тогда
			...
			Иначе
				ЭтоПопыткаЗаписиДокумента = Истина;
	...
	Исключение
		...
		Если Не ТолькоЗаполнить И ЭтоПопыткаПроведенияДокумента Тогда
			ОтчетОРозничныхПродажахОбъект.Записать(РежимЗаписиДокумента.Запись);
		КонецЕсли;
		...
		Возврат ?(Не ЭтоПопыткаЗаписиДокумента И ЗначениеЗаполнено(ОтчетОРозничныхПродажахОбъект.Ссылка), ОтчетОРозничныхПродажахОбъект.Ссылка, Неопределено);
...
Функция ЗаполнитьОтчетОРозничныхВозвратах(ОтчетОРозничныхВозвратахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		...
		Возврат ?(Не ЭтоПопыткаЗаписиДокумента И ЗначениеЗаполнено(ОтчетОРозничныхВозвратахОбъект.Ссылка), ОтчетОРозничныхВозвратахОбъект.Ссылка, Неопределено);
...
//
Функция ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц) Экспорт
...
//
Функция ЗаполнитьОтчетыОРозничныхПродажахИВозвратах(СостояниеКассовойСмены, ОписаниеОшибки) Экспорт
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;		
	ТипыНалогообложенияНДСПоКассовойСмене = ПодготовитьДанныеПоЧекамКассовойСмены(СостояниеКассовойСмены, МенеджерВременныхТаблиц);
	
	Для Каждого ТипНалогообложенияНДС Из ТипыНалогообложенияНДСПоКассовойСмене Цикл
		
		// Заполнение отчета о розничных возвратах
		ОтчетОРозничныхВозвратах = ЗаполнитьОтчетОРозничныхВозвратахПоКассовойСмене(СостояниеКассовойСмены, ТипНалогообложенияНДС.НалогообложениеНДС, МенеджерВременныхТаблиц, ОписаниеОшибки);
				
		// Заполнение отчета о розничных продажах
		ОтчетОРозничныхПродажах = ЗаполнитьОтчетОРозничныхПродажахПоКассовойСмене(СостояниеКассовойСмены, ТипНалогообложенияНДС.НалогообложениеНДС, МенеджерВременныхТаблиц, ОписаниеОшибки);
		
		Если ОтчетОРозничныхВозвратах = Неопределено Или ОтчетОРозничныхПродажах = Неопределено Тогда			
...
Функция ВыполнитьОперациюЗакрытияКассовойСмены(Объект, ОшибкаПриСнятииZОтчета, ОписаниеОшибки = "") Экспорт
	...
	Если ЗначениеЗаполнено(КассаККМ) Тогда
		...
		// Заполним по каждому типу налогообложения пару документов: Отчет о розничных продажах, Отчет о розничных возвратах
		ПарыДокументовСформированы = ЗаполнитьОтчетыОРозничныхПродажахИВозвратах(СостояниеКассовойСмены, ОписаниеОшибки);
		
		МассивДокументов.Добавить(СостояниеКассовойСмены.КассоваяСмена);
		
		// Архивация чеков ККМ
		Если ПарыДокументовСформированы И Константы.ОперацияПриЗакрытииКассовойСмены.Получить() = Перечисления.ОперацииПриЗакрытииКассовойСмены.АрхивацияЧековККМ Тогда
		...
		// Удаление чеков ККМ
		Если ПарыДокументовСформированы И Константы.ОперацияПриЗакрытииКассовойСмены.Получить() = Перечисления.ОперацииПриЗакрытииКассовойСмены.УдалениеЧековККМ Тогда
...
Функция ДобавитьДокументыПоРаспоряжениюВОчередьЧековККТ(Распоряжение) Экспорт
	...
	Пока ВыборкаПоДням.Следующий() Цикл
		...
		Пока ВыборкаПоДокументам.Следующий() Цикл
			...
			МенеджерОборудованияВызовСервера.ДобавитьЧекВОчередьЧековККТ(ПараметрыОперацииФискализацииЧека);
