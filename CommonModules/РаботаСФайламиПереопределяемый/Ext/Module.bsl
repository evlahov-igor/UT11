...
Процедура ПриОпределенииСправочниковХраненияФайлов(ТипВладелецФайла, ИменаСправочников) Экспорт
	...
	Если ТипВладелецФайла = Тип("ДокументСсылка.ПересчетТоваров") Тогда
		ИменаСправочников.Вставить("КР_ПересчетТоваровПрисоединенныеФайлы", Истина);
	КонецЕсли;
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	
	// << 14.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
	// Предположительно требуется вынести этот блок в отдельный модуль КР_РаботаСФайламиПереопределяемый
	Если ТипВладелецФайла = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
		ИменаСправочников.Вставить("КР_ПриходныйКассовыйОрдерПрисоединенныеФайлы", Истина);
	КонецЕсли;
	Если ТипВладелецФайла = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
		ИменаСправочников.Вставить("КР_РасходныйКассовыйОрдерПрисоединенныеФайлы", Истина);
	КонецЕсли;
	// >> 14.02.2023 Маскаев П.Ю., КРОК, JIRA№ A2105505-1166
