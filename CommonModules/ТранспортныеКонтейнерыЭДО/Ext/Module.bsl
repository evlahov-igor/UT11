...
Функция ПрочитатьКарточкуЭлектронногоДокумента(ИмяФайла)
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Чтение карточки электронного документа.'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			ТекстСообщения);
	...
	Для каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		Попытка
			...
			Для Каждого Документ Из МетаданныеКонтейнера.Документы Цикл
				...
				ДанныеОбъекта.ДоговорКонтрагента = ЗначенияРеквизитовКонтейнера.Контрагент;
...
Процедура ЗаполнитьКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера,
	...
	ИдентификаторКонтрагента = Карточка.ИдентификаторОтправителя;
	...
	Если ЗначениеЗаполнено(Карточка.КППКонтрагентаУточненный) Тогда
		...
		Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП("Контрагенты", Карточка.ИННКонтрагента,
		Карточка.КППКонтрагентаУточненный, Карточка.Дата);
		
		Если Не ЗначениеЗаполнено(Контрагент)
			И ЗначениеЗаполнено(Карточка.НаименованиеКонтрагентаУточненное)
			И СоздаватьКонтрагентовАвтоматически Тогда
	...
	Если Не ЗначениеЗаполнено(Контрагент)
		...
		Если Не ЗначениеЗаполнено(Контрагент)
			И ЗначениеЗаполнено(Карточка.НаименованиеКонтрагента)
			И СоздаватьКонтрагентовАвтоматически Тогда
...
Процедура ЗаполнитьКонтрагентаВПриглашении(ДанныеПриглашения, Контрагент)
	...
	УстановитьПривилегированныйРежим(Истина);
	ПриглашенияЭДО.ИзменитьКонтрагентаПриглашения(КлючПриглашения, Контрагент);
	УстановитьПривилегированныйРежим(Ложь);
