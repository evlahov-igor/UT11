...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если Не Документ.ДополнительныеСвойства.Свойство("ОтказПриЗаполненииВидовЗапасов")
		...
		КонтролироватьОтрицательныеОстатки = Не ПараметрыСеанса.ПроводитьБезКонтроляОстатковТоваровОрганизаций
			...
			И Не Документ.ДополнительныеСвойства.Свойство("ОтключитьКонтрольТоваровОрганизацийПоДокументу");
...
// * ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение - Строка - значения: Всегда, Никогда, ПоНастройкамДоговора, ПоНастройкамДоговораПроизводство, ПоХозяйственнойОперации - если по остаткам подобрался вид запасов ТоварыПринятыеНаХранение
...
// * СторнируемыйДокумент - ДокументСсылка - ссылка на сторнируемый документ в ИБ, используемый при подборе виды запасов
//												в документ Исправление или Сторно перед записью документа.
//
...
Процедура ЗаполнитьВидыЗапасовПоУмолчанию(МенеджерВременныхТаблиц,
	...
	ПоляКлюча	= "ГруппаФинансовогоУчета, ГруппаПродукции, ТипЗапасов, ЭтоВозвратнаяТара, Договор";
...
Процедура СформироватьРезервыПоТоварамОрганизаций(ДокументОбъект,
	...
	Если ДополнительныеСвойства.НужноКонтролироватьОстаткиТоваровОрганизаций
		...
		Если ДополнительныеСвойства.НужноКонтролироватьОстаткиТоваровОрганизаций
			И Не ДополнительныеСвойства.Свойство("ОтключитьКонтрольТоваровОрганизацийПоДокументу") Тогда
...
Процедура ПроверитьЗаполнениеПодразделенияВТабличнойЧасти(ДокументОбъект, Товары, ТипыНоменклатуры, ТекстОшибки, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ТаблицаТовары",		Товары.Выгрузить(, "Номенклатура, Подразделение, НомерСтроки, Назначение, СписатьНаРасходы"));
...
//
Процедура ЗаполнитьНомераГТДвТабличнойЧасти(Товары, НомерГТД, МассивСтрок, ЗаполненыНомераГТД, ИмяПоляНоменклатура = "Номенклатура") Экспорт
	...
	Запрос.УстановитьПараметр("МассивНоменклатуры", Товары.Выгрузить(, ИмяПоляНоменклатура).ВыгрузитьКолонку(ИмяПоляНоменклатура));
	...
	Для Каждого ИдентификаторСтроки Из МассивСтрок Цикл
		...
		Если МассивВедетсяУчетПоГТД.Найти(СтрокаТаблицы[ИмяПоляНоменклатура]) <> Неопределено Тогда
...
Функция ЗапросОтрицательныеОстаткиТоваровОрганизаций()
	...
	// << 06.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1768
	// Добавлено исправление ошибки на основании рекомендаций 1С: https://bugboard.v8.1c.ru/error/000129643  
	// Ниже есть изменения релиза которые повторяют условие, но текузай выборка с таким отбором 
	// 	быстрее в два раза на трех милионах результирующий записей
	|ГДЕ 
	|	НЕ ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	| 	ИЛИ (ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	...
	// Добавлено исправление ошибки на основании рекомендаций 1С: https://bugboard.v8.1c.ru/error/000129643
	|ГДЕ 
	|	НЕ Резервы.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	| 	ИЛИ (Резервы.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	| 		И Резервы.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка) 
	|	)
	// >> 06.07.2023 Марченко С.Н., КРОК, JIRA№A2105505-1768
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	// Отбор осуществлен ранее 
	// По замерам производительности более 3 млн. записей 
	//// A2105505-1952 Изменения релиза 11.5.8.287 
	//|ГДЕ
	//|	НЕ ОстаткиТекущийПериод.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	//|	ИЛИ
	//|	(ОстаткиТекущийПериод.АналитикаУчетаНоменклатуры.Номенклатура.ВестиУчетПоГТД
	//|		И ОстаткиТекущийПериод.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка))
	////
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПроверкаОтрицательныхОстатковТоваровОрганизаций(ПараметрыПроверки) Экспорт
	...
	ДополнительныеПараметры = Новый Структура("НеконтролируемыеТипыЗапасов",
												Перечисления.ТипыЗапасов.НеконтролируемыеПоТоварамОрганизацииТипыЗапасов());
...
//		* ТипЗапасовРезервы - ПеречислениеСсылка.ТипыЗапасов - тип запасов, резервируемого товара.
...
Процедура СформироватьВТТоварыОрганизацийИзменения(ДокументОбъект,
	...
	|	ГДЕ
	|		ВЫРАЗИТЬ(ТоварыОрганизаций.АналитикаУчетаНоменклатуры КАК Справочник.КлючиАналитикиУчетаНоменклатуры).Номенклатура.ТипНоменклатуры В(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|																																			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
...
Функция ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение(ПараметрыЗаполненияВидовЗапасов, ВыборкаТоваров)
	...
	Если ВРег(ПараметрыЗаполненияВидовЗапасов.ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("Всегда")
		...
		Или ВРег(ПараметрыЗаполненияВидовЗапасов.ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоНастройкамДоговора")
		Или ВРег(ПараметрыЗаполненияВидовЗапасов.ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоНастройкамДоговораПроизводство") Тогда
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	ТаблицаОстатков			= МассивРезультатов[4].Выгрузить(); // ТаблицаЗначений
	...
	// << 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2491   
	БазаЯвляетсяЦентральной = КР_ДополнительныеНастройкиПовтИсп.БазаЯвляетсяЦентральной();
	СкладыТекущейБазыДанных = КР_ДополнительныеНастройкиПовтИсп.МассивЗначений("СкладыТекущейБазыДанных", Ложь);
    // >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2491
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.ВидЗапасов) Тогда
				...
				ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение =
					ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение(ПараметрыЗаполненияВидовЗапасов, Выборка);
			...
			Если Не ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию Тогда
				...
				Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения Тогда
					...
					Если БазаЯвляетсяЦентральной
						Или СкладыТекущейБазыДанных.Найти(Выборка.Склад) <> Неопределено
						Тогда 
                    // >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2491
					...
					// << 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2491  
					ПропуститьОтказ = Ложь;
					Иначе
						ПропуститьОтказ = Истина;
					КонецЕсли; 
                    // >> 10.11.2023 Марченко С.Н., КРОК, JIRA№A2105505-2491
...
Процедура УстановитьДопСвойстваКонтролироватьОстаткиИФормироватьСторно(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов, МенеджерВременныхТаблиц)
	...
	// Отдельно выполняем запрос и помещаем таблицу доп параметров 
	//	так как обращение к результату запроса идет по "числовому индексу"
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Т.Организация КАК Организация,
	|	МАКСИМУМ(Т.АналитикаУчетаНоменклатуры.СкладскаяТерритория.КР_ВидСклада В (  
	...
	|	)) КАК ОтключитьКонтрольОстатков  
	|ПОМЕСТИТЬ КР_ДополнительныеПараметрыЗапроса
	|ИЗ
	|	ВТТоварыОрганизацийИзменения КАК Т
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Организация";
	Запрос.Выполнить();
	...
	|	ВТТоварыОрганизацийИзменения КАК ВТТоварыОрганизацийИзменения
	// << 21.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ КР_ДополнительныеПараметрыЗапроса КАК КР_ДопПараметрыЗапроса
	|	ПО ВТТоварыОрганизацийИзменения.Организация = КР_ДопПараметрыЗапроса.Организация
	|		И НЕ КР_ДопПараметрыЗапроса.ОтключитьКонтрольОстатков
	// >> 21.10.2022 Марченко С.Н., КРОК, JIRA№A2105505-702
	|ГДЕ
	|	ВТТоварыОрганизацийИзменения.ДляКонтроляОстатков
	|;
...
Процедура СформироватьВтОтборыОстатковПоТаблицеТоваров(МенеджерВременныхТаблиц, ДобавлятьАналитикиИзДвиженийПередЗаписью, ПодбиратьЗапасыРазныхПериодов = Ложь)
	...
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|			)
	|		И &ТекстАналитикиИзДвиженийПередЗаписью
	|	
	|	) КАК Отборы
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТекстАналитикиИзДвиженийПередЗаписью", ТекстАналитикиИзДвиженийПередЗаписью);
...
Процедура ЗаполнитьВидыЗапасовРазныхПериодов(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	|	ВЫБОР
	|		КОГДА &ЗаполнениеПоОстаткам
	|			ТОГДА ТаблицаТоваров.КоличествоПоРНПТПоОстатку
	|		ИНАЧЕ ТаблицаТоваров.КоличествоПоРНПТПоТекущемуПериоду
	|	КОНЕЦ КАК КоличествоПоРНПТ,
...
//	ПараметрыЗаполненияВидовЗапасов - Структура - см. ПараметрыЗаполненияВидовЗапасов().
//
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	|		ВидыЗапасов.Договор.ПорядокОформленияСписанияНедостачТоваровПринятыхНаХранение,
...
Процедура СообщитьОбОшибкахЗаполненияВидовЗапасовПоТаблицеКОформлению(
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика
...
Процедура ТаблицаОстатковТоваровУПартнеров(ДокументОбъект,
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"&ТекстВариантОстатков",
								?(ОтрицательныеОстатки, "ВложенныйЗапрос.Передано < 0", "ВложенныйЗапрос.Передано > 0"));
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								...
								?(ОтрицательныеОстатки, "-ВложенныйЗапрос.ПереданоПоРНПТ", "ВложенныйЗапрос.ПереданоПоРНПТ"));
...
Процедура ТаблицаПродукцияКОформлениюВыпуска(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков)
	...
	|	СУММА(РезервыТоваровОрганизаций.КоличествоПоРНПТ) КАК КоличествоПоРНПТОстаток,
	...
	|				И (РезервыТоваровОрганизаций.Период >= &НачалоМесяцаДокумента)
	|				И (РезервыТоваровОрганизаций.КорВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварТребующийПодтвержденияВыпуска))
	|				И (РезервыТоваровОрганизаций.Регистратор = &СторнируемыйДокумент)) КАК РезервыТоваровОрганизаций
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ДополнитьЗапросКонтролемОстатковНаКаждыйМесяц(Запрос, ТекстЗапроса, ПараметрыДополнения, МинимальнаяДатаОстатков, ДатыОстатков = Неопределено) Экспорт
	...
	|		ДвиженияДокумента.КоличествоПоРНПТ	КАК КоличествоПоРНПТОстаток
	|	ИЗ
...
Процедура ТаблицаОстатковПринятыхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|			ТОГДА 0
	|		ИНАЧЕ ДанныеДокумента.КоличествоПоРНПТ
	|	КОНЕЦ КАК КоличествоПоРНПТ
	...
	|
	|ВЫБРАТЬ
	|	ДанныеДокумента.Ссылка.ДокументПоступления,
	|	ДанныеДокумента.ВидЗапасов,
	|	ДанныеДокумента.АналитикаУчетаНоменклатуры,
	|	ДанныеДокумента.НомерГТД,
	|	-ДанныеДокумента.Количество,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СгруппированнаяТаблицаТоваров КАК СгруппированнаяТаблицаТоваров
	|		ПО СгруппированнаяТаблицаТоваров.АналитикаУчетаНоменклатуры = ПринятыеТовары.АналитикаУчетаНоменклатуры
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДанныхДокумента КАК ДанныеДокумента
	|		ПО (ИСТИНА)
	|
