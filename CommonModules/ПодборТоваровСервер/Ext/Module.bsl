		...
		Если Не Форма.РежимПодбораБезСуммовыхПараметров Тогда
			...
			ИначеЕсли Форма.РежимОтбораДляРозничныхПродаж Тогда
				Если Форма.ИспользуетсяЦенообразование25 Тогда
					...
					ВидыЦен = ВидыЦенПоОбъектуУсловийПродаж(Форма.ОбъектХраненияУсловийПродаж, "РозничныйВидЦены, ИндивидуальныйВидЦены");
					Форма.ЕстьЦеныВБудущем = ЕстьЗначенияЦенПозжеДатыПодбора(Форма.Дата, ВидыЦен);
				КонецЕсли;
			...
			ИначеЕсли Не Форма.ИспользоватьСоглашенияСКлиентами Тогда
				...
				Если Форма.ИспользуетсяЦенообразование25 Тогда
					ВидыЦен = ВидыЦенПоОбъектуУсловийПродаж(Форма.ОбъектХраненияУсловийПродаж, "ВидЦен, ИндивидуальныйВидЦены");
				КонецЕсли;
...
Функция ВыполнитьПоиск(Форма, ИмяСпискаПоиска)
	...
	ИначеЕсли ИмяСпискаПоиска = "НоменклатураКонтрагентов" Тогда	
		...
		ПараметрыПоиска.СтрокаПоиска = ФорматироватьСтрокуПоиска(Форма.СтрокаПоискаНоменклатураПартнера, ПараметрыПоиска.ТочноеСоотвествие);
...
Процедура ЗаполнитьДеревоЗначенийОтборов(Форма, ДеревоОтборов, ПоХарактеристикам)
	...
	Для Каждого Реквизит Из РеквизитыБыстрогоОтбора Цикл
		...
		Иначе
			// << 17.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955
			// Исправление типовой ошибки. При поиске в режиме "По характеристикам" нужно анализировать реквизиты
			// справочника "ХарактеристикиНоменклатуры", а не "Номенклатура"
			Если ПоХарактеристикам Тогда
				РеквизитНоменклатуры = Метаданные.Справочники.ХарактеристикиНоменклатуры.Реквизиты[Реквизит.ИмяРеквизита];
			Иначе
			// >> 17.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-955
...
// Виды цен по объекту условий продаж.
// 
// Параметры:
//  ОбъектХраненияУсловийПродаж - СправочникСсылка.Партнеры, СправочникСсылка.СоглашенияСКлиентами, СправочникСсылка.Склады, СправочникСсылка.ФорматыМагазинов - Объект хранения условий продаж
//  РеквизитыВидовЦен - Строка - Реквизиты видов цен
// 
// Возвращаемое значение:
//  Массив - Виды цен по объекту условий продаж
Функция ВидыЦенПоОбъектуУсловийПродаж(ОбъектХраненияУсловийПродаж, РеквизитыВидовЦен)
	
	МассивВидовЦен = Новый Массив; 
	
	ВидыЦенОбъектаХраненияУсловийПродаж = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОбъектХраненияУсловийПродаж, РеквизитыВидовЦен);
	Для Каждого Элемент Из ВидыЦенОбъектаХраненияУсловийПродаж Цикл
		Если ЗначениеЗаполнено(Элемент.Значение) Тогда
			МассивВидовЦен.Добавить(Элемент.Значение);
		КонецЕсли;
	КонецЦикла;
