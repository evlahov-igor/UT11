...
Функция ТранслироватьПеречисление(Имя, НаправлениеПеревода, XDTOType) //!!! XDTOType заменить на русское имя
	...
	Иначе
		...
		Для Каждого Фесет Из XDTOType.Фасеты Цикл
			Если НовоеИмя.Найти(Фесет.Значение) <> Неопределено Тогда
				Возврат Фесет.Значение; 
				Прервать;
			КонецЕсли;
		КонецЦикла;
