...
Функция СкорректироватьСтрокиЗаказа(СтруктураПараметров) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Количество >= ОтгруженноеКоличество Тогда
			ОтменитьКоличество = Мин(Выборка.КОформлениюОстаток, Выборка.Количество - Мин(Выборка.Отгружено,ОтгруженноеКоличество));
...
Процедура ПроверитьНеобходимостьСогласованияУсловийПродажи(Знач ДокументПродажи,
	...
	Для ТекИндекс = 0 По МассивРезультатов.Количество()-1 Цикл
		...
		ИначеЕсли МассивПроверок[ТекИндекс] = "КорректностьЭтаповГрафикаОплаты" Тогда
			...
			Выборка.Следующий();
			Если НЕ Выборка.НакладнаяПоЗаказам Тогда
...
//		ИмяТаблицы - Строка - Имя таблицы Товары
//		КР_ДополнительныеФильтры - Неопределено, Структура - позволяет передать дополнительные фильтры в запрос
//			 сейчас обрабатывает только один фильтр - "Склад"
//Процедура ЗаполнитьРеализацииИЦены(ДокументВозврата, ИмяТаблицы) Экспорт
Процедура ЗаполнитьРеализацииИЦены(ДокументВозврата, ИмяТаблицы, КР_ДополнительныеФильтры = Неопределено) Экспорт
// >> 25.06.2024 Вдовенко С.А., КРОК, A2105505-2676
	Запрос = Новый Запрос;
	...
	Если НЕ ВозвратОтРозничногоПокупателя Тогда
		...
		// << 25.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2236
		|	И ВЫБОР
		|		КОГДА НЕ &КР_ВозвратТоваровЧерезКомиссионера
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ РеализацияТоваровУслугТовары.Ссылка.КлиентПартнер = &КР_КлиентПартнер
		|			И РеализацияТоваровУслугТовары.Ссылка.КлиентДоговор = &КР_КлиентДоговор
		|	КОНЕЦ
		// >> 25.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2236
		// << 25.06.2024 Вдовенко С.А., КРОК, A2105505-2676
		|	И &КР_ДополнительныеФильтры
		// >> 25.06.2024 Вдовенко С.А., КРОК, A2105505-2676
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		...
		ВозвратТоваровЧерезКомиссионера = 
			(ДокументВозврата.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровЧерезКомиссионера);
			
		Если ВозвратТоваровЧерезКомиссионера
			И ТипЗнч(ДокументВозврата.Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") // на всякий суучай условие по типу
			Тогда 
			Запрос.УстановитьПараметр("КР_ВозвратТоваровЧерезКомиссионера", Истина);    
			Запрос.УстановитьПараметр("КР_КлиентПартнер", ДокументВозврата.КлиентПартнер);    
			Запрос.УстановитьПараметр("КР_КлиентДоговор", ДокументВозврата.КлиентДоговор);    
		Иначе
			Запрос.УстановитьПараметр("КР_ВозвратТоваровЧерезКомиссионера", Ложь);    
			Запрос.УстановитьПараметр("КР_КлиентПартнер", Неопределено);    
			Запрос.УстановитьПараметр("КР_КлиентДоговор", Неопределено);    
		КонецЕсли;		
		// >> 25.08.2023 Марченко С.Н., КРОК, JIRA№A2105505-2236
		
		// << 25.06.2024 Вдовенко С.А., КРОК, A2105505-2676
		Если ТипЗнч(КР_ДополнительныеФильтры) = Тип("Структура")
					И КР_ДополнительныеФильтры.Свойство("Склад") Тогда
			
			ТекстФильтра = "РеализацияТоваровУслугТовары.Ссылка.Склад = &СкладВозврата";
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&КР_ДополнительныеФильтры", ТекстФильтра);
			Запрос.УстановитьПараметр("СкладВозврата", КР_ДополнительныеФильтры.Склад);
			
		Иначе
			
			Запрос.УстановитьПараметр("КР_ДополнительныеФильтры", Истина);
...
//
Функция ПолучениеДокументовНаОснованииВФоне(ФормаДокумента, ДокументОбъект) Экспорт
	...
	ПроцедураРасчета = "ПродажиСервер.ПоместитьДокументыНаОснованииВХранилище";
...
// Помещаяет список документов на основании во временное хранилище.
//
// Параметры:
//  ДокументСсылка - ДокументСсылка - ссылка на документ.
//  АдресХранилища - Строка - адрес во временном хранилище.
//
Процедура ПоместитьДокументыНаОснованииВХранилище(ДокументСсылка, АдресХранилища) Экспорт
	
	ДанныеРезультата = ПолучитьДокументыНаОсновании(ДокументСсылка);
...
Процедура СформироватьТекстыПолейДокументовНаОсновании(АдресРезультата, ТекстыПоляДокументыНаОсновании, 
			ДокументыНаОсновании, СписокТиповДокументовДляОформления) Экспорт
...
// ** ВариантОбеспечения - ПеречислениеСсылка.ВариантыОбеспечения -
// ** Обособленно - Булево -
Функция ВариантОбеспеченияИФлагОбособленноПоУмолчанию(Соглашение, Статус, ВариантИФлагПоУмолчанию = Неопределено) Экспорт
...
Процедура СообщитьОбОшибкахКорректностьЭтаповГрафикаОплаты(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		Если НЕ Выборка.ЭтоЗаказ Тогда
			...
			ЕстьЗалог = Выборка.ПроцентЗалогаЗаТару <> 0;
		ИначеЕсли Выборка.НеОтгружатьЧастями ИЛИ НЕ ПараметрыЗаполнения.ЭтоЗаказ Тогда
...
Процедура ДополнитьСтруктуруРаспоряжений(СоответствиеРаспоряжений, ПараметрыФормирования)
	...
	Для Каждого КлючИЗначение Из СоответствиеРаспоряжений Цикл
		...
		Если Не ТаблицаДокументы = Неопределено Тогда
			...
			НайденнаяСтрока = ТаблицаДокументы.Найти(КлючИЗначение.Ключ, "Ссылка");
...
Функция ОформитьНакладнуюНаСервере(СоответствиеРаспоряжений, ПараметрыФормирования) Экспорт
	...
	ТаблицаРаспоряжений = ПреобразоватьСоответствиеВТаблицуЗначений(СоответствиеРаспоряжений);
	
	// Разбивка выделенных документов по группам
	ДокументыПоТипамНакладных = РазделитьЗаказыПоПолнымИменамНакладных(ТаблицаРаспоряжений, ПараметрыФормирования, СписокОшибок, КоличествоПакетов);
...
Функция РазделитьЗаказыПоПолнымИменамНакладных(ТаблицаРаспоряжений, ПараметрыФормирования, СписокОшибок, КоличествоПакетов)
...
Функция ОформитьНакладнуюПоТаблице(ДокументыПоТипамНакладных, ПараметрыФормирования, СписокОшибок)
	...
	Иначе
		...
		Если ВыделеннаяСтрока.ВариантОформленияПродажи = Перечисления.ВариантыОформленияПродажи.РеализацияТоваровУслуг
			...
			ПараметрыФормы.Вставить("ПараметрыОформления", Новый Структура("ПоЗаказам, ПоОрдерам", НЕ ПараметрыФормирования.ПоОрдерам, ПараметрыФормирования.ПоОрдерам));
		Иначе
