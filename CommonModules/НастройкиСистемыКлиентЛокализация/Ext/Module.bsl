...
Процедура ОповещениеОЗакрытииФЗ275(Результат, ДополнительныеПараметры) Экспорт
	...
	Элемент = Форма.Элементы.ДекорацияНастроитьПоддержка275ФЗ; // ПолеФормы
	
	Если ДополнительныеПараметры.ПоддержкаПлатежейВСоответствииС275ФЗ Тогда
...
Процедура ОбработкаНавигационнойСсылкиФормы_СинхронизацияДанных(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, Форма) Экспорт
	//++ Локализация
	СтандартнаяОбработка = Ложь;
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьРеестрНСИ" Тогда
		ОткрытьФорму("Отчет.РеестрНормативноСправочнойИнформации.Форма", , Форма);
	КонецЕсли;
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьРеестрУчетныхДанных" Тогда
		ОткрытьФорму("Отчет.РеестрУчетныхДанных.Форма", , Форма);
	КонецЕсли;
	//-- Локализация
	Возврат;
КонецПроцедуры
