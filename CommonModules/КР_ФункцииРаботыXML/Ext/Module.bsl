...
Функция СериализоватьXDTOВXML(ОбъектXDTO, Фабрика = Неопределено) Экспорт
	...
	xdtoФабрика.ЗаписатьXML(НовыйЗаписьXML, ОбъектXDTO,,,,НазначениеТипаXML.Явное); 
	
	Возврат НовыйЗаписьXML.Закрыть();
	
КонецФункции

Функция ДесериализоватьXDTOИзXML(СтрокаXML, Фабрика = Неопределено) Экспорт
	
	xdtoФабрика = ?(Фабрика = Неопределено, ФабрикаXDTO, Фабрика);
	
	НовыйЧтениеXML = Новый ЧтениеXML;
	НовыйЧтениеXML.УстановитьСтроку(СтрокаXML);	
	
	ОбъектXDTO = xdtoФабрика.ПрочитатьXML(НовыйЧтениеXML);
