...
//
// Параметры:
//	ДанныеТовара - Структура, ДанныеФормыЭлементКоллекции - коллекция, содержащая ифнормацию о товаре:
...
//		* СтранаПроисхождения - СправочникСсылка.СтраныМира - информация о стране происхождения товара.
//
Функция ПараметрыСозданияНомераГТД(ДанныеТовара, ТекстРедактирования) Экспорт
	
	ПараметрыСоздания = Новый Структура;
	ПараметрыСоздания.Вставить("Код", ТекстРедактирования);
	ПараметрыСоздания.Вставить("Номенклатура", ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка"));
	...
	Если ДанныеТовара <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыСоздания, ДанныеТовара);
	КонецЕсли;
...
Процедура ОткрытьФормуСозданияНомераГТД(Форма, ПараметрыСоздания) Экспорт
	...
	ОткрытьФорму("Справочник.НомераГТД.Форма.ФормаЭлемента", ПараметрыФормы, Форма);
...
// Выполняет указание значений номера ГТД и страны происхождения в табличной части формы.
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - форма, в которой инициировано указание номера ГТД.
...
Процедура ОбработатьУказаниеНомераГТД(Форма,
	...
	ТекущаяСтрока = Форма.Объект[ИмяТЧ].НайтиПоИдентификатору(ИдентификаторСтроки);
	...
	Если ТекущаяСтрока.Свойство("СтранаПроисхождения") Тогда
		...
		ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, ДействияОбработки, Неопределено);
	КонецЕсли;
