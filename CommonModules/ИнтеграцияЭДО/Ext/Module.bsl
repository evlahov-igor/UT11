...
Функция ОписаниеОбъектаУчета(ИсточникДанных) Экспорт
	...
	Иначе
		...
		Для каждого СтрокаПараметров Из ПараметрыИсточника Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаПараметров.Тип) Тогда
				
				Параметры = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаПараметров);
				Параметры.Вставить("ПрикладнойТипДокумента");
				Параметры.Вставить("Входящий");
				...
				Если ЗначениеЗаполнено(Параметры.ПрикладнойТипДокумента) Тогда
					
					СтрокаОписания.ТипДокумента           = Перечисления.ТипыДокументовЭДО.Прикладной;
					СтрокаОписания.ПрикладнойТипДокумента = Параметры.ПрикладнойТипДокумента;
					
					Если Параметры.Входящий = Истина Тогда
						СтрокаОписания.Направление = Перечисления.НаправленияЭДО.Входящий;
					Иначе
						СтрокаОписания.Направление = Перечисления.НаправленияЭДО.Исходящий;
					КонецЕсли;
					
					СтрокаОписания.Организация     = Параметры.Организация;
					СтрокаОписания.Контрагент      = Параметры.Контрагент;
					СтрокаОписания.Договор         = Параметры.ДоговорКонтрагента;
...
Функция КомандыПечатиДляВнутреннегоЭДО(ОбъектМетаданных) Экспорт
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		Если ЗначениеЗаполнено(КомандаПечати.Идентификатор)
			И ЗапрещенныеКоманды.Найти(КомандаПечати.Идентификатор) = Неопределено Тогда
...
Процедура ПередЗаписьюОбъектаДокументаЭДО(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Для каждого СтрокаОписания Из ОписаниеОбъектаУчета Цикл
		Если СтрокаОписания.Направление = Перечисления.НаправленияЭДО.Входящий Тогда
			Продолжить;
		КонецЕсли;
...
// Возвращает актуальные электронные документы объекта учета.
//
// Параметры:
//  ОбъектУчета - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО - объект учета.
//
// Возвращаемое значение:
// Массив - актуальные электронные документы:
//  * ДокументСсылка.ЭлектронныйДокументИсходящийЭДО, ДокументСсылка.ЭлектронныйДокументВходящийЭДО - ссылка на электронный документ. 
//
