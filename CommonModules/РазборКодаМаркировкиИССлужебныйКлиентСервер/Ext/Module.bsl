...
Функция РазобратьКодМаркировки(ДанныеДляРазбора, ВидыПродукции, ПримечаниеКРезультатуРазбора, Настройки, МодульКонтекста, ПользовательскиеПараметры) Экспорт
	...
	ПараметрыРазбораКодаМаркировки.Вставить("ПользовательскиеПараметры", ПользовательскиеПараметры);
...
Функция КодМаркировкиСоответствуетШаблону(ПараметрыРазбораКодаМаркировки, Настройки, ШаблонКодаМаркировки) Экспорт
	...
	ЗаполнитьСвойствоНормализованныйКодМаркировкиВДанныхРезультата(
		ДанныеРезультата, ПараметрыОписанияКодаМаркировки, ПараметрыРазбораКодаМаркировки, ЗначенияЭлементовКодаМаркировки);
...
Процедура ЗаполнитьСвойствоНормализованныйКодМаркировкиВДанныхРезультата(ДанныеРезультата, ПараметрыОписанияКодаМаркировки, ПараметрыРазбораКодаМаркировки, ЗначенияЭлементовКодаМаркировки)
	
	ПараметрыНормализации = ПараметрыНормализацииКодаМаркировки(); // Используем значения по умолчанию
	ПараметрыНормализации.НачинаетсяСоСкобки       = Истина;
	ПараметрыНормализации.ВключатьМРЦ              = Истина; // Табак
	ПараметрыНормализации.ВключатьСрокГодности     = Истина; // Молочная продукция
	ПараметрыНормализации.ДобавитьМРЦПоУмолчанию   = Ложь;   // НСП
	
	ИспользоватьФильтрПоВидуПродукции = Ложь;
	
	Если ДанныеРезультата.ВидыПродукции.Количество() = 1 Тогда
		// Описание:
		// 1. Шаблоны для вида продукции "Алкогольная" уникальны.
		// 2. Некоторые виды продукции обладают уникальными шаблонами.
		// 3. Настройки разбора сформированы с ограниченным перечнем видов продукции.
	ИначеЕсли ДанныеРезультата.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИС.Логистическая") Тогда
		// Описание:
		// Анализируется свойство "ТипШтрихкода" из результата разбора и "НачинаетсяСоСкобки" из параметров нормализации,
		// вид продукции не имеет значения.
	...
	ИначеЕсли ДанныеРезультата.ВидыПродукции.Количество() > 1 Тогда
		...
		Для Каждого ВидПродукции Из ДанныеРезультата.ВидыПродукции Цикл
			...
			ИначеЕсли ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Алкогольная") Тогда
				ПрочаяПродукция = ПрочаяПродукция + 1;
			Иначе
				ПрочаяПродукцияИСМП = ПрочаяПродукцияИСМП + 1;
			КонецЕсли;
		КонецЦикла;
		
		Если (Булево(ПродукцияМОТП) + Булево(МолочнаяПродукцияИСМП) + Булево(ПрочаяПродукцияИСМП) + Булево(ПрочаяПродукция)) = 1 Тогда
			// Описание:
			// Шаблоны имеют одну и туже сигнатуру
		ИначеЕсли ПрочаяПродукция = 0 Тогда
			
			// Используем только GTIN и СерийныйНомер
			
			Если ПараметрыОписанияКодаМаркировки.ВключаетИдентификаторыПрименения Тогда
				
				НормализованныйКодМаркировки = СтрШаблон("(01)%1(21)%2",
					ЗначенияЭлементовКодаМаркировки["GTIN"],
					ЗначенияЭлементовКодаМаркировки["СерийныйНомер"]);
				
			Иначе
				
				НормализованныйКодМаркировки = СтрШаблон("%1%2",
					ЗначенияЭлементовКодаМаркировки["GTIN"],
					ЗначенияЭлементовКодаМаркировки["СерийныйНомер"]);
		...
		ИспользоватьФильтрПоВидуПродукции = Истина;
		
	КонецЕсли;
	
	Если ИспользоватьФильтрПоВидуПродукции
		И ДанныеРезультата.ВидыПродукции.Количество() > 1
		И ПараметрыРазбораКодаМаркировки.ФильтрПоВидуПродукции.Использовать Тогда
		ВидыПродукцииПоФильтру = ПересечениеМассивов(
			ДанныеРезультата.ВидыПродукции, ПараметрыРазбораКодаМаркировки.ФильтрПоВидуПродукции.ВидыПродукции);
		Если ВидыПродукцииПоФильтру.Количество() = 0 Тогда
			ВидПродукцииДляНормализации = ДанныеРезультата.ВидыПродукции[0];
		Иначе
			ВидПродукцииДляНормализации = ВидыПродукцииПоФильтру[0];
		КонецЕсли;
	Иначе
		ВидПродукцииДляНормализации = ДанныеРезультата.ВидыПродукции[0];
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДанныеРезультата.ВидУпаковки) Тогда
		ВидУпаковкиДляНормализации = ДанныеРезультата.ВидУпаковки;
	Иначе
	...
	ДанныеРезультата.НормализованныйКодМаркировки = НормализоватьКодМаркировки(
		ДанныеДляПолученияНормализированногоКодаМаркировки, ВидПродукцииДляНормализации, ПараметрыНормализации);
