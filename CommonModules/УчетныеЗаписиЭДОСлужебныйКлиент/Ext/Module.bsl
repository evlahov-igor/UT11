...
Процедура ЗарегистрироватьСертификатВСервисе1СЭДО(Форма, Оповещение, ПараметрыРегистрации,
	...
	Контекст.Вставить("Доверенности"       , ПараметрыРегистрации.Доверенности);
	...
	НачатьПодписаниеРегистрационногоПакета1СЭДО(ОписаниеПодписатьЭД, ДанныеПакета, ОтборСертификатов);
	...
	Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
		...
		Если РезультатВыполнения.Свойство("НаборДанных") Тогда
			...
			Если ЗначениеЗаполнено(НаборПараметровПроверокМЧД) Тогда
				РезультатыПроверокМЧД = ЭлектронныеДокументыЭДОКлиент.ПроверитьПодписиПоМЧДПриРегистрацииСертификатов(НаборПараметровПроверокМЧД,
					Контекст.КонтекстДиагностики);
				Для Каждого ПараметрыПроверки Из НаборПараметровПроверокМЧД Цикл
					РезультатПроверки = РезультатыПроверокМЧД[ПараметрыПроверки.Сертификат];
					Если РезультатПроверки = Неопределено 
						ИЛИ Не РезультатПроверки.ПодписьВерна Тогда
						ВыполнитьОбработкуОповещения(Контекст.Оповещение, РезультатОперации);
						Контекст.Очистить();
						Возврат;
					КонецЕсли;
