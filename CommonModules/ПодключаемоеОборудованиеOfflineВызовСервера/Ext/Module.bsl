...
Процедура ОбновитьКодыТоваров(ПравилоОбмена) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
			НастройкиАссортимента = РегистрыСведений.ИсторияИзмененияФорматовМагазинов.ТекущийНастройкиАссортимента(ПравилоОбмена.Склад);
	...
	Если ПараметрыПравилаОбмена.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ККМОфлайн Тогда
		...
		Если КР_ИнтеграцияСВнешнимиСистемами.КР_ЭтоArtix(ПравилоОбмена) Тогда
	...
	Компоновщик.ЗагрузитьНастройки(ПараметрыПравилаОбмена.НастройкиКомпоновкиДанных.Получить());
	...
	Параметр = МакетКомпоновки.ЗначенияПараметров.Найти("ГруппаВидовНоменклатурыАксессуаров");
	Если Параметр <> Неопределено Тогда
		ГруппаАксессуары = КР_ДополнительныеНастройкиПовтИсп.Значение("ГруппаВидовНоменклатурыАксессуаров");
		Параметр.Значение = ГруппаАксессуары;
	КонецЕсли;
	// >> 07.04.2023, Федоров Д.Е., КРОК, Jira№ A2105505-1525
