...
Функция ОбработатьВводШтрихкода(Форма, ВходящиеДанные, КэшированныеЗначения, ПараметрыСканирования = Неопределено) Экспорт
	...
	Если РезультатыПроверок.Количество() > 0 Тогда
		...
		Если Не Результат.ОткрытьФормуВводаКодаМаркировки Тогда
	...
	Если ПолучатьИнформациюПоШтрихкодам Тогда
		...
		Для Каждого ЭлементДанных Из ДанныеПоШтрихкодамEAN Цикл
			...
			Если ЭтоGTINПотребителькойУпаковкиПоДаннымРазбораКодаМаркировки
				...
				И ОписаниеGTIN.ВидУпаковки <> Перечисления.ВидыУпаковокИС.Потребительская Тогда
				ОбщегоНазначения.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru = 'GTIN %1 найден в составе кода маркировки потребительской упакови.
			...
			Если ЗначениеЗаполнено(ЭлементДанных.Упаковка)
				...
				Если ЭлементДанных.ПлановоеКоличествоПотребительскихУпаковок = 1 Тогда
...
Процедура ЗаполнитьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, Форма = Неопределено) Экспорт
	...
	Если ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина) Тогда
		...
		Для Каждого КлючИЗначение Из ДанныеПоШтрихкодам.СтрокиДляСохраненияКодовМаркировки Цикл
			...
			КонецЕсли;
		КонецЦикла;
		МодульШтрихкодированиеИСМПСлужебный.ЗаполнитьСтрокиТаблицыДанныхПоГрупповымУпаковкам(ДанныеКодовМаркировкиДляПересчетаКоличества);
	КонецЕсли;
...
Функция СоздатьМаркированныйТоварПоДаннымШтрихкода(ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Если КоличествоСоответствуетИндивидуальнойПотребительскойУпаковке(ДанныеШтрихкода)
		...
		Иначе
			...
			Если НайденнаяСтрока.Строки.Количество()
				...
				Для Каждого КлючИЗначение Из СоответствиеВложенныхСтрокДереваУпаковокИсточника Цикл
					...
					Иначе
						ЗаполнитьЗначенияСвойств(СтрокаДереваПриемника, ВложеннаяСтрокаИсточника,, "Входящий");
					КонецЕсли;
					...
					Если СтрокаДереваПриемника.Строки.Количество() = 0
						...
						ДополнитьДеревоУпаковок(
							...
							СоответствиеСтарыхИНовыхСтрокДерева, ОтборПоВидуПродукции, КопируемыеКолонки);
...
//  Детализация - ПеречислениеСсылка.ДетализацияСтруктурыХраненияИС - Детализация структуры хранения.
...
Процедура ВыявитьРасхожденияВСоставахДеревьев(ДеревоУпаковок, ДеревоУпаковокПоДаннымСервиса,
	ЕстьРасхождения, Детализация = Неопределено, ПолноеСравнениеДеревьев = Ложь) Экспорт
	...
	ВыявитьРасхожденияВСтрокахДеревьев(
		...
		ТекстОшибкиОтсутствуетПоДаннымИнформационнойБазы(), Ложь, Детализация);
			...
			Для ТекущийУровень = 0 По УровнейВложенности Цикл
				...
				Если Значение = Неопределено Тогда
					...
					ЗаполнитьСтрокуДереваУпаковокПоВыборке(
						...
						НастройкиРазбораКодаМаркировки, ПользовательскиеПараметрыРазбораКодаМаркировки, ПараметрыСканирования);
					...
					ЗаполнитьСтрокуДереваПоДаннымРазбораШтрихкода(
						РодительскиеДанные, ОбщиеМодули, НастройкиРазбораКодаМаркировки, ПользовательскиеПараметрыРазбораКодаМаркировки, ПараметрыСканирования);
...
Процедура ОбработатьУточнениеДанныхДляФормыПроверкиИПодбора(РезультатВыбора, РезультатОбработки, ПараметрыСканирования, КэшированныеЗначения) Экспорт
	...
	Иначе
		...
		Если ДанныеВыбора.Свойство("КоличествоПотребительскихУпаковок")
			И ДанныеВыбора.КоличествоПотребительскихУпаковок > 0 Тогда
			
			ДанныеОписания = РегистрыСведений.ОписаниеGTINИС.ИнициализироватьДанныеОписания();
			ДанныеОписания.GTIN                              = ДанныеШтрихкода.GTIN;
			ДанныеОписания.КоличествоПотребительскихУпаковок = ДанныеШтрихкода.КоличествоПотребительскихУпаковок;
			ДанныеОписания.ВидУпаковки                       = ДанныеШтрихкода.ВидУпаковки;
...
//  КэшПроверенныхСтрок - Соответствие - Кэш проверенных строк
//  ЕстьРасхождения - Булево - Истина, если выявлено расхождение при сравнении
...
Процедура ВыявитьРасхожденияВСтрокахДеревьев(ДеревоЗначений1, ДеревоЗначений2, КэшПроверенныхСтрок, ЕстьРасхождения,
	ТекстОшибки, ПерваяИтерация, Детализация = Неопределено)
	...
	Для Каждого СтрокаДерева1 Из ДеревоЗначений1.Строки Цикл
		...
		Если СоответствующаяСтрокаДерева2 <> Неопределено Тогда
			ВыявитьРасхожденияВСтрокахДеревьев(
				...
				ТекстОшибки, ПерваяИтерация, Детализация);
			Продолжить;
		КонецЕсли;
		...
		Иначе
			...
			Если (Детализация = ВидДетализации.КоробаСГрупповымиУпаковками Или Детализация = ВидДетализации.ГрупповыеУпаковки)
				И СтрокаДерева1.Родитель <> Неопределено
				И СтрокаДерева1.Родитель.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда
				Продолжить;
			КонецЕсли;
			
			Если Детализация = ВидДетализации.ПалетыСМонотоварнымиКоробами
				И СтрокаДерева1.Родитель <> Неопределено
...
Процедура ЗаполнитьСведенияПоШтрихкодамEAN(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.GTIN)
			...
			Или СтрокаДанных.ВидыПродукцииКодаМаркировки.Найти(Перечисления.ВидыПродукцииИС.Алкогольная)<>Неопределено) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
			...
			Если Не ДанныеПулаПоКоду.СохраненоПриСканировании Тогда
				...
				Если Не ЗначениеЗаполнено(СтрокаДанных.ВидУпаковки) Тогда
					...
					Если ДанныеПулаПоКоду.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.АльтернативныйТабакБлок
						Или ДанныеПулаПоКоду.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.БлокТабачныхПачек
						Или ДанныеПулаПоКоду.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.НикотиносодержащаяПродукцияБлок Тогда
		...
		Для Каждого СтрокаИнформации Из НайденныеШтрихкодыEAN Цикл
			...
			Если Не СтрокаДанных.НоменклатураСопоставленаПоУПД Тогда
				...
				Если Не (РезультатПоискаУникален = Неопределено И ВариантовВыбора = 1) Тогда
					...
					ИначеЕсли ДанныеДокументаОснования <> Неопределено
						...
						Продолжить;
					// Соответствие пулу
					ИначеЕсли СтрокаДанных.ЕстьВПулеКодовМаркировки
			...
			Если КодМаркировкиЗаполненПоПулу Тогда
				...
				КонецЕсли;
			Иначе
				ЗаполнитьЗначенияСвойств(СтрокаДанных, СтрокаИнформации,,"Количество");
			...
			Если Не КодМаркировкиЗаполненПоПулу Тогда
		...
		Если ДанныеПулаПоКоду <> Неопределено
			...
			Если Не ЗначениеЗаполнено(СтрокаДанных.ВидУпаковки) Тогда
				...
				Если ДанныеПулаПоКоду.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.АльтернативныйТабакБлок
					Или ДанныеПулаПоКоду.Шаблон = Перечисления.ШаблоныКодовМаркировкиСУЗ.НикотиносодержащаяПродукцияБлок
...
Процедура ЗаполнитьДанныеИзвестнойМаркируемойПродукции(ДанныеПоШтрихкодам, ДобавленныеСтроки, ПараметрыСканирования, ЗаполнитьПоСсылкам = Ложь) Экспорт
	...
	КолонкиЗаполнения = ШтрихкодированиеИС.ИменаКолонокДляЗаполнения().Номенклатура;
	...
	Для Каждого НайденнаяУпаковка Из НайденныеУпаковки Цикл
		...
		СтрокаДанныхКодаМаркировки.Идентифицирован        = ЗначениеЗаполнено(НайденнаяУпаковка.Номенклатура) Или ЗначениеЗаполнено(СтрокаДанныхКодаМаркировки.Номенклатура);
		...
		ИначеЕсли Не СтрокаДанныхКодаМаркировки.ЭтоКодВводаОстатков
				И Не СтрокаДанныхКодаМаркировки.ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная Тогда
...
Процедура ЗаполнитьСтрокуДереваУпаковокПоВыборке(СтрокаДерева, Выборка, Уровень, ОбщиеМодули, НастройкиРазбораКодовМаркировки = Неопределено, ПользовательскиеПараметрыРазбораКодаМаркировки = Неопределено, ПараметрыСканирования = Неопределено)
	...
	Если ПараметрыСканирования <> Неопределено
		...
		ДанныеРазбора = ЗаполнитьСтрокуДереваПоДаннымРазбораШтрихкода(
			СтрокаДерева, ОбщиеМодули, НастройкиРазбораКодовМаркировки, ПользовательскиеПараметрыРазбораКодаМаркировки, ПараметрыСканирования);
		Если ДанныеРазбора <> Неопределено
	...
	ИначеЕсли СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка
		...
		СтрокаДерева.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая;
	ИначеЕсли СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МонотоварнаяУпаковка
		И СтрокаДерева.ТипШтрихкода = Перечисления.ТипыШтрихкодов.GS1_DataMatrix Тогда
...
Функция ЗаполнитьСтрокуДереваПоДаннымРазбораШтрихкода(СтрокаДерева, ОбщиеМодули, НастройкиРазбораКодаМаркировки = Неопределено, ПользовательскиеПараметрыРазбораКодаМаркировки = Неопределено, ПараметрыСканирования = Неопределено)
	...
	ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
		...
		НастройкиРазбораКодаМаркировки, ПользовательскиеПараметрыРазбораКодаМаркировки);
	...
	Если ДанныеРазбора = Неопределено
		...
		ПримечаниеКРезультатуРазбора = Неопределено;
		ДанныеРазбора = РазборКодаМаркировкиИССлужебный.РазобратьКодМаркировки(
			СтрокаДерева.Штрихкод, ВидыПродукцииДляРазбора, ПримечаниеКРезультатуРазбора,
			НастройкиРазбораКодаМаркировки, ПользовательскиеПараметрыРазбораКодаМаркировки);
...
Функция ИменаКолонокДляЗаполнения() Экспорт
	...
	КолонкиДанныеСервиса.Вставить("ПредставлениеНоменкатуры", Истина);
...
Процедура ДополнитьТаблицуДанныеКодовМаркировкиПоСтрокамДереваУпаковок(ДанныеПоШтрихкодам, ДеревоУпаковок, Изменения = Неопределено, ПараметрыСканирования = Неопределено, ПараметрыДополнения = Неопределено) Экспорт
	...
	Если ПараметрыДополнения = Неопределено Тогда
		ПараметрыДополнения = ШтрихкодированиеИС.ПараметрыДополненияТаблицыДанныеКодовМаркировки(
...
Процедура ДополнитьДеревоУпаковок(СтрокаДереваПриемник, СтрокаДереваИсточник, СоответствиеСтарыхИНовыхСтрокДерева = Неопределено, ОтборПоВидуПродукции = Неопределено, КопируемыеКолонки = Неопределено)
	...
	Для Каждого СтрокаИсточника Из СтрокаДереваИсточник.Строки Цикл
		...
		ДополнитьДеревоУпаковок(
			НоваяСтрока, СтрокаИсточника, СоответствиеСтарыхИНовыхСтрокДерева, ОтборПоВидуПродукции, КопируемыеКолонки);
			...
			Для ТекущийУровень = 0 По УровнейВложенности Цикл
				...
				Если ЗначениеЗаполнено(ТекущаяУпаковка) Тогда
					...
					Если Значение = Неопределено Тогда
						...
						ЗаполнитьСтрокуДереваУпаковокПоВыборке(
							РодительскиеДанные, Выборка, ТекущийУровень, ОбщиеМодули,
							Неопределено, ПользовательскиеПараметрыРазбораКодаМаркировки, ПараметрыСканирования);
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли (ИнтеграцияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции)
			...
			Или (ПараметрыСканирования.ЗапрашиватьКоличествоМерногоТовара
				...
				И Не (ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая И ТипЗнч(ПараметрыСканирования.СсылкаНаОбъект) = Тип("ДокументСсылка.МаркировкаТоваровИСМП"))
		...
		// Обработка ошибок и уточнений во всех формах
		ИначеЕсли Не СтрокаТаблицы.Идентифицирован Тогда
...
Процедура ЗаполнитьТребуетсяУказаниеПлановогоКоличестваПотребительскихУпаковокПоGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаДанных.ВидУпаковки)
			Или СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Групповая Тогда // АТП - пачка или блок
		...
		ИначеЕсли СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
			...
			СтрокаДанных.ТребуетсяУказаниеПлановогоКоличестваПотребительскихУпаковокПоGTIN = Не ЭтоGTIN02;
		...
		Если Не ЗначениеЗаполнено(СтрокаДанных.ШтрихкодУпаковки)
			И (ИнтеграцияИСКлиентСервер.ЭтоУпаковка(СтрокаДанных.ТипУпаковки)
				Или (Не СтрокаДанных.ЭтоШтрихкодНоменклатуры
					...
					И СтрокаДанных.МаркируемаяПродукция = Истина)) Тогда
...
//  ИзмененныеСвойства    - Массив, Неопределено - Имена свойств, которые необходимо установить.
//  ПараметрыСканирования - см. ШтрихкодированиеИСКлиент.ПараметрыСканирования.
...
Функция ОбработатьУточнениеДанныхДляМаркированногоТовара(Форма, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	Если ДанныеВыбора.Свойство("КоличествоПотребительскихУпаковок")
		И ЗначениеЗаполнено(ДанныеВыбора.КоличествоПотребительскихУпаковок) Тогда
		...
		ОписаниеНоменклатуры = РегистрыСведений.ОписаниеНоменклатурыИС.ПолучитьОписание(ДанныеШтрихкода.Номенклатура);
		ДанныеШтрихкода.Количество = ДанныеВыбора.КоличествоПотребительскихУпаковок * 
			ОписаниеНоменклатуры.Получить(ДанныеШтрихкода.Номенклатура).КоличествоВПотребительскойУпаковке;
