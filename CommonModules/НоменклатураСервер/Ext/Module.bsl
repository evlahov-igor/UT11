...
//	МассивНепроверяемыхРеквизитов - Массив - реквизиты, которые не нужно проверять платформенной проверкой;
//	Отказ - Булево - Истина - признак отказа продолжения операции;
//	ПараметрыПроверки - Структура - описание см. НоменклатураСервер.ПараметрыПроверкиЗаполненияУпаковок.
//
Процедура ПроверитьЗаполнениеУпаковок(Объект,МассивНепроверяемыхРеквизитов,Отказ,ПараметрыПроверки = Неопределено) Экспорт
	...
	|	И Не ВЫРАЗИТЬ(ТаблицаТоваров.Номенклатура КАК Справочник.Номенклатура).СкладскаяГруппа В (&КР_ГруппыБезУпаковок)
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	// << 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
	ГруппыБезУпаковок = КР_ДополнительныеНастройкиПовтИсп.МассивЗначений("СкладскиеГруппыНоменклатурыБезУпаковок", Ложь);
	Запрос.УстановитьПараметр("КР_ГруппыБезУпаковок", ГруппыБезУпаковок);
	// >> 03.02.2023 Федоров Д.Е., КРОК, JIRA№A2105505-860
...
//	ИмяПоляНоменклатура - Строка - имя поля номенклатура табличной части объекта, для которой выполняется обработка.
//
Процедура ЗаполнитьСтрануПроисхождения(ТаблицаФормы, ИмяПоляНоменклатура = "Номенклатура") Экспорт
	...
	|	ВЫРАЗИТЬ(Таблица.СтранаПроисхождения КАК Справочник.СтраныМира) КАК СтранаПроисхождения
	|ПОМЕСТИТЬ Таблица
	|ИЗ
	|	&Таблица КАК Таблица
	|;
	...
	|	ВЫБОР
	|		КОГДА НЕ Таблица.НомерГТД В(НЕОПРЕДЕЛЕНО,
	|									ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка))
	|			ТОГДА Таблица.НомерГТД.СтранаПроисхождения
	|		КОГДА НЕ Таблица.Номенклатура В(НЕОПРЕДЕЛЕНО,
	|										ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|			ТОГДА Таблица.Номенклатура.СтранаПроисхождения
	|	КОНЕЦ КАК СтранаПроисхождения
	|ИЗ
	|	Таблица КАК Таблица
	|ГДЕ
	|	Таблица.ВедетсяУчетПоГТД
	|	И Таблица.СтранаПроисхождения В(НЕОПРЕДЕЛЕНО,
	|									ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка))
	|	И (НЕ Таблица.НомерГТД В(НЕОПРЕДЕЛЕНО,
	|							ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка))
	|		ИЛИ НЕ Таблица.Номенклатура В(НЕОПРЕДЕЛЕНО,
	|										ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)))";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
								"&ИмяПоляНоменклатура",
								"Таблица." + ИмяПоляНоменклатура);
	
	ВыгружаемыеКолонки = "НомерСтроки, НомерГТД, ВедетсяУчетПоГТД, СтранаПроисхождения, " + ИмяПоляНоменклатура;
	
	Запрос.УстановитьПараметр("Таблица", ТаблицаФормы.Выгрузить(, ВыгружаемыеКолонки));
			...
			Иначе
				Для Каждого ИмяПоляСтатус Из ПараметрыУказанияСерий.ИменаПолейСтатусУказанияСерий Цикл
					Если ИмяПоляСтатус = "СтатусУказанияСерий" Тогда
						ИмяПоляСерия = "Серия";
					Иначе
						ИмяПоляСерия = ПараметрыУказанияСерий.СоответствиеИмениПоляСерияСтатусуУказанияСерий.Получить(ИмяПоляСтатус);
					КонецЕсли;
...
Процедура ХарактеристикиНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	//A2105505-2430
	|			ИЛИ СпрХарактеристики.КР_Штрихкод ПОДОБНО &СтрокаПоиска	 
	//A2105505-2430
	|)
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
