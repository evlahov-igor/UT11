...
Процедура ЗаполнитьСтруктуруПрайсЛистаИзДанныхКВыгрузке(СтруктураПрайсЛиста, ТоварыКВыгрузке) Экспорт
	...
	|	ТоварыКВыгрузке.злGTIN КАК злGTIN,		// #3967.. Фомин Д.Ю. 05.04.2024.
	|	ТоварыКВыгрузке.КР_Модель КАК КР_Модель,	// #4641.. Фомин Д.Ю. 03.09.2024.
	...
	|	Товары.злGTIN КАК злGTIN,			// #3967.. Фомин Д.Ю. 05.04.2024.
	|	Товары.КР_Модель КАК КР_Модель,		// #4641.. Фомин Д.Ю. 03.09.2024.
	...
	|	Товары.Номенклатура.КР_НаименованиеНаЦеннике КАК КР_НаименованиеНаЦеннике,
	|	Товары.Номенклатура.КР_Цвет.Код КАК КР_ЦветКод,
	// #3968.. Фомин Д.Ю. 28.03.2024.
	//|	ЕСТЬNULL(Товары.Характеристика.КР_КодТНВЭД.КР_Маркируется, ЛОЖЬ) КАК КР_Маркируется,
	|	ЕСТЬNULL(Товары.Характеристика.КР_КодТНВЭД.злМаркировкаДляРозницы, ЛОЖЬ) КАК КР_Маркируется,
	// ..#3968
	|	Товары.Номенклатура.ВидНоменклатуры.ОсобенностьУчета КАК КР_ОсобенностьУчета,
	// >> 30.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	...
	|	МАКСИМУМ(злGTIN),		// #3967.. Фомин Д.Ю. 05.04.2024
	|	МАКСИМУМ(КР_Модель),    // #4641.. Фомин Д.Ю. 03.09.2024.
	|	МАКСИМУМ(КодГруппы),
	|	МАКСИМУМ(Артикул),
	...
	// << 30.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	|	МАКСИМУМ(КР_НаименованиеНаЦеннике),
	|	МАКСИМУМ(КР_ЦветКод),
	|	МАКСИМУМ(КР_Маркируется),
	|	МАКСИМУМ(КР_ОсобенностьУчета),
	// >> 30.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	...
	Пока ВыборкаПоНоменклатуре.Следующий() Цикл
		...
		// << 30.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
		ЗаписьТовар.КР_НаименованиеНаЦеннике 	= ВыборкаПоНоменклатуре.КР_НаименованиеНаЦеннике;
		ЗаписьТовар.КР_ЦветКод 					= ВыборкаПоНоменклатуре.КР_ЦветКод;
		ЗаписьТовар.СсылкаНоменклатура 			= ВыборкаПоНоменклатуре.Номенклатура;
		ЗаписьТовар.КР_ОсобенностьУчета 		= ВыборкаПоНоменклатуре.КР_ОсобенностьУчета;
		// >> 30.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
		
		ЗаписьТовар.КР_Модель 		= ВыборкаПоНоменклатуре.КР_Модель;		// #4641.. Фомин Д.Ю. 03.09.2024.
		...
		Если ВыборкаПоНоменклатуре.ЕстьХарактеристики Тогда
			...
			Пока ВыборкаПоХарактеристикам.Следующий() Цикл
				...
				ЗаписьХарактеристика.злGTIN = ВыборкаПоХарактеристикам.злGTIN;	// #3967.. Фомин Д.Ю. 05.04.2024
				// << 30.01.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
				ЗаписьХарактеристика.КР_Маркируется 		= ВыборкаПоХарактеристикам.КР_Маркируется;
...
Процедура ДанныеТоваровДляВыгрузки(ТаблицаТоваров, Устройство, Параметры, ОбновитьКодыТоваров = Ложь, Штрихкод = "") Экспорт
	...
	// << 15.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	Драйвер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Устройство, "ДрайверОборудования");
	ЭтоArtix = Драйвер = Справочники.ДрайверыОборудования.КР_ДрайверArtix;
	Параметры.Вставить("ЭтоArtix", ЭтоArtix);
	// >> 15.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	...
	// << 15.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	Запрос.УстановитьПараметр("ЭтоArtix",               ЭтоArtix);
	// >> 15.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	...
	ТаблицаТоваров.Колонки.Добавить("злGTIN", 			  Новый ОписаниеТипов("Строка"));		// #3967.. Фомин Д.Ю. 05.04.2024.
	ТаблицаТоваров.Колонки.Добавить("КР_Модель", 		  Новый ОписаниеТипов("Строка"));		// #4641.. Фомин Д.Ю. 03.09.2024.
		...
		Пока ВыборкаПоКодам.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не ЗначениеЗаполнено(НоваяСтрока.Код) Тогда
					...
					НоваяСтрока.злGTIN  					= Выборка.злGTIN;		// #3967.. Фомин Д.Ю. 05.04.2024.
					НоваяСтрока.КР_Модель  					= Выборка.КР_Модель;	// #4641.. Фомин Д.Ю. 03.09.2024.
...
Процедура ЗагрузитьОтчетыОПродажахИзККМ(Устройство, ДанныеОПродажах, Отказ, СообщениеОбОшибке) Экспорт
	...
	Если Не ПараметрыПодключаемогоОборудования.Следующий() Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗагрузкаАкцизныхМарок <> Ложь Тогда
		ВыборкаПоСменам = РезультатЗапросаПродажи[РезультатЗапросаПродажи.Количество()-2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		ТаблицаАкцизныеМарки = РезультатЗапросаПродажи[РезультатЗапросаПродажи.Количество()-1].Выгрузить();
		ВыборкаПоСменамВозврат = РезультатЗапросаВозврат[РезультатЗапросаВозврат.Количество()-2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Функция ПолучитьДанныеДляЗаполненияДокумента(ПодключаемоеОборудование, ТаблицаТоваров, ТаблицаШтрихкодовАкцизов, ЗагрузкаАкцизныхМарок)
	...
	|	
	|	ТаблицаДанных.Количество КАК КоличествоУпаковок,
		...
		Если ВыборкаПоИдентификаторам.НайтиСледующий(Новый Структура("НомерСмены,УникальныйИдентификатор", НомерКассовойСменыДляПоиска, УникальныйИдентификаторДляПоиска)) Тогда
			...
			Пока ВыборкаПоТоварам.Следующий() Цикл
				...
				Для Каждого ВыборкаПоТоварам Из ТаблицаНаборов Цикл
					...
					Отбор.Вставить("НоменклатураНабора", ВыборкаПоТоварам.Номенклатура);
					Отбор.Вставить("ХарактеристикаНабора", ВыборкаПоТоварам.Характеристика);
					Отбор.Вставить("ДокументРеализации", ВыборкаПоТоварам.ДокументРеализации);
...
Функция ТекстЗапросаДанныеТоваровДляВыгрузки(Знач Параметры, Знач ИспользоватьСерииНоменклатуры)
	...
	|	ОстатокНаСкладе.Свободно КАК Свободно
	...
	|	И ОстатокНаСкладе.Номенклатура.ТипНоменклатуры В(
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|	И ОстатокНаСкладе.Склад = &ПравилоОбменаСклад
	...
	|	КодыТоваровПодключаемогоОборудованияOffline.Номенклатура.КР_Модель КАК КР_Модель,		// #4641.. Фомин Д.Ю. 03.09.2024.
	|	КодыТоваровПодключаемогоОборудованияOffline.Характеристика КАК Характеристика,
	// #3967.. Фомин Д.Ю. 05.04.2024.
	...
	|	ЕСТЬNULL(ХарактеристикиНоменклатуры.КиЗГИСМGTIN, """") КАК злGTIN,
	// ..#3967
	...
	|			ПО КодыТоваровПодключаемогоОборудованияOffline.Характеристика = ХарактеристикиНоменклатуры.Ссылка 
	// ..#3967	
	|ГДЕ
...
Функция ТекстЗапросаДанныеТоваровДляВыгрузки25(Знач Параметры, Знач ИспользоватьСерииНоменклатуры)
	...
	// << 15.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	|	И ВЫБОР
	|			КОГДА &ЭтоArtix = ИСТИНА
	|				ТОГДА КодыТоваровПодключаемогоОборудованияOffline.Используется
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	// >> 15.03.2023, Гашников А.П., КРОК, Jira№ A2105505-1150
	|ИТОГИ
