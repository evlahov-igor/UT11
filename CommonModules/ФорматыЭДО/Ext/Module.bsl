...
Функция ПоддерживаемыеФорматы() Экспорт
	...
	Форматы.БизнесСеть.Вставить("КоммерческоеПредложение", "BNCommerceOffering");
	Форматы.БизнесСеть.Вставить("ЗапросКоммерческогоПредложения", "BNCommerceOfferingRequest");
...
Функция СформироватьДанныеОсновногоТитулаПоОбъектуУчета(ОбъектУчета, Настройки, Данные) Экспорт
	...
	Если ДанныеДокумента.Строки.Количество() = 0 Тогда
		...
		// Ошибки обработки объекта могли быть сообщены пользователю в фоновом заднии.
...
Функция ПрочитатьСодержаниеДокумента(ОписаниеФайла) Экспорт
	...
	Если НРег(Файл.Расширение) = ".xml" Тогда
		ПараметрыДокумента = КонвертацияЭДО.ПараметрыФайлаПроизвольногоДокумента(ОписаниеФайла.ДвоичныеДанные);
...
//
// Возвращаемое значение:
//  РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла() - описание файла после добавления сведений подписанта.
//
Функция ЗаполнитьДанныеДоверенностейДокумента(ОписаниеФайла,
	Организация, Доверенности, СодержаниеДокумента) Экспорт
	...
	ТекстИнф = ДанныеФайлаЭД.Документ.СвСчФакт.ИнфПолФХЖ1.ТекстИнф;
	...
	ДвоичныеДанные = ФорматыЭДО_ФНС.УдалитьПространствоИмен(ДвоичныеДанные, ПространствоИмен);
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла, Организация, СертификатПодписи, СодержаниеДокумента) Экспорт
	...
	Если ДанныеФайлаЭД.Свойства().Получить("ИдФайл") <> Неопределено Тогда				
		ДвоичныеДанные = РаботаСФайламиБЭД.XDTOВДвоичныеДанные(ДанныеФайлаЭД, Ложь);
		ДвоичныеДанные = ФорматыЭДО_ФНС.УдалитьПространствоИмен(ДвоичныеДанные, ПространствоИменСхемы);
	Иначе
...
Процедура ПолучитьЗапросКонструктораДополнительныхПолейТаблицы(ТекстЗапроса, ВычислениеЗначений = Ложь)
	...
	ИмяТаблицыНоменклатура   = ОбщегоНазначенияБЭД.ИмяТаблицыОпределяемогоТипа("НоменклатураБЭД");
	ИмяТаблицыХарактеристики = ОбщегоНазначенияБЭД.ИмяТаблицыОпределяемогоТипа("ХарактеристикаНоменклатурыБЭД");
	ИмяТаблицыУпаковки       = ОбщегоНазначенияБЭД.ИмяТаблицыОпределяемогоТипа("УпаковкаНоменклатурыБЭД");
...
Функция ВычислитьЗначенияДополнительныхПолейТаблицы(ПараметрыЗаполнения, ОшибкиВычисления)
	...
	Если ОбщегоНазначенияБЭД.ИмяТаблицыОпределяемогоТипа("НоменклатураБЭД") <> Неопределено Тогда
		ИсточникиДанных.Вставить("Номенклатура", Новый Структура);
	КонецЕсли;
	
	Если ОбщегоНазначенияБЭД.ИмяТаблицыОпределяемогоТипа("ХарактеристикаНоменклатурыБЭД") <> Неопределено Тогда
		ИсточникиДанных.Вставить("Характеристика", Новый Структура);
	КонецЕсли;
	
	Если ОбщегоНазначенияБЭД.ИмяТаблицыОпределяемогоТипа("УпаковкаНоменклатурыБЭД") <> Неопределено Тогда
...
Процедура ОбработатьОшибкуВыполненияЗапросаКонструктораДополнительныхПолей(ИнформацияОбОшибке, ПараметрыЗаполнения)
	...
	СсылкаНаНастройкуЭДО = НастройкиЭДО.СсылкаНаОбъектНастройкиЭДО(ПараметрыЗаполнения.КлючНастройки.Отправитель,
		ПараметрыЗаполнения.КлючНастройки.Получатель, ПараметрыЗаполнения.КлючНастройки.Договор);
	...
	МассивСтрок.Добавить(СтрШаблон(НСтр("ru = 'Вид электронного документа: %1'"),
		ПараметрыЗаполнения.КлючНастройки.ВидДокумента));
...
Функция ДанныеФайлаЭД(ДанныеФайла, ОписаниеОшибки, ПространствоИменФНС = Неопределено, ИмяКорневогоУзла = "", МягкоеЧтение = Ложь)
	...
	ПотокДанныхФайла = ДанныеФайла.ДвоичныеДанные.ОткрытьПотокДляЧтения();	
...
Процедура НоменклатураАктаОРасхождениях(Знач Объект, ТаблицаНоменклатуры)
	...
	ТаблицаНоменклатуры.Колонки.Добавить("ИД", ОписаниеТиповС80);
	ТаблицаНоменклатуры.Колонки.Добавить("Наименование", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("НаименованиеХарактеристики", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("КодТовара", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("Артикул", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("БазоваяЕдиницаКод", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("БазоваяЕдиницаНаименование", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("БазоваяЕдиницаНаименованиеПолное", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("БазоваяЕдиницаМеждународноеСокращение", ОписаниеТиповС255);
	ТаблицаНоменклатуры.Колонки.Добавить("Описание", ОписаниеТиповС3000);
	ТаблицаНоменклатуры.Колонки.Добавить("Сопоставление");
	ТаблицаНоменклатуры.Колонки.Добавить("ИсторияИдентификаторов");
...
Процедура СформироватьТЗПоНаборуДанныхXDTO(ТЗ, НаборДанных, ДопПараметры = Неопределено) 
	...
	КС14 = Новый КвалификаторыСтроки(14);
	КС80 = Новый КвалификаторыСтроки(110);
	КС255 = Новый КвалификаторыСтроки(255);
	КС3000 = Новый КвалификаторыСтроки(3000);
	...
	МассивСтрока = Новый Массив;
	МассивСтрока.Добавить(Тип("Строка"));
	ОписаниеТиповС14 = Новый ОписаниеТипов(МассивСтрока, , КС14);
	ОписаниеТиповС80 = Новый ОписаниеТипов(МассивСтрока, , КС80);
	ОписаниеТиповС255 = Новый ОписаниеТипов(МассивСтрока, , КС255);
	ОписаниеТиповС3000 = Новый ОписаниеТипов(МассивСтрока, , КС3000);
	...
	ТЗ.Колонки.Добавить("Штрихкод", ОписаниеТиповС14);
	...
	ТЗ.Колонки.Добавить("БазоваяЕдиницаНаименованиеКраткое", ОписаниеТиповС255);
	ТЗ.Колонки.Добавить("СтавкаНДС", ОписаниеТиповС80);
	ТЗ.Колонки.Добавить("Описание", ОписаниеТиповС3000);
	ТЗ.Колонки.Добавить("ЭД", ОписаниеТиповЭД);
	ТЗ.Колонки.Добавить("Сопоставление");
	ТЗ.Колонки.Добавить("ИсторияИдентификаторов");
